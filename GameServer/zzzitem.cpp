...
BOOL CItem::IsItem() {
	if (this->m_Type < 0) return FALSE;	
	else return TRUE;	
}

int CItem::IsSetItem() {
	if (this->m_Type < 0) return FALSE;	
	else return this->m_SetOption & 3;	// Limit of the SetItem	
}
...
void CItem::Convert(int type, BYTE Option1, BYTE Option2, BYTE Option3, BYTE Attribute2, BYTE SetOption, BYTE ItemEffectEx, BYTE *SocketOption, BYTE SocketBonusOption, BYTE PeriodItemOption, BYTE DbVersion) {
	...
	if (_type >MAX_ITEMS - 1) g_Log.AddC(TColor::Red,  "error-L1 : ItemIndex error %d", _type);
	
	p = &ItemAttribute[_type];
	this->m_serial = ItemAttribute[_type].Serial;
	this->m_Type = type;
	
	if ((DbVersion) <= 2) this->m_Type=_type;

	this->m_NewOption = Attribute2;
	
	if (p->OptionFlag == 0) this->m_NewOption= 0;
	if (((_type >= ITEMGET(12,3)) && (_type <= ITEMGET(12,6))) || ((_type >= ITEMGET(12,36)) && (_type <= ITEMGET(12,40))) || (_type == ITEMGET(13,30)) || (_type == ITEMGET(12,41)) || (_type == ITEMGET(12,42)) || (_type == ITEMGET(12,43)) || (_type == ITEMGET(12,49)) || (_type == ITEMGET(12,50)) || ((_type >= ITEMGET(12,262) && _type <= ITEMGET(12,265)))) Attribute2 = 0;
	if (_type == ITEMGET(13,37)) Attribute2 = 0;

	SetOption = SetOption & 15;		
	SOption = SetOption & 3;
	SOptionStatValue = 0;

	if (SOption != 1 && SOption != 2) SOption = 0;
	if ((((SetOption >> 2) & 3) != 1) && (((SetOption >> 2) & 3) != 2)) SOption = 0;
	...
	if (g_SocketOptionSystem.IsEnableSocketItem(this) == true) {
		if (SocketOption) {
			...
			for (int i = 0; i < 5; i++) this->m_SocketOption[i] = SocketOption[i];
		}
		...
		if (SocketOption) {
			...
			for (int j = 0; j < 5; ++j) this->m_SocketOption[j] = SocketOption[j];
		}
		...
		if (SocketOption) {
			...
			for (int k = 0; k < 5; ++k) this->m_SocketOption[k] = SocketOption[k];
		}
		...
		for (int i = 0; i < 5; i++) this->m_SocketOption[i] = 0xFF;
	}

	this->m_PeriodItemOption = PeriodItemOption;
	memset(this->m_SkillResistance , 0, sizeof(this->m_SkillResistance));
	
	if (_type == ITEMGET(0,19) || _type == ITEMGET(4,18) || _type == ITEMGET(5,10) || _type == ITEMGET(2,13)) Attribute2 = 0;

	iChaosItem = 0;

	if (this->m_Type == ITEMGET(2,6)) iChaosItem = 15;
	else if (this->m_Type == ITEMGET(5,7)) iChaosItem = 25;
	else if (this->m_Type == ITEMGET(4,6)) iChaosItem = 30;
	if (ItemGetDurability(this->m_Type, 0, 0, 0) != 0) this->m_BaseDurability = (float)ItemGetDurability(this->m_Type, this->m_Level, Attribute2 & 0x7F, this->m_SetOption);
	else this->m_BaseDurability = 0;
	...
	if (g_SocketOptionSystem.IsSeedSphereItem(this->m_Type)) this->m_Durability = ((this->m_Type % 512 - 100) / 6 + 1);
	...
	this->EvasionRating = p->SuccessfulBlocking;
	...
	if (this->m_Durability == 0) this->m_CurrentDurabilityState = 1;
	else if (this->m_Durability < this->m_DurabilityState[2]) this->m_CurrentDurabilityState = (float)0.5;
	else if (this->m_Durability < this->m_DurabilityState[1]) this->m_CurrentDurabilityState = (float)0.3;
	else if (this->m_Durability < this->m_DurabilityState[0]) this->m_CurrentDurabilityState = (float)0.2;
	else this->m_CurrentDurabilityState = 0.0;

	memcpy(this->m_RequireClass, p->RequireClass , sizeof(this->m_RequireClass));

	for (i = 0; i < MAX_ITEM_SPECIAL_ATTRIBUTE; i++) this->m_Resistance[i] = p->Resistance[i] + (p->Resistance[i] * this->m_Level);

	ItemLevel = p->Level;

	if ((Attribute2 & 0x3F) > 0) ItemLevel = p->Level + 25;
	else {
		if (this->m_SetOption != 0) ItemLevel = p->Level + 25;
	}

	if (p->RequireStrength != 0) this->m_RequireStrength = ((p->RequireStrength * (ItemLevel + this->m_Level * 3)) * 3) / 100 + 20;
	else this->m_RequireStrength = 0;
	if (p->RequireDexterity != 0) this->m_RequireDexterity = ((p->RequireDexterity * (ItemLevel + this->m_Level * 3)) * 3) / 100 + 20;
	else this->m_RequireDexterity = 0;
	if (p->RequireEnergy != 0) this->m_RequireEnergy = ((p->RequireEnergy * (ItemLevel + this->m_Level * 3)) * 4) / 100 + 20;
	else this->m_RequireEnergy = 0;
	if (p->RequireEnergy != 0) {
		if ((_type >> 9) == 5 && p->ItemSlot == 1) this->m_RequireEnergy = ((p->RequireEnergy * (ItemLevel + this->m_Level)) * 3) / 100 + 20;
	} else this->m_RequireEnergy = 0;
	if (_type >= ITEMGET(0,0) && _type < ITEMGET(12,0)) {
		if (p->RequireVitality != 0) this->m_RequireVitality = ((p->RequireVitality * (ItemLevel + this->m_Level * 3)) * 3) / 100 + 20;
		else this->m_RequireVitality = 0;
		if (p->RequireLeadership != 0) this->m_RequireLeaderShip = ((p->RequireVitality * (ItemLevel + this->m_Level * 3)) * 3) / 100 + 20;
		else this->m_RequireLeaderShip = 0;
	}

	if(_type == ITEMGET(13,4) || _type == ITEMGET(13,5)) {
		if(this->m_PetItem_Level > MAX_PET_LEVEL) this->m_PetItem_Level = MAX_PET_LEVEL;
	}

	if (_type == ITEMGET(13,5)) this->m_RequireLeaderShip = this->m_PetItem_Level * 10 + 185;
	if (g_kJewelOfHarmonySystem.IsActive(this) == TRUE) {
		BYTE btOption = g_kJewelOfHarmonySystem.GetItemStrengthenOption(this);

		if (btOption == AT_JEWELOFHARMONY_WEAPON_DECREASE_REQUIRE_STR) this->m_HJOpStrength = g_kJewelOfHarmonySystem.GetItemEffectValue(this, AT_JEWELOFHARMONY_WEAPON_DECREASE_REQUIRE_STR);
		else if (btOption == AT_JEWELOFHARMONY_WEAPON_IMPROVE_MAXATTACKDAMAGE) this->m_HJOpStrength = g_kJewelOfHarmonySystem.GetItemEffectValue(this, AT_JEWELOFHARMONY_WEAPON_IMPROVE_MAXATTACKDAMAGE);
		else if (btOption == AT_JEWELOFHARMONY_WEAPON_DECREASE_REQUIRE_DEX) this->m_HJOpDexterity = g_kJewelOfHarmonySystem.GetItemEffectValue(this, AT_JEWELOFHARMONY_WEAPON_DECREASE_REQUIRE_DEX);
		else if (btOption == AT_JEWELOFHARMONY_WEAPON_DECREASE_REQUIRE_STR) this->m_HJOpDexterity = g_kJewelOfHarmonySystem.GetItemEffectValue(this, AT_JEWELOFHARMONY_WEAPON_DECREASE_REQUIRE_STR);
	} else {
	...
	if (p->RequireLevel != 0) {
		if (_type == ITEMGET(13,4)) this->m_RequireLevel = this->m_PetItem_Level * 2 + 218;
		else if ((_type >= ITEMGET(12,3) && _type <= ITEMGET(12,6)) || _type == ITEMGET(12,42) || _type == ITEMGET(12,49)) this->m_RequireLevel = p->RequireLevel + (this->m_Level * 5);
		else if ((_type >= ITEMGET(12,36) && _type <= ITEMGET(12,40)) || _type == ITEMGET(12,43) || _type == ITEMGET(12,50) || _type == ITEMGET(12,270)) this->m_RequireLevel = p->RequireLevel;
		else if (_type >= ITEMGET(12,7) && _type <= ITEMGET(12,24)) this->m_RequireLevel = p->RequireLevel;
		else if (_type >= ITEMGET(12, 262) && _type <= ITEMGET(12, 265)) this->m_RequireLevel = p->RequireLevel + this->m_Level * 4;
		else if (_type == ITEMGET(12, 266) || _type == ITEMGET(12, 267) || _type == ITEMGET(12,268)) this->m_RequireLevel = p->RequireLevel;
		else if (_type == ITEMGET(12, 269)) this->m_RequireLevel = p->RequireLevel * this->m_Level * 4;
		else if (_type >= ITEMGET(12,51) && _type <= ITEMGET(12,52)) {
			this->m_RequireLevel = p->RequireLevel + this->m_Level * 4;

			if (this->m_RequireLevel > 400) this->m_RequireLevel= 400;
			
		} else if (_type >= ITEMGET(0,0) && _type < ITEMGET(12,0)) {
			if (p->RequireLevel) this->m_RequireLevel = p->RequireLevel;
		} else this->m_RequireLevel = p->RequireLevel + this->m_Level * 4;
	} else this->m_RequireLevel = 0;

	if (this->m_Type == ITEMGET(13,10)) {
		if (ItemLevel <= 2) this->m_RequireLevel = 20;
		else this->m_RequireLevel = 50;
	}

	if ((Attribute2 & 0x3F) > 0) {
		if (this->m_RequireLevel > 0) {
			if ((_type >= ITEMGET(12,36) && _type <= ITEMGET(12,40)) || this->m_Type == ITEMGET(12,43) || this->m_Type == ITEMGET(12,50) || this->m_Type == ITEMGET(12,270)) this->m_RequireLevel;
			else if (_type < ITEMGET(0,0) || _type >= ITEMGET(12,0) && !((_type >= ITEMGET(12,36) && _type <= ITEMGET(12,40)) || this->m_Type == ITEMGET(12,43) || this->m_Type == ITEMGET(12,50) || this->m_Type == ITEMGET(12,266) || this->m_Type == ITEMGET(12,267) || this->m_Type == ITEMGET(12,268) || this->m_Type == ITEMGET(12,270))) this->m_RequireLevel += 20;
		}
	}

	if (this->m_RequireLevel > 400) this->m_RequireLevel = 400;
	if (this->m_SetOption != 0) ItemLevel = p->Level + 30;
	...
	if (this->m_DamageMax > 0) {
		if (this->m_SetOption != 0 && ItemLevel != 0) {
			...
			if ((Attribute2 & 0x3F) > 0) {
				if (iChaosItem != 0) this->m_DamageMax += iChaosItem;
				else if (p->Level != 0) this->m_DamageMax += (this->m_DamageMin * 25) / p->Level + 5;
			}
		}

		if (g_PentagramSystem.IsPentagramItem(this) == true) this->m_DamageMax += this->m_Level * 3;
		else this->m_DamageMax += this->m_Level * 3;
		if (this->m_Level >= 10) this->m_DamageMax += (this->m_Level - 9) * (this->m_Level - 8) / 2;
	}
	...
	if (this->m_DamageMin > 0) {
		if (this->m_SetOption != 0 && ItemLevel != 0) {
			...
			if ((Attribute2 & 0x3F) > 0) {
				if (iChaosItem != 0) this->m_DamageMin += iChaosItem;
				else if (p->Level != 0) this->m_DamageMin += (this->m_DamageMin * 25) / p->Level + 5;
			}
		}
	...
	if (p->SuccessfulBlocking > 0)	
	{
		if (this->m_SetOption != 0 && ItemLevel != 0)
		{
			this->EvasionRating += (this->EvasionRating * 25) / p->Level + 5;
			this->EvasionRating += ItemLevel / 40 + 5;
		}
		else
		{
			if ((Attribute2 & 0x3F)  > 0)
		...
		this->EvasionRating += this->m_Level * 3;

		if (this->m_Level >= 10)
		{
			this->EvasionRating += (this->m_Level - 9) * (this->m_Level - 8) / 2;
		}
	}
	...
	if (p->Defense > 0)	
	{
		...
		{
			this->Defense += this->m_Level;

			if (this->m_SetOption != 0 && ItemLevel != 0)
			{
				this->Defense += (this->Defense * 20) / ItemLevel + 2;
			}
		}
		else 
		{
			if (this->m_SetOption != 0 && ItemLevel != 0)
			{
				this->Defense += (this->Defense * 12) / p->Level + (p->Level / 5) + 4;
				this->Defense += (this->Defense * 3) / ItemLevel  + (ItemLevel / 30) + 2;
			}
			...
			{
				this->Defense += this->m_Level * 4;

				if (this->m_Level >= 10)
				{
					this->Defense += (this->m_Level - 9);
				}
			}

			else if ((_type >= ITEMGET(12,3) && _type <= ITEMGET(12,6)) || _type == ITEMGET(13,30) || _type == ITEMGET(13,4))
			{
				this->Defense += this->m_Level * 2;
			}

			else
			{
				this->Defense += this->m_Level * 3;
			}

			if (this->m_Level >= 10)
			{
				this->Defense += (this->m_Level - 9) * (this->m_Level - 8) / 2;
			}
		}
	}
	...
	switch (_type) // Wings
	{
		...
		case ITEMGET(12,269):
			this->Defense += this->m_Level*2;
			if(this->m_Level >= 10)
				this->Defense += (this->m_Level - 9) * (this->m_Level - 8) / 2 + (this->m_Level - 9);
			break;
		case ITEMGET(13,30):
			this->Defense += this->m_Level*2 + 15;
			if(this->m_Level >= 10)
				this->Defense += (this->m_Level - 9) * (this->m_Level - 8) / 2 + (this->m_Level - 9);
			break;
		...
		case ITEMGET(12,270):
			...
			if (this->m_Level >= 10)
			{
				this->Defense += (this->m_Level - 8) * (this->m_Level - 9) / 2;
			}
			break;
		case ITEMGET(12,262):
		case ITEMGET(12,263):
	...
	if (p->MagicDefense > 0) {
		...
		if (this->m_Level >= 10) this->m_MagicDefense += (this->m_Level - 9) * (this->m_Level - 8) / 2;
	}
	...
	if (this->m_Type >= ITEMGET(12,200) && this->m_Type <= ITEMGET(12,220)) this->m_Special[this->m_SpecialNum] = 106;
	...
	if (this->m_Type >= ITEMGET(12, 200) && this->m_Type <= ITEMGET(12, 220)) this->m_Special[this->m_SpecialNum] = 107;
	...
	if (Option3 != 0) {
		...
		if (_type == ITEMGET(12,0) || _type == ITEMGET(12,132)) {	// Wing elf
			...
			//WTF?? Inverted Values??
			if ((this->m_NewOption & 0x20) != 0) this->m_Special[this->m_SpecialNum] = 85; //Recover Life
			else this->m_Special[this->m_SpecialNum] = 80; //Additional Damage
		...
		} else if (_type == ITEMGET(12,38)) // Third Wings "Wing of Illusion"
		{
			...
			if ((this->m_NewOption & 0x20) != 0) this->m_Special[this->m_SpecialNum] = 81; // Additional Magic
			else this->m_Special[this->m_SpecialNum] = 85; // Recover Life
	...
	if (this->m_Type != ITEMGET(4,7) && this->m_Type != ITEMGET(4,15)) // Arrows and Bolts
	{
		...
		this->m_DefenseOrigin = this->Defense;
		...
		this->Defense -= (int)(this->Defense * this->m_CurrentDurabilityState);
		this->EvasionRating -= (int)(this->EvasionRating * this->m_CurrentDurabilityState);
	...
	if (this->m_Durability == 0.0f) memset(this->m_Resistance , 0, sizeof(this->m_Resistance));
}
...
void CItem::Value() {
	...
	if (p->BuyMoney != 0) {
		...
		// Valuable Coin Price
		if (this->m_Type == ITEMGET(12,152)) {
			this->m_SellMoney = MAX_WAREHOUSE_ZEN;
			this->m_BuyMoney = MAX_WAREHOUSE_ZEN;
		} else this->m_SellMoney /= g_ConfigRead.itemPrices.btItemSellPriceDivisor;

		if (this->m_SellMoney < 1000) {
			if (this->m_SellMoney >= 100) this->m_SellMoney = 10 * (this->m_SellMoney / 10);
		} else {
			this->m_SellMoney = 100 * (this->m_SellMoney / 100);
		}

		if (this->m_BuyMoney < 1000) {
			if (this->m_BuyMoney >= 100) this->m_BuyMoney = 10 * (this->m_BuyMoney / 10);
		} else {
	...
	if (this->m_Type == ITEMGET(4, 15)) { // Arrow
		...
		if (this->m_Durability > 0.0f) Gold = levelgold * this->m_Durability / p->Durability;
	...
	if (this->m_Type == ITEMGET(14, 13)) {
		...
		if (this->m_Level == 1) Gold = g_ConfigRead.itemPrices.iCrestOfMonarchPrice;
		else Gold = g_ConfigRead.itemPrices.iLochFeatherPrice;
		...
		if (this->m_Level == 1) Gold = 2400000;		
	} else if (this->m_Type == ITEMGET(13,7)) {
		if (this->m_Level == 0) Gold = 1500000;
		else if (this->m_Level == 1) Gold = 1200000;
		...
		for (int k = 0; k < 3; k++) {
			if ((this->m_Option3 & (1<<k)) != 0) Gold += 300000;
		}
		...
		Gold = (int)this->m_Durability * 10000 * 3;
	} else if (this->m_Type == ITEMGET(14,45) || this->m_Type == ITEMGET(14,46) || this->m_Type == ITEMGET(14,47) || this->m_Type == ITEMGET(14,48) || this->m_Type == ITEMGET(14,49) || this->m_Type == ITEMGET(14,50)) {
	...
	} else if (this->m_Type == ITEMGET(13,147))
	{
	...
	} else if (this->m_Type == ITEMGET(14,23) || this->m_Type == ITEMGET(14,24) || this->m_Type == ITEMGET(14,25) || this->m_Type == ITEMGET(14,26) || this->m_Type == ITEMGET(14,65) || this->m_Type == ITEMGET(14,66) || this->m_Type == ITEMGET(14,67) || this->m_Type == ITEMGET(14,68)) {
		Gold = 9000;
	} else if (p->Value > 0) {
		Gold += (p->Value * p->Value * 10)/12;

		if (this->m_Type >= ITEMGET(14,0) && this->m_Type <= ITEMGET(14,8)) {	// Potions + Antidote
			if (this->m_Level > 0) Gold *= (__int64)(pow(2.0f, Level));
			Gold = Gold / 10 * 10;
	...
	} else if ((Type == 12 && (this->m_Type > ITEMGET(12,6) && this->m_Type < ITEMGET(12,36))) || (Type == 12 && (this->m_Type > ITEMGET(12,43) && this->m_Type < ITEMGET(12,49))) || (Type == 12 && (this->m_Type > ITEMGET(12,50) && this->m_Type < ITEMGET(12,262))) || (Type == 12 && (this->m_Type > ITEMGET(12,265))) || (Type == 13 && (this->m_Type != ITEMGET(13,30))) || (Type == 15)) {
		...
		if (this->m_Type == ITEMGET(14,63)) Gold = 200000;
		else if (this->m_Type == ITEMGET(14,85) || this->m_Type == ITEMGET(14,86) || this->m_Type == ITEMGET(14,87) || this->m_Type == ITEMGET(14,90)) Gold = (int)this->m_Durability * 300;
		else if (this->m_Type == ITEMGET(14,141)) Gold = 672000;
		else if (this->m_Type == ITEMGET(14,142)) Gold = 546000;
		else if (this->m_Type == ITEMGET(14,143)) Gold = 471000;
		else if (this->m_Type == ITEMGET(14,144)) Gold = 363000;
		else if (this->m_Type == ITEMGET(14,176) || this->m_Type == ITEMGET(14,177) || this->m_Type == ITEMGET(14,178) || this->m_Type == ITEMGET(14,179)) Gold = Level2 * Level2 * (Level2 + 40) / 8 + 100;
		else if (this->m_Type == ITEMGET(0,41)) Gold = 100000;
		else if (this->m_Type >= ITEMGET(14,217) && this->m_Type <= ITEMGET(14,221)) Gold = 300000;
		else if (this->m_Type >= ITEMGET(14,234)) Gold = 3000000;
		...
		if (Type == 12 && this->m_Type <= ITEMGET(12,6)) Gold = 40000000 + 11 * Level2 * Level2 * (Level2 + 40);		
		else if (Type == 12 && ((this->m_Type >= ITEMGET(12,36) && this->m_Type <= ITEMGET(12,40)) || this->m_Type == ITEMGET(12,50))) Gold = 40000000 + 11 * Level2 * Level2 * (Level2 + 40);
		else if (Type == 12 && (this->m_Type >= ITEMGET(12,41) && this->m_Type <= ITEMGET(12,43))) Gold = 40000000 + 11 * Level2 * Level2 * (Level2 + 40);
		else if (Type == 12 && (this->m_Type >= ITEMGET(12,262) && this->m_Type <= ITEMGET(12,265))) Gold = 40000000 + 11 * Level2 * Level2 * (Level2 + 40);
		else if (this->m_Type == ITEMGET(13,30) || this->m_Type == ITEMGET(12,49)) Gold = 40000000 + 11 * Level2 * Level2 * (Level2 + 40);
		else Gold = Level2 * Level2 * (Level2 + 40) / 8 + 100;		

		if (Type >= 0 && Type <= 6) {
			if (p->TwoHand == 0) Gold = Gold * 80 / 100;
		}
	...
	if (g_kItemSystemFor380.Is380OptionItem(this) == TRUE) Gold += (UINT64)(Gold * 16.0 / 100.0);
	...
	if (g_SocketOptionSystem.IsSocketItem(this) == TRUE) {
		...
		for (int i = 0; i < 5; i++) {
			if (this->m_SocketOption[i] != 0xFF && this->m_SocketOption[i] != 0xFE) {
	...
	if (g_PentagramSystem.IsPentagramItem(this) == TRUE) {
		...
		for (int i = 0; i < 5; i++) {
			if (this->m_SocketOption[i] != 0xFF) iPentagramSocketSlotCount++;
		}
	...
	if (Type == 12 && this->m_Type == ITEMGET(12,266)) Gold = 300000000;
	else if (Type == 12 && this->m_Type == ITEMGET(12,267)) Gold = 3000000;
	if (Gold > 3000000000) Gold = 3000000000;
	...
	if (this->m_SellMoney >= 1000) this->m_SellMoney = this->m_SellMoney / 100 * 100;
	else if (this->m_SellMoney >= 100) this->m_SellMoney = this->m_SellMoney / 10 * 10;
	if (this->m_BuyMoney >= 1000) this->m_BuyMoney = this->m_BuyMoney / 100 * 100;
	else if (this->m_BuyMoney >= 100) this->m_BuyMoney = this->m_BuyMoney / 10 * 10;
}
void CItem::OldValue() {
	if (this->m_Type == -1) return;
	...
	if (this->m_Type == ITEMGET(14,13)) Gold = 100000; // Bless	
	else if (this->m_Type == ITEMGET(14,14)) Gold = 70000; // Soul	
	else if (this->m_Type == ITEMGET(12,15)) Gold = 40000; // Chaos	
	else if (this->m_Type == ITEMGET(14,22)) Gold = 450000; // Creation	
	else if (this->m_Type == ITEMGET(14,16)) Gold = 450000; // Life	
	else {
	...
	if (this->m_OldBuyMoney >= 1000) this->m_OldBuyMoney = this->m_OldBuyMoney / 100 * 100;
	else if (this->m_OldBuyMoney >= 100) this->m_OldBuyMoney = this->m_OldBuyMoney / 10 * 10;
}
...
int CItem::ItemDefense() {
	...
	if (this->m_Type < 0) return FALSE;
		...
		if (this->m_IsValidItem == false) return FALSE;
		else {
...
// Set Accessory Attribute Bonus
void CItem::PlusSpecialSetRing(BYTE * Value) {
	if (this->m_Type < 0) return;
	if (this->IsSetItem() == FALSE) return;
	if (this->m_Durability == 0.0f) return;
	if (this->m_IsValidItem == false) return;
	...
	for (int i = 0; i < this->m_SpecialNum; i++) {
		if (this->m_Special[i] == Special) {
			switch (Special) {
				...
				case 100:
				case 101:			*Value += this->m_Level * 5 + 50;				break;
				case 103:			*Value += 50;									break;
				case 105:			*Value += this->m_Level * 5 + 10;				break;
				default:			*Value += this->m_Option3 * 4;					break;
			}
		}
	}
}
void CItem::PlusSpecialPercent(int *Value, int Special, WORD Percent) {
	if (this->m_Type < 0) return;
	for (int i = 0; i < this->m_SpecialNum; i++) {
		if (this->m_Special[i] == Special) *Value += (*Value * Percent) / 100;
	}
}
	...
	for (int i = 0; i < this->m_SpecialNum; i++) {
		if (this->m_Special[i] == Special) {
			switch (Special) {
				...
				default:	*Value += this->m_Option3 * 3;
			}
		}
	}
}
...
int CItem::GetWeaponType() {
	if (this->m_Type <= ITEMGET(5,0)) return 1;
	if (this->m_Type <= ITEMGET(6,0)) return 2;	
	return 0;
}
...
void CItem::SetPetItemInfo(int petlevel, UINT64 petexp) {
	if (this->m_Type == ITEMGET(13,4)) {
		...
		if (this->m_PetItem_Exp < gPetItemExp.m_DarkHorseExpTable[this->m_PetItem_Level] || this->m_PetItem_Exp >= gPetItemExp.m_DarkHorseExpTable[1+this->m_PetItem_Level]) this->m_PetItem_Exp = gPetItemExp.m_DarkHorseExpTable[this->m_PetItem_Level];
		...
		this->m_PetItem_Level = petlevel;
		this->m_PetItem_Exp = petexp;
		if (this->m_PetItem_Exp < gPetItemExp.m_DarkSpiritExpTable[this->m_PetItem_Level] || this->m_PetItem_Exp >= gPetItemExp.m_DarkSpiritExpTable[1+this->m_PetItem_Level]) this->m_PetItem_Exp = gPetItemExp.m_DarkSpiritExpTable[this->m_PetItem_Level];
		this->Convert(this->m_Type, this->m_Option1, this->m_Option2,
		this->m_Option3, this->m_NewOption, this->m_SetOption, this->m_ItemOptionEx, 0, -1, 0, CURRENT_DB_VERSION);
		this->PetValue();
		this->m_IsLoadPetItemInfo = 1;
...
int CItem::DurabilityDown(int dur, int aIndex)
{
	...
	{
		if (gObj[aIndex].CharacterData->Mastery.ML_RF_Jeweller > 0.0)
		{
			iBaseDurSmall += gObj[aIndex].CharacterData->Mastery.ML_RF_Jeweller;
		}
	}

	else if (gObj[aIndex].CharacterData->Mastery.ML_GENERAL_Jeweller > 0.0)
	{
		iBaseDurSmall += gObj[aIndex].CharacterData->Mastery.ML_GENERAL_Jeweller;
	}
...
int CItem::DurabilityDown2(int dur, int aIndex)
{
	...
	if (gObj[aIndex].Class == CLASS_RAGEFIGHTER)
	{
		if (gObj[aIndex].CharacterData->Mastery.ML_RF_Efficiency > 0.0)
		{
			iBaseDurSmall += gObj[aIndex].CharacterData->Mastery.ML_RF_Efficiency;
		}
	}

	else if (gObj[aIndex].CharacterData->Mastery.ML_GENERAL_Efficiency > 0.0)
	{
		iBaseDurSmall += gObj[aIndex].CharacterData->Mastery.ML_GENERAL_Efficiency;
	}
	...
	for(pugi::xml_node section = main.child("Section"); section; section = section.next_sibling()) {
		...
		for(pugi::xml_node item = section.child("Item"); item; item = item.next_sibling()) {
			...
			if (ItemType >= 0 && ItemType < MAX_TYPE_ITEMS) {
				if (g_MaxItemIndexOfEachItemType[ItemType] <= ItemIndex) g_MaxItemIndexOfEachItemType[ItemType] = ItemIndex;
			}
			...
			if (ItemType >= 0 && ItemType <= 5) {
				...
				// New Rings
				if (ItemIndex >= 100 && ItemIndex <= 110) {
					...
					for (int n = 0; n < MAX_ITEM_SPECIAL_ATTRIBUTE; n++) {
			...
			if (ItemType == 13) {
				...
				if (ItemIndex == 148 || ItemIndex == 149 || ItemIndex == 150 || ItemIndex == 151) p->Durability = 1;
			}

			if (ItemType == 14) {
				if (ItemIndex == 42) p->BuyMoney = item.attribute("Money").as_int();
				else p->Value = item.attribute("Money").as_int();
			...
			if (ItemType == 12) {
				if (ItemIndex >= 200 && ItemIndex <= 220) p->RequireLevel = item.attribute("DropLevel").as_int();
				else p->Level = item.attribute("DropLevel").as_int();
...
int zzzItemLevel(int type, int index, int level) {
	...
	if (level < 0) level = 0;	
	if (ItemAttribute[item_num].Level == (BYTE)-1) return FALSE;
...
int GetLevelItem(int type, int index, int level) {
	...
	if (ItemAttribute[item_num].Level == 0xFF) return -1;
	if (ItemAttribute[item_num].Level == 0) return -1;
	if (type == 14) {
...
int CItem::GetDetailItemType() {
	LPITEM_ATTRIBUTE p = &ItemAttribute[this->m_Type];
	if (this->m_Type < 0) return -1;
	if (this->m_Type >= ITEMGET(0,32) && this->m_Type <= ITEMGET(0,35)) return DETAIL_ITEM_TYPE::ITEM_RAGEFIGHTER_WEAPON;
	// Fix for new RF weapons
	if (this->m_Type >= ITEMGET(0,140) && this->m_Type <= ITEMGET(0,160)) return DETAIL_ITEM_TYPE::ITEM_RAGEFIGHTER_WEAPON;

	if (this->m_Type >= ITEMGET(0,0) && this->m_Type < ITEMGET(1,0)) {
		if (this->m_TwoHand == TRUE) return DETAIL_ITEM_TYPE::ITEM_TWOHAND_SWORD;
		else return DETAIL_ITEM_TYPE::ITEM_ONEHAND_SWORD;		
	}

	if (this->m_Type >= ITEMGET(2,0) && this->m_Type < ITEMGET(3,0)) {
		if (this->m_Type == ITEMGET(2,5)) return DETAIL_ITEM_TYPE::ITEM_TWOHAND_SWORD;
		if (this->m_Type == ITEMGET(2,6)) return -1;
		if (p->ItemKindB == 6) return DETAIL_ITEM_TYPE::ITEM_DARKLORD_SCEPTER;
		else return DETAIL_ITEM_TYPE::ITEM_MACE;		
	}

	if (this->m_Type >= ITEMGET(3,0) && this->m_Type < ITEMGET(4,0)) return DETAIL_ITEM_TYPE::ITEM_SPEAR;
	if (this->m_Type >= ITEMGET(4,0) && this->m_Type < ITEMGET(5,0)) {
		if (this->m_TwoHand == TRUE && p->ItemSlot == 1) return DETAIL_ITEM_TYPE::ITEM_CROSSBOW;
		else return DETAIL_ITEM_TYPE::ITEM_BOW;
	}
	...
	if (this->m_Type >= ITEMGET(5,0) && this->m_Type < ITEMGET(6,0)) {
		...
		for(int i = 0; i < MAX_TYPE_PLAYER; i++) {
			if (this->m_RequireClass[i] != 0) nSum += i;
		}

		if (nSum == CLASS_SUMMONER) {
			if (this->m_Type != ITEMGET(5,0)) {
				if (p->ItemSlot == 0) return DETAIL_ITEM_TYPE::ITEM_SUMMONER_STAFF;
				else if (p->ItemSlot == 1) return DETAIL_ITEM_TYPE::ITEM_SUMMONER_BOOK;
			}
		} else {
			if (this->m_TwoHand == TRUE) return DETAIL_ITEM_TYPE::ITEM_TWOHAND_STAFF;
			else return DETAIL_ITEM_TYPE::ITEM_ONEHAND_STAFF;
		}
	}

	if (this->m_Type >= ITEMGET(6,0) && this->m_Type < ITEMGET(7,0)) return DETAIL_ITEM_TYPE::ITEM_SHIELD;
	return -1;
}

void CItem::MakeSocketSlot(BYTE SlotCount) {
	if (g_SocketOptionSystem.IsEnableSocketItem(this) == FALSE) return;
	for (int i = 0; i < SlotCount; i++) this->m_SocketOption[i] = 0xFE;
...
bool CheckCanWearResetItem(int aIndex, int itemid) {
	...
	for (int i = 0; i < 1000; i++) {
		if (gObjResetItem[i].itemIndex == itemid) {
			if (lpObj->CharacterData->m_iResets < gObjResetItem[i].iResets) {
...
BYTE CalcExcOption(int MaxOpt) {
	...
	while (OptionCount != 0) {
		...
		if ((ExcOption & btOption) == 0) ExcOption |= btOption;
...
BOOL IsJumpingEventItem(int iItemCode) {
	if (GetItemGroup(iItemCode) == ITEMTYPE_EVENT) return TRUE;
	else return FALSE;
}

int IsExpensiveItem(CItem * item) {
	if (item->m_Type >= ITEMGET(0,0) && item->m_Type < ITEMGET(12,0)) {
		if (item->m_Level > 6) return TRUE;
		if ((item->m_NewOption & 0x3F) != 0) return TRUE;
		if (item->m_SetOption != 0) return TRUE;
	} else if (item->m_Type == ITEMGET(13,20) && item->m_Level > 0) return TRUE;
	else if (IsDumpItem(item->m_Type) == FALSE) return TRUE;
	return FALSE;
}
