	...
	if (this->m_nCheatMode == TRUE || nDestZoneIndex <= 0 || this->GetCurrentZoneIndex(nUserIndex) == (nDestZoneIndex-1))
	{
		...
		else
		{
			...
			else
			{
				...
				{
					nMaxLevel = lpObj->Level + lpObj->CharacterData->MasterLevel;
				}
			...
			if (nGateIndex != -1)
			{
				...
				if (BUFF_TargetHasBuff(lpObj, 3) == TRUE)
				{
					BUFF_RemoveBuffEffect(lpObj, 3);
				}
						...
						lpObj->pInventory[nEnterItemPos].m_Type != ITEMGET(14,109))
					{
							...
							lpObj->pInventory[nEnterItemPos].m_Type == ITEMGET(13,127))
						{
							...
							else
							{
								...
								GSProtocol.PROTOCOL_InventoryItemDeleteSend(nUserIndex, nEnterItemPos, 1);
							}
						}
					}
...
bool CImperialGuardian::GCNotifyAllZoneClear(int nZoneIndex) {
	if (nZoneIndex < 0 || nZoneIndex > 3) return false;
	...
	for (int i = 0; i < nCount; i++) {
		...
		if (nZoneIndex == 3) pMsg.m_nRewardExp *= 2;
		if (BUFF_TargetHasBuff(&gObj[nUserIndex[i]], 31) || BUFF_TargetHasBuff(&gObj[nUserIndex[i]], 42)) pMsg.m_nRewardExp = 0;

		this->IF_CalcRewardEXP(nUserIndex[i], pMsg.m_nRewardExp);			
		...
		if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(&gObj[nUserIndex[i]]) == false) GSProtocol.PROTOCOL_PlayerExperienceSend(nUserIndex[i], -1, pMsg.m_nRewardExp, 0, 0);
...
void CImperialGuardian::DropItem(int nIndex, int nMonsterIndex)
{
	...
	if (!nMaxLevel)
	{
		nMaxLevel = gObj[nIndex].Level + gObj[nIndex].CharacterData->MasterLevel;
	}
...
void CImperialGuardian::RollBackUserPos(int nUserNumber)
{
	if (gObj[nUserNumber].Type == OBJ_USER && gObj[nUserNumber].Connected > PLAYER_LOGGED)
	{
		...
		GSProtocol.PROTOCOL_TeleportSend(&gObj[nUserNumber], -1, gObj[nUserNumber].MapNumber, gObj[nUserNumber].X, gObj[nUserNumber].Y, gObj[nUserNumber].Dir);
...
int CImperialGuardian::IF_CalcRewardEXP(int iIndex, UINT64 iAddExp) {
	if (ObjectMaxRange(iIndex) == FALSE) return FALSE;
	if (gObj[iIndex].Type != OBJ_USER) return FALSE;

	::gObjSetExpPetItem(iIndex, iAddExp);

	if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(&gObj[iIndex])) gObj[iIndex].CharacterData->MasterExperience += iAddExp;
	else gObj[iIndex].CharacterData->Experience += iAddExp;

	CHARACTER_LevelUp(&gObj[iIndex], iAddExp, 0, "Imperial Fortress");
	return iAddExp;
}
...
void CImperialGuardian::MonsterBaseAct(LPOBJ lpObj)
{
	...
	else if (lpObj->m_ActState.Emotion == 0)
	{
		...
		if (lpObj->m_MoveRange > 0 && !BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_ICE_ARROW) && !BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_STUN) && !BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_SLEEP) &&
