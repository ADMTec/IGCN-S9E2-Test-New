...
void CMuunAttack::SendAttackMsg(int aIndex, int aTargetIndex, int SubCode, int SubCode2)
{
	...
	{
		if (CHARACTER_CalcDistance(&gObj[aIndex], &gObj[aTargetIndex]) > 4)
		{
...
bool CMuunAttack::Stun(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf *lpMagic, int SubCode2)
{
	...
	{
		iStunRate -= lpTargetObj->CharacterData->C_StunResistance;
	}

	if (GetLargeRand() % 1000000 <= iStunRate)
	{
		BUFF_AddBuffEffect(lpTargetObj, DEBUFFTYPE_STUN, 0, 0, 0, 0, 2);
...
bool CMuunAttack::Attack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf *lpMagic, int SubCode2)
{
	...
	{
		if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_INVISABLE) == TRUE)
		{
			return FALSE;
		}

		if (BUFF_TargetHasBuff(lpTargetObj, BUFFTYPE_INVISABLE) == TRUE)
		{
			return FALSE;
		}
	}
	...
	int AttackDamage = this->CalcAttackDamage(lpObj, targetdefense, SubCode2, lpTargetObj);

	if ( lpTargetObj->EXC_ReduceDamageTaken )
	{
		int beforeDamage = AttackDamage;
		AttackDamage -= ( ( AttackDamage * (int)lpTargetObj->EXC_ReduceDamageTaken) / 100 );
	}
	...
	if ( CHECK_Wings(lpTargetObj) == TRUE )
	{
		...
		if ( AttackDamage > 1 && Wing->m_Type != ITEMGET(13, 30) )
		{
			...
			if (lpTargetObj->CharacterData->Mastery.iMpsAddWingDamageBlock <= 0.0)
			{
				WingDamageBlock = 0.0;
			}
			...
			{
				WingDamageBlock = lpTargetObj->CharacterData->Mastery.iMpsAddWingDamageBlock;
			}
	...
	if ( lpTargetObj->Live )
	{
		...
		if ( lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_MONSTER )
		{
			...
			{
				if ( BUFF_TargetHasBuff(lpObj, BUFFTYPE_BLESS_POTION) == TRUE || BUFF_TargetHasBuff(lpObj, BUFFTYPE_SOUL_POTION) == TRUE )
				{
	...
	if ( AttackDamage >= 1 )
	{
		if ( lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER )
		{
			...
			else if (lpObj->CharacterData->RegisterdLMS == 1 && lpTargetObj->CharacterData->RegisterdLMS == 1)
			{
				...
				{
					if (g_LastManStanding.m_Rooms[lpObj->CharacterData->RegisteredLMSRoom].bState == 3)
					{
						selfdefense = 0;
					}
				}
	...
	if ( lpObj->Type == OBJ_USER )
	{
		if ( lpObj->m_Change == 9 )
		{
			GSProtocol.PROTOCOL_MagicAttackNumberSend(lpObj, 3, lpTargetObj->m_Index, 1);
		}
	}
	...
	if ( AttackDamage > 0 )
	{
		...
		if(lpTargetObj->EXC_ReflectDamageTaken > g_ConfigRead.calc.ReflectDamage)
		{
			lpTargetObj->EXC_ReflectDamageTaken = g_ConfigRead.calc.ReflectDamage;
		}
...
int CMuunAttack::CalcAttackDamage(LPOBJ lpObj, int targetDefense, int SubCode2, LPOBJ lpTargetObj)
{
	int ad = 0;
	int AttackDamage = lpObj->m_MuunEffectList[SubCode2].nOptValue * (lpObj->CharacterData->MasterLevel + lpObj->Level) + 10;
...
int CMuunAttack::ATTACK_CalcShieldDamage(LPOBJ lpObj, LPOBJ lpTargetObj, int iAttackDamage) {
	if (g_ShieldSystemOn == FALSE) return 0;
	if (iAttackDamage <= 0) return 0;
	if (lpObj->Type != OBJ_USER) return 0;
	if (lpTargetObj->Type != OBJ_USER) return 0;
	...
	iDamageDevideToSDRate += lpTargetObj->CharacterData->JewelOfHarmonyEffect.ReduceShieldDamageTaken;
	...
	if ( lpObj->CharacterData->JewelOfHarmonyEffect.ShieldPenetrationChance > 0 ) {
		...
		if (CHARACTER_PvPResistanceProc(iAccessoryEffect) == FALSE) {
			if (CHARACTER_DebuffProc(iIgnoreSDRate) == TRUE) {
				iDamageDevideToSDRate = 0;
			}
		}
	}

	if ( (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER) && ( lpObj->CharacterData->JewelOfHarmonyEffect.ShieldBypassRate || lpTargetObj->CharacterData->JewelOfHarmonyEffect.ReduceShieldDamageTaken || lpObj->CharacterData->JewelOfHarmonyEffect.ShieldPenetrationChance ) ) {
		g_Log.Add("[JewelOfHarmony][PvP System] Attacker:[%s][%s]-SD Decrease[%d] SD Ignore[%d] Defender:[%s][%s] SD Increase Option[%d] - SD Rate[%d]", lpObj->AccountID, lpObj->Name,
			lpObj->CharacterData->JewelOfHarmonyEffect.ShieldBypassRate, lpObj->CharacterData->JewelOfHarmonyEffect.ShieldPenetrationChance, lpTargetObj->AccountID, lpTargetObj->Name, lpTargetObj->CharacterData->JewelOfHarmonyEffect.ReduceShieldDamageTaken, iDamageDevideToSDRate);
	}
	...
	if ( bReduceShieldGage == true && iReduceLife > iReduceLifeForEffect ) {
		if ( !CC_MAP_RANGE(lpTargetObj->MapNumber) && lpTargetObj->MapNumber != MAP_INDEX_CHAOSCASTLE_SURVIVAL ) {
			GSProtocol.PROTOCOL_SendEffectInfo(lpTargetObj->m_Index, 17);
		}
	}
...
void CMuunSystem::DGLoadMuunInvenItem(_tagSDHP_ANS_DBMUUN_INVEN_LOAD *lpMsg)
{
	...
	{
		g_Log.Add("[DGLoadMuunInvenItem] MapServerMove User.Can't Open Event Inven. return!! [%s], IP [%s] ", gObj[aIndex].AccountID, gObj[aIndex].CharacterData->Ip_addr);
		return;
	}
...
void CMuunSystem::CGMuunInventoryUseItemRecv(PMSG_USEITEM_MUUN_INVEN *lpMsg, int aIndex)
{
	...
	switch (iItemUseType)
	{
		...
		case 3:
			...
			GSProtocol.PROTOCOL_InventoryItemDeleteSend(aIndex, lpMsg->inventoryPos, 1);
...
int CMuunSystem::ChkMuunOptConditionLevel(OBJECTSTRUCT *lpObj, _tagMUUN_EFFECT_LIST *pUserMuunEffect, CMuunInfo *pCMuunInfo)
{
	...
	if (nMinLv > (lpObj->Level+lpObj->CharacterData->MasterLevel) || nMaxLv < (lpObj->Level+lpObj->CharacterData->MasterLevel))
	{
		pUserMuunEffect->bOptEnable = FALSE;
		return FALSE;
	}
	...
	else if (nChkInven == 0)
	{
		for (int nInven = 0; nInven < nNeedItemCnt; nInven++)
		{
				...
				lpObj->pInventory[nItemPos].m_Type == nNeedItemNum)
			{
				...
				GSProtocol.PROTOCOL_InventoryItemDeleteSend(lpObj->m_Index, nItemPos, 1);
			}
		}
	}
