...
void CUnityBattleField::DGAnsCopyCharacterInfo(int aIndex, BYTE result, BYTE subResult)
{
	...
	g_Log.Add("[UBF][DGAnsCopyCharcterInfo][Copy Complete][%s][%s] CharInfoSave : Class=%d Level=%d LVPoint=%d Exp=%I64d Str=%d Dex=%d Vit=%d Energy=%d Leadership:%d Map=%d Pk=%d",
		lpObj->AccountID, lpObj->Name, lpObj->CharacterData->DbClass, lpObj->Level, lpObj->CharacterData->LevelUpPoint, lpObj->CharacterData->Experience, lpObj->CharacterData->Strength, lpObj->CharacterData->Dexterity,
		lpObj->CharacterData->Vitality, lpObj->CharacterData->Energy, lpObj->Leadership, lpObj->MapNumber, lpObj->m_PK_Level);
...
void CUnityBattleField::DGAnsCheckJoinedUnityBattleField(int aIndex, BYTE btRegisterState)
{
	...
	{
		lpObj->CharacterData->m_JoinUnityBattle = false;
	}
