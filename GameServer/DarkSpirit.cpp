...
void CDarkSpirit::Run() {
	...
	int nAttackSpeed = this->AttackSpeed;

	if (lpObj->CharacterData->Mastery.ML_DL_SwiftCrow > 0.0) nAttackSpeed += lpObj->CharacterData->Mastery.ML_DL_SwiftCrow;
	...
	switch (iActionMode) {
		case PetItem_Mode_Normal:						this->ModeNormal();							break;
		case PetItem_Mode_Attack_Random:				this->ModeAttackRandom();					break;
		case PetItem_Mode_Attack_WithMaster:			this->ModeAttackWithMaster();				break;
		case PetItem_Mode_Attack_Target:				this->ModeAttackTarget();					break;
	}
}
void CDarkSpirit::ModeNormal() {
	EnterCriticalSection(&this->m_SpiritCriti);

	if (this->m_iMasterIndex < g_ConfigRead.server.GetObjectStartUserIndex() || this->m_iMasterIndex > g_ConfigRead.server.GetObjectMax() - 1) return;
...
void CDarkSpirit::ModeAttackRandom() {
	...
	int iDamageType = 0;
	int iActionType = 0;
	int iCriticalDamage = this->C_CriticalStrikeChance + lpObj->CharacterData->Mastery.ML_DL_SharpTalons;
	int iExcellentDamage = this->m_ExcellentDamage + lpObj->CharacterData->Mastery.ML_DL_RazorBeak;
	int iDoubleDamage = this->m_DoubleDamage + lpObj->CharacterData->Mastery.ML_DL_LethalCrow;
				...
				if (gObj[tObjNum].Life > 0.0f && (gObj[tObjNum].Class < 100 || gObj[tObjNum].Class  > 110)) {
					if (lpObj->VpPlayer2[count].type == OBJ_MONSTER && gObj[tObjNum].m_RecallMon < 0) EnableAttack = TRUE;
						...
						if (gObj[tObjNum].Type == OBJ_MONSTER) {
							if (gObj[tObjNum].m_RecallMon >= 0) lc85 = gObj[gObj[tObjNum].m_RecallMon].m_Index;
						}

						if (gObjTargetGuildWarCheck(lpObj, &gObj[lc85]) == TRUE) EnableAttack = TRUE;
						if (gObj[tObjNum].Type == OBJ_USER && g_GensSystem.IsMapBattleZone(lpObj->MapNumber) == TRUE && g_GensSystem.IsPkEnable(lpObj, &gObj[lc85]) == TRUE) EnableAttack = TRUE;
						if (gObj[tObjNum].Class >= 678 && gObj[tObjNum].Class <= 680) EnableAttack = TRUE;
						if (IMPERIAL_MAP_RANGE(gObj[tObjNum].MapNumber) == TRUE) EnableAttack = g_ImperialGuardian.IsAttackAbleMonster(tObjNum);
					}

					if (EnableAttack != FALSE) {
						if (lpObj->MapNumber == gObj[tObjNum].MapNumber) {
							dis = CHARACTER_CalcDistance(lpObj, &gObj[tObjNum]);
...
void CDarkSpirit::ModeAttackWithMaster() {
	...
	if (lpObj->MapNumber == lpTargetObj->MapNumber) {
		if (lpTargetObj->Life > 0.0f) {
			dis = CHARACTER_CalcDistance(lpObj, lpTargetObj);
			...
			if (dis < RAVEN_ATTACK_DISTANCE) {
				...
				if (iDamageType != FALSE) this->SendAttackMsg(lpObj->m_Index, lpTargetObj->m_Index, iDamageType, iActionType);
				else this->RangeAttack(lpObj->m_Index, lpTargetObj->m_Index);
			}
		} else this->ReSetTarget(lpTargetObj->m_Index);
	}
}
void CDarkSpirit::ModeAttackTarget() {
	EnterCriticalSection(&this->m_SpiritCriti);
	...
	if (g_GensSystem.IsMapBattleZone(lpObj->MapNumber) == TRUE && g_GensSystem.IsPkEnable(lpObj, lpTargetObj) == FALSE) return;
	if (lpObj->MapNumber == lpTargetObj->MapNumber) {
		if (lpTargetObj->Life > 0.0f) {
			dis = CHARACTER_CalcDistance(lpObj, lpTargetObj);

			if (dis < RAVEN_ATTACK_DISTANCE) this->SendAttackMsg(lpObj->m_Index, lpTargetObj->m_Index, 1, 1);
		} else this->ReSetTarget(lpTargetObj->m_Index);
	}
}
	...
	while (true) {
		if (lpObj->VpPlayer2[count].state != 0) {
			...
			if (tObjNum >= 0 && aTargetIndex != tObjNum) {
				...
				if (lpObj->VpPlayer2[count].type == OBJ_MONSTER && gObj[tObjNum].m_RecallMon < 0) EnableAttack = TRUE;
				else if (tObjNum == aTargetIndex) EnableAttack = TRUE;
				else {
					int lc10 = gObj[tObjNum].m_Index;

					if (gObj[tObjNum].Type == OBJ_MONSTER) {
						if (gObj[tObjNum].m_RecallMon >= 0) lc10 = gObj[gObj[tObjNum].m_RecallMon].m_Index;
					}

					if (gObjTargetGuildWarCheck(lpObj, &gObj[lc10]) == TRUE) EnableAttack = TRUE;
					if (gObj[tObjNum].Type == OBJ_USER && g_GensSystem.IsMapBattleZone(lpObj->MapNumber) == TRUE && g_GensSystem.IsPkEnable(lpObj, &gObj[tObjNum]) == TRUE) EnableAttack = TRUE;
					if (gObj[tObjNum].Class >= 678 && gObj[tObjNum].Class <= 680) EnableAttack = TRUE;
				}
				...
				if (EnableAttack != FALSE) {
					...
					if (attackcheck != FALSE) {
						if (CHARACTER_CalcDistance(&gObj[aTargetIndex], &gObj[tObjNum]) < RAVEN_ATTACK_DISTANCE - 3) {
...
void CDarkSpirit::SendAttackMsg(int aIndex, int aTargetIndex, int iDamageType, int iActionType) {
	...
	if (iDamageType == 3) iDamageType = 0;
	...
	if (iActionType == 1) {
		...
		if (iCriticalDamage > rand() % 100) iDamageType = 1;
		if (iExcellentDamage > rand() % 100) iDamageType = 2;
		if (iDoubleDamage > rand() % 100) iDamageType = 3;
	}
...
void CDarkSpirit::ReSetTarget(int aTargetIndex) {
	...
	if (iActionMode == PetItem_Mode_Attack_Target) this->SetMode(PetItem_Mode_Normal, -1);
}
...
void CDarkSpirit::SetMode(ePetItem_Mode mode, int iTargetindex) {
	...
	if (mode == PetItem_Mode_Attack_Target || this->m_ActionMode != mode) {
		if (mode == PetItem_Mode_Attack_Target) {
			if (ObjectMaxRange(iTargetindex) != FALSE) this->m_iTargetIndex = iTargetindex;
			else {
				LeaveCriticalSection(&this->m_SpiritCriti);
				return;
			}
...
void CDarkSpirit::Set(int aIndex, CItem * pPetItem) {
	...
	if (this->m_iMasterIndex == -1) bChange = TRUE;
	...
	g_ConfigRead.m_ItemCalcLua.Generic_Call("DarkSpirit_CalcValues", "ii>iiddiiii", leadership, petitemlevel, &this->m_AttackDamageMin, &this->m_AttackDamageMax, &this->C_CriticalStrikeChance, &this->m_ExcellentDamage, &this->m_DoubleDamage, &this->m_IgnoreEnemy, &this->AttackSpeed, &this->m_SuccessAttackRate);
	this->m_iMasterIndex = aIndex;
	this->m_pPetItem = pPetItem;
	LeaveCriticalSection(&this->m_SpiritCriti);

	if (bChange != FALSE) this->SetMode(PetItem_Mode_Normal, -1);
}
BOOL CDarkSpirit::Attack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf * lpMagic, int iDamageType, int iActionType) {
	...
	if (IMPERIAL_MAP_RANGE(lpObj->MapNumber) == TRUE && g_ImperialGuardian.IsAttackAbleMonster(lpTargetObj->m_Index) == false) return FALSE;
	...
	if (lpMagic) skill = lpMagic->m_Skill;
	...
	if (!gObjAttackQ(lpTargetObj)) return FALSE;
	if (g_ArcaBattle.IsArcaBattleServer() == TRUE && g_AcheronGuardianEvent.IsPlayStart() == false && g_ArcaBattle.IsEnableAttackObelisk(lpObj, lpTargetObj->Class) == false) return FALSE;
	...
	if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) {
		if (g_CastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE) {
			if (lpObj->m_btCsJoinSide > 0) {
				if (lpObj->m_btCsJoinSide == lpTargetObj->m_btCsJoinSide) return FALSE;
			}
		}
	}
	...
	if (this->PkCheck(lpCallObj, lpTargetObj) == FALSE) return FALSE;

	int Strength = lpObj->CharacterData->Strength + lpObj->AddStrength;
	int Dexterity = lpObj->CharacterData->Dexterity + lpObj->AddDexterity;
	int Vitality = lpObj->CharacterData->Vitality + lpObj->AddVitality;
	int Energy = lpObj->CharacterData->Energy + lpObj->AddEnergy;
	...
	if (iDamageType == 1) MsgDamage = 3;
	else if (iDamageType == 2) MsgDamage = 2;
	else MsgDamage = 0;
	...
	if (g_ShieldSystemOn == TRUE) {
		if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER) {
			if (!this->ATTACK_MissCheckPvP(lpObj, lpTargetObj, skill, skillSuccess, bAllMiss)) return FALSE;
		} else if (!this->ATTACK_MissCheck(lpObj, lpTargetObj, skill, skillSuccess, bAllMiss)) return FALSE;
	...
	float iDoubleDamage = this->m_DoubleDamage;

	LeaveCriticalSection(&this->m_SpiritCriti);

	if (lpObj->Type == OBJ_USER) {
		if (lpObj->CharacterData->Mastery.ML_DL_Eyepecker > 0.0) fIgnoreEnemy += lpObj->CharacterData->Mastery.ML_DL_Eyepecker;
	}
	...
	AttackDamage = this->CalcAttackDamage(lpObj, targetdefense, iDamageType);

	if (iDamageType == 0 && iActionType == 0) AttackDamage /= 1.5;
	if (bAllMiss) AttackDamage = (AttackDamage * 30) / 100;
	if (lpObj->Type == OBJ_USER) {
		if (lpObj->CharacterData->Mastery.ML_DL_LethalCrow > 0.0) {
			iDoubleDamage += lpObj->CharacterData->Mastery.ML_DL_LethalCrow;
	...
	if (lpTargetObj->EXC_ReduceDamageTaken) AttackDamage -= ((AttackDamage * (int)lpTargetObj->EXC_ReduceDamageTaken) / 100);
	...
	lpObj->pInventory[1].m_DurabilitySmall += idamage - (idamage * lpObj->CharacterData->Mastery.ML_DL_CompanionFortitude / 100);
	...
	if (lpObj->pInventory[1].m_DurabilitySmall >= DurabilityVal) {
		...
		if (lpObj->pInventory[1].m_Durability < 1.0f) {
			...
			if (lpObj->pInventory[1].DecPetItemExp(10)) {
				g_Log.Add("[%s][%s][PetItemExpDown] [%s] Level:[%d]Exp:[%I64d]DecExp[%I64d]", lpObj->AccountID, lpObj->Name, lpObj->pInventory[1].GetName(), lpObj->pInventory[1].m_PetItem_Level, lpObj->pInventory[1].m_PetItem_Exp, iPetExp - lpObj->pInventory[1].m_PetItem_Exp);
				this->Set(lpObj->m_Index, &lpObj->pInventory[1]);
				this->SendLevelmsg(lpObj->m_Index, 1, 0, 0xFF);
			}

			g_Log.Add("[%s][%s] Dark Raven Item is Broken because durability is exhausted [%I64d]", lpObj->AccountID, lpObj->Name, lpObj->pInventory[1].m_Number);
		}

		GSProtocol.GCItemDurSend(lpObj->m_Index, 1, lpObj->pInventory[1].m_Durability, 0);
	}

	if (COMPANION_GuardianAngel(lpTargetObj) == TRUE) {
		if (AttackDamage > 1) {
	...
	if (CHECK_Wings(lpTargetObj) == TRUE) {
	...
	if (CHECK_Dinorant(lpTargetObj)) {
		...
		else AttackDamage = AttackDamage * dinorantdecdamage / 100;
	...
	if (CHECK_DarkHorse(lpTargetObj)) {
		...
		else AttackDamage = AttackDamage * decdamage / 100;

		GSProtocol.PROTOCOL_ReFillSend(lpTargetObj->m_Index, lpTargetObj->Life, 0xFF, 0, lpTargetObj->iShield);
	}

	if (lpTargetObj->Live) {
		if (lpTargetObj->m_SkillInfo.ManaShieldAbsorbPercent && AttackDamage > 0) {
			int decattackdamage = AttackDamage * lpTargetObj->m_SkillInfo.ManaShieldAbsorbPercent / 100;

			if (decattackdamage <= lpTargetObj->Mana) {
				lpTargetObj->Mana -= decattackdamage;
			...
			if (g_CastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE) {
				if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER) {
					if (lpObj->MapNumber == MAP_INDEX_CASTLESIEGE && lpTargetObj->MapNumber == MAP_INDEX_CASTLESIEGE) {
						if (lpObj->m_btCsJoinSide == lpTargetObj->m_btCsJoinSide) AttackDamage = AttackDamage * g_CastleSiege.CastleSiegeSelfDmgReductionPercent / 100;
						else if (g_ShieldSystemOn == FALSE) AttackDamage = AttackDamage * g_CastleSiege.CastleSiegeDmgReductionPercent / 100;
					}
				}
			}
		}
		...
		if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_MONSTER) {
			if (lpTargetObj->Class == 283) {
				if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_BLESS_POTION) == TRUE || BUFF_TargetHasBuff(lpObj, BUFFTYPE_SOUL_POTION) == TRUE) AttackDamage += (AttackDamage * 20) / 100;
				else {
		...
		if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER) AttackDamage = AttackDamage * g_ConfigRead.calc.PvPDamageRate_DarkSpirit[lpObj->Class][lpTargetObj->Class] / 100;
		else if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_MONSTER) AttackDamage = AttackDamage * g_ConfigRead.calc.PvMDamageRate_DarkSpirit / 100;
		if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER) {
	...
	if (lpTargetObj->Type == OBJ_MONSTER) {
		...
		if (lpTargetObj->m_iCurrentAI) lpTargetObj->m_Agro->IncAgro(lpObj->m_Index, AttackDamage / 100);
	}
	...
	if (AttackDamage >= 1) {
		if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER) {
			if (gObjDuelCheck(lpObj, lpTargetObj)) selfdefense = 0;
		...
		if (lpTargetObj->Type == OBJ_USER) gObjArmorRandomDurDown(lpTargetObj, lpObj);
	}
	...
	if (ManaChange) GSProtocol.PROTOCOL_ManaSend(lpTargetObj->m_Index, lpTargetObj->Mana, 0xFF, 0, lpTargetObj->Stamina);
	if (lpObj->Type == OBJ_USER) {
	...
	if (AttackDamage > 0) {
		int atd_reflect = (int)((float)AttackDamage * lpTargetObj->EXC_ReflectDamageTaken / 100.0f);

		if (lpTargetObj->EXC_ReflectDamageTaken > g_ConfigRead.calc.ReflectDamage) lpTargetObj->EXC_ReflectDamageTaken = g_ConfigRead.calc.ReflectDamage;
		if (atd_reflect) gObjAddMsgSendDelay(lpTargetObj, 10, lpObj->m_Index, 10, atd_reflect);
		if (g_ShieldSystemOn == TRUE) AttackDamage -= iTotalShieldDamage;

		gObjLifeCheck(lpTargetObj, lpObj, AttackDamage, 0, MSBFlag, MsgDamage, skill, iTotalShieldDamage, 0);
	} else GSProtocol.PROTOCOL_DamageSend(lpObj->m_Index, lpTargetObj->m_Index, 0, 0, MsgDamage, 0);
...
int CDarkSpirit::CalcAttackDamage(LPOBJ lpObj, int targetDefense, int crititcaldamage) {
	...
	if (sub < 0) sub = 0;
	if (crititcaldamage == 1) AttackDamage = this->m_AttackDamageMin + sub;
	...
	if (lpObj->CharacterData->Mastery.ML_DL_ImprovedDarkCrow > 0.0) AttackDamage += lpObj->CharacterData->Mastery.ML_DL_ImprovedDarkCrow;
	if (lpObj->CharacterData->Mastery.ML_DL_Commander > 0.0) AttackDamage += lpObj->CharacterData->Mastery.ML_DL_Commander;
	if (g_MasterLevelSkillTreeSystem.ML_GetNodeLevel(lpObj, 506) >= 10) AttackDamage *= 1.05;															// Leadership
...
BOOL CDarkSpirit::ATTACK_MissCheck(LPOBJ lpObj, LPOBJ lpTargetObj, int skill,  int skillSuccess, BOOL& bAllMiss) {
	EnterCriticalSection(&this->m_SpiritCriti);
	...
	int TargetSuccessfulBlocking = lpTargetObj->EvasionRating;
	int MsgDamage = 0;
	...
	if (lpObj->Type == OBJ_USER) {
		if (lpObj->Class == CLASS_RAGEFIGHTER && lpObj->CharacterData->Mastery.ML_RF_Accuracy > 0.0) SuccessAttackRate += lpObj->CharacterData->Mastery.ML_RF_Accuracy;
		else if (lpObj->CharacterData->Mastery.ML_GENERAL_Accuracy > 0.0) SuccessAttackRate += lpObj->CharacterData->Mastery.ML_GENERAL_Accuracy;
	}

	if (SuccessAttackRate < TargetSuccessfulBlocking) bAllMiss = TRUE;
	if (bAllMiss != FALSE) {
		if ((rand() % 100) >= 5) {
...
BOOL CDarkSpirit::ATTACK_MissCheckPvP(LPOBJ lpObj, LPOBJ lpTargetObj, int skill,  int skillSuccess, BOOL& bAllMiss) {
	float iAttackRate = 0;
	...
	AttackLevel = lpObj->Level + lpObj->CharacterData->MasterLevel;

	switch (lpObj->Class) {
		case CLASS_WIZARD:				iAttackRate = (lpObj->CharacterData->Dexterity * 4) / 10 + 3 * AttackLevel;					break;
		case CLASS_KNIGHT:				iAttackRate = (lpObj->CharacterData->Dexterity * 45) / 10 + 3 * AttackLevel;				break;
		case CLASS_ELF:					iAttackRate = (lpObj->CharacterData->Dexterity * 6) / 10 + 3 * AttackLevel;					break;
		case CLASS_MAGICGLADIATOR:				iAttackRate = (lpObj->CharacterData->Dexterity * 35) / 10 + 3 * AttackLevel;				break;
		case CLASS_DARKLORD:			iAttackRate = (lpObj->CharacterData->Dexterity * 4) + 3 * AttackLevel;						break;
		case CLASS_SUMMONER:			iAttackRate = (lpObj->CharacterData->Dexterity * 35) / 10 + 3 * AttackLevel;				break;
		default:						iAttackRate = (lpObj->CharacterData->Dexterity * 4) + 3 * AttackLevel;						break;
	}

	DefenseLevel = lpTargetObj->Level + lpTargetObj->CharacterData->MasterLevel;
	
	switch (lpTargetObj->Class) {
		case CLASS_WIZARD:				iDefenseRate = (lpTargetObj->CharacterData->Dexterity / 4) + 2 * DefenseLevel;				break;
		case CLASS_KNIGHT:				iDefenseRate = (lpTargetObj->CharacterData->Dexterity / 2) + 2 * DefenseLevel;				break;
		case CLASS_ELF:					iDefenseRate = (lpTargetObj->CharacterData->Dexterity / 10) + 2 * DefenseLevel;				break;
		case CLASS_MAGICGLADIATOR:				iDefenseRate = (lpTargetObj->CharacterData->Dexterity / 4) + 2 * DefenseLevel;				break;
		case CLASS_DARKLORD:			iDefenseRate = (lpTargetObj->CharacterData->Dexterity / 2) + 2 * DefenseLevel;				break;
		case CLASS_SUMMONER:			iDefenseRate = (lpTargetObj->CharacterData->Dexterity / 2) + 2 * DefenseLevel;				break;
		default:						iDefenseRate = (lpTargetObj->CharacterData->Dexterity / 4) + 2 * DefenseLevel;				break;
	}

	if (iAttackRate <= 0.0f || iDefenseRate <= 0.0f || AttackLevel <= 0 || DefenseLevel <= 0) return FALSE;
...
void CDarkSpirit::ChangeCommand(int command, int targetindex) {
	...
	if (command == PetItem_Mode_Normal) setmode = PetItem_Mode_Normal;
	else if (command == PetItem_Mode_Attack_Random) setmode = PetItem_Mode_Attack_Random;
	else if (command == PetItem_Mode_Attack_WithMaster) setmode = PetItem_Mode_Attack_WithMaster;
	else if (command == PetItem_Mode_Attack_Target) setmode = PetItem_Mode_Attack_Target;
...
void __cdecl CDarkSpirit::SendLevelmsg(int aIndex, int nPos, int PetType, int InvenType) {
	...
	if (PetType == 1) pMsg.Exp = gObj[aIndex].pInventory[nPos].m_PetItem_Exp - gPetItemExp.m_DarkHorseExpTable[pMsg.Level];
	else pMsg.Exp = gObj[aIndex].pInventory[nPos].m_PetItem_Exp - gPetItemExp.m_DarkSpiritExpTable[pMsg.Level];

	pMsg.Dur = gObj[aIndex].pInventory[nPos].m_Durability;
	IOCP.DataSend(aIndex, (LPBYTE)&pMsg, pMsg.h.size);
}
int CDarkSpirit::ATTACK_CalcShieldDamage(LPOBJ lpObj, LPOBJ lpTargetObj, int iAttackDamage) {
	int iShieldDamage = 0;
