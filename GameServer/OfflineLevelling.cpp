		...
		case MAGIC_ATTACK:
			{
				...
				{		
					...
					{
						lpMagic = PLAYER_GetMagicSearch(user,obj.wSkillNumber);
						if(lpMagic == NULL)
						{
		...
		case BE_ATTACK:
			{
				...
				for (int i=0;i<MAX_VIEWPORT;i++)
				{
					...
					{	
						...
						int DistanceCheck = CHARACTER_CalcDistance(user, tObj);
...
bool COfflineLevelling::ChargePlayer(int aIndex)
{
	...
	if(this->m_PerMapAttr[lpObj->MapNumber].CoinType == -1)
	{
		switch (this->m_General.CoinType)
		{
			case 0:
			{
				if (lpObj->CharacterData->Money < this->m_General.CoinValue)
				{
					return false;
				}
				else
				{
					lpObj->CharacterData->Money -= this->m_General.CoinValue;
					return true;
				}
			}
			break;
			...
			case 1:
			{
				if (lpObj->CharacterData->m_WCoinC < this->m_General.CoinValue)
				{
					return false;
				}
				else
				{
						lpObj->CharacterData->m_WCoinC -= this->m_General.CoinValue;
			...
			case 2:
			{
				if (lpObj->CharacterData->m_WCoinP < this->m_General.CoinValue )
				{
					return false;
				}
				else
				{
						lpObj->CharacterData->m_WCoinP -= this->m_General.CoinValue;
			...
			case 3:
			{
				if (lpObj->CharacterData->m_GoblinPoint < this->m_General.CoinValue)
				{
					return false;
				}
				else
				{
					lpObj->CharacterData->m_GoblinPoint -= this->m_General.CoinValue;
	...
	else
	{
		switch (this->m_PerMapAttr[lpObj->MapNumber].CoinType)
		{
			case 0:
			{
				if (lpObj->CharacterData->Money < this->m_PerMapAttr[lpObj->MapNumber].CoinValue)
				{
					return false;
				}
				else
				{
					lpObj->CharacterData->Money -= this->m_PerMapAttr[lpObj->MapNumber].CoinValue;
					return true;
				}
			}
			break;
			...
			case 1:
			{
				if (lpObj->CharacterData->m_WCoinC < this->m_PerMapAttr[lpObj->MapNumber].CoinValue)
				{
					return false;
				}
				else
				{
						lpObj->CharacterData->m_WCoinC -= this->m_PerMapAttr[lpObj->MapNumber].CoinValue;
			...
			case 2:
			{
				if (lpObj->CharacterData->m_WCoinP < this->m_PerMapAttr[lpObj->MapNumber].CoinValue )
				{
					return false;
				}
				else
				{
						lpObj->CharacterData->m_WCoinP -= this->m_PerMapAttr[lpObj->MapNumber].CoinValue;
			...
			case 3:
			{
				if (lpObj->CharacterData->m_GoblinPoint < this->m_PerMapAttr[lpObj->MapNumber].CoinValue)
				{
					return false;
				}
				else
				{
					lpObj->CharacterData->m_GoblinPoint -= this->m_PerMapAttr[lpObj->MapNumber].CoinValue;
...
bool COfflineLevelling::CheckUseTime(int aIndex)
{
	...
	if(this->m_General.MaxDuration == 0 && lpObj->CharacterData->VipType == 0 )
	{
		return false;
	}
	else if (this->m_General.MaxVipDuration == 0 && lpObj->CharacterData->VipType != 0)
	{
		return false;
	}
	...
	if(lpObj->CharacterData->VipType == 0)
	{
		if(useTime > (this->m_General.MaxDuration * 1000 * 3600))
		{
			...
			lpObj->m_bOffLevel = false;
			this->DeleteUser(lpObj->m_Index);
			g_Log.Add("[OffLevel][%d][%s][%s] Exceed max use time [Hours:%d] [Vip:%d]",aIndex,lpObj->AccountID,lpObj->Name,(useTime * 1000 *3600), lpObj->CharacterData->VipType);
		}
	}
	else
	{
