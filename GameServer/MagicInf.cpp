...
BOOL CMagicInf::IsMagic() {
	if (this->m_Skill == -1) return false;
...
int CMagicInf::Set(BYTE aType, WORD aIndex, BYTE aLevel) {
	...
	this->m_Level = aLevel;
	this->m_Skill = MAGIC_GetNumberIndex(aType, aIndex, aLevel);

	if (this->m_Skill < 0) this->m_Skill = 1;
...
int CMagicInf::Set(int aSkill, BYTE aLevel) {
	...
	if (g_MasterLevelSkillTreeSystem.CheckMasterLevelSkill(aSkill) == true) {
		if (MagicDamageC.GetSkillUseType(aSkill) == 4) {
			...
			if (nBrandOfSkill <= 0 || nBrandOfSkill == aSkill) {
				float fValue = g_MasterLevelSkillTreeSystem.ML_GetNodeValue(MagicDamageC.SkillGet(this->m_Skill), this->m_Level);

				if (fValue == 0.0) damage = 1;
				else damage = fValue;
			} else if (g_MasterLevelSkillTreeSystem.CheckMasterLevelSkill(nBrandOfSkill) == true) {
				float fValue = g_MasterLevelSkillTreeSystem.ML_GetNodeValue(MagicDamageC.SkillGet(nBrandOfSkill), 10);

				if (fValue == 0.0) damage = 1;
				else damage = fValue;
			} else damage = MagicDamageC.SkillGet(nBrandOfSkill);
		} else {
			float fValue = g_MasterLevelSkillTreeSystem.ML_GetNodeValue(MagicDamageC.SkillGet(this->m_Skill), this->m_Level);
	...
	} else damage = MagicDamageC.SkillGet(this->m_Skill);
...
int MAGIC_GetNumberIndex(int type, int Index, int level) {
	int skillnumber = -1;

	if (type == 15) {
		if (Index >= 0 && Index < 16) skillnumber = Index + 1;
		else if (Index == 16) skillnumber = 38;
		else if (Index == 17) skillnumber = 39;
		else if (Index == 18) skillnumber = 40;
		else if (Index == 19) skillnumber = SUM_CHAINLIGHTNING;
		else if (Index == 20) skillnumber = SUM_DRAIN_LIFE;
		else if (Index == 21) skillnumber = SUM_LIGHTNINGBLAST;
		else if (Index == 22) skillnumber = SUM_MAGICMIRROR;
		else if (Index == 23) skillnumber = SUM_AMPLIFYDAMAGE;
		else if (Index == 24) skillnumber = SUM_SLEEP;
		else if (Index == 26) skillnumber = SUM_ENFEEBLE;
		else if (Index == 27) skillnumber = SUM_WEAKNESS;
		else if (Index == 28) skillnumber = DW_ARCANEPOWER;
		else if (Index == 29) skillnumber = MG_THUNDERSTORM;
		else if (Index == 30) skillnumber = RF_CHAINDRIVE;
		else if (Index == 31) skillnumber = RF_DARKSIDE;
		else if (Index == 32) skillnumber = RF_MOLTENSTRIKE;
		else if (Index == 33) skillnumber = RF_DRAGONFIST;
		else if (Index == 34) skillnumber = RF_FOCUS;
		else if (Index == 35) skillnumber = RF_TOUGHNESS;
		else if (Index == 36) skillnumber = RF_LIGHTNINGREFLEXES;
	} else if (type == 12) {
		switch (Index) {
