...
int CLifeStone::CreateLifeStone(int iIndex)
{
	...
	if ( lpObj->CharacterData->GuildStatus != 0x80 )
		return FALSE;
	...
	if ( lpObj->CharacterData->lpGuild->lpLifeStone  )
	{
	...
	if ( iMonsterIndex >= 0 )
	{
		...
		gObj[iMonsterIndex].CharacterData = new USER_DATA();
		...
		gObj[iMonsterIndex].CharacterData->lpGuild = lpObj->CharacterData->lpGuild;
		gObj[iMonsterIndex].m_btCreationState = 0;
		lpObj->CharacterData->lpGuild->lpLifeStone = &gObj[iMonsterIndex];

		MsgOutput(iIndex, Lang.GetText(0,180));

		g_Log.Add("[CastleSiege] LifeStone is created - [%s] [%s][%s] (Map:%d)(X:%d, Y:%d)",
			lpObj->CharacterData->lpGuild->Name, lpObj->AccountID, lpObj->Name, lpObj->MapNumber, cX, cY);
...
int CLifeStone::DeleteLifeStone(int iIndex)
{
	...
	if ( lpLifeStone->CharacterData->lpGuild )
	{
		g_Log.Add("[CastleSiege] LifeStone is broken - [%s]", lpLifeStone->CharacterData->lpGuild->Name);
		lpLifeStone->CharacterData->lpGuild->lpLifeStone = NULL;
		delete lpLifeStone->CharacterData;
	}
...
int CLifeStone::SetReSpawnUserXY(int iUserIndex)
{
	...
	if ( lpUser->CharacterData->lpGuild == NULL )
		return FALSE;

	BYTE btCsJoinSide = lpUser->m_btCsJoinSide;

	if ( lpUser->CharacterData->lpGuild->lpLifeStone == NULL )
		return FALSE;

	LPOBJ lpLifeStone = lpUser->CharacterData->lpGuild->lpLifeStone;
			...
			if ( gObj[tObjNum].Type == OBJ_USER && gObj[tObjNum].Live )
			{
				if ( gObj[tObjNum].m_btCsJoinSide == lpObj->m_btCsJoinSide )
				{
						 ...
						 abs(lpObj->X - gObj[tObjNum].X) <= 3 )
					{
						...
						if ( gObj[tObjNum].Stamina < (gObj[tObjNum].MaxStamina + gObj[tObjNum].AddStamina))
						{
							gObj[tObjNum].Stamina += ( gObj[tObjNum].MaxStamina + gObj[tObjNum].AddStamina ) / 100;

							if ( gObj[tObjNum].Stamina > (gObj[tObjNum].MaxStamina + gObj[tObjNum].AddStamina))
								gObj[tObjNum].Stamina = gObj[tObjNum].MaxStamina + gObj[tObjNum].AddStamina;

							bBpChange = TRUE;
						}

						if (bLifeChange )
							GSProtocol.PROTOCOL_ReFillSend(tObjNum, gObj[tObjNum].Life, 0xFF, 1, gObj[tObjNum].iShield);

						if (bManaChange ||bBpChange ) 
							GSProtocol.PROTOCOL_ManaSend(tObjNum, gObj[tObjNum].Mana, 0xFF, 0, gObj[tObjNum].Stamina);
					}
				}
			}
		}
