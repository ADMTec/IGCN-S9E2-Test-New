...
void CArcaBattle::RemoveGuildBuff(char *szGuildName, WORD wBuffIndex)
{
	...
	for (int nGuildCnt = 0; nGuildCnt < lpGuild->Count; ++nGuildCnt)
	{
		...
		{
			if (BUFF_TargetHasBuff(&gObj[iIndex], wBuffIndex) == TRUE)
			{
				BUFF_RemoveBuffEffect(&gObj[iIndex], wBuffIndex);
...
void CArcaBattle::ReqGuildMemberDelBuffDel(OBJECTSTRUCT *lpObj, char *szGuildName)
{
	for (int i = 0; i < 5; ++i)
	{
		if(strcmp(this->m_stABWinGuildInfo[i].szGuildName, szGuildName) == 0)
		{
			...
			if (BUFF_TargetHasBuff(lpObj, wBuffIndex) == TRUE)
			{
				g_BuffEffect.RequestPeriodBuffDelete(lpObj, wBuffIndex);
				BUFF_RemoveBuffEffect(lpObj, wBuffIndex);
...
{
	if (obj->CharacterData->lpGuild)
	{
		for (int i = 0; i < 5; ++i)
		{
			if(strcmp(obj->CharacterData->lpGuild->Name, this->m_stABWinGuildInfo[i].szGuildName) == 0)
			{
				if (obj->CharacterData->lpGuild->Number == this->m_stABWinGuildInfo[i].dwGuild)
					return TRUE;
			}
		}
	}
...
BOOL CArcaBattle::IsArcaBattleOccupyZone(OBJECTSTRUCT *lpObj, int gt)
{
	...
	for (int i = 0; i < 5; i++)
	{
		if(strcmp(lpObj->CharacterData->lpGuild->Name, this->m_stABWinGuildInfo[i].szGuildName))
		{
			continue;
		}
				...
				for (int j = 0; j < 3; j++)
				{
					if (this->m_stObeliskInfo[i].m_stAuraInfo[j].m_iAuraPosX == lpObj->X && this->m_stObeliskInfo[i].m_stAuraInfo[j].m_iAuraPosY == lpObj->Y)
					{
						...
						g_Log.Add("[ArcaBattle] Obelisk foot hold [on][%s][%s][%s] : Pos X[%d] Y[%d]",
							lpObj->AccountID, lpObj->Name, lpObj->CharacterData->lpGuild->Name, lpObj->X, lpObj->Y);
					}
				}
			}
		}
	...
	for (int i = 0; i < this->m_iObeliskCnt; ++i)
	{
		...
		if (iAurReleaseCnt == 3)
		{
			...
			for (int j = 0; j < 2; ++j)
			{
				...
				if (gObj[iHoverUserIndex_1].CharacterData->GuildNumber != gObj[iHoverUserIndex_2].CharacterData->GuildNumber)
			...
			if (this->m_stObeliskState[i].m_iAuraState == 2)
			{
				...
				if (this->m_stObeliskState[i].m_iAuraReleaseGuildNum != gObj[iUserIndex].CharacterData->GuildNumber)
				{
					int iObeliskIndex = this->m_stObeliskState[i].m_iMonIndex;
				...
				if (this->m_stObeliskState[i].m_iAuraReleaseGuildNum != gObj[iUserIndex].CharacterData->GuildNumber &&
					this->m_stObeliskState[i].m_iOccupyGuildNum != gObj[iUserIndex].CharacterData->GuildNumber)
				{
					this->m_stObeliskState[i].m_iAuraReleaseGuildNum = gObj[iUserIndex].CharacterData->GuildNumber;
					...
					for (int j = 0; j < 3; j++)
					{
						...
						if(gObj[this->m_stObeliskState[i].m_stAuraState[nAura].m_iHoverUserIndex].Type == OBJ_USER && ObjectMaxRange(this->m_stObeliskState[i].m_stAuraState[nAura].m_iHoverUserIndex))
						{
							if (this->m_stObeliskState[i].m_stAuraState[j].m_btAuraState != 0 && (this->m_stObeliskState[i].m_stAuraState[nAura].m_iHoverUserIndex != 0 ||
								gObj[iTarObjIndex].CharacterData->GuildNumber != gObj[this->m_stObeliskState[i].m_stAuraState[nAura].m_iHoverUserIndex].CharacterData->GuildNumber))
							{
...
BOOL CArcaBattle::IsEnableAttackObelisk(OBJECTSTRUCT *lpObj, int iMonNumber)
{
	...
	if (pObeliskState->m_iOccupyGuildNum == lpObj->CharacterData->GuildNumber)
	{
		return FALSE;
	}
	...
	if (pObeliskState->m_iAuraReleaseGuildNum != lpObj->CharacterData->GuildNumber)
	{
		return FALSE;
	}
...
int CArcaBattle::IsPkEnable(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	_GUILD_INFO_STRUCT *lpTarGuildInfo = lpTargetObj->CharacterData->lpGuild;
...
int CArcaBattle::IsOccupyObelisk(LPOBJ lpObj, int iMonIndex)
{
	...
	for (int i = 0; i < this->m_iObeliskCnt; i++)
	{
		...
		if (iMonIndex == this->m_stObeliskInfo[i].m_iMonsterNum)
		{
			...
			{
				this->GCArcaBattleSendNotiMsg(4, 0, lpObj->CharacterData->lpGuild->Name);

				for (int i = 0; i < this->m_iABJoinUserCnt; i++)
				{
					if (this->m_stABJoinUserInfo[i].dwGuild != lpObj->CharacterData->GuildNumber)
					{
			...
			this->GCArcaBattleCurrentStatus(this->m_stObeliskInfo[i].m_iGroupNumber, this->m_stObeliskInfo[i].m_iAttrKind, ARCA_BATTLE_OBELISK_CAPTURED, lpObj->CharacterData->GuildNumber, lpObj->CharacterData->lpGuild->Name, 0);
			this->m_stObeliskState[i].m_iObeliskState = 1;
			this->m_stObeliskState[i].m_iOccupyGuildNum = lpObj->CharacterData->GuildNumber;
			this->m_stObeliskState[i].m_iGroupNumber = this->m_stObeliskInfo[i].m_iGroupNumber;
			memcpy(this->m_stObeliskState[i].m_szOccupyGuildName, lpObj->CharacterData->GuildName, MAX_GUILD_LEN + 1);

			g_Log.Add("[ArcaBattle] Occupy Obelisk In ArcaBattle Progress  [%s][%s][%s] ObeliskGroup:[%d]",
				lpObj->AccountID, lpObj->Name, lpObj->CharacterData->GuildName, this->m_stObeliskState[i].m_iGroupNumber);
...
void CArcaBattle::SendPlayResult() {
	...
	for (int n = g_ConfigRead.server.GetObjectStartUserIndex(); n < g_ConfigRead.server.GetObjectMax(); n++) {
		...
		if (gObjIsConnected(n) == TRUE && lpObj->Type == OBJ_USER) {
			...
			for (int i = 0; i < 5; ++i) {
				if (lpObj->CharacterData->lpGuild->Number == this->m_stObeliskState[i].m_iOccupyGuildNum) pMsg.btABResult = 1;
			}

			_tagArcaBattleUserInfo * pUserInfo = this->GetUserInfo(lpObj->Name);

			if (!pUserInfo) {
				g_Log.Add("[ArcaBattle] Error SendPlayResult() pUserInfo NULL %s %s GuildName %s", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->GuildName);
				continue;
			}
			...
			if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_SEAL_OF_SUSTENANCE) || BUFF_TargetHasBuff(lpObj, BUFFTYPE_SEAL_OF_SUSTENANCE_EXP)) iRewardExp = 0;
			if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(&gObj[n])) gObj[n].CharacterData->MasterExperience += iRewardExp;
			else gObj[n].CharacterData->Experience += iRewardExp;				
			if (CHARACTER_LevelUp(&gObj[n], iRewardExp, 0, "Arca Battle") == true) GSProtocol.PROTOCOL_PlayerExperienceSend(n, -1, iRewardExp, 0, 0);
...
int CArcaBattle::DropItem(OBJECTSTRUCT *lpObj, OBJECTSTRUCT *lpMonsterObj) {
	if (lpObj->MapNumber != MAP_INDEX_ACHERON) return false;
	if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_EXP_REWARDS) == false) return false;
	if (g_BagManager.SearchAndUseBag(lpObj->m_Index, BAG_EVENT, EVENTBAG_ARCA, lpMonsterObj->m_Index) == false) return false;
	return true;
}
...
int CArcaBattle::GetGuildMasterPos(LPOBJ lpObj, int nGateNum, short & x, short & y)
{
	...
	{
		this->SetGuildMasterGateNum(nGateNum, lpObj->CharacterData->GuildNumber);
		return FALSE;
	}

	if (BUFF_TargetHasBuff(lpObj, 184) == TRUE)
	{
		return FALSE;
	}
	...
	for (int n = 0; n < lpObj->CharacterData->lpGuild->Count;n++)
	{
		if (lpObj->CharacterData->lpGuild->GuildStatus[n] == GUILD_MASTER)
		{
			if (gObjIsConnected(lpObj->CharacterData->lpGuild->Index[n]) == TRUE)
			{
				iMasterIndex = lpObj->CharacterData->lpGuild->Index[n];
				break;
			}
		}
	}
	...
	if (attr & 1)
	{
		this->SetGuildMasterGateNum(0, lpObj->CharacterData->GuildNumber);
		return FALSE;
	}

	if (this->GetGuildMasterGateNum(lpObj->CharacterData->GuildNumber) == nGateNum)
	{
...
bool CArcaBattle::CGReqMarkReg(int iIndex)
{
	...
	if (lpObj->CharacterData->GuildStatus != G_MASTER && lpObj->CharacterData->GuildStatus != G_SUB_MASTER && lpObj->CharacterData->GuildStatus != G_BATTLE_MASTER)
	{
		this->GCAnsMarkRegErrCode(15, iIndex);
		return false;
	}
	...
	this->GDReqMarkCnt(iIndex, lpObj->CharacterData->GuildNumber);
	return true;
}
...
void CArcaBattle::CGReqMarkRegButtonClick(OBJECTSTRUCT *lpObj)
{
	...
	g_Log.Add("[ArcaBattle] [%s][%s] ReqMarkReg Cnt [%d] - [%s][%d]",
		lpObj->AccountID, lpObj->Name, iItemCnt, lpObj->CharacterData->GuildName, lpObj->CharacterData->GuildStatus);
...
{
	this->GDReqMarkRank(iIndex, gObj[iIndex].CharacterData->GuildNumber);
}
...
{
	_GUILD_INFO_STRUCT *lpGuild = gObj[iIndex].CharacterData->lpGuild;
	...
	pMsg.h.size = sizeof(pMsg);
	pMsg.wNumber = iIndex;
	pMsg.dwGuildNum = gObj[iIndex].CharacterData->GuildNumber;
	...
	memcpy(pMsg.szGuildName, gObj[iIndex].CharacterData->GuildName, MAX_GUILD_LEN+1);
	wsDataCli.DataSend((char *)&pMsg, pMsg.h.size);
}
...
void CArcaBattle::DGAnsMarkReg(_tagPMSG_ANS_ARCA_BATTLE_MARK_REG_DS *lpMsg)
{
	...
	g_Log.Add("[ArcaBattle][MarkReg] Error Mark Reg Ans [%s][%s][%s] ",
		gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].CharacterData->GuildName);
}
...
void CArcaBattle::CheatABOccupyObelisk(OBJECTSTRUCT *lpObj, int iObeliskGroup)
{
	...
	this->m_stObeliskState[iObeliskGroup - 1].m_iOccupyGuildNum = lpObj->CharacterData->GuildNumber;
	this->m_stObeliskState[iObeliskGroup - 1].m_iGroupNumber = this->m_stObeliskInfo[iObeliskGroup - 1].m_iGroupNumber;
	memcpy(this->m_stObeliskState[iObeliskGroup - 1].m_szOccupyGuildName, lpObj->CharacterData->GuildName, MAX_GUILD_LEN + 1);
}
...
void CArcaBattle::GuildMemberAssignStatus(OBJECTSTRUCT *lpObj, int iGuildStatus)
{
	...
	if (lpObj->CharacterData->GuildNumber > 0 && lpObj->CharacterData->lpGuild)
	{
		if (lpObj->CharacterData->GuildStatus == G_MASTER)
		{
			pMsg.btResult = 17;
		...
		{
			if (iBattleMasterCount >= (lpObj->CharacterData->MasterLevel + lpObj->Level) / 200 + 1)
			{
				pMsg.btResult = 18;
				IOCP.DataSend(aIndex, (LPBYTE)&pMsg, pMsg.h.size);
							...
							for (int l = 0; l < lpGuild->Count; l++)
							{
								...
								else
								{
										...
										gObjIsConnected(iIndex) == TRUE)
									{
										...
										stABCurJoinGuildUser.btStatus = gObj[iIndex].CharacterData->GuildStatus;
