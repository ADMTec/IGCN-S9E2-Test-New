...
void EGResultRegEventChip(PMSG_ANS_REGISTER_EVENTCHIP * aRecv)
{
	...
	else
	{
		...
		GSProtocol.PROTOCOL_InventoryItemDeleteSend(aIndex, aRecv->Pos, 1);
...
void EGRecvChangeRena( PMSG_ANS_RESET_EVENTCHIP* aRecv)
{
	...
	if ( aRecv->bSUCCESS != FALSE )
	{
		lpObj->CharacterData->Money += lpObj->EventChipCount * 3000;
...
void EGRecvStoneInfo( PMSG_ANS_VIEW_STONES* aRecv)
{
	...
	GSProtocol.PROTOCOL_NPCChatSend(pEventObj, msg, lpObj->m_Index);
}
...
void EGRecvRegStone( PMSG_ANS_REGISTER_STONES* aRecv)
{
	...
	if ( aRecv->bSUCCESS != FALSE )
	{
		...
		GSProtocol.PROTOCOL_InventoryItemDeleteSend(aIndex, aRecv->iPosition, 1);
...
void EGRecvChangeStones( PMSG_ANS_RESET_EVENTCHIP* aRecv)
{
	...
	if ( aRecv->bSUCCESS != FALSE )
	{
		lpObj->CharacterData->Money += lpObj->iStoneCount * 3000;
		GSProtocol.GCMoneySend(aIndex, lpObj->CharacterData->Money);
	...
	if ( aRecv->btIsRegistered == FALSE )
	{
		...
		switch ( aRecv->iGiftNumber )
		{
			...
			case 5:
				...
				break;
			case 6:
				gObj[aRecv->iINDEX].CharacterData->Money += 500000;
				GSProtocol.GCMoneySend(aRecv->iINDEX, gObj[aRecv->iINDEX].CharacterData->Money);
				...
				break;
			case 7:
				gObj[aRecv->iINDEX].CharacterData->Money += 50000;
				GSProtocol.GCMoneySend(aRecv->iINDEX, gObj[aRecv->iINDEX].CharacterData->Money);
			...
			case 8:	case 9:	case 10:
				gObj[aRecv->iINDEX].CharacterData->Money += 30000;
				GSProtocol.GCMoneySend(aRecv->iINDEX, gObj[aRecv->iINDEX].CharacterData->Money);
...
void EGRecvRegRingGift( PMSG_ANS_REG_RINGGIFT* aRecv)
{
	...
	if ( aRecv->btIsRegistered == 1 )
	{
		if ( CHECK_LIMIT(aRecv->btGiftKind-1, 4) )
		{
			...
			GSProtocol.PROTOCOL_SendServerAnnounce(szTemp);
...
void EGAnsRegCCOfflineGift( PMSG_ANS_REG_CC_OFFLINE_GIFT* lpMsg)
{
	...
	GSProtocol.PROTOCOL_SendServerAnnounce(szText);
	...
	if(pMsg.btResult == 1)
	{
		if(lpObj->pInventory[lpMsg->Pos].IsItem() == TRUE && lpObj->pInventory[lpMsg->Pos].m_Type == ITEMGET(14,100))
		{
			...
			else
			{
				...
				GSProtocol.PROTOCOL_InventoryItemDeleteSend(lpObj->m_Index, lpMsg->Pos, 1);
			}
...
void EGAnsSantaGift(PMSG_ANS_SANTAGIFT *lpMsg)
{
	...
	{
		case 0:
			if (lpObj->CharacterData->SantaCount < g_ConfigRead.data.common.XMasSantaFirstPrizeMaxCount)
			{
				g_BagManager.SearchAndUseBag(lpObj->m_Index, BAG_EVENT, EVENTBAG_SANTAFIRST, lpObj->m_Index);
			}

			else if (lpObj->CharacterData->SantaCount < g_ConfigRead.data.common.XMasSantaSecondPrizeMaxCount)
			{
			...
			lpObj->CharacterData->SantaCount++;
			g_Log.Add("[X-MAS Event] [AccountID]: %s, [VISITCOUNT]:%d", lpObj->AccountID, lpObj->CharacterData->SantaCount);
			break;
		case 2:
			GSProtocol.GCServerCmd(lpObj->m_Index, 16, 2, 0);
			break;
