...
void GDGuildCreateRecv(SDHP_GUILDCREATE_RESULT * lpMsg)
{
	...
	if ( lpMsg->Result == 1 )
	{
		...
		{
			gObj[aIndex].CharacterData->lpGuild = lpNode;
			gObj[aIndex].CharacterData->GuildNumber = lpMsg->GuildNumber;
			gObj[aIndex].CharacterData->GuildStatus = 0x80;
			lpNode->iGuildRival = 0;
			lpNode->iGuildUnion = 0;
			lpNode->iTimeStamp = 0;
			gObj[aIndex].CharacterData->iGuildUnionTimeStamp = 0;
			...
			if ( szMaster[0] == gObj[aIndex].Name[0] )
			{
				if ( strcmp(szMaster, gObj[aIndex].Name) == 0 )
				{
					...
					if ( lpMsg->Result == 1 || lpMsg->Result == 4 )
					{
						...
						if (gObj[aIndex].CharacterData->m_bUseGuildMatching == true)
						{
							if (gObj[aIndex].CharacterData->GuildNumber > 0)
							{
								GDReqDelMatchingList(aIndex, gObj[aIndex].CharacterData->GuildNumber);
							}
	...
	if ( lpMsg->Result == 1 || lpMsg->Result == 4)
	{
		...
		for ( int n=0;n<MAX_USER_GUILD;n++)
		{
			...
			if ( lpGuild->Use[n] > 0 && iGuildMemberIndex != -1 )
			{
				...
				lpObj->CharacterData->GuildStatus = -1;
...
void GDGuildMemberAddResult(SDHP_GUILDMEMBERADD_RESULT * lpMsg)
{
	...
	if ( lpMsg->Result == 1 )
	{
		...
		{
			gObj[aIndex].CharacterData->lpGuild = lpNode;
			gObj[aIndex].CharacterData->GuildStatus = 0;
			gObj[aIndex].CharacterData->GuildNumber = lpNode->Number;
			gObj[aIndex].CharacterData->iGuildUnionTimeStamp = 0;
			GSProtocol.GCGuildViewportNowPaint(aIndex, szGuildName, gObj[aIndex].CharacterData->lpGuild->Mark, FALSE);
			...
			for ( int n = 0; n < MAX_VIEWPORT; ++n )
			{
				...
				if(iTargetIndex >= 0)
				{
					...
					{
						if ( gObjIsConnected(lpTargetObj) && lpTargetObj->CharacterData->lpGuild != NULL )
						{
...
void GDGuildMemberDelResult(SDHP_GUILDMEMBERDEL_RESULT * lpMsg)
{
	...
	if ( lpMsg->Result == 1 )
	{
		...
		while ( true )
		{
			...
			{
				if ( gObj[iMember].CharacterData->GuildNumber > 0 )
				{
					if ( gObj[iMember].Name[0] == szMember[0] && gObj[iMember].Name[1] == szMember[1] )
					{
						if ( strcmp(gObj[iMember].Name, szMember) == 0 )
						{
							...
							gObj[iMember].CharacterData->lpGuild = NULL;
							gObj[iMember].CharacterData->GuildNumber = 0;
							gObj[iMember].CharacterData->GuildStatus = -1;
		...
		if (gObj[n].Connected > PLAYER_LOGGED)
		{
			if (gObj[n].Name[0] == lpMsg->MemberID[0])
			{
				if (strcmp(lpMsg->MemberID, gObj[n].Name) == 0)
				{
					...
					strcpy(gObj[n].CharacterData->GuildName, szGuildName);
					gObj[n].CharacterData->lpGuild = Guild.SearchGuild(gObj[n].CharacterData->GuildName);

					if (gObj[n].CharacterData->lpGuild != NULL)
					{
						gObj[n].CharacterData->GuildStatus = lpMsg->btGuildStatus;
						gObj[n].CharacterData->GuildNumber = gObj[n].CharacterData->lpGuild->Number;
						Guild.ConnectUser(gObj[n].CharacterData->lpGuild, lpMsg->GuildName, gObj[n].Name, n, lpMsg->pServer);
						/*gObj[n].lpGuild->Notice[MAX_GUILD_NOTICE_LEN-1] = 0;
						gObj[n].lpGuild->Notice[MAX_GUILD_NOTICE_LEN] = 0;*/

						if (strlen(gObj[n].CharacterData->lpGuild->Notice) > 0)
						{
							GSProtocol.GCServerMsgStringSend(gObj[n].CharacterData->lpGuild->Notice, n, 2);
						}

						g_Log.Add("[U.System][Guild Status][Info] Guild : %s / [%s][%s] / %d",
							szGuildName, gObj[n].AccountID, szName, gObj[n].CharacterData->GuildStatus);

						g_Log.Add("[%s][%s] GuildName: %s", gObj[n].AccountID, szName, szGuildName);

						pMsg.GuildNumber = gObj[n].CharacterData->GuildNumber;
						pMsg.NumberH = SET_NUMBERH(n) & 0x7F;
						pMsg.NumberL = SET_NUMBERL(n);

						if (strcmp(gObj[n].CharacterData->lpGuild->Names[0], gObj[n].Name) == 0) // Case Guild Master
						{
							pMsg.NumberH |= 0x80;
						}

						pMsg.btGuildStatus = gObj[n].CharacterData->GuildStatus;

						if (gObj[n].CharacterData->lpGuild != NULL)
						{
							pMsg.btGuildType = gObj[n].CharacterData->lpGuild->btGuildType;
						}
						else
						{
						...
						if (!Guild.SearchGuild(gObj[n].CharacterData->lpGuild->szGuildRivalName))
						{
							GDGuildInfoSendRequest(gObj[n].CharacterData->lpGuild->szGuildRivalName);
						}

						if (gObj[n].CharacterData->lpGuild->WarState != 0)
						{
							GSProtocol.GCGuildWarScore(n);
							GSProtocol.GCGuildWarDeclare(n, gObj[n].CharacterData->lpGuild->TargetGuildName);
						}
						if (g_ArcaBattle.IsArcaBattleServer() && g_AcheronGuardianEvent.IsPlayStart() == false)
							g_ArcaBattle.PrintGuildMemberLog(gObj[n].CharacterData->lpGuild);
...
void GDGuildReqAssignStatus(int aIndex, int iAssignType, LPSTR szTagetName, int iGuildStatus)
{
	...
	memcpy(pMsg.szGuildName, gObj[aIndex].CharacterData->lpGuild->Name, MAX_GUILD_LEN);
...
void GDRelationShipReqJoin(int aIndex, int iTargetUserIndex, int iRelationShipType)
{
	...
	pMsg.iRequestGuildNum = gObj[aIndex].CharacterData->lpGuild->Number;
	pMsg.iTargetGuildNum = gObj[iTargetUserIndex].CharacterData->lpGuild->Number;
...
void FriendListRequest(int aIndex)
{
	...
	gObj[aIndex].CharacterData->m_nServerChannel = g_ConfigRead.server.GetGameServerCode();
...
void FriendAddRequest(PMSG_FRIEND_ADD_REQ * lpMsg, int aIndex)
{
    ...
    if(ObjectMaxRange(aTargetIndex) == true && gObj[aTargetIndex].Type != OBJ_USER  )
    {
		g_Log.AddC(TColor::Red,"[HACKTOOL] : NPC-FriendAddRequest npc:%d ip:%s account:%s name:%s State:%d",
			gObj[aTargetIndex].Class,gObj[aIndex].CharacterData->Ip_addr,gObj[aIndex].AccountID,
...
void FriendMemoSend(PMSG_FRIEND_MEMO * lpMsg, int aIndex)
{
	...
	if ( gObj[aIndex].CharacterData->Money < 1000 )
	{
...
BOOL WithdrawUserMoney(LPSTR Type, LPOBJ lpObj, int Withdraw_Money)
{
	...
	if ( (lpObj->CharacterData->Money - Withdraw_Money) >= 0 )
	{
		oldmoney = lpObj->CharacterData->Money;
		lpObj->CharacterData->Money -= Withdraw_Money;

		GSProtocol.GCMoneySend(lpObj->m_Index, lpObj->CharacterData->Money);
		
		g_Log.Add("[%s][%s] (%s) Pay Money(In Inventory) : %d - %d = %d",
			lpObj->AccountID, lpObj->Name, Type, oldmoney, Withdraw_Money, lpObj->CharacterData->Money);
...
void DGAnsRegGensMember(_tagPMSG_ANS_REG_GENS_MEMBER_EXDB *lpMsg)
{
	...
	if (gObjIsConnectedGP(lpObj->m_Index))
	{
		...
		case 5:
			g_Log.Add("[GensSystem] [%s][%s] Gens Joining Failed to Not Register Guild Leader GuildName [%s]", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->GuildName);      
			break;
...
void DGAnsSecedeGensMember(_tagPMSG_ANS_SECEDE_GENS_MEMBER_EXDB *lpMsg)
{
	...
	if (gObjIsConnectedGP(lpObj->m_Index))
	{
		...
		if (!lpMsg->bResult)
		{
			...
			for( int i = 0 ; i < 10 ; i++ )
			{
				memset(&lpObj->CharacterData->KillUserName[i*11], 0x00, 11);

				lpObj->CharacterData->m_KillUserCnt[i] = 0;
				lpObj->CharacterData->m_KillUserTotCnt = 0;
			}
...
void DGAnsAbusingInfo(_tagPMSG_ANS_ABUSING_INFO *lpMsg)
{
	...
	if (gObjIsConnectedGP(iObjIndex))
	{
		for (int i = 0; i < 10; ++i)
		{
			...
			{
				lpObj->CharacterData->KillUserName[i * 11 + MAX_ACCOUNT_LEN] = 0;
				memcpy(&lpObj->CharacterData->KillUserName[i * 11], &lpMsg->szCharNames[i * 11], 10);
				lpObj->CharacterData->m_KillUserCnt[i] = lpMsg->KillCount[i];
				++lpObj->CharacterData->m_KillUserTotCnt;
			}
		}
	}
	else
...
void DGGensServerGroupChattingRecv(EXSDHP_SERVERGROUP_GENS_CHATTING_RECV *lpMsg)
{
	...
	if(lpMsg->iGensNum == DUPRIAN_INFLUENCE || lpMsg->iGensNum == VANERT_INFLUENCE)
	{
		for(int i=g_ConfigRead.server.GetObjectStartUserIndex();i<g_ConfigRead.server.GetObjectMax();i++)
		{
			...
			{
				if(lpObj->CharacterData->m_GensInfluence == lpMsg->iGensNum)
				{
...
void DGAnsGuildMatchingList(_stAnsGuildMatchingList *lpMsg)
{
	...
	gObj[nUserIndex].CharacterData->m_bUseGuildMatching = true;
...
void DGAnsDelMatchingList(_stAnsDelGuildMatchingList *lpMsg)
{
	...
	gObj[nUserIndex].CharacterData->m_bUseGuildMatching = false;
...
void GDReqAllowJoinGuildMatching(int nUserIndex, int nAllowType, char *szName, char *szGuildName)
{
	...
	if (nAllowType == 1)
	{
		...
		if (gObj[nUserIndex].CharacterData->GuildNumber <= 0)
		{
			GSProtocol.GCResultSend(nUserIndex, 0x51, 0x04);
			bError = TRUE;
		}

		if (strcmp(gObj[nUserIndex].CharacterData->lpGuild->Names[0], gObj[nUserIndex].Name))
		{
			GSProtocol.GCResultSend(nUserIndex, 0x51, 0x04);
			bError = TRUE;
		}

		if (gObj[nUserIndex].CharacterData->lpGuild->WarState)
		{
			GSProtocol.GCResultSend(nUserIndex, 0x51, 0x06);
			bError = TRUE;
		}
		...
		if (gObj[nUserIndex].Class == CLASS_DARKLORD)
			MaxGuildMember = (gObj[nUserIndex].Level + gObj[nUserIndex].CharacterData->MasterLevel) / 10 + gObj[nUserIndex].Leadership / 10;
		else
			MaxGuildMember = (gObj[nUserIndex].Level + gObj[nUserIndex].CharacterData->MasterLevel) / 10;

		if (MaxGuildMember > g_ConfigRead.data.common.MaxGuildMember)
			MaxGuildMember = g_ConfigRead.data.common.MaxGuildMember;

		if (gObj[nUserIndex].CharacterData->lpGuild->TotalCount >= MaxGuildMember)
		{
...
void DGAnsRegWantedPartyMember(_stAnsRegWantedPartyMember *lpMsg)
{
	...
	if (lpMsg->nResult == 0)
	{
		gObj[nUserIndex].CharacterData->m_bUsePartyMatching = true;
		gObj[nUserIndex].CharacterData->m_bPartyMatchingLeader = true;
	}
...
void GDReqGetPartyMatchingList(int nPage, char *szSearchWord, int nType, int nUserIndex)
{
	...
	pMsg.nLevel = gObj[nUserIndex].Level + gObj[nUserIndex].CharacterData->MasterLevel;
...
void DGAnsSendPartyMemberListPartyMatching(_stAnsSendPartyMemberList *lpMsg)
{
	...
	if (gObj[nUserIndex].CharacterData->m_bPartyMatchingLeader == true)
	{
		int nPartyNumber = gObj[nUserIndex].PartyNumber;
		...
		if (nPartyNumber >= 0)
		{
			...
			gObj[nUserIndex].CharacterData->m_nPartyMatchingMemberCount = lpMsg->nMemberCount;
		}
	}

	if (!lpMsg->nMemberCount && !gObj[nUserIndex].CharacterData->m_bPartyMatchingLeader)
	{
		gObj[nUserIndex].CharacterData->m_bUsePartyMatching = false;
	...
	for (int n = 0; n < MAX_USER_IN_PARTY; n++)
	{
		if (lpMsg->stList[n].bUse == TRUE)
		{
			...
			if (lpMsg->stList[n].bFlag && ObjectMaxRange(nMemberIndex) && gObj[nMemberIndex].Type == OBJ_USER)
			{
				...
				gObj[nMemberIndex].CharacterData->m_nPartyMatchingIndex = lpMsg->nPartyMatchingIndex;
				nCurrentServer = TRUE;
...
void DGAnsCancelPartyMatching(_stAnsCancelPartyMatching *lpMsg)
{
	...
	if (!lpMsg->nResult)
	{
		...
		gObj[nUserIndex].CharacterData->m_bPartyMatchingLeader = false;
		gObj[nUserIndex].CharacterData->m_bUsePartyMatching = false;
		gObj[nUserIndex].CharacterData->m_nPartyMatchingIndex = -1;
		gObj[nUserIndex].CharacterData->m_nPartyMatchingMemberCount = 0;
	}
...
void DGAnsAcceptMemberJoin(_stAnsAddPartyMember *lpMsg)
{
	...
	if (lpMsg->btSendType)
	{
		...
		if (!lpMsg->btType)
		{
			gObj[lpMsg->nMemberIndex].CharacterData->m_bUsePartyMatching = false;
		}
...
void DGAnsDeletePartyUser(_stAnsDelPartyUserPartyMatching *lpMsg)
{
	...
	if (!nResult)
	{
		gObj[nTargetIndex].CharacterData->m_bUsePartyMatching = false;
		gObj[nTargetIndex].CharacterData->m_bPartyMatchingLeader = false;
		gObj[nTargetIndex].CharacterData->m_nPartyMatchingIndex = -1;
	}
...
void DGRecvChatMsgPartyMatching(_stAnsChattingPartyMatching *lpMsg)
{
	...
	if (gObj[nTargetIndex].CharacterData->m_bUsePartyMatching == false)
	{
		return;
	}

	if (gObj[nTargetIndex].CharacterData->m_nPartyMatchingIndex != lpMsg->nPartyIndex)
	{
		return;
	}
