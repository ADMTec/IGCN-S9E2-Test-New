...
BOOL CItemSystemFor380::Is380Item(CItem const* pItem) {
	...
	if (GetItemGroup(pItem->m_Type) != ITEMTYPE_380) return FALSE;
	if (this->m_itemOption[pItem->m_Type].IsLoad == FALSE) return FALSE;
	return TRUE;
}

BOOL CItemSystemFor380::Is380OptionItem(CItem const* pItem) {
	if (pItem == NULL) return FALSE;
	if (this->Is380Item(pItem) == FALSE) return FALSE;
...
void CItemSystemFor380::InitEffectValue(ITEMOPTION_FOR380ITEM_EFFECT* pItemEffect) {
	...
	pItemEffect->Damage = 0;
	pItemEffect->EvasionRatingPvP = 0;
	pItemEffect->Defense = 0;
	pItemEffect->MaximumLife = 0;
	pItemEffect->MaximumShield = 0;
	pItemEffect->ShieldRegeneration = 0;
	pItemEffect->ShieldRegenerationBonus = 0;
}

BOOL CItemSystemFor380::ApplyFor380Option(LPOBJ lpObj) {
	ITEMOPTION_FOR380ITEM_EFFECT* pItemEffect = &lpObj->CharacterData->PvPEquipmentMods;
	...
	for (iItemIndex = 0; iItemIndex < INVETORY_WEAR_SIZE; iItemIndex++) {
		...
		if (pItem->IsItem() && pItem->m_IsValidItem && this->Is380OptionItem(pItem) == TRUE) {
	...
	lpObj->AddLife += pItemEffect->MaximumLife;
	lpObj->iAddShield += pItemEffect->MaximumShield;
	return TRUE;
}
...
BOOL CItemSystemFor380::_CalcItemEffectValue(int iItemOptionType, int iItemEffectValue, ITEMOPTION_FOR380ITEM_EFFECT* pItemEffect) {
	if (iItemOptionType == 0) return FALSE;
	BOOL bResult = TRUE;
	...
	case ITEMOPTION_FOR380ITEM_EFFECT_OPADDDAMAGE:
		pItemEffect->Damage += iItemEffectValue;
		break;
	...
	case ITEMOPTION_FOR380ITEM_EFFECT_OPADDDEFENSE:
		pItemEffect->Defense += iItemEffectValue;
		break;

	case ITEMOPTION_FOR380ITEM_EFFECT_OPADDMAXHP:
		pItemEffect->MaximumLife += iItemEffectValue;
		break;

	case ITEMOPTION_FOR380ITEM_EFFECT_OPADDMAXSD:
		pItemEffect->MaximumShield += iItemEffectValue;
		break;

	case ITEMOPTION_FOR380ITEM_EFFECT_OPREFILLON:
		pItemEffect->ShieldRegeneration += 1;
		break;

	case ITEMOPTION_FOR380ITEM_EFFECT_OPADDREFILLSD:
		pItemEffect->ShieldRegenerationBonus += iItemEffectValue;
		break;
...
BOOL CItemSystemFor380::_SetOption(CItem* pItem, BOOL bOption) {
	if (pItem == NULL) return FALSE;
	if (!this->Is380Item(pItem)) return FALSE;
	if (bOption == TRUE) pItem->m_ItemOptionEx |= 0x80;
	else pItem->m_ItemOptionEx &= ~0x80;
...
BOOL CItemSystemFor380::ChaosMix380ItemOption(LPOBJ lpObj) {
	...
	g_Log.Add("[380Item][Item Mix] - Mix Init");
	...
	if (iInvalidItemCount || iValidItemCount != 1 || iJewelOfHarmony != this->m_iNeedJewelOfHarmonyCount || iJewelOfSuho != this->m_iNeedJewelOfSuhoCount || pTargetItem == NULL || iPosOfJewelOfHarmony == -1 || iPosOfJewelOfSuho == -1 || iCharmOfLuckCount > 10) {
	...
	if (iMixPrice < 0) iMixPrice = 0;
	if (lpObj->CharacterData->Money < iMixPrice) {
		pMsg.Result = 2;
		IOCP.DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
		lpObj->ChaosLock = FALSE;
		return FALSE;
	...
	lpObj->CharacterData->Money -= iMixPrice;
	...
	if (pTargetItem->m_Level < 7) iRateSuccess = this->m_iRateSuccessRateForMix1;
	else if (pTargetItem->m_Level < 10)	iRateSuccess = this->m_iRateSuccessRateForMix2;
	else if (pTargetItem->m_Level <= 13) iRateSuccess = this->m_iRateSuccessRateForMix3;
	else iRateSuccess = this->m_iRateSuccessRateForMix4;
	...
	if (iRate <= iRateSuccess) {
		...
		g_Log.Add("[380Item][ItemMix] Mix Success [%s][%s], Money(%d-%d) Rate(%d/%d) Option(%d,%d) OptionValue(%d,%d)",
			lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Money,
		...
		GSProtocol.PROTOCOL_UserChaosBoxSend(lpObj, 0);
		IOCP.DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
		g_Log.Add("[380Item][ItemMix] Mix Fail [%s][%s], Money(%d-%d) Rate(%d/%d)", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Money, iMixPrice, iRate, iRateSuccess);
	}
