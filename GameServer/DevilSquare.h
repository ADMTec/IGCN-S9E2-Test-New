...
public:
	...
	void DS_Init();
	void DS_SetState(enum eDevilSquareState eState);
	...
	UINT64 DS_ExperienceSingle(LPOBJ lpObj, LPOBJ lpTargetObj, int dmg, int tot_dmg);
	void DS_ExperienceParty(LPOBJ lpObj, LPOBJ lpTargetObj, int AttackDamage, int MSBFlag);
	void DS_ScoreDivision(LPOBJ lpMonObj, LPOBJ lpObj, int AttackDamage, int MSBFlag);
	void DS_Load(LPSTR filename);
	void DS_LoadMonster(LPSTR filename);
	void DS_CalcScore();
	void DS_Close();
	int DS_GetSquareIndex(int iGateNumber);
	int GetUserLevelToEnter(int iUserIndex, WORD& btMoveGate);
	BOOL DS_AddUser(BYTE cSquareNumber, int iUserIndex);
	BOOL DS_DelUser(BYTE cSquareNumber, int iUserIndex);
	BOOL DS_ChangeUserIndex(int iOldIndex, int iNewIndex, BYTE cSquareNumber);
	int DS_GetCurrentPlayUserCount(BYTE cSquareNumber);
	BOOL DS_LeaveDevilSquare(BYTE cSquareNumber, int iUserIndex);
	void DS_IncObjCount(BYTE cSquareNumber);
	int DS_GetObjCount(BYTE cSquareNumber);
	...
	DEVILSQUARE_MONSTER_POSITION * DS_GetMonsterPosition(int iPosNum, int iSquareNumber);
	...
	BOOL DS_GetPosition(int TableNum, int iEventLevel, short MapNumber, short & x, short & y);
...
private:
	...
	void DS_ClearDroppedItems();
	void CheckSync();
	void DS_ClearScore();
	void DS_CheckInvalidUser();
