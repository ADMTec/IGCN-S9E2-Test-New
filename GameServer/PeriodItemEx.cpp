...
int CPeriodItemEx::AddPeriodInfo(LPOBJ lpObj)
{
	...
	for ( int i=0; i<g_ConfigRead.server.GetObjectMaxUser(); i++ )
	{
		if ( this->m_PeriodData[i].btUsedInfo == FALSE )
		{
			...
			lpObj->CharacterData->m_iPeriodItemEffectIndex = i;
...
BOOL CPeriodItemEx::RemovePeriodInfo(LPOBJ lpObj)
{
	...
	int PeriodItemIndex = lpObj->CharacterData->m_iPeriodItemEffectIndex;
...
void CPeriodItemEx::SetExpireNotMatchedData(LPOBJ lpObj)
{
	...
	if ( lpObj->CharacterData->m_iPeriodItemEffectIndex == -1 )
	{
		return;
	}
	...
	for (int i=0;i<INVENTORY_SIZE;i++)
	{
			...
			lpObj->pInventory[i].IsPeriodItemExpire() == FALSE )
		{
			...
			for (int j=0;j<30;j++)
			{
				if ( this->m_PeriodData[lpObj->CharacterData->m_iPeriodItemEffectIndex].m_ItemInfo[j].btUsedInfo == TRUE &&
					this->m_PeriodData[lpObj->CharacterData->m_iPeriodItemEffectIndex].m_ItemInfo[j].wItemCode == lpObj->pInventory[i].m_Type &&
					this->m_PeriodData[lpObj->CharacterData->m_iPeriodItemEffectIndex].m_ItemInfo[j].Serial == lpObj->pInventory[i].m_Number )
				{
					bItemFound = TRUE;
				}
			}
...
void CPeriodItemEx::OnRequestPeriodItemList(LPOBJ lpObj)
{
	...
	int PeriodIndex = lpObj->CharacterData->m_iPeriodItemEffectIndex;
