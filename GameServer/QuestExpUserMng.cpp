...
int QuestExpUserMng::IsProgQuestInfo(int iEpisode, int iObjIndex) {
	...
	return gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEpisode].GetAskCnt();
}
...
bool QuestExpUserMng::IsQuestComplete(int iEpisode, int iObjIndex) {
	...
	for (int i = 0; i < iAskCnt; i++) {
		if (gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEpisode].m_UserQuestAskInfo[i].IsComplete() == false) {
			return false;
		}
	}
...
BYTE QuestExpUserMng::QuestExpOnlyInventoryRectCheck(BYTE *btTempInventoryMap, int sx, int sy, int width, int height, int iObjIndex) {
	...
	if (height + sy > 4 * gObj[iObjIndex].CharacterData->m_InventoryExpansion + 8) {
		return -2;
	}
	...
	for (int y = 0; y < height; ++y) {
		for (int x = 0; x < width; ++x) {
			...
			if (btTempInventoryMap[x + sx + (y + sy) * 8] != 0xFF) return -1;
		}
	}
...
int QuestExpUserMng::InvenChk_EnableReward(int iEpisode, int iObjIndex, int *pRandResultIndexID) {
	...
	int iQuestSwitch = gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEpisode].GetQuestSwitch();
	...
	for (; iter != end; ++iter) {
		...
		switch (iQuestType) {
			case QUESTEXP_REWARD_EXP: {
				...
				if (BUFF_TargetHasBuff(&gObj[iObjIndex], BUFFTYPE_SEAL_OF_SUSTENANCE) || BUFF_TargetHasBuff(&gObj[iObjIndex], BUFFTYPE_SEAL_OF_SUSTENANCE_EXP)) iExp = 0;
				
				if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(&gObj[iObjIndex])) {
					gObj[iObjIndex].CharacterData->MasterExperience += iExp;
				} else {
					gObj[iObjIndex].CharacterData->Experience += iExp;
				}

				if (CHARACTER_LevelUp(&gObj[iObjIndex], iExp, 0, Lang.GetText(0,308)) == true) {
					GSProtocol.PROTOCOL_PlayerExperienceSend(iObjIndex, -1, iExp, 0, 0);
				}
			...
			case QUESTEXP_REWARD_ZEN: {
				...
				gObj[iObjIndex].CharacterData->Money += pRewardZen->GetRewardZen();
				GSProtocol.GCMoneySend(iObjIndex, gObj[iObjIndex].CharacterData->Money);
			...
			case QUESTEXP_REWARD_ITEM: {
				...
				for (int i = 0; i < RewardItemCnt; ++i) {
					...
					if (itemtype != ITEMGET(12, 144) && itemtype != ITEMGET(12, 145) && itemtype != ITEMGET(12, 221) && itemtype != ITEMGET(12, 231) && itemtype != ITEMGET(12, 241) && itemtype != ITEMGET(12, 251) && itemtype != ITEMGET(12, 261)) {
						for (int j = 0; j < 5; j++) {
					...
					ItemSerialCreateSend(iObjIndex, 233, gObj[iObjIndex].X, gObj[iObjIndex].Y, pRewardItem->GetRewardItemNum(), pRewardItem->GetRewardItemLevel(), pRewardItem->GetRewardItemDur(), pRewardItem->GetRewardItemSkill(), 0, pRewardItem->GetRewardItemOpt(), iObjIndex, pRewardItem->GetRewardItemExOpt(), 0, 0, btSocketOption, btPentagramMainAttribute);
					...
					g_Log.Add(ItemLogBuff, gObj[iObjIndex].AccountID, gObj[iObjIndex].Name, iEpisode, iQuestSwitch, szRewardItemName, pRewardItem->GetRewardItemNum(), pRewardItem->GetRewardItemLevel(), pRewardItem->GetRewardItemDur(), pRewardItem->GetRewardItemSkill());
				}
			}
			break;
...
void QuestExpUserMng::InitUserQuestAskInfo(int iEpisode, int iObjIndex) {
	...
	gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEpisode].QuestAskInfoClear();
}
...
void QuestExpUserMng::SetQuestSwitch(int iEpisode, int iQuestSwitch, int iObjIndex) {
	...
	if (pUserQuestInfo->SetEpisode(iEpisode) == false) {
		int iUserQuestSwitch = gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEpisode].GetQuestSwitch();
		int iUserEpisode = gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEpisode].GetEpisode();
	...
	if (this->AddUserQuestAskInfo(dwQuestInfoIndexID, iObjIndex) == true) {
		gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEpisode].SetQuestProgState(QUESTEXP_PROG_STATE_ONGOING);
	}
}
...
int QuestExpUserMng::GetQuestSwitch(int iEpisode, int iObjIndex) {
	...
	return gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEpisode].GetQuestSwitch();
}
...
void QuestExpUserMng::UserAllQuestInfoSave(int iObjIndex) {
	...
	while (iEpCnt < MAX_QUESTEXP_EPISODE) {
		UserQuestInfo* pUserQuestInfo = &gObj[iIndex].CharacterData->m_UserQuestInfo[iEpCnt];
		...
		if (iEpisode > 0) {
			...
			for (int i = 0; i < iAskCnt; ++i) {
				...
				g_Log.Add("[QuestExp] AllQuestInfoSave [%s][%s] EP[%d] QS[%d] AskIndex[0x%x] AskValue[%d] AskState[%d]", gObj[iIndex].AccountID, gObj[iIndex].Name, iEpisode, iQuestSwitch, QuestInfo[iQuestCnt].btAskIndex[i], QuestInfo[iQuestCnt].btAskValue[i], QuestInfo[iQuestCnt].btAskState[i]);
			}
...
void QuestExpUserMng::UserQuestInfoSave(DWORD dwQuestInfoIndexID, int iObjIndex) {
	...
	int iAskCnt = gObj[iIndex].CharacterData->m_UserQuestInfo[iEpisode].GetAskCnt();
	WORD wProgState = gObj[iIndex].CharacterData->m_UserQuestInfo[iEpisode].GetQuestProgState();
	QuestInfo.lStartDate = gObj[iIndex].CharacterData->m_UserQuestInfo[iEpisode].GetStartDate();
	QuestInfo.lEndDate = gObj[iIndex].CharacterData->m_UserQuestInfo[iEpisode].GetEndDate();
	...
	for (int i = 0; i < iAskCnt; i++) {
		QuestInfo.btAskIndex[i] = gObj[iIndex].CharacterData->m_UserQuestInfo[iEpisode].m_UserQuestAskInfo[i].GetIndexID();
		QuestInfo.btAskValue[i] = gObj[iIndex].CharacterData->m_UserQuestInfo[iEpisode].m_UserQuestAskInfo[i].GetValue();
		QuestInfo.btAskState[i] = gObj[iIndex].CharacterData->m_UserQuestInfo[iEpisode].m_UserQuestAskInfo[i].IsComplete();

		g_Log.Add("[QuestExp] QuestInfoSave [%s][%s] EP[%d] QS[%d] AskIndex[0x%x] AskValue[%d] AskState[%d]", gObj[iIndex].AccountID, gObj[iIndex].Name, iEpisode, iQuestSwitch, QuestInfo.btAskIndex[i], QuestInfo.btAskValue[i], QuestInfo.btAskState[i]);
	}
...
void QuestExpUserMng::UserQuestInfoLoad(PMSG_ANS_QUESTEXP_INFO* lpRecv) {
	...
	for (int i = 0; i < iQuestCnt; i++) {
		...
		for (int j = 0; j < MAX_QUESTEXP_ASK_INFO; j++) {
			if (QuestInfo[i].btAskIndex[j]) {
				...
				g_Log.Add("[QuestExp] Get DB UserQuestInfo [%s][%s] Ep[%d] QS[%d] QuestType[0x%x] AskIndex[%d] AskValue[%d] State[%d] ProgState[%d]", gObj[iObjIndex].AccountID, gObj[iObjIndex].Name, iEpisode, iQuestSwitch, iQuestType, btAskIndex, btAskValue, btAskState, wProgState);
	...
	if (gObj[iObjIndex].CharacterData->m_bUserQuestInfoSent == false) {
		gObj[iObjIndex].CharacterData->m_bUserQuestInfoSent = true;
		PBMSG_HEAD2 pMsg;
...
UserQuestInfo* QuestExpUserMng::GetUserQuestInfo(int iEpisode, int iObjIndex) {
	...
	return &gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEpisode];
}
...
int QuestExpUserMng::GetQuestProgState(int iEp, int iObjIndex) {
	if (gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEp].GetQuestProgState() == QUESTEXP_PROG_STATE_TIME_LIMIT) {
		time_t end_date = gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEp].GetEndDate();

		if (g_QuestExpProgMng.CheckExpireDate(end_date) == true) {
			gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEp].SetQuestProgState(QUESTEXP_PROG_STATE_NONE);
		}
    }

	return gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEp].GetQuestProgState();
}
...
int QuestExpUserMng::CheckOutOfInventory(int aIndex, int sy, int height) {
	...
	if(sy >= 4 * gObj[aIndex].CharacterData->m_InventoryExpansion + 8) return -1;

    if(sy < 0) return 0;

    if(sy < 8) {
        if(nEndOfItemY >= 8) return 0;
    } else if(sy < 12) {
        if (nEndOfItemY >= 12) return 0;
    } else if(sy < 16) {
        if(nEndOfItemY >= 16) return 0;
    } else if(sy < 20) {
        if(nEndOfItemY >= 20) return 0;
    } else if(sy < 24) {
        if(nEndOfItemY >= 24) return 0;
    } else {
        return 0;
    }
