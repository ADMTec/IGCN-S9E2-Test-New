...
BOOL CMercenary::CreateMercenary(int iIndex, int iMercenaryTypeIndex, BYTE cTX, BYTE cTY)
{
	...
	if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE)
	{
		...
		if (lpObj->CharacterData->GuildStatus != 0x80 && lpObj->CharacterData->GuildStatus != 0x40)
		{
		...
		if (iMonsterIndex >= 0)
		{
			...
			if (lpObj->CharacterData->lpGuild)
			{
				g_Log.Add("[CastleSiege] Mercenary is summoned [%d] - [%d][%d] [%s][%s][%d] - (Guild : %s)",
					...
					lpObj->AccountID, lpObj->Name, lpObj->CharacterData->GuildStatus, lpObj->CharacterData->lpGuild->Name);
			}
			else
			{
				g_Log.Add("[CastleSiege] Mercenary is summoned [%d] - [%d][%d] [%s][%s][%d]",
					iMonsterIndex, iMercenaryTypeIndex, this->m_iMercenaryCount,
					lpObj->AccountID, lpObj->Name, lpObj->CharacterData->GuildStatus);
			}
		}
		else
		{
