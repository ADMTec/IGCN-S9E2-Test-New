...
public:
	...
	{
		GSProtocol.PROTOCOL_NPCChatSend(&gObj[lua_tointeger(Lua, 2)], (char*)lua_tostring(Lua, 1), lua_tointeger(Lua, 3));
		return true;
	};
	...
	{
		BOOL Result = CHARACTER_CalcDistance(&gObj[lua_tointeger(Lua, 1)], &gObj[lua_tointeger(Lua, 2)]);
	...
	{
		BOOL Result = BUFF_AddBuffEffect(&gObj[lua_tointeger(Lua, 1)], lua_tointeger(Lua, 2), lua_tointeger(Lua, 3), lua_tointeger(Lua, 4), lua_tointeger(Lua, 5), lua_tointeger(Lua, 6), lua_tointeger(Lua, 7));
		lua_pushnumber(Lua, Result);
		return Result;
	};
	...
	{
		GSProtocol.PROTOCOL_ReFillSend(lua_tointeger(Lua, 1), lua_tointeger(Lua, 2), lua_tointeger(Lua, 3), lua_tointeger(Lua, 4), lua_tointeger(Lua, 5));
		return true;
	};

	static BOOL		LuaGCManaSend(lua_State * Lua)
	{
		GSProtocol.PROTOCOL_ManaSend(lua_tointeger(Lua, 1), lua_tointeger(Lua, 2), lua_tointeger(Lua, 3), lua_tointeger(Lua, 4), lua_tointeger(Lua, 5));
		return true;
	};
	...
	{
		BOOL Result = CHARACTER_GuildMasterCapacityTest(&gObj[lua_tointeger(Lua, 1)]);
		lua_pushnumber(Lua, Result);
		return Result;
	};
