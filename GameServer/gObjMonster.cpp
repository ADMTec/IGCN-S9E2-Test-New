...
BOOL gObjMonsterRegen(LPOBJ lpObj)
{
	...
	{
		g_DevilSquare.DS_GetPosition(lpObj->m_PosNum, lpObj->m_bDevilSquareIndex, lpObj->MapNumber, lpObj->X, lpObj->Y);
		lpObj->MTX = lpObj->X;
		lpObj->MTY = lpObj->Y;
		lpObj->TX = lpObj->X;
		lpObj->TY = lpObj->Y;
	...
	else if (g_CrywolfSync.CW_OccupationState() == 0 && g_CrywolfSync.GetApplyBenefit() == TRUE)
	{
...
BOOL gObjMonsterMoveCheck(LPOBJ lpObj, int tx, int ty)
{
	...
	{
		DEVILSQUARE_MONSTER_POSITION * lpPos = g_DevilSquare.DS_GetMonsterPosition(lpObj->m_PosNum, lpObj->m_bDevilSquareIndex);
...
BOOL gObjMonsterGetTargetPos(LPOBJ lpObj)
{
	...
	if(lpObj->m_MoveRange == 0 && 
		(BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_ICE_ARROW) == true ||
		 BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_STUN) == true ||
		 BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_SLEEP) == true ||
		 BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_FREEZE) == true ||
		 BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_EARTHEN_PRISON) == true))
	{
		return FALSE;
	}
		...
		if (tObjNum >= 0) {
			if ((gObj[tObjNum].Type == t1 || gObj[tObjNum].Type == t2) && (gObj[tObjNum].Live != FALSE)) {
				if (((gObj[tObjNum].Class >= 100 && gObj[tObjNum].Class < 110) || gObj[tObjNum].Class == 523) || (gObj[tObjNum].Type == OBJ_MONSTER && gObj[tObjNum].m_RecallMon >= 0)) {
					...
					if (g_NewPVP.IsObserver(gObj[tObjNum]) == TRUE) continue;
					if ((gObj[tObjNum].Authority & 2) == 2 || (gObj[tObjNum].Authority & 0x20) == 0x20) {
						if ((gObj[tObjNum].GameMaster & GM_MONSTERS_INVISIBLE) == GM_MONSTERS_INVISIBLE) continue;
					}					
...
void gObjMonsterStateProc(LPOBJ lpObj, int aMsgCode, int aIndex, int aMsgSubCode) {
	if (lpObj->m_iMonsterBattleDelay > 0) return;
	switch (aMsgCode) {
		case 0:
			if (lpObj->m_Attribute == 0) return;			
			if (gObj[aIndex].Live == FALSE || gObj[aIndex].m_State != 2) return;
			if (lpObj->m_ActState.Emotion == 0) {
				lpObj->m_ActState.Emotion = 1;
				lpObj->m_ActState.EmotionCount = 10;
			} else if (lpObj->m_ActState.Emotion == 1) lpObj->m_ActState.EmotionCount = 10;

			if (lpObj->Class >= 504 && lpObj->Class <= 511 && lpObj->MaxLife * 0.3 >= lpObj->Life && BUFF_TargetHasBuff(lpObj, 81) == false) {
				CMagicInf cMagicInf;
				memset(&cMagicInf, 0x00, sizeof(cMagicInf));
				cMagicInf.m_Skill = 218;
				gObjUseSkill.SUM_AmplifyDamage(lpObj->m_Index, &cMagicInf, lpObj->m_Index);
				GSProtocol.PROTOCOL_UseMonsterSkillSend(lpObj, lpObj, 59);
				lpObj->m_ActState.Attack = 0;
				lpObj->m_ActState.Move = 0;
				lpObj->m_ActState.Emotion = 0;
				lpObj->NextActionTime = 300;
			...
			if (lpObj->Class >= 504 && lpObj->Class <= 521) g_ImperialGuardian.SetTargetMoveAllMonster(lpObj->m_nZoneIndex, aIndex);
			if (lpObj->m_ActState.Attack == 0 && lpObj->PathStartEnd == 0) {
				if (ObjectMaxRange(aIndex)) {
					int map = gObj[aIndex].MapNumber;
					BYTE attr;
					int dis = CHARACTER_CalcDistance(lpObj, &gObj[aIndex]);
					int range;

					if (lpObj->m_AttackType >= 100) range = lpObj->m_AttackRange + 2;
					else range = lpObj->m_AttackRange;
					if (dis <= range) {
						if (gObj[aIndex].m_RecallMon >= 0) {
							if (lpObj->m_RecallMon >= 0) {
								if (gObj[aIndex].Type == OBJ_MONSTER) lpObj->TargetNumber = aIndex;
							} else lpObj->TargetNumber = aIndex;
						} else if ((rand() % 100) < 90) {
							if (lpObj->m_RecallMon >= 0) {
								if (gObj[aIndex].Type == OBJ_MONSTER) lpObj->TargetNumber = aIndex;
							} else lpObj->TargetNumber = aIndex;
						...
						if (IMPERIAL_MAP_RANGE(lpObj->MapNumber) == TRUE && lpObj->Class >= 504 && lpObj->Class <= 521) g_ImperialGuardian.SetTargetMoveAllMonster(lpObj->m_nZoneIndex, aIndex);
						else {
							BYTE wall = 0;
							...
							if (wall == 1) {
								...
								if ((attr & 1) != 1) {
									if (lpObj->TargetNumber < 0) lpObj->TargetNumber = aIndex;
								}
							}
						}
					}
					...
					if (lpObj->m_bIsInMonsterHerd != false && lpObj->TargetNumber == aIndex) {
						if (lpObj->m_lpMonsterHerd) lpObj->m_lpMonsterHerd->BeenAttacked (lpObj, &gObj[aIndex]);
					}
				}
			} else {
				if ((rand() % 2) == 1 && lpObj->PathStartEnd == 0) {
					...
					if (ObjectMaxRange(IndexEnemy)) {
						...
						int enemydis = CHARACTER_CalcDistance(lpObj, &gObj[aIndex]);
						int range;
		...
		case 1:
			if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) {
				if (lpObj->m_btCsNpcType) {
					...
					if (lpObj->Class == 287 || lpObj->Class == 286) g_CsNPC_Mercenary.DeleteMercenary(lpObj->m_Index);
					if (lpObj->Class == 278) g_CsNPC_LifeStone.DeleteLifeStone(lpObj->m_Index);
			...
			if (KALIMA_MAP_RANGE(lpObj->MapNumber)) {
				if (lpObj->Class == 161 || lpObj->Class == 181 || lpObj->Class == 189 || lpObj->Class == 197 || lpObj->Class == 267) g_KalimaGate.CreateKalimaGate2(aIndex, lpObj->MapNumber, lpObj->X, lpObj->Y);
			}
			...
			if (lpObj->Class == 673) g_bActiveUrukTrap = false;
			if (lpObj->Class >= 678 && lpObj->Class <= 680) g_Log.Add("[LORDSILVESTER]Kill %s INDEX:%d, X(%d)/Y(%d)", lpObj->Name, lpObj->Class, lpObj->X, lpObj->Y);

			gObjMonsterDieGiveItem(lpObj, &gObj[aIndex]);
			lpObj->NextActionTime = 300;

			if (lpObj->m_RecallMon >= 0) gObjMonsterCallKill(lpObj->m_RecallMon);
			if (lpObj->Class == 409 || lpObj->Class == 410 || lpObj->Class == 411 || lpObj->Class == 412) g_QuestInfo.AddMonsterKillCount(lpObj);
		...
		case 2:
			if (gObj[aIndex].Live != FALSE) {
				if (BC_MAP_RANGE(gObj[aIndex].MapNumber) == FALSE) {
					if (gObj[aIndex].Class != 131 || (((lpObj->Class - 132) < 0) ? FALSE : ((lpObj->Class - 132) > 2) ? FALSE : TRUE) == FALSE) CHARACTER_KnockbackNormal(lpObj, &gObj[aIndex]);
				}
			}
		...
		case 5:			gObjMemFree(lpObj->m_Index);																	break;
		case 6:			if (gObj[aIndex].Live != FALSE) CHARACTER_KnockbackDistance(lpObj, &gObj[aIndex], 2);			break;
		case 7:			if (gObj[aIndex].Live != FALSE) CHARACTER_KnockbackDistance(lpObj, &gObj[aIndex], 3);			break;
		case 55:		gObjAttack(lpObj, &gObj[aIndex], NULL, FALSE, 0, 0, FALSE, 0, 0);								break;
		case 56: {
			LPOBJ lpTargetObj = &gObj[aIndex];

			if (BUFF_TargetHasBuff(lpTargetObj, DEBUFFTYPE_POISON) == FALSE) {
				if (CHARACTER_ResistanceProc(lpTargetObj, 1) == 0) {
					lpTargetObj->lpAttackObj = lpObj;
					BUFF_AddBuffEffect(lpTargetObj, DEBUFFTYPE_POISON, EFFECTTYPE_DOT_POISON, 3, 0, 0, aMsgSubCode);
				}
			}
		}
		break;
		...
		case 57: {
			LPOBJ lpTargetObj = &gObj[aIndex];
			CHARACTER_KnockbackDistance(lpTargetObj, lpObj, aMsgSubCode);
		}
		break;

		case 50: {
			if (lpObj->Class == 533) {
				CMagicInf* lpMagic = PLAYER_GetMagicSearch(lpObj, aMsgSubCode);
...
void gObjMonsterProcess(LPOBJ lpObj)
{
	...
	if (BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_SLEEP) == TRUE)
	{
		return;
	}
	...
	if (lpObj->m_Attribute == 100)
	{
		if (lpObj->m_RecallMon >= 0 && lpObj->m_RecallMon < g_ConfigRead.server.GetObjectMax())
		{
			...
			if (CHARACTER_CalcDistance(lpCallMonObj, lpObj)> 14)
			{
				Success = TRUE;
			}
			...
			if (Success == TRUE)
			{
				PLAYER_TeleportMagicUse(lpObj->m_Index, (BYTE)lpCallMonObj->X+1, lpCallMonObj->Y);
	...
	if (lpObj->m_ActState.Attack == 1)
	{
		...
		if (TMonsterSkillManager::CheckMonsterSkill(lpObj->Class))
		{
			...
			if (rand()%4 == 0 && lpObj->Class != 504)
			{
				...
				GSProtocol.PROTOCOL_ActionSend(lpObj, 120, lpObj->m_Index, iTargetNumber);
...
void gObjMonsterMagicAttack(LPOBJ lpObj, int iMonsterClass)
{
	...
	GSProtocol.PROTOCOL_DurationMagicRecv((LPBYTE)&pDuration, lpObj->m_Index);
		...
		if (lpObj->VpPlayer2[count].state)
		{
			if (lpObj->VpPlayer2[count].type == OBJ_USER)
			{
				...
				if (tObjNum >= 0)
				{
					...
					else if (lpObj->Class == 275 || CHARACTER_CalcDistance(lpObj, &gObj[tObjNum]) < 6)
					{
						pAttack.NumberH = SET_NUMBERH(tObjNum);
						pAttack.NumberL = SET_NUMBERL(tObjNum);
						memcpy(&AttackSendBuff[ASBOfs], &pAttack, sizeof(pAttack));
	...
	if (pCount.Count > 0)
	{
		...
		{
			GSProtocol.PROTOCOL_CheckAttackRecv(AttackSendBuff, lpObj->m_Index, TRUE);
		}
	}
}
...
void gObjUseMonsterSpecialAbillity(LPOBJ lpMonsterObj)
{
	if (lpMonsterObj->Class == 275)	// Kundun
	{
		...
		{
			GSProtocol.PROTOCOL_MagicAttack((LPBYTE)&pAttackMsg, lpMonsterObj->m_Index);
			GSProtocol.PROTOCOL_MagicAttack((LPBYTE)&pAttackMsg, lpMonsterObj->m_Index);
		}
...
void gObjMonsterBeattackRecv(BYTE * lpRecv, int aIndex)
{
	...
	CMagicInf * lpMagic = PLAYER_GetMagic(&gObj[aIndex], MagicNumber);
...
void gObjMonsterAttack(LPOBJ lpObj, LPOBJ lpTargetObj) {
	...
	if (lpObj->Class == 144 || lpObj->Class == 174 || lpObj->Class == 182 || lpObj->Class == 190 || lpObj->Class == 260 || lpObj->Class == 268 || lpObj->Class == 334) {
		if (rand() % 2) {
			...
			GSProtocol.PROTOCOL_MagicAttack((LPBYTE)&pAttackMsg, lpObj->m_Index);
			return;
		}
	...
	if (lpObj->Class == 161 || lpObj->Class == 181 || lpObj->Class == 189 || lpObj->Class == 197 || lpObj->Class == 267 || lpObj->Class == 275) {
		...
		GSProtocol.PROTOCOL_MagicAttack((LPBYTE)&pAttackMsg, lpObj->m_Index);
		if (MagicNumber == 1 || MagicNumber == 2 || MagicNumber == 0) gObjUseMonsterSpecialAbillity(lpObj);	// Kundun
	...
	else if (lpObj->Class >= 533 && lpObj->Class <= 539)
	{
		...
		GSProtocol.PROTOCOL_DurationMagicRecv((LPBYTE)&pAttackMsg, lpObj->m_Index);
	}
			...
			case 0:
				{
					...
					GSProtocol.PROTOCOL_Attack(&pAttackMsg, lpObj->m_Index);
				}
				break;
...
{
	if (BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_ICE_ARROW) == true ||
		BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_STUN) == true ||
		BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_SLEEP) == true ||
		BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_FREEZE) == true ||
		BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_EARTHEN_PRISON) == true)
	...
	if (bPathFound)
	{
		...
		GSProtocol.PROTOCOL_MoveProc(&pMove, lpObj->m_Index);
		return TRUE;
	}
...
void gObjMonsterBaseAct(LPOBJ lpObj) {
	...
	if (lpObj->TargetNumber >= 0) lpTargetObj = &gObj[lpObj->TargetNumber];
	else lpObj->m_ActState.Emotion = 0;
	if (BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_SLEEP) == TRUE) return;
	if (lpObj->m_ActState.Emotion == 0) {
		if (lpObj->m_Attribute) {
			...
			if (lpObj->m_Attribute == 100) actcode1 = 1;			
			if (actcode1 == 0) {
			...
			if (lpObj->Class != 673 || lpObj->VPCount2 >= 1) {
				if (lpObj->m_MoveRange > 0 && !BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_ICE_ARROW) && !BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_STUN) && !BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_SLEEP) && !BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_FREEZE) && !BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_EARTHEN_PRISON)) {
					if (lpObj->m_Attribute != 100) gObjMonsterMoveAction(lpObj);
					else {
						int tx = 0;
						int ty = 0;
				...
				if (lpObj->MaxLife > lpObj->Life) {
					...
					if (lpObj->MaxLife * 0.7 >= lpObj->Life) {
						if (lpObj->MaxLife * 0.5 >= lpObj->Life) {
							if (lpObj->MaxLife * 0.2 < lpObj->Life) lpObj->m_nPhaseLordSilvester = 2;
						} else lpObj->m_nPhaseLordSilvester = 1;
					} else lpObj->m_nPhaseLordSilvester = 0;
					if (lpObj->MaxLife < lpObj->Life) lpObj->Life = lpObj->MaxLife;

					g_Log.Add("[LORDSILVESTER][Status Log] HP:%0.2f RefillHP: %d, MAP:%d, X:%d, Y:%d", lpObj->Life, nAddLife, lpObj->MapNumber, lpObj->X, lpObj->Y);
				}

				if (lpObj->MaxLife == lpObj->Life && BUFF_TargetHasBuff(lpObj, BUFFTYPE_MONSTER_MAGIC_IMMUNE) == TRUE) {
					BUFF_RemoveBuffEffect(lpObj, BUFFTYPE_MONSTER_MAGIC_IMMUNE);
			...
			if (lpObj->Class == 249 || lpObj->Class == 247) {	// Guard
				if (g_ConfigRead.data.common.IsGuardSay) {
					if (!(rand()%g_ConfigRead.data.common.GuardTalkChance)) {
						if (lpObj->m_PK_Count == 0)	GSProtocol.PROTOCOL_ChatSend(lpObj, (char*)g_ConfigRead.data.common.GuardSayText.c_str());
					}
				}
				...
				if (iTargetNumber >= 0) {
					if (gObj[iTargetNumber].Class >= 248) lpObj->TargetNumber = -1;
				}
			} else {
				if (lpObj->m_Attribute == 100) {
					...
					if (IMPERIAL_MAP_RANGE(lpObj->MapNumber) == TRUE) {
						if (lpObj->Live == FALSE || g_ImperialGuardian.IsAttackAbleMonster(iTargetNumber) == false) lpObj->TargetNumber = -1;
					}
					...
					if (g_ArcaBattle.IsArcaBattleServer() == TRUE) {
						if (g_AcheronGuardianEvent.IsPlayStart() == false) {
							if (lpObj->m_RecallMon != -1 && ObjectMaxRange(iTargetNumber) == true) {
								if (g_ArcaBattle.IsEnableAttackObelisk(&gObj[lpObj->m_RecallMon], gObj[iTargetNumber].Class) == FALSE) lpObj->TargetNumber = -1;
							}
						}
					}
				} else lpObj->TargetNumber = gObjMonsterSearchEnemy(lpObj, OBJ_USER);
			}
		...
		if (lpObj->TargetNumber >= 0 && lpObj->PathStartEnd == 0)
		{
			...
			int dis = CHARACTER_CalcDistance(lpObj, lpTargetObj);
			int attackrange;
			...
			if (dis <= attackrange)
			{
				...
				if (MapC[map].CheckWall(lpObj->X, lpObj->Y, gObj[tuser].X, gObj[tuser].Y) == TRUE)
				{
					...
					lpObj->NextActionTime = lpObj->AttackSpeed;
				}
			}
			else
			{
...
void gObjRefillMonsterHP(LPOBJ lpMonsterObj, int iRefillHPSec) {
	...
	if (lpMonsterObj->Class == 275) {
		...
		} else lpMonsterObj->iObjectSecTimer++;

		if (lpMonsterObj->iObjectSecTimer % giKundunHPLogSaveTime == 0) {
			g_Log.Add("[KUNDUN][Status Log] HP:%d RefillHP:%d Map:%d X:%d Y:%d", iMonsterHP,iRefillHP,lpMonsterObj->MapNumber,lpMonsterObj->X,lpMonsterObj->Y);

			if(gObjIsConnected(lpMonsterObj->LastAttackerID)) {
				lpObj = &gObj[lpMonsterObj->LastAttackerID];
				g_Log.Add("[KUNDUN][Attacker Info] [%s][%s] HP:%f MP:%f X:%d Y:%d", lpObj->AccountID,lpObj->Name,lpObj->Life,lpObj->Mana,lpObj->X,lpObj->Y);
			}
		}

		if(!g_CrywolfSync.CW_OccupationState()) {
	...
	if(lpMonsterObj->MaxLife < iMonsterHP) lpMonsterObj->Life = lpMonsterObj->MaxLife;
	else lpMonsterObj->Life = iMonsterHP;
}
...
int gObjMonsterSelectSkillForMedusa(LPOBJ lpObj) {
	...
	for (int i = 0; i < MAX_VIEWPORT; i++) {
		...
		if (nObjNum < 0) break;
		if (gObj[nObjNum].Type == OBJ_USER && gObj[nObjNum].Live == TRUE) nTargetCnt++;

		int iTargetNumber = lpObj->TargetNumber;

		if (ObjectMaxRange(iTargetNumber)) {
			if (CHARACTER_CalcDistance(&gObj[iTargetNumber], &gObj[nObjNum]) <= 3) {
				if (iTargetNumber != nObjNum) nSplashTargetCnt++;
			}
		}
	}
	...
	if (nTargetCnt >= 2) {
		...
		if (nSkillNumber > 4) nSkillNumber = 4;
	} else {
		nSkillNumber = (rand() % 10) + 1;
		
		if (nSkillNumber > 4) nSkillNumber = 1;
	}

	if (nSplashTargetCnt > 0 && nSplashTargetCnt >= nTargetCnt / 2 && ((rand() % 10) < 7)) nSkillNumber = 3;
	return nSkillNumber;
}
...
void gObjMonsterDieRewardItems(LPOBJ lpObj, LPOBJ lpTargetObj, int iCount, int iDropRateCommonItem, int iDropRateExcellentItem, int iDropRateSetItem, BOOL bMustHaveSkill, BOOL bMustHaveLuck, BOOL bMustHaveAdditionalOption) {
	...
	for (int i = 0; i < iCount; i++) {
		...
		if (item_drop) {
			...
			switch (optionc) {
				case 0:				if (option3rand < 4) Option3 = 3;				break;
				case 1:				if (option3rand < 8) Option3 = 2;				break;
				case 2:				if (option3rand < 12) Option3 = 1;				break;
			}

			if (DropItem->m_serial) ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, x, y, type, level, dur, Option1, Option2, Option3, MaxHitUser, NOption, 0, 0, 0, 0);
			else MapC[lpObj->MapNumber].MonsterItemDrop(type, level, dur, x, y, Option1, Option2, Option3, NOption, 0, MaxHitUser, 0, 0, 0, -1, 0);
		}
	}
}
...
void gObjMonsterDieGiveItem(LPOBJ lpObj, LPOBJ lpTargetObj) {
	...
	//if (BUFF_TargetHasBuff(lpTargetObj, BUFFTYPE_HACKTOOL_PENALTY) == TRUE) return;
	if (lpObj->MapNumber == MAP_INDEX_DEVILSQUARE_FINAL) return;

	g_KillCountMng.CheckMonsterKillCount(lpObj->Class, lpTargetObj);
	g_CashShop.AddGPMonster(lpObj);

	if (g_MonsterRegenSystem.MonsterKillCheck(lpTargetObj, lpObj)) return;
	if (g_QuestInfo.MonsterItemDrop(lpObj)) return;
	if (g_NewPVP.IsVulcanusMonster(lpObj->Class)) {
		if (g_NewPVP.DropItem(lpTargetObj, lpObj)) return;
	}

	if (lpObj->MapNumber == MAP_INDEX_ACHERON) {
		if (g_ArcaBattle.DropItem(lpTargetObj, lpObj)) return;
	}
	...
	if (g_BagManager.IsBag(lpTargetObj->m_Index, BAG_MONSTER, lpObj->Class, lpObj->m_Index) == true) {
		...
		if (iMaxHitUser == -1) iMaxHitUser = lpTargetObj->m_Index;
		if (g_BagManager.SearchAndUseBag(iMaxHitUser, BAG_MONSTER, lpObj->Class, lpObj->m_Index) == true) return;
	}

	if (lpObj->Class >= 598 && lpObj->Class <= 602) return;
	if (lpObj->Class >= 627 && lpObj->Class <= 631) {
		int nMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);

		if (nMaxHitUser == -1) nMaxHitUser = lpTargetObj->m_Index;
	...
	if (lpObj->Class >= 504 && lpObj->Class <= 511) return;
	if (DG_MAP_RANGE(lpObj->MapNumber)) return;
	if (lpObj->Class == 523) return;
	if (lpObj->Class >= 523 && lpObj->Class <= 528) return;
	if (g_MineSystem.IsTwinkle(lpObj->Class)) return;
	if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) {
		if (lpObj->m_btCsNpcType != 0) return;
	}

	if (lpObj->Class == 275) { // Kundun
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);

		if (MaxHitUser == -1) MaxHitUser = lpTargetObj->m_Index;

		g_BagManager.SearchAndUseBag(MaxHitUser, BAG_EVENT, EVENTBAG_KUNDUN, lpObj->m_Index);
		g_Log.Add("[Kundun EVENT] Kundun die, Killer [%s][%s]", lpTargetObj->AccountID, lpTargetObj->Name);
		return;
	}

	if (lpObj->Class == 249 || lpTargetObj->Class == 249 || lpObj->Class == 247 || lpTargetObj->Class == 247) return; // Guard
	if (lpObj->m_RecallMon >= 0) return;
	if (lpObj->Class == 131) return;	// Castle Gate
	if (lpObj->Class == 283) return;
	if (lpObj->Class == 288) return;
	if (lpObj->Class == 278) return;
	if (lpObj->Class == 460 || lpObj->Class == 461 || lpObj->Class == 462) return; // Season 4.5 Addon	
	if (BC_STATUE_RANGE(lpObj->Class - 132)) return;	// Blood Castle Statue	
	...
	if (lpObj->m_bIsInMonsterHerd) {
		...
		if (lpMH) {
			if (lpMH->MonsterHerdItemDrop(lpObj)) return;
		}
	}
	...
	if (lpObj->Class == 78 || lpObj->Class == 502 || lpObj->Class == 493) {	// Golden Goblin
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
	...
	if (lpObj->Class == 501) {	// Great Golden Derkon
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
		BYTE x = lpObj->X;
		BYTE y = lpObj->Y;
	...
	if (lpObj->Class == 80 || lpObj->Class == 81 || lpObj->Class == 494) {	// Golden Lizard King
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
	...
	if (lpObj->Class == 82 || lpObj->Class == 83 || lpObj->Class == 495 || lpObj->Class == 496) {	// Golden Tantalos
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
	...
	if (lpObj->Class == 656) {
		...
		for (int iNum = 0; iNum < iWingCount; iNum++) ItemSerialCreateSend(lpTargetObj->m_Index, lpTargetObj->MapNumber, lpTargetObj->X, lpTargetObj->Y, ITEMGET(12, 267), 0, 0, 0, 0, 0, iLootIndex, 7, 0, iWingExpireTime, 0, 0);
		return;
	}
	...
	if (lpObj->Class == 657) {
		...
		for (int iNum = 0; iNum < iWingCount; iNum++) ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, ITEMGET(12, 267), 0, 0, 0, 0, 0, iLootIndex, 7, 0, iWingExpireTime, 0, 0);
		return;
	}

	if (gEventMonsterItemDrop(lpObj, lpTargetObj)) return;
	if (g_CAppointItemDrop.AppointItemDrop(lpTargetObj, lpObj)) return;
	if (g_PentagramSystem.AttributeMonsterItemDrop(lpObj)) return;
	if (SetItemDrop.DropItem(lpObj, lpTargetObj)) return;
	if (ItemDrop.DropItem(lpTargetObj, lpObj)) return;
	...
	if (lpTargetObj->Type == OBJ_USER) {
		if (rand() % 10000 < iExcDropRate) ExtDropPer = TRUE;
	}
	...
	if (lpTargetObj->Type == OBJ_USER) ItemDropPer += gItemDropPer * lpTargetObj->CharacterData->ANCIENT_ItemDropRate / 100;
	if (lpTargetObj->pInventory[8].m_Type == ITEMGET(13, 67)) ItemDropPer += gItemDropPer * 50 / 100;

	ItemDropPer += ItemDropPer * BUFF_GetTotalBuffEffectValue(lpTargetObj, EFFECTTYPE_ITEMDROPRATE) / 100.0f; // 7 - ADD_OPTION_DROP_RATE
	ItemDropPer += g_MapAttr.GetItemDropBonus(lpTargetObj->MapNumber);

	if (lpTargetObj->pInventory[10].m_Type == ITEMGET(13, 134)) ItemDropPer += 20;
	if (lpTargetObj->pInventory[11].m_Type == ITEMGET(13, 134)) ItemDropPer += 20;
	if (ExtDropPer == TRUE) {
		DropItem = g_MonsterItemMng.GetItemEx(lpObj->Level - 25);

		if (!DropItem) item_drop = FALSE;
		else {
			...
			if (foundChangeupitem) ExtDropPer = rand() % 100;

			if (ExtDropPer) {
				if ((rand() % itemrate) < ItemDropPer) item_drop = TRUE;
			}
		}
	} else {
		if ((rand() % itemrate) < ItemDropPer) {
			if (lpTargetObj->MapNumber == MAP_INDEX_VULCAN && lpObj->MapNumber == MAP_INDEX_VULCAN && BUFF_TargetHasBuff(lpTargetObj, BUFFTYPE_GLORYOFGLADIATOR)) DropItem = g_MonsterItemMng.GetItemEx(lpObj->Level + 25);
			else DropItem = g_MonsterItemMng.GetItemEx(lpObj->Level);
			if (!DropItem) item_drop = FALSE;
			else item_drop = TRUE;
		}
	}
	...
	if (item_drop) {
		if (!DropItem->IsItem()) item_drop = FALSE;
		if (DS_MAP_RANGE(lpObj->MapNumber)) {
			if (!IsCanNotItemDtopInDevilSquare(DropItem->m_Type)) {
				if ((rand() % 10)) item_drop = FALSE;
			}
		}

		if (g_SocketOptionSystem.IsEnableSocketItem(DropItem->m_Type) == true) {
			if (g_SocketOptionSystem.IsEnableDropSocketItemMap(lpObj->MapNumber) == false) item_drop = FALSE;
			else {
				if (g_ConfigRead.data.common.SocketExcLimit != 2) ExtDropPer = FALSE;
			}
		}

		if (g_kItemSystemFor380.Is380Item(DropItem) == true) {
			if (g_ConfigRead.data.common.Item380DropMap != -1) {
				if (g_ConfigRead.data.common.Item380DropMap != lpObj->MapNumber) item_drop = FALSE;
			}
		}

		if (DropItem->m_Type == ITEMGET(13, 14) && lpObj->MapNumber != MAP_INDEX_ICARUS) item_drop = FALSE; // Loch Feather
		if (g_CrywolfSync.CW_OccupationState() == TRUE && g_CrywolfSync.CW_ApplyPenalty() == TRUE) {	// Jewels
			if (DropItem->m_Type == ITEMGET(14, 13) || DropItem->m_Type == ITEMGET(14, 14) || DropItem->m_Type == ITEMGET(14, 16) || DropItem->m_Type == ITEMGET(14, 22) || DropItem->m_Type == ITEMGET(12, 15) || DropItem->m_Type == ITEMGET(14, 31)) {
				if ((rand() % 100) > g_CrywolfSync.GetGemDropPenaltiyRate()) item_drop = FALSE;
			}
		}
	}
	...
	if (item_drop) {
		...
		if (ExtDropPer == TRUE) dur = ItemGetDurability(DropItem->m_Type, 0, 1, 0);
		else dur = DropItem->m_Durability;
		...
		switch (optionc) {
			case 0:
				if (option3rand < 4) Option3 = 3;
			break;

			case 1:
				if (option3rand < 8) Option3 = 2;
			break;

			case 2:
				if (option3rand < 12) Option3 = 1;
			break;
		}
		...
		if (type == ITEMGET(12, 15) || type == ITEMGET(14, 13) || type == ITEMGET(14, 14) || type == ITEMGET(14, 31) || type == ITEMGET(14, 16) || type == ITEMGET(14, 42) || type == ITEMGET(12, 35) || (type >= ITEMGET(12, 7) && type <= ITEMGET(12, 14)) || (type >= ITEMGET(12, 16) && type <= ITEMGET(12, 24)) || (type >= ITEMGET(12, 44) && type <= ITEMGET(12, 48)) || (type >= ITEMGET(15, 0) && type <= ITEMGET(16, 0))) {
			Option1 = 0;
			Option2 = 0;
			Option3 = 0;
			NOption = 0;
	...
	} else if (lpObj->MonsterMoneyDrop <= 1) return;
	else if ((rand() % moneyrate) < 10) {
		int x = lpObj->X;
		int y = lpObj->Y;

		if (lpObj->MonsterMoneyDrop >= MAX_ZEN) lpObj->MonsterMoneyDrop = MAX_ZEN;

		double money = lpObj->MonsterMoneyDrop;
		float percent = 100;
		money *= g_ConfigRead.data.common.AddZen;

		if (lpTargetObj->Type == OBJ_USER) {
			if (lpTargetObj->CharacterData->Mastery.iMpsPlusZen > 0.0) money += money * lpTargetObj->CharacterData->Mastery.iMpsPlusZen / 100.0;
		}
		...
		if (MaxHitUser != -1 && gObj[MaxHitUser].Type == OBJ_USER) {
			if (g_ConfigRead.data.common.joinmuDropItemUnderCharacter[gObj[MaxHitUser].Class] == true) MapC[lpObj->MapNumber].MoneyItemDrop(money, gObj[MaxHitUser].X, gObj[MaxHitUser].Y);
			else MapC[lpObj->MapNumber].MoneyItemDrop(money, x, y);
		} else MapC[lpObj->MapNumber].MoneyItemDrop(money, x, y);
	}

	if (lpObj->MonsterMoneyDrop < 1) return;
	if (DS_MAP_RANGE(lpObj->MapNumber)) return;	
	if ((rand() % 400) == 1) {
		for (n = 0; n < 4; n++) {
			int x = lpObj->X - 2;
			int y = lpObj->Y - 2;
			...
			if (MaxHitUser != -1 && gObj[MaxHitUser].Type == OBJ_USER) {
				if (g_ConfigRead.data.common.joinmuDropItemUnderCharacter[gObj[MaxHitUser].Class] == true) MapC[lpObj->MapNumber].MoneyItemDrop(lpObj->MonsterMoneyDrop, gObj[MaxHitUser].X, gObj[MaxHitUser].Y);
				else MapC[lpObj->MapNumber].MoneyItemDrop(lpObj->MonsterMoneyDrop, x, y);
			} else MapC[lpObj->MapNumber].MoneyItemDrop(lpObj->MonsterMoneyDrop, x, y);
		}
	}
	...
	if (gEvent1ItemDropTodayPercent > 0) {
		if ((rand() % gEvent1ItemDropTodayPercent) == 0) {
			if (gEvent1ItemDropTodayCount < gEvent1ItemDropTodayMax) {
				...
				y = lpObj->Y;
				level = 1;
				ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, x, y, type, level, dur, 0, 0, 0, -1, 0, 0, 0, 0, 0);
			}
		}
	}
}
...
void gObjMonsterSummonSkillForLordSilvester(OBJECTSTRUCT *lpObj) {
	...
	BUFF_AddBuffEffect(lpObj, ::BUFFTYPE_MONSTER_MAGIC_IMMUNE, 0, 0, 0, 0, 1209600000);
}
