	...
	__try {
		...
		if (CreateListenSocket() != 0) {
			while (true) {
				...
				nRet = WSARecv(Accept, &gObj[ClientIndex].PerSocketContext->IOContext[0].wsabuf , 1, (unsigned long*)&RecvBytes, &Flags, &gObj[ClientIndex].PerSocketContext->IOContext[0].Overlapped, NULL);
	...
	while (true) {
		...
		if (size <= lpIOContext->nSentBytes) {
			if (xcode == 0xC3) {
				...
				if (ret < 0) {
					g_Log.AddC(TColor::Red, "[%s][Packet-Decrypt BYTE] Error: ret < 0 %x/%x/%x)", gObj[uIndex].CharacterData->Ip_addr,recvbuf[lOfs], recvbuf[lOfs+1], recvbuf[lOfs+2]);
				} else {
					...
					byDec[0] = 0xC1;
					byDec[1] = ret + 2;
					gObj[uIndex].CharacterData->PacketsPerSecond++;

					if(gObj[uIndex].CharacterData->PacketsPerSecond >= g_ConfigRead.data.common.PacketLimit) {
						g_Log.AddC(TColor::Red, "[ANTI-HACK] Packets Per Second: %d / %d", gObj[uIndex].CharacterData->PacketsPerSecond, g_ConfigRead.data.common.PacketLimit);
					...
					if (PacketStream.AddData(byDec, ret + 2) == 0) {
						g_Log.AddC(TColor::Red,  "error-L1 : CStreamPacketEngine Adding Error : ip = %s account:%s name:%s HEAD:%x (%s,%d) State:%d", gObj[uIndex].CharacterData->Ip_addr, gObj[uIndex].AccountID, gObj[uIndex].Name, headcode, __FILE__, __LINE__, gObj[uIndex].Connected);
						return 0;
					}

					if (PacketStream.ExtractPacket(byDec) != 0) {
						g_Log.AddC(TColor::Red,  "error-L1 : CStreamPacketEngine ExtractPacket Error : ip = %s account:%s name:%s HEAD:%x (%s,%d) State:%d", gObj[uIndex].CharacterData->Ip_addr, gObj[uIndex].AccountID, gObj[uIndex].Name, headcode, __FILE__, __LINE__, gObj[uIndex].Connected);
						return 0;
					}

					GSProtocol.PROTOCOL_Core(headcode, byDec, ret, uIndex, 1);
				}
			} else if (xcode == 0xC4) {
				...
				GSProtocol.PROTOCOL_Core(headcode, byDec, size, uIndex, 0);
			}
...
#ifdef EMU_NOCRYPT
	if (lpMsg[0] == 0xC3) lpMsg[0] = 0xC1;
	if (lpMsg[0] == 0xC4) lpMsg[0] = 0xC2;
#endif
	...
	if (WSASend(gObj[aIndex].m_socket, &lpIoCtxt->wsabuf , 1, &SendBytes, 0, &lpIoCtxt->Overlapped, NULL) == -1) {
		if (WSAGetLastError() != WSA_IO_PENDING) {
			...
			if (lpIoCtxt->wsabuf.buf[0] == 0xC1) {
				g_Log.Add("(%d)WSASend(%d) failed with error [%x][%x] %d %s ", __LINE__, aIndex, (BYTE)lpIoCtxt->wsabuf.buf[0], (BYTE)lpIoCtxt->wsabuf.buf[2], WSAGetLastError(), gObj[aIndex].CharacterData->Ip_addr);
			} else if (lpIoCtxt->wsabuf.buf[0] == 0xC2) {
				g_Log.Add("(%d)WSASend(%d) failed with error [%x][%x] %d %s ", __LINE__, aIndex, (BYTE)lpIoCtxt->wsabuf.buf[0], (BYTE)lpIoCtxt->wsabuf.buf[3], WSAGetLastError(), gObj[aIndex].CharacterData->Ip_addr);
			}
...
bool CIOCP::IoSendSecond(_PER_SOCKET_CONTEXT * lpPerSocketContext) {
	...
	if (WSASend(gObj[aIndex].m_socket, &lpIoCtxt->wsabuf, 1, &SendBytes, 0, &lpIoCtxt->Overlapped, NULL) == -1) {
		if (WSAGetLastError() != WSA_IO_PENDING) {
			g_Log.Add("WSASend(%d) failed with error %d %s ", __LINE__, WSAGetLastError(), gObj[aIndex].CharacterData->Ip_addr);
			CloseClient(aIndex);
			LeaveCriticalSection(&criti);
			return false;
		}
