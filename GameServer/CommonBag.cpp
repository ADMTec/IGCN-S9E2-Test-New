...
bool CCommonBag::CheckCondition(int aIndex, int ItemID, int ItemLevel) {
	if (rand() % 10000 >= this->m_BagData.dwBagUseRate) return false;
	if (this->m_BagItemID == ItemID && this->m_BagItemLevel == ItemLevel) return true;
...
bool CCommonBag::UseBag(int aIndex, int iParam2) {
	...
	if (m_ItemSection.btItemDropCount == 1) {
		...
		g_Log.Add("[%s] Drop Item [%s][%s] : (%d)(%d/%d) Item:(%s)%d", this->m_BagData.szBagName, lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, ItemAttribute[ITEMGET(m_Item.btItemType, m_Item.wItemIndex)].Name, ITEMGET(m_Item.btItemType, m_Item.wItemIndex));
		return true;
	}
