...
CMixSystem::CMixSystem() {
	...
	this->SOUL_GEM_BUNDLE_ITEMINDEX = ITEMGET(12, 31);
	...
	this->m_i3rdWingOpt_LuckRate = 30;
}

CMixSystem::~CMixSystem() {}

BOOL CMixSystem::ChaosBoxCheck(LPOBJ lpObj) {
	if (lpObj->pChaosBox == NULL) return false;
	return true;
}

BOOL CMixSystem::ChaosBoxInit(LPOBJ lpObj) {
	if (lpObj->pChaosBox == NULL) return false;
	if (lpObj->pChaosBoxMap == NULL) return false;
	for (int n = 0; n < CHAOS_BOX_SIZE; n++) lpObj->pChaosBox[n].Clear();
...
BOOL CMixSystem::ChaosBoxItemDown(LPOBJ lpObj) {
	...
	for (int n=0;n<CHAOS_BOX_SIZE;n++) {
		if (lpObj->pChaosBox[n].m_Type == ITEMGET(12, 15) || lpObj->pChaosBox[n].m_Type == ITEMGET(14, 13) || lpObj->pChaosBox[n].m_Type == ITEMGET(14, 14) ||
			...
			lpObj->pChaosBox[n].Convert(lpObj->pChaosBox[n].m_Type, lpObj->pChaosBox[n].m_Option1, lpObj->pChaosBox[n].m_Option2, lpObj->pChaosBox[n].m_Option3, lpObj->pChaosBox[n].m_NewOption, lpObj->pChaosBox[n].m_SetOption, lpObj->pChaosBox[n].m_ItemOptionEx, 0, -1, 0, CURRENT_DB_VERSION);
		}
	}
...
int CMixSystem::ChaosBoxMix(LPOBJ lpObj, int & Result2, int & WingNum) {
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
		if (lpObj->pChaosBox[n].IsItem() == TRUE) {
			...
			if (g_LuckyItemManager.IsLuckyItemEquipment(lpObj->pChaosBox[n].m_Type)) {
				return false;
			}
			...
			if (lpObj->pChaosBox[n].m_Level >= MIN_CHAOS_ITEM_LEVEL && (lpObj->pChaosBox[n].m_Option3 *4) >= MIN_CHAOS_ITEM_LEVEL) {
				...
				if (lpObj->pChaosBox[n].m_Type == ITEMGET(2,6) || lpObj->pChaosBox[n].m_Type == ITEMGET(4,6) || lpObj->pChaosBox[n].m_Type == ITEMGET(5,7)) {
					Result2 = 1;
				}
			}
			...
			if (lpObj->pChaosBox[n].m_Type == ITEMGET(12,15)) {
				...
				ChaosDiamond++;
				add = 1;
			} else if (lpObj->pChaosBox[n].m_Type == ITEMGET(14,13) || lpObj->pChaosBox[n].m_Type == ITEMGET(14,14)) {
				value += lpObj->pChaosBox[n].m_OldBuyMoney;
				nv = lpObj->pChaosBox[n].m_OldBuyMoney;
				add = 1;
			}
			...
			//season4.5 add-on
			if (lpObj->pChaosBox[n].m_Type == ITEMGET(13, 83) || lpObj->pChaosBox[n].m_Type == ITEMGET(13, 84) || lpObj->pChaosBox[n].m_Type == ITEMGET(13, 85) || lpObj->pChaosBox[n].m_Type == ITEMGET(13, 86) || lpObj->pChaosBox[n].m_Type == ITEMGET(13, 87)) {
	...
	if (iCharmOfLuckCount > 10) return FALSE;
	...
	g_Log.Add("[%s][%s] CBMix need Zen : %d SuccessRate : %d, CharmRate : %d", lpObj->AccountID, lpObj->Name, lpObj->ChaosMoney, lpObj->ChaosSuccessRate, iCharmOfLuckCount);

	if (WingCharmItemNum > -1) {
...
void CMixSystem::DefaultChaosMix(LPOBJ lpObj) {
	...
	if (lpObj->CharacterData->Money < lpObj->ChaosMoney) {
		...
		if (lpObj->ChaosSuccessRate > 0) {
			...
			if ((rand() % 100) <= (lpObj->ChaosSuccessRate - 1)) {
				...
				if (MixResult2 == TRUE) {
					...
					} else if (ChaosItemType == 1) {
						ChaosItemNum = ITEMGET(4, 6);
					} else if (ChaosItemType == 2) {
		...
		lpObj->CharacterData->Money -= lpObj->ChaosMoney;
		g_CastleSiegeSync.AddTributeMoney(iChaosTaxMoney);
		GSProtocol.GCMoneySend(lpObj->m_Index, lpObj->CharacterData->Money);

		if (fail == TRUE) {
			ChaosBoxItemDown(lpObj);
			GSProtocol.PROTOCOL_UserChaosBoxSend(lpObj, 0);
			IOCP.DataSend(aIndex, (BYTE*)&pMsg, pMsg.h.size);
			g_Log.Add("[%s][%s] CBMix Fail %d Money : %d-%d", lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, lpObj->ChaosMoney);
			lpObj->ChaosLock = FALSE;
		} else {
			g_Log.Add("[%s][%s] CBMix Success Rate:%d Money : %d", lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->ChaosMoney);
		}
	}
...
int CMixSystem::CheckDevilSquareItem(LPOBJ lpObj, int & eventitemcount, int & itemlevel) {
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
		if (lpObj->pChaosBox[n].IsItem() == TRUE) {
			if (lpObj->pChaosBox[n].m_Type == ITEMGET(12,15)) {
				bChaoseGem = TRUE;
			} else if (lpObj->pChaosBox[n].m_Type == ITEMGET(14,17)) {
				eventitemcount +=1;
				bEyeOfDevil = TRUE;
			...
			} else if (lpObj->pChaosBox[n].m_Type == ITEMGET(14,18)) {
				eventitemcount +=1;
				bKeyOfDevil = TRUE;
...
BOOL CMixSystem::DevilSquareEventChaosMix(LPOBJ lpObj, BOOL bCheckType, int iItemLevel) {
	...
	switch (iItemLevel) {
		...
		default:
			g_Log.Add("[DevilSquare] [%s][%s] Invalid DQItem Level [%d]", lpObj->AccountID, lpObj->Name, iItemLevel);
	...
	if ((lpObj->CharacterData->Money - nChaosNeedMoney) < 0) {
		g_Log.Add("[DevilSquare] [%s][%s] CBMix Not Enough Money [%d] need zen [%d]", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Money, nChaosNeedMoney);
		pMsg.Result = CB_NOT_ENOUGH_ZEN;
		IOCP.DataSend(aIndex, (BYTE *)&pMsg, pMsg.h.size);
		lpObj->ChaosLock = FALSE;
		return 1;
	...
	if (g_CrywolfSync.CW_OccupationState() == 0 && g_CrywolfSync.GetApplyBenefit() == TRUE) {	
	...
	lpObj->CharacterData->Money -= nChaosNeedMoney;
	g_CastleSiegeSync.AddTributeMoney(iChaosTaxMoney);
	GSProtocol.GCMoneySend(lpObj->m_Index, lpObj->CharacterData->Money);
	...
	if (fail == TRUE) {
		...
		g_Log.Add("[DevilSquare,%d] [%s][%s] CBMix Fail %d Money : %d-%d", iItemLevel, lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney);
		lpObj->ChaosLock = FALSE;
	} else {
		g_Log.Add("[DevilSquare,%d] [%s][%s] CBMix Success Rate:%d Money : %d", iItemLevel, lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, nChaosNeedMoney);
	}
...
void CMixSystem::LogPlusItemLevelChaosItem(LPOBJ lpObj, int iPlusMixLevel) {
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
		if (lpObj->pChaosBox[n].IsItem() == TRUE) {
			...
			if (iPlusMixLevel == 3) iMixLevel = 1;
			else if (iPlusMixLevel == 4) iMixLevel = 2;
			else if (iPlusMixLevel == 22) iMixLevel = 3;
			else if (iPlusMixLevel == 23) iMixLevel = 4;
			else if (iPlusMixLevel == 49) iMixLevel = 5;
			else if (iPlusMixLevel == 50) iMixLevel = 6;
...
BOOL CMixSystem::PlusItemLevelChaosMix(LPOBJ lpObj, int mixType) {
	...
	if (iCharmOfLuckCount > 10) {
		...
		g_Log.Add("[PlusItemLevel] [%s][%s] CBMix Charm of luck over 10 (%d)", lpObj->AccountID, lpObj->Name, iCharmOfLuckCount);
		pResult.Result = 0xF0;
	...
	if (g_PentagramSystem.IsPentagramItem(lpObj->pChaosBox[PlusItemPos].m_Type) == true) {
		if (iCharmOfLuckCount != 0 || iCharmOfAssembly != 0) {
			PMSG_CHAOSMIXRESULT pResult;
			PHeadSetB((LPBYTE)&pResult, 0x86, sizeof(pResult));
			g_Log.Add("[PlusItemLevel] [%s][%s] CBMix Wrong Charm of luck/Assembly", lpObj->AccountID, lpObj->Name, iCharmOfLuckCount);
			pResult.Result = 0xF0;
			IOCP.DataSend(lpObj->m_Index, (LPBYTE)&pResult, pResult.h.size);
			lpObj->ChaosLock = FALSE;	
			return FALSE;
	...
	if (OtherItemFound != FALSE) return FALSE;
	...
	if (MixType != mixType) MixType = 0;
	if (MixType == 0) return FALSE;
	...
	lpObj->ChaosMoney += iChaosTaxMoney;
	if (lpObj->ChaosMoney < 0) lpObj->ChaosMoney = 0;
	if (lpObj->CharacterData->Money < lpObj->ChaosMoney) {
		g_Log.Add("[PlusItemLevel] [%s][%s] CBMix Not Enough Money [%d] need zen [%d]", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Money, lpObj->ChaosMoney);
		pMsg.Result = CB_NOT_ENOUGH_ZEN;
	...
	if ((lpObj->pChaosBox[PlusItemPos].IsExtItem() == TRUE || lpObj->pChaosBox[PlusItemPos].IsSetItem() == TRUE) && g_SocketOptionSystem.IsSocketItem(&lpObj->pChaosBox[PlusItemPos]) == FALSE) nItemGrade = 1;
	else if (g_kItemSystemFor380.Is380Item(&lpObj->pChaosBox[PlusItemPos]) == TRUE || g_SocketOptionSystem.IsSocketItem(&lpObj->pChaosBox[PlusItemPos]) == TRUE) nItemGrade = 2;
	else if (g_PentagramSystem.IsPentagramItem(&lpObj->pChaosBox[PlusItemPos]) == TRUE) nItemGrade = 3;
	else nItemGrade = 0;
	if ((lpObj->pChaosBox[PlusItemPos].m_Type >= ITEMGET(12,0) && lpObj->pChaosBox[PlusItemPos].m_Type <= ITEMGET(12,6)) || (lpObj->pChaosBox[PlusItemPos].m_Type >= ITEMGET(12,36) && lpObj->pChaosBox[PlusItemPos].m_Type <= ITEMGET(12,43)) || (lpObj->pChaosBox[PlusItemPos].m_Type == ITEMGET(13,30))) nItemGrade = 0;
	if (lpObj->CharacterData->VipType != 0) {
		lpObj->ChaosSuccessRate = g_VipSystem.GetPlusItemMixRate(lpObj, mixType, nItemGrade);
		if (lpObj->pChaosBox[PlusItemPos].m_Option2 != FALSE) lpObj->ChaosSuccessRate += g_VipSystem.GetPlusItemAddLuckRate(lpObj, nItemGrade);
	} else {
		if (lpObj->pChaosBox[PlusItemPos].m_Level == 9) lpObj->ChaosSuccessRate = g_ConfigRead.mix.PlusItem10Rate[nItemGrade];
		else if (lpObj->pChaosBox[PlusItemPos].m_Level == 10) lpObj->ChaosSuccessRate = g_ConfigRead.mix.PlusItem11Rate[nItemGrade];
		else if (lpObj->pChaosBox[PlusItemPos].m_Level == 11) lpObj->ChaosSuccessRate = g_ConfigRead.mix.PlusItem12Rate[nItemGrade];
		else if (lpObj->pChaosBox[PlusItemPos].m_Level == 12) lpObj->ChaosSuccessRate = g_ConfigRead.mix.PlusItem13Rate[nItemGrade];
		else if (lpObj->pChaosBox[PlusItemPos].m_Level == 13) lpObj->ChaosSuccessRate = g_ConfigRead.mix.PlusItem14Rate[nItemGrade];
		else if (lpObj->pChaosBox[PlusItemPos].m_Level == 14) lpObj->ChaosSuccessRate = g_ConfigRead.mix.PlusItem15Rate[nItemGrade];
		if (lpObj->pChaosBox[PlusItemPos].m_Option2 != FALSE) lpObj->ChaosSuccessRate += g_ConfigRead.mix.PlusItemAddLuckRate[nItemGrade];
	}
	...
	if (Result < lpObj->ChaosSuccessRate) {
		...
		if (g_PentagramSystem.IsPentagramItem(&lpObj->pChaosBox[PlusItemPos])) {
			for (int i = 0; i < 5; i++) SocketOption[i] = lpObj->pChaosBox[PlusItemPos].m_SocketOption[i];
			SocketIndex = lpObj->pChaosBox[PlusItemPos].m_BonusSocketOption;
		}

		Item.Convert(lpObj->pChaosBox[PlusItemPos].m_Type, lpObj->pChaosBox[PlusItemPos].m_Option1, lpObj->pChaosBox[PlusItemPos].m_Option2, lpObj->pChaosBox[PlusItemPos].m_Option3, lpObj->pChaosBox[PlusItemPos].m_NewOption, lpObj->pChaosBox[PlusItemPos].m_SetOption, lpObj->pChaosBox[PlusItemPos].m_ItemOptionEx, SocketOption, SocketIndex, 0, CURRENT_DB_VERSION);
		...
		IOCP.DataSend(lpObj->m_Index, (BYTE *)&pMsg, pMsg.h.size);
		if (g_ConfigRead.mix.PlusItemMix15ResultNotice[nItemGrade] != 0 && MixType == 50) GSProtocol.GCServerMsgStringSendAllEx(0, Lang.GetText(0,611), lpObj->Name, ItemAttribute[Item.m_Type].Name);
		g_Log.Add("[PlusItemLevel] [%s][%s] CBMix Success %d / %d Money : %d-%d [%d], CharmRate : %d", lpObj->AccountID, lpObj->Name, Result, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, lpObj->ChaosMoney, Item.m_Level, iCharmOfLuckCount);
	} else if(iCharmOfAssembly == 0 && iElementalCharmOfAssembly == 0) {
		if (g_PentagramSystem.IsPentagramItem(&lpObj->pChaosBox[PlusItemPos]) == true) g_PentagramSystem.DelPentagramJewelInfo(lpObj->m_Index, &lpObj->pChaosBox[PlusItemPos]);
		int FailLevel = lpObj->pChaosBox[PlusItemPos].m_Level+1;
		ChaosBoxInit(lpObj);
		GSProtocol.PROTOCOL_UserChaosBoxSend(lpObj, 0);
		IOCP.DataSend(lpObj->m_Index, (BYTE *)&pMsg, pMsg.h.size);
		g_Log.Add("[PlusItemLevel] [%s][%s] CBMix Fail %d / %d Money : %d-%d [%d], CharmRate : %d", lpObj->AccountID, lpObj->Name, Result, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, lpObj->ChaosMoney, FailLevel, iCharmOfLuckCount);
	} else {
		int FailLevel = lpObj->pChaosBox[PlusItemPos].m_Level+1;
		this->ChaosMixCharmItemUsed(lpObj);
		GSProtocol.PROTOCOL_UserChaosBoxSend(lpObj, 0);
		IOCP.DataSend(lpObj->m_Index, (BYTE *)&pMsg, pMsg.h.size);
		g_Log.Add("[PlusItemLevel] [%s][%s] CBMix Fail, Charm of Chaos Assembly Used, %d / %d Money : %d-%d [%d], CharmRate : %d", lpObj->AccountID, lpObj->Name, Result, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, lpObj->ChaosMoney, FailLevel, iCharmOfLuckCount);
	}
...
BOOL CMixSystem::PegasiaChaosMix(LPOBJ lpObj) {
	...
	if (lpObj->CharacterData->Money < nChaosNeedMoney) {
		lpObj->ChaosLock = FALSE;
		pMsg.Result = CB_NOT_ENOUGH_ZEN;
	...
	if ((rand()%100) < lpObj->ChaosSuccessRate) {
		...
		g_Log.Add("[DinorantMix] [%s][%s] CBMix Success %d Money : %d-%d, CharmRate : %d", lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney, iCharmOfLuckCount);

		return TRUE;
	} else {
		ChaosBoxInit(lpObj);	// Erase Chaos Box
		GSProtocol.PROTOCOL_UserChaosBoxSend(lpObj, 0);
		IOCP.DataSend(lpObj->m_Index, (BYTE *)&pMsg, pMsg.h.size);
		g_Log.Add("[DinorantMix] [%s][%s] CBMix Fail %d Money : %d-%d, CharmRate : %d", lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney, iCharmOfLuckCount);
		lpObj->ChaosLock = FALSE;
		return FALSE;
	}
}
...
BOOL CMixSystem::CircleChaosMix(LPOBJ lpObj) {
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
		if (lpObj->pChaosBox[n].IsItem() == TRUE) {
			if (lpObj->pChaosBox[n].m_Type == ITEMGET(14,22)) {
				CreatureGemCount++;
			} else if (lpObj->pChaosBox[n].m_Type == ITEMGET(12,15)) {
				ChoasGemCount++;
			} else if (lpObj->pChaosBox[n].m_Type == ITEMGET(14,53)) {
	...
	if (CreatureGemCount != 1 || ChoasGemCount != 1) return FALSE;
	...
	if (nChaosNeedMoney < 0) nChaosNeedMoney = 0;
	if (lpObj->CharacterData->Money < nChaosNeedMoney) {
		lpObj->ChaosLock = FALSE;
		pMsg.Result = CB_NOT_ENOUGH_ZEN;		
		IOCP.DataSend(lpObj->m_Index, (BYTE *)&pMsg, pMsg.h.size);
	...
	if ((rand() % 100) < lpObj->ChaosSuccessRate) {
		...
		::gObjInventoryCommit(lpObj->m_Index);
		g_Log.Add("[CircleMix] [%s][%s] CBMix Success %d Money : %d-%d, CharmRate : %d", lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney, iCharmOfLuckCount);
		return TRUE;
	} else {
		ChaosBoxInit(lpObj);
		GSProtocol.PROTOCOL_UserChaosBoxSend(lpObj, 0);
		IOCP.DataSend(lpObj->m_Index, (BYTE *)&pMsg, pMsg.h.size);
		g_Log.Add("[CircleMix] [%s][%s] CBMix Fail %d Money : %d-%d, CharmRate : %d", lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney, iCharmOfLuckCount);
		lpObj->ChaosLock = FALSE;
		return FALSE;
	}
}
...
BOOL CMixSystem::WingChaosMix(LPOBJ lpObj) {
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
		if (lpObj->pChaosBox[n].IsItem() == TRUE) {
			...
			if ((lpObj->pChaosBox[n].m_Type >= ITEMGET(12,0) && lpObj->pChaosBox[n].m_Type <= ITEMGET(12,2)) || lpObj->pChaosBox[n].m_Type == ITEMGET(12,41)) 
			{
				...
				iCharmOfLuckCount += lpObj->pChaosBox[n].m_Durability;
			} else if (lpObj->pChaosBox[n].m_Type == ITEMGET(13, 88) || lpObj->pChaosBox[n].m_Type == ITEMGET(13, 89) || lpObj->pChaosBox[n].m_Type == ITEMGET(13, 90) || lpObj->pChaosBox[n].m_Type == ITEMGET(13, 91) || lpObj->pChaosBox[n].m_Type == ITEMGET(13, 92)) {
				WingCharmItemNum = lpObj->pChaosBox[n].m_Type;
	...
	if ((rand() % 100) < lpObj->ChaosSuccessRate) {
		...
		int iRandomValue2 = rand() % 10;

		if (iRandomValue2 < 4) {
			if (iRandomValue < 60) {
				iOption2 = 1;	// +4
			} else if (iRandomValue < 40) {
				iOption2 = 2;	// +8
			} else if (iRandomValue < 20) {
				iOption2 = 3;	// +12;
			}
		...
		::gObjInventoryCommit(lpObj->m_Index);
		::g_Log.Add("[WingMix,2] [%s][%s] CBMix Success %d Money : %d-%d, CharmRate : %d", lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney, iCharmOfLuckCount);
		return TRUE;
	} else {
		for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
			lpObj->pChaosBox[n].Clear();
		...
		GSProtocol.PROTOCOL_UserChaosBoxSend(lpObj, 0);
		IOCP.DataSend(lpObj->m_Index, (BYTE *)&pMsg, pMsg.h.size);
		::g_Log.Add("[WingMix,2] [%s][%s] CBMix Fail %d Money : %d-%d, CharmRate : %d", lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney, iCharmOfLuckCount);		
		lpObj->ChaosLock = FALSE;
		return FALSE;
	}
}
...
BOOL CMixSystem::IllusionTempleItemChaosMix(LPOBJ lpObj) {
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
		if (lpObj->pChaosBox[n].IsItem() == TRUE) {
			if (lpObj->pChaosBox[n].m_Type == ITEMGET(12,15)) {
				ChoasGemCount++;
			} else if (lpObj->pChaosBox[n].m_Type == ITEMGET(13,49)) {
				OldScrollCount++;
	...
	if ((rand()%100) < lpObj->ChaosSuccessRate) {
		...
		::gObjInventoryCommit(lpObj->m_Index);
		::g_Log.Add("[IllusionTemple Ticket,%d] [%s][%s] CBMix Success %d Money : %d-%d, CharmRate : %d", OldScrollLevel,lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney, iCharmOfLuckCount);
		return TRUE;
	} else {
		for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
			lpObj->pChaosBox[n].Clear();
		...
		GSProtocol.PROTOCOL_UserChaosBoxSend(lpObj, 0);
		IOCP.DataSend(lpObj->m_Index, (BYTE *)&pMsg, pMsg.h.size);
		::g_Log.Add("[IllusionTemple Ticket,%d] [%s][%s] CBMix Fail %d Money : %d-%d, CharmRate : %d", OldScrollLevel,lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney, iCharmOfLuckCount);
...
BOOL CMixSystem::CheckWingItem(int iItemCode) {
	if (iItemCode == ITEMGET(12, 0) || iItemCode == ITEMGET(12, 1) || iItemCode == ITEMGET(12, 2) || iItemCode == ITEMGET(12, 41)) {
		return true;
	}
...
BOOL CMixSystem::Check2ndWingItem(int iItemCode) {
	if (iItemCode == ITEMGET(12, 3) || iItemCode == ITEMGET(12, 4) || iItemCode == ITEMGET(12, 5) || iItemCode == ITEMGET(12, 6) || iItemCode == ITEMGET(13, 30) || iItemCode == ITEMGET(12, 42) || iItemCode == ITEMGET(12, 49)) {
		return true;
	}
...
BOOL CMixSystem::Check3rdWingItem(int iItemCode) {
	if (iItemCode == this->WING_OF_STORM_ITEMINDEX || iItemCode == this->WING_OF_ETERNAL_ITEMINDEX || iItemCode == this->WING_OF_ILLUSION_ITEMINDEX || iItemCode == this->WING_OF_RUIN_ITEMINDEX || iItemCode == this->CAPE_OF_EMPEROR_ITEMINDEX || iItemCode == this->WING_OF_DIMENSION_ITEMINDEX || iItemCode == this->CAPE_OF_OVERRULE_ITEMINDEX) {
		return true;
	}
...
BOOL CMixSystem::CheckLevelCondition(CItem * lpItem, short Level, BYTE Op1, BYTE Op2, BYTE Op3, BYTE SetOption, BYTE NewOption) {
	...
	if (Level != FALSE) { 
		if (lpItem->m_Level < Level) {
			return FALSE; 
		} 
	
	}
	if (Op1 != FALSE) {
		if (lpItem->m_Option1 < Op1) {
			return FALSE;
		} 
	}

	if (Op2 != FALSE) {
		if (lpItem->m_Option2 < Op2){ 
			return FALSE; 
		} 
	}

	if (Op3 != FALSE) { 
		if (lpItem->m_Option3 < Op3) {
			return FALSE; 
		}
	}
...
BOOL CMixSystem::ThirdWingLevel1ChaosMix(LPOBJ lpObj) {
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
		if (lpObj->pChaosBox[n].IsItem() == TRUE) {
			if (this->Check2ndWingItem(lpObj->pChaosBox[n].m_Type) != FALSE) {
				...
				ChoasGemCount++;
			} else if (lpObj->pChaosBox[n].m_Type == this->SOUL_GEM_BUNDLE_ITEMINDEX) {
				BundleOfSoulCount++;
	...
	if (lpObj->ChaosSuccessRate < 1) lpObj->ChaosSuccessRate = 1;
	...
	if ((rand()%100) < lpObj->ChaosSuccessRate) {
		...
		::gObjInventoryCommit(lpObj->m_Index);
		::g_Log.Add("[ThirdWing Mix][Level 01] [%s][%s] CBMix Success %d Money : %d-%d, CharmRate : %d", lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney, iCharmOfLuckCount);
		return TRUE;
	} else {
		this->ThirdWingMixFail(lpObj);
		GSProtocol.PROTOCOL_UserChaosBoxSend(lpObj, 0);
		IOCP.DataSend(lpObj->m_Index, (BYTE *)&pMsg, pMsg.h.size);
		::g_Log.Add("[ThirdWing Mix][Level 01] [%s][%s] CBMix Fail %d Money : %d-%d, CharmRate : %d", lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney, iCharmOfLuckCount);
		lpObj->ChaosLock = FALSE;
...
BOOL CMixSystem::ThirdWingLevel2ChaosMix(LPOBJ lpObj) {
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
		if (lpObj->pChaosBox[n].IsItem() == TRUE) {
			if (lpObj->pChaosBox[n].m_Type == this->CHAOS_GEM_ITEMINDEX) ChoasGemCount++;
			else if (lpObj->pChaosBox[n].m_Type == this->SOUL_GEM_BUNDLE_ITEMINDEX) BundleOfSoulCount++;
			else if (lpObj->pChaosBox[n].m_Type == this->BLESS_GEM_BUNDLE_ITEMINDEX) BundleOfBlessCount++;
			else if (lpObj->pChaosBox[n].m_Type == this->CREATURE_GEM_ITEMINDEX) JewelOfCreationCount++;
			else if (lpObj->pChaosBox[n].m_Type == this->CONDOR_FLAME_ITEMINDEX) CondorFlameCount++;
			else if (lpObj->pChaosBox[n].m_Type == this->CONDOR_FEATHER_ITEMINDEX) CondorFeatherCount++;
			else if (lpObj->pChaosBox[n].m_Type == ITEMGET(14, 53)) iCharmOfLuckCount += lpObj->pChaosBox[n].m_Durability;
	...
	lpObj->ChaosSuccessRate = 0;
	lpObj->ChaosSuccessRate += iChaosMoney / 3000000;

	if (lpObj->ChaosSuccessRate < 1) lpObj->ChaosSuccessRate = 1;
	if (lpObj->CharacterData->VipType != 0) {
		if (lpObj->ChaosSuccessRate > g_VipSystem.GetWing3rdRate(lpObj)) lpObj->ChaosSuccessRate = g_VipSystem.GetWing3rdRate(lpObj);
	} else {
		if (lpObj->ChaosSuccessRate > g_ConfigRead.mix.Wing3MaxRate) lpObj->ChaosSuccessRate = g_ConfigRead.mix.Wing3MaxRate;
	}
	...
	if ((rand() % 100) < lpObj->ChaosSuccessRate) {
		int iWingType = rand() % 7;
		int iWingNum = 0;

		if (iWingType == 0) iWingNum = this->WING_OF_STORM_ITEMINDEX;
		else if (iWingType == 1) iWingNum = this->WING_OF_ETERNAL_ITEMINDEX;
		else if (iWingType == 2) iWingNum = this->WING_OF_ILLUSION_ITEMINDEX;
		else if (iWingType == 3) iWingNum = this->WING_OF_RUIN_ITEMINDEX;
		else if (iWingType == 4) iWingNum = this->CAPE_OF_EMPEROR_ITEMINDEX;
		else if (iWingType == 5) iWingNum = this->WING_OF_DIMENSION_ITEMINDEX;
		else if (iWingType == 6) iWingNum = this->CAPE_OF_OVERRULE_ITEMINDEX;

		int iOption1 = 0;

		if ((rand() % 100) <= m_i3rdWingOpt_LuckRate) iOption1 = 1;
		...
		switch (iOptTypeRand) {
			case 0:
				iOptRateRand = rand() % 100;

				if (iOptRateRand < 40) iExcOpt |= 0x10;
			break;

			case 1:
				iOptRateRand = rand() % 100;

				if (iOptRateRand < 30) iExcOpt |= 0x20;
			break;
		}

		int iOpt3TypeLevel = rand() % 10;
		int iOpt3TypeLevelRate = rand() % 100;

		if (iOpt3TypeLevel > 5) {
			if (iOpt3TypeLevelRate > 20) iOption2 = 1;
			if (iOpt3TypeLevelRate > 40) iOption2 = 2;
			if (iOpt3TypeLevelRate > 60) iOption2 = 3;
		}

		int iRandOpt = rand() % 10;
		int iRandOptRate = rand() % 100;

		if (iRandOpt > 3) {
			if (iRandOptRate < 80) iExcOpt |= 8;
			else if (iRandOptRate < 60) iExcOpt |= 4;
			else if (iRandOptRate < 40) iExcOpt |= 2;
			else if (iRandOptRate < 20) iExcOpt |= 1;
		}

		::ItemSerialCreateSend(lpObj->m_Index, -1, 0, 0, iWingNum, 0, 0, 0, iOption1, iOption2, -1, iExcOpt, 0, 0, 0, 0);
		::gObjInventoryCommit(lpObj->m_Index);
		::g_Log.Add("[ThirdWing Mix][Level 02] [%s][%s] CBMix Success %d Money : %d-%d, CharmRate : %d", lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney, iCharmOfLuckCount);
		...
		GSProtocol.PROTOCOL_UserChaosBoxSend(lpObj, 0);
		IOCP.DataSend(lpObj->m_Index, (BYTE *)&pMsg, pMsg.h.size);
		::g_Log.Add("[ThirdWing Mix][Level 02] [%s][%s] CBMix Fail %d Money : %d-%d, CharmRate : %d", lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney, iCharmOfLuckCount);
		lpObj->ChaosLock = FALSE;
...
void CMixSystem::ThirdWingMixFail(LPOBJ lpObj) {
	if (lpObj->pChaosBox == NULL) return;
	for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
		if (lpObj->pChaosBox[n].IsItem() == TRUE) {
			if (Check2ndWingItem(lpObj->pChaosBox[n].m_Type) != FALSE) ThirdWingMixFailItemPanalty(&lpObj->pChaosBox[n]);
			else if (CheckLevelCondition(&lpObj->pChaosBox[n], 0, 0, 0, 0, 1, 0) != FALSE) ThirdWingMixFailItemPanalty(&lpObj->pChaosBox[n]);
			else if (CheckLevelCondition(&lpObj->pChaosBox[n], 0, 0, 0, 0, 0, 1) != FALSE) ThirdWingMixFailItemPanalty(&lpObj->pChaosBox[n]);
			else lpObj->pChaosBox[n].Clear();
		}
	}
}
...
void CMixSystem::ThirdWingMixFailItemPanalty(CItem * lpItem) {
	...
	if ((rand() % 2) < 1) lpItem->m_Level -= 2;
	else lpItem->m_Level -= 3;
...
void CMixSystem::SetItemChaosMix(LPOBJ lpObj) {
	...
	if ((rand()%100) < lpObj->ChaosSuccessRate) {
		...
		float Dur = ItemGetDurability(lpObj->pChaosBox[MixSetItemIndex].m_Type, lpObj->pChaosBox[MixSetItemIndex].m_Level, lpObj->pChaosBox[MixSetItemIndex].IsExtItem(), gSetItemOption.IsSetItem(lpObj->pChaosBox[MixSetItemIndex].m_Type));
		...
		gObjChaosItemSet(lpObj->m_Index, 0, 1);
		IOCP.DataSend(lpObj->m_Index, (BYTE *)&pMsg, pMsg.h.size);
		g_Log.Add("[SetItemMix] [%s][%s] CBMix Success %d Money : %d-%d (SetName:%s)", lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, lpObj->ChaosMoney, gSetItemOption.GetSetOptionName(lpObj->pChaosBox[MixSetItemIndex].m_Type, setoption));
	} else {
		ChaosBoxInit(lpObj);
		GSProtocol.PROTOCOL_UserChaosBoxSend(lpObj, 0);
		IOCP.DataSend(lpObj->m_Index, (BYTE *)&pMsg, pMsg.h.size);
		g_Log.Add("[SetItemMix] [%s][%s] CBMix Fail %d Money : %d-%d", lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney);
...
void CMixSystem::DarkHorseChaosMix(LPOBJ lpObj)
{
	...
	if ((rand()%100) < lpObj->ChaosSuccessRate)
	{
		...
		g_Log.Add("[DarkHorseMix] [%s][%s] CBMix Success %d Money : %d-%d, CharmRate : %d",
			...
			lpObj->CharacterData->Money, nChaosNeedMoney, iCharmOfLuckCount);
	}
	else
	{
		ChaosBoxInit(lpObj);
		GSProtocol.PROTOCOL_UserChaosBoxSend(lpObj, 1);
		IOCP.DataSend(lpObj->m_Index, (UCHAR*)&pMsg, pMsg.h.size);
...
void CMixSystem::CastleSpecialItemMix(LPOBJ lpObj) {
	...
	if(g_CastleSiegeSync.CheckCastleOwnerMember(lpObj->m_Index)) {
		if(lpObj->CharacterData->GuildStatus != GUILD_MASTER) {
			g_Log.Add("[CastleSpecialMix] [%s][%s] Item Mix Failed - No Auth", lpObj->AccountID, lpObj->Name);
			IOCP.DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
			lpObj->ChaosLock = FALSE;
			return;
		}
	...
	if(g_ConfigRead.server.GetServerType() != SERVER_CASTLE) {
		g_Log.Add("[CastleSpecialMix] [%s][%s] Item Mix Failed - Wrong Server", lpObj->AccountID, lpObj->Name);
		IOCP.DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
		lpObj->ChaosLock = FALSE;
		return;
	}
	...
	if(g_CastleSiege.GetLordMixLimit() <= 0) {
		g_Log.Add("[CastleSpecialMix] [%s][%s] Item Mix Failed - Mix Count Limit Over", lpObj->AccountID, lpObj->Name);
		IOCP.DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
		lpObj->ChaosLock = FALSE;
		return;		
	}
	...
	if(iBlessGemMixCount != 3 || iSoulGemMixCount != 3 || iDefendGemCount != 30 || iInvalidItemCount > 0) {
		g_Log.Add("[CastleSpecialMix] [%s][%s] Item Mix Failed - Item Error (DG:%d, BGx10:%d, SGx10:%d, Other:%d)", lpObj->AccountID, lpObj->Name, iBlessGemMixCount, iSoulGemMixCount, iDefendGemCount, iInvalidItemCount);
		IOCP.DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
		lpObj->ChaosLock = FALSE;
		return;	
	}
	...
	if(lpObj->CharacterData->Money < nChaosNeedMoney) {
		g_Log.Add("[CastleSpecialMix] [%s][%s] Item Mix Failed - Lack of Money (%d/%d)", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Money, nChaosNeedMoney);
	...
	if((rand()%100) < lpObj->ChaosSuccessRate) {
		...
		gObjInventoryCommit(lpObj->m_Index);		
		g_Log.Add("[CastleSpecialMix] [%s][%s] CBMix Success %d Money : %d-%d",	lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney);
		g_CastleSiege.DecreaseLordMixLimit();
	} else {
		ChaosBoxInit(lpObj);
		GSProtocol.PROTOCOL_UserChaosBoxSend(lpObj, 1);
		IOCP.DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
		g_Log.Add("[CastleSpecialMix] [%s][%s] CBMix Fail %d Money : %d-%d", lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney);
	}
...
void CMixSystem::Fenrir_01Level_Mix(LPOBJ lpObj)
{
	...
	if ((rand()%100) < lpObj->ChaosSuccessRate)
	{
		...
		g_Log.Add("[Fenrir Mix][Level 01] [%s][%s] CBMix Success %d Money : %d-%d, CharmRate : %d",
			lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney,
			iCharmOfLuckCount);
...
void CMixSystem::ShieldPotionLv1_Mix(LPOBJ lpObj)
{
	...
	if (lpObj->CharacterData->Money < iChaosMixPrice)
	{
		pMsg.Result = CB_NOT_ENOUGH_ZEN;
	...
	lpObj->CharacterData->Money -= iChaosMixPrice;
	g_CastleSiegeSync.AddTributeMoney(iChaosTaxMoney);
	GSProtocol.GCMoneySend(lpObj->m_Index, lpObj->CharacterData->Money);
	...
	if (iRate < g_iShieldPotionLv1MixSuccessRate)
	{
		...
		g_Log.Add("[ChaosMix][Shield Potion] Lv1 Potion Mix Success [%s][%s], Money(%d-%d), CharmRate : %d",
			lpObj->AccountID, lpObj->Name, 	lpObj->CharacterData->Money, iChaosMixPrice, iCharmOfLuckCount);
	}
	else
	{
		ChaosBoxInit(lpObj);
		...
		g_Log.Add("[ChaosMix][Shield Potion] Lv1 Potion Mix [%s][%s] CBMix Fail %d Money : %d-%d, CharmRate : %d",
			...
			lpObj->CharacterData->Money, iChaosMixPrice, iCharmOfLuckCount);
	}
...
void CMixSystem::SeedExtractMix(LPOBJ lpObj) {
	...
	if (lpObj->CharacterData->Money < iMixMoney) {
		pMsg.Result = CB_NOT_ENOUGH_ZEN;
		IOCP.DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
		lpObj->ChaosLock = FALSE;
		return;
	...
	lpObj->CharacterData->Money -= iMixMoney;
	g_CastleSiegeSync.AddTributeMoney(iChaosTaxMoney);
	GSProtocol.GCMoneySend(lpObj->m_Index, lpObj->CharacterData->Money);
	iSeedExtractRate = 80 + (iChaosMoney / 2000000);

	if (iSeedExtractRate > 90) iSeedExtractRate = 90;
	...
	if (lpSeedData != NULL) {
		...
		gObjInventoryCommit(lpObj->m_Index);
		g_Log.Add("[%s][%s] Seed Extract Mix Success - SeedInfo (Index : %d, Type : %d, Level : %d, ItemCode : %d)", lpObj->AccountID, lpObj->Name, lpSeedData->m_SeedIndex, lpSeedData->m_SeedType, lpSeedData->m_ItemLevel, lpSeedData->m_ItemCode);
	} else {
		lpObj->pChaosBox[iSetItemNum].m_Level -= rand() % 3;
		lpObj->pChaosBox[iExcItemNum].m_Level -= rand() % 3;
		lpObj->pChaosBox[iHarmonyItemNum].Clear();
...
void CMixSystem::SeedSphereCompositeMix(LPOBJ lpObj) {
	...
	for (int n = 0; n<CHAOS_BOX_SIZE; n++) {
		if (lpObj->pChaosBox[n].IsItem()) {
			...
			} else if (lpObj->pChaosBox[n].m_Type == ITEMGET(12, 15)) iChoasGemCount++;
			else if (lpObj->pChaosBox[n].m_Type == ITEMGET(14, 22)) iCreationGemCount++;
			else if (lpObj->pChaosBox[n].m_Type >= ITEMGET(12,112) && lpObj->pChaosBox[n].m_Type <= ITEMGET(12,117)) {
				iSeedSpear3rdCount++;
				lpSeedItem = &lpObj->pChaosBox[n];
			} else if (lpObj->pChaosBox[n].m_Type == ITEMGET(14,245)) iTetraGoldCount++;
			else if (lpObj->pChaosBox[n].m_Type == ITEMGET(14,246)) iTetraSilverCount++;
			else if (lpObj->pChaosBox[n].m_Type == ITEMGET(14,247)) iTetraBronzeCount++;
			else if (lpObj->pChaosBox[n].m_Type == ITEMGET(12,141) && lpObj->pChaosBox[n].m_Level == 0) iChaosJewel10BunchCount++;
			else if (lpObj->pChaosBox[n].m_Type == ITEMGET(12,137) && lpObj->pChaosBox[n].m_Level == 0) iCreationJewel10BunchCount++;
			else iInvalidItemCount++;
		}
	}
	...
	if (bMakeSeedSpear4th == TRUE) {
		if (iTetraGoldCount == 1) iSeedSphereCompositeRate = (iChaosMoney / 200000) + 80;
		else if (iTetraSilverCount == 1) iSeedSphereCompositeRate = (iChaosMoney / 200000) + 65;
		else if (iTetraBronzeCount == 1) iSeedSphereCompositeRate = (iChaosMoney / 200000) + 50;
		else iSeedSphereCompositeRate = (iChaosMoney / 200000) + 20;
	} else iSeedSphereCompositeRate = 80 + (iChaosMoney / 200000);

	lpObj->CharacterData->Money -= iMixMoney;
	g_CastleSiegeSync.AddTributeMoney(iChaosTaxMoney);
	GSProtocol.GCMoneySend(lpObj->m_Index, lpObj->CharacterData->Money);

	if (rand() % 100 > iSeedSphereCompositeRate) {
		...
		if (bMakeSeedSpear4th == TRUE) OptionIndex = g_SocketOptionSystem.GetSeedIndexForSeedSpearItemCode(lpSeedItem->m_Type, lpSeedItem->m_Level);
		else OptionIndex = g_SocketOptionSystem.GetSeedIndex(lpSeedItem->m_Type, lpSeedItem->m_Level);
		...
		if (SeedSphereItem.m_ItemCode != 0) {
			...
			gObjInventoryCommit(lpObj->m_Index);
			g_Log.Add("[%s][%s] SeedSphere Composite Mix Success - SeedSphere Info (Index : %d, Type : %d, Level : %d, ItemCode : %d)", lpObj->AccountID, lpObj->Name, SeedSphereItem.m_SeedSphereIndex, SeedSphereItem.m_SeedType, SeedSphereItem.m_SphereLevel, SeedSphereItem.m_ItemCode);
			return;
		}
...
void CMixSystem::SetSeedSphereMix(LPOBJ lpObj, BYTE btPos) {
	...
	Item.Convert(pSocketItem->m_Type, pSocketItem->m_Option1, pSocketItem->m_Option2, pSocketItem->m_Option3, pSocketItem->m_NewOption, pSocketItem->m_SetOption, pSocketItem->m_ItemOptionEx, pSocketItem->m_SocketOption, pSocketItem->m_BonusSocketOption, 0, CURRENT_DB_VERSION);
	lpObj->ChaosLock = FALSE;
	ItemByteConvert(pMsg.ItemInfo, Item);
	this->ChaosBoxInit(lpObj);
	::gObjChaosBoxInsertItemPos(lpObj->m_Index, Item, 0, -1);
	gObjChaosItemSet(lpObj->m_Index, 0, 1);
	GSProtocol.PROTOCOL_UserChaosBoxSend(lpObj, 0);
	g_Log.Add("[%s][%s] Set SeedSphere Mix Success - ItemInfo (Name : %s, ItemCode : %d, Level : %d, SocketOption[%d,%d,%d,%d,%d], BonusOption : %d)", lpObj->AccountID, lpObj->Name, ItemAttribute[Item.m_Type].Name, Item.m_Type, Item.m_Level, Item.m_SocketOption[0], Item.m_SocketOption[1], Item.m_SocketOption[2], Item.m_SocketOption[3], Item.m_SocketOption[4], Item.m_BonusSocketOption);
	IOCP.DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
}
...
void CMixSystem::SecromiconMix(LPOBJ lpObj)
{
	...
	if(first == 1 && second == 1 && third == 1 && fourth == 1 && fifth == 1 && sixth == 1)
	{
		...
		iMixTax = 0;
		iMixCost += iMixTax;
		if(lpObj->CharacterData->Money < iMixCost)
		{
			pMsg.Result = CB_NOT_ENOUGH_ZEN;
			IOCP.DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
			lpObj->ChaosLock = FALSE;
		...
		lpObj->CharacterData->Money -= iMixCost;
...
void CMixSystem::MonsterWingMix(LPOBJ lpObj) {
	...
	for (int i = 0; i < CHAOS_BOX_SIZE; i++) {
		if (lpObj->pChaosBox[i].IsItem() == TRUE) {
			if (lpObj->pChaosBox[i].m_Type == ITEMGET(12, 3) || lpObj->pChaosBox[i].m_Type == ITEMGET(12, 4) || lpObj->pChaosBox[i].m_Type == ITEMGET(12, 5) || lpObj->pChaosBox[i].m_Type == ITEMGET(12, 6) || lpObj->pChaosBox[i].m_Type == ITEMGET(13, 30) || lpObj->pChaosBox[i].m_Type == ITEMGET(12, 42) || lpObj->pChaosBox[i].m_Type == ITEMGET(12, 49)) {
				WingCount++;
	...
	if (lpObj->ChaosSuccessRate < 0) lpObj->ChaosSuccessRate = 0;
	...
	if (rand() % 100 <= lpObj->ChaosSuccessRate) {
		...
		if (iRandomValue2 == 1 && iRandomValue < 30) {
			NOption |= 2;
		} else if (iRandomValue < 15) {
			NOption |= 1;
		}

		ItemSerialCreateSend(lpObj->m_Index, 255, 0, 0, ITEMGET(12, WingType), 0, 0, 0, Option2, Option3, -1, NOption, 0, 0, 0, 0);
		gObjInventoryCommit(lpObj->m_Index);
		g_Log.Add("[MonsterWing Mix] [%s][%s] CBMix Success %d Money : %d-%d, CharmRate : %d", lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney, LuckyHarmAddRate);
	} else {
		for (int i = 0; i < CHAOS_BOX_SIZE; i++) {
		...
		GSProtocol.PROTOCOL_UserChaosBoxSend(lpObj, 0);
		IOCP.DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
		g_Log.Add("[MonsterWing Mix] [%s][%s] CBMix Fail %d Money : %d-%d, CharmRate : %d", lpObj->AccountID, lpObj->Name, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney, LuckyHarmAddRate);
	}
...
void CMixSystem::CCFRewardMix(LPOBJ lpObj)
{
	...
	::g_Log.Add("[Chaos Castle Survival Reward Mix] [%s][%s] CBMix Success Money : %d-%d",
		lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Money, nChaosNeedMoney);
	...
	for (int i = 0; i < iMixCount; i++)
	{
		if ((rand() % 100) < lpObj->ChaosSuccessRate)
		{
			...
			g_Log.Add("[BlessPotionMix] [%s][%s] Mass Combination ID:%d Success %d Money : %d-%d",
				lpObj->AccountID, lpObj->Name, i, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money,
				nChaosNeedMoney);
		}
...
void CMixSystem::CircleChaosMix_Multi(LPOBJ lpObj, int iMixCount) {	// Fruits Mass Mix
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++) {
		if (lpObj->pChaosBox[n].IsItem() == TRUE) {
			if (lpObj->pChaosBox[n].m_Type == ITEMGET(14, 22)) CreatureGemCount++;
			else if (lpObj->pChaosBox[n].m_Type == ITEMGET(12, 15)) ChoasGemCount++;
		}
	}

	if (CreatureGemCount != iMixCount || ChoasGemCount != iMixCount) return;
	...
	for (int i = 0; i < iMixCount; i++) {
		if ((rand() % 100) < lpObj->ChaosSuccessRate) {
			...
			pMsg.ItemInfo[i / 8] |= btMixNumber;
			lpObj->ChaosMassMixSuccessCount++;
			g_Log.Add("[CircleMix] [%s][%s] Mass Combination ID:%d Success %d Money : %d-%d", lpObj->AccountID, lpObj->Name, i, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney);
		} else {
			g_Log.Add("[CircleMix] [%s][%s] Mass Combination ID:%d Fail %d Money : %d-%d", lpObj->AccountID, lpObj->Name, i, lpObj->ChaosSuccessRate, lpObj->CharacterData->Money, nChaosNeedMoney);
		}
	}
	...
	if (lpObj->ChaosMassMixSuccessCount == 0) {
		...
		for (int i = 0; i < lpObj->ChaosMassMixSuccessCount; i++) {
			...
			if (RandonValue < 30) FruitType = 0;
			else if (RandonValue < 55) FruitType = 1;
			else if (RandonValue < 75) FruitType = 2;
			else if (RandonValue < 95) FruitType = 3;
			else FruitType = 4;
...
void CMixSystem::BloodCastleItemChaosMix_Multi(LPOBJ lpObj, int iMixCount)
{
	...
	if (gObj[iIndex].Class == CLASS_DARKLORD || gObj[iIndex].Class == CLASS_MAGICGLADIATOR || gObj[iIndex].Class == CLASS_RAGEFIGHTER)
	{
	...
	if (bIsChaosGemExist == iMixCount && bIsAngelKingPaperExist == iMixCount && bIsBloodBoneExist == iMixCount)
	{
		...
		if ((gObj[iIndex].CharacterData->Money - iMIX_NEED_MONEY) < 0)
		{
			pMsg.Result = CB_BC_NOT_ENOUGH_ZEN;
			IOCP.DataSend(iIndex, (LPBYTE)&pMsg, pMsg.h.size);
		...
		gObj[iIndex].CharacterData->Money -= iMIX_NEED_MONEY;
		g_CastleSiegeSync.AddTributeMoney(iChaosTaxMoney);
		...
		for (int i = 0; i < iMixCount; i++)
		{
			if ((rand() % 100) < iMIX_SUCCESS_RATE)
			{
				...
				g_Log.Add("[BloodCastle Ticket Mix] [%s][%s] Mass Combination ID:%d Success %d Money : %d-%d",
					lpObj->AccountID, lpObj->Name, i, iMIX_SUCCESS_RATE, lpObj->CharacterData->Money,
					iMIX_NEED_MONEY);
			}
	...
	for (int i = 0; i < iMixCount; i++)
	{
		if ((rand() % 100) < g_iShieldPotionLv1MixSuccessRate)
		{
			...
			g_Log.Add("[ChaosMix][Shield Potion] Lv1 Potion Mass Mix (%d) Success [%s][%s], Money(%d-%d)",
				lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Money, iChaosMixPrice);
		}
