...
void CChaosCastle::Load(LPSTR filename)
{
	...
	for (pugi::xml_node castle = expmultiplier.child("Castle"); castle; castle = castle.next_sibling())
	{
		...
		if (CC_FLOOR_RANGE(CastleNum)) this->m_stChaosCastleData[CastleNum].m_iCastleExperience = castle.attribute("Multiplier").as_float();
	}
				...
				if (iMonsterCount <= 0)
				{
					...
					if (iWinnerIndex != -1)
					{
						g_Log.Add("[Chaos Castle] (%d) [%s][%s] is Winner. [CharInfoSave : Class=%d, Level=%d, LvPoint=%d, Exp=%d, Str=%d, Dex=%d, Vit=%d, Energy=%d, LeaderShip=%d, Map=%d, Pk=%d]",
							...
							gObj[iWinnerIndex].Class, gObj[iWinnerIndex].Level, gObj[iWinnerIndex].CharacterData->LevelUpPoint,
							gObj[iWinnerIndex].CharacterData->Experience, gObj[iWinnerIndex].CharacterData->Strength,
							gObj[iWinnerIndex].CharacterData->Dexterity, gObj[iWinnerIndex].CharacterData->Vitality, gObj[iWinnerIndex].CharacterData->Energy,
...
void CChaosCastle::SendWinMessage(int iChaosCastleIndex, int iWinnerIndex)
{
	...
	{
		this->CC_RewardUserEXP(iChaosCastleIndex, gObj[iWinnerIndex].m_cChaosCastleSubIndex, true);
	}
...
void CChaosCastle::SendFailMessage(int iChaosCastleIndex, int iLoserIndex)
{
	...
	{
		this->CC_RewardUserEXP(iChaosCastleIndex, gObj[iLoserIndex].m_cChaosCastleSubIndex, false);
	}
		...
		if (this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1)
		{
			if (this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != iWinnerIndex)
			{
				if (this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_bSendQuitMsg == false)
				{
					...
					{
						this->CC_RewardUserEXP(iChaosCastleIndex, gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].m_cChaosCastleSubIndex, false);
					}
	...
	for (int iFALL=0;iFALL<MAX_CHAOSCASTLE_USER;iFALL++)
	{
		if (this->m_stChaosCastleData[iChaosCastleIndex].m_lFallUser[iFALL] != -1)
		{
			...
			if ((gObj[iFALL_INDEX].MapNumber) == this->GetChaosCastleMapNumber(iChaosCastleIndex) &&  gObjIsConnected(iFALL_INDEX) &&  gObj[iFALL_INDEX].Life > 0)
			{
				...
				GSProtocol.PROTOCOL_DiePlayerSend(&gObj[iFALL_INDEX], gObj[iFALL_INDEX].m_Index, 0, 0);
			}
		}
	}
}
...
BOOL CChaosCastle::CheckUserEnterMoney(int iUserIndex, int iEnterLevel)
{
	...
	if (gObj[iUserIndex].CharacterData->Money >= ::g_iChaosCastle_EnterCost[iEnterLevel])
	{
		return TRUE;
	}
...
BOOL CChaosCastle::PayUserEnterMoney(int iUserIndex, int iEnterLevel) {
	...
	if (CC_FLOOR_RANGE(iEnterLevel) == FALSE) return FALSE;
	if (gObj[iUserIndex].CharacterData->Money >= ::g_iChaosCastle_EnterCost[iEnterLevel]) {
		gObj[iUserIndex].CharacterData->Money -= ::g_iChaosCastle_EnterCost[iEnterLevel];
		GSProtocol.GCMoneySend(iUserIndex, gObj[iUserIndex].CharacterData->Money);
		return TRUE;
	}
...
void CChaosCastle::GiveUserDamage(int iUserIndex, int iDamage) {
	if (!gObjIsConnected(iUserIndex)) return;
	if (gObj[iUserIndex].Life <= 0.0) return;
	...
	if (gObj[iUserIndex].Type == OBJ_USER) IOCP.DataSend(iUserIndex, (LPBYTE)&pResult, pResult.h.size);
	...
	if (gObj[iUserIndex].Life <= 0.0) {
		...
		GSProtocol.PROTOCOL_DiePlayerSend(&gObj[iUserIndex], gObj[iUserIndex].m_Index, 0, 0);
	}
}

int CChaosCastle::CC_CalcRewardEXP(int UserIndex, UINT64 iAddExp) {
	if (ObjectMaxRange(UserIndex) == FALSE) return FALSE;
	if (gObj[UserIndex].Type != OBJ_USER) return FALSE;

	::gObjSetExpPetItem(UserIndex, iAddExp);

	if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(&gObj[UserIndex])) gObj[UserIndex].CharacterData->MasterExperience += iAddExp;
	else gObj[UserIndex].CharacterData->Experience += iAddExp;

	CHARACTER_LevelUp(&gObj[UserIndex], iAddExp, 0, "Chaos Castle");
	return iAddExp;
}

int CChaosCastle::CC_SendRewardEXP(int UserIndex, int iEXP) {
	if (iEXP <= 0) return FALSE;
	if (!gObjIsConnected(UserIndex)) return FALSE;
	...
	if (gObj[UserIndex].Type == OBJ_USER) {
		if (Reward > 0) {
			CheckItemOptForGetExpExRenewal(&gObj[UserIndex], 0, Reward, 0, TRUE);
			iEXP = this->CC_CalcRewardEXP(UserIndex, Reward);
		}

		GSProtocol.PROTOCOL_PlayerExperienceSend(UserIndex, (WORD)-1, iEXP, 0, 0);
	}
...
int CChaosCastle::GetCurrentWinUser(int iChaosCastleIndex) {
	if (CC_FLOOR_RANGE(iChaosCastleIndex) == FALSE) return -1;
	...
	for (int i = 0; i < MAX_CHAOSCASTLE_USER; i++) {
		if (this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1) {
			if (gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex) == TRUE && gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].MapNumber == this->GetChaosCastleMapNumber(iChaosCastleIndex) && this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iUserState == 0) {
	...
	if (iCC_WINNER_INDEX != -1) {
		if (gObj[iCC_WINNER_INDEX].m_cKillUserCount == 0 && gObj[iCC_WINNER_INDEX].m_cKillMonsterCount == 0) {
			...
			if (iUSER_COUNT != 1 || iMONSTER_COUNT != 0) iCC_WINNER_INDEX = -1;
		}
	}
...
int CChaosCastle::CheckEnterLevel(int iIndex, int iLevel) {
	if (ObjectMaxRange(iIndex) == FALSE) return 2;
	if (gObj[iIndex].Type != OBJ_USER || gObj[iIndex].Connected <= PLAYER_LOGGED) return 2;
	if (gObj[iIndex].Class == CLASS_DARKLORD || gObj[iIndex].Class == CLASS_MAGICGLADIATOR || gObj[iIndex].Class == CLASS_RAGEFIGHTER) {
		if (gObj[iIndex].Level >= g_sttCHAOSCASTLE_LEVEL[iLevel-1].iLOWER_BOUND_MAGUMSA && gObj[iIndex].Level <= g_sttCHAOSCASTLE_LEVEL[iLevel-1].iUPPER_BOUND_MAGUMSA) return 0;
		if (gObj[iIndex].Level < g_sttCHAOSCASTLE_LEVEL[iLevel-1].iLOWER_BOUND_MAGUMSA) return -1;
		if (gObj[iIndex].Level > g_sttCHAOSCASTLE_LEVEL[iLevel-1].iUPPER_BOUND_MAGUMSA) return 1;
	} else {
		if (gObj[iIndex].Level >= g_sttCHAOSCASTLE_LEVEL[iLevel-1].iLOWER_BOUND && gObj[iIndex].Level <= g_sttCHAOSCASTLE_LEVEL[iLevel-1].iUPPER_BOUND) return 0;
		if (gObj[iIndex].Level < g_sttCHAOSCASTLE_LEVEL[iLevel-1].iLOWER_BOUND) return -1;
		if (gObj[iIndex].Level > g_sttCHAOSCASTLE_LEVEL[iLevel-1].iUPPER_BOUND)	return 1;
	}
				 ...
				 gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].MapNumber == this->GetChaosCastleMapNumber(iChaosCastleIndex))
			{
				if (gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].Life > 0.0)
				{
					...
					if (btMapAttr == 8)
					{
						...
						GSProtocol.PROTOCOL_DiePlayerSend(&gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex],
	...
	for (int i=0;i<MAX_CHAOSCASTLE_USER;i++)
	{
		if (this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1)
		{
			if (gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex) ==TRUE &&
				 gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].MapNumber == this->GetChaosCastleMapNumber(iChaosCastleIndex))
			{
				int iFIRST_MONEY = gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].CharacterData->Money;
				int iPAYBACK_MONEY = g_iChaosCastle_EnterCost[iChaosCastleIndex] + g_iChaosCastle_GuardSetValue[iChaosCastleIndex];
				gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].CharacterData->Money += g_iChaosCastle_EnterCost[iChaosCastleIndex] + g_iChaosCastle_GuardSetValue[iChaosCastleIndex];
				int iNOW_MONEY = gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].CharacterData->Money;

				GSProtocol.GCMoneySend(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex, gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].CharacterData->Money);
...
void CChaosCastle::CC_RewardUserEXP(int iChaosCastleIndex, int iChaosCastleSubIndex, BOOL bWinner) {
	if (CC_FLOOR_RANGE(iChaosCastleIndex) == FALSE) return;
	if (CC_SUB_FLOOR_RANGE(iChaosCastleSubIndex) == FALSE) return;
	if (this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex != -1) {
		if (gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex) ==TRUE) {
			if (gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex].m_cChaosCastleIndex == -1 || gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex].m_cChaosCastleSubIndex == -1) return;
			...
			int iREWARD_EXP = this->CC_SendRewardEXP(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex, iTOT_EXP);
			...
			this->ChaosCastleRank(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex, iKILLCOUNT_USER, iKILLCOUNT_MONSTER, iTOT_EXP, bWinner);
			g_Log.Add("[Chaos Castle] (%d) [%s][%s] Reward User EXP (USER_KILL:%d, MON_KILL:%d, TOT_EXP:%d, TOT_REWARD_EXP:%d)", iChaosCastleIndex + 1, gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex].AccountID, gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex].Name, iKILLCOUNT_USER, iKILLCOUNT_MONSTER, iTOT_EXP, iREWARD_EXP);
		}
	}
}
		...
		if (this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1)
		{
				 ...
				 gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].MapNumber == this->GetChaosCastleMapNumber(iChaosCastleIndex))
			{
					   ...
					   iSY > g_iChaosCastle_DamageAxis[iTRAP_STEP][3]))
				{
					...
					if (BUFF_TargetHasBuff(&gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex], DEBUFFTYPE_CHILL) != 0)
					{
						...
						BUFF_AddBuffEffect(&gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex], DEBUFFTYPE_CHILL, 0, 0, 0, 0, 3);
					}
				}
			}
		}
...
BOOL CChaosCastle::CC_CheckTransformationRing(int iUserIndex) {
	if (gObjIsConnected(iUserIndex) == FALSE) return FALSE;
	if (gObj[iUserIndex].pInventory[10].IsItem()) {
		if (gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13,10)) return TRUE;
		if (gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13,39)) return TRUE;
		if (gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13,40)) return TRUE;
		if (gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13,41)) return TRUE;
		if (gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13,42)) return TRUE;
		if (gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13,68)) return TRUE;
		if (gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13,76)) return TRUE;
		if (gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13,77)) return TRUE;
		if (gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13,78)) return TRUE;
		if (gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13,122)) return TRUE;
		if (gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13, 163)) return TRUE;
		if (gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13, 164)) return TRUE;
		if (gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13, 165)) return TRUE;
		if (gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13, 166)) return TRUE;
	}

	if (gObj[iUserIndex].pInventory[11].IsItem()) {
		if (gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13,10)) return TRUE;
		if (gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13,39)) return TRUE;
		if (gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13,40)) return TRUE;
		if (gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13,41)) return TRUE;
		if (gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13,42)) return TRUE;
		if (gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13,68)) return TRUE;
		if (gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13,76)) return TRUE;
		if (gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13,77)) return TRUE;
		if (gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13,78)) return TRUE;
		if (gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13,122)) return TRUE;
		if (gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13, 163)) return TRUE;
		if (gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13, 164)) return TRUE;
		if (gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13, 165)) return TRUE;
		if (gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13, 166)) return TRUE;
	}
...
void CChaosCastle::GD_Req_Save_KillPoint_UBF(int index, char *Name, int KillPoint, int CastleIndex)
{
	...
	g_Log.Add("[CChaosCastle][GD_Req_Save_KillPoint][%s][%s][%s] Save Requested To DB, KillPoint:%d Castle Index:%d",
		gObj[index].AccountID, gObj[index].CharacterData->m_RealNameOfUBF, gObj[index].Name, KillPoint, CastleIndex);
}
...
void CChaosCastle::RewardUserKillPointUBF(int iChaosCastleIndex, int iChaosCastleSubIndex, bool bWinner)
{
	...
	int iREWARD_EXP = g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE ? 0 : this->CC_SendRewardEXP(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex,	iTOT_EXP);
	...
	if (g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE)
	{
		...
		g_Log.Add("[UBF][Chaos Castle] (%d) [%s][%s][%s] GameResult (USER_KILL:%d, MON_KILL:%d, TOT_KILLPOINT:%d, TOT_REWARD_EXP:%d)",
			...
			gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex].CharacterData->m_RealNameOfUBF,
...
void CChaosCastle::GDReqSetUBFReward_CCBattle(int iUserIndex, BYTE byRewardType)
{
	...
	g_Log.Add("[UBF][Chaos Castle][GDReqSetUBFReward_CCBattle]ACC:%s , NAME :%s,Real NAME :%s, RewardType:%d",
		lpObj->AccountID, lpObj->Name, lpObj->CharacterData->m_RealNameOfUBF, byRewardType);
...
void CChaosCastle::SetUBFGetReward(int iUserIndex, WORD wItemCode, UINT64 ItemSerial, BYTE btItemPos)
{
	...
	GSProtocol.PROTOCOL_InventoryItemDeleteSend(iUserIndex, btItemPos, 0);
