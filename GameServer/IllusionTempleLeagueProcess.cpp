					...
					if (this->m_UserData[nIndex].m_nIndex != -1 && gObj[this->m_UserData[nIndex].m_nIndex].Connected > PLAYER_LOGGED && gObj[this->m_UserData[nIndex].m_nIndex].MapNumber == this->m_nTempleNumber + 98)
					{
						if (byWinTeamIndex == 0 || byWinTeamIndex == 1)
						{
							...
							this->GDSaveITLUserPoint(
								...
								gObj[this->m_UserData[nIndex].m_nIndex].CharacterData->GuildName,
								0,
								0,
								0,
							...
							g_Log.Add("[ ITL ][GDSaveITLUserPoint][START_FAIL_SAVE] NAME :%s G.NAME:%s ",
								gObj[this->m_UserData[nIndex].m_nIndex].Name, gObj[this->m_UserData[nIndex].m_nIndex].CharacterData->GuildName);
						}
			...
			if (this->m_nRemainTimeMsec > 0 && this->m_bGameEndPosMove == false)
			{
				...
				for (int i = 0; i < 10; i++)
				{
					if (this->m_UserData[i].m_nIndex != -1 && gObj[this->m_UserData[i].m_nIndex].Connected > PLAYER_LOGGED && gObj[this->m_UserData[i].m_nIndex].MapNumber == this->m_nTempleNumber + 98)
					{
						...
						CHARACTER_Immobilize(this->m_UserData[i].m_nIndex, gObj[this->m_UserData[i].m_nIndex].X, gObj[this->m_UserData[i].m_nIndex].Y);
					}
					...
					if (this->m_UserData[i].m_nRelicsInvenPos > 0)
					{
						if (gObj[this->m_UserData[i].m_nIndex].pInventory[this->m_UserData[i].m_nRelicsInvenPos].m_Type == ITEMGET(14, 223))
						{
							...
							GSProtocol.PROTOCOL_InventoryItemDeleteSend(this->m_UserData[i].m_nIndex, this->m_UserData[i].m_nRelicsInvenPos, 0);
			...
			if (this->m_UserData[i].m_nIndex != -1 && gObj[this->m_UserData[i].m_nIndex].Connected > PLAYER_LOGGED)
			{
				if (gObj[this->m_UserData[i].m_nIndex].MapNumber == this->m_nTempleNumber + 98)
				{
					g_Log.Add("[ ITL ] MAP:[%d], ACC:[%s], NAME:[%s], TEAM:[%d], CLASS:[%d]",
						...
						this->m_UserData[i].m_btTeam, gObj[this->m_UserData[i].m_nIndex].CharacterData->DbClass);
				}
			}
		}
	...
	for (int i = 0; i < 10; i++)
	{
		if (this->m_UserData[i].m_nIndex != -1 && gObj[this->m_UserData[i].m_nIndex].Connected > PLAYER_LOGGED && gObj[this->m_UserData[i].m_nIndex].MapNumber == this->m_nTempleNumber + 98)
		{
			if (this->m_UserData[i].m_dwSkillProdectionTime != 0)
			{
				...
				BUFF_RemoveBuffEffect(&gObj[this->m_UserData[i].m_nIndex], BUFFTYPE_TEMPLE_PROTECTION);
			...
			if (this->m_UserData[i].m_dwSkillRestraintTime != 0)
			{
				...
				BUFF_RemoveBuffEffect(&gObj[this->m_UserData[i].m_nIndex], BUFFTYPE_RESTRICTION);
...
BOOL CIllusionTempleLeagueProcess::EnterUserIllusionTempleLeague(int aIndex, BYTE btTempleIndex, int nGuildIdx, int nPartyIdx)
{
	...
	if (nItemPos >= 0 && gObj[aIndex].pInventory[nItemPos].m_Type == ITEMGET(14, 223))
	{
		...
		GSProtocol.PROTOCOL_InventoryItemDeleteSend(aIndex, nItemPos, 0);
	}
	...
	for (int i = 0; i < 10; i++)
	{
		if (this->m_UserData[i].m_nIndex == -1)
		{
			...
			if (this->m_GuildDataSave[0].nGuild == gObj[this->m_UserData[i].m_nIndex].CharacterData->GuildNumber)
			{
				this->m_UserData[i].m_btTeam = 0;
			}

			else if (this->m_GuildDataSave[1].nGuild == gObj[this->m_UserData[i].m_nIndex].CharacterData->GuildNumber)
			{
...
BOOL CIllusionTempleLeagueProcess::LeaveUserIllusionTempleLeague(int aIndex)
{
	...
	if (gObj[aIndex].m_nITLIndex != -1 && this->m_UserData[gObj[aIndex].m_nITLIndex].m_nIndex == aIndex && gObjIsConnected(aIndex))
	{
		...
		{
			BUFF_RemoveBuffEffect(&gObj[aIndex], BUFFTYPE_TEMPLE_PROTECTION);
			this->Send_ITL_SkillEnd(&gObj[aIndex], 618);
		}

		if (this->m_UserData[gObj[aIndex].m_nITLIndex].m_dwSkillRestraintTime != 0)
		{
			BUFF_RemoveBuffEffect(&gObj[aIndex], BUFFTYPE_RESTRICTION);
							...
							this->m_UserData[nRegisterRelicsUserArray].m_btTeam == this->m_Occupied_StoneStatus[i].m_byStatus - 100)
						{
							...
							if (this->m_UserData[nRegisterRelicsUserArray].m_nRelicsInvenPos > 0)
							{
								if (lpObj->pInventory[this->m_UserData[nRegisterRelicsUserArray].m_nRelicsInvenPos].m_Type == ITEMGET(14, 223))
								{
									...
									GSProtocol.PROTOCOL_InventoryItemDeleteSend(lpObj->m_Index, this->m_UserData[nRegisterRelicsUserArray].m_nRelicsInvenPos, 0);
...
void CIllusionTempleLeagueProcess::ITLSkillProc(LPOBJ lpObj)
{
	...
	if (this->m_UserData[lpObj->m_nITLIndex].m_dwSkillProdectionTime != 0)
	{
		...
		if (this->m_UserData[lpObj->m_nITLIndex].m_dwSkillProdectionTime == 0)
		{
			...
			BUFF_RemoveBuffEffect(lpObj, BUFFTYPE_TEMPLE_PROTECTION);
	...
	if (this->m_UserData[lpObj->m_nITLIndex].m_dwSkillRestraintTime != 0)
	{
		...
		if (this->m_UserData[lpObj->m_nITLIndex].m_dwSkillRestraintTime == 0)
		{
			...
			BUFF_RemoveBuffEffect(lpObj, BUFFTYPE_RESTRICTION);
...
int CIllusionTempleLeagueProcess::UseSkillProdection(LPOBJ lpObj)
{
	...
	BUFF_AddBuffEffect(lpObj, BUFFTYPE_TEMPLE_PROTECTION, 0, 0, 0, 0, -10);
	return TRUE;
}

int CIllusionTempleLeagueProcess::UseSkillRestraint(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	if (CHARACTER_CheckSkillDistance(lpObj->m_Index, lpTargetObj->m_Index, 619) == FALSE)
	{
		return FALSE;
	}
	...
	BUFF_AddBuffEffect(lpObj, BUFFTYPE_RESTRICTION, 0, 0, 0, 0, -10);
...
{
	if (BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_STUN) == TRUE)
	{
	...
	PLAYER_TeleportMagicUse(lpObj->m_Index, addx, addy);
	return TRUE;
}

int CIllusionTempleLeagueProcess::UseSkillShieldBurn(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	if (CHARACTER_CheckSkillDistance(lpObj->m_Index, lpTargetObj->m_Index, 621) == FALSE)
	{
		return FALSE;
	}
	...
	GSProtocol.PROTOCOL_DamageSend(lpObj->m_Index, lpTargetObj->m_Index, 0, 0, 0, nShieldDamage);
	return TRUE;
}
				...
				if (byAlliedTeamKillSum <= byIllusionTeamKillSum)
				{
					if (byAlliedTeamKillSum >= byIllusionTeamKillSum)
					{
						...
						for (int i = 0; i < 10; i++)
						{
							if (this->m_UserData[i].m_btTeam == 0)
							{
								nAlliedLevelSum += (gObj[this->m_UserData[i].m_nIndex].Level + gObj[this->m_UserData[i].m_nIndex].CharacterData->MasterLevel);
							}

							else if (this->m_UserData[i].m_btTeam == 1)
							{
								nIllusionLevelSum += (gObj[this->m_UserData[i].m_nIndex].Level + gObj[this->m_UserData[i].m_nIndex].CharacterData->MasterLevel);
							}
						}
	...
	for (int i = 0; i < 10; i++)
	{
		if (this->m_UserData[i].m_nIndex != -1 && gObj[this->m_UserData[i].m_nIndex].Connected > PLAYER_LOGGED)
		{
			...
			g_Log.Add("[ ITL ] (%d) ITL Result  (Account:%s, Name:%s, G.name:%s , Team:%d, Class:%d, UserKillCount:%d,InvalidationCount:%d,OccupiedCount:%d)",
				this->m_nTempleNumber, gObj[this->m_UserData[i].m_nIndex].AccountID, gObj[this->m_UserData[i].m_nIndex].Name, gObj[this->m_UserData[i].m_nIndex].CharacterData->GuildName,
				this->m_UserData[i].m_btTeam, gObj[this->m_UserData[i].m_nIndex].CharacterData->DbClass, this->m_UserData[i].m_btUserKillCount, this->m_UserData[i].m_byInvalidationCount,
			...
			this->GDSaveITLUserPoint(this->m_UserData[i].m_nIndex, gObj[this->m_UserData[i].m_nIndex].Name, gObj[this->m_UserData[i].m_nIndex].CharacterData->GuildName,
