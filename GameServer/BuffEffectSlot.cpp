...
int CBuffEffectSlot::SetEffect(LPOBJECTSTRUCT lpObj, int iBuffIndex, BYTE btEffectType1, BYTE btEffectType2, int iEffectValue1, int iEffectValue2, int iDuration) {
	...
	for (int i = 0; i < MAX_BUFFEFFECT; i++) {
		if (lpObj->m_BuffEffectList[i].BuffIndex != BUFFTYPE_NONE) {
			...
			if (lpPrevBuffData->BuffType == BUFFEFFECT_TYPE_POSITIVE) BuffCount++;
			else if (lpPrevBuffData->BuffType == BUFFEFFECT_TYPE_NEGATIVE) DebuffCount++;
	...
	for (int i = 0; i < MAX_BUFFEFFECT; i++) {
		if (lpObj->m_BuffEffectList[i].BuffIndex == BUFFTYPE_NONE) {
			...
			g_BuffEffect.BUFFEFFECT_SetEffect(lpObj,btEffectType1,iEffectValue1); //first
			g_BuffEffect.BUFFEFFECT_SetEffect(lpObj,btEffectType2,iEffectValue2); //second
			break;
		}
	}
...
bool CBuffEffectSlot::RemoveEffect(LPOBJECTSTRUCT lpObj, int iBuffIndex) {
	...
	for (int i = 0; i < MAX_BUFFEFFECT; i++) {
		...
		if (lpObj->m_BuffEffectList[i].BuffIndex == iBuffIndex) {	//Will Clear the Effect
			g_BuffEffect.BUFFEFFECT_ClearEffect(lpObj,lpObj->m_BuffEffectList[i].EffectType1,lpObj->m_BuffEffectList[i].EffectValue1);
			g_BuffEffect.BUFFEFFECT_ClearEffect(lpObj,lpObj->m_BuffEffectList[i].EffectType2,lpObj->m_BuffEffectList[i].EffectValue2);
			memset(&lpObj->m_BuffEffectList[i],0,sizeof(lpObj->m_BuffEffectList[i]));
...
void BUFF_DOTEffectTick(LPOBJECTSTRUCT lpObj) {
	...
	if ((lpObj->Type == OBJ_USER && lpObj->Connected >= PLAYER_PLAYING) || lpObj->Type == OBJ_MONSTER) {
		for (int i = 0; i < MAX_BUFFEFFECT; i++) {
			...
			switch (lpObj->m_BuffEffectList[i].BuffIndex) {
				case DEBUFFTYPE_POISON:
					if ((lpObj->m_BuffEffectList[i].EffectDuration % 3) == 0) {
						g_BuffEffect.BUFFEFFECT_SetDOT(lpObj, lpObj->m_BuffEffectList[i].EffectType1, lpObj->m_BuffEffectList[i].EffectValue1);
						g_BuffEffect.BUFFEFFECT_SetDOT(lpObj, lpObj->m_BuffEffectList[i].EffectType2, lpObj->m_BuffEffectList[i].EffectValue2);
					}
				break;
			...
			if (lpObj->m_BuffEffectList[i].EffectDuration <= 0) BUFF_RemoveBuffEffect(lpObj, lpObj->m_BuffEffectList[i].BuffIndex);
		}
	}
}
int BUFF_GetRemainingDuration(LPOBJECTSTRUCT lpObj, int BuffIndex) {
	if (lpObj == NULL) return FALSE;
	...
	if ((lpObj->Type == OBJ_USER && lpObj->Connected >= PLAYER_PLAYING) || lpObj->Type == OBJ_MONSTER) {
		for (int i = 0; i < MAX_BUFFEFFECT; i++) {
			if (lpObj->m_BuffEffectList[i].EffectDuration == -10) continue;
			if (lpObj->m_BuffEffectList[i].BuffIndex == BUFFTYPE_NONE) continue;
			if (lpObj->m_BuffEffectList[i].BuffIndex == BuffIndex && lpObj->m_BuffEffectList[i].EffectDuration >= 0) return lpObj->m_BuffEffectList[i].EffectDuration;
		}
	}

	return FALSE;
}
bool BUFF_AddBuffEffect(LPOBJECTSTRUCT lpObj, int iBuffIndex) {
	if (lpObj == NULL)	return false;
	if (lpObj->m_Index < 0 || lpObj->m_Index > g_ConfigRead.server.GetObjectMax()) return false;
	...
	if (iRemoveBuffIndex != -1) {
		if (iRemoveBuffIndex > 0) GCUseBuffEffect(lpObj, iRemoveBuffIndex, BUFFSTATE_BUFF_NEW, lpItemEffectInfo->wOptionType, 0, 0, 0);
...
bool BUFF_AddBuffEffect(LPOBJECTSTRUCT lpObj, int iBuffIndex, BYTE EffectType1, int EffectValue1, BYTE EffectType2, int EffectValue2, int Duration) {
	if (lpObj == NULL) return false;
	if (lpObj->m_Index < 0 || lpObj->m_Index > g_ConfigRead.server.GetObjectMax()) return false;
	if (lpObj->m_bOffLevel == true && g_OffLevel.m_General.Immortal == 1) return false;
	...
	if (iRemoveBuffIndex != -1) {
		if (iRemoveBuffIndex > 0) GCUseBuffEffect(lpObj, iRemoveBuffIndex, BUFFSTATE_BUFF_NEW, lpBuffData->BuffEffectType, 0, 0, 0);
		else lpObj->m_BuffEffectCount++;
...
bool BUFF_AddBuffEffect(LPOBJECTSTRUCT lpObj, int iBuffIndex, int Duration) {
	if (lpObj == NULL)	return false;
	if (lpObj->m_Index < 0 || lpObj->m_Index > g_ConfigRead.server.GetObjectMax()) return false;
	if (lpObj->m_bOffLevel == true && g_OffLevel.m_General.Immortal == 1) return false;
...
bool BUFF_RemoveBuffEffect(LPOBJECTSTRUCT lpObj, int iBuffIndex) {
	if (lpObj == NULL)	return false;
	if (lpObj->m_Index < 0 || lpObj->m_Index > g_ConfigRead.server.GetObjectMax())	return false;
	...
	if (g_BuffEffectSlot.RemoveEffect(lpObj, iBuffIndex) == true) {
		...
		if (iBuffIndex == BUFFTYPE_TOUGHNESS || iBuffIndex == BUFFTYPE_IMPROVED_TOUGHNESS || iBuffIndex == BUFFTYPE_BLESS || iBuffIndex == BUFFTYPE_ELEMENTALSEAL_FIRE || iBuffIndex == BUFFTYPE_ELEMENTALSEAL_WATER || iBuffIndex == BUFFTYPE_ELEMENTALSEAL_EARTH || iBuffIndex == BUFFTYPE_ELEMENTALSEAL_WIND || iBuffIndex == BUFFTYPE_ELEMENTALSEAL_DARKNESS) gObjCalCharacter.CHARACTER_Calc(lpObj->m_Index);
		return true;
	}
	...
	case CLEAR_TYPE_CASTLESIEGE:
		{
			for (iBuffIndex = 0; iBuffIndex < MAX_BUFFEFFECT; iBuffIndex++)
			{
				...
				case BUFFTYPE_CASTLE_ATTACK_MARK_3:
					{
						if (BUFF_RemoveBuffEffect(lpObj, lpObj->m_BuffEffectList[iBuffIndex].BuffIndex) == true)
						{
	...
	case CLEAR_TYPE_PCBANG_SEAL_FINISH:
		{
			for (iBuffIndex = 0; iBuffIndex < MAX_BUFFEFFECT; iBuffIndex++)
			{
				switch(lpObj->m_BuffEffectList[iBuffIndex].BuffIndex)
				{
				case BUFFTYPE_SEAL_OF_ASCENSION_EXP:
				case BUFFTYPE_SEAL_OF_WEALTH_EXP:
				case BUFFTYPE_SEAL_OF_SUSTENANCE_EXP:
	...
	case CLEAR_TYPE_NON_PCS_ITEM_EFFECT:
		{
			for (iBuffIndex = 0; iBuffIndex < MAX_BUFFEFFECT; iBuffIndex++)
			{
				...
				case BUFFTYPE_SEAL_OF_ASCENSION:
				case BUFFTYPE_SEAL_OF_WEALTH:
				case BUFFTYPE_SEAL_OF_SUSTENANCE:
...
bool BUFF_TargetHasBuff(LPOBJECTSTRUCT lpObj, int iBuffIndex) {
	if (lpObj == NULL) return false;
	if (lpObj->m_Index < 0 || lpObj->m_Index > g_ConfigRead.server.GetObjectMax()) return false;
...
void BUFF_RemoveDebuffEffect(LPOBJECTSTRUCT lpObj) {
	ULONGLONG iTime = 0;
	int iIndex = -1;
	...
	if (iTime) BUFF_RemoveBuffEffect(lpObj, lpObj->m_BuffEffectList[iIndex].BuffIndex);
}

bool BUFF_ChangeBuffTime(LPOBJECTSTRUCT lpObj, int iBuffIndex, int iTime) {
	if (lpObj == NULL) return false;
	if (lpObj->Connected < PLAYER_PLAYING) return false;
	for (int i = 0; i < MAX_BUFFEFFECT; i++) {
		...
		if (lpObj->m_BuffEffectList[i].BuffIndex == iBuffIndex) {
			...
			if (lpObj->m_BuffEffectList[i].EffectDuration < 0) BUFF_RemoveBuffEffect(lpObj, iBuffIndex);
...
bool BUFF_IsStrongerBuff(LPOBJECTSTRUCT lpObj, int iBuffIndex, int EffectValue1, int EffectValue2) {
	if (lpObj == NULL) return false;
	if (lpObj->Connected < PLAYER_PLAYING) return false;
	for (int i = 0; i < MAX_BUFFEFFECT; i++) {
		if (lpObj->m_BuffEffectList[i].BuffIndex == iBuffIndex)	{
...
int BUFF_IsBuffEffectType(LPOBJECTSTRUCT lpObj, int iBuffIndex, int EffectType) {
	if (lpObj == NULL)	return false;
	if (lpObj->Connected < PLAYER_PLAYING)	return false;
	for (int i = 0; i < MAX_BUFFEFFECT; i++) {
		if (lpObj->m_BuffEffectList[i].BuffIndex == iBuffIndex) {
			if (lpObj->m_BuffEffectList[i].EffectType1 == EffectType || lpObj->m_BuffEffectList[i].EffectType2 == EffectType) return true;
		}
	}
...
int BUFF_GetTotalBuffEffectValue(LPOBJECTSTRUCT lpObj, int EffectType) {
	if (lpObj == NULL) return FALSE;
	if (lpObj->Connected < PLAYER_PLAYING) return FALSE;
	...
	for (int i = 0; i < MAX_BUFFEFFECT; i++) {
		if (lpObj->m_BuffEffectList[i].BuffIndex == BUFFTYPE_NONE) continue;
		if (lpObj->m_BuffEffectList[i].EffectType1 == EffectType) iTotalValue += lpObj->m_BuffEffectList[i].EffectValue1;
		if (lpObj->m_BuffEffectList[i].EffectType2 == EffectType) iTotalValue += lpObj->m_BuffEffectList[i].EffectValue2;
	}

	return iTotalValue;
}

bool BUFF_GetBuffEffectValue(LPOBJECTSTRUCT lpObj, int iBuffIndex, int *EffectValue1, int *EffectValue2) {
	if (lpObj == NULL)	return false;
	if (lpObj->Connected < PLAYER_PLAYING)	return false;
	for (int i = 0; i < MAX_BUFFEFFECT; i++) {
		if (lpObj->m_BuffEffectList[i].BuffIndex == iBuffIndex) {
			if (EffectValue1 != 0) *EffectValue1 = lpObj->m_BuffEffectList[i].EffectValue1;
			if (EffectValue2 != 0) *EffectValue2 = lpObj->m_BuffEffectList[i].EffectValue2;
...
void GCUseBuffEffect(LPOBJECTSTRUCT lpObj, BYTE BuffEffectIndex, BYTE EffectUseOption, WORD OptionType, WORD EffectType, int LeftTime, DWORD EffectValue) {
	...
	if (BuffEffectIndex == BUFFTYPE_USE_MOUNT_UNIRIA || BuffEffectIndex == BUFFTYPE_USE_MOUNT_DINORANT || BuffEffectIndex == BUFFTYPE_USE_MOUNT_DLHORSE || BuffEffectIndex == BUFFTYPE_USE_MOUNT_FENRIR) ItemByteConvert(pMsg.ItemInfo, lpObj->pInventory[lpObj->m_btInvenPetPos]);
	if (lpObj->Type == OBJ_USER) IOCP.DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
	if (EffectUseOption == 0) GSProtocol.PROTOCOL_StateInfoSend(lpObj, 1, BuffEffectIndex);
	else GSProtocol.PROTOCOL_StateInfoSend(lpObj, 0, BuffEffectIndex);
...
void gObjSendBuffList(LPOBJECTSTRUCT lpObj) {
	...
	for (int i = 0; i < MAX_BUFFEFFECT; i++) {
		if (lpObj->m_BuffEffectList[i].BuffIndex != BUFFTYPE_NONE) GSProtocol.PROTOCOL_StateInfoSend(lpObj, 1, lpObj->m_BuffEffectList[i].BuffIndex);
	}
}
...
BOOL gObjAddBuffEffectWideArea(LPOBJECTSTRUCT lpTargetObj, int nDistance, int& nAffectedCount, int iBuffIndex, BYTE EffectType1, int EffectValue1, BYTE EffectType2, int EffectValue2, int Duration)
{
	...
	for (int i = g_ConfigRead.server.GetObjectStartUserIndex(); i < g_ConfigRead.server.GetObjectMax(); i++)
	{
		...
		if (lpObj->Type == OBJ_USER && gObjIsConnected(i) != FALSE)
		{
			...
			{
				if (CHARACTER_CalcDistance(lpTargetObj, lpObj) < nDistance)
				{
					BUFF_AddBuffEffect(lpObj, iBuffIndex, EffectType1, EffectValue1, EffectType2, EffectValue2, Duration);
					nAffectedCount++;
				}
			}
		}
...
void CheckItemOptForGetExpExRenewal(LPOBJ lpObj, LPOBJ lpTargetObj, UINT64 &iExp, UINT64 iDefaultExp, BOOL bRewardExp) {
	...
	if (lpObj->pInventory[8].IsItem() == TRUE && lpObj->pInventory[8].m_Type == ITEMGET(13, 80) && lpObj->pInventory[8].m_Durability > 0.0) {
		if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_SEAL_OF_SUSTENANCE) || BUFF_TargetHasBuff(lpObj, BUFFTYPE_SEAL_OF_SUSTENANCE_EXP)) iRetExp = 0;
		else if (!bRewardExp) nAddExp += 50;
	}

	if ((lpObj->pInventory[11].m_Type == ITEMGET(13,77) && lpObj->pInventory[11].m_Durability > 0.0) || (lpObj->pInventory[10].m_Type == ITEMGET(13,77) && lpObj->pInventory[10].m_Durability > 0.0)) {
		if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_SEAL_OF_SUSTENANCE) || BUFF_TargetHasBuff(lpObj, BUFFTYPE_SEAL_OF_SUSTENANCE_EXP)) iRetExp = 0;
		else if (!bRewardExp) nAddExp += 30;
	}

	if ((lpObj->pInventory[11].m_Type == ITEMGET(13,78) && lpObj->pInventory[11].m_Durability > 0.0) || (lpObj->pInventory[10].m_Type == ITEMGET(13,78) && lpObj->pInventory[10].m_Durability > 0.0)) {
		if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_SEAL_OF_SUSTENANCE) || BUFF_TargetHasBuff(lpObj, BUFFTYPE_SEAL_OF_SUSTENANCE_EXP)) iRetExp = 0;
		else if (!bRewardExp) nAddExp += 50;
	}

	if (lpTargetObj && lpTargetObj->m_iPentagramMainAttribute > 0) nAddExp += 15;
	if (g_GensSystem.IsMapBattleZone(lpObj->MapNumber) == TRUE) nAddExp += g_GensSystem.GetBattleZoneExpBonus(lpObj);
	if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_SEAL_OF_SUSTENANCE) || BUFF_TargetHasBuff(lpObj, BUFFTYPE_SEAL_OF_SUSTENANCE_EXP)) iRetExp = 0;
	else {
		if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_SEAL_OF_ASCENSION) ||
			BUFF_TargetHasBuff(lpObj, BUFFTYPE_SEAL_OF_WEALTH) ||
			BUFF_TargetHasBuff(lpObj, BUFFTYPE_SEAL_OF_ASCENSION_EXP) ||
			BUFF_TargetHasBuff(lpObj, BUFFTYPE_SEAL_OF_WEALTH_EXP) ||
			BUFF_TargetHasBuff(lpObj, BUFFTYPE_PCS_MARK5) ||
			BUFF_TargetHasBuff(lpObj, BUFFTYPE_PCS_MARK1_FOR_MASTER) ||
			BUFF_TargetHasBuff(lpObj, BUFFTYPE_PCS_MARK2_FOR_MASTER) ||
			BUFF_TargetHasBuff(lpObj, BUFFTYPE_ASCENSION_SEAL1) ||
			BUFF_TargetHasBuff(lpObj, BUFFTYPE_ASCENSION_SEAL2))
		{
			int iEffectExp = BUFF_GetTotalBuffEffectValue(lpObj, EFFECTTYPE_EXPERIENCE);
			bool bMasterLevel = g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(lpObj);

			if (bMasterLevel == true) {
				if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_PCS_MARK1_FOR_MASTER) || BUFF_TargetHasBuff(lpObj, BUFFTYPE_PCS_MARK2_FOR_MASTER) || BUFF_TargetHasBuff(lpObj, BUFFTYPE_ASCENSION_SEAL2)) {
					if (bRewardExp) iEffectExp = 0;
				} else iEffectExp = 0;
			} else {
				if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_PCS_MARK1_FOR_MASTER) || BUFF_TargetHasBuff(lpObj, BUFFTYPE_PCS_MARK2_FOR_MASTER) || BUFF_TargetHasBuff(lpObj, BUFFTYPE_ASCENSION_SEAL2)) iEffectExp = 0;
				else {
					if (bRewardExp) iEffectExp = 0;
				}
			}
	...
	if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_LIGHT_BLESSING) || BUFF_TargetHasBuff(lpObj, BUFFTYPE_LIGHT_BLESSING_LOW) || BUFF_TargetHasBuff(lpObj, BUFFTYPE_LIGHT_BLESSING_MIDDLE)) {
		int iEffectExp = BUFF_GetTotalBuffEffectValue(lpObj, EFFECTTYPE_LIGHT_BLESS_EXP);
	...
	if (BUFF_TargetHasBuff(lpObj, 179) || BUFF_TargetHasBuff(lpObj, 180) || BUFF_TargetHasBuff(lpObj, 181) || BUFF_TargetHasBuff(lpObj, 182) || BUFF_TargetHasBuff(lpObj, 183)) nAddExp += 20;
	if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_ASCENSION_MARK1) || BUFF_TargetHasBuff(lpObj, BUFFTYPE_ASCENSION_MARK2) || BUFF_TargetHasBuff(lpObj, BUFFTYPE_ASCENSION_MARK3)) {
		int iEffectExp = BUFF_GetTotalBuffEffectValue(lpObj, EFFECTTYPE_ASCENSION_MARK_EXP);
	...
	if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_GLORYOFGLADIATOR)) {
		int iEffectExp = 0;

		if (g_NewPVP.IsVulcanusMap(lpObj->MapNumber) == TRUE) iEffectExp = 20;
	...
	if ((lpObj->pInventory[11].m_Type == ITEMGET(13, 166) && lpObj->pInventory[11].m_Durability > 0.0) || (lpObj->pInventory[10].m_Type == ITEMGET(13, 166) && lpObj->pInventory[10].m_Durability > 0.0)) {
		if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_SEAL_OF_SUSTENANCE) || BUFF_TargetHasBuff(lpObj, BUFFTYPE_SEAL_OF_SUSTENANCE_EXP)) iRetExp = 0;
		else if (!bRewardExp) nAddExp += 20;
	}
	...
	{
		if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_SEAL_OF_SUSTENANCE) || BUFF_TargetHasBuff(lpObj, BUFFTYPE_SEAL_OF_SUSTENANCE_EXP))
		{
			iRetExp = 0;
		}
	...
	if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_PARTYITEM_EXP))
	{
		int iEffectExp = 30;
		int iEffectValue = 0;
		BUFF_GetBuffEffectValue(lpObj, BUFFTYPE_PARTYITEM_EXP, &iEffectValue, 0);
		...
		if (lpObj->PartyNumber >= 0)
		{
			for (int i = 0; i < MAX_USER_IN_PARTY; i++)
			{
				...
				int iPartyDis = CHARACTER_CalcDistance(lpObj, lpPartyObj);
