...
void GensSystem::ReqRegGensMember(LPOBJ lpObj, unsigned char btInfluence) // done
{
	if (gObjIsConnected(lpObj->m_Index))
	{
		...
		{
			pMsg.wNumberH = SET_NUMBERHW(lpObj->CharacterData->lpGuild->Number);
			pMsg.wNumberL = SET_NUMBERLW(lpObj->CharacterData->lpGuild->Number);
		}
...
int GensSystem::SendGensInfo(LPOBJ lpObj)
{
	...
	if (gObjIsConnected(lpObj))
	{
		...
		pMsg.iGensRanking = lpObj->CharacterData->m_GensRanking;
		pMsg.iGensClass = lpObj->CharacterData->m_GensClass;
		pMsg.iContributePoint = lpObj->CharacterData->m_ContributePoint;
	...
	if (gObjIsConnected(lpObj->m_Index))
	{
		if (this->IsRegGensInfluence(lpObj))
		{
			...
			for (int i = 0; i < 10; ++i)
			{
				...
				pMsg.KillCount[i] = lpObj->CharacterData->m_KillUserCnt[i];

				memcpy(&pMsg.KillName[i * 11], &lpObj->CharacterData->KillUserName[i * 11], 10); 
			}
...
void GensSystem::GensViewportListProtocol(LPOBJ lpObj)
{
	...
	if ((lpObj->Authority & 0x20) != 0x20 || !BUFF_TargetHasBuff(lpObj, BUFFTYPE_INVISABLE))
	{
		...
		pGensMsg.iGensRanking = lpObj->CharacterData->m_GensRanking;
...
void GensSystem::SetGensInfluence(LPOBJ lpObj, int iInfluence)
{
	...
	if(lpObj->CharacterData->ISBOT == true)
	{
		return;
	}
	...
	else
	{
		...
		lpObj->CharacterData->m_GensInfluence = iInfluence;
	}
};
...
int GensSystem::GetGensInfluence(LPOBJ lpObj)
{
	...
	if (lpObj->CharacterData->m_GensInfluence == LEAVED_INFLUENCE || lpObj->CharacterData->m_GensInfluence < NONE_INFLUENCE || lpObj->CharacterData->m_GensInfluence > VANERT_INFLUENCE) 
	{
		g_Log.Add("[GensSystem] Error [%s][%s] GensInfluence [%d]", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->m_GensInfluence); 
		return 0;
	}

	return lpObj->CharacterData->m_GensInfluence;
...
char *GensSystem::GetGensInfluenceName(LPOBJ lpObj)
{
	...
	return szInfluence[lpObj->CharacterData->m_GensInfluence];
};
...
int GensSystem::SetUserBattleZoneEnable(LPOBJ lpObj, int bBattleZoneEnable)
{
	...
	return lpObj->CharacterData->m_UserBattleZoneEnable = bBattleZoneEnable;
};
...
int GensSystem::IsUserBattleZoneEnable(LPOBJ lpObj)
{
	...
	return lpObj->CharacterData->m_UserBattleZoneEnable;
};
...
int GensSystem::SetContributePoint(LPOBJ lpObj, int iContributePoint)
{
	...
	return lpObj->CharacterData->m_ContributePoint = iContributePoint;
};
...
int GensSystem::AddContributePoint(LPOBJ lpObj, int iContributePoint)
{
	...
	lpObj->CharacterData->m_ContributePoint += iContributePoint;
...
int GensSystem::SubContributePoint(LPOBJ lpObj, int iContributePoint)
{
	...
	if (lpObj->CharacterData->m_ContributePoint < 0)
	{
		lpObj->CharacterData->m_ContributePoint = 0;
	}
...
int GensSystem::GetContributePoint(LPOBJ lpObj)
{
	...
	return lpObj->CharacterData->m_ContributePoint;
};
...
void GensSystem::PkPenaltyDropZen(LPOBJ lpObj)
{
	...
	if ((rand()%30) == 0)
	{
		int iDropMoney = 3 * lpObj->CharacterData->Money / 100;

		MapC[lpObj->MapNumber].MoneyItemDrop(iDropMoney, lpObj->X, lpObj->Y);

		lpObj->CharacterData->Money -= iDropMoney;
		...
		g_Log.Add("[GensSystem] [%s][%s] PkPenalty DropZen Money[%d] DropMoney[%d]",
			lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Money, iDropMoney);
	}
};
			...
			if (iInfluence != iTarInfluence)
			{
				...
				if (this->ChkKillUserName(lpObj, szName) == 0)
				{
					...
					if (iKillerContributePoint > 0 || iContributePoint <= 0)
					{
						...
						iGensClassGap = lpObj->CharacterData->m_GensClass - lpTargetObj->CharacterData->m_GensClass;
					
						if (iGensClassGap == 1)
						{
							if (lpObj->CharacterData->m_GensClass >= 9)
							{
								iAddBonusValue = 1;
							}
							else
						...
						else
						{
							...
							if (lpObj->CharacterData->m_GensClass > 9 && iAddBonusValue + iAddValue >= this->GetNextContributePoint(lpObj))
							{
...
int GensSystem::ChkKillUserName(LPOBJ lpObj, char *szCharName)
{
	...
	while (true)
	{
		...
		if (!strcmp(&lpObj->CharacterData->KillUserName[i*11], szCharName))
	 	break;
	...
	EnterCriticalSection((LPCRITICAL_SECTION)&criti);
	++lpObj->CharacterData->m_KillUserCnt[i];
	LeaveCriticalSection((LPCRITICAL_SECTION)&criti);
...
int GensSystem::AbusingPenalty(LPOBJ lpObj, int iKillUserIndex)
{
	...
	int iKillUserCnt = lpObj->CharacterData->m_KillUserCnt[iKillUserIndex];
...
int GensSystem::InsertKillUserName(LPOBJ lpObj, char *szCharName)
{
	...
	int iKillUserToCnt = lpObj->CharacterData->m_KillUserTotCnt;
	...
	memset(&lpObj->CharacterData->KillUserName[iKillUserToCnt*11], 0, 11);
	strcpy(&lpObj->CharacterData->KillUserName[iKillUserToCnt*11], szCharName);

	++lpObj->CharacterData->m_KillUserCnt[iKillUserToCnt];
	++lpObj->CharacterData->m_KillUserTotCnt;

	for (int i = 0; i < lpObj->CharacterData->m_KillUserTotCnt; ++i)
	{
		g_Log.Add("[GensSystem] Abusing List [%s][%s] KillUserName[%s] Cnt[%d]", lpObj->AccountID, lpObj->Name, &lpObj->CharacterData->KillUserName[iKillUserToCnt*11], lpObj->CharacterData->m_KillUserCnt[iKillUserToCnt]);
		iKillUserToCnt = i + 1;
	}
...
void GensSystem::MoveInBattleZonePartySplit(LPOBJ lpObj)
{
	...
	if (lpObj->CharacterData->m_bUsePartyMatching == false && lpObj->PartyNumber >= 0)
	{
...
void GensSystem::AbusingInfoReset(LPOBJ lpObj)
{
	...
	if (gObjIsConnected(lpObj->m_Index))
	{
		if (this->IsRegGensInfluence(lpObj))
		{
			if ( this->GetAbusingResetTime() > 0 && GetTickCount64() - lpObj->CharacterData->m_GensAbusingTick >= 60000 * this->GetAbusingResetTime())
			{
				EnterCriticalSection((LPCRITICAL_SECTION)&criti);

				lpObj->CharacterData->m_GensAbusingTick = GetTickCount64();

				for (int i = 0; i < 10; ++i)
				{
					lpObj->CharacterData->m_KillUserCnt[i] = 0;
					memset(&lpObj->CharacterData->KillUserName[i*11], 0, 11);
				}
...
int GensSystem::CalGensClass(LPOBJ lpObj)
{
	...
	if (lpObj->CharacterData->m_ContributePoint < this->m_GensRanking[8].ContributePoint)
	{
		if (lpObj->CharacterData->m_ContributePoint < this->m_GensRanking[9].ContributePoint)
		{
			if (lpObj->CharacterData->m_ContributePoint < this->m_GensRanking[10].ContributePoint)
			{
				if (lpObj->CharacterData->m_ContributePoint < this->m_GensRanking[11].ContributePoint)
				{
					if (lpObj->CharacterData->m_ContributePoint < this->m_GensRanking[12].ContributePoint)
					{
						if (lpObj->CharacterData->m_ContributePoint < this->m_GensRanking[13].ContributePoint)
						{
							return 0;
						}
						else 
	...
	{
		if (lpObj->CharacterData->m_GensRanking > this->m_GensRanking[0].RankingPosition)
		{
			if (lpObj->CharacterData->m_GensRanking > this->m_GensRanking[1].RankingPosition)
			{
				if (lpObj->CharacterData->m_GensRanking > this->m_GensRanking[2].RankingPosition)
				{
					if (lpObj->CharacterData->m_GensRanking > this->m_GensRanking[3].RankingPosition)
					{
						if (lpObj->CharacterData->m_GensRanking > this->m_GensRanking[4].RankingPosition)
						{
							if (lpObj->CharacterData->m_GensRanking > this->m_GensRanking[5].RankingPosition)
							{
								if (lpObj->CharacterData->m_GensRanking > this->m_GensRanking[6].RankingPosition)
								{
									if (lpObj->CharacterData->m_GensRanking > this->m_GensRanking[7].RankingPosition)
									{
										return 9;
									}
									else
...
int GensSystem::SetGensRanking(LPOBJ lpObj, int iGensRanking)
{
	...
	return lpObj->CharacterData->m_GensRanking = iGensRanking;
};
...
int GensSystem::SetGensClass(LPOBJ lpObj, int iGensClass)
{
	...
	return lpObj->CharacterData->m_GensClass = iGensClass;
};
...
int GensSystem::GetGensClass(LPOBJ lpObj)
{
	...
	return lpObj->CharacterData->m_GensClass;
};
...
int GensSystem::GetNextContributePoint(LPOBJ lpObj)
{
	...
	if (this->IsRegGensInfluence(lpObj))
	{
		if (lpObj->CharacterData->m_ContributePoint > 499)
		{
			if (lpObj->CharacterData->m_ContributePoint > 1499)
			{
				if (lpObj->CharacterData->m_ContributePoint > 2999)
				{
					if (lpObj->CharacterData->m_ContributePoint > 5999)
					{
						if (lpObj->CharacterData->m_ContributePoint > 9999)
						{
							return -2;
						}
						else
						{
							return 10000 - lpObj->CharacterData->m_ContributePoint;
						}
					}
					else
					{
						return 6000 - lpObj->CharacterData->m_ContributePoint;
					}
				}
				else
				{
					return 3000 - lpObj->CharacterData->m_ContributePoint;
				}
			}
			else
			{
		 	return 1500 - lpObj->CharacterData->m_ContributePoint;
			}
		}
		else
		{
			return 500 - lpObj->CharacterData->m_ContributePoint;
		}
	}
	else
	{
