...
BYTE CCastleSiege::ScheduleStateCompFunc(_CS_SCHEDULE_DATA & A,_CS_SCHEDULE_DATA & B) {
	if (A.m_iSTATE > B.m_iSTATE) return false;
...
int CCastleSiege::Ready(int iMapSvrGroup) {
	if (iMapSvrGroup < 0) return false;
...
int CCastleSiege::LoadPreFixData(char * lpszFileName) {
	...
	if (this->m_tmFixCastleCycleStartDate.wYear == 0 || this->m_tmFixCastleCycleStartDate.wMonth == 0 || this->m_tmFixCastleCycleStartDate.wDay == 0) this->m_bFixCastleCycleStartDate = FALSE;
	else {
		...
		} else this->m_bFixCastleCycleStartDate = TRUE;

		if (MONTH_MACRO(this->m_tmFixCastleCycleStartDate.wMonth) == FALSE	|| DAY_MACRO(this->m_tmFixCastleCycleStartDate.wDay) == FALSE) this->m_bFixCastleCycleStartDate = FALSE;
	}

	if (this->m_tmFixCastleStateStartDate.wYear == 0 || this->m_tmFixCastleStateStartDate.wMonth == 0 || this->m_tmFixCastleStateStartDate.wDay == 0) this->m_bFixCastleStateStartDate = FALSE;
	else if (this->m_iFixCastleSpecificState < CASTLESIEGE_STATE_REGSIEGE || this->m_iFixCastleSpecificState > CASTLESIEGE_STATE_ENDCYCLE) this->m_bFixCastleStateStartDate = FALSE;
	else {
		...
		} else this->m_bFixCastleStateStartDate = TRUE;
		...
		if (MONTH_MACRO(this->m_tmFixCastleStateStartDate.wMonth) == FALSE || DAY_MACRO(this->m_tmFixCastleStateStartDate.wDay) == FALSE || HOUR_MACRO(this->m_tmFixCastleStateStartDate.wHour) == FALSE || MINUTE_MACRO(this->m_tmFixCastleStateStartDate.wMinute) == FALSE) this->m_bFixCastleStateStartDate = FALSE;
	}
...
int CCastleSiege::LoadData(char * lpszFileName) {
	...
	if (iCastleSiegeCycle <= 0) {
		...
		return false;
	} else this->m_iCastleSiegeCycle = iCastleSiegeCycle;
	...
	for (pugi::xml_node defense = gates.child("Defense"); defense; defense = defense.next_sibling()) {
		...
		if (iCountDfUpgrade_Gate >= CS_MAX_UPGRADE) break;
	...
	for (pugi::xml_node defense = statues.child("Defense"); defense; defense = defense.next_sibling()) {
		...
		if (iCountDfUpgrade_Statue >= CS_MAX_UPGRADE) break;
	...
	for (pugi::xml_node health = gates.child("Health"); health; health = health.next_sibling()) {
		...
		if (iCountHpUpgrade_Gate >= CS_MAX_UPGRADE) break;
	...
	for (pugi::xml_node health = statues.child("Health"); health; health = health.next_sibling()) {
		...
		if (iCountHpUpgrade_Statue >= CS_MAX_UPGRADE) break;
	...
	for (pugi::xml_node recovery = statuehealthrec.child("Recovery"); recovery; recovery = recovery.next_sibling()) {
		...
		if (iCountRgUpgrade_Statue >= CS_MAX_UPGRADE) break;
	...
	for (pugi::xml_node defense = gates.child("Defense"); defense; defense = defense.next_sibling()) {
		...
		if (iCountDfValue_Gate >= CS_MAX_UPGRADE + 1) break;
	...
	for (pugi::xml_node defense = statues.child("Defense"); defense; defense = defense.next_sibling()) {
		...
		if (iCountDfValue_Statue >= CS_MAX_UPGRADE + 1) break;
	...
	if (!vtScheduleData.empty()) {
		for (std::vector<_CS_SCHEDULE_DATA>::iterator it = vtScheduleData.begin(); it != vtScheduleData.end(); it++) this->m_vtScheduleData.push_back(*it);
	}
	...
	if (!m_vtScheduleData.empty()) {
		for (int iIDX = 0; iIDX < this->m_vtScheduleData.size(); iIDX++) {
			if (iIDX + 1 < this->m_vtScheduleData.size()) {
				...
				if (iIDX_RESULT < 0) g_Log.AddC(TColor::Red, "[CastleSiege] CCastleSiege::LoadData() - Date Order is wrong (sort fail) : %d-%d", this->m_vtScheduleData[iIDX].m_iSTATE, this->m_vtScheduleData[iIDX+1].m_iSTATE);
				else this->m_vtScheduleData[iIDX].m_iGAP_SEC = iIDX_RESULT;
			} else this->m_vtScheduleData[iIDX].m_iGAP_SEC = 0;
		}
	}
	...
	if (!vtNpcData.empty()) {
		for (std::vector<_CS_NPC_DATA>::iterator it = vtNpcData.begin();it != vtNpcData.end(); it++) this->m_vtNpcData.push_back(*it);
	}
...
void CCastleSiege::CS_ClearDbNPC() {
		...
		if (gObj[n].MapNumber == MAP_INDEX_CASTLESIEGE) {
			if (gObj[n].Class == 277 || gObj[n].Class == 283 || gObj[n].Class == 219) {
				if (gObj[n].Class == 277) {
					...
					BUFF_AddBuffEffect(&gObj[n], BUFFTYPE_CASTLE_DOOR_STATE, 0, 0, 0, 0, -10);
				}
...
void CCastleSiege::CS_ClearAllNPC() {
	for (int n = 0; n < g_ConfigRead.server.GetObjectMaxMonster(); n++) if (gObj[n].m_btCsNpcType) gObjDel(n);
}
void CCastleSiege::CS_CreateDbNPC() {
	EnterCriticalSection(&this->m_critNpcData);
	std::vector<_CS_NPC_DATA>::iterator it = this->m_vtNpcData.begin();
	...
	while (it != this->m_vtNpcData.end()) {
		...
		if (pNpcData.m_bIN_USE == 1 && pNpcData.m_iNPC_LIVE == 1) {
			...
			if (iNPC_INDEX >= 0) {
				...
				if (iNPC_DF_LEVEL < 0) iNPC_DF_LEVEL = 0;
				if (iNPC_DF_LEVEL > CS_MAX_UPGRADE) iNPC_DF_LEVEL = CS_MAX_UPGRADE;

				int iNPC_RG_LEVEL = pNpcData.m_iNPC_RG_LEVEL;

				if (iNPC_RG_LEVEL < 0) iNPC_RG_LEVEL = 0;
				if (iNPC_RG_LEVEL > CS_MAX_UPGRADE) iNPC_RG_LEVEL = CS_MAX_UPGRADE;
				...
				if (pNpcData.m_iNPC_NUM == 277) {
					...
					BUFF_AddBuffEffect(&gObj[iNPC_INDEX], BUFFTYPE_CASTLE_DOOR_STATE, 0, 0, 0, 0, -10);
					gObj[iNPC_INDEX].m_btCsGateOpen = 1;

					BOOL bCreateLever = FALSE;

					if (pNpcData.m_iCS_GATE_LEVER_INDEX == -1) bCreateLever = TRUE;
					else if (this->CheckLeverAlive(pNpcData.m_iCS_GATE_LEVER_INDEX) == FALSE) bCreateLever = TRUE;
					if (bCreateLever != FALSE) {
						...
						if (ObjectMaxRange(iLeverIndex)) {
							...
							if (bRETVAL == FALSE) g_Log.AddC(TColor::Red, "[CastleSiege] CCastleSiege::CS_CreateDbNPC() ERROR - Failed to Create Gate Lever : GateIndex:%d",pNpcData.m_iNPC_INDEX);
						} else {
							it->m_iCS_GATE_LEVER_INDEX = -1;
							g_Log.AddC(TColor::Red, "[CastleSiege] CCastleSiege::CS_CreateDbNPC() ERROR - Failed to Create Gate Lever : GateIndex:%d",pNpcData.m_iNPC_INDEX);
						}
					} else {
...
void CCastleSiege::CS_CreateNonDbNPC(BOOL bDelFirst) {
	if (bDelFirst) this->ClearNonDbNPC();
		...
		if (pNpcData.m_bIN_USE == TRUE && pNpcData.m_bNPC_DBSAVE == FALSE) {
			...
			if (iNPC_INDEX >= 0) {
				...
				switch (pNpcData.m_iNPC_SIDE) {
					case 1:				gObj[iNPC_INDEX].m_btCsNpcType = 2;				break;
					case 2:				gObj[iNPC_INDEX].m_btCsNpcType = 3;				break;
					default:			gObj[iNPC_INDEX].m_btCsNpcType = 3;				break;
				}
...
int CCastleSiege::CS_CheckAddDbNPC(int iIndex, int iNpcType, int iNpcIndex, BYTE & btResult) {
	int iRETVAL = 0;
	...
	while (it != this->m_vtNpcData.end()) {
		...
		if (NpcData.m_iNPC_NUM == iNpcType && NpcData.m_iNPC_INDEX == iNpcIndex) {
			...
			switch (NpcData.m_iNPC_NUM) {
				case 277:				iNpcBuyCost = NpcData.m_iNPC_BASE_MAXHP * 5;				break;
				case 283:				iNpcBuyCost = NpcData.m_iNPC_BASE_MAXHP * 3;				break;
			}

			if (gObj[iIndex].CharacterData->Money < iNpcBuyCost) {
				btResult = 3;
				iRETVAL = 0;
				break;
			}
...
int CCastleSiege::CS_AddDbNPC(int iNpcType, int iNpcIndex) {
	int iRETVAL = 0;
	...
	while (it != this->m_vtNpcData.end()) {
		...
		if ((NpcData.m_iNPC_NUM == iNpcType) && (NpcData.m_iNPC_INDEX == iNpcIndex)) {
			...
			if (iObjIndex >= 0) {
				...
				if (iBaseDFLevel < 0) iBaseDFLevel = 0;
				if (iBaseDFLevel > 3) iBaseDFLevel = 3;

				int iBaseRGLevel = NpcData.m_iNPC_BASE_RG_LEVEL;
					
				if (iBaseRGLevel < 0) iBaseRGLevel = 0;
				if (iBaseRGLevel > 3) iBaseRGLevel = 3;
				switch (NpcData.m_iNPC_NUM) {
					case 277:
				...
				if (NpcData.m_iNPC_NUM == 277) {
					...
					BUFF_AddBuffEffect(&gObj[iObjIndex], BUFFTYPE_CASTLE_DOOR_STATE, 0, 0, 0, 0, -10);
					gObj[iObjIndex].m_btCsGateOpen = 1;
						
					BOOL bCreateLever = FALSE;
						
					if (NpcData.m_iCS_GATE_LEVER_INDEX == -1) bCreateLever = TRUE;
					else if (this->CheckLeverAlive(NpcData.m_iCS_GATE_LEVER_INDEX) == FALSE) bCreateLever = TRUE;
					if (bCreateLever != 0) {
						...
						if (ObjectMaxRange(iLevelerIndex)) {
							...
							if (iLinkIndex == 0) ::g_Log.AddC(TColor::Red,  "[CastleSiege] CCastleSiege::CS_CreateDbNPC() ERROR - Failed to Create Gate Lever : GateIndex:%d", NpcData.m_iNPC_INDEX);
						} else {
							it->m_iCS_GATE_LEVER_INDEX = -1;
							::g_Log.AddC(TColor::Red,  "[CastleSiege] CCastleSiege::CS_CreateDbNPC() ERROR - Failed to Create Gate Lever : GateIndex:%d", NpcData.m_iNPC_INDEX);
						}
					} else {
				...
				if (NpcData.m_iNPC_NUM == 277 || NpcData.m_iNPC_NUM == 283) g_Log.Add("[CastleSiege][CreateDBNPC] (Index:%d, Num:%d, DF:%d, RG:%d, MaxHP:%d, HP:%d)", NpcData.m_iNPC_INDEX, NpcData.m_iNPC_NUM, NpcData.m_iNPC_DF_LEVEL, NpcData.m_iNPC_RG_LEVEL, NpcData.m_iNPC_MAXHP, NpcData.m_iNPC_HP);
...
BOOL CCastleSiege::DelNPC(int iIndex, int iNpcType, int iMonsterExistVal, int bDbSave) {
	if (!ObjectMaxRange(iIndex)) return FALSE;
		...
		if (pNpcData->m_iNPC_NUM == iNpcType && pNpcData->m_iNPC_OBJINDEX == iIndex) {
			if (pNpcData->m_iCsNpcExistVal == iMonsterExistVal && pNpcData->m_bIN_USE == TRUE) {
				if (pNpcData->m_iNPC_LIVE > 0) {
					...
					if (pNpcData->m_iNPC_NUM == 277) {
						this->SetGateBlockState(pNpcData->m_iNPC_SX, pNpcData->m_iNPC_SY, 1);
						BUFF_AddBuffEffect(&gObj[pNpcData->m_iNPC_OBJINDEX], BUFFTYPE_CASTLE_DOOR_STATE, 0, 0, 0, 0, -10);
	...
	if (bEXIST == TRUE && bDbSave == TRUE && this->m_iCastleSiegeState != CASTLESIEGE_STATE_STARTSIEGE) GS_GDReqNpcRemove(this->m_iMapSvrGroup, pTempNpcData.m_iNPC_NUM, pTempNpcData.m_iNPC_INDEX);

	return TRUE;
}
BOOL CCastleSiege::CS_RepairDbNPC(int iNpcType, int iNpcIndex, int iNpcHP, int iNpcMaxHP) {
	BOOL bEXIST = FALSE;
...
int CCastleSiege::CS_PayForUpgradeDbNpc(int iIndex, int iNpcType, int iNpcIndex, int iNpcUpType, int iNpcUpValue, int iNpcUpIndex) {
	int iNEED_GEMOFDEFEND = 0;
	int iNEED_MONEY = 0;

	if (CS_CHECK_UPGRADE(iNpcUpIndex) == FALSE) {
		g_Log.AddC(TColor::Red,  "[CastleSiege] CCastleSiege::CS_PayForUpgradeDbNpc() ERROR - lpMsg->iNpcUpIndex is Out of Bound : %d", iNpcUpIndex);
		return FALSE;
	}
	...
	if (bIsLive == FALSE) {
		g_Log.AddC(TColor::Red,  "[CastleSiege] CCastleSiege::CS_PayForUpgradeDbNpc() ERROR - bIsLive == FALSE", iNpcUpIndex);
		return FALSE;
	}
		...
		case 277:
			switch (iNpcUpType) {
				case 1: {
					...
					gObj[iIndex].CharacterData->Money -= iNEED_MONEY;
				}
				break;
...
BOOL CCastleSiege::CS_UpgradeDbNPC(int iIndex, int iNpcType, int iNpcIndex, int iNpcUpType, int iNpcUpValue, int iNpcUpIndex) {
	if (CS_CHECK_UPGRADE(iNpcUpIndex) == FALSE) {
		g_Log.AddC(TColor::Red,  "[CastleSiege] CCastleSiege::CS_UpgradeDbNPC() ERROR - lpMsg->iNpcUpIndex is Out of Bound : %d", iNpcUpIndex);
		return FALSE;
	}
	...
	if (bIsLive == FALSE) {
		g_Log.AddC(TColor::Red,  "[CastleSiege] CCastleSiege::CS_UpgradeDbNPC() ERROR - bIsLive == FALSE", iNpcUpIndex);
		return FALSE;
	}
	...
	switch (iNpcType) {
		case 277:
			switch (iNpcUpType) {
				case 1:					this->UpgradeDbNPC_DFLEVEL(iNpcType, iNpcIndex, iNpcUpValue);										break;
				case 3:					this->UpgradeDbNPC_MAXHP(iNpcType, iNpcIndex, iNpcUpValue);											break;
				default:				GSProtocol.GCAnsNpcUpgrade(iIndex, 5, iNpcType, iNpcIndex, iNpcUpType, iNpcUpValue);
										return FALSE;																						break;
			}
		break;
		...
		case 283:
			switch (iNpcUpType) {
				case 1:					this->UpgradeDbNPC_DFLEVEL(iNpcType, iNpcIndex, iNpcUpValue);										break;
				case 2:					this->UpgradeDbNPC_RGLEVEL(iNpcType, iNpcIndex, iNpcUpValue);										break;
				case 3:					this->UpgradeDbNPC_MAXHP(iNpcType, iNpcIndex, iNpcUpValue);											break;
				default:				GSProtocol.GCAnsNpcUpgrade(iIndex, 5, iNpcType, iNpcIndex, iNpcUpType, iNpcUpValue);	
										return FALSE;																						break;
			}
...
void CCastleSiege::CS_StoreDbNpc() {
	g_Log.Add("[CastleSiege] CCastleSiege::CS_StoreDbNpc() - << START >>");
	...
	while (it != this->m_vtNpcData.end()) {
		...
		if (pNpcData.m_bNPC_DBSAVE == TRUE  && gObjIsConnected(pNpcData.m_iNPC_OBJINDEX) && gObj[pNpcData.m_iNPC_OBJINDEX].Class == pNpcData.m_iNPC_NUM && gObj[pNpcData.m_iNPC_OBJINDEX].m_iCsNpcExistVal == pNpcData.m_iCsNpcExistVal && gObj[pNpcData.m_iNPC_OBJINDEX].m_iCsNpcExistVal != NULL && gObj[pNpcData.m_iNPC_OBJINDEX].Live == TRUE && gObj[pNpcData.m_iNPC_OBJINDEX].Life > 0) {
			...
			g_Log.Add("[CastleSiege] CCastleSiege::CS_StoreDbNpc() NPC INFO (CLS:%d, IDX:%d(%d), MAXHP:%d, HP:%d, DF:%d, RG:%d)", pNpcData.m_iNPC_NUM,pNpcData.m_iNPC_INDEX,pNpcData.m_iNPC_OBJINDEX,lpMsgBody[iCOUNT].iNpcMaxHp,lpMsgBody[iCOUNT].iNpcHp,lpMsgBody[iCOUNT].iNpcDfLevel,lpMsgBody[iCOUNT].iNpcRgLevel);
			iCOUNT++;
		}
	...
	g_Log.Add("[CastleSiege] CCastleSiege::CS_StoreDbNpc() - << END >>");
}
			...
			if ((NpcData.m_iNPC_NUM == 277 || NpcData.m_iNPC_NUM == 283) && NpcData.m_iNPC_OBJINDEX != OBJ_EMPTY)
			{
				if (!gObjIsConnected(NpcData.m_iNPC_OBJINDEX) || gObj[NpcData.m_iNPC_OBJINDEX].m_iCsNpcExistVal != NpcData.m_iCsNpcExistVal || gObj[NpcData.m_iNPC_OBJINDEX].m_iCsNpcExistVal == 0 || (gObj[NpcData.m_iNPC_OBJINDEX].Class != 277 && gObj[NpcData.m_iNPC_OBJINDEX].Class != 283) || gObj[NpcData.m_iNPC_OBJINDEX].Live == FALSE) //HermeX Fix
				{
					...
					switch(NpcData.m_iNPC_NUM)
					{
						case 277:
							this->SetGateBlockState(NpcData.m_iNPC_SX, NpcData.m_iNPC_SY, 1);
							BUFF_AddBuffEffect(&gObj[NpcData.m_iNPC_OBJINDEX], BUFFTYPE_CASTLE_DOOR_STATE, 0, 0, 0, 0, -10);
...
int CCastleSiege::DelGemOfDefend(int iIndex, int iNeedGemOfDefend)
{
	...
	for (int x = 0; x < INVENTORY_SIZE; x++)
	{
		if (gObj[iIndex].pInventory[x].IsItem() == TRUE)
		{
			if (gObj[iIndex].pInventory[x].m_Type == ITEMGET(14,31))
			{
				g_Log.Add("[CastleSiege] [%s][%s] Deleting Gem of Defend (GUILD:%s) - Serial:%u", gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].CharacterData->GuildName, gObj[iIndex].pInventory[x].m_Number);

				gObjInventoryItemSet(iIndex, x, 0xff);
				gObjInventoryDeleteItem(iIndex, x);
				GSProtocol.PROTOCOL_InventoryItemDeleteSend(iIndex, x, 1);
...
void CCastleSiege::SetAllUserCsJoinSide()
{
	...
	for (int iUSER = g_ConfigRead.server.GetObjectStartUserIndex(); iUSER < g_ConfigRead.server.GetObjectMax(); iUSER++)
	{
		...
		memcpy(szGuildName,gObj[iUSER].CharacterData->GuildName,MAX_GUILD_LEN);
...
void CCastleSiege::NotifySelfCsJoinSide(int iIndex)
{
	...
	case 1:
		...
		{
			BUFF_AddBuffEffect(lpObj, BUFFTYPE_CASTLE_DEFENSE_MARK, 0, 0, 0, 0, -10);
		}
		else
		{
			BUFF_RemoveBuffEffect(lpObj, BUFFTYPE_CASTLE_DEFENSE_MARK);
		}
		break;
	case 2:
		iState = 1;
		...
		if (this->m_iCastleSiegeState == CASTLESIEGE_STATE_STARTSIEGE || this->m_iCastleSiegeState == CASTLESIEGE_STATE_ENDSIEGE)
		{
			BUFF_AddBuffEffect(lpObj, BUFFTYPE_CASTLE_ATTACK_MARK, 0, 0, 0, 0, -10);
		}
		else
		{
			BUFF_RemoveBuffEffect(lpObj, BUFFTYPE_CASTLE_ATTACK_MARK);
		}
		break;
	case 3:
		iState = 1;
		...
		if (this->m_iCastleSiegeState == CASTLESIEGE_STATE_STARTSIEGE || this->m_iCastleSiegeState == CASTLESIEGE_STATE_ENDSIEGE)
		{
			BUFF_AddBuffEffect(lpObj, BUFFTYPE_CASTLE_ATTACK_MARK_2, 0, 0, 0, 0, -10);
		}
		else
		{
			BUFF_RemoveBuffEffect(lpObj, BUFFTYPE_CASTLE_ATTACK_MARK_2);
		}
		break;
	case 4:
		iState = 1;
		...
		if (this->m_iCastleSiegeState == CASTLESIEGE_STATE_STARTSIEGE || this->m_iCastleSiegeState == CASTLESIEGE_STATE_ENDSIEGE)
		{
			BUFF_AddBuffEffect(lpObj, BUFFTYPE_CASTLE_ATTACK_MARK_3, 0, 0, 0, 0, -10);
		}
		else
		{
			BUFF_RemoveBuffEffect(lpObj, BUFFTYPE_CASTLE_ATTACK_MARK_3);
		}
		break;
	...
	while (it != this->m_vtNpcData.end())
	{
		...
		if (NpcData.m_iNPC_NUM == 277 && NpcData.m_iNPC_OBJINDEX == iObjIndex && NpcData.m_bIN_USE == TRUE && NpcData.m_iNPC_LIVE == 2)
		{
			...
			if (gObj[iObjNumber].m_btCsNpcType != 0 && iMonsterExistVal == NpcData.m_iCsNpcExistVal)
			{
				...
				BUFF_AddBuffEffect(&gObj[iObjNumber], BUFFTYPE_CASTLE_DOOR_STATE, 0, 0, 0, 0, -10);
	...
	while (it != this->m_vtNpcData.end())
	{
		...
		if (NpcData.m_iNPC_NUM == 277 && NpcData.m_iNPC_INDEX == iNpcIndex && NpcData.m_bIN_USE == TRUE && NpcData.m_iNPC_LIVE == 2)
		{
			...
			if (gObjIsConnected(iObjNumber) && gObj[iObjNumber].m_btCsNpcType != 0 && gObj[iObjNumber].m_iCsNpcExistVal == NpcData.m_iCsNpcExistVal && gObj[iObjNumber].m_iCsNpcExistVal != 0)
			{
					...
					{
						BUFF_RemoveBuffEffect(&gObj[iObjNumber], BUFFTYPE_CASTLE_DOOR_STATE);
					}
					else
					{
						BUFF_AddBuffEffect(&gObj[iObjNumber], BUFFTYPE_CASTLE_DOOR_STATE, 0, 0, 0, 0, -10);
...
BOOL CCastleSiege::CheckCastleOwnerMember(int iIndex)
{
	...
	if (strcmp(gObj[iIndex].CharacterData->GuildName, this->m_szCastleOwnerGuild) != 0)
	{
		return FALSE;
	}
...
BOOL CCastleSiege::CheckCastleOwnerUnionMember(int iIndex)
{
	...
	lpGuildInfo = gObj[iIndex].CharacterData->lpGuild;
...
//		gObj[iUSER].m_ViewSkillState &= 0xFF7FFFFF; //HermeX Fix
		...
		memcpy(&szNewGuild,gObj[iUSER].CharacterData->GuildName,MAX_GUILD_LEN);
...
BOOL CCastleSiege::CheckMiddleWinnerGuild()
{
	...
	if ((strcmp(gObj[this->m_iCastleCrownAccessUser].CharacterData->GuildName, "") == 0) || (strcmp(gObj[this->m_iCastleSwitchAccessUser1].CharacterData->GuildName, "") == 0) || (strcmp(gObj[this->m_iCastleSwitchAccessUser2].CharacterData->GuildName, "") == 0))
	{
		return FALSE;
	}
	...
	if ((gObj[this->m_iCastleCrownAccessUser].m_btCsJoinSide >= 2) && (gObj[this->m_iCastleSwitchAccessUser1].m_btCsJoinSide >= 2) && (gObj[this->m_iCastleSwitchAccessUser2].m_btCsJoinSide >= 2))
	{
		if (gObj[this->m_iCastleCrownAccessUser].m_bCsGuildInvolved == 1)
		{
			if ((gObj[this->m_iCastleCrownAccessUser].m_btCsJoinSide == gObj[this->m_iCastleSwitchAccessUser1].m_btCsJoinSide) && (gObj[this->m_iCastleCrownAccessUser].m_btCsJoinSide == gObj[this->m_iCastleSwitchAccessUser2].m_btCsJoinSide))
			{
				...
				{
					g_Log.Add("[CastleSiege] Castle Winner Has been Changed (GUILD:%s)", gObj[this->m_iCastleCrownAccessUser].CharacterData->GuildName);
					...
					memset(&this->m_szMiddleWinnerGuild, 0, sizeof(this->m_szMiddleWinnerGuild));
					memcpy(&this->m_szMiddleWinnerGuild, gObj[this->m_iCastleCrownAccessUser].CharacterData->GuildName, 8);

					this->ClearNonDbNPC_MidWin();
					this->ChangeWinnerGuild(gObj[this->m_iCastleCrownAccessUser].m_btCsJoinSide);
					this->NotifyAllUserCsProgState(1, gObj[this->m_iCastleCrownAccessUser].CharacterData->GuildName);
...
BOOL CCastleSiege::CheckUnionGuildMaster(int iIndex)
{
	...
	if (gObj[iIndex].CharacterData->GuildStatus == GUILD_MASTER)
	{
...
void CCastleSiege::ReSpawnAllUser()
{
	...
	for (int iUSER = g_ConfigRead.server.GetObjectStartUserIndex(); iUSER < g_ConfigRead.server.GetObjectMax(); iUSER++)
	{
		...
		if (bMoveOK)
		{
			...
			GSProtocol.PROTOCOL_TeleportSend(&gObj[iUSER], -1, MAP_INDEX_CASTLESIEGE, gObj[iUSER].X, gObj[iUSER].Y, gObj[iUSER].Dir);
	...
	for (int iUSER = g_ConfigRead.server.GetObjectStartUserIndex(); iUSER < g_ConfigRead.server.GetObjectMax(); iUSER++)
	{
		...
		case 1:
			if (bRefreshOwnerUser != FALSE)
			{
				...
				GSProtocol.PROTOCOL_TeleportSend(&gObj[iUSER],-1,gObj[iUSER].MapNumber,gObj[iUSER].X,gObj[iUSER].Y,gObj[iUSER].Dir); //season 2.5 changed
	...
	while (it != m_vtNpcData.end())
	{
		...
		if (pNpcData.m_iNPC_NUM == 277 && gObjIsConnected(pNpcData.m_iNPC_OBJINDEX) && gObj[pNpcData.m_iNPC_OBJINDEX].Class == 277)
		{
			...
			case 0:
				...
				BUFF_RemoveBuffEffect(&gObj[pNpcData.m_iNPC_OBJINDEX], BUFFTYPE_CASTLE_DOOR_STATE);
...
void CCastleSiege::OperateMiniMapWork()
{
	...
	for (int iIDX = g_ConfigRead.server.GetObjectStartUserIndex(); iIDX < g_ConfigRead.server.GetObjectMax(); iIDX++)
	{
		...
		if (it3 != this->m_mapMiniMapData.end())
		{
			...
			if (gObj[iIDX].CharacterData->GuildStatus == GUILD_MASTER && strcmp(gObj[iIDX].CharacterData->GuildName, "") != 0 && strcmp(gObj[iIDX].CharacterData->GuildName, it3->second.m_szGuildName) == 0)
			{
...
void CCastleSiege::NotifyCrownState(BYTE btState)
{
	...
	{
		BUFF_AddBuffEffect(&gObj[m_iCrownIndex], BUFFTYPE_CASTLE_CROWN_STATE, 0, 0, 0, 0, -10);
	}
	else
	{
		BUFF_RemoveBuffEffect(&gObj[m_iCrownIndex], BUFFTYPE_CASTLE_CROWN_STATE);
	}
...
void CCastleSiege::NotifyCrownSwitchInfo(int iCrownSwitchIndex)
{
	...
	if (gObjIsConnected(iCrownSwitchUserIndex))
	{
		...
		memcpy(&pMsg.szGuildName, lpSwitchObj->CharacterData->GuildName, MAX_GUILD_LEN);
		memset(&szMasterGuildName, 0, MAX_GUILD_LEN);

		if (lpSwitchObj->CharacterData->lpGuild)
		{
			if (UnionManager.GetUnionName(lpSwitchObj->CharacterData->lpGuild->iGuildUnion, szMasterGuildName) != -1)
			{
