...
/*	GS-N	1.00.18	JPN	0x004CF4C0	-	Completed
	void COMPANION_DurabilityDecrease(LPOBJ lpObj, int damage)	-	Wierd Compilation - Status OK
...
inline bool ObjectMaxRange(int Index) {
	if (Index < 0 || Index > g_ConfigRead.server.GetObjectMax() - 1) return false;
...
void USER_DATA::Init(bool VipReset) {
	...
	this->MoveGate = -1;
	g_kJewelOfHarmonySystem.InitEffectValue(&this->JewelOfHarmonyEffect);
	g_kItemSystemFor380.InitEffectValue(&this->PvPEquipmentMods);
	this->m_iResets = 0;
	this->LastAuthTime = 0;	

	if (g_ConfigRead.server.GetServerType() != SERVER_CASTLE) this->m_bKanturuEntranceByNPC = FALSE;
	for (int x = 0; x <= 6; x++) this->ANCIENT_AttributeDamage[x] = 0;

	this->ANCIENT_MaxPhysicalDamage = 0;
	this->ANCIENT_MinPhysicalDamage = 0;
	this->ANCIENT_PhysicalDamage = 0;
	this->ANCIENT_StaminaRegeneration = 0;
	this->ANCIENT_CriticalStrikeChance = 0;
	this->ANCIENT_CriticalDamage = 0;
	this->ANCIENT_ExcellentStrikeChance = 0;
	this->ANCIENT_ExcellentDamage = 0;
	this->ANCIENT_SkillDamage = 0;	
	this->ANCIENT_StunDuration = 0;
	this->ANCIENT_Defense = 0;
	this->ANCIENT_SpellDamage = 0;
	this->ANCIENT_BurnDamage = 0;
	this->ANCIENT_ElementalAilmentsDuration = 0;
	this->ANCIENT_FatalStrikeChance = 0;
	this->ANCIENT_LethalStrikeChance = 0;
	this->ANCIENT_TwoHandedWeaponDamage = 0;
	this->ANCIENT_HitRating = 0;
	this->ANCIENT_BurnChance = 0;
	this->ANCIENT_DefenseWithShields = 0;
	this->ANCIENT_DualWieldDamage = 0;
	this->ANCIENT_ItemDropRate = 0;
	...
	for (int i = 0; i < 5; i++) this->DarkSideTarget[i] = 10000;	
	for (int i = 0; i < 250; i++) this->m_PentagramJewelInfo_Inven[i].Clear();
	for (int i = 0; i < 250; i++) this->m_PentagramJewelInfo_Warehouse[i].Clear();

	this->m_bUserQuestInfoSent = FALSE;

	for (int i = 0; i < MAX_QUESTEXP_USER_INFO; i++) this->m_UserQuestInfo[i].Clear();
	...
	this->C_FatalStrikeResistance = 0;
	this->C_ShieldPenetrationResistance = 0;
	this->C_StunResistance = 0;
	this->C_LethalStrikeResistance = 0;
	this->C_CriticalStrikeResistance = 0;
	this->C_ExcellentStrikeResistance = 0; 
	...
	for (int i = 0; i < MAX_CANCEL_ITEMS_SALE; i++)	this->m_CancelItemSaleList[i].Clear();
	for (int i = 0; i < MAX_GREMORYCASE_STORAGE_TYPES; i++)	{
		for (int j = 0; j < MAX_GREMORYCASE_STORAGE_ITEMS; j++)	this->m_GremoryCaseData[i][j].Clear();
	}
...
void gObjSkillUseProc(LPOBJ lpObj) {
	...
	if (lpObj->m_SkillInfo.LordSummonTime != 0 && lpObj->Type == OBJ_USER) {
		...
		if (lpObj->m_SkillInfo.LordSummonTime <= 0) {
			...
			} else gObjRecall(lpObj->m_Index,lpObj->m_SkillInfo.LordSummonMapNumber,lpObj->m_SkillInfo.LordSummonX,lpObj->m_SkillInfo.LordSummonY);
	...
	if (IT_MAP_RANGE(lpObj->MapNumber)) {
		if (lpObj->m_nITR_Index != -1) g_IT_Event.EventSkillProc(lpObj);
	}

	if (lpObj->m_SkillNumber < 1) return;
	if (lpObj->m_SkillNumber == 18) {
		if (GetTickCount() > lpObj->m_SkillTime) lpObj->m_SkillNumber = 0;
	}
}
...
void gObjSkillUseProcTime500(LPOBJ lpObj) {
	...
	if (lpObj->CharacterData->SkillHellFire2State != 0) {
		...
		lpObj->CharacterData->SkillHellFire2Count = (dwCurrentTick - lpObj->CharacterData->SkillHellFire2Time) / 500;

		if (lpObj->CharacterData->SkillHellFire2Count > 12) lpObj->CharacterData->SkillHellFire2Count = 12;
		if (dwCurrentTick >= (lpObj->CharacterData->SkillHellFire2Time + 6000)) {
			CMagicInf * lpMagic = PLAYER_GetMagicSearch(lpObj,40);

			if (lpMagic) gObjUseSkill.RunningSkill(lpObj->m_Index, 0, lpMagic, 0);
		} else {
			int mana = MagicDamageC.MAGIC_GetManaUsage(40);
			mana = mana * 20 / 100;
			BOOL bSuccess = TRUE;
			...
			if (mana > lpObj->Mana)	{
				...
				while (true) {
					...
					GSProtocol.PROTOCOL_UseItemRecv(&pMsg, lpObj->m_Index);
			...
			if (bSuccess == FALSE) {
				...
				if (lpObj->Mana < 0.0f)	lpObj->Mana = 0.0f;

				GSProtocol.PROTOCOL_ManaSend(lpObj->m_Index, lpObj->Mana, -1, 0, lpObj->Stamina);

				PMSG_EX_SKILL_COUNT pMsg;

				PHeadSetB((LPBYTE)&pMsg, 0xBA, sizeof(pMsg));
				pMsg.Type = 40;
				pMsg.Count = lpObj->CharacterData->SkillHellFire2Count;
	...
	if (lpObj->CharacterData->SkillStrengthenHellFire2State != 0) {
		...
		lpObj->CharacterData->SkillStrengthenHellFire2Count = (dwCurrentTick - lpObj->CharacterData->SkillStrengthenHellFire2Time) / 500;

		if (lpObj->CharacterData->SkillStrengthenHellFire2Count > 12) lpObj->CharacterData->SkillStrengthenHellFire2Count = 12;
		if (dwCurrentTick >= (lpObj->CharacterData->SkillStrengthenHellFire2Time + 6000)) {
			CMagicInf * lpMagic = PLAYER_GetMagicSearch(lpObj, 392);

			if (lpMagic) gObjUseSkill.RunningSkill(lpObj->m_Index, 0, lpMagic, 0);
		} else {
			int mana = MagicDamageC.MAGIC_GetManaUsage(392);
			BOOL bSuccess = TRUE;
			...
			if (bSuccess == FALSE) {
				...
				PHeadSetB((LPBYTE)&pMsg, 0xBA, sizeof(pMsg));
				pMsg.Type = 392;
				pMsg.Count = lpObj->CharacterData->SkillStrengthenHellFire2Count;
				pMsg.NumberH = SET_NUMBERH(lpObj->m_Index);
				pMsg.NumberL = SET_NUMBERL(lpObj->m_Index);
				IOCP.DataSend(lpObj->m_Index, (UCHAR *)&pMsg, pMsg.h.size);
				GSProtocol.MsgSendV2(lpObj, (UCHAR *)&pMsg, pMsg.h.size);
	...
	for (n = 0; n < g_ConfigRead.server.GetObjectMax(); n++) {
		...
		if (lpObj->Connected == PLAYER_PLAYING) {
			if (lpObj->Type == OBJ_MONSTER || lpObj->Type == OBJ_NPC) {
				if (lpObj->m_iCurrentAI != 0) continue;
			...
			if (lpObj->Type == OBJ_USER && lpObj->CharacterData->ISBOT == false && n >= g_ConfigRead.server.GetObjectStartUserIndex()) gDarkSpirit[n- g_ConfigRead.server.GetObjectStartUserIndex()].Run();
		} else if (lpObj->Connected >= PLAYER_LOGGED) {
			if (lpObj->Type == OBJ_USER) {
				gObjMsgProc(lpObj);

				if (lpObj->Connected == PLAYER_PLAYING)	CreateFrustrum(lpObj->X, lpObj->Y, n);
			}
		} else gObjMsgProc(lpObj);
	}
...
void MoveMonsterProc() {
	...
	for(int n = 0; n < g_ConfigRead.server.GetObjectMax(); n++)	{
		...
		if (lpObj->m_iCurrentAI != 0) {
			if (lpObj->Type == OBJ_MONSTER || lpObj->Type == OBJ_NPC) continue;
		}
		
		if (DG_MAP_RANGE(gObj[n].MapNumber)) continue;
		if (lpObj->Connected == PLAYER_PLAYING) {
			...
			if (lpObj->m_State == 2) {
				if (lpObj->PathCount != 0) {
					if (lpObj->DelayLevel != 0) DelayTime = 300;
					else DelayTime = 0;	
					if (lpObj->Type == OBJ_MONSTER && lpObj->m_RecallMon >= 100) lpObj->m_MoveSpeed = 200;
					else lpObj->m_MoveSpeed = 400;	
					if (lpObj->PathDir[lpObj->PathCur] % 2 == 0) MoveTime = (lpObj->m_MoveSpeed + DelayTime)*(double)1.3;
					else MoveTime = lpObj->m_MoveSpeed + DelayTime;	
					if ((GetTickCount() - lpObj->PathTime) > MoveTime && lpObj->PathCur < 14) {
						...
						if (lpObj->Type == OBJ_USER && ((attr & 4) == 4 || (attr & 8) == 8))	{
							g_Log.Add("[ CHECK POSITION ] MoveMosterProc [%s][%s] Map[%d]-(%d,%d) User(%d,%d) Can not Move Position Attr[%d]", lpObj->AccountID,lpObj->Name,lpObj->MapNumber,nextX,nextY,lpObj->X,lpObj->Y,attr);
							for(int n = 0; n < 15; n++) {
								lpObj->PathX[n] = 0;
								lpObj->PathY[n] = 0;
								lpObj->PathOri[n] = 0;
							...
							if (lpObj->Type == OBJ_MONSTER || lpObj->Type == OBJ_NPC) lpObj->PathStartEnd = 0;	
							if (lpObj->Type == OBJ_USER) CHARACTER_Immobilize(lpObj->m_Index,lpObj->X,lpObj->Y);
						} else {
...
void gObjInit() {
	try {
		if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) MaxViewportMonster = 75;
		...
		for (int n = 0; n < g_ConfigRead.server.GetObjectMax(); n++) {
			...
			if (n >= g_ConfigRead.server.GetObjectStartUserIndex()) {
				gObj[n].CharacterData = new USER_DATA();
			...
			if (n >= g_ConfigRead.server.GetObjectStartUserIndex()) gObj[n].PerSocketContext = new _PER_SOCKET_CONTEXT;
...
void gObjEnd() {
	...
	for (int n = 0; n < g_ConfigRead.server.GetObjectMax(); n++) {
		...
		if (gObj[n].CharacterData != NULL) {
			delete gObj[n].CharacterData;
			gObj[n].CharacterData = NULL;
		}
...
void gObjCloseSet(int aIndex, int Flag) {
	if (aIndex < 0 || aIndex > g_ConfigRead.server.GetObjectMax() - 1) return;
	...
	if (lpObj->Connected == PLAYER_PLAYING)	{
		...
		if (BC_MAP_RANGE(lpObj->MapNumber)) g_BloodCastle.SearchUserDropQuestItem(aIndex);
		if (IT_MAP_RANGE(lpObj->MapNumber))	{
		...
		if (lpObj->MapNumber == MAP_INDEX_DEVILSQUARE_FINAL) g_DevilSquareFinal.Leave_DSF(lpObj->m_Index);
		if (DG_MAP_RANGE(lpObj->MapNumber)) {
			g_DoppelGanger.LeaveDoppelganger(lpObj->m_Index);

			if (g_DoppelGanger.GetDoppelgangerState() == 2)	g_DoppelGanger.SendDoppelgangerResult(lpObj, 1);
		}
		...
		if ((lpObj->m_IfState.use != 0) && ((lpObj->m_IfState.type == 1) || (lpObj->m_IfState.type == 6) || (lpObj->m_IfState.type == 13) || (lpObj->m_IfState.type == 7))) {
			...
			switch (lpObj->m_IfState.type) {
				case 1:					GSProtocol.GCServerMsgStringSend(Lang.GetText(0,43), lpObj->m_Index, 1);					break;
				case 6:					GSProtocol.GCServerMsgStringSend(Lang.GetText(0,44), lpObj->m_Index, 1);					break;
				case 7:					GSProtocol.GCServerMsgStringSend(Lang.GetText(0,59), lpObj->m_Index, 1);					break;
				case 13:				GSProtocol.GCServerMsgStringSend(Lang.GetText(0,145), lpObj->m_Index, 1);					break;
			}
...
void gObjCharTradeClear(LPOBJ lpObj) {
	if (lpObj->Type != OBJ_USER) return;
	if (lpObj->CharacterData->ISBOT == true && lpObj->CharacterData->bt_BotType != 1) return;

	memset(lpObj->TradeMap, 0xFF , TRADE_BOX_SIZE);

	for (int i=0 ; i< TRADE_BOX_SIZE ; i++)	lpObj->Trade[i].Clear();
...
void gObjCharZeroSet(int aIndex) {
	...
	if (lpObj->Type == OBJ_USER) {
		...
		g_MasterLevelSkillTreeSystem.ML_InitPassiveSkillData(lpObj);
	}
	...
	if (g_ConfigRead.antihack.MainCRC != 0) lpObj->CrcCheckTime = GetTickCount();
	else lpObj->CrcCheckTime = 0 ;
	...
	lpObj->m_PostCMDLastUseTick = GetTickCount()-10000;
	gObjClearBuffEffect(lpObj, CLEAR_TYPE_LOGOUT);

	if (lpObj->Type == OBJ_USER && (lpObj->CharacterData->ISBOT == false && lpObj->CharacterData->bt_BotType != 1)) {
	...
	for (int i = 0; i < MAX_MAGIC; i++)	lpObj->Magic[i].Clear();

	lpObj->MagicCount = 0;

	for (int i = 0; i < MAX_MONSTER_SEND_MSG; i++) gSMMsg[lpObj->m_Index][i].MsgCode = -1;
	...
	if ((lpObj->m_Index- g_ConfigRead.server.GetObjectStartUserIndex()) >= 0) {
		for (int i = 0; i < MAX_MONSTER_SEND_ATTACK_MSG; i++) gSMAttackProcMsg[lpObj->m_Index- g_ConfigRead.server.GetObjectStartUserIndex()][i].MsgCode = -1;
	}
	...
	if (lpObj->Type == OBJ_USER && (lpObj->CharacterData->ISBOT == false && lpObj->CharacterData->bt_BotType != 1)) {
		for (int i = 0; i < WAREHOUSE_SIZE; i++) lpObj->pWarehouse[i].Clear();
		for (int i = 0; i < CHAOS_BOX_SIZE; i++) lpObj->pChaosBox[i].Clear();
	}

	::gMultiAttackHackCheck[lpObj->m_Index].Init();
	lpObj->pTransaction = 0;
	::gObjMonsterHitDamageInit(lpObj);
	::INVENTORY_SetPointer1(lpObj);
	::gObjSetEventInventory1Pointer(lpObj);
	::gObjSetMuunInventory1Pointer(lpObj);

	if (lpObj->Type == OBJ_USER && (lpObj->CharacterData->ISBOT == false && lpObj->CharacterData->bt_BotType != 1)) memset(lpObj->pInventoryMap, (BYTE)-1, INVENTORY_MAP_SIZE);

	gObjCharTradeClear(lpObj);

	if (lpObj->Type == OBJ_USER && (lpObj->CharacterData->ISBOT == false && lpObj->CharacterData->bt_BotType != 1)) {
		for (int i = 0; i < WAREHOUSE_SIZE; i++) lpObj->pWarehouse[i].Clear();
		for (int i = 0; i < INVENTORY_SIZE; i++) lpObj->Inventory1[i].Clear();
		for (int i = 0; i < INVENTORY_SIZE; i++) lpObj->Inventory2[i].Clear();
		for (int i = 0; i < TRADE_BOX_SIZE; i++) lpObj->Trade[i].Clear();
		...
		for (int i = 0; i < 2; i++)	lpObj->m_MuunEffectList[i].Clear();
		...
		CMuRummyInfo * pCMuRummyInfo = lpObj->CharacterData->m_pCMuRummyInfo;

		if (!pCMuRummyInfo)	g_Log.Add("[MuRummy][Error] pCMuRummyInfo is NULL [%s][%s] [%s, %d]", lpObj->AccountID, lpObj->Name, __FILE__, __LINE__);
		else pCMuRummyInfo->Clear();
		...
		lpObj->bSubEquip = 0;
		lpObj->m_nDSFIndex = -1;
		lpObj->CharacterData->Init(false);
	}
	...
	for (int k = 0; k < 3; k++)	lpObj->m_nRecallMonIndex[k] = -1;
...
int gObjGetSocket(SOCKET socket) {
	for (int n = g_ConfigRead.server.GetObjectStartUserIndex(); n < g_ConfigRead.server.GetObjectMax(); n++)	{
		if (gObj[n].Connected != PLAYER_EMPTY) {
			if (gObj[n].m_socket == socket)	return n;
		}
	}
...
int gObjGetHWIDUseCount(LPTSTR HWID) {
	...
	for (int n = g_ConfigRead.server.GetObjectStartUserIndex(); n < g_ConfigRead.server.GetObjectMax(); n++) {
		if (gObj[n].Connected != PLAYER_EMPTY) {
			if (strcmp(gObj[n].CharacterData->HWID, HWID) == 0) {
				if (gObj[n].m_bOff == false) count++;
			}
		}
	}
...
bool IsDuelEnable(int aIndex) {
	...
	if (gObjIsConnected(aIndex) == TRUE) {
		if ((gObj[aIndex].m_Option&2) == 2) return true;
	}
...
void gObjSetDuelOption(int aIndex, int option) {
	if (gObjIsConnected(aIndex) == TRUE) {
		if (option == 0) gObj[aIndex].m_Option = 0;
		else gObj[aIndex].m_Option |= 2;
	}
}
...
void DbItemSetInByte(LPOBJ lpObj, struct SDHP_DBCHAR_INFORESULT* lpMsg, bool* bAllItemExist) {
	...
	for(int n = 0; n < INVENTORY_SIZE;n++) {
		...
		if (lpMsg->dbInventory[n*MAX_DBITEM_INFO] == 0xFF && (lpMsg->dbInventory[n*MAX_DBITEM_INFO+DBI_NOPTION_DATA] & 0x80) == 0x80 && (lpMsg->dbInventory[n*MAX_DBITEM_INFO+DBI_OPTION380_DATA] & 0xF0) == 0xF0) continue;		
		...
		if (lpMsg->dbInventory[n*MAX_DBITEM_INFO] == 0xFF && (lpMsg->dbInventory[n*MAX_DBITEM_INFO+DBI_NOPTION_DATA] & 0x80) == 0x80 && (lpMsg->dbInventory[n*MAX_DBITEM_INFO+DBI_OPTION380_DATA] & 0xF0) == 0xF0) itype = -1;
		if (_type == -1) continue;
		if (IsItem(_type) == false) {
			bIsItemExist = false;

			if (bAllItemExist != 0) *bAllItemExist = 0;
		}

		if (bIsItemExist == false) {
			lpObj->pInventory[n].m_bItemExist = 0;			
			g_Log.Add("error-L2 : Unknown Item found [%s][%s] (Type:%d, LEV:%d, DUR:%d OP1:%d, OP2:%d, OP3:%d, NEWOP:%d, SET:%d)", lpObj->AccountID,lpObj->Name,lpObj->pInventory[n].m_Type,lpObj->pInventory[n].m_Level, lpObj->pInventory[n].m_Durability,lpObj->pInventory[n].m_Option1, lpObj->pInventory[n].m_Option2,lpObj->pInventory[n].m_Option3, lpObj->pInventory[n].m_NewOption,lpObj->pInventory[n].m_SetOption);
			lpObj->pInventory[n].Clear();
			continue;
		}

		if (_type == ITEMGET(14,17) || _type == ITEMGET(14,18)) {
			if (lpMsg->dbInventory[n * MAX_DBITEM_INFO+DBI_DUR] == 0) itype = -1;
		}
		...
		if (_type == ITEMGET(14,19)) {
			if (lpMsg->dbInventory[n * MAX_DBITEM_INFO+DBI_DUR] == (BYTE) - 1) itype = -1;
		}
		...
		if (itype != -1) {
			...
			if (_type != ITEMGET(14,11)) {
				if (item.m_Level > 15) item.m_Level = 0;
			}
			...
			if (item.m_ItemOptionEx) {
				...
				if (g_kItemSystemFor380.Is380Item(&item)==false) {
					...
					g_Log.Add("[380Item][%s][%s] Invalid 380 Item Option in Inventory pos[%d]", lpObj->AccountID,lpObj->Name,n);
				}
			}
			...
			if (g_SocketOptionSystem.IsEnableSocketItem(_type) == 1 || g_PentagramSystem.IsPentagramItem(_type) || g_PentagramSystem.IsPentagramJewel(_type)) {
				for (int i = 0; i < 5; i++) SocketOption[i] = lpMsg->dbInventory[((n*MAX_DBITEM_INFO) + DBI_SOCKET_1) + i];

				SocketOptionIndex = lpMsg->dbInventory[n*MAX_DBITEM_INFO + DBI_JOH_DATA];
			} else {
				for (int i = 0; i < 5; i++) SocketOption[i] = 0xFF;
			...
			if (_type == ITEMGET(14,7)) {}
				...
				if (_type >= ITEMGET(14, 0) && _type <= ITEMGET(14, 8) || _type >= ITEMGET(14, 38) && _type <= ITEMGET(14, 40)) {
					if (item.m_Durability == 0.0f) item.m_Durability = 1.0f;
				} else if (_type == ITEMGET(14, 70) || _type == ITEMGET(14, 71)) {
					if (item.m_Durability == 0.0f) item.m_Durability = 1.0f;
				}
				...
				if (_type == ITEMGET(14, 63) || _type == ITEMGET(14, 64)) {
					...
					if (item.m_Level == 3) {
						if (item.m_Durability > item.m_BaseDurability && bIsItemExist == 1) item.m_Durability = item.m_BaseDurability;
					}
				}
			}
			...
			if (iMaxOverlap != 0) {
				if (item.m_Durability > iMaxOverlap) item.m_Durability = iMaxOverlap;
			}
			...
			if (item.m_Type == ITEMGET(14,64)) gObjInventoryDeleteItem(lpObj->m_Index, n);
			if (item.m_Type == ITEMGET(13,107) && item.m_Durability <= 0.0f) gObjInventoryDeleteItem(lpObj->m_Index, n);
		}
	}
...
void PLAYER_CalcStamina(int aIndex) {
	...
	int Strength = gObj[aIndex].CharacterData->Strength + gObj[aIndex].AddStrength;
	int Dexterity = gObj[aIndex].CharacterData->Dexterity + gObj[aIndex].AddDexterity;
	int Vitality = gObj[aIndex].CharacterData->Vitality + gObj[aIndex].AddVitality;
	int Energy = gObj[aIndex].CharacterData->Energy + gObj[aIndex].AddEnergy;
	int Leadership = gObj[aIndex].Leadership + gObj[aIndex].AddLeadership;

	switch (gObj[aIndex].Class)	{
		case CLASS_WIZARD:					gObj[aIndex].MaxStamina = (Strength * 0.2) + (Dexterity * 0.4) + (Vitality * 0.3) + (Energy * 0.2);											break;
		case CLASS_KNIGHT:					gObj[aIndex].MaxStamina = (Strength * 0.15) + (Dexterity * 0.2) + (Vitality * 0.3) + (Energy * 1.0);										break;
		case CLASS_ELF:						gObj[aIndex].MaxStamina = (Strength * 0.3) + (Dexterity * 0.2) + (Vitality * 0.3) + (Energy * 0.2);											break;
		case CLASS_MAGICGLADIATOR:			gObj[aIndex].MaxStamina = (Strength * 0.2) + (Dexterity * 0.25) + (Vitality * 0.3) + (Energy * 0.15);										break;
		case CLASS_DARKLORD:				gObj[aIndex].MaxStamina = (Strength * 0.3) + (Dexterity * 0.2) + (Vitality * 0.1) + (Energy * 0.15) + (Leadership * 0.3);					break;		
		case CLASS_SUMMONER:				gObj[aIndex].MaxStamina = (Strength * 0.2) + (Dexterity * 0.25) + (Vitality * 0.3) + (Energy * 0.15);										break;
		case CLASS_RAGEFIGHTER:				gObj[aIndex].MaxStamina = (Strength * 0.15) + (Dexterity * 0.2) + (Vitality * 0.3) + (Energy * 1.0);										break;
		case CLASS_GROWLANCER:				gObj[aIndex].MaxStamina = (Strength * 0.15) + (Dexterity * 0.2) + (Vitality * 0.3) + (Energy * 1.0);										break;
	}
}
...
bool gObjSetCharacter(LPBYTE lpdata, int aIndex) {
	...
	if (lpObj->Connected < PLAYER_LOGGED) return FALSE;
	if (lpObj->Connected == PLAYER_PLAYING) {
	...
	if (g_MixSystem.ChaosBoxInit(lpObj) == FALSE) g_Log.Add("error-L1 : ChaosBox Init error %s %d", __FILE__, __LINE__);
	if (g_PentagramMixSystem.PentagramMixBoxInit(lpObj) == FALSE) g_Log.Add("error-L1 : PentagramMixBox Init error %s %d", __FILE__, __LINE__);

	lpObj->m_PosNum = 0;
	lpObj->Level = lpMsg->Level;
	lpObj->CharacterData->LevelUpPoint = lpMsg->LevelUpPoint;
	lpObj->CharacterData->DbClass = lpMsg->Class;
	lpObj->Class = lpMsg->Class >> 4;
	lpObj->CharacterData->ChangeUP = lpMsg->Class & 0x07;	// Set Second Type of Character
	...
	if (g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE) {
		...
		strncpy(lpObj->CharacterData->m_RealNameOfUBF, lpObj->CharacterData->m_NameConvertOfUBF.ConvertNameToRealName(tmpCmpName.c_str()), MAX_ACCOUNT_LEN);
		lpObj->CharacterData->m_nServerCodeOfHomeWorld = lpObj->CharacterData->m_NameConvertOfUBF.FindServerCodeAtCharacterName(tmpCmpName.c_str());
	}
	...
	if (lpObj->MapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL) {
		if (g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE) lpObj->MapNumber = MAP_INDEX_LORENMARKET;
		else lpObj->MapNumber = MAP_INDEX_DEVIAS;
	...
	if (lpObj->MapNumber == MAP_INDEX_SANTAVILLAGE && gGateC.CheckGateLevel(aIndex, 22) == false) {
	...
	if (IT_MAP_RANGE(lpObj->MapNumber) != FALSE) {
		if (g_ConfigRead.server.GetServerType() == SERVER_MARKET) lpObj->MapNumber = MAP_INDEX_LORENMARKET;
		else lpObj->MapNumber = MAP_INDEX_ELBELAND;

		MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
		lpObj->m_cIllusionTempleIndex = -1;
	} else lpObj->m_cIllusionTempleIndex = -1;
	...
	if (lpObj->m_bMapSvrMoveReq == true) {
		...
		if (sSVR_CODE == g_ConfigRead.server.GetGameServerCode()) {
			...
			g_Log.AddC(TColor::Red, "[MapServerMng] Dest Map Server doesn't have Map [%s][%s] : %d", lpObj->AccountID, lpObj->Name, lpObj->m_sDestMapNumber);
	...
	if (g_MapAttr.CheckVIP(lpObj->MapNumber) > lpObj->CharacterData->VipType) {
		lpObj->MapNumber = MAP_INDEX_RORENCIA;
		MapC[lpObj->MapNumber].GetLevelPos(lpObj->Level, lpObj->X, lpObj->Y);
	}
	...
	if (g_MapAttr.CheckBlockEntry(lpObj->MapNumber) == true) {
		g_Log.Add("[%s][Forced Move From %s to Lorencia] (%s)(%s) (BeforeMap:%d(%d/%d))", Lang.GetMap(0, lpObj->MapNumber), Lang.GetMap(0, lpObj->MapNumber), lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y);
		lpObj->MapNumber = MAP_INDEX_RORENCIA;
	...
	if (lpObj->MapNumber == MAP_INDEX_HATCHERY && (g_Raklion.GetRaklionState() == RAKLION_STATE_CLOSE_DOOR || g_Raklion.GetRaklionState() == RAKLION_STATE_ALL_USER_DIE || g_Raklion.GetRaklionState() == RAKLION_STATE_NOTIFY_4 || g_Raklion.GetRaklionState() == RAKLION_STATE_END)) gt = 286;
	if (lpObj->MapNumber == MAP_INDEX_RAKLION) g_RaklionUtil.NotifyRaklionCurrentState(lpObj->m_Index, g_Raklion.GetRaklionState(), g_Raklion.GetRaklionStateDetail());
	if (lpObj->MapNumber == MAP_INDEX_DEVIAS) gt = 22;
	else if (lpObj->MapNumber == MAP_INDEX_NORIA) gt = 27;
	else if (lpObj->MapNumber == MAP_INDEX_LOSTTOWER) gt = 42;

	if (gt > 0) {
		result = gGateC.GetGate(gt, x, y, mapNumber, dir, level);

		if (result >= 0) {
			if (lpObj->Class == CLASS_DARKLORD || lpObj->Class == CLASS_MAGICGLADIATOR || lpObj->Class == CLASS_RAGEFIGHTER || lpObj->Class == CLASS_GROWLANCER) {
				if (level > 0) level = level / 3 * 2;
			}
		...
		} else g_Log.Add("error : %d (%s %d)", result, __FILE__, __LINE__);
	}
	...
	lpObj->CharacterData->Experience = lpMsg->Exp;

	if (lpMsg->mlNextExp == NULL && lpMsg->mLevel == NULL && lpMsg->mlExp == NULL) g_MasterLevelSkillTreeSystem.InitData(lpObj);
	else {
		lpObj->CharacterData->MasterLevel = lpMsg->mLevel;
		lpObj->CharacterData->MasterExperience = lpMsg->mlExp;
		lpObj->CharacterData->MasterNextExp = lpMsg->mlNextExp;
		lpObj->CharacterData->MasterPoint = lpMsg->mlPoint;
	}

	lpObj->CharacterData->Strength = lpMsg->Str;
	lpObj->CharacterData->Dexterity = lpMsg->Dex;
	lpObj->CharacterData->Vitality = lpMsg->Vit;
	lpObj->CharacterData->Energy = lpMsg->Energy;
	lpObj->Life = lpMsg->Life;
	lpObj->MaxLife = lpMsg->MaxLife;
	lpObj->Mana = lpMsg->Mana;
	lpObj->MaxMana = lpMsg->MaxMana;
	lpObj->CharacterData->Money = lpMsg->Money;
	lpObj->Leadership = lpMsg->Leadership;
	lpObj->ChatLimitTime = lpMsg->ChatLitmitTime;
	lpObj->CharacterData->iFruitPoint = lpMsg->iFruitPoint;
	lpObj->CharacterData->m_iResets = lpMsg->resets;
	...
	lpObj->CharacterData->AgilityCheckCount = 0;

	if (lpObj->CharacterData->Strength > g_MaxStrength) {
		lpObj->CharacterData->Strength = g_MaxStrength;
		lpMsg->Str = g_MaxStrength;
	}

	if (lpObj->CharacterData->Dexterity > g_MaxAgility) {
		lpObj->CharacterData->Dexterity = g_MaxAgility;
		lpMsg->Dex = g_MaxAgility;
	}

	if (lpObj->CharacterData->Vitality > g_MaxVitality) {
		lpObj->CharacterData->Vitality = g_MaxVitality;
		lpMsg->Vit = g_MaxVitality;
	}

	if (lpObj->CharacterData->Energy > g_MaxEnergy) {
		lpObj->CharacterData->Energy = g_MaxEnergy;
	...
	if (lpObj->Mana < 1.0f) lpObj->Mana = 1.0f;
	if (lpObj->Life < 1.0f) lpObj->Life = 1.0f;

	lpObj->MaxLife = DCInfo.DefClass[lpObj->Class].Life + ((lpObj->Level + lpObj->CharacterData->MasterLevel) - 1) * DCInfo.DefClass[lpObj->Class].LevelLife + ((lpObj->CharacterData->Vitality - DCInfo.DefClass[lpObj->Class].Vitality) * DCInfo.DefClass[lpObj->Class].VitalityToLife);
	lpObj->MaxMana = DCInfo.DefClass[lpObj->Class].Mana + ((lpObj->Level + lpObj->CharacterData->MasterLevel) - 1) * DCInfo.DefClass[lpObj->Class].LevelMana + ((lpObj->CharacterData->Energy - DCInfo.DefClass[lpObj->Class].Energy) * DCInfo.DefClass[lpObj->Class].EnergyToMana);

	if (lpObj->Life > lpObj->MaxLife) lpObj->Life = lpObj->MaxLife;
	if (lpObj->Mana > lpObj->MaxMana) lpObj->Mana = lpObj->MaxMana;
	...
	PLAYER_CalcStamina(aIndex);
	lpObj->Stamina = lpObj->MaxStamina + lpObj->AddStamina;
	...
	lpObj->CharacterData->m_InventoryExpansion = lpMsg->InventoryExpansion;
	lpObj->CharacterData->m_WarehouseExpansion = lpMsg->WarehouseExpansion;
	...
	lpObj->MaxRegenTime = 4000;	// Resurrection Time (MS)
	lpObj->m_MoveSpeed = 1000;
	memcpy(lpObj->CharacterData->m_Quest, lpMsg->dbQuest, sizeof(lpObj->CharacterData->m_Quest));

	if (lpObj->CharacterData->m_Quest[0] == 0) memset(lpObj->CharacterData->m_Quest, (BYTE)-1, sizeof(lpObj->CharacterData->m_Quest));
	if (::g_QuestInfo.GetQuestState(lpObj, 2) == 2) {
		if (lpObj->Level < QUEST_MINLEVEL_PLUSSTAT)	{ // QUEST_MINLEVEL_PLUSSTAT = 1 (Leylei)
			::g_QuestInfo.ReSetQuestState(lpObj, 2);
			g_Log.Add("[%s][%s] Find Invalid QuestInfo (%d)", lpObj->AccountID, lpObj->Name, 2);
		} else lpObj->CharacterData->PlusStatQuestClear = true;
	}

	if (::g_QuestInfo.GetQuestState(lpObj, 3) == 2) {
		if (lpObj->Level < QUEST_MINLEVEL_PLUSSTAT) { // QUEST_MINLEVEL_PLUSSTAT = 1 (Leylei)
			::g_QuestInfo.ReSetQuestState(lpObj, 3);
			g_Log.Add("[%s][%s] Find Invalid QuestInfo (%d)", lpObj->AccountID, lpObj->Name, 3);
		} else lpObj->CharacterData->ComboSkillquestClear = true;
	}

	if (lpObj->CharacterData->m_Quest[21] > 20) lpObj->CharacterData->m_Quest[21] = 0;
	if (lpObj->CharacterData->m_Quest[22] > 20) lpObj->CharacterData->m_Quest[22] = 0;
	if (lpObj->CharacterData->m_Quest[24] > 20) lpObj->CharacterData->m_Quest[24] = 0;

	::INVENTORY_SetPointer1(lpObj);

	lpObj->CharacterData->GuildNumber = 0;
	lpObj->CharacterData->lpGuild = NULL;

	for (int i = 0; i < MAX_MAGIC; i++) lpObj->Magic[i].Clear();
	if (lpObj->Class == CLASS_WIZARD || lpObj->Class == CLASS_ELF || lpObj->Class == CLASS_KNIGHT || lpObj->Class == CLASS_DARKLORD || lpObj->Class == CLASS_MAGICGLADIATOR || lpObj->Class == CLASS_SUMMONER || lpObj->Class == CLASS_RAGEFIGHTER || lpObj->Class == CLASS_GROWLANCER) {
		BOOL bEnergyBall = TRUE;
		BOOL bForceSkill = TRUE;
		BOOL IsGetInitinityArrowSkill = FALSE;

		PLAYER_MagicAddEnergyCheckToggle(FALSE);

		for (n = 0; n < MAX_MAGIC; n++) {
			itype = lpMsg->dbMagicList[n * MAX_MAGIC_INFO];

			if ((lpMsg->dbMagicList[n * MAX_MAGIC_INFO + DBM_LEVEL] & 0x7) > 0) itype = itype * (lpMsg->dbMagicList[n * MAX_MAGIC_INFO + DBM_LEVEL] & 0x7) + lpMsg->dbMagicList[n * MAX_MAGIC_INFO + DBM_EXTRA];

			if (itype != 0xFF) {
				if (g_MasterLevelSkillTreeSystem.CheckMasterLevelSkill(itype)) {
					BYTE level = lpMsg->dbMagicList[n * MAX_MAGIC_INFO + DBM_LEVEL] >> 3;

					if (MAGIC_MagicAdd(lpObj, itype, level) > -1) {
						lpObj->CharacterData->MasterPointUsed += level;
						g_MasterLevelSkillTreeSystem.ML_CalcPassiveSkillData(lpObj, itype, level);
					}
				} else {
					if (itype == 17) bEnergyBall = FALSE;
					if (itype == 60) bForceSkill = FALSE;
					if (itype == 77) {
						IsGetInitinityArrowSkill = 1;
						MAGIC_MagicAdd(lpObj, itype, lpMsg->dbMagicList[n * MAX_MAGIC_INFO + DBM_LEVEL] & 0xF);
					} else if (itype != 18 && itype != 19 && itype != 20 && itype != 21 && itype != 22 && itype != 23 && itype != 24 && itype != 25 && itype != 49 && itype != 56 && itype != 62 && itype != 76 && itype != 54 && itype != 223 && itype != 224 && itype != 225 && itype != 260 && itype != 261 && itype != 270) MAGIC_MagicAdd(lpObj, itype, lpMsg->dbMagicList[n * MAX_MAGIC_INFO + DBM_LEVEL] & 0xF);
				}
			}
		}
		...
		if (IsGetInitinityArrowSkill == 0 && lpObj->Class == CLASS_ELF) {
			if (lpObj->Level >= g_SkillAdditionInfo.GetInfinityArrowUseLevel()) {
				if (lpObj->CharacterData->ChangeUP >= 1) {
					int iAddSkillPosition = MAGIC_MagicAdd(lpObj, 77, 0);

					if (iAddSkillPosition > 0) g_Log.Add("[%s][%s] Add Infinity Arrow Skill (Character Level : %d)(ChangeUp: %d)", lpObj->AccountID, lpObj->Name, lpObj->Level, lpObj->CharacterData->ChangeUP);
				}
			}
		}

		if (lpObj->Class == CLASS_WIZARD || lpObj->Class == CLASS_SUMMONER) MAGIC_MagicAdd(lpObj, 45, 0);
		else if (lpObj->Class == CLASS_KNIGHT) MAGIC_MagicAdd(lpObj, 44, 0);
		else if (lpObj->Class == CLASS_ELF) MAGIC_MagicAdd(lpObj, 46, 0);
		else if (lpObj->Class == CLASS_MAGICGLADIATOR) {
			MAGIC_MagicAdd(lpObj, 57, 0);
			MAGIC_MagicAdd(lpObj, 73, 0);
		} else if (lpObj->Class == CLASS_DARKLORD) MAGIC_MagicAdd(lpObj, 74, 0);
		else if (lpObj->Class == CLASS_RAGEFIGHTER) MAGIC_MagicAdd(lpObj, 269, 0);
		else if (lpObj->Class == CLASS_GROWLANCER) MAGIC_MagicAdd(lpObj, 275, 0);

		MAGIC_MagicAdd(lpObj, 67, 0);
		MAGIC_MagicAdd(lpObj, 68, 0);
		MAGIC_MagicAdd(lpObj, 69, 0);
		MAGIC_MagicAdd(lpObj, 70, 0);
		MAGIC_MagicAdd(lpObj, 71, 0);
		MAGIC_MagicAdd(lpObj, 72, 0);
		::PLAYER_MagicAddEnergyCheckToggle(TRUE);
	}
	...
	INVENTORY_FindEquipment(aIndex);
	...
	lpObj->m_OldX = lpObj->X;
	lpObj->m_OldY = lpObj->Y;
	lpMsg->NextExp = lpObj->CharacterData->NextExp;
	lpObj->CharacterData->LivePacketTime = 0;
	lpObj->CharacterData->LiveCheckDetectCount = 0;
	lpObj->CharacterData->Ruud = lpMsg->Ruud;
	...
	} else lpObj->m_State = 1;

	if ((lpMsg->CtlCode & 8) == 8 && lpMsg->GmCode != 0) {
		lpObj->Authority = 2;
		BUFF_AddBuffEffect(lpObj, BUFFTYPE_GM_MARK, 0, 0, 0, 0, -10);
	...
	if (lpObj->m_cAccountItemBlock != 0) lpObj->Penalty |= 8;
	...
	if ((weather >> 4) > 0) GSProtocol.CGWeatherSend(lpObj->m_Index, weather);
	if (DragonEvent->GetState() > 0) {
		if (DragonEvent->GetMapNumber() == lpObj->MapNumber) ::GSProtocol.GCMapEventStateSend(lpObj->MapNumber, 1, 1);
	}
	...
	if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE && lpObj->MapNumber == MAP_INDEX_CASTLESIEGE) {
		...
		if (g_CastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE) g_CastleSiege.NotifyCsSelfLeftTime(lpObj->m_Index);
	}

	::gEledoradoEvent.CheckGoldDercon(lpObj->MapNumber);
	//::GSProtocol.PROTOCOL_CheckMainExeKeySend(lpObj->m_Index);

	if (lpMsg->Married == 1) {
		memcpy(lpObj->MarryName, lpMsg->MarryName, 10);
		lpObj->Married = 1;
	} else lpObj->Married = 0;

	if (m_ObjBill[lpObj->m_Index - g_ConfigRead.server.GetObjectStartUserIndex()].GetCertify() >= 0 && m_ObjBill[lpObj->m_Index - g_ConfigRead.server.GetObjectStartUserIndex()].GetCertify() < 3) gLCount[m_ObjBill[lpObj->m_Index - g_ConfigRead.server.GetObjectStartUserIndex()].GetCertify()].Add();
	...
	if (lpObj->Level <= g_iCharacterRecuperationMaxLevel) lpObj->m_iAutoRecuperationTime = GetTickCount();
	else lpObj->m_iAutoRecuperationTime = 0;

	lpObj->dwShieldAutoRefillTimer = GetTickCount();

	g_Log.Add("[ShieldSystem][CalcSDPoint] [%s][%s] user SD Gage : %d", lpObj->AccountID, lpObj->Name, lpObj->iMaxShield + lpObj->iAddShield);

	if (g_ConfigRead.SeasonX == true) g_Log.Add("[Ruud Currency] [%s][%s] Ruud:%d", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Ruud);

	g_CashShop.AddUser(lpObj);
	g_PeriodItemEx.AddPeriodInfo(lpObj);
	g_CMuunSystem.AddMuunItemPeriodInfo(lpObj);

	if (lpObj->Class == CLASS_DARKLORD && lpObj->m_Index >= g_ConfigRead.server.GetObjectStartUserIndex()) gDarkSpirit[lpObj->m_Index - g_ConfigRead.server.GetObjectStartUserIndex()].SetMode(CDarkSpirit::ePetItem_Mode::PetItem_Mode_Normal, - 1);
...
void ItemIsBufExOption(BYTE * buf, CItem * lpItem) {
	for (int i = 0; i < MAX_EXOPTION_SIZE; i++) buf[i] = 0;
	if ((lpItem->m_NewOption & 0x20) == 0x20) buf[0] = TRUE;
	if ((lpItem->m_NewOption & 0x10) == 0x10) buf[1] = TRUE;
	if ((lpItem->m_NewOption & 8) == 8) buf[2] = TRUE;
	if ((lpItem->m_NewOption & 4) == 4) buf[3] = TRUE;
	if ((lpItem->m_NewOption & 2) == 2) buf[4] = TRUE;
	if ((lpItem->m_NewOption & 1) == 1) buf[5] = TRUE;
}

void gObjStatTextSave(LPOBJ lpObj) {
	CharSaveLog->Output("[%s][%s] CharInfoSave : Class=%d Level=%d LVPoint=%d Exp=%d Str=%d Dex=%d Vit=%d Energy=%d Leadership:%d Map=%d Pk=%d", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->DbClass,lpObj->Level, lpObj->CharacterData->LevelUpPoint, lpObj->CharacterData->Experience, lpObj->CharacterData->Strength, lpObj->CharacterData->Dexterity, lpObj->CharacterData->Vitality, lpObj->CharacterData->Energy, lpObj->Leadership, lpObj->MapNumber, lpObj->m_PK_Level);
}
	...
	for (n = 0; n < INVENTORY_SIZE; n++) {
		if (lpObj->pInventory[n].IsItem() == TRUE) {
			if (lpObj->pInventory[n].m_serial != 0) {
				...
				CharSaveLog->Output("[%s][%s] SI[%d,%s,%d,%d,%d,%d]serial:[%I64d] dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set[%d] 380:[%d] HO:[%d,%d] E:[%d]", lpObj->AccountID, lpObj->Name, n, lpObj->pInventory[n].GetName(), lpObj->pInventory[n].m_Level, lpObj->pInventory[n].m_Option1, lpObj->pInventory[n].m_Option2, lpObj->pInventory[n].m_Option3, lpObj->pInventory[n].m_Number, (BYTE)lpObj->pInventory[n].m_Durability, NewOption[0], NewOption[1], NewOption[2], NewOption[3], NewOption[4], NewOption[5], NewOption[6], lpObj->pInventory[n].m_SetOption,lpObj->pInventory[n].m_ItemOptionEx >> 7,g_kJewelOfHarmonySystem.GetItemStrengthenOption(&lpObj->pInventory[n]),g_kJewelOfHarmonySystem.GetItemOptionLevel(&lpObj->pInventory[n]), lpObj->pInventory[n].m_BonusSocketOption);
			}
		}
	}

	g_Log.Add("[%s][%s] Item Save Complete. Money [ %d ]", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Money);
}
	...
	for (n = 0; n < WAREHOUSE_SIZE; n++) {
		if (lpObj->pWarehouse[n].IsItem() == TRUE) {
			if (lpObj->pWarehouse[n].m_serial != 0) {
				...
				CharSaveLog->Output("[%s][%s] SW [%d,%s,%d,%d,%d,%d]serial:[%I64d] dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d] 380:[%d] HO:[%d,%d] E:[%d]", lpObj->AccountID, lpObj->Name, n, lpObj->pWarehouse[n].GetName(), lpObj->pWarehouse[n].m_Level, lpObj->pWarehouse[n].m_Option1, lpObj->pWarehouse[n].m_Option2, lpObj->pWarehouse[n].m_Option3, lpObj->pWarehouse[n].m_Number, (BYTE)lpObj->pWarehouse[n].m_Durability, NewOption[0], NewOption[1], NewOption[2], NewOption[3], NewOption[4], NewOption[5], NewOption[6], lpObj->pWarehouse[n].m_SetOption, lpObj->pWarehouse[n].m_ItemOptionEx>>7, g_kJewelOfHarmonySystem.GetItemStrengthenOption(&lpObj->pWarehouse[n]), g_kJewelOfHarmonySystem.GetItemOptionLevel(&lpObj->pWarehouse[n]), lpObj->pWarehouse[n].m_BonusSocketOption);
			}
		}
	}
...
void gObjAuthorityCodeSet(LPOBJ lpObj) {
	...
	if ((lpObj->PenaltyMask&1) == 1) lpObj->Penalty |= 2;
}
...
int gObjSetPosMonster(int aIndex, int PosTableNum) {	
	...
	if (iPentagramMainAttribute) lpObj->m_iPentagramMainAttribute = iPentagramMainAttribute;
...
int gObjSetMonster(int aIndex, int MonsterClass) {
	...
	if (g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE) {
		if (MonsterClass > 547)	{
			if (MonsterClass == 651 || MonsterClass == 669) return FALSE;
		} else if (MonsterClass >= 545 || (MonsterClass == 240 || MonsterClass == 385 || MonsterClass == 540)) return FALSE;
	} else {
		if (MonsterClass >= 683 && MonsterClass <= 688) return FALSE;
	}

	lpObj->TargetNumber = -1;
	lpObj->m_RecallMon = -1;

	if (MonsterClass == 200) gBSGround[0]->m_BallIndex = lpObj->m_Index;
	if (MonsterClass == 240) lpObj->m_btNpcType = NPC_TYPES::NPC_WAREHOUSE;
	else if (MonsterClass == 238 || MonsterClass == 450 || MonsterClass == 478 || MonsterClass == 368 || MonsterClass == 369 || MonsterClass == 370 || MonsterClass == 452 || MonsterClass == 453) lpObj->m_btNpcType = NPC_TYPES::NPC_CHAOSMIX;
	else if (MonsterClass == 236) lpObj->m_btNpcType == NPC_TYPES::NPC_GOLDARCHER;
	else if (MonsterClass == 582)	lpObj->m_btNpcType = NPC_TYPES::NPC_PENTAGRAMMIX;
	else lpObj->m_btNpcType = NPC_TYPES::NPC_NONE;

	lpObj->Connected = PLAYER_PLAYING;
	lpObj->Live = TRUE;
	lpObj->m_State = 1;

	if ((MonsterClass >= 204 && MonsterClass <= 259) || (MonsterClass >= 368 && MonsterClass <= 370) ||	MonsterClass == 367 || MonsterClass == 375 || MonsterClass == 376 || MonsterClass == 377 || MonsterClass == 379 ||  MonsterClass == 406 ||
		MonsterClass == 407 || MonsterClass == 408 || MonsterClass == 415 || MonsterClass == 416 || MonsterClass == 417 || MonsterClass == 465 || MonsterClass == 467 || MonsterClass == 468 || MonsterClass == 469 || MonsterClass == 470 ||
		MonsterClass == 471 || MonsterClass == 472 || MonsterClass == 473 || MonsterClass == 474 ||	MonsterClass == 475 || MonsterClass == 478 || MonsterClass == 371 || MonsterClass == 479 || MonsterClass == 540 || MonsterClass == 492 ||
		MonsterClass == 522 || MonsterClass == 464 || MonsterClass == 450 || MonsterClass == 451 || MonsterClass == 452 || MonsterClass == 453 || MonsterClass == 543 || MonsterClass == 544 || MonsterClass == 385 || MonsterClass == 381 || 
		MonsterClass == 382 || MonsterClass == 380 || MonsterClass == 383 || MonsterClass == 384 || MonsterClass == 541 || MonsterClass == 542 || MonsterClass == 546 || MonsterClass == 545 || MonsterClass == 547 || MonsterClass == 566 || 
		MonsterClass == 567 || MonsterClass == 568 || MonsterClass == 577 || MonsterClass == 578 || MonsterClass == 579 || MonsterClass == 583 || MonsterClass == 580 || MonsterClass == 581 || MonsterClass == 582 || MonsterClass == 584 || 
		MonsterClass == 603 || MonsterClass == 604 || MonsterClass == 643 || MonsterClass == 651 || MonsterClass == 658 || MonsterClass == 659 || MonsterClass == 660 || MonsterClass == 661 || MonsterClass == 662 || MonsterClass == 663 || 
		MonsterClass == 664 || MonsterClass == 665 || MonsterClass == 666 || MonsterClass == 667 || MonsterClass == 668 || MonsterClass == 682 || MonsterClass == 683 || MonsterClass == 684 || MonsterClass == 685 || MonsterClass == 686 || 
		MonsterClass == 687 || MonsterClass == 688) lpObj->Type = OBJ_NPC;
	else lpObj->Type = OBJ_MONSTER;

	if (MonsterClass == 385) g_IT_Event.SetMirageIndex(lpObj->m_Index);
	if (MonsterClass >= 523 && MonsterClass <= 528)	lpObj->Type = OBJ_MONSTER;
	...
	if (lpm->m_Level == 0) g_Log.Add("Monster level can't be 0, Class: %d, %s %d", lpObj->Class, __FILE__, __LINE__);

	lpObj->Level = lpm->m_Level;
	lpObj->AttackSpeed = lpm->AttackSpeed;
	lpObj->m_AttackDamageMin = lpm->m_DamageMin;
	lpObj->m_AttackDamageMax = lpm->m_DamageMax;
	lpObj->Defense = lpm->Defense;
	lpObj->m_MagicDefense = lpm->m_MagicDefense;
	lpObj->HitRating = lpm->HitRating;
	lpObj->EvasionRating = lpm->m_Successfulblocking;
	lpObj->Life = lpm->m_Hp;
	...
	if (lpm->m_PentagramMainAttribute == 6)	{
		switch (rand() % 5)	{
			case 0:				lpObj->m_iPentagramMainAttribute = EL_FIRE;				break;
			case 1:				lpObj->m_iPentagramMainAttribute = EL_WATER;			break;
			case 2:				lpObj->m_iPentagramMainAttribute = EL_EARTH;			break;
			case 3:				lpObj->m_iPentagramMainAttribute = EL_WIND;				break;
			case 4:				lpObj->m_iPentagramMainAttribute = EL_DARKNESS;			break;
		}
	} else if (lpm->m_PentagramMainAttribute > 0) lpObj->m_iPentagramMainAttribute = lpm->m_PentagramMainAttribute;
	...
	memcpy(lpObj->C_Resistance, lpm->m_Resistance, MAX_RESISTANCE_TYPE);
	INVENTORY_SetPointer1(lpObj);

	if (lpObj->m_AttackType != 0) {
		if (DG_MAP_RANGE(lpObj->MapNumber) == TRUE && lpObj->Type == OBJ_MONSTER) gObjMonsterMagicAdd(lpObj, lpObj->m_AttackType, 1);
		else if (lpObj->m_AttackType >= 100) gObjMonsterMagicAdd(lpObj, lpObj->m_AttackType - 100, 1);
		else gObjMonsterMagicAdd(lpObj, lpObj->m_AttackType, 1);
		if (lpObj->Class == 144 || lpObj->Class == 174 || lpObj->Class == 182 || lpObj->Class == 190 || lpObj->Class == 260 || lpObj->Class == 268 || lpObj->Class == 334) gObjMonsterMagicAdd(lpObj, 17, 1);
		if (lpObj->Class == 145 || lpObj->Class == 175 || lpObj->Class == 183 || lpObj->Class == 191 || lpObj->Class == 261 || lpObj->Class == 269 || lpObj->Class == 146 || lpObj->Class == 176 || lpObj->Class == 184 || lpObj->Class == 192 
			|| lpObj->Class == 262 || lpObj->Class == 270 || lpObj->Class == 147 || lpObj->Class == 177 || lpObj->Class == 185 || lpObj->Class == 193 || lpObj->Class == 263 || lpObj->Class == 271 || lpObj->Class == 148 || lpObj->Class == 178
			|| lpObj->Class == 186 || lpObj->Class == 194 || lpObj->Class == 264 || lpObj->Class == 272 || lpObj->Class == 160 || lpObj->Class == 180 || lpObj->Class == 188 || lpObj->Class == 196 || lpObj->Class == 266 || lpObj->Class == 274
			|| lpObj->Class == 332 || lpObj->Class == 333 || lpObj->Class == 336 || lpObj->Class == 337) gObjMonsterMagicAdd(lpObj, 17, 1);
		...
		if (lpObj->Class == 66 || lpObj->Class == 73 || lpObj->Class == 77 || lpObj->Class == 89 || lpObj->Class == 95 || lpObj->Class == 112 || lpObj->Class == 118 || lpObj->Class == 124 || lpObj->Class == 130 || lpObj->Class == 143 || lpObj->Class == 163 || lpObj->Class == 165 || lpObj->Class == 167 || lpObj->Class == 169 || lpObj->Class == 171 || lpObj->Class == 173 || lpObj->Class == 427)	gObjMonsterMagicAdd(lpObj, 17, 1);		
		if (lpObj->Class == 89 || lpObj->Class == 95 || lpObj->Class == 112 || lpObj->Class == 118 || lpObj->Class == 124 || lpObj->Class == 130 || lpObj->Class == 143 || lpObj->Class == 433) gObjMonsterMagicAdd(lpObj, 3, 1);
		if (lpObj->Class == 561) {
		...
		if (lpObj->Class == 673) gObjMonsterMagicAdd(lpObj, 622, 1);
	}
...
short gObjAddSearch(SOCKET aSocket, char* ip) {
	...
	while (true) {
		if (gObj[count].Connected == PLAYER_EMPTY) return count;

		count++;

		if (count >= g_ConfigRead.server.GetObjectMax()) count = g_ConfigRead.server.GetObjectStartUserIndex();

		totalcount++;

		if (totalcount >= g_ConfigRead.server.GetObjectMaxUser()) break;
	}
...
short gObjAdd(SOCKET aSocket, char* ip, int aIndex) {
	if (ObjectMaxRange(aIndex) == FALSE) return -1;
	if (gObj[aIndex].Connected != PLAYER_EMPTY) return -1;
	...
	strcpy(gObj[aIndex].CharacterData->Ip_addr, ip);
	g_Log.Add("Connection Accept : [%d][%s]", aIndex, ip);
	gObjCount++;
	
	if (gObjCount >= g_ConfigRead.server.GetObjectMax()) gObjCount = g_ConfigRead.server.GetObjectStartUserIndex();
...
short gObjAddMonster(int iMapNumber) {
	...
	if (g_ConfigRead.server.GetServerType() == SERVER_MARKET || g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE) {
		if (IT_MAP_RANGE(iMapNumber) || ITL_MAP_RANGE(iMapNumber) || iMapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL || iMapNumber == MAP_INDEX_DEVILSQUARE_FINAL || CC_MAP_RANGE(iMapNumber)) bCheckCanMove = false;
	}

	if (bCheckCanMove == true) {
		if (g_MapServerManager.CheckMapCanMove(iMapNumber) == FALSE) return -1;
	}
	...
	while (true) {
		if (gObj[count].Connected == PLAYER_EMPTY) {
			...
			if (gObjMonCount >= g_ConfigRead.server.GetObjectMaxMonster()) gObjMonCount = 0;
		...
		if (count >= g_ConfigRead.server.GetObjectMaxMonster()) count = 0;
...
short gObjAddCallMon() {
	...
	while (true) {
		if (gObj[count].Connected == PLAYER_EMPTY) {
			...
			if (gObjCallMonCount >= g_ConfigRead.server.GetObjectStartUserIndex()) gObjCallMonCount = g_ConfigRead.server.GetObjectMaxMonster();
			return count;
		}

		count++;

		if (count >= g_ConfigRead.server.GetObjectStartUserIndex()) count = g_ConfigRead.server.GetObjectMaxMonster();
	...
	for (int n= g_ConfigRead.server.GetObjectStartUserIndex(); n < g_ConfigRead.server.GetObjectMax(); n++) {
		if (gObj[n].Connected > PLAYER_EMPTY) {
			if (gObj[n].Type == OBJ_USER) {
				...
				if (gDisconnect == TRUE) GSProtocol.GCSendDisableReconnect(n);
				else IOCP.CloseClient(n);
			}
		}
	}
}
...
void gObjBillRequest(LPOBJ lpObj) {
	...
	if (m_ObjBill[lpObj->m_Index- g_ConfigRead.server.GetObjectStartUserIndex()].GetPayCode() == 0) {
		if (m_ObjBill[lpObj->m_Index- g_ConfigRead.server.GetObjectStartUserIndex()].GetCertify() == 0) wsprintf(szMsg, Lang.GetText(0,22));
		else if (m_ObjBill[lpObj->m_Index- g_ConfigRead.server.GetObjectStartUserIndex()].GetCertify() == 1) {
			if (m_ObjBill[lpObj->m_Index- g_ConfigRead.server.GetObjectStartUserIndex()].GetEndTime() != 0) wsprintf(szMsg, Lang.GetText(0,23), m_ObjBill[lpObj->m_Index- g_ConfigRead.server.GetObjectStartUserIndex()].GetEndTime(), g_VipSystem.GetVipName(lpObj->CharacterData->VipType));
		}
		...
		if (m_ObjBill[lpObj->m_Index- g_ConfigRead.server.GetObjectStartUserIndex()].GetCertify() == 0) wsprintf(szMsg, Lang.GetText(0,24), Day, Month, Year, g_VipSystem.GetVipName(lpObj->CharacterData->VipType));
		else if (m_ObjBill[lpObj->m_Index- g_ConfigRead.server.GetObjectStartUserIndex()].GetCertify() == 1) wsprintf(szMsg, Lang.GetText(0,25), Day, Month, Year, g_VipSystem.GetVipName(lpObj->CharacterData->VipType));
		...
		wsprintf(szMsg, Lang.GetText(0,148), m_ObjBill[lpObj->m_Index- g_ConfigRead.server.GetObjectStartUserIndex()].GetEndTime(), szYear, szMonth, szDay, szHour, szMin);
		g_Log.Add("[%s][%s] BillType : (Time) RemainPoint : (%d)", lpObj->AccountID, lpObj->Name, m_ObjBill[lpObj->m_Index- g_ConfigRead.server.GetObjectStartUserIndex()].GetEndTime(), g_VipSystem.GetVipName(lpObj->CharacterData->VipType));
		...
		strncpy(szMonth, m_ObjBill[lpObj->m_Index- g_ConfigRead.server.GetObjectStartUserIndex()].GetEndsDays()+4, 2);
		strncpy(szDay, m_ObjBill[lpObj->m_Index- g_ConfigRead.server.GetObjectStartUserIndex()].GetEndsDays()+6, 2);
		wsprintf(szMsg, Lang.GetText(0,149),  szYear, szMonth,	szDay, g_VipSystem.GetVipName(lpObj->CharacterData->VipType));
		g_Log.Add("[%s][%s] BillType : (Date) RemainDate : (%s-%s-%s)", lpObj->AccountID, lpObj->Name, szYear, szMonth, szDay);
	} else if (m_ObjBill[lpObj->m_Index-g_ConfigRead.server.GetObjectStartUserIndex()].GetPayCode() == 5) {
		wsprintf(szMsg, Lang.GetText(0,150));
		g_Log.Add("[%s][%s] BillType : (NoCharge)", lpObj->AccountID, lpObj->Name);
	} else wsprintf(szMsg, Lang.GetText(0,26), g_VipSystem.GetVipName(lpObj->CharacterData->VipType));
...
short gObjMemFree(int index) {
	...
	if (lpObj->Connected < PLAYER_CONNECTED) return FALSE;
	...
	if (lpObj->Connected >= PLAYER_LOGGED) {
		if (lpObj->Type == OBJ_USER) {
			if (lpObj->Connected == PLAYER_PLAYING) {
				if (BC_MAP_RANGE(lpObj->MapNumber)) g_BloodCastle.SearchUserDropQuestItem(lpObj->m_Index);
				if (IT_MAP_RANGE(lpObj->MapNumber)) {
					g_IT_Event.DropRelicsItem(lpObj->MapNumber, lpObj->m_Index);
					g_IT_Event.Leave_ITR(lpObj->m_Index, lpObj->MapNumber);
...
BOOL gObjGameClose(int aIndex) {
	...
	if (( (lpObj->m_IfState.use != 0) &&  (lpObj->m_IfState.type == 7)) || ((lpObj->m_IfState.use != 0) && (lpObj->m_IfState.type == 13))) {
		if (lpObj->bIsChaosMixCompleted == true || lpObj->m_bIsCastleNPCUpgradeCompleted == true) gObjInventoryCommit(aIndex);
		else gObjInventoryRollback(aIndex);
	}

	gObjTradeSave(lpObj, aIndex);

	if ((lpObj->m_IfState.use != 0) && (lpObj->m_IfState.type == 10)) {
		if (lpObj->TargetNpcNumber != -1 && gObj[lpObj->TargetNpcNumber].Class == 229) gQeustNpcTeleport.TalkRefDel();
	}

	if (lpObj->m_IfState.use != 0) {
		if (lpObj->TargetNpcNumber != -1 && gObj[lpObj->TargetNpcNumber].Class == 568) gQuestExpNpcTeleport.TalkRefDel();
	}

	if ((lpObj->Authority&2) == 2) cManager.ManagerDel(lpObj->Name);
	...
	if (lpObj->CharacterData->GuildNumber > 0) {
		if (lpObj->CharacterData->lpGuild->WarState == 1) gObjGuildWarMasterClose(lpObj);

		GDCharClose(0, lpObj->CharacterData->lpGuild->Name, lpObj->Name);
	} else GDCharClose(1, NULL, lpObj->Name);
	...
	if (DS_MAP_RANGE(lpObj->MapNumber)) g_DevilSquare.DS_DelUser(lpObj->m_bDevilSquareIndex, aIndex);
	if (BC_MAP_RANGE(lpObj->MapNumber)) g_BloodCastle.SearchUserDropQuestItem(lpObj->m_Index);
	if (IT_MAP_RANGE(lpObj->MapNumber)) {
		g_IT_Event.DropRelicsItem(lpObj->MapNumber, lpObj->m_Index);
		g_IT_Event.Leave_ITR(lpObj->m_Index, lpObj->MapNumber);
	...
	if (lpObj->Type == OBJ_USER && lpObj->MapNumber == g_LastManStanding.m_Cfg.iPVPMap && lpObj->CharacterData->RegisterdLMS == 1) g_LastManStanding.UserQuit(lpObj->m_Index);

	gObjSaveChaosBoxItemList(lpObj);

	if (gObjFixInventoryPointer(aIndex) == false) g_Log.Add("[Fix Inv.Ptr] False Location - %s, %d", __FILE__, __LINE__);
	...
	if (lpObj->CharacterData->m_bIsMining) g_MineSystem.CheckIsUPTUserWhenDisconnected(aIndex);

	GJSetCharacterInfo(lpObj, aIndex, 0);
	gObjViewportClose(lpObj);
	g_Log.Add("(%d)(%s)(%s) Character closed", lpObj->m_Index, lpObj->AccountID, lpObj->Name);

	if (lpObj->Name) WhisperCash.DelCash(lpObj->Name);

	gObjClearBuffEffect(lpObj, CLEAR_TYPE_LOGOUT);
	memcpy(lpObj->BackName, lpObj->Name, sizeof(lpObj->Name)-1);
	lpObj->BackName[10] = 0;
	lpObj->Connected = PLAYER_LOGGED;
	lpObj->CharacterData->m_bUsePartyMatching = false;
	lpObj->CharacterData->m_bPartyMatchingLeader = false;
	lpObj->CharacterData->m_bUseGuildMatching = false;
	lpObj->CharacterData->m_bSendNpcPos = FALSE;

	if (m_ObjBill[aIndex- g_ConfigRead.server.GetObjectStartUserIndex()].GetCertify() >= 0 && m_ObjBill[aIndex- g_ConfigRead.server.GetObjectStartUserIndex()].GetCertify() < 3) gLCount[m_ObjBill[aIndex- g_ConfigRead.server.GetObjectStartUserIndex()].GetCertify()].Delete();
...
short gObjDel(int index) {
	...
	if (IMPERIAL_MAP_RANGE(lpObj->MapNumber) == TRUE) g_Log.Add("[IMPERIALGUARDIAN_TEST] gObjDel() => Class:%d, Index:%d", lpObj->Class, index);

	int UserConnectState = lpObj->Connected;

	if (lpObj->Type == OBJ_USER && lpObj->CharacterData->ISBOT == false && lpObj->m_bOff == false) {
		gObjGameClose(index);

		if (UserConnectState >= PLAYER_LOGGED) {
			if (lpObj->m_bMapSvrMoveQuit == false && lpObj->m_bOff == false) {
				GJPUserClose(lpObj);
				g_Log.Add("(%d)Logout Request : %s [%s]", index, lpObj->AccountID, lpObj->CharacterData->Ip_addr);
			} else g_Log.Add("[MapServerMng] User Quit without JoinServer logout-send [%s][%s]", lpObj->AccountID, lpObj->Name);

			memset(lpObj->AccountID, 0, sizeof(lpObj->AccountID) -1);
			memset(lpObj->CharacterData->Password, 0, sizeof(lpObj->CharacterData->Password));
			memset(lpObj->CharacterData->HWID, 0, sizeof(lpObj->CharacterData->HWID));
		}

		g_Log.Add("(%d)Disconnected : %s [%s]", index, lpObj->AccountID, lpObj->CharacterData->Ip_addr);
	}

	if (lpObj->Type == OBJ_USER && lpObj->CharacterData->ISBOT == false && lpObj->m_bOff == false) lpObj->Connected = PLAYER_EMPTY;
	else lpObj->Connected = PLAYER_EMPTY;

	if (lpObj->Type == OBJ_USER) {
		lpObj->CharacterData->m_bUsePartyMatching = false;
		lpObj->CharacterData->m_bPartyMatchingLeader = false;
		lpObj->CharacterData->m_nServerChannel = -1;
		lpObj->CharacterData->m_nPartyMatchingIndex = -1;
		lpObj->CharacterData->m_bSendNpcPos = FALSE;
		lpObj->CharacterData->VipType = 0;
		lpObj->m_bPShopOpen = false;

		if (g_OffLevel.FindUser(i) != -1) g_OffLevel.DeleteUser(lpObj->m_Index);
	}
...
BOOL gObjIsConnectedGP(int aIndex) {
	...
	if (gObj[aIndex].Connected != PLAYER_PLAYING) return FALSE;
...
BOOL gObjIsAccontConnect(int aIndex, char* accountid) {
	if (aIndex < g_ConfigRead.server.GetObjectStartUserIndex() || aIndex > g_ConfigRead.server.GetObjectMax() - 1) return FALSE;
	if (gObj[aIndex].Connected < PLAYER_CONNECTED) return FALSE;
	if (strlen(&gObj[aIndex].AccountID[0]) < 1) return FALSE;
	if (strcmp(accountid, &gObj[aIndex].AccountID[0]) != 0) return FALSE;
...
BOOL gObjPasswordCheck(int aIndex, char* szInPassword) {
	if (gObj[aIndex].Type != OBJ_USER) return FALSE;
	if (strcmp(gObj[aIndex].CharacterData->Password, szInPassword) == FALSE) return TRUE;
	else return FALSE;
}
...
BOOL gObjCheckXYMapTile(LPOBJ lpObj, int iDbgName) {
	...
	if (mapnumber > MAX_NUMBER_MAP - 1) mapnumber = MAP_INDEX_RORENCIA;
		...
		if (attr2 > 1 && attr3 > 1 && attr4 > 1 && attr5 > 1) {
			...
			if (IT_MAP_RANGE(lpObj->MapNumber)) {
				if (g_IT_Event.GetIllusionTempleState(lpObj->MapNumber) == 2) {
					...
					if (btTeam == 0xFF) pMsg.MoveNumber = 17;
					else if (btTeam == 1) pMsg.MoveNumber = lpObj->MapNumber + 109;
					else if (btTeam == 0) pMsg.MoveNumber = lpObj->MapNumber + 103;
					else pMsg.MoveNumber = 17;
				} else pMsg.MoveNumber = 17;

				gObjMoveGate(lpObj->m_Index, pMsg.MoveNumber);
				return TRUE;
			} else if (lpObj->MapNumber == MAP_INDEX_DEVILSQUARE_FINAL && g_DevilSquareFinal.m_btDSFCurState == 2) {
				if (g_DevilSquareFinal.GetUserTeam(lpObj->m_Index) == -1) pMsg.MoveNumber = 17;
				else pMsg.MoveNumber = 474;
...
BOOL gObjSetAccountLogin(int aIndex, char * szId, int aUserNumber, int aDBNumber, char* Password) {
	...
	strcpy(gObj[aIndex].CharacterData->Password, Password);
...
char* gObjGetAccountId(int aIndex) {
	...
	if (gObj[aIndex].Connected == 2) return &gObj[aIndex].AccountID[0];
	...
	for (int n = g_ConfigRead.server.GetObjectStartUserIndex(); n < g_ConfigRead.server.GetObjectMax(); n++) {
		if (gObj[n].Connected >= PLAYER_PLAYING) {
			if (gObj[n].Name[0] == *szId) {
				if (strcmp(&gObj[n].Name[0], szId) == 0) return n;
			}
		}
	}
...
BOOL gObjUserIdConnectCheck(char* szId,int index) {
	...
	if (gObj[index].Connected != PLAYER_PLAYING) return FALSE;
	if (gObj[index].Name[0] != *szId) return FALSE;
...
int CHARACTER_CalcDistance(LPOBJ lpObj1, LPOBJ lpObj2) {
	if (lpObj1 == NULL || lpObj2 == NULL) return FALSE;
	if (lpObj1->X == lpObj2->X && lpObj1->Y == lpObj2->Y) return FALSE;
...
BOOL gObjCheckTileArea(int aIndex, int x, int y, int dis) {
	...
	if (x < (lpObj->X - dis) || x > (lpObj->X + dis) || y < (lpObj->Y - dis) || y > (lpObj->Y + dis)) return FALSE;
...
BOOL InventoryExtentCheck(int x, int y, int w, int h) {
	...
	if (nEndOfRange > h) nEndOfRange = h;
...
BOOL CheckOutOfInventory(int aIndex, int sy, int height) {
	...
	if (sy >= 4 * gObj[aIndex].CharacterData->m_InventoryExpansion + 8 && (sy < 24 || sy > 27)) return -1;
	...
	if (sy >= 8) {
		if (sy >= 12) {
			if (sy >= 16) {
				if (sy >= 20) {
					if (sy >= 24) {
						if (sy >= 28) return false;
						if (nEndofItemY >= 28) return false;
					} else {
						if (nEndofItemY >= 24) return false;
					}
				} else {
					if (nEndofItemY >= 20) return false;
				}
			} else {
				if (nEndofItemY >= 16) return false;
			}
		} else {
			if (nEndofItemY >= 12) return false;
		}
	} else {
		if (nEndofItemY >= 8) return false;
	}
...
BOOL CHARACTER_CheckOutOfWarehouse(int aIndex, int sy, int height) {
	if (sy >= 15 * gObj[aIndex].CharacterData->m_WarehouseExpansion + 15) return false;

	int nEndofItemY = sy + height - 1;

	if (sy < 0) return false;
	if (sy >= 15) {
		if (sy >= 30) return false;
		if (nEndofItemY >= 30) return false;
	} else {
		if (nEndofItemY >= 15) return false;
	}

	return true;
}
BOOL CHARACTER_WarehouseExtentCheck(int x, int y, int w, int h) {
	int nEndOfRange, nStartOfRange;
	...
	for (int n = 0; n < MAX_MONSTER_SEND_MSG; n++) {
		if (gSMMsg[aIndex][n].MsgCode >= 0) {
			if (GetTickCount64() > gSMMsg[aIndex][n].MsgTime) {
				if (lpObj->Type == OBJ_MONSTER || lpObj->Type == OBJ_NPC) gObjMonsterStateProc(lpObj, gSMMsg[aIndex][n].MsgCode, gSMMsg[aIndex][n].SendUser, gSMMsg[aIndex][n].SubCode);
...
void gObjStateProc(LPOBJ lpObj, int aMsgCode, int aIndex, int SubCode) {
	...
	switch (aMsgCode) {
		case  2:
			CHARACTER_KnockbackNormal(lpObj, &gObj[aIndex]);
		break;
		...
		case  4:
			if (lpObj->Type == OBJ_USER && lpObj->Connected == 3) {
				if (lpObj->CharacterData->lpGuild != NULL && lpObj->CharacterData->lpGuild->lpTargetGuildNode != NULL) gObjGuildWarEnd(lpObj->CharacterData->lpGuild, lpObj->CharacterData->lpGuild->lpTargetGuildNode);
			}
		break;

		case  5:
			if (lpObj->Type == OBJ_USER && lpObj->Connected == 3) {
				if (lpObj->CharacterData->lpGuild != NULL && lpObj->CharacterData->lpGuild->WarState != NULL && lpObj->CharacterData->lpGuild->WarType == 1) BattleSoccerGoalStart(0);
			}
		break;

		case  7:
			if (lpObj->Type == OBJ_USER && lpObj->Connected == 3) {
				if (lpObj->CharacterData->lpGuild != NULL && lpObj->CharacterData->lpGuild->WarState != NULL && lpObj->CharacterData->lpGuild->WarType == 1) {
					if (lpObj->CharacterData->lpGuild->BattleGroundIndex >= 0) GSProtocol.GCManagerGuildWarEnd(lpObj->CharacterData->lpGuild->Name);
				}
			}
		break;

		case 10:
			if (lpObj->EXC_ReflectDamageTaken != 0) gObjAttack(lpObj, &gObj[aIndex], 0, 0, 0, SubCode, 0, 0, 1);
		break;
		...
		case 13:
			...
			GSProtocol.PROTOCOL_ReFillSend(lpObj->m_Index, lpObj->Life, -1, 0, lpObj->iShield);
		break;
		...
		case 16:
			...
			if (lpObj->Life > lpObj->AddLife + lpObj->MaxLife) lpObj->Life = lpObj->AddLife + lpObj->MaxLife;
		...
		case 58: //Eletric Spark
			if (lpObj->Live) {
				CMagicInf * lpMagic = &DefMagicInf[216]; //Eletric Spark
		...
		case 59:
			if (lpObj->Live) {
				CMagicInf * lpMagic = PLAYER_GetMagicSearch(lpObj, 225);

				if (lpMagic) {
					BYTE PosX = LOBYTE(LOWORD(SubCode));
					BYTE PosY = HIBYTE(LOWORD(SubCode));
					BYTE Msg = HIWORD(SubCode);
					gObjUseSkill.SUM_SpiritOfTerror(lpObj->m_Index, lpMagic, PosX, PosY, aIndex, Msg+1, 0);
				}
			}
		break;
		...
		case 65:
			if (lpObj->Live) {
				CMagicInf * lpMagic = PLAYER_GetMagicSearch(lpObj, 339);

				if (lpMagic) gObjUseSkill.SUM_SpiritOfTerror(lpObj->m_Index, lpMagic, 0, 0, aIndex, SubCode+1, 0);
			}
		break;
	}
}
void gObjStateAttackProc(LPOBJ lpObj, int aMsgCode, int aIndex, int SubCode, int SubCode2) {
	...
	switch (aMsgCode) {
		case 50:
			if (lpObj->Live != FALSE) {
				CMagicInf* lpMagic = PLAYER_GetMagicSearch(lpObj, SubCode);

				if (!lpMagic) return;

				gObjAttack(lpObj, &gObj[aIndex], lpMagic, 0, 1, 0, SubCode2, 0, 0);

				if (lpMagic->m_Skill == MONSTER_SUICIDE) {
					lpObj->m_PosNum = -1;
					lpObj->Life = 0.0;
		...
		case 53:
			if (lpObj->Live) {
				...
				if (lpMagic != NULL) gObjAttack(lpObj, &gObj[aIndex], lpMagic, 1, 1, 0, 0, 0, 0);
			}
		break;

		case 51:
			if (lpObj->Type == OBJ_USER && lpObj->CharacterData->ISBOT == false && lpObj->m_Index >= g_ConfigRead.server.GetObjectStartUserIndex()) gDarkSpirit[lpObj->m_Index- g_ConfigRead.server.GetObjectStartUserIndex()].Attack(lpObj, &gObj[aIndex], NULL, SubCode, SubCode2);
		break;
		...
		case 60:
			if (lpObj->Live != FALSE) {
				...
				if (lpMagic != NULL) gObjAttack(lpObj, &gObj[aIndex], lpMagic, 1, 1, 0, SubCode2, 0, 0);
			}
		break;
		...
		case 61: {
			...
			if (lpMagic != NULL) {
				if (lpMagic->m_Skill == 263) gObjUseSkill.RF_WeaponSkill(lpObj->m_Index, lpMagic, SubCode2);
				else if (lpMagic->m_Skill == 559 || lpMagic->m_Skill == 563) g_MasterLevelSkillTreeSystem.ML_RF_ImprovedWeaponSkill(lpObj->m_Index, lpMagic, SubCode2);
			}
		}
		break;
...
// Knockback
BOOL CHARACTER_KnockbackCheck(int& x, int& y, int& dir, BYTE map) {
	BYTE attr;
	int tx = x;
	int ty = y;
	int tdir = dir / 2;
...
BOOL CHARACTER_KnockbackNormal(LPOBJ lpObj, LPOBJ lpTargetObj) {
	int tdir;
	int x;
	int y;
	BYTE attr;
	...
	if (lpObj->Type == OBJ_USER) {
		if (lpObj->m_LastTeleportTime > 0) return FALSE;
	}

	if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) {
		if (lpObj->Type == OBJ_USER) {
			if ((lpObj->Authority & 0x02) == 2) return FALSE;
		}
	}

	if (lpObj->Class >= 131 && lpObj->Class <= 134) return FALSE;
	if (lpObj->Class >= 523 && lpObj->Class <= 528) return FALSE;
	if (lpObj->Class >= 598 && lpObj->Class <= 602) return FALSE;
	if (lpObj->Class >= 627 && lpObj->Class <= 631) return FALSE;
	if (lpObj->Class == 275 || lpObj->Class == 657 || lpObj->Class == 673) return FALSE;
	if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) {
		if (lpObj->Class == 277 || lpObj->Class == 283 || lpObj->Class == 288 || lpObj->Class == 278 || lpObj->Class == 215 || lpObj->Class == 216 || lpObj->Class == 217 || lpObj->Class == 218 || lpObj->Class == 219) return FALSE;
		if (g_CastleSiege.GetCrownUserIndex() == lpObj->m_Index) {
			if (lpObj->pInventory[lpObj->m_btInvenPetPos].m_Type == ITEMGET(13, 4)) return FALSE;
		}
		...
		if (g_Crywolf.GetCrywolfState() == 4 || g_Crywolf.GetCrywolfState() == 3) {
			if (lpObj->Type == OBJ_USER) {
				for (int i = 205; i <= 209; i++) {
					...
					if (iAltarIndex != -1) {
						if (iAltarIndex == lpObj->m_Index) return FALSE;
					}
				}
			}
		}

		if (CRYWOLF_ALTAR_CLASS_RANGE(lpObj->Class) || CHECK_CLASS(204, lpObj->Class) != FALSE) return FALSE;
		if (lpObj->Class == 348) return FALSE;
	}

	if (lpObj->Class == 204) return FALSE;

	//Season 4.0 add-on
	if (lpObj->Class == 459 || lpObj->Class == 460 || lpObj->Class == 461 || lpObj->Class == 462) return FALSE;
	...
	for (int i = 0; i < MAX_BUFFEFFECT; i++) {
		...
		if (EffectID == DEBUFFTYPE_ICE_ARROW || EffectID == DEBUFFTYPE_STUN || EffectID == BUFFTYPE_RESTRICTION || EffectID == DEBUFFTYPE_FREEZE || EffectID == DEBUFFTYPE_EARTHEN_PRISON) return FALSE;
	}

	if ((rand() % 3) == 0) {
		if (lpTargetObj->Dir < 4) tdir = lpTargetObj->Dir + 4;
		else tdir = lpTargetObj->Dir - 4;

		tdir *= 2;
	} else tdir = lpTargetObj->Dir * 2;
	...
	if ((attr & 1) == 1 || (attr & 2) == 2 || (attr & 4) == 4 || (attr & 8) == 8 || (attr & 16)== 16) return FALSE;
	...
	lpObj->m_OldX = x;
	lpObj->m_OldY = y;
	GSProtocol.PROTOCOL_RecvPositionSetProc(&pMove, lpObj->m_Index);

	if (lpObj->Type == OBJ_USER) lpObj->PathCount = 0;

	return TRUE;
}
BOOL CHARACTER_KnockbackDistance(LPOBJ lpObj, LPOBJ lpTargetObj, int count) {
	int tdir;
	...
	if (lpObj->Class == 287 || lpObj->Class == 286) return FALSE;
	if (lpObj->Class >= 131 && lpObj->Class <= 134) return FALSE;
	if (lpObj->Class >= 523 && lpObj->Class <= 528) return FALSE;
	if (lpObj->Class >= 598 && lpObj->Class <= 602) return FALSE;
	if (lpObj->Class >= 627 && lpObj->Class <= 631) return FALSE;
	if (lpObj->Class == 275 || lpObj->Class == 657 || lpObj->Class == 673) return FALSE;
	if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) {
		if (lpObj->Class == 278) return FALSE;
		if (lpObj->Class == 277 || lpObj->Class == 283 || lpObj->Class == 288 || lpObj->Class == 278 || lpObj->Class == 215 || lpObj->Class == 216 || lpObj->Class == 217 || lpObj->Class == 218 || lpObj->Class == 219) return FALSE;
		if (g_CastleSiege.GetCrownUserIndex() == lpObj->m_Index) {
			if (lpObj->pInventory[lpObj->m_btInvenPetPos].m_Type == ITEMGET(13, 4)) return FALSE;
		}
	...
	if (lpObj->Class == 275) return FALSE;
	if (lpObj->Class == 204) return FALSE;
	...
	for (int n = 0; n < count; n++) {
		if (n >= 2) {
			if (lpObj->Class == 4 || lpObj->Class == 3 || lpObj->Class == 1) CHARACTER_KnockbackCheck(x, y, tdir, lpObj->MapNumber);
		} else CHARACTER_KnockbackCheck(x, y, tdir, lpObj->MapNumber);
	}
...
bool CHARACTER_LevelUp(LPOBJ lpObj, UINT64& addexp, int iMonsterType, const char * szEventType) {
	if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(lpObj) == true) return g_MasterLevelSkillTreeSystem.ML_LevelUp(lpObj, addexp, iMonsterType, szEventType);
	g_Log.Add("[%s] Experience : Map[%s]-(%d,%d) [%s][%s](%d) Experience: %I64d + %I64d  MonsterClass : %d", szEventType, Lang.GetMap(0, lpObj->MapNumber),lpObj->X,lpObj->Y,lpObj->AccountID,lpObj->Name,lpObj->Level,lpObj->CharacterData->Experience - addexp,addexp,iMonsterType);

	if (lpObj->Level >= g_ConfigRead.data.common.UserMaxLevel && lpObj->CharacterData->Experience >= gLevelExperience[lpObj->Level]) {
		lpObj->CharacterData->Experience = gLevelExperience[lpObj->Level];
	...
	if (lpObj->CharacterData->Experience < lpObj->CharacterData->NextExp) return TRUE;
	while (lpObj->CharacterData->Experience >= lpObj->CharacterData->NextExp) {
		if (lpObj->Level >= g_ConfigRead.data.common.UserMaxLevel && lpObj->CharacterData->Experience >= gLevelExperience[lpObj->Level]) {
			lpObj->CharacterData->Experience = gLevelExperience[lpObj->Level];
			GSProtocol.GCServerMsgStringSend(Lang.GetText(0, 45), lpObj->m_Index, 1);
			return FALSE;
		...
		addexp = lpObj->CharacterData->Experience - lpObj->CharacterData->NextExp;
		lpObj->Level++;
		CHARACTER_NextExpCal(lpObj);

		if (g_ConfigRead.data.reset.iBlockLevelUpPointAfterResets == -1 || lpObj->CharacterData->m_iResets < g_ConfigRead.data.reset.iBlockLevelUpPointAfterResets) {
			if (lpObj->Class == 4) lpObj->CharacterData->LevelUpPoint += gLevelUpPointMGDL;
			else if (lpObj->Class == 3) lpObj->CharacterData->LevelUpPoint += gLevelUpPointMGDL;
			else if (lpObj->Class == 6) lpObj->CharacterData->LevelUpPoint += gLevelUpPointMGDL;
			else if (lpObj->Class == 7) lpObj->CharacterData->LevelUpPoint += gLevelUpPointMGDL;
			else lpObj->CharacterData->LevelUpPoint += gLevelUpPointNormal;

			if (lpObj->CharacterData->PlusStatQuestClear != false) {
				lpObj->CharacterData->LevelUpPoint += 1;
				g_Log.Add("[%s][%s] LevelUp PlusStatQuest Clear AddStat %d", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->LevelUpPoint);
			}
		}
		...
		GSProtocol.PROTOCOL_ReFillSend(lpObj->m_Index, lpObj->MaxLife + lpObj->AddLife, -1, 0, lpObj->iMaxShield + lpObj->iAddShield);
		GSProtocol.PROTOCOL_ManaSend(lpObj->m_Index, lpObj->MaxMana + lpObj->AddMana, -1, 0, lpObj->MaxStamina + lpObj->AddStamina);
		g_Log.Add("Level Up [%s][%s][%d]", lpObj->AccountID, lpObj->Name, lpObj->Level);

		if (lpObj->Class == CLASS_ELF) {
			if (lpObj->Level >= g_SkillAdditionInfo.GetInfinityArrowUseLevel() && lpObj->CharacterData->ChangeUP >= 1) {
				int iAddSkillPosition = MAGIC_MagicAdd(lpObj, 0x4D, 0);

				if (iAddSkillPosition >= 0) {
					GSProtocol.GCMagicListOneSend(lpObj->m_Index, iAddSkillPosition, 0x4D, 0xDC, 0, 0);
					g_Log.Add("[%s][%s] Add Infinity Arrow Skill (Character Level : %d)(ChangeUp: %d)", lpObj->AccountID, lpObj->Name, lpObj->Level, lpObj->CharacterData->ChangeUP);
				} else g_Log.Add("[%s][%s] Fail - Add Infinity Arrow Skill (Character Level : %d)(ChangeUp: %d)", lpObj->AccountID, lpObj->Name, lpObj->Level, lpObj->CharacterData->ChangeUP);
			}
		}
...
BOOL CHARACTER_LevelUpPointAdd(BYTE type, LPOBJ lpObj) {
	if (lpObj->Type != OBJ_USER) {
		g_Log.AddC(TColor::Red, "error-L1: LevelUp point value error %s %s %s %d", lpObj->AccountID, lpObj->Name, __FILE__, __LINE__);
		return FALSE;
	}

	if (lpObj->CharacterData->LevelUpPoint < 1) return FALSE;
	switch (type) {
		case 0:
			if (lpObj->CharacterData->Strength < g_MaxStrength) lpObj->CharacterData->Strength++;
			else return false;
		break;

		case 1:
			if (lpObj->CharacterData->Dexterity < g_MaxAgility) lpObj->CharacterData->Dexterity++;
			else return false;
		break;

		case 2:
			if (lpObj->CharacterData->Vitality < g_MaxVitality) {
				lpObj->CharacterData->Vitality++;
		...
		case 3:
			if (lpObj->CharacterData->Energy < g_MaxEnergy) {
				lpObj->CharacterData->Energy++;
				lpObj->MaxMana += lpObj->EnergyToMana;
			} else return false;
		break;

		case 4:
			if (lpObj->Leadership < g_MaxCommand) lpObj->Leadership++;
			else return false;
		break;
	}

	lpObj->CharacterData->LevelUpPoint--;
	gObjCalCharacter.CHARACTER_Calc(lpObj->m_Index);
	return TRUE;
}
BOOL CHARACTER_GuildMasterCapacityTest(LPOBJ lpObj) {
	if (lpObj->Level < g_ConfigRead.data.common.GuildCreateLevel) return FALSE;

	return TRUE;
}
void CHARACTER_NextExpCal(LPOBJ lpObj) {
	if (lpObj->Type == OBJ_USER) lpObj->CharacterData->NextExp = gLevelExperience[lpObj->Level];
}
...
void CHARACTER_Killer(LPOBJ lpObj, LPOBJ lpTargetObj, int MSBDamage) {
	...
	for (int r = 0; r < LMS_MAX_ROOMS; r++) {
		if (g_LastManStanding.m_Rooms[r].bState == 3 || g_LastManStanding.m_Rooms[r].bState == 4) {
			for (int i = 0; i < LMS_MAX_USER_PER_ROOM; i++) {
				if (g_LastManStanding.m_Rooms[r].m_Data[i].iIndex == lpObj->m_Index) return;
			}
		}
	}

	if (gObjGetRelationShip(lpObj, lpTargetObj) == 2) {
		if (lpObj->CharacterData->lpGuild != NULL && lpTargetObj->CharacterData->lpGuild != NULL)	g_Log.Add("[U.System][Rival][Player Kill] (Killer [%s][%s] / %s / U:%d / R:%d) vs (Victim [%s][%s] / %s / U:%d / R:%d)", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->lpGuild->Name, lpObj->CharacterData->lpGuild->iGuildUnion, lpObj->CharacterData->lpGuild->iGuildRival, lpTargetObj->AccountID, lpTargetObj->Name, lpTargetObj->CharacterData->lpGuild->Name, lpTargetObj->CharacterData->lpGuild->iGuildUnion, lpTargetObj->CharacterData->lpGuild->iGuildRival);
		else g_Log.Add("[U.System][Rival][Player Kill][ERROR : Can't find GuildInfo] ([%s][%s]) vs ([%s][%s])", lpObj->AccountID, lpObj->Name, lpTargetObj->AccountID, lpTargetObj->Name);

		return;
	}

	if (CC_MAP_RANGE(lpTargetObj->MapNumber)) {
		if (g_ChaosCastle.GetCurrentState(g_ChaosCastle.GetChaosCastleIndex(lpTargetObj->MapNumber)) == 2) return;
	}

	if (lpTargetObj->MapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL) {
		if (g_ChaosCastleFinal.GetCurrentState() == 2) return;
	}

	if (lpObj->MapNumber == MAP_INDEX_DEVILSQUARE_FINAL || lpTargetObj->MapNumber == MAP_INDEX_DEVILSQUARE_FINAL) return;
	if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) {
		if (lpTargetObj->MapNumber == MAP_INDEX_CASTLESIEGE) {
			if (g_CastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE) {
				if (lpObj->m_btCsJoinSide != FALSE) return;
			}

			if (lpTargetObj->m_PK_Level >= 6) return;
		}
	}

	if ((lpObj->Authority & 16) || (lpTargetObj->Authority & 16)) return;
	if (IT_MAP_RANGE(lpObj->MapNumber) || IT_MAP_RANGE(lpTargetObj->MapNumber)) return;
	if (g_GensSystem.IsMapBattleZone(lpObj->MapNumber) && g_GensSystem.IsMapBattleZone(lpTargetObj->MapNumber)) return;
	if (g_ArcaBattle.IsArcaBattleServer() == TRUE) return;
	...
	if (lpTargetObj->m_PK_Level >= 5) return;

	int iOriginPkLevel = lpObj->m_PK_Level;
	int iOriginPkCount = lpObj->m_PK_Count;

	if (MSBDamage == 4) return;
	if (g_ConfigRead.pk.bPkLevelIncreaseOff == true) return;

	BYTE btDefaultPartyPkLevel = 3; //Season 2.5 add-on

	if (lpTargetObj->PartyNumber >= 0) {
		if (gParty.GetPKPartyPenalty(lpTargetObj->PartyNumber) >= 5) btDefaultPartyPkLevel = gParty.GetPKPartyPenalty(lpTargetObj->PartyNumber);
		else btDefaultPartyPkLevel = lpTargetObj->m_PK_Level;
	} else btDefaultPartyPkLevel = lpTargetObj->m_PK_Level;
	if (lpObj->m_PK_Level == 3) {
		if (((char)btDefaultPartyPkLevel) == 3) lpObj->m_PK_Count = 1;
		else if (((char)btDefaultPartyPkLevel) < 3) lpObj->m_PK_Count = 1;
		else if (((char)btDefaultPartyPkLevel) == 4) lpObj->m_PK_Count = 1;
		else {
			if (((char)btDefaultPartyPkLevel) >= 6 && lpTargetObj->Level > 20) {
				lpObj->m_PK_Count--;
		...
		if (((char)btDefaultPartyPkLevel) <= 4) lpObj->m_PK_Count++;
		else return;
	} else if (lpObj->m_PK_Level < 3) {
		if (((char)btDefaultPartyPkLevel) == 3) lpObj->m_PK_Count = 1;
		else if (((char)btDefaultPartyPkLevel) < 3) lpObj->m_PK_Count = 1;
			...
			if (((char)btDefaultPartyPkLevel) >= 6 && lpTargetObj->Level > 20) {
				...
				if (lpObj->m_PK_Count < -3) lpObj->m_PK_Count = -3;
			} else return;
		}
	}
	...
	if (lpObj->m_PK_Count == 0) {
		...
		if (lpObj->PartyNumber >= 0) {
			...
			gParty.UpdatePKPartyPenalty(lpObj->PartyNumber);
		}
	...
	if (lpObj->m_PK_Level == 4) GSProtocol.GCServerMsgStringSend(Lang.GetText(0, 46), lpObj->m_Index, 1);
	if (lpObj->CharacterData->m_bUseGuildMatching) {
		if (lpObj->CharacterData->GuildNumber > 0) GDReqDelMatchingList(lpObj->m_Index, lpObj->CharacterData->GuildNumber);
		else GDReqCancelJoinGuildMatching(lpObj->m_Index, lpObj->Name, 0);
	}

	if (lpObj->CharacterData->m_bUsePartyMatching) {
		if (lpObj->CharacterData->m_bPartyMatchingLeader) GDReqCancelPartyMatching(lpObj->m_Index, 0);
		else {
			if (lpObj->PartyNumber >= 0) {
	...
	g_Log.Add("[%s][%s] PK Info Set (Killed Player) Origin(PkLevel:%d, PkCount:%d) Changed(PkLevel:%d, PkCount:%d)", lpObj->AccountID, lpObj->Name, iOriginPkLevel, iOriginPkCount, lpObj->m_PK_Level, lpObj->m_PK_Count);
}
void CHARACTER_Die(LPOBJ lpObj, LPOBJ lpTargetObj) {
	...
	BUFF_RemoveBuffEffect(lpObj, CLEAR_TYPE_NON_PCS_ITEM_EFFECT);
	gObjUseSkill.RemoveAllCharacterInvalidMagicAndSkillState(lpObj);
	lpObj->CharacterData->m_MuBotEnable = false;
	GSProtocol.GCAnsMuBotUse(lpObj->m_Index, 1, lpObj->CharacterData->m_MuBotTotalTime, 0);

	if (gObjTargetGuildWarCheck(lpObj, lpTargetObj) == 1) return;
	...
	if (DS_MAP_RANGE(lpObj->MapNumber)) {
		if (lpTargetObj->Type == OBJ_MONSTER) {
			...
			if (mAttr != 0) g_Log.Add("[DevilSquare] Dead In DevilSquare [%s][%s][%s]", lpObj->AccountID, lpObj->Name, mAttr->m_Name);
		}

		return;
	}

	if (lpObj->MapNumber == MAP_INDEX_DEVILSQUARE_FINAL) g_DevilSquareFinal.DSFUserDie(lpObj);
	...
	if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER) {
		if (lpObj->MapNumber == g_LastManStanding.m_Cfg.iPVPMap) g_LastManStanding.UserDie(lpObj->m_Index, lpTargetObj->m_Index);
	}

	if (g_AcheronGuardianEvent.IsPlayStart() == true) return;
	...
	if (IT_MAP_RANGE(lpObj->MapNumber)) g_IT_Event.IllusionTempleUserDie(lpObj);
	else if (CC_MAP_RANGE(lpObj->MapNumber)) return;
	else if (lpObj->MapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL) return;
	if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) {
		if (g_CastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE && lpObj->MapNumber == MAP_INDEX_CASTLESIEGE) {
			if (g_CastleSiege.CastleSiegeResetAccumulationAfterKill == TRUE) lpObj->m_iAccumulatedCrownAccessTime = 0;

			return;
		}
	}

	if ((lpObj->Authority & 0x10) || (lpTargetObj->Authority & 0x10)) return;
	if (lpTargetObj->Type == OBJ_MONSTER) {
		switch (rand() % 10) {
		case 1:	GSProtocol.PROTOCOL_ChatSend(lpTargetObj, Lang.GetText(0, 649));	break;
		case 2:	GSProtocol.PROTOCOL_ChatSend(lpTargetObj, Lang.GetText(0, 650));	break;
		case 3:	GSProtocol.PROTOCOL_ChatSend(lpTargetObj, Lang.GetText(0, 651));	break;
		case 4:	GSProtocol.PROTOCOL_ChatSend(lpTargetObj, Lang.GetText(0, 652));	break;
		case 5:	GSProtocol.PROTOCOL_ChatSend(lpTargetObj, Lang.GetText(0, 653));	break;
		case 6:	GSProtocol.PROTOCOL_ChatSend(lpTargetObj, Lang.GetText(0, 654));	break;
		case 7:	GSProtocol.PROTOCOL_ChatSend(lpTargetObj, Lang.GetText(0, 655));	break;
		case 8:	GSProtocol.PROTOCOL_ChatSend(lpTargetObj, Lang.GetText(0, 656));	break;
		case 9:	GSProtocol.PROTOCOL_ChatSend(lpTargetObj, Lang.GetText(0, 657));	break;
		default: GSProtocol.PROTOCOL_ChatSend(lpTargetObj, Lang.GetText(0, 335)); break;
		}
	...
	if (rand() % 100 < iItemDropRate) itemdrop = TRUE;
	if (gObjCanItemTouch(lpObj, 0) == FALSE) itemdrop = FALSE;
	if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER) {
		if (IsOnDuel(lpObj->m_Index, lpTargetObj->m_Index)) itemdrop = FALSE;
	}

	if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_CHARM_PROTECTITEM) != FALSE) itemdrop = FALSE;
	if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) {
		if (g_Crywolf.GetCrywolfState() == 4) {
			if (CRYWOLF_MAP_RANGE(lpObj->MapNumber)) itemdrop = FALSE;
		}
	}

	if (lpObj->Type == OBJ_USER && lpObj->CharacterData->RegisterdLMS == TRUE) itemdrop = FALSE;
	if (lpTargetObj->Type == OBJ_USER) {
		if (g_GensSystem.IsMapBattleZone(lpTargetObj->MapNumber) && g_GensSystem.IsMapBattleZone(lpObj->MapNumber)) return;
		else g_GensSystem.PkPenaltyDropInvenItem(lpObj);
	}
	...
	if (itemdrop == TRUE) {
		if (lpObj->m_PK_Level >= 4) {
			...
			while (count-- != FALSE) {
				...
				if (lpObj->pInventory[number].IsItem() == TRUE && lpObj->pInventory[number].m_Level < g_ConfigRead.pk.iPkMaxLevelItemDrop) {
					if (g_kJewelOfHarmonySystem.IsStrengthenByJewelOfHarmony(&lpObj->pInventory[number]) == 0 && IsCashItem(lpObj->pInventory[number].m_Type) == FALSE) {
						PMSG_ITEMTHROW lpMsg;

						lpMsg.Ipos = number;
						lpMsg.px = lpObj->X;
						lpMsg.py = lpObj->Y;
						if (GSProtocol.CGItemDropRequest(&lpMsg, lpObj->m_Index, 1) == TRUE) {
							dropresult = TRUE;
							break;
						}
					}
		...
		if (dropresult == FALSE) {
			...
			while (count-- != FALSE) {
				...
				if (lpObj->pInventory[number].IsItem() == TRUE && lpObj->pInventory[number].m_Level <= g_ConfigRead.pk.iPkMaxLevelItemDrop) {
					if (g_kJewelOfHarmonySystem.IsStrengthenByJewelOfHarmony(&lpObj->pInventory[number]) == 0 && IsCashItem(lpObj->pInventory[number].m_Type) == FALSE) {
						if (lpObj->pInventory[number].m_Type == ITEMGET(13, 20) && (lpObj->pInventory[number].m_Level >= 1 && lpObj->pInventory[number].m_Level <= 2)) continue;
						if (g_PentagramSystem.IsPentagramItem(lpObj->pInventory[number].m_Type) == true || g_PentagramSystem.IsPentagramJewel(lpObj->pInventory[number].m_Type) == true) continue;
	...
	if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER) {
		if (IsOnDuel(lpObj->m_Index, lpTargetObj->m_Index)) g_NewPVP.SetScore(*lpTargetObj);
	...
	if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(lpObj) == TRUE) {
		minexp = g_MasterLevelSkillTreeSystem.gMasterExperience[lpObj->CharacterData->MasterLevel];
		maxexp = g_MasterLevelSkillTreeSystem.gMasterExperience[lpObj->CharacterData->MasterLevel + 1];
	}
	...
	if (lpObj->PartyNumber >= 0) {
		if ((gParty.GetPKPartyPenalty(lpObj->PartyNumber)) >= 5) btDefaultPartyPkLevel = gParty.GetPKPartyPenalty(lpObj->PartyNumber);
		else btDefaultPartyPkLevel = lpObj->m_PK_Level;
	} else btDefaultPartyPkLevel = lpObj->m_PK_Level;
	if (lpObj->Level > 10) decexprate = g_ConfigRead.pk.GetExpDecreaseRate(btDefaultPartyPkLevel, lpObj->Level + lpObj->CharacterData->MasterLevel);

	decexp = 0;

	if (decexprate > 0) decexp = ((maxexp - minexp) * decexprate) / 1000;

	if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) {
		if (g_Crywolf.GetCrywolfState() == 4) {
			if (lpObj->MapNumber == MAP_INDEX_CRYWOLF_FIRSTZONE) decexp = 0;
		}
	}

	if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_CHARM_GUARDIAN) == FALSE) {
		if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(lpObj) != FALSE) subexp = lpObj->CharacterData->MasterExperience - decexp;
		else subexp = lpObj->CharacterData->Experience - decexp;
	...
	if (lpObj->CharacterData->Money > 0 && BUFF_TargetHasBuff(lpObj, BUFFTYPE_CHARM_GUARDIAN) == FALSE) {
		int iMoneyLevel = 0;

		if (lpObj->Level > 10) iMoneyLevel = g_ConfigRead.pk.GetZenLossRate(lpObj->m_PK_Level, (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER));
		if (iMoneyLevel > 0) submoney = (DWORD)(lpObj->CharacterData->Money * iMoneyLevel) / 100;

		lpObj->CharacterData->Money = lpObj->CharacterData->Money - submoney;

		if (lpObj->CharacterData->Money < 0) lpObj->CharacterData->Money = 0;

		g_Log.Add("[%s][%s] Death reduces money %d - %d", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Money, submoney);
	}

	if (lpObj->WarehouseMoney > 0 && BUFF_TargetHasBuff(lpObj, BUFFTYPE_CHARM_GUARDIAN) == FALSE) {
		submoney = 0;

		int iMoneyLevel = 0;

		if (lpObj->Level > 10) iMoneyLevel = g_ConfigRead.pk.GetZenLossRate(lpObj->m_PK_Level, (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER));
		if (iMoneyLevel > 0) submoney = (DWORD)(lpObj->WarehouseMoney * iMoneyLevel) / 100;
	...
	if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(lpObj) != FALSE) lpObj->CharacterData->MasterExperience = subexp.convert_to<UINT64>();
	else lpObj->CharacterData->Experience = subexp.convert_to<UINT64>();
}
void CHARACTER_TradeInterfaceTimeCheck(LPOBJ lpObj) {
	int tObjNum;
	BOOL TimeCheck = FALSE;

	if (lpObj->m_IfState.use == 0) return;
...
void CHARACTER_PkCheck(LPOBJ lpObj, int TargetLevel) {
	...
	if (TargetLevel >= 2) lpObj->m_PK_Time += TargetLevel / g_ConfigRead.pk.iPKPointDeductionDivisor;
	else lpObj->m_PK_Time++;
	if (lpObj->m_PK_Level >= 3) {
		if (lpObj->m_PK_Level > 3) {
			if (lpObj->m_PK_Count <= 3) {
				if (lpObj->m_PK_Count == 3) {
					if (lpObj->m_PK_Time > g_ConfigRead.pk.GetPKReqPoint(5)) {
						...
						if (lpObj->m_PK_Count > 0) {
							if (lpObj->m_PK_Count == 2) lpObj->m_PK_Level = 5;
							else if (lpObj->m_PK_Count == 1) lpObj->m_PK_Level = 4;
						} else {
							lpObj->m_PK_Count = 0;
							lpObj->m_PK_Level = 3;
						}

						if (lpObj->m_PK_Level <= 5) GSProtocol.GCPkLevelSend(lpObj->m_Index, lpObj->m_PK_Level);
						if (lpObj->m_PK_Level == 4) {
...
// Resistance & Skill Debuff
int CHARACTER_ResistanceProc(LPOBJ lpObj, int Resistance_Type) {
	BYTE r = (BYTE)lpObj->C_Resistance[Resistance_Type] + (BYTE)lpObj->C_ResistanceBonus[Resistance_Type];
	int value = 0;
	float dec = 1.0;

	if (BUFF_IsBuffEffectType(lpObj, DEBUFFTYPE_SHOCK, EFFECTTYPE_LIGHTNING_MASTERY_DEBUFF)) dec -= 0.25;
	if (BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_SEARING_SLASH) && Resistance_Type == R_FIRE) BUFF_GetBuffEffectValue(lpObj, DEBUFFTYPE_SEARING_SLASH, NULL, &value);
	if (value) dec -= value / 100;
	if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_SOUL_POTION)) dec *= 1.20;

	r *= dec;

	if (r >= 10) return TRUE;
	if (r <= 0) return FALSE;
	if (rand() % 11 >= r) return FALSE;

	return TRUE;
}
int CHARACTER_PvPResistanceProc(int Resistance) {
	if (Resistance >= 10) return TRUE;
	if (Resistance <= 0) return FALSE;
	if (rand() % 10 >= Resistance) return FALSE;
...
BOOL gObjAttackQ(LPOBJ lpObj) {
	if (ATTRIBUTE_RANGE(lpObj->m_Attribute)) return FALSE;
	if (lpObj->Class == 221 || lpObj->Class == 222) return FALSE;
	if (g_MineSystem.IsTwinkle(lpObj->Class) == TRUE) return FALSE;
	if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) {
		if (lpObj->Class == 277 || lpObj->Class == 283) {
			if (g_CastleSiege.GetCastleState() != CASTLESIEGE_STATE_STARTSIEGE) return FALSE;
		}

		if (lpObj->Class == 221 || lpObj->Class == 222) return FALSE;
		if (lpObj->Class == 277) {
			if (lpObj->m_btCsGateOpen == TRUE) return FALSE;
		}
	}
	...
	if (BC_MAP_RANGE(lpObj->MapNumber)) {
		if (lpObj->Type >= OBJ_MONSTER) {
			...
			if (lpObj->Class == 131 && g_BloodCastle.CheckMonsterKillSuccess(iBridgeIndex) == false) return FALSE;
			if ((((lpObj->Class - 132) < 0) ? FALSE : ((lpObj->Class - 132) > 2) ? FALSE : TRUE) != FALSE && g_BloodCastle.CheckBossKillSuccess(iBridgeIndex) == false) return FALSE;

		}
	}

	if ((lpObj->Class >= 100 && lpObj->Class <= 110) || lpObj->Class == 523 || lpObj->Class == 247 || lpObj->Class == 249) return FALSE;
	if (lpObj->Class == 689) return FALSE;
	if (lpObj->m_State != 2 && lpObj->m_bOffLevel == false) return FALSE;
	if (lpObj->Live == false) return FALSE;
	if (lpObj->Teleport == 1) return FALSE;
	if (g_ConfigRead.antihack.EnableAttackBlockInSafeZone == TRUE) {
		BYTE btAttr = MapC[lpObj->MapNumber].GetAttr(lpObj->X, lpObj->Y);

		if ((btAttr & 1) == 1) return FALSE;
	}
...
// Companions
BOOL COMPANION_GuardianAngel(LPOBJ lpObj) {
	if (lpObj->Type != OBJ_USER) return FALSE;
	if (lpObj->pInventory[8].IsItem() == FALSE) return FALSE;

	CItem * Angel = &lpObj->pInventory[8];

	if (Angel->m_Type == ITEMGET(13,0) && Angel->m_Durability > 0.0f) return TRUE;

	return FALSE;
}
BOOL COMPANION_Satan(LPOBJ lpObj) {
	if (lpObj->Type != OBJ_USER) return FALSE;
	if (lpObj->pInventory[8].IsItem() == FALSE) return FALSE;

	CItem * Satan = &lpObj->pInventory[8];

	if (Satan->m_Type == ITEMGET(13,1) && Satan->m_Durability > 0.0f) return TRUE;

	return FALSE;
}
BOOL COMPANION_Demon(LPOBJ lpObj) {
	if (lpObj->Type != OBJ_USER) return FALSE;
	if (lpObj->pInventory[8].IsItem() == FALSE) return FALSE;

	CItem* Demon = &lpObj->pInventory[8];

	if (Demon->m_Type == ITEMGET(13,64) && Demon->m_Durability > 0.0f) return TRUE;
	return FALSE;
}
BOOL CHECK_Wings(LPOBJ lpObj) {
	if (lpObj->Type != OBJ_USER) return FALSE;
	if (lpObj->pInventory[7].IsItem() == FALSE) return FALSE;

	CItem* Wing = &lpObj->pInventory[7];

	if (GetWingType(lpObj->pInventory[7].m_Type, WCF_ITEMNUMBER) != NONE_WING) {
		if (Wing->m_Durability > 0.0f) return TRUE;
	}

	return FALSE;
}
BOOL MOUNT_Uniria(LPOBJ lpObj) {
	if (lpObj->m_btInvenPetPos == 0 || lpObj->m_wInvenPet == (WORD) - 1) return FALSE;

	CItem* Uniria = &lpObj->pInventory[lpObj->m_btInvenPetPos];

	if (Uniria->m_Type == ITEMGET(13, 2) && Uniria->m_Durability > 0.0f) return TRUE;

	return FALSE;
}
BOOL COMPANION_GuardianSpirit(LPOBJ lpObj) {
	if (lpObj->Type != OBJ_USER) return FALSE;
	if (lpObj->pInventory[8].IsItem() == FALSE) return FALSE;

	CItem* SafeGuard = &lpObj->pInventory[8];

	if (SafeGuard->m_Type == ITEMGET(13, 65) && SafeGuard->m_Durability > 0.0f) return TRUE;
...
void COMPANION_DurabilityDecrease(LPOBJ lpObj, int damage) {
	...
	if (lpObj->pInventory[8].IsItem() == FALSE) return;
	if (lpObj->pInventory[8].IsPeriodItem() == TRUE) return;

	float fN = 10.0f;
	CItem* companion = &lpObj->pInventory[8];
	float fdamage = damage;

	if (companion->m_Type == ITEMGET(13, 0)) {
		if (lpObj->Class == CLASS_RAGEFIGHTER) {
			if (lpObj->CharacterData->Mastery.ML_RF_Caretaker > 0.0) fdamage = damage / lpObj->CharacterData->Mastery.ML_RF_Caretaker;
		} else if (lpObj->CharacterData->Mastery.ML_GENERAL_Caretaker > 0.0) fdamage = damage / lpObj->CharacterData->Mastery.ML_GENERAL_Caretaker;

		fdamage = fdamage / 10.0f;
		fdamage /= fN;
		companion->m_Durability -= fdamage;
		send_dur = TRUE;
	} else if (companion->m_Type == ITEMGET(13, 1)) {
		if (lpObj->Class == CLASS_RAGEFIGHTER) {
			if (lpObj->CharacterData->Mastery.ML_RF_Caretaker > 0.0) fdamage = damage / lpObj->CharacterData->Mastery.ML_RF_Caretaker;
		} else if (lpObj->CharacterData->Mastery.ML_GENERAL_Caretaker > 0.0) fdamage = damage / lpObj->CharacterData->Mastery.ML_GENERAL_Caretaker;
	...
	if (companion->m_Type == ITEMGET(13, 64)) {
		if (lpObj->Class == CLASS_RAGEFIGHTER) {
			if (lpObj->CharacterData->Mastery.ML_RF_Caretaker > 0.0) fdamage = damage / lpObj->CharacterData->Mastery.ML_RF_Caretaker;
		} else if (lpObj->CharacterData->Mastery.ML_GENERAL_Caretaker > 0.0) fdamage = damage / lpObj->CharacterData->Mastery.ML_GENERAL_Caretaker;
	...
	if (companion->m_Type == ITEMGET(13, 65)) {
		if (lpObj->Class == CLASS_RAGEFIGHTER) {
			if (lpObj->CharacterData->Mastery.ML_RF_Caretaker > 0.0) fdamage = damage / lpObj->CharacterData->Mastery.ML_RF_Caretaker;
		} else if (lpObj->CharacterData->Mastery.ML_GENERAL_Caretaker > 0.0) fdamage = damage / lpObj->CharacterData->Mastery.ML_GENERAL_Caretaker;
	...
	if (companion->m_Type == ITEMGET(13, 67)) {
		if (lpObj->Class == CLASS_RAGEFIGHTER) {
			if (lpObj->CharacterData->Mastery.ML_RF_Caretaker > 0.0) fdamage = damage / lpObj->CharacterData->Mastery.ML_RF_Caretaker;
		} else if (lpObj->CharacterData->Mastery.ML_GENERAL_Caretaker > 0.0) fdamage = damage / lpObj->CharacterData->Mastery.ML_GENERAL_Caretaker;
	...
	if (companion->m_Type == ITEMGET(13, 80)) {
		if (lpObj->Class == CLASS_RAGEFIGHTER) {
			if (lpObj->CharacterData->Mastery.ML_RF_Caretaker > 0.0) fdamage = damage / lpObj->CharacterData->Mastery.ML_RF_Caretaker;
		} else if (lpObj->CharacterData->Mastery.ML_GENERAL_Caretaker > 0.0) fdamage = damage / lpObj->CharacterData->Mastery.ML_GENERAL_Caretaker;
	...
	if (companion->m_Type == ITEMGET(13, 106)) {
		if (lpObj->Class == CLASS_RAGEFIGHTER) {
			if (lpObj->CharacterData->Mastery.ML_RF_Caretaker > 0.0) fdamage = damage / lpObj->CharacterData->Mastery.ML_RF_Caretaker;
		} else if (lpObj->CharacterData->Mastery.ML_GENERAL_Caretaker > 0.0) fdamage = damage / lpObj->CharacterData->Mastery.ML_GENERAL_Caretaker;
	...
	if (companion->m_Type == ITEMGET(13, 123)) {
		if (lpObj->Class == CLASS_RAGEFIGHTER) {
			if (lpObj->CharacterData->Mastery.ML_RF_Caretaker > 0.0) fdamage = damage / lpObj->CharacterData->Mastery.ML_RF_Caretaker;
		} else if (lpObj->CharacterData->Mastery.ML_GENERAL_Caretaker > 0.0) fdamage = damage / lpObj->CharacterData->Mastery.ML_GENERAL_Caretaker;
	...
	if (companion->m_Durability < 0.0f) companion->m_Durability = 0.0f;
	if (send_dur != FALSE) {
		GSProtocol.GCItemDurSend(lpObj->m_Index, 8, companion->m_Durability, 0);

		if (companion->m_Durability < 1.0f) {
			int itemnum = companion->m_Type;

			g_Log.Add("[%s][%s] %s+%d Item is broken because durability is exhausted", lpObj->AccountID, lpObj->Name, lpObj->pInventory[8].GetName(), lpObj->pInventory[8].m_Level);
			gObjInventoryDeleteItem(lpObj->m_Index, 8);
			GSProtocol.PROTOCOL_InventoryItemDeleteSend(lpObj->m_Index, 8, 1); // flag changed (0 -> 1)
			gObjMakePreviewCharSet(lpObj->m_Index);
			...
			if (BC_MAP_RANGE(lpObj->MapNumber) || CC_MAP_RANGE(lpObj->MapNumber) || lpObj->MapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL) {
				if (itemnum == ITEMGET(13, 0) || itemnum == ITEMGET(13, 1)) {
					...
					if (pos != -1) {
						...
						GSProtocol.PROTOCOL_InventoryItemDeleteSend(lpObj->m_Index, pos, 0);
			...
			if (lpObj->MapNumber == MAP_INDEX_ICARUS && itemnum == ITEMGET(13, 3) || lpObj->MapNumber == MAP_INDEX_ICARUS && itemnum == ITEMGET(13, 37)) {
				...
				if (pos == -1) {
					if (lpObj->pInventory[7].IsItem() == FALSE) gObjMoveGate(lpObj->m_Index, 22);
				} else {
					BYTE ItemInfo[MAX_ITEM_INFO];
					BOOL DurSend;
					BOOL DurTargetSend;
...
BOOL INVENTORY_FindEquipment(int aIndex) {
	LPOBJ lpObj = &gObj[aIndex];

	if (lpObj->Type != OBJ_USER) return FALSE;
	for (int i = INVETORY_WEAR_SIZE; i < MAIN_INVENTORY_SIZE; i++) INVENTORY_SetEquipment(lpObj, i);

	return TRUE;
}
BOOL INVENTORY_SetEquipment(LPOBJ lpObj, BYTE btPos) {
	if (lpObj->Type != OBJ_USER) return FALSE;
	if (lpObj->pInventory[btPos].IsItem() == FALSE) return FALSE;
	switch (lpObj->pInventory[btPos].m_Type) {
		case ITEMGET(13,128):
		case ITEMGET(13,129):
			if (lpObj->pInventory[btPos].m_Durability == 254.0 && (lpObj->CharacterData->m_btSculptPos < INVETORY_WEAR_SIZE || lpObj->CharacterData->m_btSculptPos >= MAIN_INVENTORY_SIZE)) lpObj->CharacterData->m_btSculptPos = btPos;
		break;
		...
		case ITEMGET(13,132):
		case ITEMGET(13,133):
			if (lpObj->pInventory[btPos].m_Durability == 254.0 && (lpObj->CharacterData->m_btCharmPos < INVETORY_WEAR_SIZE || lpObj->CharacterData->m_btCharmPos >= MAIN_INVENTORY_SIZE)) lpObj->CharacterData->m_btCharmPos = btPos;
		break;

		case ITEMGET(13, 134):
			if (lpObj->pInventory[btPos].m_Durability == 254.0 && (lpObj->CharacterData->m_btArtifactPos < INVETORY_WEAR_SIZE || lpObj->CharacterData->m_btArtifactPos >= MAIN_INVENTORY_SIZE)) lpObj->CharacterData->m_btArtifactPos = btPos;
		break;

		case ITEMGET(13,177):
		case ITEMGET(13,178):
		case ITEMGET(13,179):
			if (lpObj->pInventory[btPos].m_Durability == 254.0 && (lpObj->CharacterData->m_btExpUpCharmPos < INVETORY_WEAR_SIZE || lpObj->CharacterData->m_btExpUpCharmPos >= MAIN_INVENTORY_SIZE)) lpObj->CharacterData->m_btExpUpCharmPos = btPos;
		break;
		...
		case ITEMGET(13,37):
			if (lpObj->pInventory[btPos].m_JewelOfHarmonyOption == 1 && (lpObj->m_btInvenPetPos < 12 || lpObj->m_btInvenPetPos >= 204)) {
				...
				g_Log.Add("[CRenewal][InvenPet][SearchRidePet][%s][%s] [Success] PetIndex:[%d], Pos:[%d]", lpObj->AccountID, lpObj->Name, lpObj->pInventory[btPos].m_Type, btPos);
			} else g_Log.Add("[CRenewal][InvenPet][SearchRidePet][%s][%s] [Fail] Not Use InvenPet", lpObj->AccountID, lpObj->Name);
			break;
	}

	return TRUE;
}
BOOL INVENTORY_PremiumEquipment(LPOBJ lpObj) {
	if (lpObj->Type != OBJ_USER) return FALSE;
	if (lpObj->CharacterData->m_btSculptPos >= INVETORY_WEAR_SIZE && lpObj->CharacterData->m_btSculptPos < MAIN_INVENTORY_SIZE) {
		LPBUFF_EFFECT_DATE lpEffect = g_BuffEffectSlot.GetEffectDataFromItemCode(lpObj->pInventory[lpObj->CharacterData->m_btSculptPos].m_Type);

		if (lpEffect == NULL) return FALSE;
		if (lpObj->pInventory[lpObj->CharacterData->m_btSculptPos].m_Durability == 254.0) BUFF_AddBuffEffect(lpObj, lpEffect->BuffIndex);
		else {
			lpObj->CharacterData->m_btSculptPos = 0;
			BUFF_RemoveBuffEffect(lpObj, lpEffect->BuffIndex);
		}
	}

	if (lpObj->CharacterData->m_btCharmPos >= INVETORY_WEAR_SIZE && lpObj->CharacterData->m_btCharmPos < MAIN_INVENTORY_SIZE) {
		LPBUFF_EFFECT_DATE lpEffect = g_BuffEffectSlot.GetEffectDataFromItemCode(lpObj->pInventory[lpObj->CharacterData->m_btCharmPos].m_Type);

		if (lpEffect == NULL) return FALSE;
		if (lpObj->pInventory[lpObj->CharacterData->m_btCharmPos].m_Durability == 254.0) BUFF_AddBuffEffect(lpObj, lpEffect->BuffIndex);
		else {
			lpObj->CharacterData->m_btCharmPos = 0;
			BUFF_RemoveBuffEffect(lpObj, lpEffect->BuffIndex);
		}
	}

	if (lpObj->CharacterData->m_btArtifactPos >= INVETORY_WEAR_SIZE && lpObj->CharacterData->m_btArtifactPos < MAIN_INVENTORY_SIZE) {
		LPBUFF_EFFECT_DATE lpEffect = g_BuffEffectSlot.GetEffectDataFromItemCode(lpObj->pInventory[lpObj->CharacterData->m_btArtifactPos].m_Type);

		if (lpEffect == NULL) return FALSE;
		if (lpObj->pInventory[lpObj->CharacterData->m_btArtifactPos].m_Durability == 254.0) BUFF_AddBuffEffect(lpObj, lpEffect->BuffIndex);
		else {
			lpObj->CharacterData->m_btArtifactPos = 0;
			BUFF_RemoveBuffEffect(lpObj, lpEffect->BuffIndex);
		}
	}

	if (lpObj->CharacterData->m_btExpUpCharmPos >= INVETORY_WEAR_SIZE && lpObj->CharacterData->m_btExpUpCharmPos < MAIN_INVENTORY_SIZE) {
		LPBUFF_EFFECT_DATE lpEffect = g_BuffEffectSlot.GetEffectDataFromItemCode(lpObj->pInventory[lpObj->CharacterData->m_btExpUpCharmPos].m_Type);

		if (lpEffect == NULL) return FALSE;
		if (lpObj->pInventory[lpObj->CharacterData->m_btExpUpCharmPos].m_Durability == 254.0) BUFF_AddBuffEffect(lpObj, lpEffect->BuffIndex);
		else {
			lpObj->CharacterData->m_btExpUpCharmPos = 0;
			BUFF_RemoveBuffEffect(lpObj, lpEffect->BuffIndex);
		}
	}
	...
	if (lpObj->m_btInvenPetPos >= INVETORY_WEAR_SIZE && lpObj->m_btInvenPetPos < MAIN_INVENTORY_SIZE) {
		...
		if (lpObj->pInventory[lpObj->m_btInvenPetPos].m_JewelOfHarmonyOption == 1) {
			if (lpObj->m_wInvenPet != (WORD) - 1) return FALSE;

			BUFF_AddBuffEffect(lpObj, lpEffect->BuffIndex);
			...
			if (lpItem->m_Option1) {
				int addskill = MAGIC_WeaponMagicAdd(lpObj, lpItem->m_Special[0], lpItem->m_Level);

				if (addskill >= 0) GSProtocol.GCMagicListOneSend(lpObj->m_Index, addskill, lpItem->m_Special[0], lpItem->m_Level, 0, 0);
			}
			...
			int result = MAGIC_MagicDel(lpObj, lpObj->pInventory[lpObj->m_btInvenPetPos].m_Special[0], lpObj->pInventory[lpObj->m_btInvenPetPos].m_Level);

			if (result >= 0) GSProtocol.GCMagicListOneDelSend(lpObj->m_Index, result, lpObj->pInventory[lpObj->m_btInvenPetPos].m_Special[0], lpObj->pInventory[lpObj->m_btInvenPetPos].m_Level, 0, 0);
		...
		if (lpObj->pInventory[10].m_Type == ITEMGET(13,10))
		{
			...
			if (m_Durability > (int)lpObj->pInventory[10].m_Durability)
			{
				...
				if (dur == 0)
				{
					...
					GSProtocol.PROTOCOL_InventoryItemDeleteSend(lpObj->m_Index,10,0);
				}
			}
		}
		...
		if (lpObj->pInventory[11].m_Type == ITEMGET(13,10))
		{
			...
			if (m_Durability > (int)lpObj->pInventory[11].m_Durability)
			{
				...
				if (dur == 0)
				{
					...
					GSProtocol.PROTOCOL_InventoryItemDeleteSend(lpObj->m_Index,11,0);
				}
			}
		}
...
void gObjWingDurProc(LPOBJ lpObj) {
	...
	if (lpObj->pInventory[7].IsItem() == TRUE) {	
		...
		if (dur > (int)lpObj->pInventory[7].m_Durability) {
			if (lpObj->pInventory[7].m_Durability < 1) lpObj->pInventory[7].m_Durability = 0;
			...
			if (send_dur == 0) {
				...
				GSProtocol.PROTOCOL_InventoryItemDeleteSend(lpObj->m_Index, 7, 0);
			}
		}
...
void gObjPenaltyDurDown(LPOBJ lpObj, LPOBJ lpTargetObj)	// NOT USED
{
	int ret = 0;
	int reCalCharacter = 0;
...
void gObjWeaponDurDown(LPOBJ lpObj, LPOBJ lpTargetObj, int type) {
	if (lpObj->Type == OBJ_MONSTER || lpObj->Type == OBJ_NPC) return;
	if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_CHARM_PROTECTITEM) != 0) return;
	if (g_MineSystem.IsEquipPickax(lpObj->m_Index)) return;
	...
	if (type == 0) {
		itargetdefence = lpTargetObj->Defense;

		if ((lpObj->Class == 1 || lpObj->Class == 3 || lpObj->Class == 4) && (Right->m_Type >= 0 && Right->m_Type < ITEMGET(4,0) && Left->m_Type >= 0 && Left->m_Type < ITEMGET(4,0))) {
			if (rand() % 2 == 0) {
				...
				if (ret != 0) {
					...
					if (ret == 2) gObjCalCharacter.CHARACTER_Calc(lpObj->m_Index);
				}
			} else {
	...
	if (lpTargetObj->Type == OBJ_MONSTER || lpTargetObj->Type == OBJ_NPC) itargetdefence = lpTargetObj->m_MagicDefense;
	else itargetdefence = lpTargetObj->Defense;
...
void gObjArmorRandomDurDown(LPOBJ lpObj, LPOBJ lpAttackObj) {
	...
	if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_CHARM_PROTECTITEM) != 0) {
		return;
	}
	...
	for (int n=0; n < MAX_SELF_DEFENSE; n++) {
		if (lpObj->SelfDefense[n] >= 0)	{
			if (GetTickCount() > lpObj->SelfDefenseTime[n]) {
				...
				g_Log.Add("[%s][%s] ReSet SelfDefence [%s][%s]", lpObj->AccountID, lpObj->Name, gObj[lpObj->SelfDefense[n]].AccountID, gObj[lpObj->SelfDefense[n]].Name);
...
BOOL gObjAttack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf * lpMagic, BOOL magicsend, BYTE MSBFlag, int AttackDamage, BOOL bCombo, BYTE RFAttack, BYTE byReflect, BYTE byPentagramAttack) {
	return gclassObjAttack.ATTACK_Normal(lpObj, lpTargetObj, lpMagic, magicsend, MSBFlag, AttackDamage, bCombo, RFAttack, byReflect, byPentagramAttack);
}
	...
	for (int n = 0; n < MAX_ST_HIT_DAMAGE; n++) {
		if (lpMonObj->sHD[n].number >= 0) {
			...
			if (lpMonObj->sHD[n].HitDamage > 0) {
				...
				exp = CHARACTER_ExperienceSingle(lpTargetObj, lpMonObj, lpMonObj->sHD[n].HitDamage, lpMonObj->MaxLife, bSendExp, nDropZen);

				if (exp > 0) {
					if (bSendExp == 1) {
						if (lpTargetObj->m_Index == lpObj->m_Index) GSProtocol.PROTOCOL_PlayerExperienceSend(lpTargetObj->m_Index, lpMonObj->m_Index, exp, AttackDamage, MSBFlag);
					}
				}
				...
				if (ItemDrop.IsZenDropActive(lpMonObj->MapNumber) == true) {
					...
					if (dwZenDropMoney > 0) lpMonObj->MonsterMoneyDrop = dwZenDropMoney;
				} else lpMonObj->MonsterMoneyDrop = nDropZen;

				if ((lpObj->pInventory[10].IsItem() == TRUE && lpObj->pInventory[10].m_Type == ITEMGET(13, 76) && lpObj->pInventory[10].IsPeriodItemExpire() == FALSE) || lpObj->pInventory[11].IsItem() == TRUE && lpObj->pInventory[11].m_Type == ITEMGET(13, 76) && lpObj->pInventory[11].IsPeriodItemExpire() == FALSE) lpMonObj->MonsterMoneyDrop *= 1.5;
				else if ((lpObj->pInventory[10].IsItem() == TRUE && lpObj->pInventory[10].m_Type == ITEMGET(13, 77) && lpObj->pInventory[10].IsPeriodItemExpire() == FALSE) || lpObj->pInventory[11].IsItem() == TRUE && lpObj->pInventory[11].m_Type == ITEMGET(13, 77) && lpObj->pInventory[11].IsPeriodItemExpire() == FALSE) lpMonObj->MonsterMoneyDrop *= 1.5;
				else if ((lpObj->pInventory[10].IsItem() == TRUE && lpObj->pInventory[10].m_Type == ITEMGET(13, 78) && lpObj->pInventory[10].IsPeriodItemExpire() == FALSE) || lpObj->pInventory[11].IsItem() == TRUE && lpObj->pInventory[11].m_Type == ITEMGET(13, 78) && lpObj->pInventory[11].IsPeriodItemExpire() == FALSE) lpMonObj->MonsterMoneyDrop *= 1.5;
				else if ((lpObj->pInventory[10].IsItem() == TRUE && lpObj->pInventory[10].m_Type == ITEMGET(13, 166) && lpObj->pInventory[10].IsPeriodItemExpire() == FALSE) || lpObj->pInventory[11].IsItem() == TRUE && lpObj->pInventory[11].m_Type == ITEMGET(13, 166) && lpObj->pInventory[11].IsPeriodItemExpire() == FALSE) lpMonObj->MonsterMoneyDrop *= 1.5;
				else if (lpObj->pInventory[8].IsItem() == TRUE && lpObj->pInventory[8].m_Type == ITEMGET(13, 106) && lpObj->pInventory[8].m_Durability > 0.0) lpMonObj->MonsterMoneyDrop *= 1.5;
			}
		}
	}
}
UINT64 CHARACTER_ExperienceSingle(LPOBJ lpObj, LPOBJ lpTargetObj, int dmg, int tot_dmg, bool& bSendExp, UINT64& nDropZen) {
	...
	if (g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE) return FALSE;
	if (g_MasterLevelSkillTreeSystem.ML_ReceiveExp(lpObj, lpTargetObj) == FALSE) {
		bSendExp = false;
		return FALSE;
	}

	if (DS_MAP_RANGE(lpObj->MapNumber)) {
		nDropZen = g_DevilSquare.DS_ExperienceSingle(lpObj, lpTargetObj, dmg, tot_dmg);
		return nDropZen;
	}
	...
	if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(lpObj) == TRUE) userlevel += lpObj->CharacterData->MasterLevel;
	if ((lpTargetObj->Level + 10) < userlevel) level = level * (lpTargetObj->Level + 10) / userlevel;
	if (lpTargetObj->Level >= 65) level += (lpTargetObj->Level - 64) * (lpTargetObj->Level / 4);
	if (level > 0) maxexp = level / 2;
	else level = 0;
	if (maxexp < 1) exp = level;
	else exp = maxexp / 2 + level;

	exp = dmg * exp / tot_dmg;

	if (BC_MAP_RANGE(lpObj->MapNumber)) exp = exp * 50 / 100;
	if (CC_MAP_RANGE(lpObj->MapNumber)) exp *= g_ChaosCastle.GetExperienceBonus(lpObj->m_cChaosCastleIndex);
	...
	if (DG_MAP_RANGE(lpObj->MapNumber)) {
		if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(lpObj)) exp *= g_DoppelGanger.GetDoppelGangerMLExpRate();
		else exp *= g_DoppelGanger.GetDoppelGangerExpRate();
	}

	if (exp > 0) {
		// Randomization
		if (exp > 100) exp -= (rand() % (exp * 10 / 100));
		else exp += rand() % 10;
		if (nDropZen > 100) nDropZen += (rand() % (nDropZen * 10 / 100));
		else nDropZen += 7;

		CheckItemOptForGetExpExRenewal(lpObj, lpTargetObj, exp, dwDefaultExp, false);

		if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(lpObj)) lpObj->CharacterData->MasterExperience += exp;
		else lpObj->CharacterData->Experience += exp;

		bSendExp = CHARACTER_LevelUp(lpObj, exp, lpTargetObj->Class, "Single");
	}
...
void CHARACTER_ExperienceParty(LPOBJ lpObj, LPOBJ lpTargetObj, int AttackDamage, int MSBFlag) {
	if (DS_MAP_RANGE(lpObj->MapNumber)) {
		g_DevilSquare.DS_ExperienceParty(lpObj, lpTargetObj, AttackDamage, MSBFlag);
		return;
	}
	...
	for (int n = 0; n < MAX_USER_IN_PARTY; n++) {
		...
		if (number >= 0) {
			...
			int dis = CHARACTER_CalcDistance(lpTargetObj, lpPartyObj); //Season 2.5 add-on
			...
			if (dis < 10) {
				if (lpPartyObj->Level + lpPartyObj->CharacterData->MasterLevel > toplevel) toplevel = lpPartyObj->Level + lpPartyObj->CharacterData->MasterLevel;
			}
		}
	}
		...
		if (number >= 0) {
			...
			if (lpTargetObj->MapNumber == lpPartyObj->MapNumber) {
				...
				if (dis[n] < 10) {
					...
					if (toplevel >= (lpPartyObj->Level + lpPartyObj->CharacterData->MasterLevel + 200)) totallevel += lpPartyObj->Level + lpPartyObj->CharacterData->MasterLevel + 200;
					else totallevel += lpPartyObj->Level + lpPartyObj->CharacterData->MasterLevel;
					viewplayer += 1;
	...
	if ((bCheckSetParty[0] != 0 && bCheckSetParty[1] != 0 && bCheckSetParty[2] != 0) || (bCheckSetParty[3] != 0 && bCheckSetParty[4] != 0 && bCheckSetParty[5] != 0) || (bCheckSetParty[4] != 0 && bCheckSetParty[5] != 0 && bCheckSetParty[6] != 0) || (bCheckSetParty[0] != 0 && bCheckSetParty[2] != 0 && bCheckSetParty[7] != 0)) bApplySetParty = 1;
	if (viewplayer > 1) {
		if (bApplySetParty != 0) {
	...
	if (lpTargetObj->Level >= 65) level = level + (lpTargetObj->Level - 64) * (lpTargetObj->Level / 4);
	if (level > 0) maxexp = level / 2;
	else level = 0;
	if (maxexp < 1) totalexp = level;
	else totalexp = maxexp / 2 + level;

	// Zen Randomization
	UINT64 zenDrop = totalexp;
	if (zenDrop > 100) zenDrop += (rand() % (zenDrop * 2));
	else zenDrop += (rand() % 10);
	...
	if (lpTargetObj->Type == OBJ_MONSTER) {
		if (ItemDrop.IsZenDropActive(lpTargetObj->MapNumber) == true) {
			...
			if (dwZenDropMoney > 0) lpTargetObj->MonsterMoneyDrop = dwZenDropMoney;
		} else lpTargetObj->MonsterMoneyDrop = zenDrop;

		if ((lpObj->pInventory[10].IsItem() == TRUE && lpObj->pInventory[10].m_Type == ITEMGET(13, 76) && lpObj->pInventory[10].IsPeriodItemExpire() == FALSE) || lpObj->pInventory[11].IsItem() == TRUE && lpObj->pInventory[11].m_Type == ITEMGET(13, 76) && lpObj->pInventory[11].IsPeriodItemExpire() == FALSE) lpTargetObj->MonsterMoneyDrop *= 1.5;
		else if ((lpObj->pInventory[10].IsItem() == TRUE && lpObj->pInventory[10].m_Type == ITEMGET(13, 77) && lpObj->pInventory[10].IsPeriodItemExpire() == FALSE) || lpObj->pInventory[11].IsItem() == TRUE && lpObj->pInventory[11].m_Type == ITEMGET(13, 77) && lpObj->pInventory[11].IsPeriodItemExpire() == FALSE) lpTargetObj->MonsterMoneyDrop *= 1.5;
		else if ((lpObj->pInventory[10].IsItem() == TRUE && lpObj->pInventory[10].m_Type == ITEMGET(13, 78) && lpObj->pInventory[10].IsPeriodItemExpire() == FALSE) || lpObj->pInventory[11].IsItem() == TRUE && lpObj->pInventory[11].m_Type == ITEMGET(13, 78) && lpObj->pInventory[11].IsPeriodItemExpire() == FALSE) lpTargetObj->MonsterMoneyDrop *= 1.5;
		else if ((lpObj->pInventory[10].IsItem() == TRUE && lpObj->pInventory[10].m_Type == ITEMGET(13, 166) && lpObj->pInventory[10].IsPeriodItemExpire() == FALSE) || lpObj->pInventory[11].IsItem() == TRUE && lpObj->pInventory[11].m_Type == ITEMGET(13, 166) && lpObj->pInventory[11].IsPeriodItemExpire() == FALSE) lpTargetObj->MonsterMoneyDrop *= 1.5;
	}
	...
	for (int n = 0; n < MAX_USER_IN_PARTY; n++) {
		...
		if (number >= 0) {
			...
			if (lpTargetObj->MapNumber == lpPartyObj->MapNumber && dis[n] < 10) {
				...
				if (!g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(lpPartyObj)) mymaxexp = gLevelExperience[lpPartyObj->Level];
				else mymaxexp = lpPartyObj->CharacterData->MasterNextExp;
				exp = (totalexp * viewpercent * (lpPartyObj->Level + lpPartyObj->CharacterData->MasterLevel)) / totallevel / 100;
				if (g_MasterLevelSkillTreeSystem.ML_ReceiveExp(lpPartyObj, lpTargetObj) == FALSE) exp = 0;
				if (BC_MAP_RANGE(lpPartyObj->MapNumber)) exp = exp * 50 / 100;
				if (exp > mymaxexp) exp = mymaxexp;
				if (lpPartyObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER) exp = 0;
				UINT64 dwDefaultExp = exp;
				...
				if (DG_MAP_RANGE(lpPartyObj->MapNumber)) {
					if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(lpPartyObj)) exp *= g_DoppelGanger.GetDoppelGangerMLExpRate();
					else exp *= g_DoppelGanger.GetDoppelGangerExpRate();
				}
				...
				if (exp > 0) {
					...
					if (CHARACTER_LevelUp(lpPartyObj, exp, lpTargetObj->Class, "Party") == FALSE) continue;
					GSProtocol.PROTOCOL_PlayerExperienceSend(lpPartyObj->m_Index, lpTargetObj->m_Index, exp, AttackDamage, MSBFlag);
				}
			}
		}
	}
...
void CHARACTER_ExperienceDivisionSpecial(LPOBJ lpMonObj, LPOBJ lpObj, int AttackDamage, int MSBFlag) {
	...
	for (int n = 0; n < MAX_ST_HIT_DAMAGE; n++) {
		if (lpMonObj->sHD[n].number >= 0) {
			...
			if (gObj[nTargetNumber].PartyNumber < 0) {
				...
				UINT64 nTempDropZen = CHARACTER_ExperienceSingleSpecial(lpTargetObj, lpMonObj, lpMonObj->sHD[n].HitDamage, lpMonObj->MaxLife, bSendExp);

				if (lpObj->m_Index == lpMonObj->TargetNumber) nDropZen = nTempDropZen;
			} else {
				for (int k = 0; k < 40; k++) {
	...
	if (bIsPartyDamage) {
		for (int i = 0; i < 40; i++) {
			if (stHitParty[i].nPartyNumber >= 0) {
				UINT64 nTempDropZen = CHARACTER_ExperiencePartySpecial(stHitParty[i].nPartyNumber, lpObj->m_Index, lpMonObj, stHitParty[i].nHitDamage, lpMonObj->MaxLife, 1);

				if (nTempDropZen > 0) nDropZen = nTempDropZen;
			}
		}
	}
	...
	if (lpMonObj->Type == OBJ_MONSTER) {
		...
		if ((lpObj->pInventory[10].IsItem() == TRUE && lpObj->pInventory[10].m_Type == ITEMGET(13, 76) && lpObj->pInventory[10].IsPeriodItemExpire() == FALSE) || (lpObj->pInventory[11].IsItem() == TRUE && lpObj->pInventory[11].m_Type == ITEMGET(13, 76) && lpObj->pInventory[11].IsPeriodItemExpire() == FALSE)) lpMonObj->MonsterMoneyDrop *= 1.5;
		if ((lpObj->pInventory[10].IsItem() == TRUE && lpObj->pInventory[10].m_Type == ITEMGET(13, 77) && lpObj->pInventory[10].IsPeriodItemExpire() == FALSE) || (lpObj->pInventory[11].IsItem() == TRUE && lpObj->pInventory[11].m_Type == ITEMGET(13, 77) && lpObj->pInventory[11].IsPeriodItemExpire() == FALSE)) lpMonObj->MonsterMoneyDrop *= 1.5;
		if ((lpObj->pInventory[10].IsItem() == TRUE && lpObj->pInventory[10].m_Type == ITEMGET(13, 78) && lpObj->pInventory[10].IsPeriodItemExpire() == FALSE) || (lpObj->pInventory[11].IsItem() == TRUE && lpObj->pInventory[11].m_Type == ITEMGET(13, 78) && lpObj->pInventory[11].IsPeriodItemExpire() == FALSE)) lpMonObj->MonsterMoneyDrop *= 1.5;
	}
}
UINT64 CHARACTER_ExperienceSingleSpecial(LPOBJ lpObj, LPOBJ lpTargetObj, int dmg, int tot_dmg, bool& bSendExp) {
	if (!lpObj || !lpTargetObj) return FALSE;
	if (lpObj->Type != OBJ_USER) {
		g_Log.AddC(TColor::Red, "[ERROR] lpObj->Type != OBJ_USER (%s)(%d)", __FILE__, __LINE__);
		return FALSE;
	}
	...
	UINT64 nExp = 0;
	int nTotalLevel = lpObj->Level + lpObj->CharacterData->MasterLevel;
	...
	if (nTotalLevel > nMonsterLevel) nExp = ((15 * nMonsterLevel) + (15 * nTotalLevel) * 0.3 / 200.0 * (nMonsterLevel - nTotalLevel));
	else nExp = ((15 * nTotalLevel) + (15 * nTotalLevel) * 0.3 / 200.0 * (nMonsterLevel - nTotalLevel));
	if (nExp <= 0) nExp = 1;
	if (dmg > tot_dmg) dmg = tot_dmg;

	nExp = dmg * nExp / tot_dmg;

	if (BC_MAP_RANGE(lpObj->MapNumber)) nExp = nExp * 50 / 100;
	...
	if (nExp > 0) {
		...
		if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(lpObj)) lpObj->CharacterData->MasterExperience += nExp;
		else lpObj->CharacterData->Experience += nExp;

		if (CHARACTER_LevelUp(lpObj, nExp, lpTargetObj->Class, "Single") == FALSE) bSendExp = 0;
	}

	if (bSendExp) GSProtocol.PROTOCOL_PlayerExperienceSend(lpObj->m_Index, lpTargetObj->m_Index, nExp, dmg, bSendExp);
...
UINT64 CHARACTER_ExperiencePartySpecial(int nPartyNumber, int nLastAttackUserIndex, LPOBJ lpTargetObj, int AttackDamage, int nTotalDamage, int MSBFlag) {
	...
	if (!lpTargetObj) return FALSE;
	if (!ObjectMaxRange(nPartyNumber)) return FALSE;
		...
		if (nNumber >= 0) {
			if (gObj[nNumber].Connected == PLAYER_PLAYING) {
				if (lpTargetObj->MapNumber == gObj[nNumber].MapNumber) {
					dis[n] = CHARACTER_CalcDistance(lpTargetObj, &gObj[nNumber]);
					...
					if (dis[n] < 10) {
						...
						nTotalLevel += lpPartyObj->CharacterData->MasterLevel + lpPartyObj->Level;

						if (lpPartyObj->CharacterData->MasterLevel + lpPartyObj->Level > nTopLevel) nTopLevel = lpPartyObj->CharacterData->MasterLevel + lpPartyObj->Level;
					}
				}
			}
		}
	...
	if (nTopLevel > nMonsterLevel) nTempExp = ((15 * nMonsterLevel) + (15 * nTopLevel) * 0.3 / 200.0 * (nMonsterLevel - nTopLevel));
	else nTempExp = ((15 * nTopLevel) + (15 * nTopLevel) * 0.3 / 200.0 * (nMonsterLevel - nTopLevel));
	if (nTempExp <= 0) nTempExp = 1;
	if (AttackDamage > nTotalDamage) AttackDamage = nTotalDamage;
	...
	if ((bCheckSetParty[0] != 0 && bCheckSetParty[1] != 0 && bCheckSetParty[2] != 0) || (bCheckSetParty[3] != 0 && bCheckSetParty[4] != 0 && bCheckSetParty[5] != 0) ||	(bCheckSetParty[4] != 0 && bCheckSetParty[5] != 0 && bCheckSetParty[6] != 0) ||	(bCheckSetParty[0] != 0 && bCheckSetParty[2] != 0 && bCheckSetParty[7] != 0)) bApplaySetParty = true;
	if (bApplaySetParty) {
		switch (nViewPlayer) {
			case 3:				fPartyCntAddExp = 1.05;				break;
			case 4:				fPartyCntAddExp = 1.06;				break;
			case 5:				fPartyCntAddExp = 1.07;				break;
			default:			fPartyCntAddExp = 1.00;				break;
		}
	} else {
		switch (nViewPlayer) {
			case 2:				fPartyCntAddExp = 1.02;				break;
			case 3:				fPartyCntAddExp = 1.03;				break;
			case 4:				fPartyCntAddExp = 1.04;				break;
			case 5:				fPartyCntAddExp = 1.05;				break;
			default:			fPartyCntAddExp = 1.00;				break;
		}
	}
	...
	for (int i = 0; i < MAX_USER_IN_PARTY; i++)	{
		...
		if (nNumber >= 0) {
			if (gObj[nNumber].Connected == PLAYER_PLAYING) {
				LPOBJ lpPartyObj = &gObj[nNumber];
				int nMemberLevel = lpPartyObj->CharacterData->MasterLevel + lpPartyObj->Level;

				if (lpTargetObj->MapNumber == lpPartyObj->MapNumber) {
					if (dis[i] < 10) {
						UINT64 nExp;

						if (g_MasterLevelSkillTreeSystem.ML_ReceiveExp(lpPartyObj, lpTargetObj) == FALSE) nExp = 0;
						else {
							UINT64 i64MyMaxExp;

							if (!g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(lpPartyObj)) i64MyMaxExp = gLevelExperience[lpPartyObj->Level];
							else i64MyMaxExp = lpPartyObj->CharacterData->MasterNextExp;

							nExp = nTempExp * fPartyCntAddExp * nMemberLevel / nTotalLevel;

							if (BC_MAP_RANGE(lpPartyObj->MapNumber)) nExp = nExp * 50 / 100;
							if (lpPartyObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER) nExp = 0;
							...
							if (nLastAttackUserIndex == lpPartyObj->m_Index) nDropZen = nExp;
							if (!g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(lpPartyObj)) {
								fEventBonus = g_BonusEvent.GetAddExp();
								fMapBonus = g_MapAttr.GetExpBonus(lpPartyObj->MapNumber);
								fBaseExp = g_ExpManager.GetExpMultiplier(lpPartyObj->Level, lpPartyObj->CharacterData->m_iResets, false);
							...
							if (DG_MAP_RANGE(lpPartyObj->MapNumber)) {
								if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(lpPartyObj)) nExp *= g_DoppelGanger.GetDoppelGangerMLExpRate();
								else nExp *= g_DoppelGanger.GetDoppelGangerExpRate();
							}

							if (nExp > 0) {
								CheckItemOptForGetExpExRenewal(lpPartyObj, lpTargetObj, nExp, dwDefaultExp, false);

								if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(lpPartyObj)) lpPartyObj->CharacterData->MasterExperience += nExp;
								else lpPartyObj->CharacterData->Experience += nExp;
								if (CHARACTER_LevelUp(lpPartyObj, nExp, lpTargetObj->Class, "Party") == FALSE) continue;
							}

							GSProtocol.PROTOCOL_PlayerExperienceSend(lpPartyObj->m_Index, lpTargetObj->m_Index, nExp, AttackDamage, MSBFlag);
						}
					}
				}
			}
...
void gObjMonsterDieLifePlus(LPOBJ lpObj, LPOBJ lpTartObj) {
	...
	if (lpObj->MonsterDieGetLife != 0 && lpDieLifeOption != NULL) lpObj->Life += (lpObj->MaxLife + lpObj->AddLife) / lpDieLifeOption->OptionValue * lpObj->MonsterDieGetLife;
	if (lpObj->MonsterDieGetMana != 0 && lpDieManaOption != NULL) lpObj->Mana += (lpObj->MaxMana + lpObj->AddMana) / lpDieManaOption->OptionValue * lpObj->MonsterDieGetMana;
	if (lpObj->CharacterData->m_wSocketOptionMonsterDieGetHp > 0) lpObj->Life += lpObj->CharacterData->m_wSocketOptionMonsterDieGetHp;
	if (lpObj->CharacterData->m_wSocketOptionMonsterDieGetMana > 0) lpObj->Mana += lpObj->CharacterData->m_wSocketOptionMonsterDieGetMana;
	if (lpObj->Class == CLASS_RAGEFIGHTER) {
		if (lpObj->CharacterData->Mastery.ML_RF_Slayer > 0.0) lpObj->Life = lpObj->Life + ((lpObj->MaxLife + lpObj->AddLife) / lpObj->CharacterData->Mastery.ML_RF_Slayer);
	} else if (lpObj->CharacterData->Mastery.ML_GENERAL_Slayer > 0.0) lpObj->Life = lpObj->Life + ((lpObj->MaxLife + lpObj->AddLife) / lpObj->CharacterData->Mastery.ML_GENERAL_Slayer);

	if (lpObj->Class == CLASS_RAGEFIGHTER) {
		if (lpObj->CharacterData->Mastery.ML_RF_Vanquisher > 0.0) lpObj->Mana = lpObj->Mana + ((lpObj->MaxMana + lpObj->AddMana) / lpObj->CharacterData->Mastery.ML_RF_Vanquisher);
	} else if (lpObj->CharacterData->Mastery.ML_GENERAL_Vanquisher > 0.0) lpObj->Mana = lpObj->Mana + ((lpObj->MaxMana + lpObj->AddMana) / lpObj->CharacterData->Mastery.ML_GENERAL_Vanquisher);

	if (lpObj->Class == CLASS_RAGEFIGHTER) {
		if (lpObj->CharacterData->Mastery.ML_RF_Leech > 0.0) lpObj->iShield = lpObj->iShield + ((lpObj->iMaxShield + lpObj->iAddShield) / lpObj->CharacterData->Mastery.ML_RF_Leech);
	} else if (lpObj->CharacterData->Mastery.ML_GENERAL_Leech > 0.0) lpObj->iShield = lpObj->iShield + ((lpObj->iMaxShield + lpObj->iAddShield) / lpObj->CharacterData->Mastery.ML_GENERAL_Leech);
	
	if (lpObj->Life > (lpObj->MaxLife + lpObj->AddLife)) lpObj->Life = lpObj->MaxLife + lpObj->AddLife;
	if (lpObj->iShield > (lpObj->iMaxShield + lpObj->iAddShield)) lpObj->iShield = lpObj->iMaxShield + lpObj->iAddShield;

	GSProtocol.PROTOCOL_ReFillSend(lpObj->m_Index, lpObj->Life, -1, 0, lpObj->iShield);

	if (lpObj->MonsterDieGetMana != 0 || lpObj->CharacterData->m_wSocketOptionMonsterDieGetMana != 0 || lpObj->CharacterData->Mastery.ML_GENERAL_Vanquisher != 0.0 || lpObj->CharacterData->Mastery.ML_RF_Vanquisher != 0.0) {
		if (lpObj->Mana > (lpObj->MaxMana + lpObj->AddMana)) lpObj->Mana = lpObj->MaxMana + lpObj->AddMana;
...
void gObjLifeCheck(LPOBJ lpTargetObj, LPOBJ lpObj, int AttackDamage, int DamageSendType, int MSBFlag, int MSBDamage, WORD Skill, int iShieldDamage, int iElementalDamage) {
	...
	if (ATTRIBUTE_RANGE(lpObj->m_Attribute) || ATTRIBUTE_RANGE(lpTargetObj->m_Attribute)) return;
	if (lpObj->Class == 689) return;

	LPOBJ lpCallObj = lpObj;

	if (lpObj->Type == OBJ_MONSTER && lpObj->m_RecallMon >= 0) lpCallObj = &gObj[lpObj->m_RecallMon];
	if (AttackDamage != 0 && lpTargetObj->Type == OBJ_MONSTER) {
		if (lpTargetObj->m_RecallMon >= 0) GSProtocol.GCRecallMonLife(lpTargetObj->m_RecallMon,lpTargetObj->MaxLife,lpTargetObj->Life);
		gObjMonsterSetHitDamage(lpTargetObj,lpCallObj->m_Index,AttackDamage);
	}

	if (lpTargetObj->Life <= 0 && lpTargetObj->Live != 0) {
		if (DamageSendType == 0) GSProtocol.PROTOCOL_DamageSend(lpCallObj->m_Index,lpTargetObj->m_Index,AttackDamage,MSBFlag,MSBDamage,iShieldDamage);
		else if (DamageSendType == 1) GSProtocol.PROTOCOL_DamageSendPoison(lpTargetObj->m_Index,AttackDamage,iShieldDamage);
		else if (DamageSendType == 2) GSProtocol.PROTOCOL_DamageSend(lpCallObj->m_Index,lpTargetObj->m_Index,AttackDamage,MSBFlag,5,iShieldDamage);

		if (iElementalDamage != 0) GSProtocol.PROTOCOL_ElementalDamageSend(lpCallObj->m_Index, lpTargetObj->m_Index, iElementalDamage, lpCallObj->m_iPentagramMainAttribute);
		...
		if (lpCallObj->Type == OBJ_USER && lpCallObj->CharacterData->ISBOT == false && lpObj->m_Index >= g_ConfigRead.server.GetObjectStartUserIndex()) {
		...
		if (lpCallObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER) gObjSetKillCount(lpCallObj->m_Index,1);
		if (IT_MAP_RANGE(lpCallObj->MapNumber)) {
			if (g_IT_Event.GetIllusionTempleState(lpCallObj->MapNumber) == 2) {
				...
				g_Log.Add("[ ITR ] [IncreaseKillCount] Map:%d, (%s)(%s) Kill User TargetInfo (%s)(%s)", lpCallObj->MapNumber - 44, lpCallObj->AccountID, lpCallObj->Name, lpTargetObj->AccountID, lpTargetObj->Name);
			}
		}
		...
		if (lpTargetObj->Type == OBJ_MONSTER) {
			if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) {
				...
				} else gObjAddMsgSendDelay(lpTargetObj, 1, lpCallObj->m_Index, 800, 0);

				g_Crywolf.CrywolfMonsterDieProc(lpTargetObj->m_Index, lpCallObj->m_Index);
			} else {
				if (lpTargetObj->Class == 43 ||	// Golden Budge Dragon
					lpTargetObj->Class == 44 || // Red Dragon
					lpTargetObj->Class == 53 ||	// Golden Titan
					lpTargetObj->Class == 54 ||	// Golden Soldier
					lpTargetObj->Class == 55 ||	// Death King
					lpTargetObj->Class == 56 ||	// Death Bone
					lpTargetObj->Class == 59 ||	// Ancient Tantalos
					lpTargetObj->Class == 63 ||	// Ancient Vulture
					lpTargetObj->Class == 77 ||	// Dark Phoenix
					lpTargetObj->Class == 78 ||	// Golden Goblin
					lpTargetObj->Class == 79 ||	// Golden Derkon
					lpTargetObj->Class == 80 ||	// Golden Lizard
					lpTargetObj->Class == 81 ||	// Golden Mermaid
					lpTargetObj->Class == 82 ||	// Golden Tantalos
					lpTargetObj->Class == 83 ||	// Golden Wheel
					lpTargetObj->Class == 135 ||	// White Wizard
					lpTargetObj->Class == 275 ||	// Kundun
					lpTargetObj->Class == 295 ||	// Erohim
					lpTargetObj->Class == 309 ||	// Hellmaine
					lpTargetObj->Class == 459 ||	// Selupan
					lpTargetObj->Class == 476 ||	// Cursed Santa
					lpTargetObj->Class == 481 ||	// Zombie
					lpTargetObj->Class == 483 ||	// Gladiator
					lpTargetObj->Class == 485 ||	// Slaughterer
					lpTargetObj->Class == 488 ||	// Assassin
					lpTargetObj->Class == 491 ||	// Lava Giant
					lpTargetObj->Class == 493 ||	// Golden Dark Knight
					lpTargetObj->Class == 494 ||	// Golden Devil
					lpTargetObj->Class == 495 ||	// Golden Golem
					lpTargetObj->Class == 496 ||	// Golden Wing
					lpTargetObj->Class == 497 ||	// Golden Satyr
					lpTargetObj->Class == 498 ||	// Golden Twin Tail
					lpTargetObj->Class == 499 ||	// Golden Knight
					lpTargetObj->Class == 500 ||	// Golden Living Tree
					lpTargetObj->Class == 501 ||	// Great Golden Derkon
					lpTargetObj->Class == 502 ||	// Golden Rabbit
					lpTargetObj->Class == 561 ||	// Medusa
					lpTargetObj->Class == 627 ||	// Cursed Fire Tower
					lpTargetObj->Class == 628 ||	// Cursed Water Tower
					lpTargetObj->Class == 629 ||	// Cursed Earth Tower
					lpTargetObj->Class == 630 ||	// Cursed Wind Tower
					lpTargetObj->Class == 631 ||	// Cursed Darkness Tower
					lpTargetObj->Class == 655 ||	// Golden Erohim
					lpTargetObj->Class == 656 ||	// Golden Hellmaine
					lpTargetObj->Class == 657 ||	// Golden Kundun
					lpTargetObj->Class == 673) {	// Lord Silvester
					gObjAddMsgSendDelayInSpecificQPos(lpTargetObj, 1, lpCallObj->m_Index, 800, 0, 0);
					...
					if (g_ConfigRead.data.common.BossKillMessage == true) {
						...
						if (iMaxHitUser == -1) iMaxHitUser = lpCallObj->m_Index;
				...
				if (lpTargetObj->m_Attribute == 62) {
					...
					switch (lpTargetObj->Class) {
						...
						case 303: {
								...
								for (int i = g_ConfigRead.server.GetObjectStartUserIndex(); i < g_ConfigRead.server.GetObjectMax(); i++) {
									if (gObj[i].Connected == PLAYER_PLAYING && gObj[i].Type == OBJ_USER) IOCP.DataSend(i, (LPBYTE)&pNotice, pNotice.h.size);
								}
							}
						break;
			...
			} else g_Kanturu.KanturuMonsterDieProc(lpTargetObj->m_Index, lpCallObj->m_Index);

			if (lpTargetObj->Class >= 678 && lpTargetObj->Class <= 680) KillLordSilvesterRecallMon(lpTargetObj->m_RecallMon, lpTargetObj->m_Index);	
			if (BC_MAP_RANGE(lpTargetObj->MapNumber)) {
				int iBridgeIndex = g_BloodCastle.GetBridgeIndex(lpTargetObj->MapNumber);

				if (lpTargetObj->Class == 131) g_BloodCastle.DestroyCastleDoor(iBridgeIndex, lpTargetObj);
				if (BC_STATUE_RANGE(lpTargetObj->Class - 132)) g_BloodCastle.DestroySaintStatue(iBridgeIndex, lpTargetObj);
			}
			...
			if (g_ArcaBattle.IsArcaBattleServer() == TRUE && lpTargetObj->Type >= OBJ_MONSTER && g_ArcaBattle.GetState() == 7) {
				...
				if (lpTargetObj->Class == 592 || lpTargetObj->Class == 593 || lpTargetObj->Class == 594 || lpTargetObj->Class == 595 || lpTargetObj->Class == 596 || lpTargetObj->Class == 611 || lpTargetObj->Class == 612 || lpTargetObj->Class == 613 || lpTargetObj->Class == 614 || lpTargetObj->Class == 615) g_ArcaBattle.AddContributePoint(lpObj, lpTargetObj);
				
				g_ArcaBattle.AddKillPoint(lpObj, lpTargetObj);
			}

			if (CC_MAP_RANGE(lpTargetObj->MapNumber)) {
				if (rand() % 100 < 50) g_ChaosCastle.SearchNBlowObjs(lpTargetObj->MapNumber,lpTargetObj->X,lpTargetObj->Y);
			...
			if (lpTargetObj->MapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL) {
				if (rand() % 100 < 50) g_ChaosCastleFinal.SearchNBlowObjs(lpTargetObj->MapNumber, lpTargetObj->X, lpTargetObj->Y);

				g_ChaosCastleFinal.DelMonsterList(lpTargetObj->m_Index);

				int nMaxHitterIndex = gObjMonsterTopHitDamageUser(lpTargetObj);

				if (nMaxHitterIndex != -1 && gObj[nMaxHitterIndex].MapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL && gObjIsConnected(nMaxHitterIndex))	gObj[nMaxHitterIndex].m_byKillMobCount++;
			}
			...
			if (lpTargetObj->MapNumber == MAP_INDEX_DEVILSQUARE_FINAL && g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE && lpObj->Type == OBJ_USER) {
				...
				g_Log.Add("[DSF][MonsterKill][%s][%s][%s][%d] MonsterType:%d, KillPoint:%d", lpObj->AccountID, lpObj->CharacterData->m_RealNameOfUBF, lpObj->Name, lpObj->CharacterData->m_nServerCodeOfHomeWorld, lpTargetObj->Class, nPoint);
			}

			CHARACTER_PkCheck(lpCallObj,lpTargetObj->Level);

			if (lpCallObj->Type == OBJ_USER) gObjAddMsgSendDelay(lpCallObj, 3, lpTargetObj->m_Index, 2000, 0);
		} else if (lpTargetObj->Type == OBJ_USER) {
			if (BC_MAP_RANGE(lpTargetObj->MapNumber)) {
			...
			if (IT_MAP_RANGE(lpTargetObj->MapNumber)) g_IT_Event.DropRelicsItem(lpTargetObj->MapNumber, lpTargetObj->m_Index);
			if (CC_MAP_RANGE(lpTargetObj->MapNumber)) {
			...
			if (lpTargetObj->MapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL) {
				...
				if (lpCallObj->MapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL && gObjIsConnected(lpObj) == TRUE) lpCallObj->m_byKillUserCount++;
			...
			CHARACTER_Die(lpTargetObj,lpCallObj);

			int iGuildWar = gObjGuildWarCheck(lpCallObj,lpTargetObj);
			int iDuel = gObjDuelCheck(lpCallObj,lpTargetObj);

			if (iGuildWar == 0 && iDuel == 0) {
				if (lpTargetObj->m_bAttackerKilled == 0) CHARACTER_Killer(lpCallObj, lpTargetObj, MSBDamage);
			}

			lpTargetObj->m_bAttackerKilled = 0;

			if (lpTargetObj->m_RecallMon >= 0) gObjMonsterCallKill(lpTargetObj->m_Index);
		...
		if (gObjTargetGuildWarCheck(lpTargetObj,lpCallObj) == TRUE) lpTargetObj->KillerType = 2;
		...
		GSProtocol.PROTOCOL_DiePlayerSend(lpTargetObj,lpTargetObj->m_Index,Skill,lpCallObj->m_Index);

		if (lpTargetObj->Class >= 524 && lpTargetObj->Class <= 528 && lpTargetObj->Class != 526) g_ImperialGuardian.DestroyGate(lpTargetObj->m_nZoneIndex, lpTargetObj->m_Index, lpObj->m_Index);
		if (lpObj->Class == 523) g_Log.Add("[IMPERIALGUARDIAN] Player Killed by Trap [AccountID]:%s, [NAME]:%s, [ZONE]:%d", lpTargetObj->AccountID, lpTargetObj->Name, lpObj->m_nZoneIndex + 1);
		if (lpObj->Class >= 504 && lpObj->Class <= 521) g_Log.Add("[IMPERIALGUARDIAN] Player Dead [Zone]:%d [AccountID]:%s, [NAME]:%s by Monster => %s(%d)", lpObj->m_nZoneIndex + 1, lpTargetObj->AccountID, lpTargetObj->Name, lpObj->Name, lpObj->m_Index);
		if (lpTargetObj->Class >= 504 && lpTargetObj->Class <= 521) g_Log.Add("[IMPERIALGUARDIAN] Monster Dead [TYPE]: %d,[NAME]: %s, [ZONE]:%d", lpTargetObj->Class, lpTargetObj->Name, lpTargetObj->m_nZoneIndex + 1);
		if ((lpTargetObj->Class >= 504 && lpTargetObj->Class <= 511) || lpTargetObj->Class == 526) g_ImperialGuardian.DropItem(lpObj->m_Index, lpTargetObj->m_Index);
		if (lpTargetObj->MapNumber == MAP_INDEX_HATCHERY) {
			if (lpTargetObj->Type >= OBJ_MONSTER) {
		...
		if (lpTargetObj->m_RecallMon >= 0) return;
		if (lpTargetObj->Class == 340 || lpTargetObj->Class == 348 || lpTargetObj->Class == 349) return;
		if (lpTargetObj->Class == 287 || lpTargetObj->Class == 286) return;
		if (lpTargetObj->Class == 275) return;
		if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) {
			if (lpTargetObj->Class == 288) return;
		}

		if (lpTargetObj->Class == 295) return;
		if (g_ConfigRead.server.GetServerType() != SERVER_CASTLE) {
			if (lpTargetObj->Class == 362 || lpTargetObj->Class == 363 || lpTargetObj->Class == 361) return;
		}

		if (lpTargetObj->Class == 412) return;
		if (lpTargetObj->Class >= 524 && lpTargetObj->Class <= 528 && lpTargetObj->Class != 526) return;
		if (lpTargetObj->MapNumber == MAP_INDEX_DEVILSQUARE_FINAL) return;
		if (lpCallObj->Type == OBJ_USER) {
			if (lpTargetObj->m_btExpType == 1) CHARACTER_ExperienceDivisionSpecial(lpTargetObj, lpCallObj, AttackDamage, MSBFlag);
			else {
				if (lpCallObj->PartyNumber >= 0) CHARACTER_ExperienceParty(lpCallObj, lpTargetObj, AttackDamage, MSBFlag);
				else if (lpTargetObj->Type == OBJ_MONSTER) CHARACTER_ExperienceDivision(lpTargetObj, lpCallObj, AttackDamage, MSBFlag);
			}

			if (lpTargetObj->Type == OBJ_MONSTER && DS_MAP_RANGE(lpTargetObj->MapNumber)) g_DevilSquare.DS_ScoreDivision(lpTargetObj,lpCallObj,AttackDamage,MSBFlag);
		}

		if (lpTargetObj->Type == OBJ_MONSTER) {
			if (lpTargetObj->m_bIsInMonsterHerd != 0 && lpTargetObj->m_lpMonsterHerd != 0) lpTargetObj->m_lpMonsterHerd->MonsterDieAction(lpTargetObj);
		}
	} else {
		if (lpTargetObj->Class == 673) {
			if (!lpTargetObj->m_nPhaseLordSilvester && lpTargetObj->MaxLife * 0.7 >= lpTargetObj->Life) gObjMonsterSummonSkillForLordSilvester(lpTargetObj);
			if (lpTargetObj->m_nPhaseLordSilvester == 1 && lpTargetObj->MaxLife * 0.5 >= lpTargetObj->Life) gObjMonsterSummonSkillForLordSilvester(lpTargetObj);
			if (lpTargetObj->m_nPhaseLordSilvester == 2 && lpTargetObj->MaxLife * 0.2 >= lpTargetObj->Life) gObjMonsterSummonSkillForLordSilvester(lpTargetObj);
		}

		if (iElementalDamage == TRUE) GSProtocol.PROTOCOL_ElementalDamageSend(lpCallObj->m_Index,lpTargetObj->m_Index,AttackDamage,MSBDamage);
		else if (DamageSendType == 0) GSProtocol.PROTOCOL_DamageSend(lpCallObj->m_Index,lpTargetObj->m_Index,AttackDamage,MSBFlag,MSBDamage,iShieldDamage);
		else if (DamageSendType == 1) GSProtocol.PROTOCOL_DamageSendPoison(lpTargetObj->m_Index,AttackDamage,iShieldDamage);
		else if (DamageSendType == 2) GSProtocol.PROTOCOL_DamageSend(lpCallObj->m_Index,lpTargetObj->m_Index,AttackDamage,MSBFlag,5,iShieldDamage);
		else if (DamageSendType == 3) GSProtocol.PROTOCOL_DamageSend(lpCallObj->m_Index,lpTargetObj->m_Index,AttackDamage,MSBFlag,6,iShieldDamage);
	}

	if (lpTargetObj->Type == OBJ_MONSTER) GSProtocol.GCSendObjectHP(lpCallObj->m_Index, lpTargetObj->m_Index);
	if (lpTargetObj->Type == OBJ_MONSTER) {
		for (int i = 0; i < MAX_VIEWPORT; i++) {
			if (lpTargetObj->VpPlayer[i].type == OBJ_USER) GSProtocol.GCObjectLifeInfo(lpTargetObj->VpPlayer[i].number, lpTargetObj->m_Index, lpTargetObj->MaxLife + lpTargetObj->AddLife, lpTargetObj->Life);
		}
	}
}
...
BOOL gObjInventoryTrans(int aIndex) {
	...
	if (gObj[aIndex].pTransaction == 1 && gObj[aIndex].CharacterData->ISBOT == FALSE) {
		g_Log.Add("[%s][%s] error-L3 : pTransaction(%d) status error %s %d", gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].pTransaction, __FILE__, __LINE__);
		return FALSE;
	}
	...
	INVENTORY_SetPointer2(&gObj[aIndex]);
...
int gObjInventoryCommit(int aIndex) {
	...
	INVENTORY_SetPointer1(&gObj[aIndex]);
...
void gObjInventoryItemSet(int aIndex, int itempos, BYTE set_byte) {
	int width;
	int height;
	if (itempos < INVETORY_WEAR_SIZE || itempos == 236) return;
	else if (itempos > (INVENTORY_SIZE-1)) return;
...
void gObjInventoryItemBoxSet(int aIndex, int itempos, int xl, int yl, BYTE set_byte) {
	...
	if (!CheckOutOfInventory(aIndex, itemposy, yl)) return;
	for (int y = 0; y < yl; y++) {
		yy = itemposy + y;
		...
		for (int x = 0; x < xl; x++) {
			...
			if (InventoryExtentCheck(xx, yy, 8, MAX_INVENTORY_H2)) *(BYTE*)(gObj[aIndex].pInventoryMap + (itemposy + y) * 8 + (itemposx + x)) = set_byte;
			else return;
		}
	}
}
...
BYTE gObjInventoryDeleteItem(int aIndex ,int itempos) {
	if (itempos < INVETORY_WEAR_SIZE || itempos == 236) {
		int MagicDel = MAGIC_MagicDel(&gObj[aIndex],gObj[aIndex].pInventory[itempos].m_Special[0],gObj[aIndex].pInventory[itempos].m_Level);
...
BYTE gObjInventoryInsertItem(int aIndex, CMapItem * item) {
	...
	if (item->GetSize((int &)iwidth, (int &)iheight) == 0) return -1;
	for (h = 0; h < MAX_INVENTORY_H; h++) {
		for (w = 0; w < 8; w++) {
			if (*(BYTE*)(gObj[aIndex].pInventoryMap + h * 8 + w) == 255) {
				blank = gObjOnlyInventoryRectCheck(aIndex, w, h, iwidth, iheight);
...
BYTE gObjOnlyInventoryRectCheck(int aIndex, int sx, int sy, int width, int height) {
	...
	if (sy + height > 4 * gObj[aIndex].CharacterData->m_InventoryExpansion + 8) {
		return -2;
	}
	...
	for (y = 0; y < height; y++) {
		...
		for (x = 0; x < width; x++) {
			...
			if (InventoryExtentCheck(xx, yy, 8, MAX_INVENTORY_H)) {
				if (*(BYTE*)(gObj[aIndex].pInventoryMap + (sy + y) * 8 + (sx + x)) != 255) {
					blank += 1;
					return -1;
				}
	...
	for (h = 0; h<MAX_INVENTORY_H; h++) {
		for (w = 0; w < 8; w++) {
			if (*(BYTE*)(lpObj->pInventoryMap + (h)* 8 + (w)) == 255) {
				...
				if (blank == 0xFE) return FALSE;

				if (blank != 0xFF) return TRUE;
			}
		}
	}
...
BOOL gObjIsItemPut(LPOBJ lpObj, CItem * lpItem, int pos) // check this
{
	...
	if ((lpObj->CharacterData->Strength + lpObj->AddStrength) < (lpItem->m_RequireStrength - lpItem->m_HJOpStrength))
	{
		return false;
	}

	if ((lpObj->CharacterData->Dexterity + lpObj->AddDexterity) < (lpItem->m_RequireDexterity - lpItem->m_HJOpDexterity))
	{
		return false;
	}
	...
	if (lpObj->Level + lpObj->CharacterData->MasterLevel < lpItem->m_RequireLevel)
	{
		return false;
	}
	...
	{
		if ((lpObj->CharacterData->Vitality + lpObj->AddVitality) < lpItem->m_RequireVitality)
		{
			return false;
		}

		if ((lpObj->CharacterData->Energy + lpObj->AddEnergy) < lpItem->m_RequireEnergy)
		{
			return false;
		}
	}
...
BYTE gObjWerehouseRectCheck(int aIndex, int sx, int sy, int width, int height)
{
	...
	if (!CHARACTER_CheckOutOfWarehouse(aIndex, sy, height))
	{
		return -1;
	}
	...
	for (y = 0; y < height; y++)
	{
		...
		for (x = 0; x < width; x++)
		{
			...
			if (CHARACTER_WarehouseExtentCheck(xx, yy, 8, MAX_WAREHOUSE_H))
			{
...
void gObjWarehouseItemBoxSet(int aIndex, int itempos, int xl, int yl, BYTE set_byte)
{
	...
	if (!CHARACTER_CheckOutOfWarehouse(aIndex, itemposy, yl))
	{
		return;
	}
...
BYTE gObjWarehouseInsertItemPos(int aIndex, CItem item, int pos, int source)
{
	...
	if (!CHARACTER_WarehouseExtentCheck(w, h, 8, MAX_WAREHOUSE_H))
	{
		return -1;
	}
...
BYTE gObjInventoryInsertItemPos(int aIndex, CItem item, int pos, BOOL RequestCheck)
{
	...
	if (pos < 12 || pos == 236)
	{
		useClass = item.IsClass(lpObj->Class, lpObj->CharacterData->ChangeUP);
	...
	if (pos < 12 || pos == 236)
	{
		...
		if (lpItem->m_Option1)
		{
			int result = MAGIC_WeaponMagicAdd(lpObj, lpItem->m_Special[0], lpItem->m_Level);
...
BYTE gObjInventoryMoveItem(int aIndex, unsigned char source, unsigned char target, int& durSsend, int& durTsend, unsigned char sFlag, unsigned char tFlag, unsigned char* siteminfo)
{
	...
	__try
	{
		...
		case 17:
			...
			if (lpObj->CharacterData->pPentagramMixBox[source].IsItem() == 0)
			{
				g_Log.Add("Item does not exist [%s][%s] %d", lpObj->AccountID, lpObj->Name, __LINE__);
				return -1;
			}

			sitem = &lpObj->CharacterData->pPentagramMixBox[source];
			break;
		default: return -1;
		}
		...
		case 17:
			...
			titem = &lpObj->CharacterData->pPentagramMixBox[target];
			break;
		default: return -1;
		}
			...
			if (t_dur < max_count)
			{
				...
				if (titem->m_Type == ITEMGET(14, 29) && max_count <= titem->m_Durability)
				{
					...
					if (titem->m_Durability == 0)
					{
						...
						GSProtocol.PROTOCOL_InventoryItemDeleteSend(aIndex, target, 1);
						durTsend = 0;
					}
					else
					{
				...
				else
				{
					...
					case 0:
						...
						GSProtocol.PROTOCOL_InventoryItemDeleteSend(lpObj->m_Index, source, 1);
						durTsend = 1;
					default:	return 0;
					}
				}
		...
		case 4:
			if (target < INVETORY_WEAR_SIZE || target == 236)
			{
				...
				if (g_MineSystem.IsPickax(lpObj->pInventory[source].m_Type) == TRUE && lpObj->m_bPShopOpen) return -1;

				useClass = sitem->IsClass(lpObj->Class,lpObj->CharacterData->ChangeUP);
			...
			BOOL bSendEquip;
				...
				case 4:
					...
					if ((source == 11 || source == 10) && (sitem->m_Type == ITEMGET(13, 169) || sitem->m_Type == ITEMGET(13, 170)) || (target == 11 || target == 10) && (titem->m_Type == ITEMGET(13, 169) || titem->m_Type == ITEMGET(13, 170))) bSendEquip = TRUE;
					...
					INVENTORY_SetEquipment(lpObj, target);
				...
				case 2:
					...
					break;
				case 17:
					lpObj->pInventory[target] = lpObj->CharacterData->pPentagramMixBox[source];
				...
				if (target < INVETORY_WEAR_SIZE || target == 236)
				{
					...
					if (lpItem->m_Option1)
					{
						int s_pos = MAGIC_WeaponMagicAdd(&gObj[aIndex],lpItem->m_Special[0],lpItem->m_Level);
						if (s_pos >= 0)
						{
		...
		case 17:
			BYTE res_3;
				if (sFlag == 17)
				{
					res_3 = gObjPentagramMixBoxInsertItemPos(aIndex, lpObj->CharacterData->pPentagramMixBox[source], target, source);
				}
				...
				if (sFlag == 17)
				{
					lpObj->CharacterData->pPentagramMixBox[target] = lpObj->CharacterData->pPentagramMixBox[source];
					gObjPentagramMixBoxDeleteItem(aIndex, source);
				}

				else
				{
					lpObj->CharacterData->pPentagramMixBox[target] = lpObj->pInventory[source];
					gObjInventoryDeleteItem(aIndex, source);
				}
...
BYTE gObjTradeInventoryMove(LPOBJ lpObj, BYTE source, BYTE target)
{
	...
	if (gObj[lpObj->TargetNumber].CharacterData->ISBOT == true && g_BotSystem.GetBotType(lpObj->TargetNumber) == BOT_TRADE)
	{
		g_BotSystem.CheckAlchemist(lpObj->m_Index,gObj[lpObj->TargetNumber].CharacterData->wBotIndex);
	}
...
BYTE gObjInventoryTradeMove(LPOBJ lpObj, BYTE source, BYTE target)
{
	...
	if (gObj[lpObj->TargetNumber].CharacterData->bt_BotType == 1)
	{
		//check for item
	...
	if (*(BYTE*)(lpObj->TradeMap + h * 8 + w) == 255)
	{
		...
		/*if (lpObj->CharacterData->ISBOT == TRUE && g_BotSystem.GetBotType(lpObj->m_Index) == BOT_TRADE)
		{
...
void gObjTradeCancel(int aIndex)
{
	...
	GSProtocol.GCMoneySend(aIndex,gObj[aIndex].CharacterData->Money);
	...
	if (gObj[aIndex].CharacterData->GuildNumber > 0)
	{
		GSProtocol.GCGuildViewportNowPaint(aIndex,gObj[aIndex].CharacterData->GuildName,0,0);
...
BYTE gObjTempInventoryInsertItem(LPOBJ lpObj, CItem item, BYTE * TempMap)
{
	...
	for (h = 0; h < 4 * lpObj->CharacterData->m_InventoryExpansion + 8; h++)
	{
		for (w = 0; w < 8; w++)
		{
		...
		if (gObj[number].Trade[n].IsItem() == 1)
		{
			...
			if (insert != 255)
			{
				...
				g_Log.Add("[%s][%s][%s] (%d,%d,%d) Trade Item  Opposite :[%s][%s][%s] (%d,%d,%d) (Item:%s/Serial:%I64d [%d][%d][%d][%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d] E:[%d]",
					gObj[number].AccountID,gObj[number].Name,gObj[number].CharacterData->Ip_addr,gObj[number].MapNumber,gObj[number].X,gObj[number].Y,
					gObj[aIndex].AccountID,gObj[aIndex].Name,gObj[aIndex].CharacterData->Ip_addr,gObj[aIndex].MapNumber,gObj[aIndex].X,gObj[aIndex].Y,
...
void gObjTradeOkButton(int aIndex) {
	...
	if (result == 1) {
		...
		gObj[aIndex].CharacterData->Money -= gObj[aIndex].TradeMoney;

		if (gObj[aIndex].CharacterData->Money < 0) {
			gObj[aIndex].CharacterData->Money = 0;
		}

		gObj[number].CharacterData->Money -= gObj[number].TradeMoney;

		if (gObj[number].CharacterData->Money < 0) {
			gObj[number].CharacterData->Money = 0;
		}

		gObj[aIndex].CharacterData->Money += gObj[number].TradeMoney;
		gObj[number].CharacterData->Money += gObj[aIndex].TradeMoney;
	...
	GSProtocol.GCMoneySend(aIndex,gObj[aIndex].CharacterData->Money);
	GSProtocol.GCMoneySend(number,gObj[number].CharacterData->Money);
	GSProtocol.CGTradeResult(aIndex,result);
	GSProtocol.CGTradeResult(number,result);
	GCItemListSend(aIndex);
...
void gObjMakePreviewCharSet(int aIndex) {
	...
	if (g_ConfigRead.SeasonX == false) {
		...
		if (lpObj->CharacterData->ChangeUP == 1) lpObj->CharSet[CS_CLASS] |= CS_SET_CHANGEUP;
		...
		if (lpObj->CharacterData->ChangeUP == 1) lpObj->CharSet[CS_CLASS] |= CS_SET_CHANGEUP_SX;
		if (lpObj->CharacterData->ChangeUP == 2) {
	...
	// Set Class
	if (g_ConfigRead.SeasonX == false) {
		if (lpObj->m_ActionNumber == 0x80) lpObj->CharSet[CS_CLASS] |= 2;
		else if (lpObj->m_ActionNumber == 0x81) lpObj->CharSet[CS_CLASS] |= 3;
		else lpObj->CharSet[CS_CLASS] = lpObj->CharSet[CS_CLASS];
	} else {
		if (lpObj->m_ActionNumber == 0x80) lpObj->CharSet[CS_CLASS] |= 1;
		else if (lpObj->m_ActionNumber == 0x81) lpObj->CharSet[CS_CLASS] |= 2;
		else lpObj->CharSet[CS_CLASS] = lpObj->CharSet[CS_CLASS];
	}
	...
	// Set Helmet
	if (lpObj->pInventory[2].m_Type < ITEMGET(0,0) && lpObj->CharacterData->ISBOT == false) {
	...
	// Set Armor
	if (lpObj->pInventory[3].m_Type < ITEMGET(0,0) && lpObj->CharacterData->ISBOT == false) {
	...
	// Set Pants
	if (lpObj->pInventory[4].m_Type < ITEMGET(0,0) && lpObj->CharacterData->ISBOT == false) {
	...
	// Set Gloves
	if (lpObj->pInventory[5].m_Type < ITEMGET(0,0) && lpObj->CharacterData->ISBOT == false) {
	...
	// Set Boots
	if (lpObj->pInventory[6].m_Type < ITEMGET(0,0) && lpObj->CharacterData->ISBOT == false) {
	...
	// Set Part of Wings
	if (lpObj->pInventory[7].m_Type < ITEMGET(0,0)) index = 0;
		
	index |= 3;
	
	if (lpObj->pInventory[8].m_Type > 0) {
		if (lpObj->pInventory[8].m_Type == ITEMGET(13,67) || lpObj->pInventory[8].m_Type == ITEMGET(13,123)) index = 2;
		else index = lpObj->pInventory[8].m_Type & 3;
	}

	int levelindex;
	lpObj->CharSet[5] |= index;

	if (lpObj->CharacterData->ISBOT == false) {
	...
	if (lpObj->pInventory[6].IsSetItem() != FALSE) lpObj->CharSet[11] |= 0x8;
	...
	if (lpObj->CharacterData->IsFullSetItem != false) lpObj->CharSet[11] |= 0x01;
...
*/
	if ((lpObj->pInventory[8].m_Type & 0x03) != 0) {
		if (lpObj->pInventory[8].m_Type > 0) lpObj->CharSet[10] |= 0x01;
	}

	if (lpObj->pInventory[lpObj->m_btInvenPetPos].m_Type == ITEMGET(13,4)) lpObj->CharSet[12] |= 0x01; // Dark Horse
	...
	if ((lpObj->pInventory[10].IsItem() == TRUE && lpObj->pInventory[10].m_Type == ITEMGET(13,169) && lpObj->pInventory[10].m_Durability > 0.0) || (lpObj->pInventory[11].IsItem() == TRUE && lpObj->pInventory[11].m_Type == ITEMGET(13,169) && lpObj->pInventory[11].m_Durability > 0.0)) lpObj->CharSet[12] |= 0x08;
	if ((lpObj->pInventory[10].IsItem() == TRUE && lpObj->pInventory[10].m_Type == ITEMGET(13,170) && lpObj->pInventory[10].m_Durability > 0.0) || (lpObj->pInventory[11].IsItem() == TRUE && lpObj->pInventory[11].m_Type == ITEMGET(13,170) && lpObj->pInventory[11].m_Durability > 0.0)) lpObj->CharSet[12] |= 0x08;
	if (lpObj->pInventory[lpObj->m_btInvenPetPos].m_Type == ITEMGET(13,37)) { // Fenrir
		...
		if (lpObj->pInventory[lpObj->m_btInvenPetPos].IsFenrirIncLastAttackDamage() != FALSE) lpObj->CharSet[16] |= 0x01; //1
		if (lpObj->pInventory[lpObj->m_btInvenPetPos].IsFenrirDecLastAttackDamage() != FALSE) lpObj->CharSet[16] |= 0x02; //2
		if (lpObj->pInventory[lpObj->m_btInvenPetPos].IsFenrirIllusion() != FALSE) lpObj->CharSet[17] |= 0x01; //4
	}
	...
	if (g_ConfigRead.SeasonX == false) {
		if ((lpObj->pInventory[7].m_Type >= ITEMGET(12, 36) && lpObj->pInventory[7].m_Type <= ITEMGET(12, 40)) || lpObj->pInventory[7].m_Type == ITEMGET(12, 43) || lpObj->pInventory[7].m_Type == ITEMGET(12, 50)) {
			...
			if (lpObj->pInventory[7].m_Type == ITEMGET(12, 43)) lpObj->CharSet[9] |= 6;
			else if (lpObj->pInventory[7].m_Type == ITEMGET(12, 50)) lpObj->CharSet[9] |= 7;
			else if (lpObj->pInventory[7].m_Type == ITEMGET(12, 51)) lpObj->CharSet[9] |= 8;
			else lpObj->CharSet[9] |= (lpObj->pInventory[7].m_Type - 35) & 7;
			...
			if (lpObj->pInventory[7].m_Type == ITEMGET(13, 30)) lpObj->CharSet[9] |= 5;
			else if (lpObj->pInventory[7].m_Type == ITEMGET(12, 42)) lpObj->CharSet[9] |= 6;
			else if (lpObj->pInventory[7].m_Type == ITEMGET(12, 49)) lpObj->CharSet[9] |= 7;
			else lpObj->CharSet[9] |= (lpObj->pInventory[7].m_Type - 2) & 7;
			...
			if (lpObj->pInventory[7].m_Type == ITEMGET(12, 41)) lpObj->CharSet[9] |= 4;
			else lpObj->CharSet[9] |= (lpObj->pInventory[7].m_Type + 1) & 7;
		} else if (lpObj->pInventory[7].m_Type >= ITEMGET(12, 51) && lpObj->pInventory[7].m_Type <= ITEMGET(12, 57)) {
			lpObj->CharSet[5] |= CS_SET_WING1(3);

			if (lpObj->pInventory[7].m_Type == ITEMGET(12, 51)) lpObj->CharSet[16] |= 8;
			else if (lpObj->pInventory[7].m_Type == ITEMGET(12, 52)) lpObj->CharSet[16] |= 9;
			else if (lpObj->pInventory[7].m_Type == ITEMGET(12, 53)) lpObj->CharSet[16] |= 10;
			else if (lpObj->pInventory[7].m_Type == ITEMGET(12, 54)) lpObj->CharSet[16] |= 11;
			else if (lpObj->pInventory[7].m_Type == ITEMGET(12, 55)) lpObj->CharSet[16] |= 12;
			else if (lpObj->pInventory[7].m_Type == ITEMGET(12, 56)) lpObj->CharSet[16] |= 13;
			else if (lpObj->pInventory[7].m_Type == ITEMGET(12, 57)) lpObj->CharSet[16] |= 14;
			...
			if (lpObj->pInventory[7].m_Type == ITEMGET(12, 266) || lpObj->pInventory[7].m_Type == ITEMGET(12, 268)) lpObj->CharSet[9] |= 5;
			else if (lpObj->pInventory[7].m_Type == ITEMGET(12, 267)) lpObj->CharSet[9] |= 6;
		} else if (lpObj->pInventory[7].m_Type == ITEMGET(12, 269) || lpObj->pInventory[7].m_Type == ITEMGET(12, 270)) {
			lpObj->CharSet[9] |= 3;

			if (lpObj->pInventory[7].m_Type == ITEMGET(12, 269)) lpObj->CharSet[16] |= 28;
			else if (lpObj->pInventory[7].m_Type == ITEMGET(12, 270)) lpObj->CharSet[16] |= 32;
		}
	} else {
		WORD ItemIndex = ITEM_GET_INDEX(lpObj->pInventory[7].m_Type);

		if (ItemIndex >= 0 && ItemIndex <= 6) lpObj->CharSet[16] |= (0x04 * (ItemIndex + 1));
			...
			if (ItemIndex == 49 || ItemIndex == 50) lpObj->CharSet[16] |= (0x04 * (ItemIndex - 49));
			else if (ItemIndex >= 130 && ItemIndex <= 135) lpObj->CharSet[16] |= (0x04 * (0x02 + (ItemIndex - 130)));
		} else if (ItemIndex >= 262 && ItemIndex <= 269) {
			lpObj->CharSet[9] |= 0x03;

			if (ItemIndex >= 262 && ItemIndex <= 265) lpObj->CharSet[16] |= (0x04 * (ItemIndex - 262));
			if (ItemIndex == 266 || ItemIndex == 268) lpObj->CharSet[16] |= 0x10;
			if (ItemIndex == 267) lpObj->CharSet[16] |= 0x14;
			if (ItemIndex == 269) lpObj->CharSet[16] |= 0x1C;
		} else if (ItemIndex == 30) {
			lpObj->CharSet[9] |= 0x03;
			lpObj->CharSet[16] |= 0x18;
		} else if (ItemIndex == 270) lpObj->CharSet[9] |= 0x04;
	...
	for (n=0;n<g_ConfigRead.server.GetObjectMax();n++)
	{
		if (gObj[n].Live != FALSE)
		{
			if (gObj[n].Connected != PLAYER_EMPTY)
			{
				...
				{
					if (gObj[n].Type == OBJ_USER && gObj[n].CharacterData->ISBOT == false)
					{
						playerc++;
					}
				}
		...
		if (gObj[n].Type == OBJ_USER && gObj[n].Connected != PLAYER_EMPTY && gObj[n].CharacterData->ISBOT == false)
		{
			totalplayer++;
		}
...
void gObjStateSetCreate(int aIndex)
{
	...
	else if (lpObj->Teleport == 2)
	{
		...
		if (lpObj->Type == OBJ_USER)
		{
			...
			GSProtocol.PROTOCOL_TeleportSend(&gObj[aIndex],0,lpObj->MapNumber,lpObj->X,lpObj->Y,lpObj->Dir);
		...
		if (lpObj->Connected > PLAYER_LOGGED)
		{
			...
			if (lpObj->DieRegen == 2)
			{
				...
				if (State != 0)
				{
					...
					BUFF_RemoveBuffEffect(lpObj, DEBUFFTYPE_POISON);
					BUFF_RemoveBuffEffect(lpObj, DEBUFFTYPE_CHILL);
					BUFF_RemoveBuffEffect(lpObj, DEBUFFTYPE_ICE_ARROW);
					...
					if (lpObj->Type == OBJ_USER)
					{
						...
						if (lpObj->CharacterData->ISBOT == false && lpObj->m_Index >= g_ConfigRead.server.GetObjectStartUserIndex())
						{
						...
						else if (lpObj->KillerType == 2)
						{
							...
							if (lpObj->CharacterData->lpGuild != 0)
							{
							...
							else if (lpObj->CharacterData->lpGuild->WarState)
							{
								int BattleGroundIndex = lpObj->CharacterData->lpGuild->BattleGroundIndex;
								gBattleGetTeamPosition(BattleGroundIndex,lpObj->CharacterData->lpGuild->BattleTeamCode,(short &)lpObj->X,(short &)lpObj->Y);
							}
						}

						else if (lpObj->MapNumber == 6 && lpObj->CharacterData->RegisterdLMS == TRUE)
						{
							int room = lpObj->CharacterData->RegisteredLMSRoom;
						...
						UINT64 Experience = lpObj->CharacterData->Experience;

						if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(lpObj) == TRUE)
						{
							Experience = lpObj->CharacterData->MasterExperience;
						}
						...
						pMsg.Money = lpObj->CharacterData->Money;
						pMsg.wShield = lpObj->iShield;
						pMsg.Stamina = lpObj->Stamina;
...
void gObjSecondProc() {
	if (CharSaveLog == NULL) CharSaveLog = new CLogToFile("CHAR_SAVE_LOG", g_ConfigRead.server.GetUserSaveLogPath(), 256);
	...
	for(int n = 0; n < g_ConfigRead.server.GetObjectMax(); n++) {
		...
		if (lpObj->Connected > PLAYER_LOGGED) {
			...
			BUFF_DOTEffectTick(lpObj);

			if (lpObj->Type == OBJ_USER) g_CMuunSystem.CheckMuunItemPeriodData(lpObj);
			if (lpObj->Type == OBJ_NPC && lpObj->Class == 229) gQeustNpcTeleport.Run(n);
			if (lpObj->Type == OBJ_NPC && lpObj->Class == 568) gQuestExpNpcTeleport.Run(n);
			if (lpObj->Type == OBJ_MONSTER) {
				if (lpObj->Class == 275) gObjRefillMonsterHP(lpObj,giKundunRefillHPSec);
				if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) {
					if (lpObj->Class == 283) {
				...
				if (lpObj->Class == 653 || lpObj->Class == 654 || lpObj->Class == 655 || lpObj->Class == 656 || lpObj->Class == 657) {
					if (lpObj->Live == TRUE && GetTickCount() - lpObj->m_Disappear_Monster > 1800000) {
						...
						lpObj->PathCount = 0;
						lpObj->m_Disappear_Monster = 0;
						g_Log.Add("[ColossusEvent][GoldMonsterTimeout] MonsterClass : [%d], MonsterIndex : [%d], Map[%d], X[%d], Y[%d]", lpObj->Class, lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y);
						gObjDel(lpObj->m_Index);
					}
				}
				...
				if (lpObj->Class == 674 || lpObj->Class == 675 || lpObj->Class == 676 || lpObj->Class == 677) {
					if (lpObj->Live == TRUE && lpObj->m_Attribute == 60 && GetTickCount() - lpObj->m_Disappear_Monster > 1800000) {
						...
						lpObj->PathCount = 0;
						lpObj->m_Disappear_Monster = 0;
						g_Log.Add("[PetEvent][GoldMonsterTimeout] MonsterClass : [%d], MonsterIndex : [%d], Map[%d], X[%d], Y[%d]", lpObj->Class, lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y);
						gObjDel(lpObj->m_Index);
					}
				}
			...
			if (lpObj->Type == OBJ_NPC) {
				if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) {
					if ((lpObj->Class < 204) ? FALSE : (lpObj->Class > 209) ? FALSE : TRUE) g_Crywolf.CrywolfNpcAct(lpObj->m_Index);
					if (lpObj->Class == 216) {
				...
				if (lpObj->Class == 221 || lpObj->Class == 222) g_CsNPC_Weapon.WeaponAct(lpObj->m_Index);
			}

			if (lpObj->Type == OBJ_USER) {
				PLAYER_RegenManaStamina(lpObj);
				PLAYER_UsePotion(lpObj);				
				...
				if (lpObj->m_dwDCTimer > 0) {
					if ((GetTickCount64() - lpObj->m_dwDCTimer) > 5000) {
						...
						g_Log.AddC(TColor::Red,"(%d)[%s][%s][%s] User did not disconnect upon request. Modified client?", lpObj->m_Index, lpObj->CharacterData->Ip_addr,lpObj->AccountID, lpObj->Name);
					}
				}

				if (g_iUseCharacterAutoRecuperationSystem != 0) {
					if (lpObj->Level <= g_iCharacterRecuperationMaxLevel) gProcessAutoRecuperation(lpObj);
				}
	
				if (lpObj->Type == OBJ_USER) gObjShieldAutoRefill(lpObj);	
				if (lpObj->Type == OBJ_USER && lpObj->m_LastTeleportTime > 0) lpObj->m_LastTeleportTime--;
				if ((GetTickCount() - lpObj->CharacterData->PacketTime) > 1000) {
					lpObj->CharacterData->PacketsPerSecond = 0;
					lpObj->CharacterData->PacketTime = GetTickCount();
				}
				...
				if (lpObj->PartyNumber >= 0) {
					...
					if (lpObj->Type == OBJ_USER) {
						if (lpObj->CharacterData->m_bSendMemberPos) GSProtocol.GCPartyMemberPosition(n);
					}
				}
	
				if (lpObj->m_ShopTime >= 1 && lpObj->m_ShopTime < 255) lpObj->m_ShopTime++;
				...
				if (lpObj->MapNumber == MAP_INDEX_BATTLESOCCER && BattleGround == 1) {
					...
					if (lpObj->CharacterData->lpGuild != 0) {
						if (lpObj->CharacterData->lpGuild->WarState != 0) {
							int CheckBattleGround = gCheckBattleGround(lpObj);
	
							if (CheckBattleGround != lpObj->CharacterData->lpGuild->BattleGroundIndex) {
								if (lpObj->CharacterData->lpGuild->WarType == 1) BattleGround = 0;
								else BattleGround = 0;
							} else BattleGround = 0;
						}
					} else BattleGround = 1;
					...
					if (BattleGround != 0) {
						...
						if (CheckBattleGround >= 0) {
							if (lpObj->MapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL && g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE) gObjMoveGate(lpObj->m_Index, 333);
							else gObjMoveGate(lpObj->m_Index, 17);
						}	
					}
				}
			}
		...
		if (lpObj->Connected >= PLAYER_LOGGED && lpObj->Type == OBJ_USER && lpObj->CloseCount > 0) {
			if (lpObj->CloseCount == 1) {
				...
				} else if (lpObj->CloseType == 2) GSProtocol.GCCloseMsgSend(lpObj->m_Index,2);
			} else {
				char Msg[128];
		...
		if (lpObj->Connected > PLAYER_LOGGED && lpObj->Type == OBJ_USER && lpObj->CharacterData->ISBOT == false) {
			...
			if (lpObj->CharacterData->AntiHackCheckTime > 0 && GetTickCount() - lpObj->CharacterData->AntiHackCheckTime > 180000 && lpObj->m_bOff == false) {
				g_Log.AddC(TColor::Red, "[%s][%s][%s][%s] AntiHack breach -> No response", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Ip_addr, lpObj->CharacterData->HWID);
				AntiHackLog->Output("[%s][%s][%s][%s] AntiHack breach -> No response", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Ip_addr, lpObj->CharacterData->HWID);

				if (g_ConfigRead.antihack.AntiHackBreachDisconnectUser == true) GSProtocol.GCSendDisableReconnect(lpObj->m_Index);
			}

			if (g_ConfigRead.data.common.bHitHackDetection) {
				if (lpObj->CharacterData->dwLastHitHackTick > 0 && GetTickCount64() - lpObj->CharacterData->dwLastHitHackTick > 180000 && lpObj->m_bOff == false) {
					g_Log.AddC(TColor::Red, "[%s][%s][%s][%s] AntiHack breach #2 -> No response", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Ip_addr, lpObj->CharacterData->HWID);
			...
			if (!lpObj->m_bOff) {
				if (lpObj->CheckSumTime > 0 && GetTickCount() - lpObj->CheckSumTime > 3000); // GSProtocol.GCSendDisableReconnect(n);	// Experimental
				...
				if (lpObj->AgiCheckTime > 0 && GetTickCount() - lpObj->AgiCheckTime > 180000) {
					if (g_ConfigRead.antihack.EnabledAgiCheck == true) ;//GSProtocol.GCSendDisableReconnect(n);							// Experimental
					else lpObj->AgiCheckTime = GetTickCount();
				}

				CHARACTER_TradeInterfaceTimeCheck(lpObj);
				gObjTimeCheckSelfDefense(lpObj);
			}
		}
			...
			if (lpObj->Type == OBJ_USER && lpObj->CharacterData->ISBOT == false && lpObj->m_bOff == false) {
				if (lpObj->Connected >= PLAYER_LOGGED) {
					if (GetTickCount() - lpObj->ConnectCheckTime > 60000) {
						...
						g_Log.Add("Game working response error causes conclusion [%d][%s][%s][%s]",lpObj->m_Index,lpObj->AccountID,lpObj->Name,lpObj->CharacterData->Ip_addr);
					}
				} else {
					if (GetTickCount() - lpObj->ConnectCheckTime > 30000) {
						IOCP.ResponErrorCloseClient(n);
						g_Log.Add("Response error after connection causes conclusion [%d][%s][%s][%s]",lpObj->m_Index,lpObj->AccountID,lpObj->Name,lpObj->CharacterData->Ip_addr);
					}
				}

			}

			if (lpObj->Connected == PLAYER_PLAYING && lpObj->Type == OBJ_USER && lpObj->CharacterData->m_MuBotEnable == true) {
				if (GetTickCount() - lpObj->CharacterData->m_MuBotTick > 60000) {
					lpObj->CharacterData->m_MuBotPayTime++;
					lpObj->CharacterData->m_MuBotTotalTime++;
					lpObj->CharacterData->m_MuBotTick = GetTickCount();
				}

				if (lpObj->CharacterData->m_MuBotTotalTime >= g_ConfigRead.data.mubot.autodisabletime && g_ConfigRead.data.mubot.autodisabletime > 0) {
					lpObj->CharacterData->m_MuBotEnable = false;
					GSProtocol.GCAnsMuBotUse(lpObj->m_Index, 1, lpObj->CharacterData->m_MuBotTotalTime, 0);
				}

				if (lpObj->CharacterData->m_MuBotPayTime == 5) gObjMuBotPayForUse(lpObj);
			}
		}
	}
}
void PLAYER_RegenManaStamina(LPOBJ lpObj) {
	...
	if (lpObj->m_ManaFillCount > 2) {
		...
		if (lpObj->MaxMana + lpObj->AddMana > lpObj->Mana) {
			...
			if (!(BC_MAP_RANGE(lpObj->MapNumber) ||	CC_MAP_RANGE(lpObj->MapNumber) || KALIMA_MAP_RANGE(lpObj->MapNumber) ||	DS_MAP_RANGE(lpObj->MapNumber) || IT_MAP_RANGE(lpObj->MapNumber) || lpObj->MapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL) && !(lpObj->MapNumber == MAP_INDEX_CASTLESIEGE || lpObj->MapNumber == MAP_INDEX_CASTLEHUNTZONE)) {
				int iValue = BUFF_GetTotalBuffEffectValue(lpObj, EFFECTTYPE_RECOVERY_MANA); //season3 add-on

				if (iValue > 0) percent += percent * iValue / 100.0;
			}

			if (lpObj->Class == CLASS_RAGEFIGHTER) {
				if (percent > 0.0 && lpObj->CharacterData->Mastery.ML_RF_MentalQuickness > 0.0) percent += lpObj->CharacterData->Mastery.ML_RF_MentalQuickness;
			} else if (percent > 0.0 && lpObj->CharacterData->Mastery.ML_GENERAL_MentalQuickness > 0.0) percent += lpObj->CharacterData->Mastery.ML_GENERAL_MentalQuickness;

			tmana = lpObj->Mana;
			tmana += (lpObj->MaxMana + lpObj->AddMana) * percent / 100.0f;
			// Ring of Magic Mana Regeneration (5%)
			CItem RightRing = lpObj->pInventory[10];
			CItem LeftRing = lpObj->pInventory[11];

			if (LeftRing.IsItem() == TRUE && LeftRing.m_Type == ITEMGET(13, 24)) tmana += (lpObj->MaxMana + lpObj->AddMana) * 5 / 100;
			if (RightRing.IsItem() == TRUE && RightRing.m_Type == ITEMGET(13, 24)) tmana += (lpObj->MaxMana + lpObj->AddMana) * 5 / 100;
			// Elf Wings Mana Regeneration
			if (lpObj->pInventory[7].IsItem() == TRUE && lpObj->pInventory[7].m_IsValidItem == true) {
				...
				if (Wings->m_Durability > 0.0f) {
					switch (Wings->m_Type) {
						case ITEMGET(12, 0):			tmana += (lpObj->MaxMana + lpObj->AddMana) * 3 / 100;				break;
						case ITEMGET(12, 3):			tmana += (lpObj->MaxMana + lpObj->AddMana) * 4 / 100;				break;
						case ITEMGET(12, 265):			tmana += (lpObj->MaxMana + lpObj->AddMana) * 5 / 100;				break;
						case ITEMGET(12, 38):			tmana += (lpObj->MaxMana + lpObj->AddMana) * 7 / 100;				break;
					}
				}
			}

			// Buffs
			if ((BUFF_TargetHasBuff(lpObj, BUFFTYPE_IMPROVED_ARCANE_POWER) || BUFF_TargetHasBuff(lpObj, BUFFTYPE_ARCANE_FOCUS)) && g_MasterLevelSkillTreeSystem.ML_GetNodeLevel(lpObj, 380) == 20) tmana += (lpObj->MaxMana + lpObj->AddMana) * 5 / 100;

			tmana += lpObj->CharacterData->JewelOfHarmonyEffect.JoH_ManaRegeneration;

			if (lpObj->Class == CLASS_SUMMONER && g_MasterLevelSkillTreeSystem.ML_GetNodeLevel(lpObj, 457) >= 10) tmana += (lpObj->MaxMana + lpObj->AddMana) * 5 / 100;
			if (lpObj->CharacterData->m_btRefillMPSocketOption > 0) tmana += lpObj->CharacterData->m_btRefillMPSocketOption;
			if (lpObj->MaxMana + lpObj->AddMana < tmana) tmana = lpObj->MaxMana + lpObj->AddMana;

			lpObj->Mana = tmana;
			ChangeMana = 1;
		}

		Stam = lpObj->MaxStamina + lpObj->AddStamina;

		if (lpObj->Stamina < Stam) {
			MapAttr = MapC[lpObj->MapNumber].GetAttr((short &)lpObj->X,(short &)lpObj->Y);
			rStam = 2;

			if ((MapAttr & 1) != 0) rStam = 5;
			if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_SANTA_LUCKY)) {
				rStam = 5;

				if ((MapAttr & 1) != 0) rStam = 10;
			}

			if (lpObj->Class == CLASS_KNIGHT) AddStam = rStam + (Stam * 5 / 100);
			else AddStam = rStam + (Stam * 3 / 100);
			if (lpObj->Class == CLASS_RAGEFIGHTER) {
				if (lpObj->CharacterData->Mastery.ML_RF_Endurance > 0.0) AddStam += Stam * lpObj->CharacterData->Mastery.ML_RF_Endurance / 100;
			} else if (lpObj->CharacterData->Mastery.ML_GENERAL_Endurance > 0.0) AddStam += Stam * lpObj->CharacterData->Mastery.ML_GENERAL_Endurance / 100;
			if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_SOUL_POTION) != 0) AddStam += 8;
			// Pendant of Ability Stamina Regeneration (5%)
			CItem Amulet = lpObj->pInventory[9];

			if (Amulet.IsItem() == TRUE && Amulet.m_Type == ITEMGET(13, 28)) AddStam += Stam * 5 / 100;
			// Elf Wings Stamina Regeneration
			if (lpObj->pInventory[7].IsItem() == TRUE && lpObj->pInventory[7].m_IsValidItem == true) {
				CItem* Wings = &lpObj->pInventory[7];

				if (Wings->m_Durability > 0.0f) {
					switch (Wings->m_Type) {
						case ITEMGET(12, 0):			AddStam += Stam * 3 / 100;				break;
						case ITEMGET(12, 3):			AddStam += Stam * 4 / 100;				break;
						case ITEMGET(12, 265):			AddStam += Stam * 5 / 100;				break;
						case ITEMGET(12, 38):			AddStam += Stam * 7 / 100;				break;
					}
				}
			}

			AddStam += lpObj->CharacterData->ANCIENT_StaminaRegeneration;

			if (AddStam < 1) AddStam = 1;

			CurStam = lpObj->Stamina;
			CurStam += AddStam;

			if (CurStam > Stam) CurStam = Stam;

			lpObj->Stamina = CurStam;
			ChangeMana = 1;
		}

		if (ChangeMana != 0) GSProtocol.PROTOCOL_ManaSend(lpObj->m_Index, lpObj->Mana, 255, 0, lpObj->Stamina);
	}
}
void PLAYER_UsePotion(LPOBJ lpObj) {
	...
	if (lpObj->m_Rest != 0) {
		...
		if (lpObj->m_LifeFillCount > 4) {
			...
			if (lpObj->pInventory[10].IsItem() == 1) {
				if (lpObj->pInventory[10].m_Option3 != 0 && lpObj->pInventory[10].m_Type != ITEMGET(13, 24)) percent += lpObj->pInventory[10].m_Option3;
			} else if (lpObj->pInventory[11].IsItem() == 1) {
				if (lpObj->pInventory[11].m_Option3 != 0 && lpObj->pInventory[11].m_Type != ITEMGET(13, 24)) percent += lpObj->pInventory[11].m_Option3;
			} else if (lpObj->pInventory[9].IsItem() == 1) {
				if (lpObj->pInventory[9].m_Option3 != 0 && lpObj->pInventory[9].m_Type != ITEMGET(13, 28)) percent += lpObj->pInventory[9].m_Option3;
			}

			if (lpObj->Class == CLASS_RAGEFIGHTER) {
				if (percent > 0.0 && lpObj->CharacterData->Mastery.ML_RF_Regeneration > 0.0) percent += lpObj->CharacterData->Mastery.ML_RF_Regeneration;
			} else if (percent > 0.0 && lpObj->CharacterData->Mastery.ML_GENERAL_Regeneration > 0.0) percent += lpObj->CharacterData->Mastery.ML_GENERAL_Regeneration;

			percent += BUFF_GetTotalBuffEffectValue(lpObj, EFFECTTYPE_RECOVERY_HP);
			percent += lpObj->LifeRegeneration;
			tlife += (lpObj->MaxLife + lpObj->AddLife) * percent / 100.0f;
			tlife += lpObj->CharacterData->JewelOfHarmonyEffect.JoH_LifeRegeneration;

			if (lpObj->CharacterData->m_btRefillHPSocketOption > 0) tlife += lpObj->CharacterData->m_btRefillHPSocketOption;
			if ((lpObj->MaxLife + lpObj->AddLife) < tlife) tlife = lpObj->MaxLife + lpObj->AddLife;
		...
		percent = 3.0f;
		percent += BUFF_GetTotalBuffEffectValue(lpObj, EFFECTTYPE_RECOVERY_MANA);
		...
		if (lpObj->pInventory[10].IsItem() == TRUE) {
			if (lpObj->pInventory[10].m_Option3 != 0 && lpObj->pInventory[10].m_Durability != 0.0f && lpObj->pInventory[10].m_Type != ITEMGET(13, 24)) percent += lpObj->pInventory[10].m_Option3;
		}

		if (lpObj->pInventory[11].IsItem() == TRUE) {
			if (lpObj->pInventory[11].m_Option3 != 0 && lpObj->pInventory[11].m_Durability != 0.0f && lpObj->pInventory[11].m_Type != ITEMGET(13, 24)) percent += lpObj->pInventory[11].m_Option3;
		}

		if (lpObj->pInventory[9].IsItem() == TRUE) {
			if (lpObj->pInventory[9].m_Option3 != 0 && lpObj->pInventory[9].m_Durability != 0.0f && lpObj->pInventory[9].m_Type != ITEMGET(13, 28)) percent += lpObj->pInventory[9].m_Option3;
		}

		if (lpObj->pInventory[7].IsItem() == TRUE) {
			if (lpObj->pInventory[7].m_Option3 != 0 && lpObj->pInventory[7].m_Special[2] == 85 && lpObj->pInventory[7].m_Durability != 0.0f) percent += lpObj->pInventory[7].m_Option3;
		}

		if (!(BC_MAP_RANGE(lpObj->MapNumber) || CC_MAP_RANGE(lpObj->MapNumber) || KALIMA_MAP_RANGE(lpObj->MapNumber) || DS_MAP_RANGE(lpObj->MapNumber) || IT_MAP_RANGE(lpObj->MapNumber) || lpObj->MapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL) && !(lpObj->MapNumber == MAP_INDEX_CASTLESIEGE || lpObj->MapNumber == MAP_INDEX_CASTLEHUNTZONE)) {
			int iValue = BUFF_GetTotalBuffEffectValue(lpObj, EFFECTTYPE_RECOVERY_HP);

			if (iValue > 0) percent += iValue;
		}

		if (lpObj->Class == CLASS_RAGEFIGHTER) {
			if (lpObj->CharacterData->Mastery.ML_RF_Regeneration > 0.0) percent += lpObj->CharacterData->Mastery.ML_RF_Regeneration;
		} else if (lpObj->CharacterData->Mastery.ML_GENERAL_Regeneration > 0.0) percent += lpObj->CharacterData->Mastery.ML_GENERAL_Regeneration;
		// Elf Wings Regeneration
		if (lpObj->pInventory[7].IsItem() == TRUE && lpObj->pInventory[7].m_IsValidItem == true) {
			CItem* Wings = &lpObj->pInventory[7];

			if (Wings->m_Durability > 0.0f) {
				switch (Wings->m_Type) {
					case ITEMGET(12, 0):			percent += 3;				break;
					case ITEMGET(12, 3):			percent += 4;				break;
					case ITEMGET(12, 265):			percent += 5;				break;
					case ITEMGET(12, 38):			percent += 7;				break;
				}
			}
		}
	...
	if (lpObj->iFillShield > 0) {
		...
		if (lpObj->iFillShieldCount <= 0) lpObj->iFillShieldCount = 1;
		...
		if (lpObj->iFillShield < iFillSDPoint) {
			...
			if (iFillSDPoint < 0) iFillSDPoint = 0;
		}
	...
	if (lpObj->FillLife > 0) {
		...
		if (lpObj->FillLifeCount == 2) cur = lpObj->FillLifeMax * 54.0f / 100.0f;
		if (lpObj->FillLifeCount == 4) cur = lpObj->FillLifeMax * 25.0f / 100.0f;
		if (lpObj->FillLife < cur) {
			cur = lpObj->FillLife;
			lpObj->FillLife = 0;

			if (cur < 0) cur = 0;
		} else lpObj->FillLife -= cur;
...
void gObjViewportListProtocolCreate(LPOBJ lpObj) {
	...
	if (lpObj->Type == OBJ_MONSTER) MVL = MaxViewportMonster;
	if (lpObj->Type == OBJ_USER) {
		if (lpObj->m_Change >= 0 && (lpObj->CharacterData->m_EnableUseChangeSkin == true || IsTransformCharacterSkin(lpObj->m_Change) == false)) {	
			...
			if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_GM_MARK) == true) BUFF_AddBuffEffect(lpObj, BUFFTYPE_GM_MARK, 0, 0, 0, 0, -10);

			pViewportCreateChange.DirAndPkLevel  = lpObj->Dir << 4;

			if (lpObj->CharacterData->RegisterdLMS == 1 && lpObj->MapNumber == g_LastManStanding.m_Cfg.iPVPMap && g_LastManStanding.m_Rooms[lpObj->CharacterData->RegisteredLMSRoom].bState >= 2) pViewportCreateChange.DirAndPkLevel |= (6&0x0F);
			else pViewportCreateChange.DirAndPkLevel |= (lpObj->m_PK_Level&0x0F);
			...
			if (g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE) {
				memcpy(pViewportCreateChange.Id, lpObj->CharacterData->m_RealNameOfUBF, MAX_ACCOUNT_LEN);
				pViewportCreateChange.nServerCodeOfHomeWorld = lpObj->CharacterData->m_nServerCodeOfHomeWorld;
			} else {
				memcpy(pViewportCreateChange.Id, lpObj->Name, MAX_ACCOUNT_LEN);
				pViewportCreateChange.nServerCodeOfHomeWorld = 0;
			}

			if (strlen(pViewportCreateChange.Id) < 1) g_Log.Add("[UBF][gObjViewportListProtocolCreate][Name Setting Failed] (%d) ID : %s, Name: %s)", lpObj->m_Index, lpObj->AccountID, lpObj->Name);
			...
			if (lpObj->m_Change < 0) lpObj->CharacterData->m_EnableUseChangeSkin = false;
			else {
				if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_GM_MARK) == TRUE) BUFF_AddBuffEffect(lpObj, BUFFTYPE_GM_MARK, 0, 0, 0, 0, -10);
			}

			if (lpObj->m_State == 1) {
				if (!lpObj->Teleport) pViewportCreate.NumberH |= (0x01 << 7);
			}
			...
			if (lpObj->CharacterData->RegisterdLMS == 1 && lpObj->MapNumber == g_LastManStanding.m_Cfg.iPVPMap && g_LastManStanding.m_Rooms[lpObj->CharacterData->RegisteredLMSRoom].bState >= 2) pViewportCreate.DirAndPkLevel |= (6&0x0F);
			else pViewportCreate.DirAndPkLevel |= (lpObj->m_PK_Level&0x0F);

			memcpy(&pViewportCreate.CharSet, lpObj->CharSet, 18);

			if (g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE) {
				memcpy(pViewportCreate.Id, lpObj->CharacterData->m_RealNameOfUBF, MAX_ACCOUNT_LEN);
				pViewportCreate.nServerCodeOfHomeWorld = lpObj->CharacterData->m_nServerCodeOfHomeWorld;
			} else {
				memcpy(pViewportCreate.Id, lpObj->Name, MAX_ACCOUNT_LEN);
				pViewportCreate.nServerCodeOfHomeWorld = 0;
			}

			if (strlen(pViewportCreate.Id) < 1) g_Log.Add("[UBF][gObjViewportListProtocolCreate_2][Name Setting Failed] (%d) ID : %s, Name: %s)", lpObj->m_Index, lpObj->AccountID, lpObj->Name);
		...
		if (lpObj->CharacterData->GuildNumber != 0) {
			...
			pGuild.GuildNumber = lpObj->CharacterData->lpGuild->Number;
			pGuild.NumberH = SET_NUMBERH(lpObj->m_Index) & 0x7F;
			pGuild.NumberL = SET_NUMBERL(lpObj->m_Index);

			if (strcmp(lpObj->CharacterData->lpGuild->Names[0],lpObj->Name) == 0) pGuild.NumberH |= 0x80;

			pGuild.btGuildType = lpObj->CharacterData->lpGuild->btGuildType;
			pGuild.btGuildStatus = lpObj->CharacterData->GuildStatus;

			if (g_CastleSiegeSync.CheckCastleOwnerMember(lpObj->m_Index) == TRUE || g_CastleSiegeSync.CheckCastleOwnerUnionMember(lpObj->m_Index) == TRUE) pGuild.btOwnerStatus = 1;
			else pGuild.btOwnerStatus = 0;
			...
			if (lpObj->VpPlayer2[n].state != 0) {
				...
				if (tObjNum >= 0) {
					if (lpObj->VpPlayer2[n].type == OBJ_USER) {
						...
						if (GuildInfoCount != 0) IOCP.DataSend(tObjNum,GuildInfoBuf,GuildInfoOfs);
						if (GuildUserCount != 0) IOCP.DataSend(tObjNum,GuildUserBuf,GuildUserOfs);
					}
				}
			}
		}
		...
		if (lpObj->m_RecallMon >= 0 && (lpObj->Class < 678 || lpObj->Class > 680)) {
			...
			if (lpObj->m_State == 1) pCallMonsterViewportCreate.NumberH |= (0x01<<7);
			...
			if (lpObj->m_RecallMon >= 0 && lpObj->m_RecallMon < g_ConfigRead.server.GetObjectMax() - 1) memcpy(pCallMonsterViewportCreate.Id, gObj[lpObj->m_RecallMon].Name, MAX_ACCOUNT_LEN);
		...
		if (lpObj->Type == OBJ_USER)
		{
			for (int n = 0; n < MAX_VIEWPORT; n++)
			{
				if (lpObj->VpPlayer[n].state == 1)
				{
					...
					if (tObjNum >= 0)
					{
						if ((gObj[tObjNum].Authority & 0x20) == 0x20 && BUFF_TargetHasBuff(&gObj[tObjNum], BUFFTYPE_INVISABLE))
							continue;
						...
						case 1:
							...
							if (lpTargetObj->m_Change >= 0 && (lpTargetObj->CharacterData->m_EnableUseChangeSkin == true || IsTransformCharacterSkin(lpTargetObj->m_Change) == false))
							{
								...
								{
									BUFF_AddBuffEffect(lpTargetObj, BUFFTYPE_GM_MARK, 0, 0, 0, 0, -10);
								}
								...
								if (lpTargetObj->CharacterData->RegisterdLMS == 1 && lpObj->MapNumber == g_LastManStanding.m_Cfg.iPVPMap && g_LastManStanding.m_Rooms[lpObj->CharacterData->RegisteredLMSRoom].bState >= 2)
								{
								...
								if (g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE)
								{
									memcpy(pViewportCreateChange.Id, lpTargetObj->CharacterData->m_RealNameOfUBF, MAX_ACCOUNT_LEN);
									pViewportCreateChange.nServerCodeOfHomeWorld = lpTargetObj->CharacterData->m_nServerCodeOfHomeWorld;
								}
							...
							else
							{
								...
								{
									lpTargetObj->CharacterData->m_EnableUseChangeSkin = false;
								}

								else
								{
									if (BUFF_TargetHasBuff(lpTargetObj, BUFFTYPE_GM_MARK) == TRUE)
									{
										BUFF_AddBuffEffect(lpTargetObj, BUFFTYPE_GM_MARK, 0, 0, 0, 0, -10);
									}
								}
								...
								if (lpObj->CharacterData->RegisterdLMS == 1 && lpObj->MapNumber == g_LastManStanding.m_Cfg.iPVPMap && g_LastManStanding.m_Rooms[lpObj->CharacterData->RegisteredLMSRoom].bState >= 2)
								{
								...
								if (g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE)
								{
									memcpy(pViewportCreate.Id, lpTargetObj->CharacterData->m_RealNameOfUBF, MAX_ACCOUNT_LEN);
									pViewportCreate.nServerCodeOfHomeWorld = lpTargetObj->CharacterData->m_nServerCodeOfHomeWorld;
								}
							...
							if (lpObj->Type == OBJ_USER)
							{
								if (lpTargetObj->CharacterData->lpGuild != 0)
								{
									PMSG_SIMPLE_GUILDVIEWPORT pGuild;

									pGuild.GuildNumber = lpTargetObj->CharacterData->lpGuild->Number;
									pGuild.NumberH = SET_NUMBERH(lpTargetObj->m_Index) & 0x7F;
									pGuild.NumberL = SET_NUMBERL(lpTargetObj->m_Index);

									pGuild.btGuildStatus = lpTargetObj->CharacterData->GuildStatus;
									pGuild.btGuildType = lpTargetObj->CharacterData->lpGuild->btGuildType;
									...
									if (strcmp(lpTargetObj->CharacterData->lpGuild->Names[0], lpTargetObj->Name) == 0)
									{
										pGuild.NumberH |= 0x80;
									}
								...
								if (false)
								{
									...
									if (ViewGuildMng.Add(lpTargetObj->CharacterData->GuildNumber, lpTargetObj->m_Index) == 1 && lpTargetObj->CharacterData->lpGuild != 0)
									{
										PMSG_GUILDVIEWPORT pGuildViewport;

										pGuildViewport.NumberH = SET_NUMBERH(lpTargetObj->CharacterData->lpGuild->Number);
										pGuildViewport.NumberL = SET_NUMBERL(lpTargetObj->CharacterData->lpGuild->Number);

										strcpy(pGuildViewport.GuildName, lpTargetObj->CharacterData->lpGuild->Name);
										memcpy(pGuildViewport.Mark, lpTargetObj->CharacterData->lpGuild->Mark, sizeof(pGuildViewport.Mark));
									...
									if (lpTargetObj->CharacterData->lpGuild != 0 && strcmp(lpTargetObj->CharacterData->lpGuild->Names[0], lpTargetObj->Name) == 0)
									{
									...
									if (lpTargetObj->CharacterData->lpGuild != 0)
									{
										pGuildUserViewport.GNumberH = SET_NUMBERH(lpTargetObj->CharacterData->lpGuild->Number);
										pGuildUserViewport.GNumberL = SET_NUMBERL(lpTargetObj->CharacterData->lpGuild->Number);
								...
								if (g_GensSystem.GetGensInfluence(lpTargetObj))
								{
									...
									pGensMsg.iGensRanking = lpTargetObj->CharacterData->m_GensRanking;
									pGensMsg.iGensClass = lpTargetObj->CharacterData->m_GensClass;
									pGensMsg.iContributePoint = lpTargetObj->CharacterData->m_ContributePoint;
								...
								else
								{
									...
									if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE)
									{
										if (lpTargetObj->Class == 278)
										{
											...
											if (lpObj->CharacterData->lpGuild && lpTargetObj->CharacterData->lpGuild)
											{
												if (lpObj->CharacterData->lpGuild == lpTargetObj->CharacterData->lpGuild)
												{
													pMonsterViewportCreate.Type_HI |= 0x80;
									...
									if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE)
									{
										if (lpTargetObj->Class == 277)
										{
											...
											{
												BUFF_RemoveBuffEffect(lpTargetObj, BUFFTYPE_CASTLE_DOOR_STATE);
											}
											else
											{
												BUFF_AddBuffEffect(lpTargetObj, BUFFTYPE_CASTLE_DOOR_STATE);
											}
										}
										...
										if (lpTargetObj->Class == 216)
										{
											...
											{
												BUFF_RemoveBuffEffect(lpTargetObj, 20);
											}
											else
											{
												BUFF_AddBuffEffect(lpTargetObj, 20);
											}
										}
									}
...
void PLAYER_TeleportMagicUse(int aIndex, BYTE x, BYTE y) {
	LPOBJ lpObj = &gObj[aIndex];

	if (MAX_MAP_RANGE(lpObj->MapNumber) == FALSE) return;
...
BOOL gObjMoveGate(int aIndex, int gt) {
	...
	if (lpObj->Type == OBJ_USER && lpObj->CharacterData->ISBOT == false && lpObj->m_Index >= g_ConfigRead.server.GetObjectStartUserIndex())
	{
		gDarkSpirit[lpObj->m_Index- g_ConfigRead.server.GetObjectStartUserIndex()].SetMode(CDarkSpirit::ePetItem_Mode::PetItem_Mode_Normal,-1);
	}
	...
	if (lpObj->Type == OBJ_USER && lpObj->CharacterData->m_bIsMining)
	{
	...
	if (gObj[aIndex].RegenOk != 0)
	{
		...
		GSProtocol.PROTOCOL_TeleportSend(&gObj[aIndex],gt,mapNumber,gObj[aIndex].X,gObj[aIndex].Y,gObj[aIndex].Dir);
	...
	if (mapNumber == MAP_INDEX_ACHERON || mapNumber == MAP_INDEX_DEBENTER)
	{
		...
		{
			if (BUFF_TargetHasBuff(lpObj, 179) || BUFF_TargetHasBuff(lpObj, 180) || BUFF_TargetHasBuff(lpObj, 181) || BUFF_TargetHasBuff(lpObj, 182) || BUFF_TargetHasBuff(lpObj, 183))
			{
				BUFF_AddBuffEffect(lpObj, 187, 0, 0, 0, 0, -10);
			}

			else
			{
				if (BUFF_TargetHasBuff(lpObj, 187))
				{
					BUFF_RemoveBuffEffect(lpObj, 187);
				}
			}
		}
	...
	if (g_GensSystem.IsMapBattleZone(lpObj->MapNumber) == TRUE && lpObj->Type == OBJ_USER && GetTickCount() - lpObj->CharacterData->m_dwGensHitTeleportDelay < (g_GensSystem.GetGensAttackWarpDelay() * 1000))
	{
		x = lpObj->X;
		y = lpObj->Y;
		mapNumber = lpObj->MapNumber;
	...
	else
	{
		if (lpObj->Type == OBJ_USER)
		{
			lpObj->CharacterData->m_dwGensHitTeleportDelay = 0;
		}
	}
	...
	if (lpObj->m_bPShopOpen == 1) {
		...
		if (lpObj->MapNumber == MAP_INDEX_CASTLESIEGE && mapNumber == MAP_INDEX_RORENCIA) bMove = 0;
		if (lpObj->MapNumber == MAP_INDEX_RORENCIA && mapNumber == MAP_INDEX_CASTLESIEGE) bMove = 0;
		if (bMove == 0) {
			...
			if (lpObj->m_Change >= 0) gObjViewportListProtocolCreate(&gObj[aIndex]);
	...
	// Remove Buffs When Entering Event Maps
	/*if (BC_MAP_RANGE(mapNumber) || CC_MAP_RANGE(mapNumber) || DS_MAP_RANGE(mapNumber) || IT_MAP_RANGE(mapNumber) || DG_MAP_RANGE(mapNumber) || IMPERIAL_MAP_RANGE(mapNumber) || mapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL || mapNumber == MAP_INDEX_DEVILSQUARE_FINAL) {
		if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_ATTDEF_POWER_INC) != NULL) BUFF_RemoveBuffEffect(lpObj, BUFFTYPE_ATTDEF_POWER_INC);
		if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_EMPOWER) != NULL) BUFF_RemoveBuffEffect(lpObj, BUFFTYPE_EMPOWER);
		if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_ENHANCE) != NULL) BUFF_RemoveBuffEffect(lpObj, BUFFTYPE_ENHANCE);
		if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_CONCENTRATION) != NULL) BUFF_RemoveBuffEffect(lpObj, BUFFTYPE_CONCENTRATION);
		if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_HP_INC) != NULL) BUFF_RemoveBuffEffect(lpObj, BUFFTYPE_HP_INC);
		if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_MANA_SHIELD) != NULL) BUFF_RemoveBuffEffect(lpObj, BUFFTYPE_MANA_SHIELD);
		if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_HP_INC_STR) != NULL) BUFF_RemoveBuffEffect(lpObj, BUFFTYPE_HP_INC_STR);		
		if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_INNERVATION) != NULL) BUFF_RemoveBuffEffect(lpObj, BUFFTYPE_INNERVATION);
		if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_IMPROVED_MANA_SHIELD) != NULL) BUFF_RemoveBuffEffect(lpObj, BUFFTYPE_IMPROVED_MANA_SHIELD);
	}*/
	...
	if (lpObj->MapNumber == MAP_INDEX_DEVILSQUARE_FINAL && g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE && mapNumber != MAP_INDEX_DEVILSQUARE_FINAL) g_DevilSquareFinal.Leave_DSF(lpObj->m_Index);
			...
			if (gParty.GetCount(nPartyNumber) <= 1) {
				for (int i = 0; i < 5; i++) {
					...
					if (index >= 0) {
						...
						if (gObj[index].Connected == PLAYER_PLAYING) GSProtocol.GCPartyDelUserSend(index);
					}
				}
			} else GSProtocol.CGPartyListAll(nPartyNumber);
		}
	}
	...
	if (lpObj->Type == OBJ_USER && g_MapAttr.CheckVIP(mapNumber) > lpObj->CharacterData->VipType)
	{
	...
	if (lpObj->Type == OBJ_USER && lpObj->CharacterData->m_bUsePartyMatching == true) {
		if (mapNumber == MAP_INDEX_ARCA_WAR || mapNumber == MAP_INDEX_DEBENTER_ARCA_WAR) {
			x = lpObj->X;
			y = lpObj->Y;
			mapNumber = lpObj->MapNumber;
			dir = lpObj->Dir;
	...
	if (movefail == 0)
	{
		...
		if (lpObj->Type == OBJ_USER)
		{
			lpObj->CharacterData->MoveGate = gt;
		}
		...
		if (gObj[aIndex].CharacterData->m_bUsePartyMatching)
		{
			if (DG_MAP_RANGE(gObj[aIndex].MapNumber) || IMPERIAL_MAP_RANGE(gObj[aIndex].MapNumber))
			{
				if (gObj[aIndex].CharacterData->m_bPartyMatchingLeader)
				{
	...
	if (gObj[aIndex].Type == OBJ_USER)
	{
		...
		if (BUFF_TargetHasBuff(lpObj, BUFFTYPE_INVISABLE) != 0)
		{
			if (gObj[aIndex].MapNumber != MAP_INDEX_CASTLESIEGE)
			{
				gObjViewportListProtocolDestroy(&gObj[aIndex]);
...
void gObjTeleport(int aIndex, int map, int x, int y) {
	...
	if (lpObj->m_IfState.use == 1 && lpObj->m_IfState.type == 1) GSProtocol.CGTradeCancelButtonRecv(lpObj->m_Index);
	if (lpObj->m_IfState.use == 1 && lpObj->m_IfState.type == 6) GSProtocol.CGWarehouseUseEnd(lpObj->m_Index);
	...
	if (gObj[aIndex].Type == OBJ_USER && gObj[aIndex].CharacterData->m_bIsMining) g_MineSystem.CheckMoveMapWhileMining(aIndex);

	gObjViewportListProtocolDestroy(&gObj[aIndex]);
	GSProtocol.PROTOCOL_TeleportSend(&gObj[aIndex], 1, map, gObj[aIndex].X, gObj[aIndex].Y, gObj[aIndex].Dir);
	gObjViewportListProtocolCreate(&gObj[aIndex]);
	gObjClearViewport(&gObj[aIndex]);
...
void gObjMoveDataLoadingOK(int aIndex) {
	if (gObj[aIndex].RegenOk == 1) gObj[aIndex].RegenOk = 2;
}
...
CMagicInf* PLAYER_GetMagicSearch(LPOBJ lpObj, WORD skillnumber) {
	if (skillnumber == 58) skillnumber = 40;
	if (skillnumber == 395) skillnumber = 392;
	for(int n = 0; n < MAGIC_SIZE; n++) {
...
int	PLAYER_MagicManaUse(LPOBJ lpObj, CMagicInf* lpMagic) {
	...
	if (g_MasterLevelSkillTreeSystem.CheckMasterLevelSkill(lpMagic->m_Skill) == true) {
		DecreaseMana = MagicDamageC.MAGIC_GetManaUsage(lpMagic->m_Skill);
		DecreaseMana += DecreaseMana * g_MasterLevelSkillTreeSystem.GetMasterSkillManaIncRate(lpMagic->m_Level) / 100.0;
	} else DecreaseMana = MagicDamageC.MAGIC_GetManaUsage(lpMagic->m_Skill);

	if (lpObj->Class == CLASS_RAGEFIGHTER) DecreaseMana -= DecreaseMana * lpObj->CharacterData->Mastery.ML_RF_ArcaneEfficiency / 100.0;
	else DecreaseMana -= DecreaseMana * lpObj->CharacterData->Mastery.ML_GENERAL_ArcaneEfficiency / 100.0;
	if (DecreaseMana < 1.0) DecreaseMana = 1.0;

	// Pendant of Ability Cost Reduce (10%)
	CItem Amulet = lpObj->pInventory[9];

	if (Amulet.IsItem() == TRUE && Amulet.m_Type == ITEMGET(13, 28)) DecreaseMana -= DecreaseMana * 10 / 100;
...
int PLAYER_MagicStamUse(LPOBJ lpObj, CMagicInf* lpMagic) {
	int stam = lpObj->Stamina;

	if (lpObj->Type != OBJ_USER) return false;
	if (lpMagic == 0) return false;

	int useStam = MagicDamageC.MAGIC_GetStamUsage(lpMagic->m_Skill);

	if (lpObj->CharacterData->m_btAGReduceRate > 0) useStam -= useStam * lpObj->CharacterData->m_btAGReduceRate / 100;

	// Pendant of Ability Cost Reduce (10%)
	CItem Amulet = lpObj->pInventory[9];

	if (Amulet.IsItem() == TRUE && Amulet.m_Type == ITEMGET(13, 28)) useStam -= useStam * 10 / 100;
...
int MAGIC_MagicAdd(LPOBJ lpObj, WORD Type, WORD Index, BYTE Level, WORD &SkillNumber) {
	int skill = -1,n,reqeng,reqleadership;

	skill = MAGIC_GetNumberIndex(Type,Index,Level);
	...
	if (g_EnergyCheckOff == 1) {
		reqeng = MagicDamageC.MAGIC_GetRequireEnergy(skill);
	...
	reqleadership = MagicDamageC.MAGIC_GetRequireLeadership(skill);
	...
	if (MagicDamageC.MAGIC_GetRequireClass(lpObj->Class,lpObj->CharacterData->ChangeUP,skill) < 1) {
		g_Log.Add("error-L2: His class is not allowed to use the skill [ID: %d]", skill);
		return -1;
	}

	if (g_MasterLevelSkillTreeSystem.IsExistMLSBasedSkill(lpObj, skill) == TRUE) return -1;
	for (int n = 0; n < MAGIC_SIZE; n++) {
		if (lpObj->Magic[n].IsMagic() == 1) {
			skill = MAGIC_GetNumberIndex(Type,Index,Level);
...
int MAGIC_MagicDel(LPOBJ lpObj, WORD aSkill, BYTE Level) {
	if (g_MasterLevelSkillTreeSystem.MLS_WeaponSkillDel(lpObj, aSkill, Level) != FALSE) return -1;
	for (int n = 0; n < MAGIC_SIZE; n++) {
		if (lpObj->Magic[n].IsMagic() == 1) {
...
int MAGIC_MagicAdd(LPOBJ lpObj, WORD aSkill, BYTE Level) {
	int skill = -1,n,reqeng;

	skill = -1;

	if (g_EnergyCheckOff == 1 && lpObj->Type == OBJ_USER && lpObj->CharacterData->ISBOT == false) {
		reqeng = MagicDamageC.MAGIC_GetRequireEnergy(aSkill);
	...
	if (MagicDamageC.MAGIC_GetRequireClass(lpObj->Class,lpObj->CharacterData->ChangeUP,aSkill) < 1 && lpObj->Type == OBJ_USER && lpObj->CharacterData->ISBOT == false) {
		g_Log.Add("error-L2: His class is not allowed to use the skill [ID: %d]", aSkill);
		return -1;
	}

	if (g_MasterLevelSkillTreeSystem.IsExistMLSBasedSkill(lpObj, aSkill) == true) return -1;
	for (int n = 0; n < MAGIC_SIZE; n++) {
		if (lpObj->Magic[n].IsMagic() == 1) {
			if (lpObj->Magic[n].m_Skill == aSkill) {
				if (g_EnergyCheckOff == 0) return -1;
	...
	for (int n = 0; n < MAGIC_SIZE; n++) {
		if (lpObj->Magic[n].IsMagic() == 0) {
			...
			if (skill < 0) {
				...
				lpObj->MagicCount++;
				g_Log.Add("[PLAYER_MagicAdd] (%s) %d - Skill:%d, Level:%d", lpObj->Name, lpObj->MagicCount, aSkill, Level);
				return n;
			}
		}
	}
...
int MAGIC_WeaponMagicAdd(LPOBJ lpObj, WORD aSkill, BYTE Level) {
	int skill = -1,n;
	skill = -1;

	if (MagicDamageC.MAGIC_GetRequireClass(lpObj->Class,lpObj->CharacterData->ChangeUP,aSkill) < 1) return -1;
	if (g_MasterLevelSkillTreeSystem.MLS_WeaponSkillAdd(lpObj, aSkill, Level) == true) return -1;
	for(int n = 0; n < MAGIC_SIZE; n++) {
		if (lpObj->Magic[n].IsMagic() == 0) {
			skill = lpObj->Magic[n].Set(aSkill,Level);
			...
			if (skill < 0) {
				...
				lpObj->MagicCount++;
				g_Log.Add("[MAGIC_WeaponMagicAdd] (%s) %d - Skill:%d, Level:%d", lpObj->Name, lpObj->MagicCount, aSkill, Level);
				return n;
			}
		}
	}
...
BOOL ITEM_JewelOfBlessLevelUp(LPOBJ lpObj, int source, int target) {
	if (source < 0 || source > MAIN_INVENTORY_SIZE - 1) return false;
	if (target < 0 || target > MAIN_INVENTORY_SIZE - 1) return false;
	...
	if (bExceptionCheckResult) return bExceptionCheckResult;
	if (ITEM_InvalidJewelTarget(lpObj->pInventory[target].m_Type) == TRUE) return false;
	if (g_LuckyItemManager.IsLuckyItemEquipment(lpObj->pInventory[target].m_Type) && g_ConfigRead.data.common.LuckyItemAllowUseJewels == false) return false;
	if (lpObj->pInventory[target].m_Level >= 6) return false;
	if (lpObj->pInventory[target].m_Type >= ITEMGET(12, 130) && lpObj->pInventory[target].m_Type <= ITEMGET(12, 135)) return false;
	...
	lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type, lpObj->pInventory[target].m_Option1, lpObj->pInventory[target].m_Option2, lpObj->pInventory[target].m_Option3, lpObj->pInventory[target].m_NewOption, lpObj->pInventory[target].m_SetOption, lpObj->pInventory[target].m_ItemOptionEx, lpObj->pInventory[target].m_SocketOption, lpObj->pInventory[target].m_BonusSocketOption, 0, CURRENT_DB_VERSION);
	g_Log.Add("[%s][%s]After using Jewel of Bless(%d) : (%s) serial:(%I64d)(%I64d)",lpObj->AccountID,lpObj->Name,lpObj->pInventory[source].m_Number, lpObj->pInventory[target].GetName(),lpObj->pInventory[target].m_Number,lpObj->pInventory[target].m_Level);
	return true;
}

BOOL ITEM_JewelOfSoulLevelUp(LPOBJ lpObj, int source, int target) {
	if (source < 0 || source > MAIN_INVENTORY_SIZE - 1) return false;
	if (target < 0 || target > MAIN_INVENTORY_SIZE - 1) return false;
	if (lpObj->pInventory[source].IsItem() == FALSE) return false;
	if (lpObj->pInventory[target].IsItem() == FALSE) return false;
	if (ITEM_InvalidJewelTarget(lpObj->pInventory[target].m_Type) == TRUE) return false;
	if (g_LuckyItemManager.IsLuckyItemEquipment(lpObj->pInventory[target].m_Type) && g_ConfigRead.data.common.LuckyItemAllowUseJewels == false) return false;
	if (lpObj->pInventory[target].m_Level >= 9) return false;
	if (lpObj->pInventory[target].m_Type >= ITEMGET(12, 130) && lpObj->pInventory[target].m_Type <= ITEMGET(12, 135)) return false;

	g_Log.Add("[%s][%s]Before using Jewel of Soul : (%s) serial:(%I64d)(%I64d)", lpObj->AccountID,lpObj->Name,lpObj->pInventory[target].GetName(), lpObj->pInventory[target].m_Number,lpObj->pInventory[target].m_Level);

	int _r = rand() % 100;

	if (lpObj->pInventory[target].m_Option2) {
		if (_r < g_ConfigRead.data.common.SoulLuckRate) lpObj->pInventory[target].m_Level++;
		else {
			if (lpObj->pInventory[target].m_Level >= 7) lpObj->pInventory[target].m_Level = 0;			
			else {
				lpObj->pInventory[target].m_Level--;

				if (lpObj->pInventory[target].m_Level < 0) lpObj->pInventory[target].m_Level = 0;
			}
		}
	} else {
	...
	if (lpObj->pInventory[target].m_Level > 9) lpObj->pInventory[target].m_Level = 9;
	...
	lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type, lpObj->pInventory[target].m_Option1, lpObj->pInventory[target].m_Option2, lpObj->pInventory[target].m_Option3, lpObj->pInventory[target].m_NewOption, lpObj->pInventory[target].m_SetOption, lpObj->pInventory[target].m_ItemOptionEx, lpObj->pInventory[target].m_SocketOption, lpObj->pInventory[target].m_BonusSocketOption, 0, CURRENT_DB_VERSION);
	g_Log.Add("[%s][%s]After using Jewel of Soul (%d) : (%s) serial:(%I64d)(%I64d)",lpObj->AccountID,lpObj->Name,lpObj->pInventory[source].m_Number, lpObj->pInventory[target].GetName(),lpObj->pInventory[target].m_Number,lpObj->pInventory[target].m_Level);

	if (g_kJewelOfHarmonySystem.IsStrengthenByJewelOfHarmony(&lpObj->pInventory[target]) == TRUE) {
		if (g_kJewelOfHarmonySystem.IsActive(&lpObj->pInventory[target]) == FALSE) GSProtocol.GCServerMsgStringSend(Lang.GetText(0,271), lpObj->m_Index, 1);
	}

	return true;
}

BOOL ITEM_JewelOfLifeLevelUp(LPOBJ lpObj, int source, int target) {
	if (source < 0 || source > MAIN_INVENTORY_SIZE -1) return false;
	if (target < 0 || target > MAIN_INVENTORY_SIZE -1) return false;
	if (lpObj->pInventory[source].IsItem() == 0) return false;
	if (lpObj->pInventory[target].IsItem() == 0) return false;
	if (lpObj->pInventory[target].m_Type >= ITEMGET(12,200) && lpObj->pInventory[target].m_Type <= ITEMGET(12,220)) return false;
	if (ITEM_InvalidJewelTarget(lpObj->pInventory[target].m_Type) == TRUE) return false;

	g_Log.Add("[%s][%s]Before using Jewel of Bless : (%s) serial:(%I64d)(%I64d)", lpObj->AccountID,lpObj->Name, lpObj->pInventory[target].GetName(), lpObj->pInventory[target].m_Number, lpObj->pInventory[target].m_Option3);

	int _r = rand() % 100;
	char maxopt = g_ConfigRead.data.common.Is28Opt == true ? 7 : 4;

	if (lpObj->pInventory[target].m_Option3 == 0) {
		if ((lpObj->pInventory[target].m_Type >= ITEMGET(12,3) && lpObj->pInventory[target].m_Type <= ITEMGET(12,6)) || lpObj->pInventory[target].m_Type == ITEMGET(12,49) || lpObj->pInventory[target].m_Type == ITEMGET(12,42)) {
			lpObj->pInventory[target].m_NewOption &= 0xDF;

			if (rand() % 2 == 1) lpObj->pInventory[target].m_NewOption |= 0x20;
		}
	}

	if (lpObj->pInventory[target].m_Option3 >= maxopt) return false;
	if (_r < g_ConfigRead.data.common.LifeRate) {
		if (lpObj->pInventory[target].m_Option3 == 0) {
			if ((lpObj->pInventory[target].m_Type >= ITEMGET(12,36) && lpObj->pInventory[target].m_Type <= ITEMGET(12,40)) || lpObj->pInventory[target].m_Type == ITEMGET(12,43) || lpObj->pInventory[target].m_Type == ITEMGET(12,50)) {
				lpObj->pInventory[target].m_NewOption &= 0xEF;
				lpObj->pInventory[target].m_NewOption &= 0xDF;
				
				if (rand() % 2 == 0) {
					if (rand() % 1000 < 400) lpObj->pInventory[target].m_NewOption |= 0x10;
				} else {
					if (rand() % 1000 < 300) lpObj->pInventory[target].m_NewOption |= 0x20;
				}
			} else if (lpObj->pInventory[target].m_Type >= ITEMGET(12,262) && lpObj->pInventory[target].m_Type <= ITEMGET(12,265)) {
				lpObj->pInventory[target].m_NewOption &= 0xEF;

				if (rand() % 2 == 0) lpObj->pInventory[target].m_NewOption |= 0x10;
			}
		}

		lpObj->pInventory[target].m_Option3++;
	} else lpObj->pInventory[target].m_Option3 = 0;
	...
	lpObj->pInventory[target].m_Durability = levelitemdur * lpObj->pInventory[target].m_Durability / lpObj->pInventory[target].m_BaseDurability;
	lpObj->pInventory[target].Convert(lpObj->pInventory[target].m_Type, lpObj->pInventory[target].m_Option1, lpObj->pInventory[target].m_Option2, lpObj->pInventory[target].m_Option3, lpObj->pInventory[target].m_NewOption, lpObj->pInventory[target].m_SetOption, lpObj->pInventory[target].m_ItemOptionEx, lpObj->pInventory[target].m_SocketOption, lpObj->pInventory[target].m_BonusSocketOption, 0, CURRENT_DB_VERSION);
	g_Log.Add("[%s][%s]After using Jewel of Bless(%d) : (%s) serial:(%I64d)(%I64d)",lpObj->AccountID,lpObj->Name,lpObj->pInventory[source].m_Number, lpObj->pInventory[target].GetName(),lpObj->pInventory[target].m_Number,lpObj->pInventory[target].m_Option3);

	unsigned char NewOption[8];

	ItemIsBufExOption(NewOption, &lpObj->pInventory[target]);
	g_Log.Add("[%s][%s] JewelofLife Result [%d,%s,%d,%d,%d,%d]serial:[%I64d] dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d]", lpObj->AccountID,lpObj->Name,target,lpObj->pInventory[target].GetName(), lpObj->pInventory[target].m_Level,lpObj->pInventory[target].m_Option1,lpObj->pInventory[target].m_Option2, lpObj->pInventory[target].m_Option3,lpObj->pInventory[target].m_Number, BYTE(lpObj->pInventory[target].m_Durability),NewOption[0],NewOption[1],NewOption[2],NewOption[3], NewOption[4],NewOption[5],NewOption[6]);
	return true;
}

void gObjAbilityReSet(LPOBJ lpObj) {
	lpObj->CharacterData->LevelUpPoint = (lpObj->Level - 1) * 5;
	lpObj->CharacterData->Strength = DCInfo.DefClass[lpObj->Class].Strength;
	lpObj->CharacterData->Dexterity = DCInfo.DefClass[lpObj->Class].Dexterity;
	lpObj->CharacterData->Vitality = DCInfo.DefClass[lpObj->Class].Vitality;
	lpObj->CharacterData->Energy = DCInfo.DefClass[lpObj->Class].Energy;
...
BOOL gObjTargetGuildWarCheck(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if (lpObj->CharacterData->GuildNumber < 1 || lpTargetObj->CharacterData->GuildNumber < 1)
	{
		return false;
	}
	...
	if (strcmp(lpObj->CharacterData->lpGuild->TargetGuildName,lpTargetObj->CharacterData->lpGuild->Name))
	{
		return false;
	}
	return true;
...
void gObjGuildWarEndSend(LPOBJ lpObj, BYTE Result1, BYTE Result2)
{
	...
	if (lpObj->CharacterData->lpGuild->WarState != 1)
	{
		g_Log.Add("error-L3 : %s %d",__FILE__,__LINE__);
		return;
	}

	if (lpObj->CharacterData->lpGuild->lpTargetGuildNode == 0)
	{
		g_Log.Add("error-L3 : %s %d",__FILE__,__LINE__);
		return;
	}
	...
	if (Result1 == 1)
	{
		gObjGuildWarItemGive(lpObj->CharacterData->lpGuild,lpObj->CharacterData->lpGuild->lpTargetGuildNode);
	}
	else if (Result1 == 3)
	{
		gObjGuildWarItemGive(lpObj->CharacterData->lpGuild->lpTargetGuildNode,lpObj->CharacterData->lpGuild);
	}
	else if (Result1 == 1)
	{
		gObjGuildWarItemGive(lpObj->CharacterData->lpGuild->lpTargetGuildNode,lpObj->CharacterData->lpGuild);
	...
	{
		if (lpObj->CharacterData->lpGuild->Use[n] && lpObj->CharacterData->lpGuild->Index[n] >= 0)
		{
			gObj[lpObj->CharacterData->lpGuild->Index[n]].IsInBattleGround = 0;
			GSProtocol.GCGuildWarEnd(lpObj->CharacterData->lpGuild->Index[n],Result1,lpObj->CharacterData->lpGuild->lpTargetGuildNode->Name);
		}
	}

	for(int n = 0; n < MAX_USER_GUILD;n++)
	{
		if (lpObj->CharacterData->lpGuild->lpTargetGuildNode->Use[n] && lpObj->CharacterData->lpGuild->lpTargetGuildNode->Index[n] >= 0)
		{
			gObj[lpObj->CharacterData->lpGuild->lpTargetGuildNode->Index[n]].IsInBattleGround = 0;
			GSProtocol.GCGuildWarEnd(lpObj->CharacterData->lpGuild->lpTargetGuildNode->Index[n],Result2,lpObj->CharacterData->lpGuild->Name);
		}
	}

	PMSG_NOTICE pNotice;
	TNotice::MakeNoticeMsgEx(&pNotice, 0, Lang.GetText(0,416), lpObj->CharacterData->lpGuild->Name, lpObj->CharacterData->lpGuild->lpTargetGuildNode->Name, Result1, Result2);
	TNotice::SendNoticeToAllUser(&pNotice);
}
...
BOOL gObjGuildWarCheck(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if (strcmp(lpTargetObj->Name,lpTargetObj->CharacterData->lpGuild->Names[0])==0)
	{
		score = 2;
	}

	_GUILD_INFO_STRUCT * lpGuild = lpObj->CharacterData->lpGuild;
	_GUILD_INFO_STRUCT * lpTargetGuild = lpTargetObj->CharacterData->lpGuild;
...
{
	if (lpObj->CharacterData->GuildNumber < 1)
	{
		return false;
	}

	if (strcmp(lpObj->CharacterData->lpGuild->Names[0],lpObj->Name))
	{
		return false;
	}
	...
	if (lpObj->CharacterData->lpGuild->WarType == 1 && lpObj->CharacterData->lpGuild->BattleGroundIndex < 1)
	{
		gBattleGroundEnable(lpObj->CharacterData->lpGuild->BattleGroundIndex,0);
		cManager.BattleInfoSend(GetBattleTeamName(0,0),255,GetBattleTeamName(0,1),255);
	}
	gObjGuildWarEnd(lpObj->CharacterData->lpGuild,lpObj->CharacterData->lpGuild->lpTargetGuildNode);
	return true;
}
...
void gObjMonsterCallKill(int aIndex) {
	...
	if (callmon < 0 || callmon > g_ConfigRead.server.GetObjectMax() -1) return;
	if (gObj[callmon].Type != OBJ_MONSTER) return;
...
void gObjSendUserStatistic(int aIndex, int startLevel, int endLevel)
{
	...
	MsgOutput(gObj[aIndex].m_Index,Lang.GetText(0,578),startLevel,endLevel,sClassCount[7],
		...
		Lang.GetText(0,205),sClassCount[CLASS_MAGICGLADIATOR],
	...
	for(int n = 0; n < MAIN_INVENTORY_SIZE;n++)
	{
		if (lpObj->pInventory[n].IsItem() == 1)
		{
			if (lpObj->pInventory[n].m_Type == ITEMGET(14,21))
			{
				...
				GSProtocol.PROTOCOL_InventoryItemDeleteSend(aIndex,n,1);
...
void CHARACTER_Immobilize(int aIndex, int x, int y) {
	if (MAX_MAP_RANGE(gObj[aIndex].MapNumber) == FALSE) return;
...
void gObjGetStatPointState(int aIndex, short& AddPoint, short& MaxAddPoint, short& MinusPoint, short& MaxMinusPoint) {
	...
	LPOBJ lpObj = &gObj[aIndex];
	int total_point = lpObj->CharacterData->Strength + lpObj->CharacterData->Dexterity + lpObj ->CharacterData->Vitality + lpObj->CharacterData->Energy + lpObj->Leadership + lpObj->CharacterData->LevelUpPoint;
	int ori_point;

	if (lpObj->Class == CLASS_DARKLORD || lpObj->Class == CLASS_MAGICGLADIATOR || lpObj->Class == CLASS_RAGEFIGHTER) ori_point = DCInfo.GetDefPoint(lpObj->Class) + (lpObj->Level) * 7;
	else ori_point = DCInfo.GetDefPoint(lpObj->Class) + (lpObj->Level) * 5;
	if (g_QuestInfo.GetQuestState(lpObj, 0) == 2) ori_point += 60;
	if (g_QuestInfo.GetQuestState(lpObj, 1) == 2) ori_point += 60;
	if (g_QuestInfo.GetQuestState(lpObj, 2) == 2) ori_point += lpObj->Level - 220;
	if (g_QuestInfo.GetQuestState(lpObj, 4) == 2) ori_point += 60;
	if (g_QuestInfo.GetQuestState(lpObj, 5) == 2) ori_point += 60;
	if (g_QuestInfo.GetQuestState(lpObj, 6) == 2) ori_point += 60;
	...
	MinusPoint = lpObj->CharacterData->iFruitPoint;
...
BOOL gObjCheckStatPointUp(int aIndex) {
	...
	if (total_point < ori_point) return true;
	return false;
}
...
BOOL gObjCheckStatPointDown(int aIndex) {
	...
	int total_point = lpObj->CharacterData->Strength + lpObj->CharacterData->Dexterity + lpObj->CharacterData->Vitality + lpObj->CharacterData->Energy + lpObj->CharacterData->LevelUpPoint;
	int ori_point;
...
void gObjUseCircle(int aIndex, int pos) {
	...
	GSProtocol.PROTOCOL_InventoryItemDeleteSend(aIndex,pos,1);
	...
	if (AddPoint <= 10) iSuccessRate = 100;
	else if ((AddPoint - 10) < MaxAddPoint * (double)0.2) iSuccessRate = 90;
	else if ((AddPoint - 10) < MaxAddPoint * (double)0.4) iSuccessRate = 80;
	else if ((AddPoint - 10) < MaxAddPoint * (double)0.6) iSuccessRate = 70;
	else if ((AddPoint - 10) < MaxAddPoint * (double)0.8) iSuccessRate = 60;
	else iSuccessRate = 50;
	...
	if (rand() % 100 < iSuccessRate) {
		...
		if (iSuccessRate < 50) AddStat = 1;
		else if (iSuccessRate < 75) AddStat = 2;
		else AddStat = 3;
		if ((AddStat + AddPoint) > MaxAddPoint) AddStat = 1;
	...
	switch(level) {
		case 3:					gObj[aIndex].CharacterData->Strength += incvalue;				break;
		case 2:					gObj[aIndex].CharacterData->Dexterity += incvalue;				break;
		case 1:
			gObj[aIndex].CharacterData->Vitality += incvalue;
			gObj[aIndex].MaxLife += gObj[aIndex].VitalityToLife * incvalue;
			GSProtocol.PROTOCOL_ReFillSend(aIndex,gObj[aIndex].MaxLife+gObj[aIndex].AddLife,0xFE,0,gObj[aIndex].iAddShield+gObj[aIndex].iMaxShield);
		break;

		case 0:
			gObj[aIndex].CharacterData->Energy += incvalue;
			gObj[aIndex].MaxMana += gObj[aIndex].EnergyToMana * incvalue;
			PLAYER_CalcStamina(aIndex);
			GSProtocol.PROTOCOL_ManaSend(gObj[aIndex].m_Index,gObj[aIndex].MaxMana + gObj[aIndex].AddMana,0xFE,0,gObj[aIndex].MaxStamina);
		break;
...
void gObjUsePlusStatFruit(int aIndex,int pos) {
	if (!ObjectMaxRange(aIndex)) return;
	if (gObj[aIndex].Class > 6 || gObj[aIndex].Class < 0) return;
	...
	g_Log.Add("[StatUp] [%s][%s] Use StatUpItem Level:%d Pos:%d serial:%I64d", gObj[aIndex].AccountID,gObj[aIndex].Name,iItemLevel,pos,iItemSerial);
	gObjInventoryItemSet(aIndex,pos,0xFF);
	gObj[aIndex].pInventory[pos].Clear();
	GSProtocol.PROTOCOL_InventoryItemDeleteSend(aIndex,pos,1);
	...
	if (rand() % 100 < iSuccessRate) bSuccess = 1;
	if (bSuccess != 0) {
		int AddStat = 0;

		iSuccessRate = rand() % 100;

		if (iSuccessRate < 10) AddStat = 1;
		else if (iSuccessRate < 20) AddStat = 2;
		else if (iSuccessRate < 30) AddStat = 3;
		else if (iSuccessRate < 40) AddStat = 4;
		else if (iSuccessRate < 50) AddStat = 5;
		else if (iSuccessRate < 60) AddStat = 6;
		else if (iSuccessRate < 70) AddStat = 7;
		else if (iSuccessRate < 80) AddStat = 8;
		else if (iSuccessRate < 90) AddStat = 9;
		else AddStat = 10;
		if ((AddStat + AddPoint) > MaxAddPoint) AddStat = MaxAddPoint - AddPoint;
		...
		IOCP.DataSend(aIndex,(unsigned char *)&pResult,pResult.h.size);
		iIncStat = AddStat;
		g_Log.Add("[StatUp] [%s][%s] Success [%d][%d] %d", gObj[aIndex].AccountID,gObj[aIndex].Name,iItemLevel,iIncStat,iItemSerial);
	} else {
		pResult.result = 1;
		pResult.btStatValue = 0;
		pResult.btFruitType = iItemLevel;
		IOCP.DataSend(aIndex,(unsigned char *)&pResult,pResult.h.size);
		g_Log.Add("[StatUp] [%s][%s] Fail [%d] %d",	gObj[aIndex].AccountID,gObj[aIndex].Name,iItemLevel,iItemSerial);
		return;
	}

	switch(iItemLevel) {
		case 4:				gObj[aIndex].Leadership += iIncStat;							break;
		case 3:				gObj[aIndex].CharacterData->Strength += iIncStat;				break;
		case 2:				gObj[aIndex].CharacterData->Dexterity += iIncStat;				break;
		case 1:
			gObj[aIndex].CharacterData->Vitality += iIncStat;
			gObj[aIndex].MaxLife += gObj[aIndex].VitalityToLife * iIncStat;
		break;

		case 0:
			gObj[aIndex].CharacterData->Energy += iIncStat;
	...
	GSProtocol.PROTOCOL_ReFillSend(aIndex,gObj[aIndex].MaxLife+gObj[aIndex].AddLife,0xFE,0,gObj[aIndex].iAddShield+gObj[aIndex].iMaxShield);
	PLAYER_CalcStamina(aIndex);
	GSProtocol.PROTOCOL_ManaSend(gObj[aIndex].m_Index,gObj[aIndex].MaxMana + gObj[aIndex].AddMana,0xFE,0,gObj[aIndex].MaxStamina + gObj[aIndex].AddStamina);
}
...
void gObjUseMinusStatFruit(int aIndex, int pos) {
	...
	if (iItemLevel == 0){
		...
		iPresentStat = gObj[aIndex].CharacterData->Energy;

		if (iPresentStat <= iDefaultStat) bEnableUseFruit = 0;
	} else if (iItemLevel == 1) {
		iDefaultStat = DCInfo.DefClass[iCharacterClass].Vitality;
		iPresentStat = gObj[aIndex].CharacterData->Vitality;

		if (iPresentStat <= iDefaultStat) bEnableUseFruit = 0;
	} else if (iItemLevel == 2) {
		iDefaultStat = DCInfo.DefClass[iCharacterClass].Dexterity;
		iPresentStat = gObj[aIndex].CharacterData->Dexterity;

		if (iPresentStat <= iDefaultStat) bEnableUseFruit = 0;
	} else if (iItemLevel == 3) {
		iDefaultStat = DCInfo.DefClass[iCharacterClass].Strength;
		iPresentStat = gObj[aIndex].CharacterData->Strength;
	...
	g_Log.Add("[StatDown] [%s][%s] Use StatDownItem Level:%d Pos:%d serial:%I64d", gObj[aIndex].AccountID,gObj[aIndex].Name,iItemLevel,pos,iItemSerial);
	gObjInventoryItemSet(aIndex,pos,0xFF);
	gObj[aIndex].pInventory[pos].Clear();
	GSProtocol.PROTOCOL_InventoryItemDeleteSend(aIndex,pos,1);
	...
	if (rand() % 100 < iSuccessRate) bSuccess = 1;
	if (MinusPoint <= 10) bSuccess = 1;
	if (bSuccess != 0) {
		iSuccessRate = rand() % 100;

		if (iSuccessRate < 12) iDecStat = 1;
		else if (iSuccessRate < 18) iDecStat = 2;
		else if (iSuccessRate < 24) iDecStat = 3;
		else if (iSuccessRate < 30) iDecStat = 4;
		else if (iSuccessRate < 36) iDecStat = 5;
		else if (iSuccessRate < 42) iDecStat = 6;
		else if (iSuccessRate < 48) iDecStat = 7;
		else if (iSuccessRate < 54) iDecStat = 8;
		else if (iSuccessRate < 60) iDecStat = 9;
		else if (iSuccessRate < 66) iDecStat = 10;
		else if (iSuccessRate < 72) iDecStat = 11;
		else if (iSuccessRate < 78) iDecStat = 12;
		else if (iSuccessRate < 84) iDecStat = 13;
		else if (iSuccessRate < 90) iDecStat = 14;
		else iDecStat = 15;
		if ((MinusPoint + iDecStat) >= MaxMinusPoint) iDecStat = MaxMinusPoint - MinusPoint;
		if ((iPresentStat - iDecStat) < iDefaultStat) iDecStat = iPresentStat - iDefaultStat;

		pResult.result = 3;
		pResult.btFruitType = iItemLevel;
		pResult.btStatValue = iDecStat;
		IOCP.DataSend(aIndex,(unsigned char *)&pResult,pResult.h.size);
		g_Log.Add("[StatDown] [%s][%s] Success [%d][%d] %d", gObj[aIndex].AccountID,gObj[aIndex].Name,iItemLevel,iDecStat,iItemSerial);
	} else {
		pResult.result = 4;
		pResult.btFruitType = iItemLevel;
		pResult.btStatValue = 0;
		IOCP.DataSend(aIndex,(unsigned char *)&pResult,pResult.h.size);
		g_Log.Add("[StatDown] [%s][%s] Fail [%d] %d", gObj[aIndex].AccountID,gObj[aIndex].Name,iItemLevel,iItemSerial);
		return;
	}

	switch(iItemLevel) {
		case 4:				gObj[aIndex].Leadership -= iDecStat;						break;
		case 3:				gObj[aIndex].CharacterData->Strength -= iDecStat;			break;
		case 2:				gObj[aIndex].CharacterData->Dexterity -= iDecStat;			break;
		case 1:
			gObj[aIndex].CharacterData->Vitality -= iDecStat;
			gObj[aIndex].MaxLife -= gObj[aIndex].VitalityToLife *iDecStat;
		break;

		case 0:
			gObj[aIndex].CharacterData->Energy -= iDecStat;
	...
	int iOldLevelUpPoint = gObj[aIndex].CharacterData->LevelUpPoint;
	int iOldFruitPoint = gObj[aIndex].CharacterData->iFruitPoint;

	gObj[aIndex].CharacterData->LevelUpPoint += iDecStat;
	gObj[aIndex].CharacterData->iFruitPoint += iDecStat;
	g_Log.Add("[StatDown] [%s][%s] MinusStat[%d] -> LevelUpPoint Old(%d)/New(%d)  FruitPoint Old(%d)/New(%d)", gObj[aIndex].AccountID,gObj[aIndex].Name,iDecStat,iOldLevelUpPoint,gObj[aIndex].CharacterData->LevelUpPoint,iOldFruitPoint,gObj[aIndex].CharacterData->iFruitPoint);
	gObjCalCharacter.CHARACTER_Calc(aIndex);
	GSProtocol.PROTOCOL_ReFillSend(aIndex,gObj[aIndex].MaxLife+gObj[aIndex].AddLife,0xFE,0,gObj[aIndex].iAddShield+gObj[aIndex].iMaxShield);
	PLAYER_CalcStamina(aIndex);
	GSProtocol.PROTOCOL_ManaSend(gObj[aIndex].m_Index,gObj[aIndex].MaxMana + gObj[aIndex].AddMana,0xFE,0,gObj[aIndex].MaxStamina);
...
void CashShopExMinusStatFruit(int aIndex, int pos) {
	...
	switch(gObj[aIndex].pInventory[pos].m_Type)	{
		case 6710:			iItemLevel = 3;				break;
		case 6711:			iItemLevel = 2;				break;
		case 6712:			iItemLevel = 1;				break;
		case 6713:			iItemLevel = 0;				break;
		case 6714:			iItemLevel = 4;				break;
	}
	...
	switch (pResult.btFruitType) {
		case 3:				if (gObj[aIndex].CharacterData->Strength == DCInfo.DefClass[iCharacterClass].Strength) bEnableFruit = false;				break;
		case 2:				if (gObj[aIndex].CharacterData->Dexterity == DCInfo.DefClass[iCharacterClass].Dexterity) bEnableFruit = false;			break;
		case 1:				if (gObj[aIndex].CharacterData->Vitality == DCInfo.DefClass[iCharacterClass].Vitality) bEnableFruit = false;				break;
		case 0:				if (gObj[aIndex].CharacterData->Energy == DCInfo.DefClass[iCharacterClass].Energy) bEnableFruit = false;					break;
		case 5:				if (gObj[aIndex].Leadership = DCInfo.DefClass[iCharacterClass].Leadership) bEnableFruit = false;						break;
	}
	...
	switch (pResult.btFruitType) {
		case 3:
			...
			if (iPresentStat - iStatPoint < iDefaultStat) iStatPoint = iPresentStat - iDefaultStat;
		break;
	...
	if (bEnableFruit) {
		g_Log.Add("[InGameShop][ExMinusStatPoint] [%s][%s] Use StatDownItem Level:%d,Pos:%d,Dur:%d,Serial:%I64d", gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].pInventory[pos].m_Level, pos, gObj[aIndex].pInventory[pos].m_Durability, iItemSerial);
		gObjInventoryItemSet(aIndex,pos,0xFF);
		gObj[aIndex].pInventory[pos].Clear();
		GSProtocol.PROTOCOL_InventoryItemDeleteSend(aIndex,pos,1);		
		pResult.result = 6;
		...
		switch(iItemLevel) {
			...
			case 3:				gObj[aIndex].CharacterData->Strength -= iStatPoint;				break;
			case 2:				gObj[aIndex].CharacterData->Dexterity -= iStatPoint;				break;
			case 1:
				gObj[aIndex].CharacterData->Vitality -= iStatPoint;
				gObj[aIndex].MaxLife -= gObj[aIndex].VitalityToLife *iStatPoint;
			break;

			case 0:
				gObj[aIndex].CharacterData->Energy -= iStatPoint;
				gObj[aIndex].MaxMana -= gObj[aIndex].EnergyToMana * iStatPoint;
				PLAYER_CalcStamina(aIndex);
			break;
		}

		gObj[aIndex].CharacterData->LevelUpPoint += iStatPoint;
		gObjCalCharacter.CHARACTER_Calc(aIndex);
	} else {
		pResult.result = 38;
		IOCP.DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
		g_Log.Add("[InGameShop][ExMinusStatPoint] [%s][%s] MinusStat[%d]", gObj[aIndex].AccountID,gObj[aIndex].Name,iStatPoint);
		gObjCalCharacter.CHARACTER_Calc(aIndex);
		GSProtocol.PROTOCOL_ReFillSend(aIndex,gObj[aIndex].MaxLife+gObj[aIndex].AddLife,0xFE,0,gObj[aIndex].iAddShield+gObj[aIndex].iMaxShield);
		PLAYER_CalcStamina(aIndex);
		GSProtocol.PROTOCOL_ManaSend(gObj[aIndex].m_Index,gObj[aIndex].MaxMana + gObj[aIndex].AddMana,0xFE,0,gObj[aIndex].MaxStamina);
...
void gObjCalcMaxLifePower(int aIndex) {
	...
	switch(lpObj->Class) {
		case CLASS_WIZARD:				lpObj->m_MaxLifePower = lpObj->Level + lpObj->CharacterData->Vitality + 110;					break;
		case CLASS_KNIGHT:				lpObj->m_MaxLifePower = lpObj->Level * 2 + lpObj->CharacterData->Vitality + 60;				break;
		case CLASS_ELF:					lpObj->m_MaxLifePower = lpObj->Level + lpObj->CharacterData->Vitality + 80;					break;
		case CLASS_MAGICGLADIATOR:
		case CLASS_DARKLORD:			lpObj->m_MaxLifePower = lpObj->Level + lpObj->CharacterData->Vitality + 110;					break;			
		default :						lpObj->m_MaxLifePower = 0;																	break;
	}
}
...
void gObjDelayLifeCheck(int aIndex) {
	...
	if (lpObj->m_CheckLifeTime > 0) {
		...
		if (lpObj->m_CheckLifeTime <= 0) {
			...
			if (lpObj->lpAttackObj != 0) gObjLifeCheck(lpObj,lpObj->lpAttackObj,0,1,0,0,0,0,0);
		}
	}
}
...
BOOL gObjDuelCheck(LPOBJ lpObj) {
	...
	if (lpObj->Type == OBJ_USER)	{
		...
		if (CHECK_LIMIT(iDuelIndex, g_ConfigRead.server.GetObjectMax())) bRetVal = TRUE;
	}
...
BOOL gObjDuelCheck(LPOBJ lpObj, LPOBJ lpTargetObj) {
	...
	if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER) {
		if (lpObj->m_iDuelUser == lpTargetObj->m_Index && lpTargetObj->m_iDuelUser == lpObj->m_Index) bRetVal = TRUE;
	}
...
BOOL SkillTestFrustrum(int x, int y,int aIndex) {
	...
	if (gObj[aIndex].m_bOffLevel) return true;
...
BOOL gObjCheckMaxZen(int aIndex, int nAddZen) {
	...
	LPOBJ lpObj = &gObj[aIndex];	
	__int64 _Money = 0;
	_Money = (__int64)lpObj->CharacterData->Money + (__int64)nAddZen;
...
void gObjRecall(int aIndex, int mapnumber, int x, int y)
{
	if (gObj[aIndex].MapNumber != mapnumber)
	{
		...
		gObjClearViewport(&gObj[aIndex]);
		GSProtocol.PROTOCOL_TeleportSend(&gObj[aIndex],-1,mapnumber,gObj[aIndex].X,gObj[aIndex].Y,gObj[aIndex].Dir);
	...
	else
	{
		...
		PLAYER_TeleportMagicUse(aIndex,x,y);
	}
}
...
int gObjGetGuildUnionNumber(LPOBJ lpObj)
{
	...
	if (lpObj->CharacterData->lpGuild != NULL)
	{
		iUnion = (lpObj->CharacterData->lpGuild->iGuildUnion == 0)? lpObj->CharacterData->lpGuild->Number : lpObj->CharacterData->lpGuild->iGuildUnion;
	}
...
void gObjGetGuildUnionName(LPOBJ lpObj, char* szUnionName, int iUnionNameLen)
{
	...
	if (lpObj->CharacterData->lpGuild->iGuildUnion == 0)
	{
		return;
	}

	TUnionInfo * pUnionInfo = UnionManager.SearchUnion(lpObj->CharacterData->lpGuild->iGuildUnion);
...
BOOL gObjCheckRival(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if (lpTargetObj->CharacterData->lpGuild == 0 || lpObj->CharacterData->lpGuild == 0)
	{
		return false;
	}

	if (lpTargetObj->CharacterData->lpGuild->iGuildRival == 0 || lpObj->CharacterData->lpGuild->iGuildRival == 0)
	{
		return false;
	}

	int iUnion = !lpObj->CharacterData->lpGuild->iGuildUnion ? lpObj->CharacterData->lpGuild->Number : lpObj->CharacterData->lpGuild->iGuildUnion;

	if (UnionManager.GetGuildRelationShip(iUnion,lpTargetObj->CharacterData->lpGuild->Number) == 2)
	{
		return true;
	}
	return false;
...
int gObjGetRelationShip(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	return UnionManager.GetGuildRelationShip(iUnion, lpTargetObj->CharacterData->lpGuild->Number);
}
...
void gObjNotifyUpdateUnionV1(LPOBJ lpObj) {
	...
	for(int n = 0; n < MAX_VIEWPORT; n++) {
		if (!ObjectMaxRange(lpObj->VpPlayer[n].number)) continue;		
		if (lpObj->VpPlayer[n].state == 2 && lpObj->VpPlayer[n].type == OBJ_USER) {
			LPOBJ lpTargetObj = &gObj[lpObj->VpPlayer[n].number];
			if (lpTargetObj == 0) continue;
			if (lpTargetObj->CharacterData->lpGuild == 0) continue;			
			...
			lpMsgBody[iVp1Count].iGuildNumber = lpTargetObj->CharacterData->lpGuild->Number;
			iVp1Count++;
		}
	}
	...
	if (iVp1Count > 0 && iVp1Count <= MAX_VIEWPORT) {
		...
		if (iVp1Count != 0) g_Log.Add("[Union ViewPort] ERROR : iVp1Count is OUT of BOUND: %d",iVp1Count);
	}
}
...
void gObjNotifyUpdateUnionV2(LPOBJ lpObj) {
	if (lpObj == 0) return;
	if (lpObj->Type != OBJ_USER) return;
	if (lpObj->CharacterData->lpGuild == 0) return;
	...
	lpMsgBody2->iGuildNumber = lpObj->CharacterData->lpGuild->Number;
	...
	for(int n = 0; n < MAX_VIEWPORT; n++) {
		if (lpObj->VpPlayer2[n].type == OBJ_USER && lpObj->VpPlayer2[n].state != 0) {
			...
			if (lpTargetObj->CharacterData->lpGuild != 0) lpMsgBody2->btGuildRelationShip = gObjGetRelationShip(lpTargetObj,lpObj);
			if (lpMsgBody2->btGuildRelationShip != 1) IOCP.DataSend(lpObj->VpPlayer2[n].number,(unsigned char *)lpMsg2,((lpMsg2->h.sizeL & 0xFF) & 0xFF | ((lpMsg2->h.sizeH & 0xFF) & 0xFF) << 8) & 0xFFFF);
		}
	}
}

void gObjUnionUpdateProc(int iIndex) {
	if (gObjIsConnected(iIndex) == FALSE) return;
	LPOBJ lpObj = &gObj[iIndex];
	if (lpObj->Type != OBJ_USER) return;
	if (lpObj->RegenOk > 0) return;
	if (lpObj->CloseCount > -1) return;
	if (lpObj->CharacterData->lpGuild == NULL) return;
	if (lpObj->CharacterData->lpGuild->CheckTimeStamp(lpObj->CharacterData->iGuildUnionTimeStamp) != FALSE) return;
	lpObj->CharacterData->iGuildUnionTimeStamp = lpObj->CharacterData->lpGuild->GetTimeStamp();
...
void gObjSetKillCount(int aIndex, int iOption) {	// Option : [0 : SetToZero] [1 : Increase] [2 : Decrease]
	...
	if (iOption== 1) {
		if (lpObj->m_btKillCount <= 254) lpObj->m_btKillCount++;		
	} else if (iOption == 2) {
		if (lpObj->m_btKillCount > 0) lpObj->m_btKillCount--;
	} else if (iOption == 0) {
...
void gObjNotifyUseWeaponV1(LPOBJ lpOwnerObj, LPOBJ lpWeaponObj, int iTargetX, int iTargetY) {
	if (lpOwnerObj == 0) return;
	if (lpWeaponObj == 0) return;
	...
	for(int n = 0; n < g_ConfigRead.server.GetObjectMax(); n++) {
		...
		if (gObjIsConnected(n) == 0) continue;
		if (lpOwnerObj->MapNumber != lpTargetObj->MapNumber) continue;
		if (abs(lpTargetObj->X - iTargetX) > 6) continue;
		if (abs(lpTargetObj->Y - iTargetY) > 6) continue;
		...
		if (lpTargetObj->Type == OBJ_USER) {
			...
			if (abs(lpTargetObj->X - iTargetX) > 3) continue;
			if (abs(lpTargetObj->Y - iTargetY) > 3) continue;
...
void gObjNotifyUseWeaponDamage(LPOBJ lpWeaponObj, int iTargetX, int iTargetY) {
	if (lpWeaponObj->Class == 221 || lpWeaponObj->Class == 222) {
		for (int i = 0; i < g_ConfigRead.server.GetObjectMax(); i++) {
			LPOBJ lpObj = &gObj[i];
			if (lpWeaponObj->MapNumber != lpObj->MapNumber) continue;
			if (abs(lpObj->X - iTargetX) > 3) continue;
			if (abs(lpObj->Y - iTargetY) > 3) continue;
...
void gObjUseBlessAndSoulPotion(int aIndex, int iItemLevel) {
	...
	if (iItemLevel == 0) {
		BUFF_AddBuffEffect(lpObj, BUFFTYPE_BLESS_POTION, 0, 0, 0, 0, 60);
	} else if (iItemLevel == 1) {
		BUFF_AddBuffEffect(lpObj, BUFFTYPE_SOUL_POTION, 0, 0, 0, 0, 60);
		PLAYER_UseDrink(lpObj, 0x4D);
	}
}
...
void gObjWeaponDurDownInCastle(LPOBJ lpObj, LPOBJ lpTargetObj, int iDecValue) {
	...
	if (Left->m_Type >= ITEMGET(4,0) && Left->m_Type < ITEMGET(4,7) || Left->m_Type == ITEMGET(4,20) || Left->m_Type == ITEMGET(4,21)) bIsLeftDurDown = 1;
	else if (Right->m_Type >= ITEMGET(4,8) && Right->m_Type < ITEMGET(4,15) || Right->m_Type >= ITEMGET(4,16) && Right->m_Type < ITEMGET(5,0)) bIsRightDurDown = 1;
	else if (Right->m_Type >= 0 && Right->m_Type < ITEMGET(4,0)) bIsRightDurDown = 1;
	else if (Right->m_Type >= ITEMGET(5,0) && Right->m_Type < ITEMGET(6,0)) bIsRightDurDown = 1;
...
void gProcessAutoRecuperation(LPOBJ lpObj) {
	if (lpObj->Life == (lpObj->MaxLife + lpObj->AddLife) && lpObj->Mana == (lpObj->MaxMana + lpObj->AddMana) && lpObj->Stamina == (lpObj->MaxStamina + lpObj->AddStamina)) {
		lpObj->m_iAutoRecuperationTime = GetTickCount();
		return;
	}
	...
	if ((GetTickCount() - lpObj->m_iAutoRecuperationTime) >= 10000 && (GetTickCount() - lpObj->m_iAutoRecuperationTime) < 15000) {
		...
		if (lpObj->Mana < (lpObj->MaxMana + lpObj->AddMana) || lpObj->Stamina < (lpObj->MaxStamina + lpObj->AddStamina)) {
			lpObj->Mana += 1.0f;
			if (lpObj->Mana > (lpObj->MaxMana + lpObj->AddMana)) lpObj->Mana = lpObj->MaxMana + lpObj->AddMana;			
			lpObj->Stamina += 1;
			if (lpObj->Stamina > (lpObj->MaxStamina + lpObj->AddStamina)) lpObj->Stamina = lpObj->MaxStamina + lpObj->AddStamina;
...
void gObjShieldAutoRefill(LPOBJ lpObj) {
	...
	if (g_ShieldSystemOn == 0) return;
	if (g_ShieldAutoRefillOn == 0) return;
	if (lpObj->Type != OBJ_USER) return;
	if (MAX_MAP_RANGE(lpObj->MapNumber) == FALSE) return;
	...
	if (g_ShieldAutoRefillOnSafeZone == 1) {
		...
		if ((btMapAttribute & 1) != 1 && lpObj->CharacterData->PvPEquipmentMods.ShieldRegeneration == 0) {
			lpObj->dwShieldAutoRefillTimer = GetTickCount();
			return;
		}
	}
	...
	iShieldRefillOption = lpObj->CharacterData->PvPEquipmentMods.ShieldRegenerationBonus;
	...
	if (iRefillPoint == 0) return;

	if (lpObj->Class == CLASS_RAGEFIGHTER) {
		iRefillPoint += iRefillPoint * lpObj->CharacterData->Mastery.ML_RF_GladiatorsVigor / 100.0;
	} else {
		iRefillPoint += iRefillPoint * lpObj->CharacterData->Mastery.ML_GENERAL_GladiatorsVigor / 100.0;
	}
...
int gObjCheckOverlapItemUsingDur(int iUserIndex, int iMaxOverlapped, int iItemType, int iItemLevel) {
	for(int x = INVETORY_WEAR_SIZE; x < MAIN_INVENTORY_SIZE; x++) {
		if (gObj[iUserIndex].pInventory[x].IsItem() == 1 && gObj[iUserIndex].pInventory[x].m_Type == (short)iItemType && gObj[iUserIndex].pInventory[x].m_Level == (short)iItemLevel) {
			int iITEM_DUR = gObj[iUserIndex].pInventory[x].m_Durability;
			if ((((iITEM_DUR) < 0) ? FALSE : ((iITEM_DUR) > iMaxOverlapped - 1) ? FALSE : TRUE)) return x;
		}
	}
	...
	for(int iLoop = 0; iLoop < MAIN_INVENTORY_SIZE; iLoop ++) {
		...
		if (MAIN_INVENTORY_RANGE(iInventoryIndex)) {
			...
			if (iItemDur <= iMaxOverlapped) {
				if (MapC[iMapNumber].ItemGive(iUserIndex, iItemNumber, true) == TRUE) return iInventoryIndex;
			} else {
...
BOOL CHARACTER_CheckSkillDistance(int aIndex, int aTargetIndex, int iSkillNum) {
	if (!g_iSkillDistanceCheck) return TRUE;
	if (iSkillNum == 40) return TRUE;
	if (iSkillNum == 392) return TRUE;

	int iSkillDistance = MagicDamageC.GetSkillDistance(iSkillNum);

	if (iSkillDistance == -1) return FALSE;
	...
	if (abs(lpObj->X - lpTargetObj->X) > iSkillDistance || abs(lpObj->Y - lpTargetObj->Y) > iSkillDistance) {
		g_Log.Add("[SKILL DISTANCE CHECK] [%s][%s] Invalid Skill attacker(%d, %d), defender(%d, %d), skillnum:%d skilldistance:%d", lpObj->AccountID, lpObj->Name, lpObj->X, lpObj->Y,  lpTargetObj->X, lpTargetObj->Y, iSkillNum, iSkillDistance);
		return FALSE;
	}
...
void gObjMuBotPayForUse(LPOBJ lpObj)
{
	...
	if (!lpObj->CharacterData->m_MuBotEnable)
		return;

	if (lpObj->CharacterData->m_MuBotPayTime == 5)
	{
		int iMoney = (lpObj->Level+lpObj->CharacterData->MasterLevel) * g_ConfigRead.data.mubot.cost;

		if (iMoney > lpObj->CharacterData->Money)
		{
			lpObj->CharacterData->m_MuBotEnable = false;
			GSProtocol.GCAnsMuBotUse(lpObj->m_Index, 1, lpObj->CharacterData->m_MuBotTotalTime, 0);
			return;
		...
		lpObj->CharacterData->Money -= iMoney;
		GSProtocol.GCMoneySend(lpObj->m_Index, lpObj->CharacterData->Money);

		lpObj->CharacterData->m_MuBotPayTime = 0;

		GSProtocol.GCAnsMuBotUse(lpObj->m_Index, 0, lpObj->CharacterData->m_MuBotTotalTime, iMoney);
	}
}
...
int gGetPartyMaxLevel(int nPartyNumber)
{
	...
	for(int i=0; i<MAX_USER_IN_PARTY; i++)
	{
		...
		if (nResult < (gObj[nUserIndex].Level+gObj[nUserIndex].CharacterData->MasterLevel))
		{
			nResult = gObj[nUserIndex].Level+gObj[nUserIndex].CharacterData->MasterLevel;
		}
	}
...
int gGetLowHPMonster(int nZoneIndex, int nIndex, int nDistance)
{
	...
	for (int i=0;i<nMonsterCount;i++)
	{
		...
		if (CHARACTER_CalcDistance(lpObj, &gObj[monsterIndexInfo.m_nIndex]) < nDistance)
		{
...
void gObjPentagramMixBoxSet(int aIndex, int itempos, int xl, int yl, BYTE set_byte)
{
	...
	for (int y = 0; y < yl; y++)
	{
		...
		for (int x = 0; x < xl; x++)
		{
			...
			if (ExtentCheck(xx, yy, 8, 4) == 1)
			{
				*(BYTE*)(gObj[aIndex].CharacterData->pPentagramMixBoxMap + (itemposy + y) * 8 + (itemposx + x)) = set_byte;
			}
			else
			{
				g_Log.Add("error : %d/%d  %s %d", xx, yy, __FILE__, __LINE__);
...
void gObjPentagramMixItemSet(int aIndex, int itempos, BYTE set_byte)
{
	...
	if (gObj[aIndex].CharacterData->pPentagramMixBox[itempos].GetSize((int&)width, (int &)height) == 0)
	{
		g_Log.Add("error: Item does not exist %s %d", __FILE__, __LINE__);
		return;
	}
...
BYTE gObjPentagramMixBoxInsertItemPos(int aIndex, CItem item, int pos, int source)
{
	...
	if (lpObj->CharacterData->pPentagramMixBox[pos].IsItem() == 1)
	{
		return -1;
	}
	...
	if (source >= 0)
	{
		memcpy(TempMap, lpObj->CharacterData->pPentagramMixBoxMap, CHAOS_BOX_MAP_SIZE);
		gObjPentagramMixBoxSet(lpObj->m_Index, source, iwidth, iheight, 255);
	}

	if (*(BYTE*)(lpObj->CharacterData->pPentagramMixBoxMap + h * 8 + w) != 255)
	{
		if (source >= 0)
		{
			memcpy(lpObj->CharacterData->pPentagramMixBoxMap, TempMap, CHAOS_BOX_MAP_SIZE);
		}
		return -1;
	}

	blank = gObjMapRectCheck(lpObj->CharacterData->pPentagramMixBoxMap, w, h, 8, 4, iwidth, iheight);
	...
	lpObj->CharacterData->pPentagramMixBox[pos] = item;
	return pos;
}
...
BYTE gObjPentagramMixBoxInsertItem(int aIndex, CItem item)
{
	...
	for (int h = 0; h < 4; h++)
	{
		for (int w = 0; w < 8; w++)
		{
			if (*(BYTE*)(gObj[aIndex].CharacterData->pPentagramMixBoxMap + h * 8 + w) == 255)
			{
...
BYTE gObjPentagramMixBoxRectCheck(int aIndex, int sx, int sy, int width, int height)
{
	...
	for (y = 0; y<height; y++)
	{
		...
		for (x = 0; x<width; x++)
		{
			...
			if (::ExtentCheck(xx, yy, 8, 4) == TRUE)
			{
				if (*(BYTE*)(gObj[aIndex].CharacterData->pPentagramMixBoxMap + yy * 8 + xx) != (BYTE)-1)
				{
					return -1;
				}
			}
...
BYTE gObjPentagramMixBoxDeleteItem(int aIndex, int itempos)
{
	...
	gObj[aIndex].CharacterData->pPentagramMixBox[itempos].Clear();
	return TRUE;
}
...
BOOL gTransPentagramJewelInfoTest(int aIndex, int targetIndex, CItem sourceItem)
{
	...
	for (int index = 0; index < 250; index++)
	{
		...
		for (int j = 0; j < 250; j++)
		{
			if (gObj[aIndex].CharacterData->m_PentagramJewelInfo_Inven[j].btJewelIndex != 0xFF)
			{
				if (gObj[aIndex].CharacterData->m_PentagramJewelInfo_Inven[j].btJewelIndex == index)
				{
					bIndexExist = TRUE;
				}
			}
	...
	for (int i = 0; i < 5; i++)
	{
		if (sourceItem.m_SocketOption[i] < 0xFE)
		{
			for (int index = 0; index < 250; index++)
			{
				...
				for (int j = 0; j < 250; j++)
				{
					if (lpObj->CharacterData->m_PentagramJewelInfo_Inven[j].btJewelIndex != 0xFF)
					{
						if (lpObj->CharacterData->m_PentagramJewelInfo_Inven[j].btJewelIndex == index)
						{
							bIndexExist = TRUE;
						}
					}
			...
			*nJewelCount++;
			...
			for (int k = 0; k < 250; k++)
			{
				if (lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[k].btJewelIndex == sourceItem.m_SocketOption[i])
				{
					btJewelPos = 0;
					btItemType = lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[k].btItemType;
					wItemIndex = lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[k].wItemIndex;
					btMainAttribute = lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[k].btMainAttribute;
					btLevel = lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[k].btLevel;
					btRank1OptionNum = lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[k].btRank1OptionNum;
					btRank1Level = lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[k].btRank1Level;
					btRank2OptionNum = lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[k].btRank2OptionNum;
					btRank2Level = lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[k].btRank2Level;
					btRank3OptionNum = lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[k].btRank3OptionNum;
					btRank3Level = lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[k].btRank3Level;
					btRank4OptionNum = lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[k].btRank4OptionNum;
					btRank4Level = lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[k].btRank4Level;
					btRank5OptionNum = lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[k].btRank5OptionNum;
					btRank5Level = lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[k].btRank5Level;
					break;
				}
			}
			...
			for (int l = 0; l < 250; l++)
			{
				if (lpObj->CharacterData->m_PentagramJewelInfo_Inven[l].btJewelIndex == 0xFF)
				{
					lpObj->CharacterData->m_PentagramJewelInfo_Inven[l].btJewelPos = 0;
					lpObj->CharacterData->m_PentagramJewelInfo_Inven[l].btJewelIndex = btTargetDBJewelIndex;
					lpObj->CharacterData->m_PentagramJewelInfo_Inven[l].btItemType = btItemType;
					lpObj->CharacterData->m_PentagramJewelInfo_Inven[l].wItemIndex = wItemIndex;
					lpObj->CharacterData->m_PentagramJewelInfo_Inven[l].btMainAttribute = btMainAttribute;
					lpObj->CharacterData->m_PentagramJewelInfo_Inven[l].btLevel = btLevel;
					lpObj->CharacterData->m_PentagramJewelInfo_Inven[l].btRank1OptionNum = btRank1OptionNum;
					lpObj->CharacterData->m_PentagramJewelInfo_Inven[l].btRank1Level = btRank1Level;
					lpObj->CharacterData->m_PentagramJewelInfo_Inven[l].btRank2OptionNum = btRank2OptionNum;
					lpObj->CharacterData->m_PentagramJewelInfo_Inven[l].btRank2Level = btRank2Level;
					lpObj->CharacterData->m_PentagramJewelInfo_Inven[l].btRank3OptionNum = btRank3OptionNum;
					lpObj->CharacterData->m_PentagramJewelInfo_Inven[l].btRank3Level = btRank3Level;
					lpObj->CharacterData->m_PentagramJewelInfo_Inven[l].btRank4OptionNum = btRank4OptionNum;
					lpObj->CharacterData->m_PentagramJewelInfo_Inven[l].btRank4Level = btRank4Level;
					lpObj->CharacterData->m_PentagramJewelInfo_Inven[l].btRank5OptionNum = btRank5OptionNum;
					lpObj->CharacterData->m_PentagramJewelInfo_Inven[l].btRank5Level = btRank5Level;
					break;
				}
			}
			...
			for (int m = 0; m < 250; m++)
			{
				if (lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[m].btJewelIndex == sourceItem.m_SocketOption[i])
				{
					lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[m].btJewelPos = 0;
					lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[m].btJewelIndex = -1;
					lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[m].btItemType = -1;
					lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[m].wItemIndex = -1;
					lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[m].btMainAttribute = -1;
					lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[m].btLevel = 0;
					lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[m].btRank1OptionNum = -1;
					lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[m].btRank1Level = -1;
					lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[m].btRank2OptionNum = -1;
					lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[m].btRank2Level = -1;
					lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[m].btRank3OptionNum = -1;
					lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[m].btRank3Level = -1;
					lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[m].btRank4OptionNum = -1;
					lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[m].btRank4Level = -1;
					lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[m].btRank5OptionNum = -1;
					lpTargetObj->CharacterData->m_PentagramJewelInfo_Inven[m].btRank5Level = -1;
					break;
				}
			}
...
void gObjAutoPartySet(LPOBJ lpObj /* master */, LPOBJ lpTargetObj /*requestor*/)
{
	...
	if (lpTargetObj->CharacterData->m_bUsePartyMatching)
	{
	...
	if (g_ArcaBattle.IsArcaBattleServer() && lpObj->CharacterData->GuildNumber != lpObj->CharacterData->GuildNumber) 
	{
...
BOOL ITEM_InvalidJewelTarget(WORD sItemType) {
	if (sItemType == ITEMGET(4, 7) || sItemType == ITEMGET(4, 15)) return TRUE;
	if (sItemType == ITEMGET(0, 41)) return TRUE;

	WING_TYPE enWingType = GetWingType(sItemType, WCF_ITEMNUMBER);

	if (enWingType != NONE_WING) {
		if (enWingType == CHAOSCASTLE_WING || enWingType == GOODANDDEVIL_WING) return TRUE;
		else return FALSE;
	}

	if (sItemType >= ITEMGET(12, 200) && sItemType <= ITEMGET(12, 220)) return FALSE;
	if (sItemType < ITEMGET(12, 0)) return FALSE;
	
	return TRUE;
}

BOOL ITEM_JewelOfBlessPackLevelUp(LPOBJ lpObj, int source, int target) {
	if (source < 0 || source > MAIN_INVENTORY_SIZE) return FALSE;
	if (target < 0 || target > MAIN_INVENTORY_SIZE)	return FALSE;
	if (lpObj->pInventory[source].IsItem() == FALSE) return FALSE;
	if (lpObj->pInventory[target].IsItem() == FALSE) return FALSE;
	if (g_MineSystem.IsPickax(lpObj->pInventory[target].m_Type) == FALSE) return FALSE;
	...
	if (sRepairValue < 0) ItemPickax->m_Durability = 255.0;
	if (ItemPickax->m_Durability + sRepairValue > 255.0) return FALSE;
...
BOOL IsExceptionJewelOfBlessInchant(LPOBJ lpObj, int source, int target) {
	if (lpObj->pInventory[target].m_Type == ITEMGET(13, 37)) {
		...
		if (ItemFenrir->m_Durability >= 255) return false;
		if (ItemFenrir->IsPeriodItem() == TRUE) return false;
		if (rand() % 10000 < g_iFenrirRepairRate) {
			int iAddDur = rand() % 150 + 50;

			if ((ItemFenrir->m_Durability + iAddDur) > 255) ItemFenrir->m_Durability = 255.0f;
			else ItemFenrir->m_Durability += iAddDur;
		...
		if (sRepairValue < 0) return FALSE;
		if (ItemPickax->m_Durability + sRepairValue > 255.0) return FALSE;

		ItemPickax->m_Durability += sRepairValue;
		g_Log.Add("[MineSystem][PICKAX][REPAIR][SUCCESS][%s][%s][Type:%d][Index:%d][Level:%d][Serial:%I64d] - %d/255 (+%d)", lpObj->AccountID, lpObj->Name, lpObj->pInventory[source].m_Type / 512, lpObj->pInventory[source].m_Type % 512, lpObj->pInventory[source].m_Level, lpObj->pInventory[source].m_Number, (int)lpObj->pInventory[source].m_Durability, sRepairValue);
		return TRUE;
	} else if (lpObj->pInventory[target].m_Type == ITEMGET(13, 268)) {
		CItem * DSFRing = &lpObj->pInventory[target];

		if (DSFRing->m_Durability >= 255) return false;
		if ((DSFRing->m_Durability + 50) > 255) DSFRing->m_Durability = 255.0f;
		else DSFRing->m_Durability += 50;
			...
			if (lpBuyerObj->pInventory[n].m_Type == ITEMGET(12, 30))
			{
				if (lpBuyerObj->pInventory[n].m_Level == 2)
				{
					if (s30BJNeedCount > 0)
					{
						...
						GSProtocol.PROTOCOL_InventoryItemDeleteSend(lpBuyerObj->m_Index, n, TRUE);
						s30BJNeedCount--;
					}
				}
			}
...
BOOL SummonGoldColossusMonster(LPOBJ lpObj, int monsterIndex, int nCount, int bIsHelpMon) {
	...
	for (int i = 0; i < nCount; i++) {
		...
		if (bIsHelpMon == 1) gObj[nResult].m_bIsHelpMon = 1;

		g_Log.Add("[ColossusEvent][SummonMonster][%s][%s] MonsterClass : [%d], MonsterIndex : [%d], Map[%d], X[%d], Y[%d]", lpObj->AccountID, lpObj->Name, monsterIndex, nResult, gObj[nResult].MapNumber, gObj[nResult].X, gObj[nResult].Y);
	}
...
int gObjGetAutoPartyUserCount()
{
	...
	for(int i=g_ConfigRead.server.GetObjectStartUserIndex();i<g_ConfigRead.server.GetObjectMax();i++)
	{
		...
		{
			if (gObj[i].CharacterData->bActiveSetParty)
				counter++;
		}
	}
	return counter;
...
void gObjInvenPetDamage(LPOBJ lpObj, int damage)
{
	...
	if (sprite->m_Type == ITEMGET(13,2))
	{
		if (lpObj->Class == CLASS_RAGEFIGHTER)
		{
			if (lpObj->CharacterData->Mastery.ML_RF_Caretaker > 0.0)
			{
				fdamage = damage / lpObj->CharacterData->Mastery.ML_RF_Caretaker;
			}
		}

		else if (lpObj->CharacterData->Mastery.ML_GENERAL_Caretaker > 0.0)
		{
			fdamage = damage / lpObj->CharacterData->Mastery.ML_GENERAL_Caretaker;
		}
	...
	else if (sprite->m_Type == ITEMGET(13,4))
	{
		...
		int DurabilityVal = 1500 + lpObj->CharacterData->Mastery.ML_DL_CompanionFortitude;
	...
	if (send_dur == TRUE)
	{
		...
		if (sprite->m_Durability < 1.0)
		{
			...
			GSProtocol.PROTOCOL_InventoryItemDeleteSend(lpObj->m_Index, iInvenPetPos, 0);
...
BOOL gObjUseInvenPetCheck(LPOBJ lpObj, CItem *lpItem, int flag)
{
	if (flag == 1)
	{
		...
		int useClass = lpItem->IsClass(lpObj->Class, lpObj->CharacterData->ChangeUP);
		...
		{
			int nEffectValue = 0;
			BUFF_GetBuffEffectValue(lpObj, BUFFTYPE_BLESS, &nEffectValue, NULL);
			...
			if (lpObj->CharacterData->Strength - nEffectValue + lpObj->AddStrength < lpItem->m_RequireStrength - lpItem->m_HJOpStrength)
			{
				return FALSE;
			}

			if (lpObj->CharacterData->Dexterity - nEffectValue + lpObj->AddDexterity < lpItem->m_RequireDexterity - lpItem->m_HJOpDexterity)
			{
				return FALSE;
			}
			...
			{
				if (lpObj->CharacterData->Vitality - nEffectValue + lpObj->AddVitality < lpItem->m_RequireVitality)
				{
					return FALSE;
				}

				if (lpObj->CharacterData->Energy - nEffectValue + lpObj->AddEnergy < lpItem->m_RequireEnergy)
				{
					return FALSE;
				}
			}
...
void Check_SameSerialItem(int aIndex, BYTE bCase, bool & bCheckFail)
{
	...
	if (bCase == 1)
	{
		lpObj->CharacterData->dwOpenWareCnt++;
	}

	else if (bCase == 2)
	{
		lpObj->CharacterData->dwCloseWareCnt++;
	}
	...
	if (lpObj->CharacterData->dwOpenWareCnt == 1 || lpObj->CharacterData->dwCloseWareCnt == 1)
	{
		...
		SerialCheck->Output("[SERIAL CHECK] Performing Serial Checking due to Open/Close WareHouse - ACC:[%s], NAME:[%s],Open:[%d], Close:[%d], case:[%d]",
			lpObj->AccountID, lpObj->Name, lpObj->CharacterData->dwOpenWareCnt, lpObj->CharacterData->dwCloseWareCnt, bCase);
	}
	...
	if (bNeedFullItemLog == true)
	{
		...
		{
			lpObj->CharacterData->bHasSameSerialItem = true;
			bCheckFail = true;

			SerialCheck->Output("[SERIAL CHECK][RESULT] ~~~~~~~ FAILED ~~~~~~ ACC : [%s], CharName :[%s], AllCnt:[%d], Case : [%d] InvenCnt:[%d], WareCnt:[%d] , IP :[%s]",
				lpObj->AccountID, lpObj->Name, dwSameSerialItemCount, bCase, dwInvenDeleteCnt, dwWareDeleteCnt, lpObj->CharacterData->Ip_addr);

			SerialCheck->Output("[SERIAL CHECK] ACC:[%s],NAME:[%s],IP:[%s] - DISCONNECT",
				lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Ip_addr);
		...
		else
		{
			SerialCheck->Output("[SERIAL CHECK][RESULT] ~~~~~~~ SUCCESS ~~~~~~ ACC : [%s], CharName :[%s], Case : [%d], IP :[%s]",
				lpObj->AccountID, lpObj->Name, bCase, lpObj->CharacterData->Ip_addr);
		}
	}
	...
	else if (bCase == 3)
	{
		...
		if (dwSameSerialItemCount)
		{
			...
			SerialCheck->Output("[SERIAL CHECK][RESULT] ~~~~~~~ FAILED ~~~~~~ ACC : [%s], CharName :[%s], AllCnt:[%d], Case : [%d] InvenCnt:[%d], IP :[%s]",
				lpObj->AccountID, lpObj->Name, dwSameSerialItemCount, bCase, dwInvenDeleteCnt, lpObj->CharacterData->Ip_addr);
