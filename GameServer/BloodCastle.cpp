...
CBloodCastle::CBloodCastle() {
	...
	for (int i =0; i<MAX_BLOOD_CASTLE_LEVEL ; i++) {
		...
		for (int j = 0; j < MAX_BLOOD_CASTLE_SUB_BRIDGE; j++) this->m_BridgeData[i].m_UserData[j].m_iIndex = -1;
		this->ClearBridgeData(i);
	}
}

CBloodCastle::~CBloodCastle() {
	for (int i = 0; i < MAX_BLOOD_CASTLE_LEVEL; i++) DeleteCriticalSection(&this->m_BridgeData[i].m_critUserData);
}

void CBloodCastle::Init() {
	if (g_ConfigRead.server.GetStateFromEventTable(g_ConfigRead.server.GetServerType(), EV_TABLE_BC) == false) this->SetEventEnable(false);
	for (int i = 0; i < MAX_BLOOD_CASTLE_LEVEL; i++) {
		if (this->IsEventEnable() == false) this->SetState(i, BC_STATE_NONE);
		else this->SetState(i, BC_STATE_CLOSED);
	}
}
...
void CBloodCastle::Load(char* filename) {
	...
	if (g_ConfigRead.server.GetStateFromEventTable(g_ConfigRead.server.GetServerType(), EV_TABLE_BC) == false) bEnable = false;
	...
	for (pugi::xml_node castle = reward_exp.child("Castle"); castle; castle = castle.next_sibling()) {
		...
		if (BC_BRIDGE_RANGE(iBridgeNum) != FALSE) this->m_BridgeData[iBridgeNum].m_iBC_REWARD_EXP = castle.attribute("Multiplier").as_float();				
		else {
			g_Log.MsgBox("Error - wrong Event Level (%d)", iBridgeNum);
			continue;
		}
...
int  CBloodCastle::CheckEnterLevel(int iIndex, int iLevel)	// RET : [2:Error][1:OverLevel][0:InLevel - Success][-1:UnderLevel]
{
	...
	{
		if (gObj[iIndex].CharacterData->ChangeUP == 2)
		{
			if(iLevel != 8)
			{
				return 1;
...
bool CBloodCastle::BloodCastleChaosMix(int iIndex, int iLEVEL)
{
	...
	if ((rand()%100) < iMIX_SUCCESS_RATE)
	{
		...
		g_Log.Add("[BloodCastle Ticket Mix] [%s][%s] CBMix Success %d Money : %d-%d", gObj[iIndex].AccountID, gObj[iIndex].Name, iMIX_SUCCESS_RATE, gObj[iIndex].CharacterData->Money, iMIX_NEED_MONEY);
	}
	else
	{
		g_MixSystem.ChaosBoxInit(&gObj[iIndex]);
		GSProtocol.PROTOCOL_UserChaosBoxSend(&gObj[iIndex], 0);
		IOCP.DataSend(iIndex, (LPBYTE)&pMsg, pMsg.h.size);
		g_Log.Add("[BloodCastle Ticket Mix] [%s][%s] CBMix Fail %d Money : %d-%d", gObj[iIndex].AccountID, gObj[iIndex].Name, iMIX_SUCCESS_RATE, gObj[iIndex].CharacterData->Money, iMIX_NEED_MONEY);
		return false;
	}
...
{
    20000, 20000, 30000, 160,
	100000, 100000, 150000, 180,
	220000, 220000, 330000, 200,
	400000, 400000, 600000, 220,
	1200000, 1200000, 1800000, 240,
	6000000, 6000000, 9000000, 260,
	12000000, 12000000, 18000000, 280,
	18000000, 18000000, 27000000, 300
};
	...
	for (int x=0;x<MAIN_INVENTORY_SIZE;x++)
	{
		if (gObj[iIndex].pInventory[x].IsItem() == TRUE)
		{
			if (gObj[iIndex].pInventory[x].m_Type == ITEMGET(13,18)) // Invisibility Cloak
			{
				...
				if (CHECK_LIMIT(iITEM_LEVEL, MAX_CLOAK_LEVEL) == FALSE)
				{
					iITEM_LEVEL = 0;
...
bool CBloodCastle::AddExperience(int iIndex, int iEXP) {
	if (ObjectMaxRange(iIndex) == FALSE) return false;
	if (BC_BRIDGE_RANGE(gObj[iIndex].m_cBloodCastleIndex) == FALSE) return false;
	if (BC_SUB_BRIDGE_RANGE(gObj[iIndex].m_cBloodCastleSubIndex) == FALSE) return false;
	if (iEXP > 0) {
		...
		if (gObj[iIndex].pInventory[x].IsItem() == TRUE)
		{
			if (gObj[iIndex].pInventory[x].m_Type == ITEMGET(13,19))
			{
				...
				if (BC_WEAPON_LEVEL_RANGE(iLEVEL) != FALSE)
				{
					...
					::GSProtocol.PROTOCOL_InventoryItemDeleteSend(iIndex, x, TRUE);
...
void CBloodCastle::GiveReward_Win(int iIndex, int iBridgeIndex) {
	...
	g_Log.Add("[Blood Castle] (%d) Winner Party Point - Alive Party Member : %d, Alive Party Point : %d", iBridgeIndex+1, iALIVE_PARTYCOUNT, iADD_PARTYPOINT);
	if (ObjectMaxRange(iIndex) == FALSE) return;
	if (BC_BRIDGE_RANGE(iBridgeIndex) == FALSE)	return;	
	if (this->m_BridgeData[iBridgeIndex].m_bBC_REWARDED != false) return;
	...
	} else wsprintf(szNOTIFY_MSG, Lang.GetText(0,71));

	if (this->m_BridgeData[iBridgeIndex].m_iMISSION_SUCCESS != -1) {
		int iTOTAL_EXP = this->GetAliveUserTotalEXP(iBridgeIndex);
		g_Log.Add("[Blood Castle] (%d) Quest Complete - All Alive User's Total EXP:%d", iBridgeIndex+1, iTOTAL_EXP);

		for (int i = 0; i < MAX_BLOOD_CASTLE_SUB_BRIDGE; i++) {
			if (this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex == -1) continue;
			if (gObj[ this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex ].Connected < PLAYER_PLAYING) continue;
			if (gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].m_cBloodCastleIndex == -1 || gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].m_cBloodCastleSubIndex == -1 ||  gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].m_cBloodCastleIndex != iBridgeIndex) continue;
			PMSG_NOTICE pNotice;
			...
			if ((gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].PartyNumber != -1 && gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].PartyNumber == this->m_BridgeData[iBridgeIndex].m_iExtraEXP_Kill_Door_Party) || this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex == this->m_BridgeData[iBridgeIndex].m_iExtraEXP_Kill_Door_Index) iADD_EXP += ::g_iBC_Add_Exp[iBridgeIndex].unk0;
				...
				if (gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].PartyNumber != -1) {
					if (this->m_BridgeData[iBridgeIndex].m_UserData[i].m_bLiveWhenDoorBreak != false) iADD_EXP += ::g_iBC_Add_Exp[iBridgeIndex].unk0 / 2;
				}
			}
			if ((gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].PartyNumber != -1 && gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].PartyNumber == this->m_BridgeData[iBridgeIndex].m_iExtraEXP_Kill_Statue_Party) || this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex == this->m_BridgeData[iBridgeIndex].m_iExtraEXP_Kill_Statue_Index) iADD_EXP += ::g_iBC_Add_Exp[iBridgeIndex].unk4;
			if ((gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].PartyNumber != -1 && gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].PartyNumber == this->m_BridgeData[iBridgeIndex].m_iExtraEXP_Win_Quest_Party) || this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex == this->m_BridgeData[iBridgeIndex].m_iExtraEXP_Win_Quest_Index) iADD_EXP += ::g_iBC_Add_Exp[iBridgeIndex].unk8;
			...
			switch (this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iUserState) {
				case 0:
					...
					iREWARD_EXP = this->BC_SendRewardEXP(this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex, iADD_EXP);
					iREWARD_ZEN = this->SendRewardZEN(this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex, ::g_iQuestWinExpendZEN[iBridgeIndex].SpecialCharacter);
				...
				case 2:
					iADD_EXP += (int)this->m_BridgeData[iBridgeIndex].m_i64_BC_REMAIN_MSEC / 1000 * ::g_iBC_Add_Exp[iBridgeIndex].unkC;
					iREWARD_EXP = this->BC_SendRewardEXP(this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex, iADD_EXP);
					iREWARD_ZEN = this->SendRewardZEN(this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex, ::g_iQuestWinExpendZEN[iBridgeIndex].NormalCharacter);
					iREWARD_SCR = ::g_iBC_EventScore[iBridgeIndex].unk8;
					iREWARD_SCR += iADD_PARTYPOINT;
					if (BC_MAP_RANGE(gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].MapNumber) != FALSE) this->DropReward(this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex);
...
void CBloodCastle::GiveReward_Fail(int iBridgeIndex)
{
	...
	for (int i=0;i<MAX_BLOOD_CASTLE_SUB_BRIDGE;i++)
	{
		...
		this->BC_SendRewardEXP(this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex, iADD_EXP);
...
int CBloodCastle::BC_CalcRewardEXP(int UserIndex, UINT64 iAddExp) {
	...
	CHARACTER_LevelUp(&gObj[UserIndex], iAddExp, 0, "Blood Castle");
	return iAddExp;
}
int CBloodCastle::BC_SendRewardEXP(int UserIndex, int iEXP) {
	...
	if (g_CrywolfSync.CW_OccupationState() == TRUE && g_CrywolfSync.CW_ApplyPenalty() == TRUE) {
		...
		CWpenalty /= 100;
		iEXP *= CWpenalty; //season 2.5 changed
	}

	if (ObjectMaxRange(UserIndex) == FALSE) return FALSE;
	if (gObj[UserIndex].Connected != PLAYER_PLAYING) return FALSE;
	...
	if (gObj[UserIndex].Type == OBJ_USER) {
		if (Reward > 0) {
			...
			Reward = (Reward) * this->m_BridgeData[gObj[UserIndex].m_cBloodCastleIndex].m_iBC_REWARD_EXP; //season 4.5 add-on
			iEXP = this->BC_CalcRewardEXP(UserIndex, Reward);
		}
...
int CBloodCastle::SendRewardZEN(int iIndex, int iZEN) {
	if (iZEN <= 0) return FALSE;
	...
	if (ObjectMaxRange(iIndex) == FALSE) return FALSE;
	if (gObj[iIndex].Connected != PLAYER_PLAYING) return FALSE;
	if (gObjCheckMaxZen(iIndex, iZEN) == FALSE) {
		iRET_ZEN = MAX_ZEN - gObj[iIndex].CharacterData->Money;
		gObj[iIndex].CharacterData->Money += iRET_ZEN;
		return iRET_ZEN;
	}

	gObj[iIndex].CharacterData->Money += iZEN;
	iRET_ZEN = iZEN;
	GSProtocol.GCMoneySend(iIndex, gObj[iIndex].CharacterData->Money);
	return iRET_ZEN;
}
void CBloodCastle::DropReward(int iIndex) {
	...
	switch (gObj[iIndex].m_cBloodCastleIndex) {
		case 0:					g_BagManager.UseBag_GremoryCase(iIndex, BAG_EVENT, EVENTBAG_BC1, iIndex, GC_STORAGE_CHARACTER, GC_REWARD_BLOOD_CASTLE, 30);						break;
		case 1:					g_BagManager.UseBag_GremoryCase(iIndex, BAG_EVENT, EVENTBAG_BC2, iIndex, GC_STORAGE_CHARACTER, GC_REWARD_BLOOD_CASTLE, 30);						break;
		case 2:					g_BagManager.UseBag_GremoryCase(iIndex, BAG_EVENT, EVENTBAG_BC3, iIndex, GC_STORAGE_CHARACTER, GC_REWARD_BLOOD_CASTLE, 30);						break;
		case 3:					g_BagManager.UseBag_GremoryCase(iIndex, BAG_EVENT, EVENTBAG_BC4, iIndex, GC_STORAGE_CHARACTER, GC_REWARD_BLOOD_CASTLE, 30);						break;
		case 4:					g_BagManager.UseBag_GremoryCase(iIndex, BAG_EVENT, EVENTBAG_BC5, iIndex, GC_STORAGE_CHARACTER, GC_REWARD_BLOOD_CASTLE, 30);						break;
		case 5:					g_BagManager.UseBag_GremoryCase(iIndex, BAG_EVENT, EVENTBAG_BC6, iIndex, GC_STORAGE_CHARACTER, GC_REWARD_BLOOD_CASTLE, 30);						break;
		case 6:					g_BagManager.UseBag_GremoryCase(iIndex, BAG_EVENT, EVENTBAG_BC7, iIndex, GC_STORAGE_CHARACTER, GC_REWARD_BLOOD_CASTLE, 30);						break;
		case 7:					g_BagManager.UseBag_GremoryCase(iIndex, BAG_EVENT, EVENTBAG_BC8, iIndex, GC_STORAGE_CHARACTER, GC_REWARD_BLOOD_CASTLE, 30);						break;
	}
...
void CBloodCastle::ChangeMonsterState(int iBridgeIndex, int iIndex)
{
	...
	gObj[iIndex].Defense = lpMA->Defense + iAddDefense;
}
