...
SHOP_DATA * CShopMng::GetShop(LPOBJ lpObj, LPOBJ lpNpc)
{
	...
	if (lpObj->CharacterData->VipType < ItShopData->btReqVipLevel && ItShopData->btOnlyForGameMaster == FALSE)
	{
		if (lpObj->CharacterData->VipType == 0)
		{
			GSProtocol.PROTOCOL_NPCChatSend(lpNpc, Lang.GetText(0,579), lpObj->m_Index);
		}

		else
		{
			char szMsg[256];
			sprintf(szMsg, Lang.GetText(0,605), g_VipSystem.GetVipName(lpObj->CharacterData->VipType), g_VipSystem.GetVipName(ItShopData->btReqVipLevel));
			GSProtocol.PROTOCOL_NPCChatSend(lpNpc, szMsg, lpObj->m_Index);
		}
	...
	if (ItShopData->btOnlyForGameMaster == TRUE)
	{
		...
		{
			GSProtocol.PROTOCOL_NPCChatSend(lpNpc, Lang.GetText(0,580), lpObj->m_Index);
			return NULL;
		}
	}
	...
	{
		if ((rand() % 2) != 0)
			GSProtocol.PROTOCOL_ChatSend(lpNpc, Lang.GetText(0,51));
		else
			GSProtocol.PROTOCOL_ChatSend(lpNpc, Lang.GetText(0,52));
	...
	if (g_ConfigRead.pk.bPkPenaltyDisable == FALSE && g_GensSystem.IsMapBattleZone(lpObj->MapNumber) == FALSE && g_ConfigRead.pk.bPkCanUseShops == false)
	{
		...
		{
			if ((rand() % 2) != 0)
				GSProtocol.PROTOCOL_ChatSend(lpNpc, Lang.GetText(0,53));
			else
				GSProtocol.PROTOCOL_ChatSend(lpNpc, Lang.GetText(0,54));
...
void CCancelItemSale::GCAnsSoldItemList(int iIndex)
{
	...
	{
		if (gObj[iIndex].CharacterData->m_CancelItemSaleList[i].btUsed == TRUE)
		{
			pItem.wItemCount = gObj[iIndex].CharacterData->m_CancelItemSaleList[i].wItemCount;
			pItem.dwItemPrice = gObj[iIndex].CharacterData->m_CancelItemSaleList[i].dwItemPrice;
			pItem.iLeftTime = gObj[iIndex].CharacterData->m_CancelItemSaleList[i].iLeftTime;
			pItem.btItemNumber = gObj[iIndex].CharacterData->m_CancelItemSaleList[i].btItemNumber;
			memcpy(&pItem.btItemInfo, gObj[iIndex].CharacterData->m_CancelItemSaleList[i].btItemInfo, MAX_ITEM_INFO);
...
void CCancelItemSale::CGReqEndCancelItemSale(int iIndex)
{
	...
	gObj[iIndex].CharacterData->m_bIsCancelItemSale = false;

	for (int i = 0; i < MAX_CANCEL_ITEMS_SALE; i++)
	{
		gObj[iIndex].CharacterData->m_CancelItemSaleList[i].Clear();
	}
}
...
void CCancelItemSale::CGReqReBuyItem(PMSG_REQ_REBUY_ITEM *lpMsg, int iIndex)
{
	...
	if (gObj[iIndex].CharacterData->m_bIsCancelItemSale == false)
	{
	...
	for (int i = 0; i < MAX_CANCEL_ITEMS_SALE; i++)
	{
		if (gObj[iIndex].CharacterData->m_CancelItemSaleList[i].btUsed == TRUE)
		{
			if (gObj[iIndex].CharacterData->m_CancelItemSaleList[i].btItemNumber == lpMsg->btItemNumber &&
				gObj[iIndex].CharacterData->m_CancelItemSaleList[i].wItemCode == lpMsg->wItemCode &&
				gObj[iIndex].CharacterData->m_CancelItemSaleList[i].dwItemPrice == lpMsg->dwItemPrice &&
				gObj[iIndex].CharacterData->m_CancelItemSaleList[i].iLeftTime == lpMsg->dwLeftTime)
			{
				btItemNumber = i;
				break;
			}
...
void CCancelItemSale::DGAnsSoldItemList(LPBYTE lpRecv)
{
	...
	{
		gObj[iIndex].CharacterData->m_bIsCancelItemSale = true;
		...
		for (int i = 0; i < lpMsg->iCount; i++)
		{
			...
			gObj[iIndex].CharacterData->m_CancelItemSaleList[i].btUsed = TRUE;
			gObj[iIndex].CharacterData->m_CancelItemSaleList[i].btItemNumber = lpItemList->btItemNumber;
			gObj[iIndex].CharacterData->m_CancelItemSaleList[i].wItemCode = lpItemList->wItemCode;
			gObj[iIndex].CharacterData->m_CancelItemSaleList[i].dwItemPrice = lpItemList->dwSellPrice;
			gObj[iIndex].CharacterData->m_CancelItemSaleList[i].iLeftTime = lpItemList->SellExpireDate - time(NULL);
			gObj[iIndex].CharacterData->m_CancelItemSaleList[i].wItemCount = lpItemList->wItemCount;

			ItemByteConvert(gObj[iIndex].CharacterData->m_CancelItemSaleList[i].btItemInfo,
...
void CCancelItemSale::DGAnsGetReBuyItem(SDHP_ANS_SHOP_REBUY_GET_ITEM *lpMsg)
{
	...
	{
		if (gObj[iIndex].CharacterData->Money < lpMsg->dwSellPrice)
		{
		...
		gObj[iIndex].CharacterData->Money -= lpMsg->dwSellPrice;
