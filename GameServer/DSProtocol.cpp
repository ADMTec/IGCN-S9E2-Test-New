...
void JGPGetCharList(unsigned char * lpRecv)
{
	...
	gObj[aIndex].CharacterData->m_WarehouseExpansion = lpCount->WhExpansion;
	gObj[aIndex].CharacterData->m_iSecurityCode = lpCount->SecurityCode;

	
	if (gObj[aIndex].CharacterData->m_iSecurityCode != 0)
	{
		gObj[aIndex].CharacterData->m_bSecurityCheck = false;
		GSProtocol.GCServerMsgStringSend(Lang.GetText(0,534), aIndex, 0);
	}

	else
	{
		gObj[aIndex].CharacterData->m_bSecurityCheck = true;
	}
	...
	if (pCLCount.Count > 0)
	{
		gObj[aIndex].CharacterData->m_NameConvertOfUBF.InitData(); // UBF Name Conversion
		...
		for (int n=0;n<pCLCount.Count ;n++)
		{
			...
			if (g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE)
			{
				...
				gObj[aIndex].CharacterData->m_NameConvertOfUBF.InputData(n, tmpKeyName.c_str(), tmpRealName.c_str(), lpCL->ServerCodeOfHomeWorld, MAX_ACCOUNT_LEN);
...
void JGGetCharacterInfo(SDHP_DBCHAR_INFORESULT * lpMsg) {
		...
		if (gObj[i].Connected == PLAYER_PLAYING) {
			if (gObj[i].Type == OBJ_USER) {
				if (!strncmp(szName, gObj[i].Name, MAX_ACCOUNT_LEN) || !strncmp(szAccountId, gObj[i].AccountID, MAX_ACCOUNT_LEN)) {
					g_Log.Add("[Anti-HACK][JGGetCharacterInfo] Attempted Character-Copy by double logging [%s][%s]", szName, gObj[aIndex].AccountID);
	...
	if (lpObj->CharacterData->Money < 0) {
		g_Log.AddC(TColor::Red, "ERROR: MONEY < 0 [%s][%s] %d -> 0", szName, gObj[aIndex].AccountID, lpObj->CharacterData->Money);
	...
	if (lpObj->m_bMapSvrMoveReq == false) {
		...
		if (lpObj->m_PK_Level > 4) {
			...
			g_Log.Add("[CRenewal][PKSetting][%s][%s] PkLevel:[%d],KillCount:[%d], PkPoint:[%d] PK Character move to LORENCIA", lpObj->AccountID, lpObj->Name, lpObj->m_PK_Level, lpObj->m_PK_Count, lpObj->m_PK_Time);
		}
		...
		if (wGameServerCode != g_ConfigRead.server.GetGameServerCode()) {
			...
			g_Log.Add("[MapServerMng] Request to Move Map Server : (%d) - [%s][%s] (%d)", wGameServerCode, lpObj->AccountID, lpObj->Name, lpObj->m_Index);
			return;
		}
	}
	...
	UINT64 NextExperience = lpObj->CharacterData->NextExp;

	if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(lpObj) != FALSE) {
		NextExperience = lpObj->CharacterData->MasterNextExp;
	}
	...
	pjMsg.LevelUpPoint = lpObj->CharacterData->LevelUpPoint;
	pjMsg.Str = lpObj->CharacterData->Strength;
	pjMsg.Dex = lpObj->CharacterData->Dexterity;
	pjMsg.Vit = lpObj->CharacterData->Vitality;
	pjMsg.Energy = lpObj->CharacterData->Energy;
	pjMsg.Money = lpObj->CharacterData->Money;
	...
	pjMsg.Stamina = lpObj->Stamina;
	pjMsg.MaxStamina = lpObj->MaxStamina + lpObj->AddStamina;
	pjMsg.Leadership = lpObj->Leadership;
	pjMsg.InvExpansion = lpObj->CharacterData->m_InventoryExpansion;
	...
	pjMsg.Ruud = lpObj->CharacterData->Ruud;
	short AddPoint = 0;
	short MaxAddPoint = 0;
	short MinusPoint = 0;
	...
	g_Log.Add("[FRUIT System] [%s][%s] (MinusPoint:%d/PlusPoint:%d) (MaxMinus:%d/MaxPlus:%d)", lpObj->AccountID, lpObj->Name, MinusPoint, AddPoint, MaxMinusPoint, MaxAddPoint);

	if (AddPoint < 0 || AddPoint > MaxAddPoint || MinusPoint < 0 || MinusPoint > MaxMinusPoint) {
		g_Log.Add("[FRUIT System] Character Stat Error [%s][%s] (MinusPoint:%d/PlusPoint:%d) (MaxMinus:%d/MaxPlus:%d)", lpObj->AccountID, lpObj->Name, MinusPoint, AddPoint, MaxMinusPoint, MaxAddPoint);
	}
	...
	pRSMsg.Resets = lpObj->CharacterData->m_iResets;
	...
	lpObj->CharacterData->m_dwMapMoveKeyValue = g_KeyGenerater.GenerateSeedValue();
	GSProtocol.PROTOCOL_SendMapMoveCheckSum(lpObj);
	...
	if (g_ConfigRead.data.common.EnterGameMessage == 1)	{
		...
		GSProtocol.PROTOCOL_DataSendAll((LPBYTE)&pMsg, pMsg.h.size);
	}

	if (lpObj->CharacterData->VipType > 0) {
		MsgOutput(aIndex, Lang.GetText(0,493), g_VipSystem.GetVipName(lpObj->CharacterData->VipType));
	}
	...
	GSProtocol.PROTOCOL_ReFillSend(aIndex, gObj[aIndex].Life, 0xFF, 0, gObj[aIndex].iShield);
	GSProtocol.PROTOCOL_ReFillSend(aIndex, gObj[aIndex].AddLife + gObj[aIndex].MaxLife, 0xFE, 0, gObj[aIndex].iAddShield + gObj[aIndex].iMaxShield);
	GSProtocol.PROTOCOL_ManaSend(aIndex, gObj[aIndex].Mana, 0xFF, 0, gObj[aIndex].Stamina);
	GSProtocol.PROTOCOL_ManaSend(aIndex, gObj[aIndex].MaxMana, 0xFE, 0, gObj[aIndex].MaxStamina);
	...
	if (lpObj->Class == CLASS_RAGEFIGHTER || lpObj->Class == CLASS_ELF) { // Add DK Class condition for DeathStab Visual Fix
...
void GJSetCharacterInfo(LPOBJ lpObj, int aIndex, BOOL bMapServerMove) {
	if (lpObj->m_bMapSvrMoveQuit == true) {
		g_Log.AddC(TColor::Red, "[MapServerMng] GJSetCharacterInfo() - Inventory Already Saved [%s][%s] (%d)", lpObj->AccountID, lpObj->Name, lpObj->m_Index);
		return;
	}

	if (bMapServerMove == TRUE) {
		lpObj->m_bMapSvrMoveQuit = TRUE;
		g_Log.AddC(TColor::Brown,  "[MapServerMng][%s][%s] GJSetCharacterInfo() - MapServerMove Character Save Start (%d)", lpObj->AccountID, lpObj->Name, lpObj->m_Index);
	}
	...
	pCSave.Class = lpObj->CharacterData->DbClass;
	pCSave.Level = lpObj->Level;
	pCSave.mLevel = lpObj->CharacterData->MasterLevel;
	pCSave.LevelUpPoint = lpObj->CharacterData->LevelUpPoint;
	pCSave.mlPoint = lpObj->CharacterData->MasterPoint;
	pCSave.Exp = lpObj->CharacterData->Experience;
	pCSave.mlExp = lpObj->CharacterData->MasterExperience;
	pCSave.NextExp = lpObj->CharacterData->NextExp;
	pCSave.mlNextExp = lpObj->CharacterData->MasterNextExp;
	pCSave.Money = lpObj->CharacterData->Money;
	pCSave.Str = lpObj->CharacterData->Strength;
	pCSave.Dex = lpObj->CharacterData->Dexterity;
	pCSave.Vit = lpObj->CharacterData->Vitality;
	pCSave.Energy = lpObj->CharacterData->Energy;
	...
	pCSave.iFruitPoint = lpObj->CharacterData->iFruitPoint;
	pCSave.CharInfoSave = 0;
	pCSave.resets = lpObj->CharacterData->m_iResets;

	memcpy(pCSave.dbQuest, lpObj->CharacterData->m_Quest, sizeof(pCSave.dbQuest));
...
void GDGetWarehouseList(int aIndex, char* AccountID) {
	...
	if (gObj[aIndex].m_bMapSvrMoveReq == true) {
		g_Log.Add("[GetWarehouseList] MapServerMove User. Can't Open WhareHouse. return!! [%s], IP [%s] ", AccountID, gObj[aIndex].CharacterData->Ip_addr);
		return;
	}
...
void DGGetWarehouseList(SDHP_GETWAREHOUSEDB_SAVE * lpMsg)
{
	...
	if (gObj[aIndex].m_bMapSvrMoveReq == true) 
	{
		g_Log.Add("[DGGetWarehouseList] MapServerMove User. Can't Open WhareHouse. return!! [%s], IP [%s] ",
			szId, gObj[aIndex].CharacterData->Ip_addr);
		return;
	}
	...
	if (bCanWarehouseLock == TRUE)
	{
		if (gObj[aIndex].WarehousePW > 0)
		{
			...
			{
				GSProtocol.PROTOCOL_WarehouseStateSend(aIndex, 12);
			}
			else
			{
				GSProtocol.PROTOCOL_WarehouseStateSend(aIndex, 1);
			}
		}
		else
		{
			GSProtocol.PROTOCOL_WarehouseStateSend(aIndex, 0);
		}
	}

	lpObj->LoadWareHouseInfo = true;
	lpObj->CharacterData->m_ReqWarehouseOpen = false;
...
void GDGetWarehouseNoItem(SDHP_GETWAREHOUSEDB_RESULT * lpMsg)
{
	...
	if (lpObj->m_IfState.type == 6)
	{
		if (lpObj->m_IfState.state == 0)
		{
			...
			pMsg.Result = 1;
			pMsg.iMoney = lpObj->CharacterData->Money;
...
void ItemSerialCreateRecv(SDHP_ITEMCREATERECV * lpMsg) {
	...
	if (mapnumber != (BYTE) - 1) {
		if (mapnumber < 0 || (mapnumber > MAX_NUMBER_MAP - 1 && mapnumber < 218)) return;
	}
	...
	if (lpMsg->MapNumber == (BYTE) - 1 || lpMsg->MapNumber == (BYTE) - 2) {
		...
		if (lpMsg->MapNumber == (BYTE) - 2) pMsg.Result = 100;
		else pMsg.Result = 1;
		...
		if (lpMsg->Type == ITEMGET(14,7)) NewItem.m_Durability = lpMsg->Dur; // Siege Potion
		if (lpMsg->Type == ITEMGET(13,37)) NewItem.m_Durability = lpMsg->Dur;
		...
		if (g_SocketOptionSystem.IsEnableSocketItem(&NewItem) == true) {
			...
			for (int i = 0; i < 5; i++) {
				if (i < SocketSlotCount) SocketOption[i] = 0xFE;
				else SocketOption[i] = 0xFF;
			}
		} else {
			SocketBonusOption = 0;
		...
		if (lpObj->ChaosMassMixCurrItem == 0) g_MixSystem.ChaosBoxInit(lpObj);
...
;
	...
	if (lpMsg->MapNumber == 230) {
		...
		if (btPos == 0xFF) {
			pMsg.Result = CB_ERROR;
			g_Log.Add("[PentagramRefine] Error : Failed To Insert Item Type:%d, TypeIndex:%d to [%s][%s]", iType, iTypeIndex, lpObj->AccountID, lpObj->Name);
			IOCP.DataSend(aIndex, (LPBYTE)&pMsg, pMsg.h.size);
			lpObj->CharacterData->m_PentagramMixLock = FALSE;
		} else {
		...
		if (btRes == 0xFF) {
			...
			g_Log.Add("[Make Pentagram Item] Error : Failed To Insert Item Type:%d, TypeIndex:%d to [%s][%s]", iType, iTypeIndex, lpObj->AccountID, lpObj->Name);
		} else {
		...
		if (gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED) {
			...
			if (iItemPos == (BYTE) - 1) g_Log.Add("[QuestExp RewardItem] Error : Failed To Add Item Type:%d, TypeIndex:%d to [%s][%s]", iType, iTypeIndex, gObj[lpMsg->aIndex].AccountID, gObj[lpMsg->aIndex].Name);
			else {
		...
		if (gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED) {
			...
			if (iItemPos == (BYTE) - 1) {
				g_Log.Add("[Lucky Coin Trade] Error : Failed To Add Item Type:%d, TypeIndex:%d to [%s][%s]", iType, iTypeIndex, gObj[lpMsg->aIndex].AccountID, gObj[lpMsg->aIndex].Name);
				GSProtocol.GCAnsLuckyCoinTrade(lpMsg->aIndex, 2);
			} else {
				if (lpMsg->lDuration > 0) g_PeriodItemEx.SetPeriodItemInfo(lpObj, lpMsg->Type, lpMsg->m_Number, lpMsg->lDuration);				
				GSProtocol.GCInventoryItemOneSend(lpMsg->aIndex, iItemPos);
				if (lpMsg->lDuration > 0) g_PeriodItemEx.SendPeriodItemInfoOnce(lpObj, &Item);
		...
		if (gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED) {
			...
			if (iItemPos == (BYTE) - 1) g_Log.Add("[Inventory Insert Item] Error : Failed To Insert Item Type:%d, TypeIndex:%d to [%s][%s]", iType, iTypeIndex, gObj[lpMsg->aIndex].AccountID, gObj[lpMsg->aIndex].Name);
			else {
				if (lpMsg->lDuration > 0) g_PeriodItemEx.SetPeriodItemInfo(lpObj, lpMsg->Type, lpMsg->m_Number, lpMsg->lDuration);				
				GSProtocol.GCInventoryItemOneSend(lpMsg->aIndex, iItemPos);
				if (lpMsg->lDuration > 0) g_PeriodItemEx.SendPeriodItemInfoOnce(lpObj, &Item);
		...
		if (gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED) {
			...
			if (btItemPos == (BYTE) - 1) g_Log.Add("[ItemShop] Error : Failed To Insert Period Item Type:%d, TypeIndex:%d to [%s][%s]", iItemType, iItemIndex, gObj[lpMsg->aIndex].AccountID, gObj[lpMsg->aIndex].Name);
			else {
				if (lpMsg->lDuration > 0) g_PeriodItemEx.SetPeriodItemInfo(lpObj, lpMsg->Type, lpMsg->m_Number, lpMsg->lDuration);				
				GSProtocol.GCInventoryItemOneSend(lpMsg->aIndex, btItemPos);
				if (lpMsg->lDuration > 0) g_PeriodItemEx.SendPeriodItemInfoOnce(lpObj, &pCreateItem);
			}
		}
		...
		if (gObj[lpMsg->aIndex].Connected > PLAYER_LOGGED) {
			...
			if (bIsBlank == TRUE) {
				...
				gObj[lpMsg->aIndex].CharacterData->Money -= iBuyTaxMoney + g_GamblingItemBag.GetGamblingValue();
				if (gObj[lpMsg->aIndex].CharacterData->Money < 0) gObj[lpMsg->aIndex].CharacterData->Money = 0;
				GSProtocol.GCMoneySend(aIndex, gObj[lpMsg->aIndex].CharacterData->Money);
				GSProtocol.GCInventoryItemOneSend(lpMsg->aIndex, btItemPos);

				BYTE ExOption[6];
				ItemIsBufExOption(ExOption, &pCreateItem);

				if (lpMsg->Type == ITEMGET(3,11) || lpMsg->Type == ITEMGET(5,33) || lpMsg->Type == ITEMGET(4,24) || lpMsg->Type == ITEMGET(2,18) || lpMsg->Type == ITEMGET(5,34)) {
					char szBuffer[256] = { 0 };
		...
		if (gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED) {
			...
			if (iItemPos == (BYTE) - 1) g_Log.Add("[Gens Reward] Error : Failed To Insert Item Type:%d, TypeIndex:%d to [%s][%s]", iType, iTypeIndex, gObj[lpMsg->aIndex].AccountID, gObj[lpMsg->aIndex].Name);
			else {
				GSProtocol.GCInventoryItemOneSend(lpMsg->aIndex, iItemPos);
				g_Log.Add("[GensRanking] Reward User:[%s][%s] Influence:%d, GensClass:%d, ContributePoint:%d, Item:%s", gObj[lpMsg->aIndex].AccountID, gObj[lpMsg->aIndex].Name, gObj[lpMsg->aIndex].CharacterData->m_GensInfluence, gObj[lpMsg->aIndex].CharacterData->m_GensClass, gObj[lpMsg->aIndex].CharacterData->m_ContributePoint, Item.GetName());
			}
		}
		...
		} else Item.Convert(ITEMGET(iType, iTypeIndex), lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, lpMsg->NewOption, lpMsg->SetOption, 0, lpMsg->SocketOption, lpMsg->MainAttribute, 0, CURRENT_DB_VERSION);

		int iItemCount = 0;

		if (g_CMuunSystem.IsMuunItem(lpMsg->Type) == TRUE) {
			int nMuunRank = g_CMuunSystem.GetMuunRankOfMuunInfo(lpMsg->Type);
			int iItemCount = MapC[gObj[lpMsg->aIndex].MapNumber].MonsterItemDrop(lpMsg->Type, lpMsg->Level, lpMsg->Dur, lpMsg->x, lpMsg->y, lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, lpMsg->NewOption, lpMsg->SetOption, lootindex, lpMsg->m_Number, 0, lpMsg->SocketOption, nMuunRank, lpMsg->lDuration);
		} else int iItemCount = MapC[gObj[lpMsg->aIndex].MapNumber].MonsterItemDrop(lpMsg->Type, lpMsg->Level, lpMsg->Dur, lpMsg->x, lpMsg->y, lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, lpMsg->NewOption, lpMsg->SetOption, lootindex, lpMsg->m_Number, 0, lpMsg->SocketOption, lpMsg->MainAttribute, lpMsg->lDuration);

		BYTE NewOption[MAX_EXOPTION_SIZE];

		for (int i = 0; i < MAX_EXOPTION_SIZE; i++) NewOption[i] = FALSE;
		if ((lpMsg->NewOption & 0x20) != 0) NewOption[0] = TRUE;
		if ((lpMsg->NewOption & 0x10) != 0) NewOption[1] = TRUE;
		if ((lpMsg->NewOption & 0x8) != 0) NewOption[2] = TRUE;
		if ((lpMsg->NewOption & 0x4) != 0) NewOption[3] = TRUE;
		if ((lpMsg->NewOption & 0x2) != 0) NewOption[4] = TRUE;
		if ((lpMsg->NewOption & 0x1) != 0) NewOption[5] = TRUE;
		...
		if (gObj[lpMsg->aIndex].Connected > 1) {
			...
			if (btItemPos == (BYTE) - 1) g_Log.Add("[EventInven][ItemSerialCreateRecv][FAIL][Type:%d][Index:%d][Level:%d][Serial:%I64d][%s][%s]", iType, iTypeIndex, Item.m_Level, Item.m_Number, gObj[lpMsg->aIndex].AccountID, gObj[lpMsg->aIndex].Name);
			else {
				GSProtocol.GCEventInvenItemOneSend(lpMsg->aIndex, btItemPos);
				g_Log.Add("[EventInven][ItemSerialCreateRecv][SUCCESS][Type:%d][Index:%d][Level:%d][Serial:%I64d][%s][%s]", iType, iTypeIndex, Item.m_Level, Item.m_Number, gObj[lpMsg->aIndex].AccountID, gObj[lpMsg->aIndex].Name);
			}
		}
		...
		if (gObj[lpMsg->aIndex].Connected > 1) {
			...
			if (g_PentagramSystem.IsPentagramItem(lpMsg->Type) == TRUE) {
				...
				for (int i = 0; i < 5; i++) {
					if (btEnableSlot[i]) btSocketSlotCount[i] = 0xFE;
					else btSocketSlotCount[i] = 0xFF;
				}
		...
		if (gObj[lpMsg->aIndex].Connected > 1) {
			...
			for (int k = 0; k < 5; k++) SocketOption[k] = lpMsg->SocketOption[k];
			...
			if (g_CMuunSystem.IsMuunExpireDate(lpMsg->Type) == TRUE) MuunItem.SetMuunItemPeriodExpire();

			BYTE btItemPos = gObjMuunInventoryInsertItem(lpMsg->aIndex, MuunItem);

			if (btItemPos == (BYTE) - 1) g_Log.Add("[MuunSystem] Error : Failed To Insert Item Type:%d, TypeIndex:%d to [%s][%s]", ItemType, ItemIndex, gObj[lpMsg->aIndex].AccountID, gObj[lpMsg->aIndex].Name);
			else {
				GSProtocol.GCMuunInventoryItemOneSend(lpMsg->aIndex, btItemPos);
				g_Log.Add("[MuunSystem][ItemSerialCreateRecv][SUCCESS][Type:%d][Index:%d][Level:%d][Serial:%I64d][%s][%s]", ItemType, ItemIndex, MuunItem.m_Level, MuunItem.m_Number, lpObj->AccountID, lpObj->Name);
			}
		}
		...
		if (gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED) {
			...
			if (iItemPos == (BYTE) - 1) g_Log.Add("[MineSystem][ItemSerialCreateRecv][FAIL][Type:%d][Index:%d][Level:%d][Serial:%I64d][%s][%s]", iType, iTypeIndex, Item.m_Level, Item.m_Number, lpObj->AccountID, lpObj->Name);
			else {
				g_Log.Add("[MineSystem][ItemSerialCreateRecv][SUCCESS][Type:%d][Index:%d][Level:%d][Serial:%I64d][%s][%s]",	iType, iTypeIndex, Item.m_Level, Item.m_Number, lpObj->AccountID, lpObj->Name);
				::GSProtocol.GCInventoryItemOneSend(lpMsg->aIndex, iItemPos);
			}
		...
		if (gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED) {
			...
			if (iItemPos == (BYTE)-1) {
				pResult.Result = -1;
				g_Log.Add("[Shop Buy][ItemSerialCreateRecv][FAIL][Type:%d][Index:%d][Level:%d][Serial:%I64d][%s][%s]", iType, iTypeIndex, Item.m_Level, Item.m_Number, lpObj->AccountID, lpObj->Name);
			} else {
				pResult.Result = iItemPos;
				ItemByteConvert(pResult.ItemInfo, Item);
				g_Log.Add("[Shop Buy][ItemSerialCreateRecv][SUCCESS][Type:%d][Index:%d][Level:%d][Serial:%I64d][%s][%s]", iType, iTypeIndex, Item.m_Level, Item.m_Number, lpObj->AccountID, lpObj->Name);
			}
		...
		if (gObj[lpMsg->aIndex].Connected > 1) {
			...
			if (btItemPos == (BYTE) - 1) g_Log.Add("[Extract Seed] Error : Failed To Insert Item Type:%d, TypeIndex:%d to [%s][%s]", ItemType, ItemIndex, gObj[lpMsg->aIndex].AccountID, gObj[lpMsg->aIndex].Name);
			else {
				GSProtocol.GCInventoryItemOneSend(lpMsg->aIndex, btItemPos);
		...
		} else gObj[lpMsg->aIndex].ChaosLock = FALSE;
		...
		if (gObj[lpMsg->aIndex].Connected > 1) {
			...
			if (g_CMuunSystem.IsMuunItem(lpMsg->Type) == TRUE) nRank = g_CMuunSystem.GetMuunRankOfMuunInfo(lpMsg->Type);
			else if (g_CMuunSystem.IsStoneofEvolution(lpMsg->Type) == TRUE) nRank = 0;
			for (int n = 0; n < 5; n++) SocketOption[n] = lpMsg->SocketOption[n];
			...
			if (btRet == 0xFF) g_Log.Add("[MuunSystem][MuunExchange][ItemSerialCreateRecv][FAIL][Type:%d][Index:%d][Level:%d][Serial:%I64d][%s][%s]", ItemType, ItemIndex, Item.m_Level, Item.m_Number, lpObj->AccountID, lpObj->Name);
			else {
				BYTE NewOption[MAX_EXOPTION_SIZE];
				ItemIsBufExOption(NewOption, &Item);
				...
				if (g_CMuunSystem.IsStoneofEvolution(lpMsg->Type) == TRUE) {
					...
					if (lpItemAttribute != NULL) g_Log.Add("[%s][%s][MuunSystem][MuunExchange][ItemSerialCreateRecv][SUCCESS] Item:[%s(%s),%d,%d,%d,%d]serial:[%I64d] Dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d]", lpObj->AccountID, lpObj->Name, Item.GetName(), lpItemAttribute->Name, Item.m_Level, Item.m_Option1, Item.m_Option2, Item.m_Option3, Item.m_Number, Item.m_Durability, NewOption[0], NewOption[1], NewOption[2], NewOption[3], NewOption[4], NewOption[5], NewOption[6]);
					else g_Log.Add("[MuunSystem][Error] pItemAttribute is NULL : %s %d", __FILE__, __LINE__);
				} else g_Log.Add("[%s][%s][MuunSystem][MuunExchange][ItemSerialCreateRecv][SUCCESS] Item:[%s,%d,%d,%d,%d]serial:[%I64d] Dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d]", lpObj->AccountID, lpObj->Name, Item.GetName(), Item.m_Level, Item.m_Option1, Item.m_Option2, Item.m_Option3, Item.m_Number, Item.m_Durability, NewOption[0], NewOption[1], NewOption[2], NewOption[3], NewOption[4], NewOption[5], NewOption[6]);				
			}
		}
	} else {
		int iRetMapNumber = lpMsg->MapNumber;

		if (lpMsg->MapNumber >= 238 && lpMsg->MapNumber <= 245) mapnumber = g_BloodCastle.GetItemMapNumberFirst(iRetMapNumber);
		if (lpMsg->MapNumber >= 246 && lpMsg->MapNumber <= 253) mapnumber = g_BloodCastle.GetItemMapNumberSecond(iRetMapNumber);
		if (mapnumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL && g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE) {
			...
			if ((btAttr & 8) == 8) g_ChaosCastleFinal.GetRandomDropPos(lpMsg->x, lpMsg->y);
		}
		...
		if (g_SocketOptionSystem.IsEnableSocketItem(lpMsg->Type) == 1) {
			if (memcmp(lpMsg->SocketOption, SocketOption, 5) != 0) {
				for (int i = 0; i < 5; i++) {
					if (lpMsg->SocketOption[i] == 0xFE) SocketSlotCount++;
				}
			} else SocketSlotCount = g_SocketOptionSystem.GetMakeSocketSlotCount(lpMsg->Type);
			...
			if (memcmp(lpMsg->SocketOption, SocketOption, 5) != 0) {
				for (int i = 0; i < 5; i++) {
					if (lpMsg->SocketOption[i] == 0xFE) {
						btEnableSlot[i] = TRUE;
						SocketSlotCount++;
					} else btEnableSlot[i] = FALSE;
				}
			} else SocketSlotCount = g_PentagramSystem.GetMakePentagramSlotCountNKind(btEnableSlot, lpMsg->Type);
			...
			for (int k = 0; k < 5; k++) {
				if (btEnableSlot[k]) SocketOption[k] = 0xFE;
				else SocketOption[k] = 0xFF;
			}
			...
			SocketOptionIndex = lpMsg->MainAttribute;
			for (int l = 0; l < 5; l++) SocketOption[l] = lpMsg->SocketOption[l];
		} else {
			SocketOptionIndex = 0;
			for (int i = 0; i < 5; i++) SocketOption[i] = 0xFF;
		}

		int iItemCount = MapC[mapnumber].MonsterItemDrop(lpMsg->Type, lpMsg->Level, lpMsg->Dur, lpMsg->x, lpMsg->y, lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, lpMsg->NewOption, lpMsg->SetOption, lootindex, lpMsg->m_Number, 0, SocketOption, SocketOptionIndex, lpMsg->lDuration);
		...
		g_Log.Add("Item Drop [%d][%d][%d] : serial:%I64d [%s][%d][%d][%d][%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d]", mapnumber, lpMsg->x, lpMsg->y, lpMsg->m_Number, ItemAttribute[lpMsg->Type].Name, lpMsg->Level, lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, NewOption[0],NewOption[1],NewOption[2],NewOption[3],NewOption[4],NewOption[5],NewOption[6], lpMsg->SetOption);		
	}
}
...
void DGOptionDataRecv(SDHP_SKILLKEYDATA_SEND * lpMsg) {
	...
	gObj[aIndex].CharacterData->m_EnableUseChangeSkin = lpMsg->EnableTransformMode;
...
void DGMoveOtherServer(SDHP_CHARACTER_TRANSFER_RESULT * lpMsg) {
	...
	if (lpMsg->Result != 0)	{
		...
		g_Log.Add("[CharTrasfer] Fail [%s][%s] (%d)", lpObj->AccountID, lpObj->Name, lpMsg->Result);		
	...
	g_Log.Add("[CharTrasfer] Success [%s][%s] (%d)", lpObj->AccountID, lpObj->Name, lpMsg->Result);
...
void gObjSavePetItemInfo(int aIndex, int inventype)
{
	...
	if (founditemcount)
	{
		...
		if (g_ConfigRead.server.GetServerType() == SERVER_BATTLECORE)
		{
			pMsg.ServerCode = lpObj->CharacterData->m_nServerCodeOfHomeWorld;
		}
...
void GS_GDReqGuildMarkRegInfo(int iMapSvrGroup, int aIndex)
{
	...
	pMsg.wMapSvrNum = iMapSvrGroup;
	pMsg.iIndex = aIndex;
	memcpy(pMsg.szGuildName, gObj[aIndex].CharacterData->GuildName, 8);
...
void GS_GDReqRegAttackGuild(int iMapSvrGroup, int aIndex)
{
	...
	pMsg.wMapSvrNum = iMapSvrGroup;
	pMsg.iIndex = aIndex;
	memcpy(pMsg.szEnemyGuildName, gObj[aIndex].CharacterData->GuildName, 8);
...
void GS_GDReqRegGuildMark(int iMapSvrGroup, int aIndex, int iItemPos)
{
	...
	memcpy(&pMsg.szGuildName,gObj[aIndex].CharacterData->GuildName,sizeof(pMsg.szGuildName));
...
void GS_DGAnsCastleNpcBuy(LPBYTE lpRecv)
{
	...
	if (lpMsg->iResult == 1)
	{
		BOOL bAddResult = g_CastleSiege.CS_AddDbNPC(lpMsg->iNpcNumber,lpMsg->iNpcIndex);
	
		if (bAddResult == TRUE)
		{
			if (gObjIsConnected(lpMsg->iIndex))
			{
				gObj[lpMsg->iIndex].CharacterData->Money -= lpMsg->iBuyCost;

				if (gObj[lpMsg->iIndex].CharacterData->Money < 0)
				{
					gObj[lpMsg->iIndex].CharacterData->Money = 0;
				}

				GSProtocol.GCMoneySend(lpMsg->iIndex,gObj[lpMsg->iIndex].CharacterData->Money);
			}

			g_Log.Add("[CastleSiege] GS_DGAnsCastleNpcBuy() - CCastleSiege::CS_AddDbNPC() OK - Npc:(CLS:%d, IDX:%d)",lpMsg->iNpcNumber,lpMsg->iNpcIndex);
		}
		else
		{
			g_Log.Add("[CastleSiege] GS_DGAnsCastleNpcBuy() - CCastleSiege::CS_AddDbNPC() FAILED - Npc:(CLS:%d, IDX:%d)",lpMsg->iNpcNumber,lpMsg->iNpcIndex);
		}
	}
	else
	{
...
void GS_DGAnsCastleNpcRepair(LPBYTE lpRecv)
{
	...
	if (lpMsg->iResult == 1)
	{
		BOOL bRepairResult =  g_CastleSiege.CS_RepairDbNPC(lpMsg->iNpcNumber,lpMsg->iNpcIndex,lpMsg->iNpcHp,lpMsg->iNpcMaxHp);
		if (bRepairResult == TRUE)
		{
			if (gObjIsConnected(lpMsg->iIndex))
			{
				gObj[lpMsg->iIndex].CharacterData->Money -= lpMsg->iRepairCost;
			...
			g_Log.Add("[CastleSiege] GS_DGAnsCastleNpcRepair() - CCastleSiege::CS_RepairDbNPC() OK - Npc:(CLS:%d, IDX:%d)",lpMsg->iNpcNumber,lpMsg->iNpcIndex);
		}
		else
		{
			g_Log.Add("[CastleSiege] GS_DGAnsCastleNpcRepair() - CCastleSiege::CS_RepairDbNPC() FAILED - Npc:(CLS:%d, IDX:%d)",lpMsg->iNpcNumber,lpMsg->iNpcIndex);
		}
	}
	else
	{
...
void GS_DGAnsCastleNpcUpgrade(LPBYTE lpRecv)
{
	...
	{
		g_CastleSiege.CS_UpgradeDbNPC(lpMsg->iIndex,lpMsg->iNpcNumber,lpMsg->iNpcIndex,lpMsg->iNpcUpType,lpMsg->iNpcUpValue,lpMsg->iNpcUpIndex);
...
void GS_DGAnsCastleMoneyChange(LPBYTE lpRecv)
{
	...
	if (lpMsg->iResult == TRUE)
	{
		if (gObjIsConnected(lpMsg->iIndex))
		{
			...
			{
				gObj[lpMsg->iIndex].CharacterData->Money -= lpMsg->iMoneyChanged;

				if (gObj[lpMsg->iIndex].CharacterData->Money > MAX_ZEN)
				{
					gObj[lpMsg->iIndex].CharacterData->Money = MAX_ZEN;
				}
...
void GS_DGAnsGlobalPostMultiCast(LPBYTE lpRecv)
{
	...
	GSProtocol.PROTOCOL_DataSendAll((LPBYTE)&pMsg, sizeof(pMsg));
}
...
void GS_DGAnsCastleNpcUpdate(LPBYTE lpRecv) {
	if (g_ConfigRead.server.GetServerType() != SERVER_CASTLE) return;
	...
	if (lpMsg->iResult != FALSE) szResult = "SUCCEED";
	else szResult = "FAIL";
...
void GDReqCrywolfSync(int iMapSvrGroup, int iCrywolfState, int iOccupationState) {
	...
	pMsg.iApplyPenalty = g_CrywolfSync.CW_ApplyPenalty();
	pMsg.iPlusChaosRate = g_CrywolfSync.GetPlusChaosRate();
	pMsg.iGemDropPenaltyRate = g_CrywolfSync.GetGemDropPenaltiyRate();
	pMsg.iGettingExpPenaltyRate = g_CrywolfSync.CW_ExperiencePentaltyRate();
...
void DGAnsCrywolfSync(LPBYTE lpRecv)
{
	...
	int iOldOccupationState = g_CrywolfSync.CW_OccupationState();
...
void DGAnsInGameShopPoint(ISHOP_ANS_POINT *lpMsg)
{
	...
	gObj[lpMsg->aIndex].CharacterData->m_WCoinC = lpMsg->CoinC;
	gObj[lpMsg->aIndex].CharacterData->m_WCoinP = lpMsg->CoinP;
	gObj[lpMsg->aIndex].CharacterData->m_GoblinPoint = lpMsg->Goblin;
...
void ReqMonsterCountAns(DS_GET_MONSTERCNT * aRecv)
{
	...
	{
		gObj[aRecv->aIndex].CharacterData->IGC_KillCount = aRecv->cnt;
	}
}
...
void GDReqArcaBattleGuildJoin(OBJECTSTRUCT *obj)
{
	...
	memcpy(pMsg.szGuildName, obj->CharacterData->lpGuild->Name, MAX_GUILD_LEN+1);
	pMsg.wNumber = obj->m_Index;
	pMsg.dwGuild= obj->CharacterData->GuildNumber;
	wsDataCli.DataSend((char*) &pMsg, pMsg.h.size);
}
...
void DGAnsArcaBattleGuildJoinSelect(_tagPMSG_ANS_ARCA_BATTLE_GUILD_JOIN_DS *lpMsg)
{
	...
	if (gObjIsConnected(aIndex))
	{
		...
		{
			case 0:
				g_Log.Add("[ArcaBattle] [%s][%s][%s] ArcaBattle Guild Join",gObj[aIndex].AccountID,gObj[aIndex].Name,gObj[aIndex].CharacterData->GuildName);
...
void DGAnsArcaBattleGuildMemberJoin(_tagPMSG_ANS_ARCA_BATTLE_GUILD_MEMBER_JOIN_DS *lpMsg)
{
	...
	if (gObjIsConnected(aIndex))
	{
		...
		{
			case 0:
				g_Log.Add("[ArcaBattle] [%s][%s][%s] ArcaBattle Guild Member Join" ,gObj[aIndex].AccountID,gObj[aIndex].Name,gObj[aIndex].CharacterData->GuildName);
				return;
...
void DGAnsArcaBattleEnter(_tagPMSG_ANS_ARCA_BATTLE_ENTER_DS *lpMsg)
{
	...
	else if (lpMsg->btResult == 0)
	{
		if (lpMsg->btEnterSeq == TRUE)
		{
			...
			g_Log.Add("[ArcaBattle] [%s][%s][%s] ArcaBattle Enter", gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].CharacterData->GuildName);
			return;
		}
	}
...
void DGAns_ChaosCastle_KillPoint_Result(SDHP_ANS_KILLPOINT_RESULT_CC_UBF *lpMsg)
{
	...
	g_Log.Add("[CChaosCastle][DGAns_ChaosCastle_KillPoint_Result][%s][%s][%s] Ans Result-Return:%d / KillPoint:%d/ TotalPoint:%d",
		lpObj->AccountID, lpObj->Name, lpObj->CharacterData->m_RealNameOfUBF, lpMsg->nResult, lpMsg->nCurrentPoint, lpMsg->nTotalPoint);
}
...
void GDReqDSFCanPartyEnter(int aIndex)
{
	...
	g_Log.Add("[DSF][GDReqCanPartyEnter][%s][%s][%s][%d] DSF Enter Request",
		gObj[aIndex].AccountID, gObj[aIndex].CharacterData->m_RealNameOfUBF, 
		gObj[aIndex].Name, gObj[aIndex].CharacterData->m_nServerCodeOfHomeWorld);
...
void DG_DSF_GetReward(PMSG_ANS_GET_DSF_REWARD *lpMsg)
{
	...
	if (lpMsg->btResult == 1)
	{
		g_Log.Add("[DSF][DG_DSF_GetReward] [%s][%s][%s][%d] Can't Get Reward : Not League Winner!",
			gObj[lpMsg->nUserIndex].AccountID, gObj[lpMsg->nUserIndex].CharacterData->m_RealNameOfUBF,
			gObj[lpMsg->nUserIndex].Name, gObj[lpMsg->nUserIndex].CharacterData->m_nServerCodeOfHomeWorld);
	}
