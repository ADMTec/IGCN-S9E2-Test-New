...
#define MAX_CLOAK_LEVEL	(MAX_BLOOD_CASTLE_LEVEL+1)
...
#define BC_CLOACK_LEVEL_RANGE(x)  (  ((x)<0)?FALSE:((x)>MAX_CLOAK_LEVEL-1)?FALSE:TRUE )
...
public:
	...
	int BC_SendRewardEXP(int iIndex, int iEXP);
	int SendRewardZEN(int iIndex, int iZEN);
	...
	int BC_CalcRewardEXP(int iIndex, UINT64 iAddExp);
	...
	void SetCheatEventOpen(LONGLONG i64RemainMsec) {
		for (int i = 0; i < MAX_BLOOD_CASTLE_LEVEL; i++) this->m_BridgeData[i].m_i64_BC_REMAIN_MSEC = i64RemainMsec;
	}
...
static const int g_iBC_ChoasMixSuccessRate[MAX_BLOOD_CASTLE_LEVEL] = { 90, 90, 90, 90, 90, 90, 90, 90 };
static const int g_iBC_ChoasMixMoney[MAX_BLOOD_CASTLE_LEVEL] = { 55000 , 110000 , 220000, 440000, 770000, 1210000, 1760000, 2200000 };
