...
BOOL gJoomin15Check(char* szJN) {
	int tyear = 1900;
	char szTyear[3] = "";	
	if (szJN[6] == 51 || szJN[6] == 52) tyear = 2000;
	memcpy(&szTyear[0], szJN, sizeof(szTyear)-1);
	tyear = tyear + atoi(&szTyear[0]);
	if ((gYear - 15) < tyear) return 0;
	return 1;
}
	
BOOL gJoominCheck(char* szJN, int iLimitAge) {
	if (iLimitAge < 0) return 1;
	int tyear = 1900;
	char szTyear[3] = "";
	if (szJN[6] == 51 || szJN[6] == 52) tyear = 2000;
	memcpy(&szTyear[0], szJN, sizeof(szTyear)-1);
	tyear = tyear + atoi(&szTyear[0]);
	if ((gYear - iLimitAge) < tyear) return 0;	
	return 1;
}
...
void GameMainInit(HWND hWnd) {
	...
	Lang.Init();	
	gCheckSumSwitch=GetPrivateProfileInt("GameServerInfo","CheckSumCheck",1,g_ConfigRead.GetPath("commonserver.cfg"));
	...
	g_MasterLevelSkillTreeSystem.ML_SetExpTable();
	LoadMapFile();
			...
			if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) {
				...
				if (CRYWOLF_MAP_RANGE(gObj[result].MapNumber)) {
					if (gObj[result].Type == OBJ_NPC) {
						...
						} else if (CRYWOLF_ALTAR_CLASS_RANGE(gObj[result].Class)) {
...
void GameMonsterAllCloseAndReLoad() {
	for (int n = 0; n < g_ConfigRead.server.GetObjectMaxMonster(); n++) {
		...
		if (gObj[n].Type == OBJ_USER && gObj[n].CharacterData->ISBOT == true) {
			gObj[n].Type = OBJ_EMPTY;
			delete gObj[n].CharacterData;
			gObj[n].CharacterData = nullptr;
			gObjDel(n);
		}
	}
		...
		case 32:
			...
			for (int i = 0; i < g_ConfigRead.server.GetObjectMax(); i++) {
				if (gObjIsConnectedGP(i) == TRUE) {
					...
					if (gObj[i].CharacterData->m_bUsePartyMatching == true) {
					...
					gObj[i].CharacterData->m_nPartyMatchingIndex = -1;
					gObj[i].CharacterData->m_bUsePartyMatching = false;
					gObj[i].CharacterData->m_bPartyMatchingLeader = false;
					gObj[i].CharacterData->m_nPartyMatchingMemberCount = 0;
					gObj[i].CharacterData->m_bUseGuildMatching = false;
				}
			}
...
void ReadCommonServerInfo() {
	...
	if (!IsFile(g_ConfigRead.GetPath("commonserver.cfg"))) {
		g_Log.MsgBox("[commonserver.cfg] file not found");
	}

	g_ConfigRead.ReadConfig();
	gStalkProtocol = GetPrivateProfileInt("GameServerInfo", "StalkProtocol", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	GetPrivateProfileString("GameServerInfo", "StalkProtocolId", "0", gStalkProtocolId, 10, g_ConfigRead.GetPath("commonserver.cfg"));
	...
	gWriteChatLog = GetPrivateProfileInt("GameServerInfo", "WriteChatLog", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	...
	gItemDropPer = GetPrivateProfileInt("GameServerInfo", "ItemDropPer", 10, g_ConfigRead.GetPath("commonserver.cfg"));
	gExcDropRate = GetPrivateProfileInt("GameServerInfo", "ExcItemDropPer", 2000, g_ConfigRead.GetPath("commonserver.cfg"));
	gSkillDropRate = GetPrivateProfileInt("GameServerInfo", "ItemSkillDrop", 6, g_ConfigRead.GetPath("commonserver.cfg"));
	gLuckDropRate = GetPrivateProfileInt("GameServerInfo", "ItemLuckDrop", 4, g_ConfigRead.GetPath("commonserver.cfg"));
	gExcSkillDropRate = GetPrivateProfileInt("GameServerInfo", "ExcItemSkillDrop", 100, g_ConfigRead.GetPath("commonserver.cfg"));
	gExcLuckDropRate = GetPrivateProfileInt("GameServerInfo", "ExcItemLuckDrop", 1, g_ConfigRead.GetPath("commonserver.cfg"));
	g_Log.Add("[Option] Item drop rate : %d%%", gItemDropPer);
	g_iFenrirRepairRate = GetPrivateProfileInt("GameServerInfo", "FenrirRepairRate", 4000, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iFenrirDefaultMaxDurSmall = GetPrivateProfileInt("GameServerInfo", "FenrirDefaultMaxDurSmall", 200, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iFenrirElfMaxDurSmall = GetPrivateProfileInt("GameServerInfo", "FenrirElfMaxDurSmall", 160, g_ConfigRead.GetPath("commonserver.cfg"));
	...
	gIsItemDropRingOfTransform = GetPrivateProfileInt("GameServerInfo", "IsItemDropRingOfTransform", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	gItemDropRingOfTransform = GetPrivateProfileInt("GameServerInfo", "ItemDropRingOfTransform", 1, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iMarkOfTheLord = GetPrivateProfileInt("GameServerInfo", "MarkOfTheLord", 1, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iLuckyCoin = GetPrivateProfileInt("GameServerInfo", "LuckyCoinDrop", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	gIsDropCmd = GetPrivateProfileInt("GameServerInfo", "DropCommand", 1, g_ConfigRead.GetPath("commonserver.cfg"));
	g_bFenrirStuffItemDrop = GetPrivateProfileInt("GameServerInfo", "FenrirStuffItemDrop", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iFenrirStuff_01_DropLv_Min = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_01_DropLv_Min", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iFenrirStuff_01_DropLv_Max = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_01_DropLv_Max", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iFenrirStuff_01_DropMap = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_01_DropMap", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iFenrirStuff_01_DropRate = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_01_DropRate", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iFenrirStuff_02_DropLv_Min = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_02_DropLv_Min", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iFenrirStuff_02_DropLv_Max = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_02_DropLv_Max", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iFenrirStuff_02_DropMap = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_02_DropMap", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iFenrirStuff_02_DropRate = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_02_DropRate", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iFenrirStuff_03_DropLv_Min = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_03_DropLv_Min", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iFenrirStuff_03_DropLv_Max = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_03_DropLv_Max", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iFenrirStuff_03_DropMap = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_03_DropMap", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iFenrirStuff_03_DropRate = GetPrivateProfileInt("GameServerInfo", "FenrirStuff_03_DropRate", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iCondorFlameDropRate = GetPrivateProfileInt("GameServerInfo", "CondorFlameDropRate", 20, g_ConfigRead.GetPath("commonserver.cfg"));
	g_CompoundPotionDropOn = GetPrivateProfileInt("GameServerInfo", "CompoundPotionDropOn", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iCompoundPotionLv1DropRate = GetPrivateProfileInt("GameServerInfo", "CompoundPotionLv1DropRate", 100, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iCompoundPotionLv2DropRate = GetPrivateProfileInt("GameServerInfo", "CompoundPotionLv2DropRate", 80, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iCompoundPotionLv3DropRate = GetPrivateProfileInt("GameServerInfo", "CompoundPotionLv3DropRate", 50, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iCompoundPotionLv1DropLevel = GetPrivateProfileInt("GameServerInfo", "CompoundPotionLv1DropLevel", 68, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iCompoundPotionLv2DropLevel = GetPrivateProfileInt("GameServerInfo", "CompoundPotionLv2DropLevel", 96, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iCompoundPotionLv3DropLevel = GetPrivateProfileInt("GameServerInfo", "CompoundPotionLv3DropLevel", 118, g_ConfigRead.GetPath("commonserver.cfg"));
	g_MineSystem.LoadMiningDropScript();
	...
	gMonsterHp = GetPrivateProfileInt("GameServerInfo", "MonsterHp", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	giKundunRefillHPSec = GetPrivateProfileInt("GameServerInfo", "KundunRefillHPSec", 500, g_ConfigRead.GetPath("commonserver.cfg"));
	giKundunRefillHP = GetPrivateProfileInt("GameServerInfo", "KundunRefillHP", 10000, g_ConfigRead.GetPath("commonserver.cfg"));
	giKundunRefillHPTime = GetPrivateProfileInt("GameServerInfo", "KundunRefillHPTime", 600, g_ConfigRead.GetPath("commonserver.cfg"));
	giKundunHPLogSaveTime = GetPrivateProfileInt("GameServerInfo", "KundunHPLogSaveTime", 120, g_ConfigRead.GetPath("commonserver.cfg"));
	...
	// Skill Settings
	g_MasterLevelSkillTreeSystem.ML_Load(g_ConfigRead.GetPath("IGC_MasterSkillTree.xml"));
	...
	gWriteSkillLog = GetPrivateProfileInt("GameServerInfo", "WriteSkillLog", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	...
	gQuestNPCTeleportTime = GetPrivateProfileInt("GameServerInfo", "QuestNPCTeleportTime", 900, g_ConfigRead.GetPath("commonserver.cfg"));

	// Item Bags Settings
	gSilverBoxDropRate = GetPrivateProfileInt("GameServerInfo", "SilverBoxDropRate", 10, g_ConfigRead.GetPath("commonserver.cfg"));
	gGoldBoxDropRate = GetPrivateProfileInt("GameServerInfo", "GoldBoxDropRate", 10, g_ConfigRead.GetPath("commonserver.cfg"));
	...
	g_iFenrir_01Level_MixRate = GetPrivateProfileInt("GameServerInfo", "Fenrir_01Level_MixRate", 70, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iFenrir_02Level_MixRate = GetPrivateProfileInt("GameServerInfo", "Fenrir_02Level_MixRate", 50, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iFenrir_03Level_MixRate = GetPrivateProfileInt("GameServerInfo", "Fenrir_03Level_MixRate", 30, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iShieldPotionLv1MixSuccessRate = GetPrivateProfileInt("GameServerInfo", "ShieldPotionLv1MixSuccessRate", 50, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iShieldPotionLv1MixMoney = GetPrivateProfileInt("GameServerInfo", "ShieldPotionLv1MixMoney", 100000, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iShieldPotionLv2MixSuccessRate = GetPrivateProfileInt("GameServerInfo", "ShieldPotionLv2MixSuccessRate", 30, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iShieldPotionLv2MixMoney = GetPrivateProfileInt("GameServerInfo", "ShieldPotionLv2MixMoney", 500000, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iShieldPotionLv3MixSuccessRate = GetPrivateProfileInt("GameServerInfo", "ShieldPotionLv3MixSuccessRate", 30, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iShieldPotionLv3MixMoney = GetPrivateProfileInt("GameServerInfo", "ShieldPotionLv3MixMoney", 1000000, g_ConfigRead.GetPath("commonserver.cfg"));
	...
	gEventChipEvent = GetPrivateProfileInt("GameServerInfo", "EventChipEvent", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	gBoxOfGoldDropRate = GetPrivateProfileInt("GameServerInfo", "BoxOfGoldDropRate", 2, g_ConfigRead.GetPath("commonserver.cfg"));
	g_ItemDropRateForBoxOfGold = GetPrivateProfileInt("GameServerInfo", "ItemDropRateForBoxOfGold", 2, g_ConfigRead.GetPath("commonserver.cfg"));
	g_EventChipDropRateForBoxOfGold = GetPrivateProfileInt("GameServerInfo", "EventChipDropRateForBoxOfGold", 2, g_ConfigRead.GetPath("commonserver.cfg"));
	g_bSantaPolymorphRingDropOn = GetPrivateProfileInt("GameServerInfo", "SantaPolymorphRingDropOn", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iSantaPolymorphRingDropRate = GetPrivateProfileInt("GameServerInfo", "SantaPolymorphRingDropRate", 20, g_ConfigRead.GetPath("commonserver.cfg"));
	g_bRibbonBoxEvent = GetPrivateProfileInt("GameServerInfo", "RibbonBoxEvent", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iRedRibbonBoxDropLevelMin = GetPrivateProfileInt("GameServerInfo", "RedRibbonBoxDropLv_Min", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iRedRibbonBoxDropLevelMax = GetPrivateProfileInt("GameServerInfo", "RedRibbonBoxDropLv_Max", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iRedRibbonBoxDropRate = GetPrivateProfileInt("GameServerInfo", "RedRibbonBoxDropRate", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iGreenRibbonBoxDropLevelMin = GetPrivateProfileInt("GameServerInfo", "GreenRibbonBoxDropLv_Min", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iGreenRibbonBoxDropLevelMax = GetPrivateProfileInt("GameServerInfo", "GreenRibbonBoxDropLv_Max", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iGreenRibbonBoxDropRate = GetPrivateProfileInt("GameServerInfo", "GreenRibbonBoxDropRate", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iBlueRibbonBoxDropLevelMin = GetPrivateProfileInt("GameServerInfo", "BlueRibbonBoxDropLv_Min", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iBlueRibbonBoxDropLevelMax = GetPrivateProfileInt("GameServerInfo", "BlueRibbonBoxDropLv_Max", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iBlueRibbonBoxDropRate = GetPrivateProfileInt("GameServerInfo", "BlueRibbonBoxDropRate", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_bChocolateBoxEvent = GetPrivateProfileInt("GameServerInfo", "ChocolateEvent", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iPinkChocolateBoxDropLevelMin = GetPrivateProfileInt("GameServerInfo", "PinkChocolateBoxDropLv_Min", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iPinkChocolateBoxDropLevelMax = GetPrivateProfileInt("GameServerInfo", "PinkChocolateBoxDropLv_Max", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iPinkChocolateBoxDropRate = GetPrivateProfileInt("GameServerInfo", "PinkChocolateBoxDropRate", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iRedChocolateBoxDropLevelMin = GetPrivateProfileInt("GameServerInfo", "RedChocolateBoxDropLv_Min", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iRedChocolateBoxDropLevelMax = GetPrivateProfileInt("GameServerInfo", "RedChocolateBoxDropLv_Max", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iRedChocolateBoxDropRate = GetPrivateProfileInt("GameServerInfo", "RedChocolateBoxDropRate", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iBlueChocolateBoxDropLevelMin = GetPrivateProfileInt("GameServerInfo", "BlueChocolateBoxDropLv_Min", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iBlueChocolateBoxDropLevelMax = GetPrivateProfileInt("GameServerInfo", "BlueChocolateBoxDropLv_Max", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iBlueChocolateBoxDropRate = GetPrivateProfileInt("GameServerInfo", "BlueChocolateBoxDropRate", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_bCandyBoxEvent = GetPrivateProfileInt("GameServerInfo", "CandyBoxEvent", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iLightPurpleCandyBoxDropLevelMin = GetPrivateProfileInt("GameServerInfo", "LightPurpleCandyBoxDropLv_Min", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iLightPurpleCandyBoxDropLevelMax = GetPrivateProfileInt("GameServerInfo", "LightPurpleCandyBoxDropLv_Max", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iLightPurpleCandyBoxDropRate = GetPrivateProfileInt("GameServerInfo", "LightPurpleCandyBoxDropRate", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iVermilionCandyBoxDropLevelMin = GetPrivateProfileInt("GameServerInfo", "VermilionCandyBoxDropLv_Min", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iVermilionCandyBoxDropLevelMax = GetPrivateProfileInt("GameServerInfo", "VermilionCandyBoxDropLv_Max", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iVermilionCandyBoxDropRate = GetPrivateProfileInt("GameServerInfo", "VermilionCandyBoxDropRate", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iDeepBlueCandyBoxDropLevelMin = GetPrivateProfileInt("GameServerInfo", "DeepBlueCandyBoxDropLv_Min", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iDeepBlueCandyBoxDropLevelMax = GetPrivateProfileInt("GameServerInfo", "DeepBlueCandyBoxDropLv_Max", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iDeepBlueCandyBoxDropRate = GetPrivateProfileInt("GameServerInfo", "DeepBlueCandyBoxDropRate", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_bHallowinDayEventOn = GetPrivateProfileInt("GameServerInfo", "HallowinEventOn", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iHallowinDayEventItemDropRate = GetPrivateProfileInt("GameServerInfo", "HallowinEventPumpkinOfLuckDropRate", 100, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iHallowinDayEventJOLBlessDropRate = GetPrivateProfileInt("GameServerInfo", "HallowinEventJOLBlessDropRate", 10, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iHallowinDayEventJOLAngerDropRaTe = GetPrivateProfileInt("GameServerInfo", "HallowinEventJOLAngerDropRate", 15, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iHallowinDayEventJOLScreamDropRate = GetPrivateProfileInt("GameServerInfo", "HallowinEventJOLScreamDropRate", 15, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iHallowinDayEventJOLFoodDropRate = GetPrivateProfileInt("GameServerInfo", "HallowinEventJOLFoodDropRate", 30, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iHallowinDayEventJOLDrinkDropRate = GetPrivateProfileInt("GameServerInfo", "HallowinEventJOLDrinkDropRate", 25, g_ConfigRead.GetPath("commonserver.cfg"));
		g_iHallowinDayEventJOLPolymorphRingDropRate = GetPrivateProfileInt("GameServerInfo", "HallowinEventJOLPolymorphRingDropRate", 5, g_ConfigRead.GetPath("commonserver.cfg"));
	...
	// General Settings
	gCreateCharacter = GetPrivateProfileInt("GameServerInfo", "CreateCharacter", 1, g_ConfigRead.GetPath("commonserver.cfg"));
	gCharacterDeleteMinLevel = GetPrivateProfileInt("GameServerInfo", "CharacterDeleteMinLevel", 40, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iServerGroupGuildChatting = GetPrivateProfileInt("GameServerInfo","ServerGroupGuildChatting",0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iServerGroupUnionChatting = GetPrivateProfileInt("GameServerInfo","ServerGroupUnionChatting",0, g_ConfigRead.GetPath("commonserver.cfg"));
	gLevelUpPointNormal = GetPrivateProfileInt("GameServerInfo", "LevelUpPointNormal", 5, g_ConfigRead.GetPath("commonserver.cfg"));
	gLevelUpPointMGDL = GetPrivateProfileInt("GameServerInfo", "LevelUpPointMGDL", 7, g_ConfigRead.GetPath("commonserver.cfg"));
	bCanTrade = GetPrivateProfileInt("GameServerInfo", "Trade", 1, g_ConfigRead.GetPath("commonserver.cfg"));
	...
	gLootingTime = GetPrivateProfileInt("GameServerInfo", "LootingTime", 3, g_ConfigRead.GetPath("commonserver.cfg"));
	gItemDisapearTime = GetPrivateProfileInt("GameServerInfo", "ItemDisapearTime", 120, g_ConfigRead.GetPath("commonserver.cfg"));

	if (gLootingTime != 0) {
		g_Log.Add("[Option] Auto Looting Time Set : %d", gLootingTime);
	}

	gZenDurationTime = GetPrivateProfileInt("GameServerInfo", "ZenDurationTime", 30, g_ConfigRead.GetPath("commonserver.cfg"));
	gSetPartyExp2 = GetPrivateProfileInt("GameServerInfo", "SetParty2ExpBonus", 120, g_ConfigRead.GetPath("commonserver.cfg"));
	gSetPartyExp3 = GetPrivateProfileInt("GameServerInfo", "SetParty3ExpBonus", 230, g_ConfigRead.GetPath("commonserver.cfg"));
	gSetPartyExp4 = GetPrivateProfileInt("GameServerInfo", "SetParty4ExpBonus", 270, g_ConfigRead.GetPath("commonserver.cfg"));
	gSetPartyExp5 = GetPrivateProfileInt("GameServerInfo", "SetParty5ExpBonus", 300, g_ConfigRead.GetPath("commonserver.cfg"));
	gPartyExp1 = GetPrivateProfileInt("GameServerInfo", "NormalParty1ExpBonus", 150, g_ConfigRead.GetPath("commonserver.cfg"));
	gPartyExp2 = GetPrivateProfileInt("GameServerInfo", "NormalParty2ExpBonus", 160, g_ConfigRead.GetPath("commonserver.cfg"));
	gPartyExp3 = GetPrivateProfileInt("GameServerInfo", "NormalParty3ExpBonus", 180, g_ConfigRead.GetPath("commonserver.cfg"));
	gPartyExp4 = GetPrivateProfileInt("GameServerInfo", "NormalParty4ExpBonus", 200, g_ConfigRead.GetPath("commonserver.cfg"));
	gPartyExp5 = GetPrivateProfileInt("GameServerInfo", "NormalParty5ExpBonus", 220, g_ConfigRead.GetPath("commonserver.cfg"));
	IsMultiWareHouse = GetPrivateProfileIntA("GameServerInfo", "IsMultiWareHouse", 1, g_ConfigRead.GetPath("commonserver.cfg"));
	g_WareHouseCount = GetPrivateProfileInt("GameServerInfo", "MultiWareHouseCount", 3, g_ConfigRead.GetPath("commonserver.cfg"));
	g_MaxStrength = GetPrivateProfileInt("GameServerInfo", "MaxStrength", 65535, g_ConfigRead.GetPath("commonserver.cfg"));
	g_MaxAgility = GetPrivateProfileInt("GameServerInfo", "MaxAgility", 65535, g_ConfigRead.GetPath("commonserver.cfg"));
	g_MaxVitality = GetPrivateProfileInt("GameServerInfo", "MaxVitality", 65535, g_ConfigRead.GetPath("commonserver.cfg"));
	g_MaxEnergy = GetPrivateProfileInt("GameServerInfo", "MaxEnergy", 65535, g_ConfigRead.GetPath("commonserver.cfg"));
	g_MaxCommand = GetPrivateProfileInt("GameServerInfo", "MaxCommand", 65535, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iShadowPhantomMaxLevel = GetPrivateProfileInt("GameServerInfo", "ShadowPhantomMaxLevel", 220, g_ConfigRead.GetPath("commonserver.cfg"));
	gEnableBattleSoccer = GetPrivateProfileInt("GameServerInfo", "EnableBattleSoccer", 1, g_ConfigRead.GetPath("commonserver.cfg"));
	gEnableEventNPCTalk = GetPrivateProfileInt("GameServerInfo", "EnableEventNPCTalk", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	gEnableServerDivision = GetPrivateProfileInt("GameServerInfo", "EnableServerDivision", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_ShieldSystemOn = GetPrivateProfileInt("GameServerInfo", "ShieldSystemOn", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_bShieldComboMissOptionOn = GetPrivateProfileInt("GameServerInfo", "ShieldComboMissOptionOn", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iShieldGageConstA = GetPrivateProfileInt("GameServerInfo", "ShieldGageConstA", 12, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iShieldGageConstB = GetPrivateProfileInt("GameServerInfo", "ShieldGageConstB", 30, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iDamageDevideToSDRate = GetPrivateProfileInt("GameServerInfo", "DamageDevideToSD", 90, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iDamageDevideToHPRate = GetPrivateProfileInt("GameServerInfo", "DamageDevideToHP", 10, g_ConfigRead.GetPath("commonserver.cfg"));
	g_fSuccessAttackRateOption = (double)(GetPrivateProfileInt("GameServerInfo", "SuccessAttackRateOption", 20000, g_ConfigRead.GetPath("commonserver.cfg")) / 10000);
	g_iSDChargingOption = GetPrivateProfileInt("GameServerInfo", "SDChargingOption", 1, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iConstNumberOfShieldPoint = GetPrivateProfileInt("GameServerInfo", "ConstNumberOfShieldPoint", 20, g_ConfigRead.GetPath("commonserver.cfg"));
	g_ShieldAutoRefillOn = GetPrivateProfileInt("GameServerInfo", "ShieldAutoRefillOn", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_ShieldAutoRefillOnSafeZone = GetPrivateProfileInt("GameServerInfo", "ShieldAutoRefilOnSafeZone", 1, g_ConfigRead.GetPath("commonserver.cfg"));
	gDoPShopOpen = GetPrivateProfileInt("GameServerInfo", "PersonalShopOpen", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iUseCharacterAutoRecuperationSystem = GetPrivateProfileInt("GameServerInfo", "UseCharacterAutoRecuperationSystem", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iCharacterRecuperationMaxLevel = GetPrivateProfileInt("GameServerInfo", "CharacterRecuperationMaxLevel", 100, g_ConfigRead.GetPath("commonserver.cfg"));
	...
	// Anti-Hack Settings
	SpeedHackPlayerBlock = GetPrivateProfileInt("GameServerInfo", "SpeedHackPlayerBlock", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	gItemSerialCheck = GetPrivateProfileInt("GameServerInfo", "ItemSerialCheck", 1, g_ConfigRead.GetPath("commonserver.cfg"));
	gSerialZeroCheck = GetPrivateProfileInt("GameServerInfo", "SerialZeroCheck", 1, g_ConfigRead.GetPath("commonserver.cfg"));
	bIsIgnorePacketSpeedHackDetect = GetPrivateProfileInt("GameServerInfo", "IsIgnorePacketHackDetect", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	gDisconnectHackUser = GetPrivateProfileInt("GameServerInfo", "DisconnectHackUser", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	GetPrivateProfileString("GameServerInfo", "DecTimePerAttackSpeed", "5.33", szTemp, 5, g_ConfigRead.GetPath("commonserver.cfg"));
	gDecTimePerAttackSpeed = atof(szTemp);
	gEnableCheckPenetrationSkill = GetPrivateProfileInt("GameServerInfo", "EnableCheckPenetrationSkill", 1, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iSkillDistanceCheck = GetPrivateProfileInt("GameServerInfo", "SkillDistanceCheck", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iSkillDistanceCheckTemp = GetPrivateProfileInt("GameServerInfo", "SkillDistanceCheckTemp", 2, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iSkillDistanceKick = GetPrivateProfileInt("GameServerInfo", "SkillDistanceKick", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iSkillDistanceKickCount = GetPrivateProfileInt("GameServerInfo", "SkillDistanceKickCount", 5, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iSkillDiatanceKickCheckTime = GetPrivateProfileInt("GameServerInfo", "SkillDistanceKickCheckTime", 10, g_ConfigRead.GetPath("commonserver.cfg"));
	gAttackSpeedTimeLimit = GetPrivateProfileInt("GameServerInfo", "AttackSpeedTimeLimit", 800, g_ConfigRead.GetPath("commonserver.cfg"));
	bIsIgnorePacketSpeedHackDetect = GetPrivateProfileInt("GameServerInfo", "IsIgnorePacketHackDetect", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	gHackCheckCount = GetPrivateProfileInt("GameServerInfo", "HackCheckCount", 5, g_ConfigRead.GetPath("commonserver.cfg"));
	gMinimumAttackSpeedTime = GetPrivateProfileInt("GameServerInfo", "MinimumAttackSpeedTime", 200, g_ConfigRead.GetPath("commonserver.cfg"));
	gDetectedHackKickCount = GetPrivateProfileInt("GameServerInfo", "DetectedHackKickCount", 10, g_ConfigRead.GetPath("commonserver.cfg"));
	gIsKickDetecHackCountLimit = GetPrivateProfileInt("GameServerInfo", "IsKickDetecHackCountLimit", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	gSpeedHackPenalty = GetPrivateProfileInt("GameServerInfo", "SpeedHackPenalty", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	...
	switch (g_ConfigRead.server.GetServerType()) {
		case SERVER_NORMAL:
			strcpy(szType, "Main");
		break;

		case SERVER_CASTLE:
			strcpy(szType, "Castle Siege");
		break;

		case SERVER_ARCA:
			strcpy(szType, "Arca War");
		break;

		case SERVER_MARKET:
			strcpy(szType, "Market");
		break;

		case SERVER_BATTLECORE:
			strcpy(szType, "Battlecore");
		break;
	...
	wsprintf(szTemp, "[Server Code: %d] [Server Name: %s] [Server Type: %s] [Server Version: %s] [Client Version: %c.%c%c.%c%c] [Language Code: %d] [S/N: %s]", g_ConfigRead.server.GetGameServerCode(), g_ConfigRead.server.GetServerName(), szType, GAMESERVER_VERSION, ::szClientVersion[0], szClientVersion[1], szClientVersion[2], szClientVersion[3], szClientVersion[4], Lang.GetDefaultLangID(), szGameServerExeSerial);
...
void ReadEventInfo(MU_EVENT_TYPE eEventType) {
	...
	g_DevilSquare.DS_Load(g_ConfigRead.GetPath("\\Events\\DevilSquare\\IGC_DevilSquare_Classic.xml"));
	g_DevilSquare.DS_LoadMonster(g_ConfigRead.GetPath("\\Events\\DevilSquare\\IGC_DevilSquare_Classic_Monsters.xml"));
	...
	gIsDropDarkLordItem = GetPrivateProfileInt("GameServerInfo", "IsDropDarkLordItem", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	gSleeveOfLordDropRate = GetPrivateProfileInt("GameServerInfo", "SleeveOfLordDropRate", 10, g_ConfigRead.GetPath("commonserver.cfg"));
	gSleeveOfLordDropLevel = GetPrivateProfileInt("GameServerInfo", "SleeveOfLordDropLevel", 86, g_ConfigRead.GetPath("commonserver.cfg"));
	gSoulOfDarkHorseDropRate = GetPrivateProfileInt("GameServerInfo", "SoulOfDarkHorseDropRate", 10, g_ConfigRead.GetPath("commonserver.cfg"));
	gSoulOfDarkHorseropLevel = GetPrivateProfileInt("GameServerInfo", "SoulOfDarkHorseDropLevel", 125, g_ConfigRead.GetPath("commonserver.cfg"));
	gSoulOfDarkSpiritDropRate = GetPrivateProfileInt("GameServerInfo", "SoulOfDarkSpiritDropRate", 10, g_ConfigRead.GetPath("commonserver.cfg"));
	gSoulOfDarkSpiritDropLevel = GetPrivateProfileInt("GameServerInfo", "SoulOfDarkSpiritDropLevel", 96, g_ConfigRead.GetPath("commonserver.cfg"));
	GetPrivateProfileString("GameServerInfo", "DarkSpiritAddExperience", "1", szTemp, 10, g_ConfigRead.GetPath("commonserver.cfg"));
	gDarkSpiritAddExperience = (float)atof(szTemp);
	gIsDropGemOfDefend = GetPrivateProfileInt("GameServerInfo", "IsDropGemOfDefend", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	gGemOfDefendDropRate = GetPrivateProfileInt("GameServerInfo", "GemOfDefendDropRate", 5, g_ConfigRead.GetPath("commonserver.cfg"));
	gGemOfDefendDropLevel = GetPrivateProfileInt("GameServerInfo", "GemOfDefendDropLevel", 75, g_ConfigRead.GetPath("commonserver.cfg"));

	if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) {
		gIsDropSetItemInCastleHuntZone = GetPrivateProfileInt("GameServerInfo", "IsDropSetItemInCastleHuntZone", 0, g_ConfigRead.GetPath("commonserver.cfg"));
		gSetItemInCastleHuntZoneDropRate = GetPrivateProfileInt("GameServerInfo", "SetItemInCastleHuntZoneDropRate", 1, g_ConfigRead.GetPath("commonserver.cfg"));
		gSetItemInCastleHuntZoneDropLevel = GetPrivateProfileInt("GameServerInfo", "SetItemInCastleHuntZoneDropLevel", 75, g_ConfigRead.GetPath("commonserver.cfg"));
	}

	gWeaponAttackDamage = GetPrivateProfileInt("GameServerInfo", "CastleSiegeWeaponAttackDamage", 300, g_ConfigRead.GetPath("commonserver.cfg"));
	gWeaponDefenseDamage = GetPrivateProfileInt("GameServerInfo", "CastleSiegeWeaponDefenseDamage", 200, g_ConfigRead.GetPath("commonserver.cfg"));
	...
	gEvent1 = GetPrivateProfileInt("GameServerInfo", "EVENT1", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	gEvent1ItemDropTodayMax = GetPrivateProfileInt("GameServerInfo", "Event1ItemDropTodayMax", 1, g_ConfigRead.GetPath("commonserver.cfg"));
	gEvent1ItemDropTodayCount = GetPrivateProfileInt("GameServerInfo", "Event1ItemDropTodayCount", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	gEvent1ItemDropTodayPercent = GetPrivateProfileInt("GameServerInfo", "Event1ItemDropTodayPercent", 80, g_ConfigRead.GetPath("commonserver.cfg"));
	gFireCrackerEvent = GetPrivateProfileInt("GameServerInfo", "FireCrackerEvent", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	gFireCrackerDropRate = GetPrivateProfileInt("GameServerInfo", "FireCrackerDropRate", 5000, g_ConfigRead.GetPath("commonserver.cfg"));
	g_ItemDropRateForgFireCracker = GetPrivateProfileInt("GameServerInfo", "ItemDropRateForFireCracker", 2, g_ConfigRead.GetPath("commonserver.cfg"));
	gOnlyFireCrackerEffectUse = GetPrivateProfileInt("GameServerInfo", "OnlyFireCrackerEffectUse", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	gMedalEvent = GetPrivateProfileInt("GameServerInfo", "MedalEvent", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	gGoldMedalDropRate = GetPrivateProfileInt("GameServerInfo", "GoldMedalDropRate", 2, g_ConfigRead.GetPath("commonserver.cfg"));
	gSilverMedalDropRate = GetPrivateProfileInt("GameServerInfo", "SilverMedalDropRate", 2, g_ConfigRead.GetPath("commonserver.cfg"));
	gXMasEvent = GetPrivateProfileInt("GameServerInfo", "XMasEvent", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_XMasEvent_StarOfXMasDropRate = GetPrivateProfileInt("GameServerInfo", "XMasEvent_StarOfXMasDropRate", 5000, g_ConfigRead.GetPath("commonserver.cfg"));
	gHeartOfLoveEvent = GetPrivateProfileInt("GameServerInfo", "HeartOfLoveEvent", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	gHeartOfLoveDropRate = GetPrivateProfileInt("GameServerInfo", "HeartOfLoveDropRate", 5000, g_ConfigRead.GetPath("commonserver.cfg"));
	g_ItemDropRateForgHeartOfLove = GetPrivateProfileInt("GameServerInfo", "ItemDropRateForHeartOfLove", 2, g_ConfigRead.GetPath("commonserver.cfg"));
	gHappyNewYearNpcEvent = GetPrivateProfileInt("GameServerInfo", "HappyNewYearTalkNpc", 0, g_ConfigRead.GetPath("commonserver.cfg"));

	if (gHappyNewYearNpcEvent != 0) {
		g_Log.Add("[Option] Happy-new-year NPC speaks");
	}

	gMerryXMasNpcEvent = GetPrivateProfileInt("GameServerInfo", "MerryXMasTalkNpc", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	...
	g_iKundunMarkDropRate = GetPrivateProfileInt("GameServerInfo", "KundunMarkDropRate", 0, g_ConfigRead.GetPath("commonserver.cfg"));

	g_iMysteriousBeadDropRate1 = GetPrivateProfileInt("GameServerInfo", "MysteriouseBeadDropRate1", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iMysteriousBeadDropRate2 = GetPrivateProfileInt("GameServerInfo", "MysteriouseBeadDropRate2", 0, g_ConfigRead.GetPath("commonserver.cfg"));

	g_iHiddenTreasureBoxOfflineRate = GetPrivateProfileInt("GameServerInfo", "HiddenTreasureBoxOfflineRate", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	...
	g_iDarkLordHeartDropRate = GetPrivateProfileInt("GameServerInfo", "DarkLordHeartDropRate", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iDarkLordHeartOffEventRate = GetPrivateProfileInt("GameServerInfo", "DarkLoadHeartOffEventRate", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	...
	//Cherry Blossom
	g_bBlossomBoxDropOn = GetPrivateProfileInt("GameServerInfo", "CherryBlossomEventOn", 0, g_ConfigRead.GetPath("commonserver.cfg"));
	g_iBlossomBoxDropRate = GetPrivateProfileInt("GameServerInfo", "CherryBlossomEventItemDropRate", 50, g_ConfigRead.GetPath("commonserver.cfg"));
