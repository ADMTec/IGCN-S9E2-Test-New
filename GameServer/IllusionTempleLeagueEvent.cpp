...
BOOL CIllusionTempleLeagueEvent::Enter_ITL(int aIndex, BYTE byTempleIndex, BYTE bySlotNum)
{
	...
	if (this->m_cIllusionTempleLeagueProc[byTempleIndex].EnterUserIllusionTempleLeague(aIndex, byTempleIndex, lpObj->CharacterData->GuildNumber, lpObj->PartyNumber) == TRUE)
	{
		g_Log.Add("[ ITL ] ITL index:(%d) EnterUser: (%s)(%s) class:%d",
			byTempleIndex + 1, lpObj->AccountID, lpObj->Name, lpObj->CharacterData->DbClass);
...
BYTE CIllusionTempleLeagueEvent::Check_ITLRewardList(int nUserIndex)
{
	...
	if (gObjIsConnected(nUserIndex) == TRUE && lpObj->Type == OBJ_USER)
	{
		for (int i = 0; i < 5; i++)
		{
			if (strcmp(this->m_ITL_Winner_RewardList[i].szGuildName, lpObj->CharacterData->GuildName))
			{
				return 2;
			}
