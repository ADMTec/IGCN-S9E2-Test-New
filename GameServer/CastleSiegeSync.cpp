...
int CCastleSiegeSync::GetTaxRateChaos(int iIndex) {
	...
	if (this->CheckCastleOwnerMember(iIndex) == TRUE || this->CheckCastleOwnerUnionMember(iIndex) == TRUE) iCurTaxRateChaos = 0;
...
int CCastleSiegeSync::GetTaxRateStore(int iIndex) {
	...
	if (this->CheckCastleOwnerMember(iIndex) == TRUE || this->CheckCastleOwnerUnionMember(iIndex) == TRUE) iCurTaxRateStore = 0;
...
int CCastleSiegeSync::GetTaxHuntZone(int iIndex, BOOL bCheckOwnerGuild) {
	...
	if (bCheckOwnerGuild == TRUE) {
		if (this->CheckCastleOwnerMember(iIndex) == TRUE || this->CheckCastleOwnerUnionMember(iIndex) == TRUE) iCurTaxHuntZone = 0;
	}

	return iCurTaxHuntZone;
}
void CCastleSiegeSync::AddTributeMoney(int iMoney) {
	if (this->m_lCastleTributeMoney < 0) InterlockedExchange((LPLONG)&this->m_lCastleTributeMoney, 0);
	if (iMoney <= 0) return;
	if ((this->m_lCastleTributeMoney + iMoney) > MAX_TRIBUTE_MONEY) return;
...
void CCastleSiegeSync::AdjustTributeMoney() {
	if (this->m_lCastleTributeMoney < 0) InterlockedExchange((LPLONG)&this->m_lCastleTributeMoney, 0);
	if (this->m_lCastleTributeMoney == 0) return;

	this->m_iCsTributeMoneyTimer++;
	this->m_iCsTributeMoneyTimer %= 180;

	if (this->m_iCsTributeMoneyTimer != 0) return;
...
BOOL CCastleSiegeSync::CheckCastleOwnerMember(int iIndex) {
	if (gObjIsConnected(iIndex) == FALSE) return FALSE;
	if (strcmp(this->m_szCastleOwnerGuild, "") == FALSE) return FALSE;
	if (strcmp(gObj[iIndex].CharacterData->GuildName, this->m_szCastleOwnerGuild) != FALSE) return FALSE;
...
BOOL CCastleSiegeSync::CheckCastleOwnerUnionMember(int iIndex) {
	...
	if (lpGuildInfo == NULL) return FALSE;
	
	TUnionInfo * pUnionInfo = UnionManager.SearchUnion(lpGuildInfo->iGuildUnion);

	if (pUnionInfo == NULL) return FALSE;
	if (strcmp(pUnionInfo->m_szMasterGuild, this->m_szCastleOwnerGuild) == FALSE) return TRUE;
		...
		if (gObj[iIndex].pInventory[x].IsItem() == TRUE) {
			if (gObj[iIndex].pInventory[x].m_Type == ITEMGET(14,21)) {
				if (gObj[iIndex].pInventory[x].m_Level == 3) {
					...
					if (op >= 0 && op < IsOverlapItem(gObj[iIndex].pInventory[x].m_Type)) return x;
				}
			}
		}
	}
