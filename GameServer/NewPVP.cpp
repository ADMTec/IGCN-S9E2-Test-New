...
int CNewPVP::Reserve(OBJECTSTRUCT & requester,OBJECTSTRUCT& responsor)
{
	...
	{
		if(IsVulcanusMap(requester.MapNumber) || requester.CharacterData->Money - lpData->NeedZen < 0)	return ENEWPVP::E_NOT_ENOUGH_MONEY;
	}
	if( !IsVulcanusMap(responsor.MapNumber) )
	{
		if(IsVulcanusMap(responsor.MapNumber) || responsor.CharacterData->Money - lpData->NeedZen < 0)	return ENEWPVP::E_NOT_ENOUGH_MONEY;
	}
...
int CNewPVP::Join(OBJECTSTRUCT &requester, OBJECTSTRUCT &responsor)
{
	...
	if( !IsVulcanusMap(requester.MapNumber) )
	{
		if( requester.CharacterData->Money - lpData->NeedZen >= 0 )
		{
			requester.CharacterData->Money -= lpData->NeedZen;
			GSProtocol.GCMoneySend(requester.m_Index, requester.CharacterData->Money);
		}
	...
	if( !IsVulcanusMap(responsor.MapNumber) )
	{
		if( responsor.CharacterData->Money - lpData->NeedZen >= 0 )
		{
			responsor.CharacterData->Money -= lpData->NeedZen;
			GSProtocol.GCMoneySend(responsor.m_Index, responsor.CharacterData->Money);
		}
		else return ENEWPVP::E_NOT_ENOUGH_MONEY;
	}

	if (requester.CharacterData->m_bIsMining)
		g_MineSystem.CheckMoveMapWhileMining(requester.m_Index);
	if (responsor.CharacterData->m_bIsMining)
		g_MineSystem.CheckMoveMapWhileMining(responsor.m_Index);

	if (requester.CharacterData->m_MuBotEnable)
	{
		requester.CharacterData->m_MuBotEnable = false;
		GSProtocol.GCAnsMuBotUse(requester.m_Index, 1, requester.CharacterData->m_MuBotTotalTime, 0);
	}

	if (responsor.CharacterData->m_MuBotEnable)
	{
		responsor.CharacterData->m_MuBotEnable = false;
		GSProtocol.GCAnsMuBotUse(responsor.m_Index, 1, responsor.CharacterData->m_MuBotTotalTime, 0);
	}
...
void CNewPVP::CheckScore(OBJECTSTRUCT& obj, OBJECTSTRUCT& target)
{
	...
	if( obj.m_btDuelScore >= 10 )
    {
		...
		BUFF_AddBuffEffect(&obj, BUFFTYPE_GLORYOFGLADIATOR);
...
void CNewPVP::ReFill(OBJECTSTRUCT &obj)
{
	...
	obj.Stamina = obj.MaxStamina + obj.AddStamina;
	obj.Mana = obj.MaxMana + obj.AddMana;
	GSProtocol.PROTOCOL_ManaSend(obj.m_Index, (WORD)obj.Mana, 0xFF, 0, (WORD)obj.Stamina);

	obj.Life = obj.MaxLife + obj.AddLife;
	obj.iShield = obj.iMaxShield + obj.iAddShield;
	GSProtocol.PROTOCOL_ReFillSend(obj.m_Index, obj.Life, 0xFF, 0, obj.iShield);
}
...
void CNewPVP::SetBuff(OBJECTSTRUCT &obj, BOOL bUse)
{
	if( bUse )
	{
		BUFF_AddBuffEffect(&obj, BUFFTYPE_INVISABLE, 0, 0, 0, 0, -10);
		BUFF_AddBuffEffect(&obj, BUFFTYPE_OBSERVER, 0, 0, 0, 0, -10);
		gObjViewportListProtocolDestroy(&obj);
		return;
	}

	BUFF_RemoveBuffEffect(&obj, BUFFTYPE_INVISABLE);
	BUFF_RemoveBuffEffect(&obj, BUFFTYPE_OBSERVER);
...
{
	return BUFF_TargetHasBuff(&obj, BUFFTYPE_OBSERVER);
}
...
int CNewPVP::JoinChannel(int nId,OBJECTSTRUCT& obj)
{
	...
	if (obj.CharacterData->m_MuBotEnable == true)
	{
		obj.CharacterData->m_MuBotEnable = false;
		GSProtocol.GCAnsMuBotUse(obj.m_Index, 1, obj.CharacterData->m_MuBotTotalTime, 0);
	}
...
BOOL CNewPVP::IsGuildWar(LPOBJECTSTRUCT lpObj)
{
	if( lpObj->CharacterData->lpGuild && 
		lpObj->CharacterData->lpGuild->WarState == 1)
	{
		return TRUE;
	}
	return FALSE;
...
BOOL CNewPVP::DropItem(LPOBJECTSTRUCT lpObj, LPOBJECTSTRUCT lpMonsterObj)
{
	...
	if( !BUFF_TargetHasBuff(lpObj, BUFFTYPE_GLORYOFGLADIATOR) )	return FALSE;
	if( !BUFF_GetTotalBuffEffectValue(lpObj, EFFECTTYPE_VULCANUS_ITEMDROPRATE) )	return FALSE;
