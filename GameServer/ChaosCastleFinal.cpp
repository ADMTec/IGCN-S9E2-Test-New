				...
				else
				{
					...
					else
					{
						g_Log.Add("[Chaos Castle Survival][CCS1] [%s][%s][%s][ServerCode: %d] is Winner. [CharInfoSave : Class=%d, Level=%d, LvPoint=%d, Exp=%d, Str=%d, Dex=%d, Vit=%d, Energy=%d, LeaderShip=%d, Map=%d, Pk=%d]",
							gObj[iCCF_WINNER_INDEX].AccountID, gObj[iCCF_WINNER_INDEX].Name, gObj[iCCF_WINNER_INDEX].CharacterData->m_RealNameOfUBF,
							gObj[iCCF_WINNER_INDEX].CharacterData->m_nServerCodeOfHomeWorld, gObj[iCCF_WINNER_INDEX].CharacterData->DbClass,
							gObj[iCCF_WINNER_INDEX].Level, gObj[iCCF_WINNER_INDEX].CharacterData->LevelUpPoint, gObj[iCCF_WINNER_INDEX].CharacterData->Experience,
							gObj[iCCF_WINNER_INDEX].CharacterData->Strength, gObj[iCCF_WINNER_INDEX].CharacterData->Dexterity, gObj[iCCF_WINNER_INDEX].CharacterData->Vitality,
							gObj[iCCF_WINNER_INDEX].CharacterData->Energy, gObj[iCCF_WINNER_INDEX].Leadership, gObj[iCCF_WINNER_INDEX].MapNumber, gObj[iCCF_WINNER_INDEX].m_PK_Level);
			...
			&& gObjIsConnected(this->m_stCCFData.m_UserData[i].m_nIndex))
		{
			if (gObj[this->m_stCCFData.m_UserData[i].m_nIndex].m_nCCF_UserIndex != -1)
			{
				...
				g_Log.Add("[Chaos Castle Survival][GAME-RESULT][FAIL-CASE] NAME:[%s] UBFNAME:[%s] SERVERCODE:[%d],MOBKILL:[%d],USERKILL:[%d],POINT:[%d]",
					...
					gObj[this->m_stCCFData.m_UserData[i].m_nIndex].CharacterData->m_RealNameOfUBF,
					gObj[this->m_stCCFData.m_UserData[i].m_nIndex].CharacterData->m_nServerCodeOfHomeWorld,
...
void ChaosCastleFinal::SendCCFWinMessage(int iWinnerIndex, int nPoint, bool bFinalWinner)
{
	...
	if (gObj[iWinnerIndex].m_bCCF_Quit_Msg == false)
	{
		...
		g_Log.Add("[Chaos Castle Survival][GAME-RESULT][WIN-CASE] NAME:[%s] UBFNAME:[%s] SERVERCODE:[%d],MOBKILL:[%d],USERKILL:[%d],POINT:[%d]",
			...
			gObj[iWinnerIndex].CharacterData->m_RealNameOfUBF,
			gObj[iWinnerIndex].CharacterData->m_nServerCodeOfHomeWorld,
	...
	for (int i = 0; i < 70; i++)
	{
			...
			&& !this->m_stCCFData.m_UserData[i].m_nUserState)
		{
			...
			if (iCCF_SCORE >= iCCF_TEMP_SCORE)
			{
				if (iCCF_SCORE == iCCF_TEMP_SCORE)
				{
					int tempNewAllLevel = lpObj->CharacterData->MasterLevel + lpObj->Level;
					if (OldWinnerLevel <= tempNewAllLevel)
					{
						if (OldWinnerLevel == tempNewAllLevel)
						{
							if (Oldi64MasterLevelExp > lpObj->CharacterData->MasterExperience)
							{
								iCCF_SCORE = lpObj->m_byKillMobCount + 2 * lpObj->m_byKillUserCount;
								iCCF_WINNER_INDEX = lpObj->m_Index;
								OldWinnerLevel = lpObj->CharacterData->MasterLevel + lpObj->Level;
								*nPoint = iCCF_TEMP_SCORE;
								this->m_stCCFData.m_UserData[i].m_nScore = iCCF_TEMP_SCORE;
								Oldi64MasterLevelExp = lpObj->CharacterData->MasterExperience;
							}
						}
					}
					else
				...
				gObj[this->m_stCCFData.m_UserData[i].m_nIndex].MapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL)
			{
				if (gObj[this->m_stCCFData.m_UserData[i].m_nIndex].Life > 0.0)
				{
					...
					if (btMapAttr == 8)
					{
						...
						GSProtocol.PROTOCOL_DiePlayerSend(&gObj[this->m_stCCFData.m_UserData[i].m_nIndex],
...
void ChaosCastleFinal::RewardUserEXP(BOOL bWinner)
{
	for (int i = 0; i < 70; i++)
	{
			...
			&& gObjIsConnected(this->m_stCCFData.m_UserData[i].m_nIndex) == TRUE)
		{
			...
			int iREWARD_EXP = this->SendRewardEXP(this->m_stCCFData.m_UserData[i].m_nIndex, iTOT_EXP, iKILLCOUNT_USER, iKILLCOUNT_MONSTER);
...
int ChaosCastleFinal::SendRewardEXP(int iUserIndex, int iEXP, int iKILLCOUNT_USER, int iKILLCOUNT_MONSTER) {
	if (iEXP <= 0) return FALSE;
	if (!gObjIsConnected(iUserIndex)) return FALSE;
	...
	if (gObj[iUserIndex].Type == OBJ_USER) {
		if (Reward > 0) {
			...
			Reward = this->CalcRewardEXP(iUserIndex, Reward);
		}
...
BOOL ChaosCastleFinal::CalcRewardEXP(int iUserIndex, UINT64& iAddExp) {
	...
	CHARACTER_LevelUp(&gObj[iUserIndex], iAddExp, 0, "Chaos Castle Survival");
	return iAddExp;
}
...
void ChaosCastleFinal::CCF_Start_Fail_So_RollBack()
{
	...
	for (int i = 0; i < 70; i++)
	{
		if (this->m_stCCFData.m_UserData[i].m_nIndex != -1 && gObjIsConnected(this->m_stCCFData.m_UserData[i].m_nIndex) == TRUE && gObj[this->m_stCCFData.m_UserData[i].m_nIndex].MapNumber == MAP_INDEX_CHAOSCASTLE_SURVIVAL)
		{
			if (this->m_stCCFData.m_UserData[i].m_nIndex != -1)
			{
				int iFIRST_MONEY = gObj[this->m_stCCFData.m_UserData[i].m_nIndex].CharacterData->Money;
				int iPAYBACK_MONEY = g_nCCF_EnterFee[nCCF_Type];
				gObj[this->m_stCCFData.m_UserData[i].m_nIndex].CharacterData->Money += iPAYBACK_MONEY;
				int iNOW_MONEY = gObj[this->m_stCCFData.m_UserData[i].m_nIndex].CharacterData->Money;

				GSProtocol.GCMoneySend(this->m_stCCFData.m_UserData[i].m_nIndex, gObj[this->m_stCCFData.m_UserData[i].m_nIndex].CharacterData->Money);
...
BOOL ChaosCastleFinal::CheckUserEnterMoney(int iUserIndex, BYTE byBattleType)
{
	...
	if (gObj[iUserIndex].CharacterData->Money < nEnterFee)
	{
		g_Log.Add("[Chaos Castle Survival] %s don't have EnterCCF_Fee: %d , User Money: %d ",
			gObj[iUserIndex].Name, nEnterFee, gObj[iUserIndex].CharacterData->Money);
...
void ChaosCastleFinal::GD_Req_Save_CCF_Result(int index, char *Name, int Point, int nCCFtype)
{
	...
	pMsg.nCharExp = gObj[index].CharacterData->Experience;
...
BOOL ChaosCastleFinal::PayUserEnterMoney(int iUserIndex, int nCCFType)
{
	...
	if (gObj[iUserIndex].CharacterData->Money < g_nCCF_EnterFee[nCCFType])
	{
		return FALSE;
	}

	gObj[iUserIndex].CharacterData->Money -= g_nCCF_EnterFee[nCCFType];
...
void ChaosCastleFinal::SendAllMemberOfCCF(int index)
{
	...
	for (int i = 0; i < 70; i++)
	{
		if (this->m_stCCFData.m_UserData[i].m_nIndex != -1 && gObj[this->m_stCCFData.m_UserData[i].m_nIndex].Connected > PLAYER_LOGGED)
		{
			...
			{
				char szMsg[128];
				wsprintf(szMsg, "[CCF Entry]-> No.%d, Name:%s, Level:%d, MasterLevel:%d, Class%d", i, lpObj->Name, lpObj->Level, lpObj->CharacterData->MasterLevel, lpObj->Class);
...
void ChaosCastleFinal::GDReqSetCCFReward_UBF(int iUserIndex, BYTE btCCFType, BYTE btRewardType)
{
	...
	g_Log.Add("[UBF][ChaosCastleFinal][GDReqSetCCFWinerInformation]ACC:%s , NAME :%s,Real NAME :%s, CCFType:%d, RewardType:%d",
		lpObj->AccountID, lpObj->Name, lpObj->CharacterData->m_RealNameOfUBF, btCCFType, btRewardType);
