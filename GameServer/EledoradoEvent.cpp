...
void CEledoradoEvent::RegenGoldGoblen() {
	...
	for (int i = 0; i < It->second.m_Count; i++) {
		...
		LeaveCriticalSection(&this->m_MonCriti);
		g_Log.Add("[Eledorado Event] Respawn Golden Goblin : %d, %d,%d", MapNumber, gObj[result].X, gObj[result].Y);
	}
}
...
void CEledoradoEvent::RegenTitan() {
	...
	for (int i = 0; i < It->second.m_Count; i++) {
		...
		this->m_mapMonsterInfo.insert(std::pair<int, ELEDORADO_MONSTER_INFO>(result, m_MonsterInfo));
		LeaveCriticalSection(&this->m_MonCriti);
		g_Log.Add("[Eledorado Event] Respawn Golden Titan : %d, %d,%d", MapNumber, gObj[result].X, gObj[result].Y);
	}
...
void CEledoradoEvent::RegenGoldDercon() {
	...
	for (int i = 0; i < It->second.m_Count; i++) {
		...
		this->m_mapMonsterInfo.insert(std::pair<int, ELEDORADO_MONSTER_INFO>(result, m_MonsterInfo));
		LeaveCriticalSection(&this->m_MonCriti);
		g_Log.Add("[Eledorado Event] Respawn Golden Dragon : %d, %d,%d", MapNumber, gObj[result].X, gObj[result].Y);
