...
void CRaklionSelupan::RaklionSelupanAct_FirstSkill() {
	...
	if (iTargetIndex == -1) iTargetIndex = iSelupanIndex;
	else {
		CMagicInf cMagicInf;
...
void CRaklionSelupan::InitSelupanIndex() {
	for (int iCount = 0; iCount < g_ConfigRead.server.GetObjectMaxMonster(); iCount++) {
		if (gObj[iCount].Connected == PLAYER_PLAYING && gObj[iCount].Class == 459) this->m_iSelupanObjIndex = iCount;
	}
	if (this->m_iSelupanObjIndex == -1) g_Log.AddC(TColor::Red, "[Raklion][InitSelupanIndex] error : SelupanObjIndex(%d)", this->m_iSelupanObjIndex);
}
...
BOOL CRaklionSelupan::RegenSelupan() {
	...
	for (int i = 0; i < g_RaklionMonsterMng.m_iMaxMonsterCount; i++) {
		if (g_RaklionMonsterMng.m_SetBaseInfo[i].wType == 459) iSelupanPosNum = i;
	}
	...
	for (int n = 0; n < g_ConfigRead.server.GetObjectMax(); n++) {
		lpObjUser = &gObj[n];
		if (lpObjUser->MapNumber == MAP_INDEX_HATCHERY) gObjStateSetCreate(n);
	}
