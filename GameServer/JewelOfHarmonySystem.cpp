...
{
	JEWELOFHARMONY_ITEM_EFFECT * pItemEffect = &lpObj->CharacterData->JewelOfHarmonyEffect;
	...
	lpObj->m_AttackDamageMinLeft += pItemEffect->JoH_MinimumPhysicalDamage;
	lpObj->m_AttackDamageMinLeft += pItemEffect->JoH_PhysicalDamage;
	lpObj->m_AttackDamageMinRight += pItemEffect->JoH_MinimumPhysicalDamage;
	lpObj->m_AttackDamageMinRight += pItemEffect->JoH_PhysicalDamage;

	lpObj->m_AttackDamageMaxLeft += pItemEffect->JoH_MaximumPhysicalDamage;
	lpObj->m_AttackDamageMaxLeft += pItemEffect->JoH_PhysicalDamage;
	lpObj->m_AttackDamageMaxRight += pItemEffect->JoH_MaximumPhysicalDamage;
	lpObj->m_AttackDamageMaxRight += pItemEffect->JoH_PhysicalDamage;

	lpObj->m_MagicDamageMin += pItemEffect->JoH_SpellDamage;
	lpObj->m_MagicDamageMax += pItemEffect->JoH_SpellDamage;

	lpObj->Defense += pItemEffect->JoH_Defense;

	lpObj->AddStamina += pItemEffect->JoH_MaximumStamina;
	
	lpObj->AddLife += pItemEffect->JoH_MaximumLife;

	lpObj->EXC_ReduceDamageTaken += pItemEffect->JoH_ReduceDamageTaken;
}
		...
		case JEWELOFHARMONY_ITEM_TYPE_WEAPON:
			switch (iItemOptionType)
			{
				case AT_JEWELOFHARMONY_WEAPON_IMPROVE_MINATTACKDAMAGE:
					pItemEffect->JoH_MinimumPhysicalDamage += iItemEffectValue;
					break;
				case AT_JEWELOFHARMONY_WEAPON_IMPROVE_MAXATTACKDAMAGE:
					pItemEffect->JoH_MaximumPhysicalDamage += iItemEffectValue;
					break;
				...
				case AT_JEWELOFHARMONY_WEAPON_IMPROVE_CRITICALDAMAGE:
					pItemEffect->CriticalDamage += iItemEffectValue;
					break;
				case AT_JEWELOFHARMONY_WEAPON_IMPROVE_SKILLDAMAGE:
					pItemEffect->SkillDamage += iItemEffectValue;
					break;
				case AT_JEWELOFHARMONY_WEAPON_IMPROVE_ATTACKSUCCESRATE_PVP:
					pItemEffect->HitRatingPvP += iItemEffectValue;
					break;
				case AT_JEWELOFHARMONY_WEAPON_DECREASE_SD_RATE:
					pItemEffect->ShieldBypassRate += iItemEffectValue;
					break;
				case AT_JEWELOFHARMONY_WEAPON_IMPROVE_IGNORE_SD_RATE:
					pItemEffect->ShieldPenetrationChance += iItemEffectValue;
					break;
				default:
					bResult = FALSE;
			}
		...
		case JEWELOFHARMONY_ITEM_TYPE_STAFF:
			switch (iItemOptionType)
			{
				case AT_JEWELOFHARMONY_STAFF_IMPROVE_MAGICDAMAGE:
					pItemEffect->JoH_SpellDamage += iItemEffectValue;
					break;
		...
		case JEWELOFHARMONY_ITEM_TYPE_DEFENSE:
			switch (iItemOptionType)
			{
				case AT_JEWELOFHARMONY_DEFENSE_IMPROVE_DEFENSE:
					pItemEffect->JoH_Defense += iItemEffectValue;
					break;
				case AT_JEWELOFHARMONY_DEFENSE_IMPROVE_MAX_AG:
					pItemEffect->JoH_MaximumStamina += iItemEffectValue;
					break;
				case AT_JEWELOFHARMONY_DEFENSE_IMPROVE_MAX_HP:
					pItemEffect->JoH_MaximumLife += iItemEffectValue;
					break;
				case AT_JEWELOFHARMONY_DEFENSE_IMPROVE_AUTO_REFILL_HP:
					pItemEffect->JoH_LifeRegeneration += iItemEffectValue;
					break;
				case AT_JEWELOFHARMONY_DEFENSE_IMPROVE_AUTO_REFILL_MP:
					pItemEffect->JoH_ManaRegeneration += iItemEffectValue;
					break;
				case AT_JEWELOFHARMONY_DEFENSE_IMPROVE_BLOCKSUCCESRATE_PVP:
					pItemEffect->EvasionRatingPvP += iItemEffectValue;
					break;
				case AT_JEWELOFHARMONY_DEFENSE_IMPROVE_DAMAGE_DECREASE:
					pItemEffect->JoH_ReduceDamageTaken += iItemEffectValue;
					break;
				case AT_JEWELOFHARMONY_DEFENSE_IMPROVE_SD_RATE:
					pItemEffect->ReduceShieldDamageTaken += iItemEffectValue;
					break;
				default:
					bResult = FALSE;
			}
...
{
	pItemEffect->JoH_MinimumPhysicalDamage = 0;
	pItemEffect->JoH_MaximumPhysicalDamage = 0;
	pItemEffect->JoH_ReduceStrengthRequirement = 0;
	pItemEffect->JoH_ReduceAgilityRequirement = 0;
	pItemEffect->JoH_PhysicalDamage = 0;
	pItemEffect->CriticalDamage = 0;
	pItemEffect->SkillDamage = 0;
	pItemEffect->HitRatingPvP = 0;
	pItemEffect->ShieldBypassRate = 0;
	pItemEffect->ShieldPenetrationChance = 0;
	pItemEffect->JoH_SpellDamage = 0;
	pItemEffect->JoH_Defense = 0;
	pItemEffect->JoH_MaximumStamina = 0;
	pItemEffect->JoH_MaximumLife = 0;
	pItemEffect->JoH_LifeRegeneration = 0;
	pItemEffect->JoH_ManaRegeneration = 0;
	pItemEffect->EvasionRatingPvP = 0;
	pItemEffect->JoH_ReduceDamageTaken = 0;
	pItemEffect->ReduceShieldDamageTaken = 0;
}
...
BOOL CJewelOfHarmonySystem::MakeSmeltingStoneItem_MultiMix(OBJECTSTRUCT * lpObj, int iMixCount)
{
	...
	if (lpObj->CharacterData->Money < iMakeSmeltingStoneMixPrice)
	{
		pMsg.Result = 2;
		IOCP.DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
		lpObj->ChaosLock = FALSE;
	...
	lpObj->CharacterData->Money -= iMakeSmeltingStoneMixPrice;
	g_CastleSiegeSync.AddTributeMoney(iChaosTaxMoney);
	GSProtocol.GCMoneySend(lpObj->m_Index, lpObj->CharacterData->Money);
	...
	for (int i = 0; i < iMixCount; i++)
	{
		...
		if (iRate < JEWEL_OF_HARMONY_MAKE_SMELTINGSTONE_RATE)
		{
			...
			g_Log.Add("[JewelOfHarmony][Smelt Item Mix] Smelting Stone Normal[%d] Mass Mix (%d) Success [%s][%s], Money(%d-%d) Rate(%d/%d)",
				...
				lpObj->CharacterData->Money, iMakeSmeltingStoneMixPrice, iRate,
...
BOOL CJewelOfHarmonySystem::RestoreStrengthenItem(LPOBJ lpObj)
{
	...
	if ( lpObj->CharacterData->Money < JEWEL_OF_HARMONY_RETORE_NEEDZEN )
	{
		pMsg.Result = 2;
		IOCP.DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
		lpObj->ChaosLock = FALSE;
	...
	lpObj->CharacterData->Money -= JEWEL_OF_HARMONY_RETORE_NEEDZEN;
	g_CastleSiegeSync.AddTributeMoney(iChaosTaxMoney);
	GSProtocol.GCMoneySend(lpObj->m_Index, lpObj->CharacterData->Money);
...
BOOL CJewelOfHarmonySystem::NpcJewelOfHarmony(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	else  if ( lpNpc->Class == 369 )
	{
		...
		if ( this->m_bSystemMixSmeltingStone != TRUE )
		{
			GSProtocol.PROTOCOL_NPCChatSend(lpNpc, Lang.GetText(0,282), lpObj->m_Index);
			return TRUE;
		}
	}
	else if ( lpNpc->Class == 370 )
	{
		...
		if ( this->m_bSystemRestoreStrengthen != TRUE )
		{
			GSProtocol.PROTOCOL_NPCChatSend(lpNpc, Lang.GetText(0,283), lpObj->m_Index);
			return TRUE;
		}
	}
	else
