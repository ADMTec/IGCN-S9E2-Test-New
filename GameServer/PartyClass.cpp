...
int PartyClass::Create(int usernumber, int dbnumber, int level)
{
	...
	while ( true )
	{
		if ( this->m_PartyS[n].Count == 0 )
		{
			...
			if (gObj[usernumber].CharacterData->m_bUsePartyMatching && gObj[usernumber].CharacterData->m_bPartyMatchingLeader)
			{
				this->m_PartyS[n].bUsePartyMatching = true;
				this->m_PartyS[n].nMatchingPartyCount = gObj[usernumber].CharacterData->m_nPartyMatchingMemberCount;
			}
	...
	for ( int i = 0; i<MAX_USER_IN_PARTY ; i++ )
	{
		if ( (this->m_PartyS[party_number].Number[i] >= 0 ) && (this->m_PartyS[party_number].DbNumber[i] >= 0) )
		{
			...
			if ( gObjIsConnected(usern) != PLAYER_EMPTY )
			{
				...
				{
					memcpy(pList.Name, gObj[usern].CharacterData->m_RealNameOfUBF, MAX_ACCOUNT_LEN+1);
				}
...
void PartyClass::UpdatePKPartyPenalty(int party_number)
{
	int PKUserCount = 0;
	int usern = 0;
	for(int i = 0; i<MAX_USER_IN_PARTY; i++)
