...
void CItemOptionTypeMng::Initialize() {
	for (int iClearIndex = 0; iClearIndex < MAX_EXCELLENT_COMMON_OPTIONS; iClearIndex++) this->m_CommonExtOptionType[iClearIndex].Clear();
	for (int iClearIndex = 0; iClearIndex < MAX_EXCELLENT_WING_OPTIONS; iClearIndex++) this->m_WingExtOptionType[iClearIndex].Clear();
}
...
void CItemOptionTypeMng::Initialize_AccessoryItemOption() {
	for (int iClearIndex = 0; iClearIndex < 6; iClearIndex++) this->m_AccessoryItemOptionValue[iClearIndex].Clear();
}
...
int CItemOptionTypeMng::CommonExcOptionRand(int ItemKindA) {
	...
	for (int i = 0; i < 20; i++) {
		if (this->m_CommonExtOptionType[i].ItemKindA_1 == ItemKindA || this->m_CommonExtOptionType[i].ItemKindA_2 == ItemKindA || this->m_CommonExtOptionType[i].ItemKindA_3 == ItemKindA) {
			...
			if (iRand < OneOp + TwoOp || iRand >= OneOp + TwoOp + ThreeOp) {
				if (iRand < OneOp + TwoOp + ThreeOp || iRand >= OneOp + TwoOp + ThreeOp + FourOp) {
					if (iRand < OneOp + TwoOp + ThreeOp + FourOp || iRand >= OneOp + TwoOp + ThreeOp + FourOp + FiveOp) {
						if (iRand >= OneOp + TwoOp + ThreeOp + FourOp + FiveOp || iRand < OneOp + TwoOp + ThreeOp + FourOp + FiveOp + SixOp) iOptionCount = 6;
					} else iOptionCount = 5;
				} else iOptionCount = 4;
			} else iOptionCount = 3;
		} else iOptionCount = 2;
	...
	if (iOptionCount == 0) return 0;
	if (iOptionCount > iExcOption) iOptionCount = iExcOption;
...
void CItemOptionTypeMng::CalcExcOptionEffect(LPOBJ lpObj) {
	for (int i = 0; i < INVETORY_WEAR_SIZE; i++) {
		if (lpObj->pInventory[i].IsItem() == FALSE) continue;
		if (lpObj->pInventory[i].m_Durability == 0.0f) continue;
		if (lpObj->pInventory[i].m_IsValidItem == false) continue;
		if (lpObj->pInventory[i].IsExtItem() == FALSE) continue;

		LPITEM_ATTRIBUTE lpAttr = GetItemAttr(lpObj->pInventory[i].m_Type);

		if (lpAttr == NULL) continue;
		for (int j = 0; j < MAX_EXCELLENT_COMMON_OPTIONS; j++) {
			if (this->m_CommonExtOptionType[j].OptionID != -1) {
				if(this->m_CommonExtOptionType[j].ItemKindA_1 == lpAttr->ItemKindA || this->m_CommonExtOptionType[j].ItemKindA_2 == lpAttr->ItemKindA || this->m_CommonExtOptionType[j].ItemKindA_3 == lpAttr->ItemKindA) {
					if ((lpObj->pInventory[i].m_NewOption & this->m_CommonExtOptionType[j].OptionNumber) == this->m_CommonExtOptionType[j].OptionNumber) this->SetExcOptionEffect(lpObj, this->m_CommonExtOptionType[j].OptionID, lpAttr, i);
				}
			}
		}
	}
...
void CItemOptionTypeMng::SetExcOptionEffect(OBJECTSTRUCT *lpObj, int ExcOptionID, LPITEM_ATTRIBUTE lpItem, BYTE btItemPos) {
	...
	if (lpOption == NULL) return;
	switch (ExcOptionID) {
		case OPTION_INC_MANA_MONSTER_DIE:						lpObj->MonsterDieGetMana++;																break;
		case OPTION_INC_LIFE_MONSTER_DIE:						lpObj->MonsterDieGetLife++;																break;	
		case OPTION_INC_ATTACK_SPEED:							lpObj->AttackSpeed += lpOption->OptionValue;
																lpObj->MagicSpeed += lpOption->OptionValue;												break;
		case OPTION_INC_ATTACK_PERCENT:
			if ((lpObj->pInventory[btItemPos].m_Type >= ITEMGET(5, 0) && lpObj->pInventory[btItemPos].m_Type < ITEMGET(6, 0)) || lpObj->pInventory[btItemPos].m_Type == ITEMGET(13,12) || lpObj->pInventory[btItemPos].m_Type == ITEMGET(13,25) || lpObj->pInventory[btItemPos].m_Type == ITEMGET(13,27)) {
		...
		case OPTION_INC_ATTACK_LEVEL:
			if ((lpObj->pInventory[btItemPos].m_Type >= ITEMGET(5, 0) && lpObj->pInventory[btItemPos].m_Type < ITEMGET(6, 0)) || lpObj->pInventory[btItemPos].m_Type == ITEMGET(13,12) || lpObj->pInventory[btItemPos].m_Type == ITEMGET(13,25) || lpObj->pInventory[btItemPos].m_Type == ITEMGET(13,27)) {
				lpObj->m_MagicDamageMax += (lpObj->Level+lpObj->CharacterData->MasterLevel) / lpOption->OptionValue;
				lpObj->m_MagicDamageMin += (lpObj->Level+lpObj->CharacterData->MasterLevel) / lpOption->OptionValue;
			} else {
				if (btItemPos == 0 || btItemPos == 9) {
					lpObj->m_AttackDamageMinRight += (lpObj->Level+lpObj->CharacterData->MasterLevel) / lpOption->OptionValue;
					lpObj->m_AttackDamageMaxRight += (lpObj->Level+lpObj->CharacterData->MasterLevel) / lpOption->OptionValue;
				}
							
				if (btItemPos == 1 || btItemPos == 9) {
					lpObj->m_AttackDamageMinLeft += (lpObj->Level+lpObj->CharacterData->MasterLevel) / lpOption->OptionValue;
					lpObj->m_AttackDamageMaxLeft += (lpObj->Level+lpObj->CharacterData->MasterLevel) / lpOption->OptionValue;
				}
			}
		break;

		case OPTION_INC_EXCELLENT_DAMAGE:							lpObj->C_ExcellentStrikeChance += lpOption->OptionValue;							break;
		case OPTION_INC_ZEN_MONSTER_DIE:							lpObj->MonsterDieGetMoney += lpOption->OptionValue;									break;
		case OPTION_INC_DEFENSE_RATE:								lpObj->EvasionRating += lpObj->EvasionRating * lpOption->OptionValue / 100;			break;
		case OPTION_INC_DAMAGE_REFLECT:								lpObj->EXC_ReflectDamageTaken += lpOption->OptionValue;								break;
		case OPTION_DEC_VICTIM_DAMAGE:								lpObj->EXC_ReduceDamageTaken += lpOption->OptionValue;								break;
		case OPTION_INC_MAXIMUM_MANA:								lpObj->AddMana += (lpObj->MaxMana * lpOption->OptionValue / 100.0f);				break;
		case OPTION_INC_MAXIMUM_LIFE:								lpObj->AddLife += (lpObj->MaxLife * lpOption->OptionValue / 100.0f);				break;
	}
}
COMMON_EXT_OPTION_TYPE * CItemOptionTypeMng::GetCommonExcOption(int ExcOptionID) {
	for (int i = 0; i < MAX_EXCELLENT_COMMON_OPTIONS; i++) {
		if (this->m_CommonExtOptionType[i].OptionID == ExcOptionID) return &this->m_CommonExtOptionType[i];
	}
...
void CItemOptionTypeMng::CalcWingOptionEffect(LPOBJ lpObj) {
	if (lpObj->pInventory[7].IsItem() == FALSE) return;
	if (lpObj->pInventory[7].m_Durability == 0.0f) return;
	if (lpObj->pInventory[7].m_IsValidItem == false) return;
	if (lpObj->pInventory[7].IsExtItem() == FALSE) return;

	LPITEM_ATTRIBUTE lpAttr = GetItemAttr(lpObj->pInventory[7].m_Type);

	if (lpAttr == NULL) return;
	for (int i = 0; i < MAX_EXCELLENT_WING_OPTIONS; i++) {
		if (this->m_WingExtOptionType[i].OptionID != -1) {
			if (this->m_WingExtOptionType[i].ItemKindA == lpAttr->ItemKindA && this->m_WingExtOptionType[i].ItemKindB == lpAttr->ItemKindB) {
				if ((lpObj->pInventory[7].m_NewOption & this->m_WingExtOptionType[i].OptionNumber) == this->m_WingExtOptionType[i].OptionNumber) this->SetWingOptionEffect(lpObj, this->m_WingExtOptionType[i].OptionID, lpAttr);
			}
		}
	}
}
void CItemOptionTypeMng::SetWingOptionEffect(OBJECTSTRUCT *lpObj, int ExcOptionID, LPITEM_ATTRIBUTE lpItem) {
	...
	switch (ExcOptionID) {
		...
		case 18:
		case 23:
			lpObj->CharacterData->m_WingExcOption.iWingOpIgnoreEnemyDefense = lpOption->OptionValue;
		break;

		case 3:
			lpObj->AddStamina += lpOption->OptionValue;
		break;

		case 4:
			lpObj->AttackSpeed += lpOption->OptionValue;
			lpObj->MagicSpeed += lpOption->OptionValue;
		break;

		case 6:
		case 19:
			lpObj->CharacterData->m_WingExcOption.iWingOpReturnEnemyDamage = lpOption->OptionValue;
		break;
		...
		case 20:
		case 24:
			lpObj->CharacterData->m_WingExcOption.iWingOpRecoveryHP = lpOption->OptionValue;
		break;

		case 8:
		case 21:
			lpObj->CharacterData->m_WingExcOption.iWingOpRecoveryMana = lpOption->OptionValue;
		break;
		...
		case 22:
			lpObj->CharacterData->m_WingExcOption.iWingOpAddDoubleDamage = lpOption->OptionValue;
		break;
	}
}
WING_EXT_OPTION_TYPE * CItemOptionTypeMng::GetWingOption(int ExcOptionID) {
	for (int i = 0; i < MAX_EXCELLENT_WING_OPTIONS; i++) {
		if (this->m_WingExtOptionType[i].OptionID == ExcOptionID) return &this->m_WingExtOptionType[i];
	}
