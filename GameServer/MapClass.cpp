...
inline bool MaxItemTypeRange(int x) {
	if (x < 0 || x > g_ConfigRead.server.GetObjectMaxItem() - 1) return false;
	return true;
}
...
BOOL MapNumberCheck(int map) {
	if (map < 0 || map > MAX_NUMBER_MAP - 1) return FALSE;
	return TRUE;
}
...
void MapClass::GetLevelPos(short level, short& ox, short& oy) {
	...
	g_Log.Add("Error: Cannot find the location %s %d", __FILE__, __LINE__);
...
void MapClass::GetMapPos(short Map, short& ox, short& oy) {
	if (Map != 8) {
		if (Map == 33) Map = 33;
		else if (Map == 34) Map = 34;
		else if (Map == 41) Map = 41;
		else if (Map == 42) Map = 42;
		else if (Map == 51) Map = 51;
		else if (Map > 4 && Map != 7) Map = 0;
	}
...
void MapClass::SetWeather(BYTE a_weather, BYTE a_variation) {
	...
	for (n= g_ConfigRead.server.GetObjectStartUserIndex() ; n<g_ConfigRead.server.GetObjectMax() ; n++) {
		...
		if (lpObj->Connected > PLAYER_CONNECTED && lpObj->Live != 0 && lpObj->MapNumber == this->thisMapNumber) GSProtocol.CGWeatherSend(n, weather);
	}
}
...
int MapClass::MonsterItemDrop(int type, int level, float dur, int x, int y, BYTE Option1, BYTE Option2, BYTE Option3, BYTE NOption, BYTE SOption, int aIndex, UINT64 number, BYTE ItemEffectEx, BYTE *SocketOption, BYTE SocketBonusOption, DWORD PeriodDuration) {
	...
	while (true) {
		if (this->m_cItem[count].IsItem() == FALSE) {
			...
			if (this->m_ItemCount > g_ConfigRead.server.GetObjectMaxItem() - 1) this->m_ItemCount = 0;
			return count;
		}

		count++;
		if (count > g_ConfigRead.server.GetObjectMaxItem() - 1) count = 0;

		counttot++;
		if (counttot > g_ConfigRead.server.GetObjectMaxItem() - 1) break;
	}
...
BOOL MapClass::ItemDrop(int type, int level, float dur, int x, int y, BYTE Option1, BYTE Option2, BYTE Option3, BYTE NOption, BYTE SOption, UINT64 number, int aIndex, int PetLevel, UINT64 PetExp, BYTE ItemEffectEx, BYTE *SocketOption, BYTE SocketBonusOption, DWORD PeriodDuration) {
	...
	if ((attr & 4) == 4 || (attr & 8) == 8) return FALSE;
	count = this->m_ItemCount;
	...
	while (true) {
		...
		counttot++;
		if (counttot > g_ConfigRead.server.GetObjectMaxItem() - 1) return FALSE;
	}
...
BOOL MapClass::ItemGive(int aIndex, int item_num, bool bFailNotSend) {
	...
	if (this->m_cItem[item_num].IsItem() == FALSE) {
		g_Log.Add("Error: Item does not exist %s %d %s", __FILE__, __LINE__, gObj[aIndex].Name);
		return FALSE;
	}

	if (this->m_cItem[item_num].Give == true) return FALSE;
	if (this->m_cItem[item_num].live == false) return FALSE;
						...
						if (gObj[aIndex].PartyNumber >= 0) {
							if (gObj[aIndex].PartyNumber == gObj[this->m_cItem[item_num].m_UserIndex].PartyNumber) {
								if (BC_MAP_RANGE(gObj[aIndex].MapNumber) != FALSE) {
									if (this->m_cItem[item_num].m_Type == ITEMGET(12,15) || (this->m_cItem[item_num].m_Type == ITEMGET(13,19) && ((this->m_cItem[item_num].m_Level < 0) ? FALSE : (this->m_cItem[item_num].m_Level > 2) ? FALSE : TRUE) != FALSE)) lootresult = 0;
									else lootresult = 1;
								} else lootresult = 1;
							}
						}
...
void MapClass::StateSetDestroy() {
	...
	for (int n = 0; n < g_ConfigRead.server.GetObjectMaxItem(); n++) {
		if (this->m_cItem[n].IsItem() == TRUE) {
			if (this->m_cItem[n].m_State == 1) this->m_cItem[n].m_State = 2;
			else if (this->m_cItem[n].m_State == 8) {
				this->m_cItem[n].Clear();
				this->m_cItem[n].live = false;
				this->m_cItem[n].Give = true;
			}
		}
		...
		if (this->m_cItem[n].IsItem() == TRUE) {
			if (this->m_cItem[n].m_State != 8) {
				if (CurTime > this->m_cItem[n].m_Time) {
					...
					if (this->m_cItem[n].m_Type == ITEMGET(14, 223)) g_IT_Event.SetStatusRegenTime(this->thisMapNumber);
				}
			}
		}
	}
...
BYTE MapClass::GetAttr(int x, int y) {
	...
	if(x > 255) return 4;
	if (y > 255) return 4;
	if (x > this->m_width - 1) return 4;
	if (y > this->m_height - 1) return 4;
	return this->m_attrbuf[y * 256 + x];
}

BOOL MapClass::GetStandAttr(int x, int y) {
	if (x > (this->m_width - 1)) return FALSE;
	if (y > (this->m_height - 1)) return FALSE;
	if (x < 1 || x > 255) return FALSE;
	if (y < 1 || y > 255) return FALSE;
	BYTE attr = this->m_attrbuf[(y * 256 + x)];
	if ((attr & 2) == 2) return FALSE;
	if ((attr & 4) == 4) return FALSE;
	if ((attr & 8) == 8) return FALSE;
	return TRUE;
}
...
void MapClass::SetStandAttr(int x, int y) {
	if (x > this->m_width - 1) return;
	if (y > this->m_height - 1) return;
	this->m_attrbuf[y * 256 + x] |= 2;
}

void MapClass::ClearStandAttr(int x, int y) {
	if (((x < 0) ? FALSE : (x > (this->m_width - 1)) ? FALSE : TRUE) == FALSE) return;
	if (((y < 0) ? FALSE : (y > (this->m_height - 1)) ? FALSE : TRUE) == FALSE) return;
	if ((this->m_attrbuf[y * 256 + x] & 2) == 2) this->m_attrbuf[y * 256 + x] &= 0xFD;
}

bool MapClass::PathFinding2(int sx, int sy, int tx, int ty, PATH_t* a) {
	bool Success = this->path->FindPath(sx, sy, tx, ty, true);

	if (Success == false) Success = this->path->FindPath(sx, sy, tx, ty, false);
	if (Success != false) {
...
BOOL MapClass::CheckWall(int sx1, int sy1, int sx2, int sy2) {
	...
	do {
		if ((this->m_attrbuf[Index] & 4) == 4) return FALSE;
		error += len2;
...
bool MapClass::PathFinding3(int sx, int sy, int tx, int ty, PATH_t* a) {
	...
	if (Success == false) Success = this->path->FindPath2(sx, sy, tx, ty, FALSE);
	if (Success != false) {
...
bool MapClass::PathFinding4(int sx, int sy, int tx, int ty, PATH_t *a) {
	...
	if (Success == false) Success = this->path->FindPath3(sx, sy, tx, ty, false);
	if (Success != false) {
		int lc3 = this->path->GetPath();
...
BYTE MapClass::CheckWall2(int sx1, int sy1, int sx2, int sy2) {
	...
	do {
		if (start == 0) {
			if ((this->m_attrbuf[Index] & 2) == 2) return 2;
		} else start = 1;
		if ((this->m_attrbuf[Index] & 4) == 4) return 4;
