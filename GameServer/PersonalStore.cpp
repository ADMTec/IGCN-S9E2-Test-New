...
void CPersonalStore::CGPShopReqSetItemPrice(PMSG_REQ_PSHOP_SETITEMPRICE * lpMsg, int aIndex)
{
	...
	if (gObjCheckSerial0ItemList(sitem) != FALSE)
	{
		...
		GSProtocol.PROTOCOL_ReFillSend(aIndex, gObj[aIndex].Life, 0xFD, 1, gObj[aIndex].iShield);
...
void CPersonalStore::CGPShopReqOpen(PMSG_REQ_PSHOP_OPEN * lpMsg, int aIndex)
{
	...
	if (gObj[aIndex].CharacterData->m_bIsMining)
	{
...
void CPersonalStore::CGPShopReqBuyList(PMSG_REQ_BUYLIST_FROM_PSHOP * lpMsg, int aSourceIndex)
{
	...
	if(lpObj->CharacterData->ISBOT == true && g_BotSystem.GetBotType(lpObj->m_Index) == BOT_SHOP)
	{
		GSProtocol.GCAlterPShopVault(aSourceIndex,g_BotSystem.m_BotData[lpObj->CharacterData->wBotIndex].m_Shop.iCoinType);
	}
	...
	if (gObj[aSourceIndex].CharacterData->m_JoinUnityBattle == TRUE ||
		lpObj->CharacterData->m_JoinUnityBattle == TRUE ||
...
void CPersonalStore::CGPShopReqBuyItem(LPBYTE lpRecv, int aSourceIndex)
{
	...
	else
	{
		...
		if (gObjCheckSerial0ItemList(&lpObj->Inventory1[lpMsg->btItemPos]) != FALSE)
		{
			...
			GSProtocol.PROTOCOL_ReFillSend(aSourceIndex, gObj[aSourceIndex].Life, 0xFD, 1, gObj[aSourceIndex].iShield);
		...
		{
			GSProtocol.PROTOCOL_ReFillSend(lpObj->m_Index, lpObj->Life, 0xFD, 1, lpObj->iShield);
		...
		__try
		{
			...
			if (lpObj->Inventory1[lpMsg->btItemPos].IsItem() == TRUE)
			{
				...
				if (lpObj->m_bOff == false && lpObj->CharacterData->ISBOT == false)
				{
					if (gObj[aSourceIndex].CharacterData->Money < lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue)
					{
				...
				{
					if (!lpObj->m_bOff && lpObj->CharacterData->ISBOT == false)
					{
						if (gObj[aSourceIndex].CharacterData->Money < lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue)
						{
							g_Log.Add("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Zen",
					...
					else if(lpObj->m_bOff && lpObj->CharacterData->ISBOT == false)
					{
						...
						case 1:
						{
							if (gObj[aSourceIndex].CharacterData->m_WCoinC < lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue)
							{
								g_Log.Add("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Zen",
									gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
									lpObj->AccountID, lpObj->Name);
						...
						case 2:
						{
							if (gObj[aSourceIndex].CharacterData->m_WCoinP < lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue)
							{
								g_Log.Add("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Zen",
									gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
									lpObj->AccountID, lpObj->Name);
						...
						case 3:
						{
							if (gObj[aSourceIndex].CharacterData->m_GoblinPoint < lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue)
							{
								g_Log.Add("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Zen",
									gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
									lpObj->AccountID, lpObj->Name);
					...
					{
						switch (g_BotSystem.m_BotData[lpObj->CharacterData->wBotIndex].m_Shop.iCoinType)
						{
						case 0:
							{
								if (gObj[aSourceIndex].CharacterData->Money < lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue)
				...
				if(lpObj->CharacterData->ISBOT == false)
				{
					...
					GSProtocol.PROTOCOL_InventoryItemDeleteSend(lpObj->m_Index, lpMsg->btItemPos, TRUE);
				}
				if (lpObj->m_bOff && lpObj->CharacterData->ISBOT == false)
				{
					switch (g_ConfigRead.offtrade.CoinType)
					...
					case 0:
					{
						gObj[aSourceIndex].CharacterData->Money -= dwCost;

						lpObj->CharacterData->Money += (dwCost - round(dwCost * 0.01));
					}
						break;
					case 1:
					{
						gObj[aSourceIndex].CharacterData->m_WCoinC -= dwCost;
					...
					case 2:
					{
						gObj[aSourceIndex].CharacterData->m_WCoinP -= dwCost;
					...
					case 3:
					{
						gObj[aSourceIndex].CharacterData->m_GoblinPoint -= dwCost;
				...
				else if(lpObj->CharacterData->ISBOT == TRUE && g_BotSystem.GetBotType(lpObj->m_Index) == BOT_SHOP)
				{
					switch(g_BotSystem.m_BotData[lpObj->CharacterData->wBotIndex].m_Shop.iCoinType)
					{
						case 0:
				...
				{
					gObj[aSourceIndex].CharacterData->Money -= dwCost;
					lpObj->CharacterData->Money += (dwCost - round(dwCost * 0.01)); // tax
				}

				GSProtocol.GCMoneySend(aSourceIndex, gObj[aSourceIndex].CharacterData->Money);
				...
				if(lpObj->CharacterData->ISBOT == false && g_BotSystem.GetBotType(lpObj->m_Index) == BOT_SHOP)
				{
				...
				g_Log.Add("[PShop] [%s][%s][%s] PShop Item Buy Request Succeed : [%s][%s][%s](ZenPrice = %d(ZenCommision:%d), BJPrice = %d(BJCommision:%d), SJPrice = %d(SJCommision:%d), CJPrice = %d(CJCommision:%d), ItemType:%d(%s), ItemLevel : %d, ItemDur : %d, Serial : %u, BuyKind : %d",
					gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name, gObj[aSourceIndex].CharacterData->Ip_addr,
					lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Ip_addr, dwCost, round(dwCost * 0.01),
				...
				if(lpObj->CharacterData->ISBOT == true && g_BotSystem.GetBotType(lpObj->m_Index) == BOT_SHOP)
				{
					BotShopLog->Output("[BOT SHOP] [%s][%s][%s] Buy %s from %s for %d",gObj[aSourceIndex].CharacterData->Ip_addr, gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name, ItemAttribute[iITEM_LOG_TYPE].Name, lpObj->Name, dwCost);
				}
