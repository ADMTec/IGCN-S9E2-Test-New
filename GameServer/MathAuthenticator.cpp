...
void MathAuthenticator::Update()
{
	...
	{
		if (gObj[i].Connected == PLAYER_PLAYING && gObj[i].Type == OBJ_USER && gObj[i].CharacterData->ISBOT == false)
		{
			gObj[i].CharacterData->MathAuthenElapsed += elapsed;

			if (gObj[i].CharacterData->MathAuthenElapsed > 5000)
			{
				this->authen(i);
				gObj[i].CharacterData->MathAuthenElapsed = 0;
			}
		}
	}
}
