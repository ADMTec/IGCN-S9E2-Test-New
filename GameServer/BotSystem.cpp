...
bool CBotSystem::LoadBotSpecializationData(int aIndex, LPSTR szFile, BYTE type)
{
	...
	int iBuffCounter = 0;
	int wBotIndex = lpObj->CharacterData->wBotIndex;
	switch(type)
	{
		case BOT_BUFFER:
			{
				...
				for( pugi::xml_node buffs = BuffList.child("Buff"); buffs; buffs = buffs.next_sibling())
				{
					...
					//MAGIC_MagicAdd(lpObj,lpObj->m_BotBuffs[iBuffCounter].wBuffId ,0);
					iBuffCounter++;
...
//--- assign
	...
	lpBotObj->X = pBot.btX;
	lpBotObj->Y = pBot.btY;
	lpBotObj->CharacterData = new USER_DATA();
	...
	lpBotObj->CharacterData->ISBOT = true;
	lpBotObj->Live = TRUE;
	lpBotObj->DieRegen = 0;
	lpBotObj->m_State = 1;
	...
	lpBotObj->CharacterData->bt_BotType = 0;
	lpBotObj->CharacterData->lpGuild = NULL;
	...
	lpBotObj->Level = 400;	
	lpBotObj->CharacterData->Energy = pBot.wEnergy;
	lpBotObj->CharacterData->Dexterity = pBot.wDexterity;
	lpBotObj->CharacterData->Strength = pBot.wStrength;
	lpBotObj->CharacterData->Vitality = pBot.wVitality;
	lpBotObj->CharacterData->Mastery.Clear();
	lpBotObj->Leadership = 0;
	lpBotObj->AttackSpeed = 30;
	lpBotObj->CharacterData->m_bSecurityCheck = true;

	lpBotObj->MaxLife = DCInfo.DefClass[lpBotObj->Class].Life + ((lpBotObj->Level+lpBotObj->CharacterData->MasterLevel) - 1) * DCInfo.DefClass[lpBotObj->Class].LevelLife  + ((lpBotObj->CharacterData->Vitality - DCInfo.DefClass[lpBotObj->Class].Vitality ) * DCInfo.DefClass[lpBotObj->Class].VitalityToLife);
	lpBotObj->MaxMana = DCInfo.DefClass[lpBotObj->Class].Mana + ((lpBotObj->Level+lpBotObj->CharacterData->MasterLevel) - 1) * DCInfo.DefClass[lpBotObj->Class].LevelMana  + ((lpBotObj->CharacterData->Energy - DCInfo.DefClass[lpBotObj->Class].Energy ) * DCInfo.DefClass[lpBotObj->Class].EnergyToMana);

	lpBotObj->Life = lpBotObj->MaxLife;
	lpBotObj->Mana = lpBotObj->MaxMana;
	lpBotObj->CharacterData->wBotIndex = pBot.wID;
...
void CBotSystem::MakePreviewCharSet(int aIndex, _sBOT_INVENTORY_WEAR_ITEMS * pInv)
{
	...
	if ( lpObj->CharacterData->IsFullSetItem != false )
...
void CBotSystem::BuffPlayer(WORD  wBufferindex,short aIndex)
{
	...
	if(gObj[aIndex].CharacterData->Money < lpBot->iCoinValue)
	{
		GSProtocol.PROTOCOL_NPCChatSend(&gObj[lpBot->aIndex], Lang.GetText(0,364),aIndex);
		MsgOutput(aIndex, Lang.GetText(0,365), lpBot->iCoinValue);
		return;
	}

	if(gObj[aIndex].CharacterData->VipType == 0 && lpBot->btVipType > 0)
	{
		GSProtocol.PROTOCOL_NPCChatSend(&gObj[lpBot->aIndex], Lang.GetText(0,366), aIndex);
		return;
	}

	if (gObj[aIndex].CharacterData->VipType > 0 && gObj[aIndex].CharacterData->VipType < lpBot->btVipType)
	{
		char szTemp[256];
		sprintf(szTemp, Lang.GetText(0,636), g_VipSystem.GetVipName(lpBot->btVipType));

		GSProtocol.PROTOCOL_NPCChatSend(&gObj[lpBot->aIndex], szTemp, aIndex);
		return;
	}
	...
	for(int i=0;i<MAX_BUFFS_PER_BOT;i++)
	{
		...
		{
			/*CMagicInf * lpMagic = PLAYER_GetMagicSearch(gBotObj,gBotObj->m_BotBuffs[i].wBuffId);
			...
			BUFF_AddBuffEffect(&gObj[aIndex], gBotObj->m_BotBuffs[i].wBuffId, gBotObj->m_BotBuffs[i].wEffectType, gBotObj->m_BotBuffs[i].iEffect, 0, 0, gBotObj->m_BotBuffs[i].wDuration);
		}
	}
	GSProtocol.PROTOCOL_NPCChatSend(gBotObj, Lang.GetText(0,367), aIndex);
	gObj[aIndex].CharacterData->Money -= lpBot->iCoinValue;
...
void CBotSystem::AlchemistTradeOk(int aIndex, int botIndex)
{
	...
	if(!lpObj || !lpBot)
		return;
	sBOT_REWARD_STRUCT reward = ConfirmMixSuccess(aIndex,gObj[botIndex].CharacterData->wBotIndex);
