...
void CItemShop::Run()
{
	...
	for(int i=g_ConfigRead.server.GetObjectStartUserIndex();i<g_ConfigRead.server.GetObjectMax();i++)
	{
		...
		{
			if((GetTickCount() - gObj[i].CharacterData->m_GoblinTime) >= (this->GoblinTime * 1000))
			{
				GDReqInGameShopPointAdd(i, this->GoblinCoinType, this->GoblinAddCoin);
				gObj[i].CharacterData->m_GoblinTime = GetTickCount();
			}
		}
	}
}
...
BOOL CItemShop::AddUser(LPOBJ lpObj)
{
	...
	lpObj->CharacterData->m_GoblinTime = GetTickCount();
...
void CItemShop::GCCashPoint(LPOBJ lpObj)
{
	...
	pMsg.btViewType = 0;
	pMsg.GoblinPoint = lpObj->CharacterData->m_GoblinPoint;
	pMsg.dwCash_C = lpObj->CharacterData->m_WCoinC;
...
void CItemShop::GCCashInventoryItemUse(LPOBJ lpObj, int Result, int UniqueCode, int AuthCode, int ID1, int ID2, int ID3)
{
	...
	{
		if (lpObj->CharacterData->VipType == 0 && Iter->second.btItemLevel == 0)
		{
			GDReqInGameShopItemRollbackUse(lpObj->m_Index, UniqueCode, AuthCode);
		...
		if (lpObj->CharacterData->VipType != 0 && lpObj->CharacterData->VipType > Iter->second.btItemLevel)
		{
			GDReqInGameShopItemRollbackUse(lpObj->m_Index, UniqueCode, AuthCode);
			pMsg.result = CS_USE_ITEM_TICKET_ERROR;
			IOCP.DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
...
void CItemShop::CGCashItemBuy(LPOBJ lpObj, PMSG_REQ_INGAMESHOP_ITEMBUY *lpMsg)
{
	...
	{
		case 0:
			UserCoin = lpObj->CharacterData->m_WCoinC;
			break;
		case 1:
			UserCoin = lpObj->CharacterData->m_WCoinP;
			break;
		case 2:
			UserCoin = lpObj->CharacterData->m_GoblinPoint;
			break;
		default:
...
bool CItemShop::CheckBuyCondition(LPOBJ lpObj, int ItemCode)
{
	...
	if(ItemCode == ITEMGET(14,162) && lpObj->CharacterData->m_InventoryExpansion == 2)
		return false;

	if(ItemCode == ITEMGET(14,163) && lpObj->CharacterData->m_WarehouseExpansion == 1)
		return false;
