...
int CBag::GetDropSection(int aIndex, BAG_SECTION_DROP &pRetDrop)
{
	...
	for (std::vector<BAG_SECTION_DROP>::iterator It = this->m_BagData.m_vtSectionDrop.begin(); It != this->m_BagData.m_vtSectionDrop.end(); It++, SectionID++)
	{
		...
		{
			if (It->btUseSectionAllowedClass[lpObj->Class] > (lpObj->CharacterData->ChangeUP + 1))
			{
				continue;
			}
		}

		if ((lpObj->Level + lpObj->CharacterData->MasterLevel) < It->wUseSectionMinLevel)
		{
			continue;
		}

		if ((lpObj->Level + lpObj->CharacterData->MasterLevel) > It->wUseSectionMaxLevel)
		{
			continue;
		}
