...
BOOL CQuestInfo::LoadQuestInfo(char * filename) {
	...
	for (pugi::xml_node quest_info = main.child("QuestInfo"); quest_info; quest_info = quest_info.next_sibling()) {
		...
		g_Log.Add("[Quest] (%s) Load : Index %d, SubInfoCount %d, ConditionCount %d", this->QuestInfo[iQuestIndex].Name, iQuestIndex, this->QuestInfo[iQuestIndex].QuestSubInfoCount, this->QuestInfo[iQuestIndex].QuestConditionCount);
		this->m_QuestCount++;
	}
...
int CQuestInfo::GetQuestState(LPOBJ lpObj, int QuestIndex) {
	...
	return (lpObj->CharacterData->m_Quest[index] >> shift) &3;
}
...
BYTE CQuestInfo::GetQuestStateBYTE(LPOBJ lpObj, int QuestIndex) {
	...
	int index = (QuestIndex / 4);
	return lpObj->CharacterData->m_Quest[index];
}
...
BYTE CQuestInfo::SetQuestState(LPOBJ lpObj, int QuestIndex, int State)
{
	...
	int index = (QuestIndex / 4);
	int shift =  (QuestIndex % 4) *2;
	lpObj->CharacterData->m_Quest[index] &= QuestBitMask[shift];
	lpObj->CharacterData->m_Quest[index] |= (State&3) << shift;
	return 0;
}
...
LPQUEST_SUB_INFO CQuestInfo::GetSubquestInfo(LPOBJ lpObj, LPQUEST_INFO lpQuestInfo, int subquestindex)
{
	...
	int ChangeUP = lpObj->CharacterData->ChangeUP;
...
int CQuestInfo::GetQuestKillCount(LPOBJ lpObj, int uClass)
{
	...
	{
		return lpObj->CharacterData->m_Quest[(uClass%100)-5];
	}
	else
	{
		return 0;
...
BOOL CQuestInfo::CompareCondition(LPOBJ lpObj, LPQUEST_CONDITION lpCondition)
{
	...
	if (lpCondition->NeedZen > lpObj->CharacterData->Money)
	{
		return FALSE;
	}

	if (lpCondition->ReqStr > lpObj->CharacterData->Strength + lpObj->AddStrength)
	{
		return FALSE;
	}
						...
						if (lpSubInfo != NULL)
						{
							if (lpSubInfo->QuestType == 2)
							{
								if (lpObj->Class == lpSubInfo->NeedType && this->GetQuestState(lpPartyObj, lpQuestInfo->QuestIndex) == TRUE)
								{
									...
									{
										case 409:
											if(lpPartyObj->CharacterData->m_Quest[20] < lpSubInfo->NeedNumber)
											{
												lpPartyObj->CharacterData->m_Quest[20]++;
												if(g_ConfigRead.data.common.ThirdQuestCounter)
												{
													MsgOutput(lpPartyObj->m_Index, Lang.GetText(0,540), lpPartyObj->CharacterData->m_Quest[20], lpSubInfo->NeedNumber);
												}
											}
											break;
										case 410:
											if(lpPartyObj->CharacterData->m_Quest[21] < lpSubInfo->NeedNumber)
											{
												lpPartyObj->CharacterData->m_Quest[21]++;
												if(g_ConfigRead.data.common.ThirdQuestCounter)
												{
													MsgOutput(lpPartyObj->m_Index, Lang.GetText(0,541), lpPartyObj->CharacterData->m_Quest[21], lpSubInfo->NeedNumber);
												}
											}
											break;
										case 411:
											if(lpPartyObj->CharacterData->m_Quest[22] < lpSubInfo->NeedNumber)
											{
												lpPartyObj->CharacterData->m_Quest[22]++;
												if(g_ConfigRead.data.common.ThirdQuestCounter)
												{
													MsgOutput(lpPartyObj->m_Index, Lang.GetText(0,542), lpPartyObj->CharacterData->m_Quest[22], lpSubInfo->NeedNumber);
												}
											}
											break;
										case 412:
											if(lpPartyObj->CharacterData->m_Quest[24] < lpSubInfo->NeedNumber)
											{
												lpPartyObj->CharacterData->m_Quest[24]++;
												if(g_ConfigRead.data.common.ThirdQuestCounter)
												{
													MsgOutput(lpPartyObj->m_Index, Lang.GetText(0,543), lpPartyObj->CharacterData->m_Quest[24], lpSubInfo->NeedNumber);
												}
											}
											break;
									}
				...
				if (lpSubInfo != NULL)
				{
					if (lpSubInfo->QuestType == 2)
					{
						...
						if (lpObj->Class == lpSubInfo->NeedType && this->GetQuestState(lpTarget, lpQuestInfo->QuestIndex) == TRUE)
						{
							...
							{
								case 409:
									if(lpTarget->CharacterData->m_Quest[20] < lpSubInfo->NeedNumber)
									{
										lpTarget->CharacterData->m_Quest[20]++;
										if(g_ConfigRead.data.common.ThirdQuestCounter)
										{
											MsgOutput(lpTarget->m_Index, Lang.GetText(0,540), lpTarget->CharacterData->m_Quest[20], lpSubInfo->NeedNumber);
										}
									...
									break;
									case 410:
										if(lpTarget->CharacterData->m_Quest[21] < lpSubInfo->NeedNumber)
										{
											lpTarget->CharacterData->m_Quest[21]++;
											if(g_ConfigRead.data.common.ThirdQuestCounter)
											{
												MsgOutput(lpTarget->m_Index, Lang.GetText(0,541), lpTarget->CharacterData->m_Quest[21], lpSubInfo->NeedNumber);
											}
										...
										break;
									case 411:
										if(lpTarget->CharacterData->m_Quest[22] < lpSubInfo->NeedNumber)
										{
											lpTarget->CharacterData->m_Quest[22]++;
											if(g_ConfigRead.data.common.ThirdQuestCounter)
											{
												MsgOutput(lpTarget->m_Index, Lang.GetText(0,542), lpTarget->CharacterData->m_Quest[22], lpSubInfo->NeedNumber);
											}
										...
										break;
									case 412:
										if(lpTarget->CharacterData->m_Quest[24] < lpSubInfo->NeedNumber)
										{
											lpTarget->CharacterData->m_Quest[24]++;
											if(g_ConfigRead.data.common.ThirdQuestCounter)
											{
												MsgOutput(lpTarget->m_Index, Lang.GetText(0,543), lpTarget->CharacterData->m_Quest[24], lpSubInfo->NeedNumber);
											}
...
BOOL CQuestInfo::QuestAccept(LPOBJ lpObj, int QuestIndex)
{
	...
	if (QuestIndex > 3 && QuestIndex < 6) //Third Quest Fix
	{
		...
		{
			lpObj->CharacterData->m_Quest[19] = 0;
			lpObj->CharacterData->m_Quest[20] = 0;
			lpObj->CharacterData->m_Quest[21] = 0;
			lpObj->CharacterData->m_Quest[22] = 0;
			lpObj->CharacterData->m_Quest[23] = 0;
			lpObj->CharacterData->m_Quest[24] = 0;
	...
	lpObj->CharacterData->Money -= NeedZen;
...
BOOL CQuestInfo::QuestClear(LPOBJ lpObj, int QuestIndex)
{
	...
	for (int subquest=0;subquest<subquestcount;subquest++)
	{
		...
		if (lpSubInfo != NULL)
		{
			...
			{
				lpObj->CharacterData->LevelUpPoint += lpSubInfo->RewardCount;
				GSProtocol.GCSendQuestPrize(lpObj->m_Index, QUEST_COMPENSATION_STATUP, lpSubInfo->RewardCount);
				lpObj->CharacterData->ChangeUP = true;
				lpObj->CharacterData->DbClass |= 1;
				gObjMakePreviewCharSet(lpObj->m_Index);
				BYTE btClass = (lpObj->Class * 32) & 224 ;
				btClass |= (lpObj->CharacterData->ChangeUP * 16) & 16;
				...
				g_Log.Add("[Quest] Quest Clear (%s) : [%s][%s] Stat(%d,%d), ChangeUp",
					lpQuestInfo->Name, lpObj->AccountID, lpObj->Name, lpObj->CharacterData->LevelUpPoint,
			...
			else if (lpSubInfo->RewardType == QUEST_COMPENSATION_PLUSSSTAT)
			{
				...
				lpObj->CharacterData->LevelUpPoint += level;
				lpObj->CharacterData->PlusStatQuestClear = true;
			...
			else if (lpSubInfo->RewardType == QUEST_COMPENSATION_COMBOSKILL)
			{
				...
				lpObj->CharacterData->ComboSkillquestClear = true;
			...
			else if (lpSubInfo->RewardType == QUEST_COMPENSATION_CHANGEUP2)
			{
				lpObj->CharacterData->LevelUpPoint += lpSubInfo->RewardCount;
				GSProtocol.GCSendQuestPrize(lpObj->m_Index, QUEST_COMPENSATION_STATUP, lpSubInfo->RewardCount);
				lpObj->CharacterData->ChangeUP = 2;
				g_MasterLevelSkillTreeSystem.InitData(lpObj);

				BYTE btClass = (lpObj->Class * 32) + 24;

				if (lpObj->CharacterData->DbClass != 2 && lpObj->CharacterData->DbClass != 18 && lpObj->CharacterData->DbClass != 34 && lpObj->CharacterData->DbClass != 50 && lpObj->CharacterData->DbClass != 66 && lpObj->CharacterData->DbClass != 82 && lpObj->CharacterData->DbClass != 98)
				{
					if(lpObj->CharacterData->DbClass == 48 || lpObj->CharacterData->DbClass == 64 || lpObj->CharacterData->DbClass == 96)
					{
						lpObj->CharacterData->DbClass += 2;
					}
					else
					{
						lpObj->CharacterData->DbClass += 1;
				...
				g_Log.Add("[Quest] Quest Clear (%s) : [%s][%s] Class:%d(%d), 3rd ChangeUp",
					lpQuestInfo->Name, lpObj->AccountID, lpObj->Name, lpObj->CharacterData->DbClass, lpObj->CharacterData->DbClass-1);
				...
				g_Log.Add("[Quest] Quest Clear (%s) : [%s][%s] Stat(%d,%d), Class:%d PlusStat",
					lpQuestInfo->Name, lpObj->AccountID, lpObj->Name, lpObj->CharacterData->LevelUpPoint, lpSubInfo->RewardCount, lpObj->CharacterData->DbClass);
			}
...
void CQuestInfo::QuestInfoSave(LPOBJ lpObj)
{
	...
	if (iQuestState_1 == 1 || iQuestState_2 == 1)
	{
		g_Log.Add("[Quest] Save MonsterKillInfo [%s][%s] (%d, %d, %d, %d)", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->m_Quest[20], lpObj->CharacterData->m_Quest[21], lpObj->CharacterData->m_Quest[22], lpObj->CharacterData->m_Quest[24]);
	}
}
...
void CQuestInfo::SendQuestMonsterKill(LPOBJ lpObj, int iQuestIndex)
{
	...
	if(lpObj->CharacterData->m_Quest[20] == 255)
	{
		lpObj->CharacterData->m_Quest[20] = 0;
	}

	if(lpObj->CharacterData->m_Quest[21] == 255)
	{
		lpObj->CharacterData->m_Quest[21] = 0;
	}

	if(lpObj->CharacterData->m_Quest[22] == 255)
	{
		lpObj->CharacterData->m_Quest[22] = 0;
	}

	if(lpObj->CharacterData->m_Quest[24] == 255)
	{
		lpObj->CharacterData->m_Quest[24] = 0;
	}
	...
	for (int subquest = 0; subquest < subquestcount; subquest++)
	{
		...
		if (lpSubInfo && lpSubInfo->QuestType == 2)
		{
			...
			{
				if (lpObj->CharacterData->m_Quest[20] > lpSubInfo->NeedNumber)
				{
					lpObj->CharacterData->m_Quest[20] = lpSubInfo->NeedNumber;
				}

				pMsg.iKillCountInfo[iSendCount] = 409;
				iSendCount++;
				pMsg.iKillCountInfo[iSendCount] = lpObj->CharacterData->m_Quest[20];
				iSendCount++;
			}

			else if (lpSubInfo->NeedType == 410)
			{
				if (lpObj->CharacterData->m_Quest[21] > lpSubInfo->NeedNumber)
				{
					lpObj->CharacterData->m_Quest[21] = lpSubInfo->NeedNumber;
				}

				pMsg.iKillCountInfo[iSendCount] = 410;
				iSendCount++;
				pMsg.iKillCountInfo[iSendCount] = lpObj->CharacterData->m_Quest[21];
				iSendCount++;
			}

			else if (lpSubInfo->NeedType == 411)
			{
				if (lpObj->CharacterData->m_Quest[22] > lpSubInfo->NeedNumber)
				{
					lpObj->CharacterData->m_Quest[22] = lpSubInfo->NeedNumber;
				}

				pMsg.iKillCountInfo[iSendCount] = 411;
				iSendCount++;
				pMsg.iKillCountInfo[iSendCount] = lpObj->CharacterData->m_Quest[22];
				iSendCount++;
			}

			else if (lpSubInfo->NeedType == 412)
			{
				if (lpObj->CharacterData->m_Quest[24] > lpSubInfo->NeedNumber)
				{
					lpObj->CharacterData->m_Quest[24] = lpSubInfo->NeedNumber;
				}

				pMsg.iKillCountInfo[iSendCount] = 412;
				iSendCount++;
				pMsg.iKillCountInfo[iSendCount] = lpObj->CharacterData->m_Quest[24];
				iSendCount++;
			}
		}
	}
...
void CQuestInfo::CheckQuestMapEnterOnWerwolf(int iIndex)
{
	...
	if (CHARACTER_CalcDistance(lpObj, lpNpc) > 5)
	{
		IOCP.DataSend(iIndex, (LPBYTE)&pMsg, pMsg.h.size);
		return;
	}
	...
	if (lpObj->CharacterData->Money < 3000000)
	{
		IOCP.DataSend(iIndex, (LPBYTE)&pMsg, pMsg.h.size);
		return;
	}
		...
		for (int i = 0; i < MAX_USER_IN_PARTY; i++)
		{
			...
			if (iPartyUserIndex >= 0 && iPartyUserIndex != iIndex)
			{
				...
				if (lpPartyObj->Level >= 350 && 
					...
					CHARACTER_CalcDistance(lpObj, lpPartyObj) < 10 && 
	...
	{
		lpObj->CharacterData->Money -= 3000000;
		GSProtocol.GCMoneySend(lpObj->m_Index, lpObj->CharacterData->Money);
	}
