...
CCrywolf::CCrywolf() {
	...
	memset(this->m_iMonsterGroupNumberArray, 0, sizeof(this->m_iMonsterGroupNumberArray));
}
...
BOOL CCrywolf::LoadData(LPSTR lpszFileName) {
	...
	if (g_ConfigRead.server.GetStateFromEventTable(g_ConfigRead.server.GetServerType(), EV_TABLE_CW) == false) bEnable = false;
...
void CCrywolf::DelAllData() {
	for (int i = 0; i < MAX_CRYWOLF_STATE; i++) this->m_StateTimeInfo[i].Reset();
	for (int j = 0; j < MAX_CRYWOLF_STATE_TIME; j++) this->m_StartTimeInfo[j].Reset();

	this->m_StartTimeInfoCount = 0;

	for (int k = 0; k < MAX_CRYWOLF_MONSTER_GROUP; k++) this->m_iMonsterGroupNumberArray[k] = -1;
...
void CCrywolf::Init() {
	if (g_ConfigRead.server.GetStateFromEventTable(g_ConfigRead.server.GetServerType(), EV_TABLE_CW) == false) g_CrywolfSync.SetEnableCrywolf(false);
	return;
}

void CCrywolf::LoadCrywolfMapAttr(LPSTR lpszFileName, int iOccupationState) {
	if (!lpszFileName) return;
	if (iOccupationState < 0 || iOccupationState >= 3) return;
...
void CCrywolf::SetCrywolfCommonNPC(int iOccupationState) {
	for (int i = 0; i < this->m_ObjCommonNPC.m_iObjCount; i++) {
		if (!gObjIsConnected(this->m_ObjCommonNPC.m_iObjIndex[i])) break;

		LPOBJ lpObj = &gObj[this->m_ObjCommonNPC.m_iObjIndex[i]];

		if (iOccupationState == 2) BUFF_AddBuffEffect(&gObj[this->m_ObjCommonNPC.m_iObjIndex[i]], 27, 0, 0, 0, 0, -10);
		else if (iOccupationState == 0) BUFF_RemoveBuffEffect(&gObj[this->m_ObjCommonNPC.m_iObjIndex[i]], 27);
		else if (iOccupationState == 1) BUFF_AddBuffEffect(&gObj[this->m_ObjCommonNPC.m_iObjIndex[i]], 27, 0, 0, 0, 0, -10);
	}
}
...
void CCrywolf::SetCrywolfAllCommonMonsterState2(int iMonsterState, int iMode) {
	for (int i = 0; i < this->m_ObjCommonMonster.m_iObjCount; i++) {
		if (!gObjIsConnected(this->m_ObjCommonMonster.m_iObjIndex[i])) break;

		LPOBJ lpObj = &gObj[this->m_ObjCommonMonster.m_iObjIndex[i]];

		if (iMode == 0) {
			//lpObj->m_ViewSkillState &= ~iMonsterState;
			//PROTOCOL_StateInfoSend(lpObj, 0, lpObj->m_ViewSkillState);
		} else if (iMode == 1) {
			//lpObj->m_ViewSkillState |= iMonsterState;
			//PROTOCOL_StateInfoSend(lpObj, 1, lpObj->m_ViewSkillState);
		}
	}
}

void CCrywolf::RemoveCrywolfCommonMonster() {
	for (int i = 0; i < this->m_ObjCommonMonster.m_iObjCount; i++) gObjDel(this->m_ObjCommonMonster.m_iObjIndex[i]);
...
void CCrywolf::CreateCrywolfCommonMonster() {
	for (int n = 0; n < gMSetBase.m_Count; n++) {
		if (CHECK_CLASS(gMSetBase.m_Mp[n].m_MapNumber, MAP_INDEX_CRYWOLF_FIRSTZONE)) {
			if (gMSetBase.m_Mp[n].m_Type >= 204 && gMSetBase.m_Mp[n].m_Type <= 257) continue;
			...
			if (iIndex >= 0) {
				...
				if (gObj[iIndex].Type == OBJ_MONSTER) g_Crywolf.m_ObjCommonMonster.AddObj(iIndex);
			}
		}
	}
}
...
void CCrywolf::ChangeAI(int iAIOrder) {
	for (int n = 0; n < this->m_iMonsterGroupNumberCount; n++) TMonsterAIGroup::ChangeAIOrder(this->m_iMonsterGroupNumberArray[n], iAIOrder);
}
...
int CCrywolf::CheckStateTimeSync() {
	if (this->m_iCrywolfState != CRYWOLF_STATE_NONE) {
		if (this->m_StateTimeInfo[this->m_iCrywolfState].m_bUsed == FALSE) return -1;
		...
		if (this->m_StateTimeInfo[this->m_iCrywolfState].CheckContinuanceTime() == FALSE) {
			...
			if (this->m_iCrywolfState == CRYWOLF_STATE_ENDCYCLE) {
				if (this->m_iScheduleMode == 1) this->SetState(CRYWOLF_STATE_NOTIFY_1);
				else this->SetState(CRYWOLF_STATE_NONE);
			} else this->SetNextState(this->m_iCrywolfState);
		}
...
void CCrywolf::Run() {
	if (!g_CrywolfSync.CheckEnableCrywolf()) return;
	if (this->m_StartTimeInfo[0].m_bUsed == FALSE) return;
...
void CCrywolf::SetState_READY() {
	...
	for (int n = 0; n < this->m_iMonsterGroupNumberCount; n++) TMonsterAIGroup::Init(this->m_iMonsterGroupNumberArray[n]);
...
void CCrywolf::SetState_START() {
	...
	for (int n = 0; n < this->m_iMonsterGroupNumberCount; n++) {
		...
		if (!CHECK_LIMIT(iLeaderIndex, g_ConfigRead.server.GetObjectMaxMonster())) continue;
		if (gObj[iLeaderIndex].Class != 340) continue;
		...
		for (int i = 0; i < MaxViewportMonster; i++) {
			if (lpObj->VpPlayer2[i].state) {
				...
				if (ObjectMaxRange(number) != FALSE) {
					if (gObj[number].Type == OBJ_USER) CHARACTER_KnockbackDistance(&gObj[number], lpObj, 3);
				}
			}
		}
	}
...
void CCrywolf::SetState_END() {
	...
	for (int iAltar = 205; iAltar <= 209; iAltar++) {
		...
		if (iUserIndex != -1) {
			...
			if (this->GetOccupationState() != 1) this->MakeRewardForAltarElf(iUserIndex);
		}
	}
...
void CCrywolf::SetState_ENDCYCLE() {
	...
	for (int n = 0; n < this->m_iMonsterGroupNumberCount; n++) TMonsterAIGroup::DelGroupInstance(this->m_iMonsterGroupNumberArray[n]);
...
void CCrywolf::ProcState_NOTIFY_2() {
	if ((GetTickCount() - this->m_dwCrywolfNotifyMsgStartTick) > 70000) {
		...
		} else UTIL.SendMapServerGroupMsg(Lang.GetText(0,229), iTime);
...
void CCrywolf::ProcState_READY() {
	...
	if ((GetTickCount() - this->m_dwCrywolfNotifyMsgStartTick) > 70000) {
		...
		} else UTIL.SendMapServerGroupMsg(Lang.GetText(0,231), iTime);
...
void CCrywolf::ProcState_START() {
	...
	if ((GetTickCount() - this->m_dwCrywolfStartProcTick) > (this->m_iBossTurnUpTime * 1000) && !this->m_bTurnUpBoss) {
	...
	if ((GetTickCount() - this->m_dwCrywolfStartProcTick) > (this->m_iMonsterGroupChangeAITime * 1000) && !this->m_bChangeAI) {
...
void CCrywolf::NotifyCrywolfCurrentState() {
	...
	g_Log.AddC(TColor::Green,  "[ Crywolf ] SetState - StateInfo : Occupation/%d, State/%d", pMsg.btOccupationState, pMsg.btCrywolfState);
}
...
void CCrywolf::NotifyCrywolfStateLeftTime() {
	...
	if ((GetTickCount() - dwTickCount) < 20000) return;
	...
	UTIL.SendCrywolfUserAnyData((LPBYTE)&pMsg,sizeof(pMsg));
	g_Log.AddC(TColor::Green,  "[ Crywolf ] War LeftTime - (%02d:%02d)", pMsg.btHour, pMsg.btMinute);
}

void CCrywolf::NotifyCrywolfStatueAndAltarInfo() {
	static DWORD dwTickCount = 0;

	if ((GetTickCount() - dwTickCount) < 2000) return;
	...
	UTIL.SendCrywolfUserAnyData((LPBYTE)&pMsg,sizeof(pMsg));
	g_Log.Add("[ Crywolf ][Statue Info] Statue HP : %d", pMsg.iCrywolfStatueHP);

	for (int iAltar = 205; iAltar <= 209; iAltar ++) {
		int iUserIndex = g_CrywolfNPC_Altar.GetAltarUserIndex(iAltar);

		if (iUserIndex != -1) g_Log.Add("[ Crywolf ][Altar Info] Altar(%d) Contractor: [%s][%s] ", iAltar, gObj[iUserIndex].AccountID, gObj[iUserIndex].Name);
	}
}
...
void CCrywolf::NotifyCrywolfBossMonsterInfo() {
	...
	if ((GetTickCount() - dwTickCount) < 5000) return;
	...
	for (int i = 0; i < g_ConfigRead.server.GetObjectMaxMonster(); i++) {
		if (!gObjIsConnected(i)) continue;
		if (gObj[i].MapNumber != MAP_INDEX_CRYWOLF_FIRSTZONE) continue;
	...
	UTIL.SendCrywolfUserAnyData((LPBYTE)&pMsg,sizeof(pMsg));
	g_Log.Add("[ Crywolf ][Boss Monster Info] Balgass HP:%d, DarkElf:%d", pMsg.iMonster1HP, pMsg.btMonster2);
}
...
void CCrywolf::NotifyCrywolfPersonalRank() {
	...
	for (int i = g_ConfigRead.server.GetObjectStartUserIndex(); i < g_ConfigRead.server.GetObjectMax(); i++) {
		if (gObj[i].Connected == PLAYER_PLAYING && gObj[i].Type == OBJ_USER && gObj[i].MapNumber == MAP_INDEX_CRYWOLF_FIRSTZONE) {
			...
			g_Log.Add("[Crywolf][Personal Rank & Exp.] [%s][%s] Score(%d) Rank(%d) Exp(%d)", gObj[i].AccountID, gObj[i].Name, gObj[i].m_iCrywolfMVPScore, pMsg.btRank, pMsg.iGettingExp);
...
void CCrywolf::NotifyCrywolfHeroList() {
	...
	for (int i = g_ConfigRead.server.GetObjectStartUserIndex(); i < g_ConfigRead.server.GetObjectMax(); i++) {
		if (gObj[i].Connected == PLAYER_PLAYING && gObj[i].Type == OBJ_USER && gObj[i].MapNumber == MAP_INDEX_CRYWOLF_FIRSTZONE) HeroSet.insert(&gObj[i]);
	}
	...
	for (int j = 0; j < 5 && _Itor != HeroSet.end(); j++, _Itor++) {
		...
		iHeroCount++;
		g_Log.Add("[ Crywolf ][Hero List] [%s][%s] Score(%d) Ranking(%d)", lpHeroObj->AccountID, lpHeroObj->Name, lpHeroObj->m_iCrywolfMVPScore, iHeroCount);

		if (this->GetOccupationState() != 1) {
			if (lpMsgBody[j].iRank <= 5) this->MakeRewardForHeroListTop5(lpHeroObj->m_Index);
		}
	}
...
void CCrywolf::OperateGmCommand(int iUserIndex, int iCommand) {
	if (!gObjIsConnectedGP(iUserIndex)) return;
	if ((gObj[iUserIndex].Authority & 0x02) != 0x02 && (gObj[iUserIndex].Authority & 0x20) != 0x20)	return;
	if ((gObj[iUserIndex].GameMaster & GM_EVENT_MODIFY) != GM_EVENT_MODIFY) {
		MsgOutput(iUserIndex, Lang.GetText(0,882));
		return;
	}

	g_Log.AddC(TColor::Red, "[Crywolf][Use Admin Command] [%s][%s][%s] Command : %d", gObj[iUserIndex].AccountID, gObj[iUserIndex].Name, gObj[iUserIndex].CharacterData->Ip_addr, iCommand);
	...
	switch (iCommand) {
		...
		case 2:
			if (this->GetCrywolfState() == CRYWOLF_STATE_READY) this->SetState(CRYWOLF_STATE_START);
			else MsgOutput(iUserIndex, Lang.GetText(0,238));
		break;
...
void CCrywolf::CrywolfInfoDBSave() {
	if (this->m_bDBDataLoadOK != FALSE) GDReqCrywolfInfoSave(g_MapServerManager.GetMapSvrGroup(), this->GetCrywolfState(), this->GetOccupationState());
}
...
void CCrywolf::ApplyCrywolfDBInfo(int iCrywolfState, int iOccupationState) {
	...
	this->SetCrywolfCommonNPC(this->GetOccupationState());
	this->SetCrywolfMapAttr(this->GetOccupationState());
	g_Log.Add("[ Crywolf ][ ApplyDBInfo] OccupationState : %d", iOccupationState);
}
			...
			if (lpMonObj->m_iCurrentAI != 0) {
				if (lpMonObj->Class == 349) {
					if (bExistKiller) {
						...
						g_Log.Add("[ Crywolf ][MVP] [Balgass Dead] by [%s][%s]", lpKillerObj->AccountID, lpKillerObj->Name);
					} else {
				...
				if (lpMonObj->Class == 340) {
					if (bExistKiller) {
						UTIL.SendCrywolfUserAnyMsg(2, Lang.GetText(0,241), lpMonObj->m_iGroupNumber, lpKillerObj->Name);
						g_Log.Add("[Crywolf][MVP] [DarkElf Dead] by [%s][%s]", lpKillerObj->AccountID, lpKillerObj->Name);
					}
				}
		
				int iScore = this->CalcGettingScore(iKillerIndex, iMonIndex, 100);

				if (iScore != -1) MsgOutput(iKillerIndex, Lang.GetText(0,242), iScore);
			}
		}
	}
}
...
void CCrywolf::MakeRewardForAltarElf(int iAltarUserIndex) {
	...
	ItemSerialCreateSend(gObj[iAltarUserIndex].m_Index, gObj[iAltarUserIndex].MapNumber, gObj[iAltarUserIndex].X, gObj[iAltarUserIndex].Y, iItemNumber, 0, 0, 0, 0, 0, iAltarUserIndex, 0, 0, 0, 0, 0);
	g_Log.Add("[ Crywolf ][Reward] [Altar Elf] [%s][%s] Success Reward - Gem Of Bless", gObj[iAltarUserIndex].AccountID, gObj[iAltarUserIndex].Name);
...
void CCrywolf::MakeRewardForHeroListTop5(int iUserIndex) {
	...
	ItemSerialCreateSend(gObj[iUserIndex].m_Index, gObj[iUserIndex].MapNumber, gObj[iUserIndex].X, gObj[iUserIndex].Y, iItemNumber, 0, 0, 0, 0, 0, iUserIndex, 0, 0, 0, 0, 0);
	g_Log.Add("[ Crywolf ][Reward] [Hero List Top 5] [%s][%s] Success Reward - Gem Of Bless", gObj[iUserIndex].AccountID, gObj[iUserIndex].Name);
...
int CCrywolf::CalcGettingScore(int iUserIndex, int iMonIndex, int iScoreType) {
	...
	if (iScoreType == 6) iAddMVPScore = this->m_iMVPScoreTable[6];

	gObj[iUserIndex].m_iCrywolfMVPScore += iAddMVPScore;
	g_Log.Add("[ Crywolf ][MVP] Plus MVP Score : %d, Accumulated : %d [%s][%s]", iAddMVPScore, gObj[iUserIndex].m_iCrywolfMVPScore, gObj[iUserIndex].AccountID, gObj[iUserIndex].Name);
	return gObj[iUserIndex].m_iCrywolfMVPScore;
}

int CCrywolf::CalcGettingRewardExp(int iUserIndex, int iMVPRank) {
	if (!gObjIsConnected(iUserIndex)) return 0;
	if (CHECK_LIMIT(iMVPRank, 5) == FALSE) return 0;

	int iRewardExp = this->m_iMVPRankExpTable[iMVPRank];

	if (this->GetOccupationState() == 1) iRewardExp = (iRewardExp * 10) / 100;
...
int CCrywolf::CalcGettingRank(int iUserIndex) {
	...
	for (int i = 0; i <= 4; i++) {
		if (gObj[iUserIndex].m_iCrywolfMVPScore < this->m_iMVPRankScoreTable[i]) break;
		iRank = i;
	}
...
void CCrywolf::GiveRewardExp(int iUserIndex, int iRewardExp) {
	...
	if (iRewardExp <= -1) return;

	UINT64 Reward = iRewardExp;

	if (gObj[iUserIndex].Type == OBJ_USER) {
		if (Reward > 0) {
			CheckItemOptForGetExpExRenewal(&gObj[iUserIndex], 0, Reward, 0, TRUE);
			Reward = UTIL.CalcRewardEXP(iUserIndex, Reward);
		}

		GSProtocol.PROTOCOL_PlayerExperienceSend(iUserIndex, (WORD)-1, Reward, 0, 0);
	}
}

void CCrywolf::ResetAllUserMVPScore() {
	for (int i = g_ConfigRead.server.GetObjectStartUserIndex(); i < g_ConfigRead.server.GetObjectMax(); i++) {
		if (gObj[i].Connected == PLAYER_PLAYING && gObj[i].Type == OBJ_USER && gObj[i].MapNumber == MAP_INDEX_CRYWOLF_FIRSTZONE) gObj[i].m_iCrywolfMVPScore = 0;
	}
}
...
void CCrywolf::CrywolfSecondAct() {
	...
	if (this->m_bDBDataLoadOK == FALSE && this->m_bDBDataLoading == FALSE) this->CrywolfInfoDBLoad();
}
