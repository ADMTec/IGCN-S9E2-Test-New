...
BOOL CMoveCommand::Move(LPOBJ lpObj, int iMapIndex)
{
	...
	if ( GateNumber >= 0 )
	{
		...
		{
			if ( (lpObj->CharacterData->Money - NeedZen ) >= 0 )
			{
				if ( gObjMoveGate(lpObj->m_Index, GateNumber ) != FALSE )
				{
					...
					g_Log.Add("[%s][%s] [Using Warp to: %s] Success : reduces money %d - %d", lpObj->AccountID, lpObj->Name, this->m_MoveCommandData[index].Name, lpObj->CharacterData->Money, NeedLevel);
...
{
	if ( lpObj->Class == CLASS_MAGICGLADIATOR || lpObj->Class == CLASS_DARKLORD || lpObj->Class == CLASS_RAGEFIGHTER )
	{
