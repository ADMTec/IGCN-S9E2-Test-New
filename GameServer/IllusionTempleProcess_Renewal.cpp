...
{
	if (CHARACTER_CheckSkillDistance(lpObj->m_Index, lpTargetObj->m_Index, 211) == FALSE)
	{
		return FALSE;
	}
	...
	BUFF_AddBuffEffect(lpTargetObj, BUFFTYPE_RESTRICTION, 0, 0, 0, 0, -10);
...
{
	if (CHARACTER_CheckSkillDistance(lpObj->m_Index, lpTargetObj->m_Index, 213) == FALSE)
	{
		return FALSE;
	}
		...
		if (this->m_UserData[j].m_nIndex != -1)
		{
			if (gObj[this->m_UserData[j].m_nIndex].Connected > PLAYER_LOGGED)
			{
				...
				g_Log.Add("[ ITR ] MAP:(%d) ITR Result  (Account:%s, Name:%s, G.Name:%s Team:%d, Class:%d, UserKillCount:%d,InvalidationCount:%d,OccupiedCount:%d)",
					this->m_nTempleNumber + 1, gObj[this->m_UserData[j].m_nIndex].AccountID, gObj[this->m_UserData[j].m_nIndex].Name, gObj[this->m_UserData[j].m_nIndex].CharacterData->GuildName,
					this->m_UserData[j].m_btTeam, gObj[this->m_UserData[j].m_nIndex].CharacterData->DbClass, this->m_UserData[j].m_btUserKillCount, this->m_UserData[j].m_byInvalidationCount,
