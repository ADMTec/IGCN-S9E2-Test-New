...
int QuestExpLuaBind::IsUsingExtendedWarehouse(lua_State *L)
{
	...
	int iExtendedWarehouseCount = gObj[iObjIndex].CharacterData->m_WarehouseExpansion;
	lua_settop(L, -2);
...
{
	int iObjIndex = luaL_checkinteger(L, 2);
	int iUserLv = gObj[iObjIndex].Level + gObj[iObjIndex].CharacterData->MasterLevel;
	lua_settop(L, -2);
...
int QuestExpLuaBind::SubQuestZen(lua_State *L)
{
	...
	if (gObj[iObjIndex].CharacterData->Money >= iSubMoney)
	{
		gObj[iObjIndex].CharacterData->Money -= iSubMoney;
	}
...
int QuestExpLuaBind::SendQuestConfirmBtn(lua_State *L)
{
    ...
    if( gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEpisode].GetAskCnt() > 0 )
    {
		gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEpisode].SetAskCnt(0);

		for (int i = 0; i < MAX_QUESTEXP_USER_INFO; i++)
		{
			gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEpisode].m_UserQuestAskInfo[i].Clear();
		}
    }
