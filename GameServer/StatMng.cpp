...
int CStatMng::GetMaxStat(int level, int Class) {
	...
	if (Class == CLASS_MAGICGLADIATOR)	{
		return this->gObjMagumAddStatTable[level-1] + (level * 0.3);
	}
	
	if (Class == CLASS_DARKLORD) {
		return this->gObjDarkLordAddStatTable[level-1] + (level * 1.3);
	}

	if (Class == CLASS_RAGEFIGHTER)	{
		return this->gObjMagumAddStatTable[level - 1] + (level * 0.3);
	}
	
	return this->gObjNormalAddStatTable[level-1] + (level * 0.5);
}
...
int CStatMng::GetMaxMinusStat(int iLevel, int iClass) {
	...
	if (iClass == CLASS_MAGICGLADIATOR) {
		return this->gObjMagumMinusStatTable[iLevel-1] + (iLevel * 0.5);
	}
	
	if (iClass == CLASS_DARKLORD) {
		return this->gObjDarkLordMinusStatTable[iLevel-1] + (iLevel * 1.5);
	}

	if (iClass == CLASS_RAGEFIGHTER) {
		return this->gObjMagumMinusStatTable[iLevel-1] + (iLevel * 0.5);
	}
	
	return this->gObjNormalMinusStatTable[iLevel-1] + (iLevel * 0.75);
}