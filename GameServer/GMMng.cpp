...
void CGMMng::Init() {
	...
	this->cCommand.Add(m_CommandNames.PostCMD, 391);
	this->cCommand.Add(m_CommandNames.PKCLEARCMD, 392);
...
int CGMMng::GetCmd(char* szCmd) {
	for (int n = 0; n < MAX_GM_COMMAND; n++) {
		if (this->cCommand.nCmdCode[n] > 0)	{
			if (stricmp(szCmd, this->cCommand.szCmd[n]) == 0) return this->cCommand.nCmdCode[n];
		}
	}
...
int CGMMng::ManagementProc(LPOBJ lpObj, char* szCmd, int aIndex) {
	...
	if(command_number != 392 && command_number != 412) CommandLog->Output("[%s][%s][%s] Used command: %s [AUTH: %d]", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Ip_addr, szCmdToken, lpObj->Authority);
	switch (command_number)	{
		case 100: {
			if ((lpObj->AuthorityCode & 4) != 4) return FALSE;
			...
			if (iTargetIndex >= 0) {
				...
				g_Log.Add("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] / Target : [%s][%s] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, lpTargetObj->AccountID, lpTargetObj->Name, "User Disconnect");
				GMLog->Output("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] / Target : [%s][%s] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, lpTargetObj->AccountID, lpTargetObj->Name, "User Disconnect");
		...
		case 101: {
			if ((lpObj->GameMaster & GM_COMMAND_GUILD) == GM_COMMAND_GUILD && (lpObj->GameMaster & GM_COMMAND_DC_MOVE) == GM_COMMAND_DC_MOVE) {
				...
				if (iIndex >= 0) {
					...
					g_Log.Add("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] / Target : [%s][%s] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, lpTargetObj->AccountID, lpTargetObj->Name, "User SetPosition");
					GMLog->Output("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] / Target : [%s][%s] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, lpTargetObj->AccountID, lpTargetObj->Name, "User SetPosition");
		...
		case 104: {
			if ((lpObj->AuthorityCode & 0x20) != 0x20) return FALSE;
			if ((lpObj->GameMaster & GM_COMMAND_DC_MOVE) != GM_COMMAND_DC_MOVE) {
				MsgOutput(lpObj->m_Index, Lang.GetText(0, 425));
				return FALSE;
			}

			g_Log.Add("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, "Ban Chatting");
			GMLog->Output("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, "Ban Chatting");
			pId = this->GetTokenString();
		...
		case 106: {
			if ((lpObj->AuthorityCode & 32) != 32) return FALSE;
			if ((lpObj->GameMaster & GM_COMMAND_DC_MOVE) != GM_COMMAND_DC_MOVE) {
				MsgOutput(lpObj->m_Index, Lang.GetText(0, 425));
				return FALSE;
			}

			g_Log.Add("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, "Free Ban-Chatting");
			GMLog->Output("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, "Free Ban-Chatting");
			pId = this->GetTokenString();
		...
		case 108: {
			...
			g_Log.Add("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, "Guild SetPosition");
			GMLog->Output("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, "Guild SetPosition");
		...
		case 109: {
			if ((lpObj->Authority & 2) == 2) {
				...
				g_Log.Add("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, "Start BattleSoccer");
				GMLog->Output("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, "Start BattleSoccer");
		...
		case 110: {
			if ((lpObj->Authority & 2) == 2) {
				...
				g_Log.Add("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, "Stop BattleSoccer");
				GMLog->Output("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, "Stop BattleSoccer");

				BattleSoccerGoalEnd(0);
			} else {
				if (gObj[aIndex].CharacterData->lpGuild != NULL) {
					if (gObj[aIndex].CharacterData->lpGuild->WarType == 1) {
						strcmp(gObj[aIndex].Name, gObj[aIndex].CharacterData->lpGuild->Names[0]);
					}
				}
			}
		}
		...
		case 111: {
			if ((lpObj->Authority & 2) == 2) {
				...
				g_Log.Add("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, "End GuildWar");
				GMLog->Output("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, "End GuildWar");
				...
				if (gObj[aIndex].CharacterData->lpGuild != NULL && gObj[aIndex].CharacterData->lpGuild->lpTargetGuildNode != NULL) {
					if (strcmp(gObj[aIndex].Name, gObj[aIndex].CharacterData->lpGuild->Names[0]) == 0) {
						if (gObj[aIndex].CharacterData->lpGuild->BattleGroundIndex >= 0 && gObj[aIndex].CharacterData->lpGuild->WarType == 1) {
							::gObjAddMsgSendDelay(&gObj[aIndex], 7, aIndex, 10000, 0);

							char szTemp[100];

							wsprintf(szTemp, Lang.GetText(0, 61), gObj[aIndex].CharacterData->lpGuild->Names[0]);
							GSProtocol.GCServerMsgStringSendGuild(gObj[aIndex].CharacterData->lpGuild, szTemp, 1);
							GSProtocol.GCServerMsgStringSendGuild(gObj[aIndex].CharacterData->lpGuild->lpTargetGuildNode, szTemp, 1);
						}
					}
				}
			}
		...
		case 112: {
			...
			g_Log.Add("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, "Guild Disconnect");

			GMLog->Output("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, "Guild Disconnect");
		...
		case 202: {
			if ((lpObj->Authority & 2) == 2) {
				...
				g_Log.Add("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, "Set GuildWar");
				GMLog->Output("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, "Set GuildWar");
		...
		case 203:
			...
			if (pId != NULL) {
				...
				if (strcmp(pId, "on") == 0) bState = TRUE;
				else if (strcmp(pId, "off") == 0) bState = FALSE;
		...
		case 214: {
			...
			g_Log.Add("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, "User Stat (connection)");
			int lc151 = 0;
			int lc152 = 400;
		...
		case 215: {
			...
			g_Log.Add("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] / Target : [%s][%s] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, lpTargetObj->AccountID, lpTargetObj->Name, "User Tracking");

			GMLog->Output("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] / Target : [%s][%s] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, lpTargetObj->AccountID, lpTargetObj->Name, "User Tracking");
		...
		case 217: {
			...
			g_Log.Add("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, "FIRECRACK.");
			CommandLog->Output("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, "FIRECRACK.");
		...
		case 320: {
			...
			g_Log.Add("[KUNDUN] Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID,	lpObj->Name, "Kundun Event Start");
		...
		case 321: {
			...
			g_Log.Add("[KUNDUN] Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID,	lpObj->Name, "Kundun SetHP");
		...
		case 322: {
			...
			g_Log.Add("[KUNDUN] Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID,	lpObj->Name, "Kundun SetHPRefill");
		...
		case 323: {
			...
			g_Log.Add("[KUNDUN] Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID,	lpObj->Name, "Kundun SetHPRefillSec");
		...
		case 324: {
			...
			g_Log.Add("[KUNDUN] Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID,	lpObj->Name, "Kundun SetHPRefillTime");
		...
		case 390: {
			...
			if ((type >= 0 && type <= 15)) {
				...
				g_Log.Add("[%s][%s][%s] Created Item using Admin Command (%s/%d/%d) Auth: %d", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Ip_addr, Lang.GetMap(0, lpObj->MapNumber), lpObj->X, lpObj->Y, lpObj->Authority);
				GMLog->Output("[%s][%s][%s] Created Item using Admin Command (%s/%d/%d) Auth: %d", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Ip_addr, Lang.GetMap(0, lpObj->MapNumber), lpObj->X, lpObj->Y, lpObj->Authority);
			}
		}
		break;
		...
		case 391: {
			...
			if (lpObj->CharacterData->Money < g_ConfigRead.data.post.iPostPrice) {
			...
			if (SwearFilter.CompareText(szCmd + strlen(this->m_CommandNames.PostCMD)) == true) {
				...
				if (lpObj->CharacterData->SwearWarning >= 3) {
					...
					MsgOutput(aIndex, Lang.GetText(0, 432), lpObj->CharacterData->SwearWarning);
					lpObj->CharacterData->Money -= g_ConfigRead.data.post.iPostPrice * 2;
			...
			lpObj->CharacterData->Money -= g_ConfigRead.data.post.iPostPrice;
			GSProtocol.GCMoneySend(lpObj->m_Index, lpObj->CharacterData->Money);
		...
		case 392: {
			if (!g_ConfigRead.pk.bPkClearEnable)	break;
			...
			if (gObj[aIndex].CharacterData->Money < Price) {
				MsgOutput(aIndex, Lang.GetText(0, 450), Price);
			}
			else {
				gObj[aIndex].CharacterData->Money -= Price;
		...
		case 393: {
			...
			if (lpObj->CharacterData->LevelUpPoint < Pts || Pts < 1) {
				GSProtocol.GCServerMsgStringSend(Lang.GetText(0, 435), lpObj->m_Index, 1);
				return FALSE;
			}

			if ((Pts + lpObj->CharacterData->Strength) > g_MaxStrength) {
				GSProtocol.GCServerMsgStringSend(Lang.GetText(0, 436), lpObj->m_Index, 1);
				return FALSE;
			}

			lpObj->CharacterData->LevelUpPoint -= Pts;
			lpObj->CharacterData->Strength += Pts;
			...
			pStats.subcode = 0x90;
			pStats.Points = lpObj->CharacterData->Strength;
			pStats.statstype = 0;
			pStats.LUP = lpObj->CharacterData->LevelUpPoint;
		...
		case 394: {
			...
			if ((Pts + lpObj->CharacterData->Dexterity) > g_MaxAgility) {
				GSProtocol.GCServerMsgStringSend(Lang.GetText(0, 436), lpObj->m_Index, 1);
				return FALSE;
			}
			...
			lpObj->CharacterData->Dexterity += Pts;
			lpObj->CharacterData->LevelUpPoint -= Pts;
			...
			pStats.h.size = sizeof(pStats);
			pStats.subcode = 0x90;
			pStats.Points = lpObj->CharacterData->Dexterity;
			pStats.statstype = 1;
			pStats.LUP = lpObj->CharacterData->LevelUpPoint;
			IOCP.DataSend(aIndex, (LPBYTE)&pStats, sizeof(pStats));
		...
		case 395: {
			...
			if ((Pts + lpObj->CharacterData->Vitality) > g_MaxVitality) {
				GSProtocol.GCServerMsgStringSend(Lang.GetText(0, 436), lpObj->m_Index, 1);
				return FALSE;
			}

			lpObj->CharacterData->LevelUpPoint -= Pts;
			lpObj->CharacterData->Vitality += Pts;
			lpObj->MaxLife += lpObj->VitalityToLife * Pts;
			...
			pStats.h.size = sizeof(pStats);
			pStats.subcode = 0x90;
			pStats.Points = lpObj->CharacterData->Vitality;
			pStats.statstype = 2;
			pStats.LUP = lpObj->CharacterData->LevelUpPoint;
		...
		case 396: {
			...
			if ((Pts + lpObj->CharacterData->Energy) > g_MaxEnergy) {
				GSProtocol.GCServerMsgStringSend(Lang.GetText(0, 436), lpObj->m_Index, 1);
				return FALSE;
			}

			lpObj->CharacterData->Energy += Pts;
			lpObj->CharacterData->LevelUpPoint -= Pts;
			lpObj->MaxMana += lpObj->EnergyToMana * Pts;
			...
			pStats.h.size = sizeof(pStats);
			pStats.subcode = 0x90;
			pStats.Points = lpObj->CharacterData->Energy;
			pStats.statstype = 3;
			pStats.LUP = lpObj->CharacterData->LevelUpPoint;
		...
		case 397: {
			...
			if ((Pts + lpObj->Leadership) > g_MaxCommand) {
				GSProtocol.GCServerMsgStringSend(Lang.GetText(0, 436), lpObj->m_Index, 1);
				return FALSE;
			}

			lpObj->Leadership += Pts;
			lpObj->CharacterData->LevelUpPoint -= Pts;
		...
		case 399: {
			...
			MsgOutput(aIndex, Lang.GetText(0, 446), lpObj->AccountID, lpObj->Name, lpObj->Level, lpObj->CharacterData->MasterLevel, lpObj->Class);
			MsgOutput(aIndex, Lang.GetText(0, 447), lpObj->CharacterData->SwearWarning, Lang.GetMap(0, lpObj->MapNumber), lpObj->X, lpObj->Y);
		...
		case 401: {
			if ((lpObj->Authority & 2) != 2 && (lpObj->Authority & 0x20) != 0x20) return FALSE;
			...
			if (!ObjectMaxRange(iTargetIndex)) return FALSE;
			if (gObj[iTargetIndex].Type != OBJ_USER) return FALSE;

			MsgOutput(lpObj->m_Index, "Character: %s", gObj[iTargetIndex].Name);
			MsgOutput(lpObj->m_Index, "AccountID: %s, Level:%d MasterLevel:%d", gObj[iTargetIndex].AccountID, gObj[iTargetIndex].Level, gObj[iTargetIndex].CharacterData->MasterLevel);
		...
		case 406: {
			if (gObj[aIndex].Married == 0) return FALSE;

			if (strlen(gObj[aIndex].MarryName) <= 0) return FALSE;
		...
		case 408: {
			if ((gObj[aIndex].Authority & 2) != 2 && (gObj[aIndex].Authority & 0x20) != 0x20) return FALSE; /*Check if user is GM*/
		...
		case 409: {
			if((gObj[aIndex].Authority & 2) != 2 && (gObj[aIndex].Authority & 0x20) != 0x20) return FALSE;
		...
		case 412: {
			if(!g_ConfigRead.data.reset.enable)	return FALSE;
			...
			if(lpObj->CharacterData->Money < g_ConfigRead.data.reset.iCost) {
				MsgOutput(aIndex,Lang.GetText(0,459),g_ConfigRead.data.reset.iCost);
				return FALSE;
			}

			if(lpObj->CharacterData->m_iResets >= g_ConfigRead.data.reset.iResetLimit) {
			...
			if (BC_MAP_RANGE(lpObj->MapNumber) || DS_MAP_RANGE(lpObj->MapNumber) || CC_MAP_RANGE(lpObj->MapNumber) || IT_MAP_RANGE(lpObj->MapNumber) ||
				...
				|| lpObj->MapNumber == MAP_INDEX_KALIMA2 || lpObj->MapNumber == MAP_INDEX_KALIMA3 || lpObj->MapNumber == MAP_INDEX_KALIMA4 || lpObj->MapNumber == MAP_INDEX_KALIMA5 || 
				lpObj->MapNumber == MAP_INDEX_KALIMA6 || lpObj->MapNumber == MAP_INDEX_KALIMA7 || lpObj->MapNumber == MAP_INDEX_ARCA_WAR || lpObj->MapNumber == MAP_INDEX_DEBENTER_ARCA_WAR) {
			...
			if (lpObj->CharacterData->m_MuBotEnable == true)	{
			...
			if(g_ConfigRead.data.reset.ResetStats == true) {
				...
				if (lpObj->CharacterData->VipType == 0) {
					lpObj->CharacterData->LevelUpPoint += (lpObj->CharacterData->m_iResets + 1) * g_ConfigRead.data.reset.NormalPointPerReset;
				}

				else if (lpObj->CharacterData->VipType > 0) {
					lpObj->CharacterData->LevelUpPoint += (lpObj->CharacterData->m_iResets + 1) * g_VipSystem.GetPointPerReset(lpObj);
				}
			...
			lpObj->CharacterData->m_iResets += 1;
			CHARACTER_NextExpCal(lpObj);
			lpObj->CharacterData->Money -=  g_ConfigRead.data.reset.iCost;
			GSProtocol.GCMoneySend(aIndex,lpObj->CharacterData->Money);
			gObjCalCharacter.CHARACTER_Calc(lpObj->m_Index);
			...
			MsgOutput(aIndex,Lang.GetText(0,462),lpObj->CharacterData->m_iResets);
		...
		case 416:
			if((lpObj->Authority & 2) == 2 || (lpObj->Authority &0x20) == 0x20)	{
				...
				BUFF_AddBuffEffect(lpObj, BUFFTYPE_INVISABLE, 0, 0, 0, 0, -10);
		...
		case 417:
			if((lpObj->Authority & 2) == 2 || (lpObj->Authority &0x20) == 0x20)	{
				...
				BUFF_RemoveBuffEffect(lpObj, BUFFTYPE_INVISABLE);
		...
		case 418:
			for(int i=0;i<INVENTORY_SIZE;++i) {
				if(lpObj->pInventory[i].IsItem() == TRUE) {
					...
					GSProtocol.PROTOCOL_InventoryItemDeleteSend(aIndex, i, 1);
				}				
			}
		...
		case 420: {
			if((lpObj->Authority & 2) != 2 && (lpObj->Authority &0x20) != 0x20)	break;
		...
		case 429: {
			...
			if (lpObj->CharacterData->Money < g_ConfigRead.data.post.iGPostPrice) {
				MsgOutput(lpObj->m_Index, Lang.GetText(0, 430), g_ConfigRead.data.post.iPostPrice);
				return FALSE;
			}
		...
		case 431: {
			...
			if(gObj[aIndex].CharacterData->m_iSecurityCode == code) {
		...
		case 432: {
			...
			gObj[aIndex].CharacterData->m_iSecurityCode = code;
		...
		case 435: {
			...
			lpObj->CharacterData->Strength += Pts;
			char Msg[100];
			sprintf(Msg, Lang.GetText(0, 437), Pts);
		...
		case 436: {
			...
			char Msg[100];
			sprintf(Msg, Lang.GetText(0, 438), Pts);
			GSProtocol.GCServerMsgStringSend(Msg, lpObj->m_Index, 1);
		...
		case 437: {
			...
			lpObj->CharacterData->Vitality += Pts;
			char Msg[100];
			sprintf(Msg, Lang.GetText(0, 439), Pts);
			GSProtocol.GCServerMsgStringSend(Msg, lpObj->m_Index, 1);
			gObjCalCharacter.CHARACTER_Calc(lpObj->m_Index);
			return TRUE;
		}
		...
		case 438: {
			...
			lpObj->CharacterData->LevelUpPoint -= Pts;
			char Msg[100];
			sprintf(Msg, Lang.GetText(0, 440), Pts);
			GSProtocol.GCServerMsgStringSend(Msg, lpObj->m_Index, 1);
			gObjCalCharacter.CHARACTER_Calc(lpObj->m_Index);
			return TRUE;
		}
		...
		case 439: {
			...
			lpObj->CharacterData->LevelUpPoint -= Pts;
			char Msg[100];
			sprintf(Msg, Lang.GetText(0, 442), Pts);
			GSProtocol.GCServerMsgStringSend(Msg, lpObj->m_Index, 1);
			return TRUE;
		}
		break;
		...
		case 446: {
			...
			g_Log.Add("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] / Target : [%s][%s] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, lpTargetObj->AccountID, lpTargetObj->Name, "User Trans");

			GMLog->Output("Use GM Command -> [ %s ]\t[ %s ]\t[ %s ] / Target : [%s][%s] : %s", lpObj->CharacterData->Ip_addr, lpObj->AccountID, lpObj->Name, lpTargetObj->AccountID, lpTargetObj->Name, "User Trans");
		...
		case 447: {
			...
			memcpy(lpObj->CharacterData->PartyPassword, pId,10);

			MsgOutput(aIndex, Lang.GetText(0,555), lpObj->CharacterData->PartyPassword);

			lpObj->CharacterData->bActiveSetParty = true;
		}
		break;
		...
		case 449: {
			if(g_OffLevel.b_Enabled == true) {
				if(lpObj->CharacterData->VipType < g_OffLevel.m_General.VipType)	{
				...
				if(g_OffLevel.m_PerMapAttr[lpObj->MapNumber].Disable != 1) {
					for(int i=0;i<5;i++) {
						CMagicInf* lpMagic = PLAYER_GetMagicSearch(lpObj,g_OffLevel.m_SkillDef[lpObj->Class].SkillNumber[i]);
		...
		case 450: { // GM Gift by Gremory Case
			...
			g_Log.Add("[%s][%s][%s] Added GM Gift to Gremory Case using Admin Command (%s/%d/%d) Auth: %d Target: %s", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Ip_addr, Lang.GetMap(0, lpObj->MapNumber), lpObj->X, lpObj->Y, lpObj->Authority, lpTargetObj->Name);
			GMLog->Output("[%s][%s][%s] Created Added GM Gift to Gremory Case using Admin Command (%s/%d/%d) Auth: %d Target: %s", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->Ip_addr, Lang.GetMap(0, lpObj->MapNumber), lpObj->X, lpObj->Y, lpObj->Authority, lpTargetObj->Name);
		}
		break;
...
void CGMMng::SetInfinityArrowTime(LPOBJ lpObj, int iValue) {
	if (lpObj->Class == CLASS_ELF && lpObj->Type == OBJ_USER && lpObj->CharacterData->ChangeUP >= 1) {
		BUFF_AddBuffEffect(lpObj, BUFFTYPE_INFINITY_ARROW, 0, 0, 0, 0, iValue);
...
void ServerMsgSend(LPOBJ lpObj,int Type,char Sender[20],const char*Message,...) {
	...
	*(Packet+1)=Len;
	if(!lpObj)GSProtocol.PROTOCOL_DataSendAll(Packet,Len);
	else
		if(lpObj->Connected)
