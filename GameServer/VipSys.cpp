...
void CVipSystem::Run()
{
	...
	for (int i = g_ConfigRead.server.GetObjectStartUserIndex(); i < g_ConfigRead.server.GetObjectMax(); i++)
	{
		...
		if (gObj[i].CharacterData->VipType == 0)
		{
			continue;
		}
		...
		std::map<int, VIP_INFO_DATA>::iterator it = this->m_mapVipInfo.find(gObj[i].CharacterData->VipType);
		...
		if (iVipEffect != gObj[i].CharacterData->VipEffect)
		{
			gObj[i].CharacterData->VipEffect = iVipEffect;
...
void CVipSystem::SetVipForUser(LPOBJ lpObj, BYTE btVipType)
{
	...
	lpObj->CharacterData->VipType = btVipType;
	...
	if ((m_time.wHour * 60 + m_time.wMinute) >= (it->second.wNightStartHour * 60 + it->second.wNightStartMinute))
	{
		if ((m_time.wHour * 60 + m_time.wMinute) < (it->second.wNightEndHour * 60 + it->second.wNightEndMinute))
		{
			lpObj->CharacterData->VipEffect = VIP_EFFECT_NIGHT;
		}

		else
		{
			lpObj->CharacterData->VipEffect = VIP_EFFECT_DAY;
		}
	}
...
float CVipSystem::GetExpBonus(LPOBJ lpObj)
{
	...
	std::map<int, VIP_INFO_DATA>::iterator it = this->m_mapVipInfo.find(lpObj->CharacterData->VipType);
	...
	if (lpObj->CharacterData->VipEffect < 0 || lpObj->CharacterData->VipEffect > 1)
	{
		LeaveCriticalSection(&this->m_criti);
		return 0;
	}
	...
	{
		fEffect = it->second.m_VipEffect[lpObj->CharacterData->VipEffect].fMLExpBonus;
	}

	else
	{
		fEffect = it->second.m_VipEffect[lpObj->CharacterData->VipEffect].fExpBonus;
	}
...
int CVipSystem::GetDropBonus(LPOBJ lpObj)
{
	...
	if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(lpObj))
	{
		iEffect = it->second.m_VipEffect[lpObj->CharacterData->VipEffect].iMLDropBonus;
	}

	else
	{
		iEffect = it->second.m_VipEffect[lpObj->CharacterData->VipEffect].iDropBonus;
	}
...
int CVipSystem::GetExcDropBonus(LPOBJ lpObj)
{
	...
	if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter(lpObj))
	{
		iEffect = it->second.m_VipEffect[lpObj->CharacterData->VipEffect].iMLExcDropBonus;
	}

	else
	{
		iEffect = it->second.m_VipEffect[lpObj->CharacterData->VipEffect].iExcDropBonus;
	}
