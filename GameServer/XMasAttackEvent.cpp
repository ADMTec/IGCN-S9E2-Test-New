...
BOOL CXMasMonsterHerd::MonsterHerdItemDrop(LPOBJ lpObj)
{
	...
	if ( lpObj->Class == 476 )
	{
		...
		if(buff == 1)
		{
			gObjAddBuffEffectWideArea(lpObj, 10, count, BUFFTYPE_SANTA_HEAL, EFFECTTYPE_INCREASE_LIFE, 500, 0, 0, g_XMasAttackEvent.GetRestBuffDuration());
		}
		else if(buff == 2)
		{
			gObjAddBuffEffectWideArea(lpObj, 10, count, BUFFTYPE_SANTA_PROTECT, EFFECTTYPE_INCREASE_MANA, 500, 0, 0, g_XMasAttackEvent.GetRestBuffDuration());
		}
		else
		{
			int Damage;
			...
			gObjAddBuffEffectWideArea(lpObj, 10, count, BUFFTYPE_CHRISTMAS_BLESS, EFFECTTYPE_INCREASE_DAMAGE, Damage, EFFECTTYPE_INCREASE_DEFENSE, Defense, g_XMasAttackEvent.GetSantaBuffDuration());
		}
