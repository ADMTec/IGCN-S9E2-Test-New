...
void CKalimaGate::KalimaGateAct(int iIndex)
{
	...
	if ( gObjIsConnected( gObj[iIndex].m_RecallMon ) == PLAYER_EMPTY )
	{
		...
		GSProtocol.PROTOCOL_DiePlayerSend( &gObj[iIndex], iIndex, 0, 0);
		gObjDel(iIndex);
...
{
	if ( gObj[iIndex].Class == CLASS_MAGICGLADIATOR || gObj[iIndex].Class == CLASS_DARKLORD || gObj[iIndex].Class == CLASS_RAGEFIGHTER )	// DarkLord and MagicGLadiaro
	{
...
BOOL CKalimaGate::DeleteKalimaGate(int iKalimaGateIndex, int iCallOwnerIndex)
{
	...
	__try
	{
		...
		GSProtocol.PROTOCOL_DiePlayerSend( &gObj[iKalimaGateIndex], iKalimaGateIndex, 0 , 0);
