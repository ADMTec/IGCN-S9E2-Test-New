...
BOOL CMonsterStatCalc::ConvertMonsterStat(LPOBJ lpObj, int nUserMaxLevel) {
	...
	for (int i=0; i<nSize; i++) {
		...
		if (nUserMaxLevel >= monsterStatRef.m_nMinLevel && nUserMaxLevel <= monsterStatRef.m_nMaxLevel) {
			...
			lpObj->Defense = lpAttr->Defense * monsterStatRef.m_nDefense;
			lpObj->m_MagicDefense = lpAttr->m_MagicDefense * monsterStatRef.m_nMagicDefense;
			lpObj->HitRating = lpAttr->HitRating * monsterStatRef.m_nAttackRating;
			lpObj->EvasionRating = lpAttr->m_Successfulblocking * monsterStatRef.m_nSuccessfulblocking;
			lpObj->m_MoveRange = lpAttr->m_MoveRange;
			lpObj->AttackSpeed = lpAttr->AttackSpeed;
			...
			lpObj->C_Resistance[R_COLD] = lpAttr->m_Resistance[R_COLD];
			lpObj->C_Resistance[R_POISON] = lpAttr->m_Resistance[R_POISON];
			lpObj->C_Resistance[R_LIGHTNING] = lpAttr->m_Resistance[R_LIGHTNING];
			lpObj->C_Resistance[R_FIRE] = lpAttr->m_Resistance[R_FIRE];
			lpObj->C_Resistance[R_EARTH] = lpAttr->m_Resistance[R_EARTH];
			lpObj->C_Resistance[R_WIND] = lpAttr->m_Resistance[R_WIND];
			lpObj->C_Resistance[R_WATER] = lpAttr->m_Resistance[R_WATER];
			...
			memcpy(lpObj->C_Resistance, lpAttr->m_Resistance, MAX_RESISTANCE_TYPE );
