...
BOOL NpcTalk(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	switch (npcnum)
	{
		...
		case 220:
			if (NPC_LandOfTrialsGatekeeper(lpNpc,lpObj) == TRUE)
			{
				return TRUE;
			}
			break;
		...
		case 256:
			if (NPC_Jeweller(lpNpc, lpObj) == TRUE)
			{
				return TRUE;
			}
			break;
		...
		case 223:
			if (NPC_CastleLordAdvisor(lpNpc, lpObj) == TRUE)
			{
				return TRUE;
			}
			break;
		...
		case 224:
			if (NPC_CastleGuard(lpNpc, lpObj) == TRUE)
			{
				return TRUE;
			}
			break;
		...
		case 219:
			if (NPC_CastleGateLever(lpNpc, lpObj) == TRUE)
			{
				return TRUE;
			}
			break;
		...
		case 216:
			if (NPC_CastleCrown(lpNpc, lpObj) == TRUE)
			{
				return TRUE;
			}
			break;
		...
		case 217:
		case 218:
			if (NPC_CrownRoomSwitch(lpNpc, lpObj) == TRUE)
			{
				return TRUE;
			}
			break;
		...
		case 478:
			...
			break;
		case 452:
			if (NPC_SeedMaster(lpNpc, lpObj) == TRUE)
			{
				return TRUE;
			}
			break;
		case 453:
			if (NPC_SeedResearcher(lpNpc, lpObj) == TRUE)
			{
				return TRUE;
			}
			break;
		...
		case 468:
			{
				BUFF_AddBuffEffect(lpObj, BUFFTYPE_SANTA_POWER, EFFECTTYPE_POWER_UP, 30, 0, 0, 1800);
			}
			break;
		case 469:
			{
				BUFF_AddBuffEffect(lpObj, BUFFTYPE_SANTA_GUARD, EFFECTTYPE_GUARD_UP, 100, 0, 0, 1800);
			}
			break;
		case 470:
			{
		...
		case 472:
			{
				BUFF_AddBuffEffect(lpObj, BUFFTYPE_SANTA_HEAL, EFFECTTYPE_INCREASE_LIFE, 500, 0, 0, 1800);
			}
			break;
		case 473:
			{
				BUFF_AddBuffEffect(lpObj, BUFFTYPE_SANTA_PROTECT, EFFECTTYPE_INCREASE_MANA, 500, 0, 0, 1800);
			}
			break;
		case 474:
			{
				BUFF_AddBuffEffect(lpObj, BUFFTYPE_SANTA_SPEED, EFFECTTYPE_INCREASE_ATTACKSPEED, 15, 0, 0, 1800);
			}
			break;
		case 475:
			{
				BUFF_AddBuffEffect(lpObj, BUFFTYPE_SANTA_LUCKY, EFFECTTYPE_AG_RECOVERY_UP, 10, 0, 0, 1800);
			}
			break;
		case 479:
			{
				return NPC_Titus(lpNpc, lpObj);
			}
			break;
		case 522:
			{
				return NPC_Jerint(lpNpc, lpObj);
			}
			break;
		case 540:
			{
				return NPC_Lugard(lpNpc, lpObj);
			}
			break;
		case 541:
			{
				return NPC_ForeignDimensionSilverBox(lpNpc, lpObj);
			}
			break;
		case 542:
			{
				return NPC_ForeignDimensionGoldenBox(lpNpc, lpObj);
			}
			break;
		case 543:
		case 544:
		...
		case 546:
			{
				return NPC_Jeweller(lpNpc, lpObj);
			}
			break;
		case 547:
			{
				return NPC_Julia(lpNpc, lpObj);
			}
			break;
		case 566:
		case 567:
	...
	case 258:
			{
				if (g_LastManStanding.m_Cfg.bEnable)
				{
					if (lpNpc->MapNumber == g_LastManStanding.m_Cfg.iRegNPCMap && lpNpc->X == g_LastManStanding.m_Cfg.iRegNPCX && lpNpc->Y ==  g_LastManStanding.m_Cfg.iRegNPCY)
					{
						...
						if (lpObj->CharacterData->RegisterdLMS == 1)
						{
							...
							for (short i = 0; i < MAIN_INVENTORY_SIZE; ++i)
							{
								if (lpObj->pInventory[i].m_Type > 0 && lpObj->pInventory[i].m_Type == ITEMGET(g_LastManStanding.m_Cfg.iReqItemCat,g_LastManStanding.m_Cfg.iReqItemIdx))
								{
									if (removedCount < g_LastManStanding.m_Cfg.iReqItemCount)
									{
										...
										GSProtocol.PROTOCOL_InventoryItemDeleteSend(lpObj->m_Index, i, 1);
										removedCount++;
									}
									else
									{
						...
						/*	if (invitationpos == -1)
							...
							if (lpObj->CharacterData->Money < g_LastManStanding.m_Cfg.iReqZen)
							{
								MsgOutput(lpObj->m_Index,Lang.GetText(0,399),g_LastManStanding.m_Cfg.iReqZen);
								return false;
							}
							lpObj->CharacterData->Money -= g_LastManStanding.m_Cfg.iReqZen;
...
BOOL NpcMainatenceMachine(LPOBJ lpNpc, LPOBJ lpObj) {
	if (g_ConfigRead.server.GetServerType() == SERVER_CASTLE) return TRUE;
...
BOOL NpcFasi(LPOBJ lpNpc, LPOBJ lpObj) {
	if (g_DevilSquare.IsEventEnable() != FALSE) GSProtocol.GCServerCmd(lpObj->m_Index, 1, 6, 0);

	BOOL bPlayerKiller = FALSE; //Season 2.5 add-on
	
	if (lpObj->PartyNumber >= 0) {
		if (gParty.GetPKPartyPenalty(lpObj->PartyNumber) > 4) bPlayerKiller = TRUE;
	} else if (lpObj->m_PK_Level > 4) bPlayerKiller = TRUE;
		...
		if (lpNpc->TargetNumber != lpObj->m_Index) {
			if (gObjGetPkTime(lpObj, hour, min) == TRUE) {
				...
				switch (lpObj->m_PK_Count) {
					...
					default:
						if (lpObj->m_PK_Level > 3) dwNeedPoint = g_ConfigRead.pk.iPKPointIncForMurderer * (lpObj->m_PK_Count - 3) + g_ConfigRead.pk.GetPKReqPoint(6);
					break;
				}

				if (dwNeedPoint != 0) dwNeedPoint -= lpObj->m_PK_Time;
	
				char szTemp[100];
				wsprintf(szTemp, Lang.GetText(0,16), dwNeedPoint);
				GSProtocol.PROTOCOL_NPCChatSend(lpNpc, szTemp, lpObj->m_Index);
			...
			switch (rand() % 3)	{
				case 0:
					GSProtocol.PROTOCOL_NPCChatSend(lpNpc, Lang.GetText(0,34), lpObj->m_Index);
				break;

				case 1:
					GSProtocol.PROTOCOL_NPCChatSend(lpNpc, Lang.GetText(0,35), lpObj->m_Index);
				break;

				case 2:
					GSProtocol.PROTOCOL_NPCChatSend(lpNpc, Lang.GetText(0,36), lpObj->m_Index);
				break;
			}
...
BOOL NpcGuildMasterTalk(LPOBJ lpNpc, LPOBJ lpObj) {
	if (g_ConfigRead.data.common.GuildCreate == FALSE) {
		GSProtocol.PROTOCOL_ChatSend(lpNpc, Lang.GetText(0,403));
		return TRUE;
	}
	...
	if (lpObj->CharacterData->GuildNumber > 0) {
		GSProtocol.PROTOCOL_ChatSend(lpNpc, Lang.GetText(0,48));
		return TRUE;
	}

	int capacity = CHARACTER_GuildMasterCapacityTest(lpObj);

	if (capacity == 0) GSProtocol.PROTOCOL_ChatSend(lpNpc, Lang.GetText(0,49));
	else if (capacity == 2) GSProtocol.PROTOCOL_ChatSend(lpNpc, Lang.GetText(0,50));
	else GSProtocol.GCGuildMasterQuestionSend(lpObj->m_Index);
	return TRUE;
}

BOOL NpcRolensiaGuard(LPOBJ lpNpc, LPOBJ lpObj) {
	if (gEnableEventNPCTalk != FALSE) GSProtocol.GCServerCmd(lpObj->m_Index, 4, 0, 0);
	return TRUE;
}
...
BOOL NpcChaosCardMaster(LPOBJ lpNpc, LPOBJ lpObj) {
	if (g_bUseLotterySystem == FALSE) return FALSE;
	if (bCanChaosBox == TRUE) {
		if (lpObj->m_IfState.use > 0) return FALSE;
...
BOOL NpcRusipher(LPOBJ lpNpc, LPOBJ lpObj) {
	if (g_DevilSquare.IsEventEnable() == FALSE) return TRUE;

	int aIndex = lpObj->m_Index;

	if ((lpObj->m_IfState.use) > 0) return TRUE;
	if (gUserFindDevilSquareInvitation(aIndex) == FALSE) GSProtocol.GCServerCmd(lpObj->m_Index, 1, 2 ,0);
	...
	} else GSProtocol.GCServerCmd(lpObj->m_Index, 1, 3, 0);
	return TRUE;
}
...
BOOL NpcNoriaRara(LPOBJ lpNpc, LPOBJ lpObj) {
	if (gEnableEventNPCTalk != FALSE) GSProtocol.GCServerCmd(lpObj->m_Index, 4, 2, 0);
	return FALSE;
}

BOOL NpcDeviasMadam(LPOBJ lpNpc, LPOBJ lpObj) {
	if (gEnableEventNPCTalk != FALSE) GSProtocol.GCServerCmd(lpObj->m_Index, 4, 1, 0);
	return FALSE;
}
...
BOOL NPC_Jeweller(LPOBJ lpNpc, LPOBJ lpObj) {
	if (bCanChaosBox == TRUE) {
		if ((lpObj->m_IfState.use) > 0) return TRUE;
...
BOOL NPC_CastleLordAdvisor(LPOBJ lpNpc, LPOBJ lpObj) { // CS Mix NPC
	if ((lpObj->m_IfState.use) > 0) return TRUE;
	if (g_ConfigRead.server.GetServerType() != SERVER_CASTLE) return TRUE;
	if (g_CastleSiege.GetCastleState() == 7) {
		GSProtocol.PROTOCOL_ChatSend(lpNpc, (Lang.GetText(0, 164)));
		return TRUE;
	}

	if (strcmp(lpObj->CharacterData->GuildName, "") == 0) {
		GSProtocol.PROTOCOL_ChatSend(lpNpc, (Lang.GetText(0, 165)));
		return TRUE;
	}

	if (g_CastleSiege.CheckGuildOwnCastle(lpObj->CharacterData->GuildName)) {
		if (lpObj->CharacterData->GuildStatus != 128) {
			if (lpObj->CharacterData->GuildStatus != 64) {
				GSProtocol.PROTOCOL_ChatSend(lpNpc, (Lang.GetText(0, 165)));
				return TRUE;
			}
		}
...
BOOL NPC_CastleGuard(LPOBJ lpNpc, LPOBJ lpObj) {
	if ((lpObj->m_IfState.use) > 0) return TRUE;
	if (g_ConfigRead.server.GetServerType() != SERVER_CASTLE) return TRUE;
	if (g_CastleSiege.GetCastleState() == 7)	{
		GSProtocol.PROTOCOL_ChatSend(lpNpc, (Lang.GetText(0, 166)));
		return TRUE;
	}
...
BOOL NPC_CastleGateLever(LPOBJ lpNpc, LPOBJ lpObj) {
	if ((lpObj->m_IfState.use) > 0) return TRUE;
	if (g_ConfigRead.server.GetServerType() != SERVER_CASTLE) return TRUE;
	...
	if (btResult != 1) return TRUE;

	lpObj->m_IfState.use = 1;
	lpObj->m_IfState.type = 12;
	return TRUE;
}
BOOL NPC_CastleCrown(LPOBJ lpNpc, LPOBJ lpObj) {
	if (g_ConfigRead.server.GetServerType() != SERVER_CASTLE) return TRUE;
	if (gObjIsConnected(lpObj->m_Index) == FALSE) return TRUE;
	if (g_CastleSiege.GetCastleState() != 7) return TRUE;	
	if (lpObj->m_btCsJoinSide < 2 || lpObj->m_bCsGuildInvolved == FALSE) return TRUE;
	if ((lpObj->Y - lpNpc->Y) > 3 || (lpObj->X - lpNpc->X) > 3) return TRUE;
	if (g_CastleSiege.GetRegCrownAvailable() == FALSE) return TRUE;
	if (g_CastleSiege.CheckUnionGuildMaster(lpObj->m_Index) == FALSE) return TRUE;
	...
	if (gObjIsConnected(iUserIndex) == FALSE) {
		...
		if (gObjIsConnected(iSwitchIndex1) == FALSE || gObjIsConnected(iSwitchIndex2) == FALSE) {
			g_Log.Add("[CastleSiege] [%s][%s] Failed to Register Castle Crown (GUILD:%s)", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->GuildName);
			return TRUE;
		}
		...
		if (lpObj->m_btCsJoinSide != gObj[iSwitchIndex1].m_btCsJoinSide || lpObj->m_btCsJoinSide != gObj[iSwitchIndex2].m_btCsJoinSide) {
			...
			g_Log.Add("[CastleSiege] [%s][%s] Failed to Register Castle Crown (GUILD:%s) (S1:%s/%s)(S2:%s/%s)", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->GuildName, gObj[iSwitchIndex1].Name, gObj[iSwitchIndex1].CharacterData->GuildName, gObj[iSwitchIndex2].Name, gObj[iSwitchIndex2].CharacterData->GuildName);
			return TRUE;
		} else {
			...
			g_CastleSiege.NotifyAllUserCsProgState(0,lpObj->CharacterData->GuildName);
			g_Log.Add("[CastleSiege] [%s][%s] Start to Register Castle Crown (GUILD:%s)", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->GuildName);
		}
	} else if (lpObj->m_Index != iUserIndex) GSProtocol.GCAnsCsAccessCrownState(lpObj->m_Index, 3);

	return TRUE;
}
BOOL NPC_CrownRoomSwitch(LPOBJ lpNpc, LPOBJ lpObj) {
	if (g_ConfigRead.server.GetServerType() != SERVER_CASTLE) return TRUE;
	if (gObjIsConnected(lpObj->m_Index) == FALSE) return TRUE;
	if (g_CastleSiege.GetCastleState() != 7) return TRUE;	
	if (lpObj->m_btCsJoinSide < 2) return TRUE;
	if ((lpObj->Y - lpNpc->Y) > 3 || (lpObj->X - lpNpc->X) > 3) return TRUE;
	...
	if (!gObjIsConnected(iUserIndex)) {
		...
		g_Log.Add("[CastleSiege] [%s][%s] Start to Push Castle Crown Switch (GUILD:%s) - CS X:%d/Y:%d", lpObj->AccountID, lpObj->Name, lpObj->CharacterData->GuildName, lpNpc->X, lpNpc->Y);
	} else if (lpObj->m_Index != iUserIndex) GSProtocol.GCAnsCsAccessSwitchState(lpObj->m_Index, lpNpc->m_Index, iUserIndex, 2);

	return TRUE;
}
BOOL NPC_SeedMaster(LPOBJ lpNpc, LPOBJ lpObj) {
	PMSG_TALKRESULT pMsg;
	...
	lpObj->bIsChaosMixCompleted = false;
	gObjInventoryTrans(lpObj->m_Index);
	g_Log.Add("[SeedMaster][%s][%s] Open Chaos Box", lpObj->AccountID, lpObj->Name);
	gObjItemTextSave(lpObj);
	gObjWarehouseTextSave(lpObj);
	GSProtocol.GCAnsCsMapSvrTaxInfo(lpObj->m_Index, 1, g_CastleSiegeSync.GetTaxRateChaos(lpObj->m_Index));
	IOCP.DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
...
BOOL NPC_SeedResearcher(LPOBJ lpNpc, LPOBJ lpObj) {
	PMSG_TALKRESULT pMsg;
	...
	lpObj->bIsChaosMixCompleted = false;
	gObjInventoryTrans(lpObj->m_Index);
	g_Log.Add("[SeedResearcher][%s][%s] Open Chaos Box", lpObj->AccountID, lpObj->Name);
	gObjItemTextSave(lpObj);
	gObjWarehouseTextSave(lpObj);
	GSProtocol.GCAnsCsMapSvrTaxInfo(lpObj->m_Index, 1, g_CastleSiegeSync.GetTaxRateChaos(lpObj->m_Index));
	IOCP.DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
...
bool NPC_Titus(LPOBJ lpNpc, LPOBJ lpObj) {
	if (lpObj->m_IfState.use > 0) return TRUE;
...
bool NPC_Lugard(LPOBJ lpNpc, LPOBJ lpObj) {
	if (gObjIsConnected(lpObj) == false) return false;
	if (g_DoppelGanger.IsEventEnable() == false) return false;

	g_DoppelGanger.PlatformLugardAct(lpNpc, lpObj);
	return true;
}
bool NPC_ForeignDimensionSilverBox(LPOBJ lpNpc, LPOBJ lpObj) {
	if (gObjIsConnected(lpObj) == false) return false;

	g_DoppelGanger.MiddleTreasureAct(lpNpc, lpObj);
	return true;
}
bool NPC_ForeignDimensionGoldenBox(LPOBJ lpNpc, LPOBJ lpObj) {
	if (gObjIsConnected(lpObj) == false) return false;

	g_DoppelGanger.LastTreasureAct(lpNpc, lpObj);
	return true;
}
bool NPC_Jerint(LPOBJ lpNpc, LPOBJ lpObj) {
	PMSG_TALKRESULT pMsg;
...
bool NPC_Julia(LPOBJ lpNpc, LPOBJ lpObj) {
	PMSG_TALKRESULT pMsg;
...
BOOL NPC_LandOfTrialsGatekeeper(LPOBJ lpNpc, LPOBJ lpObj) {
	if (gObjIsConnected(lpObj->m_Index) == FALSE) return TRUE;
	if ((lpObj->m_IfState.use) > 0) return TRUE;
	if (g_ConfigRead.server.GetServerType() != SERVER_CASTLE) return TRUE;
	...
	if (g_CastleSiege.GetHuntZoneEnter()) pResult.btUsable = TRUE;
	if (lpObj->CharacterData->lpGuild != NULL) {
		if (g_CastleSiege.CheckCastleOwnerUnionMember(lpObj->m_Index)) pResult.btResult = 2;
		if (g_CastleSiege.CheckCastleOwnerMember(lpObj->m_Index)) pResult.btResult = 3;
	} else {
		if (g_CastleSiege.GetHuntZoneEnter()) pResult.btResult = TRUE;
	}
...
bool NpcDavid(LPOBJ lpNpc, LPOBJ lpObj) {
	...
	lpObj->m_IfState.use = 1;
	lpObj->bIsChaosMixCompleted = false;
	g_Log.Add("[LuckyItem Master][%s][%s] Open Chaos Box", lpObj->AccountID, lpObj->Name);
	gObjItemTextSave(lpObj);
	gObjWarehouseTextSave(lpObj);
	GSProtocol.GCAnsCsMapSvrTaxInfo(lpObj->m_Index, 1, g_CastleSiegeSync.GetTaxRateChaos(lpObj->m_Index));
	IOCP.DataSend(lpObj->m_Index,(LPBYTE)&pMsg,pMsg.h.size);
...
bool NpcSantaClause(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	if (lpObj->CharacterData->m_iResets < g_ConfigRead.data.common.XMasSantaMinReset)
	{
		return TRUE;
	}
...
bool NpcElementalMaster(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	lpObj->m_IfState.state = 0;
	lpObj->m_IfState.use = 1;
	lpObj->CharacterData->m_bIsPentagramMixCompleted = false;
...
bool NpcShadowPhantom(int iObjIndex) //-> Complete
{
	...
	if (lpObj->Level > g_iShadowPhantomMaxLevel || lpObj->CharacterData->ChangeUP == 2)
	{
	...
	BUFF_AddBuffEffect(lpObj, BUFFTYPE_ATTDEF_POWER_INC, EFFECTTYPE_INCREASE_DAMAGE, iIncleaseAttackPower, EFFECTTYPE_INCREASE_DEFENSE, iIncleaseDefensePower, (lpObj->Level / 6 + 30) * 60);
	return true;
}
