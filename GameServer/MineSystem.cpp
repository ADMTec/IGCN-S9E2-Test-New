...
void CMineSystem::LoadMiningDropScript() {
	...
	for (pugi::xml_node twinkle = successinfo.child("Twinkle"); twinkle; twinkle = twinkle.next_sibling()) {
		...
		if (stMineSuccessInfo.wTwinkleType < 605 || stMineSuccessInfo.wTwinkleType > 607) g_Log.MsgBox("[Mining System] Invalid Monster Index");
	...
	for (pugi::xml_node twinkle = rewardinfo.child("Twinkle"); twinkle; twinkle = twinkle.next_sibling()) {
		...
		if (stRewardValue.wTwinkleType < 605 || stRewardValue.wTwinkleType > 607) g_Log.MsgBox("[Mining System] Invalid Monster Index");
	...
	for (pugi::xml_node repair = repairvalue.child("Repair"); repair; repair = repair.next_sibling()) {
		...
		if (iMaxRepairValue < stRepairValue.sRepairValue) g_Log.MsgBox("[Mining System] The repair value is over the max repair value");
...
void CMineSystem::CheckIsUPTUserWhenDisconnected(int aIndex) { 
	...
	while (true) {
		if (it == this->m_mapTwinkle.end()) return;
		if (it->second.wUserIndex == aIndex) break;
		it++;
	}

	if (it->second.iCurrentStage > 0) {
		g_Log.Add("[MineSystem][CheckIsUPTUserWhenDisconnect][Request Insert Reward][%s][%s][Type:%d][Index:%d][Stage:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, it->second.wTwinkleType, it->second.wTwinkleIndex, it->second.iCurrentStage);
...
void CMineSystem::GiveRewardItemToUPTUser(int aIndex, WORD wTwinkleType, int iStage) {
	...
	g_Log.Add("[MineSystem][GiveRewardItemToUPTUser][START][%s][%s][Type:%d][Stage:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, wTwinkleType, iStage);

	if (this->GiveRewardItem(aIndex, wTwinkleType, iStage, 0)) {
		g_Log.Add("[MineSystem][GiveRewardItemToUPTUser][END][%s][%s][Type:%d][Stage:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, wTwinkleType, iStage);
		this->RequestDBToModifyUPTUserInfo(aIndex, gObj[aIndex].Name, 0, 0, 1);
	} else g_Log.Add("[MineSystem][GiveRewardItemToUPTUser][ERROR][%s][%s][Type:%d][Stage:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, wTwinkleType, iStage);
}

void CMineSystem::MineTwinkle(PMSG_REQ_MINETWINKLE *lpMsg, int aIndex) {
	if (gObj[aIndex].Type != OBJ_USER) return;
	if (gObj[aIndex].CharacterData->m_ReqWarehouseOpen || gObj[aIndex].CloseType != -1) return;
	...
	if (this->CheckValidationMineState(aIndex, lpMsg->wTwinkleIndex, lpMsg->wTwinkleType, lpMsg->byMapNumber, lpMsg->wUserIndex, &iMineResult, 0)) {
		...
		if (gObj[aIndex].pInventory[0].m_Durability >= itMineSuccessInfo->second.iDurabilityDecrement) {
			...
			if (this->SuccessOrFailure(it->second.wTwinkleType, it->second.iCurrentStage)) {
				...
				gObj[aIndex].CharacterData->m_bIsMining = true;
				it->second.iCurrentStage++;
				if (it->second.iCurrentStage == 5) iMineResult = 1;
				else {
					iMineResult = 0;
					std::vector<_ST_MINESYSTEM_REWARD_VALUE>::iterator itDropInformation = this->m_vecRewardValue.begin();

					while (true) {
						if (itDropInformation == this->m_vecRewardValue.end()) break;
				...
				g_Log.Add("[MineSystem][MineTwinkle][FAIL][%s][%s][Type:%d][Index:%d][Stage:%d][TotalDecrementDur:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, it->second.wTwinkleType, it->second.wTwinkleIndex, it->second.iCurrentStage, iTotalUseDurability);
			...
			g_Log.Add("[MineSystem][CheckValidationMineState][ERROR][MINESYSTEM_RESULT_LACK_OF_PICKAX_DURABILITY][%s][%s][Type:%d][Index:%d][Stage:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, it->second.wTwinkleType, it->second.wTwinkleIndex, it->second.iCurrentStage);
...
void CMineSystem::RewardMineTwinkle(PMSG_REQ_MINETWINKLE_REWARD *lpMsg, int aIndex) {
	...
	if (!this->CheckValidationMineState(aIndex, lpMsg->wTwinkleIndex, lpMsg->wTwinkleType, lpMsg->byMapNumber, lpMsg->wUserIndex, &iRewardResult, true)) {
		if (iRewardResult == 20 || iRewardResult == 7 || iRewardResult == 14) return;
	...
	if (lpMsg->byIsStopMine) {
		if (this->GiveRewardItem(aIndex, it->second.wTwinkleType, it->second.iCurrentStage, 1)) {
			...
			iRewardResult = 3;
			g_Log.Add("[MineSystem][RewardMineTwinkle][GiveRewardItem][END][%s][%s][Type:%d][Index:%d][Stage:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, it->second.wTwinkleType, it->second.wTwinkleIndex, it->second.iCurrentStage);
		} else {
			iRewardResult = 4;
			g_Log.Add("[MineSystem][RewardMineTwinkle][GiveRewardItem][ERROR][%s][%s][Type:%d][Index:%d][Stage:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, it->second.wTwinkleType, it->second.wTwinkleIndex, it->second.iCurrentStage);
		}
	}
...
void CMineSystem::FailMineTwinkle(PMSG_ANS_MINETWINKLE_END_ANIMATION *lpMsg, int aIndex, bool bError) {
	...
	if (it == this->m_mapTwinkle.end()) return;
	if (!ObjectMaxRange(lpMsg->wTwinkleIndex)) return;
	if (this->IsTwinkle(it->second.wTwinkleType) == TRUE) return;
	if (gObj[lpMsg->wTwinkleIndex].Life == 0.0 && gObj[lpMsg->wTwinkleIndex].MapNumber == lpMsg->byMapNumber) gObjLifeCheck(&gObj[lpMsg->wTwinkleIndex], &gObj[aIndex], 0, 1, 0, 0, 0, 0, 0);
}
...
void CMineSystem::GDAnsModifyMineSystemUPTUserInfo(SDHP_ANS_MINESYSTEM_UPT_USERINFO *lpMsg) {
	...
	if (lpMsg->Result == 0) g_Log.Add("[MineSystem][UPT Info][DELETE/INSERT][ERROR][ID:%s][Name:%s]", gObj[lpMsg->wUserIndex].AccountID, gObj[lpMsg->wUserIndex].Name);
	else if (lpMsg->Result == 1) {
		if (lpMsg->byRequestType == 0) g_Log.Add("[MineSystem][UPT Info][INSERT][SUCCESS][%d]", lpMsg->wUserIndex);
		else if (lpMsg->byRequestType == 1) g_Log.Add("[MineSystem][UPT Info][DELETE][SUCCESS][ID:%s][Name:%s]", gObj[lpMsg->wUserIndex].AccountID, gObj[lpMsg->wUserIndex].Name);
		else g_Log.Add("[MineSystem][UPT Info][DELETE][ERROR][Undefined Request Type][ID:%s][Name:%s]", gObj[lpMsg->wUserIndex].AccountID, gObj[lpMsg->wUserIndex].Name);
	}
}

void CMineSystem::GDAnsLoadMineSystemUPTUserInfo(SDHP_ANS_LOAD_MINESYSTEM_UPT_USERINFO *lpMsg) {
	if (lpMsg->byResult == TRUE) this->GiveRewardItemToUPTUser(lpMsg->wUserIndex, lpMsg->wTwinkleType, lpMsg->iCurrentStage);
}
...
BOOL CMineSystem::CheckMoveMapWhileMining(int aIndex) {
	...
	while (true) {
		if (it == this->m_mapTwinkle.end()) break;
...
void CMineSystem::SetTwinklesInfo() {
	...
	for (std::map<int, _ST_MINESYSTEM_TWINKLE_SPAWN>::iterator It = this->m_mapTwinkleSpawn.begin(); It != this->m_mapTwinkleSpawn.end(); It++) {
		if (g_MapServerManager.CheckMapCanMove(It->second.byMapNumber) == FALSE) continue;
		...
		if (gObj[result].Dir == (BYTE)-1) gObj[result].Dir = rand() % 8;
...
void CMineSystem::SetTwinkleInfo(int aIndex) {
	if (!ObjectMaxRange(aIndex)) g_Log.Add("[MineSystem][SetTwinkleInfo][ERROR][Invalid TwinkleIndex]");
...
BOOL CMineSystem::SuccessOrFailure(WORD wType, int iStage) {
	...
	if (iStage >= 0 && iStage <= 5) {
		...
		if (it == this->m_mapMineSuccessInfo.end()) g_Log.Add("[MineSystem][SuccessOrFailure][ERROR][MINESYSTEM_RESULT_INVALID_TWINKLE_TYPE][Type:%d][Stage:%d]", wType, iStage);
		else {
			if (it->second.wTwinkleType == wType) {
				if ((GetLargeRand() % this->m_iChanceStartValue) < it->second.iSuccessRate[iStage]) IsSuccess = TRUE;
			} else g_Log.Add("[MineSystem][SuccessOrFailure][ERROR][MINESYSTEM_RESULT_NOT_MATCH_TWINKLE_TYPE][Type:%d][Stage:%d]", wType, iStage);
		}
	} else return FALSE;
	return IsSuccess;
}
...
void CMineSystem::ResetTwinkleInfo(int aIndex, WORD wTwinkleIndex, bool bFailMining) {
	...
	if (it == this->m_mapTwinkle.end()) return;
	g_Log.Add("[MineSystem][ResetTwinkleInfo][Type:%d][Index:%d][Map:%d][UserIndex:%d]", it->second.wTwinkleType, it->second.wTwinkleIndex, it->second.byMapNumber, it->second.wUserIndex);
	memset(&it->second, 0, sizeof(it->second));
	gObj[wTwinkleIndex].Life = 0.0;
	if (!bFailMining) gObjLifeCheck(&gObj[wTwinkleIndex], &gObj[aIndex], 0, 1, 0, 0, 0, 0, 0);
	gObj[aIndex].CharacterData->m_bIsMining = false;
}
...
BOOL CMineSystem::CheckValidationMineState(int aIndex, WORD wTwinkleIndex, WORD wTwinkleType, BYTE byMapNumber, WORD wUserIndex, int *iResult, bool bRewardCheck) {
	...
	if (!ObjectMaxRange(wUserIndex) || aIndex != wUserIndex) {
		g_Log.Add("[MineSystem][CheckValidationMineState][ERROR][MINESYSTEM_RESULT_INVALID_USERINDEX][%s][%s][Map:%d][Type:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, byMapNumber, wTwinkleIndex);
		*iResult = 20;
	...
	if (it == this->m_mapTwinkle.end()) {
		g_Log.Add("[MineSystem][CheckValidationMineState][ERROR][MINESYSTEM_RESULT_INVALID_TWINKLE_INDEX][%s][%s][Map:%d][Index:%d][Type:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, byMapNumber, wTwinkleIndex, wTwinkleType);
		*iResult = 14;
		GSProtocol.GCSendDisableReconnect(aIndex);
		//IOCP.CloseClient(aIndex);
		return FALSE;
	...
	if (bRewardCheck == false) {
		if (!this->IsEquipPickax(aIndex)) {
			g_Log.Add("[MineSystem][CheckValidationMineState][ERROR][MINESYSTEM_RESULT_NOT_EQUIPT_PICKAX][%s][%s][Map:%d][Type:%d][Index:%d][Stage:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, byMapNumber, it->second.wTwinkleType, it->second.wTwinkleIndex, it->second.iCurrentStage);
			*iResult = 7;
			GSProtocol.GCSendDisableReconnect(aIndex);
			//IOCP.CloseClient(aIndex);
			return FALSE;
		...
		if (gObj[aIndex].pInventory[10].IsItem() == TRUE) {
			...
			if (p->ItemKindB == 33) {
				g_Log.Add("[MineSystem][CheckValidationMineState][ERROR][MINESYSTEM_RESULT_EQUIP_TRANSFORM_RING][%s][%s][Map:%d][Type:%d][Index:%d][Stage:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, byMapNumber, it->second.wTwinkleType, it->second.wTwinkleIndex, it->second.iCurrentStage);
				*iResult = 10;
				return FALSE;
			}
		}
	...
	if (gObj[aIndex].pTransaction == 1 || gObj[aIndex].m_bPShopTransaction == true || gObj[aIndex].m_bPShopWantDeal == true) {
		g_Log.Add("[MineSystem][CheckValidationMineState][ERROR][MINESYSTEM_RESULT_REQUEST_TRADE][%s][%s][Map:%d][Type:%d][Index:%d][Stage:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, byMapNumber, it->second.wTwinkleType, it->second.wTwinkleIndex, it->second.iCurrentStage);
		*iResult = 12;
	...
	if (gObj[aIndex].m_bPShopOpen == true) {
		g_Log.Add("[MineSystem][CheckValidationMineState][ERROR][MINESYSTEM_RESULT_OPEN_PSHOP][%s][%s][Map:%d][Type:%d][Index:%d][Stage:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, byMapNumber, it->second.wTwinkleType, it->second.wTwinkleIndex, it->second.iCurrentStage);
		*iResult = 13;
	...
	int iTotalInvenCount = 8 * (4 * gObj[aIndex].CharacterData->m_InventoryExpansion + 8);

	for (int x = 0; x < iTotalInvenCount; x++) {
		if (gObj[aIndex].pInventoryMap[x] == 0xFF) iEmptyInvenSize++;
	}

	if (iEmptyInvenSize < 40) {
		g_Log.Add("[MineSystem][CheckValidationMineState][ERROR][MINESYSTEM_RESULT_LACK_OF_INVENTORY][%s][%s][Map:%d][Type:%d][Index:%d][Stage:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, byMapNumber, it->second.wTwinkleType, it->second.wTwinkleIndex, it->second.iCurrentStage);
		*iResult = 11;
		return FALSE;
	}

	if (it->second.bIsDominated == true && aIndex != it->second.wUserIndex) {
		g_Log.Add("[MineSystem][CheckValidationMineState][ERROR][MINESYSTEM_RESULT_DOMINATED_OTHER_USER][%s][%s][Map:%d][Type:%d][Index:%d][Stage:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, byMapNumber, it->second.wTwinkleType, it->second.wTwinkleIndex, it->second.iCurrentStage);
		*iResult = 8;
		return FALSE;
	}
	...
	while (true) {
		if (itTwinkle == this->m_mapTwinkle.end()) break;
		if (itTwinkle->second.bIsDominated == true && itTwinkle->second.wUserIndex == aIndex) iDominateTwinkleCount++;
		if (iDominateTwinkleCount > 1) {
			g_Log.Add("[MineSystem][CheckValidationMineState][ERROR][MINESYSTEM_RESULT_DOMINATED_OTHER_TWINKLE][%s][%s][Map:%d][Type:%d][Index:%d][Stage:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, byMapNumber, it->second.wTwinkleType, it->second.wTwinkleIndex, it->second.iCurrentStage);
			*iResult = 9;

			std::map<int, _ST_MINESYSTEM_TWINKLE>::iterator itError = this->m_mapTwinkle.begin();

			while (true) {
				if (itError == this->m_mapTwinkle.end()) break;
	...
	if (wTwinkleType != it->second.wTwinkleType) {
		g_Log.Add("[MineSystem][CheckValidationMineState][ERROR][MINESYSTEM_RESULT_NOT_MATCH_TWINKLE_TYPE][%s][%s][Map:%d][Type:%d][Index:%d][Stage:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, byMapNumber, it->second.wTwinkleType, it->second.wTwinkleIndex, it->second.iCurrentStage);
		*iResult = 17;
	...
	if (it->second.iCurrentStage < 0 || it->second.iCurrentStage > 5) {
		g_Log.Add("[MineSystem][CheckValidationMineState][ERROR][MINESYSTEM_RESULT_INVALID_STAGE][%s][%s][Map:%d][Type:%d][Index:%d][Stage:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, byMapNumber, it->second.wTwinkleType, it->second.wTwinkleIndex, it->second.iCurrentStage);
		*iResult = 19;
	...
	if (byMapNumber != it->second.byMapNumber || gObj[aIndex].MapNumber != it->second.byMapNumber) {
		g_Log.Add("[MineSystem][CheckValidationMineState][ERROR][MINESYSTEM_RESULT_NOT_MATCH_MAPNUMBER][%s][%s][Map:%d][Type:%d][Index:%d][Stage:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, byMapNumber, it->second.wTwinkleType, it->second.wTwinkleIndex, it->second.iCurrentStage);
		*iResult = 16;
	...
	if (abs(gObj[aIndex].X - gObj[wTwinkleIndex].X) > 3 || abs(gObj[aIndex].Y - gObj[wTwinkleIndex].Y) > 3) {
		g_Log.Add("[MineSystem][CheckValidationMineState][ERROR][MINESYSTEM_RESULT_INVALID_DISTANCE][%s][%s][Map:%d][Type:%d][Index:%d][Stage:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, byMapNumber, it->second.wTwinkleType, it->second.wTwinkleIndex, it->second.iCurrentStage);
		*iResult = 21;
...
BOOL CMineSystem::GiveRewardItem(int aIndex, WORD wTwinkleType, int iStage, int bNotify) {
	if (gObj[aIndex].Type != OBJ_USER || gObj[aIndex].Connected != PLAYER_PLAYING) return FALSE;
	...
	if (this->MiracleSuccessOrFailure(wTwinkleType, iStage) == TRUE) {
		...
		g_Log.Add("[MineSystem][GiveRewardItem][Miracle][TwinkleType:%d][Stage:%d][%s][%s]", wTwinkleType, iStage, gObj[aIndex].AccountID, gObj[aIndex].Name);
	} else {
	...
	while (itDropInformation != itDropInformation_end) {
		if (itDropInformation->wTwinkleType == wTwinkleType && itDropInformation->iStage == iStage) {
			g_Log.Add("[MineSystem][GiveRewardItem][START][%s][%s][Type:%d][Stage:%d][ItemType:%d][ItemIndex:%d][Count:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, wTwinkleType, iStage, itDropInformation->wType, itDropInformation->wIndex, itDropInformation->iTotalJewelNumber);
			bGaveReward = true;

			std::vector<_ST_MINESYSTEM_DROP_ITEM>::iterator itDropItem = itDropInformation->vecDropItem.begin();

			while (true) {
				if (itDropItem == itDropInformation->vecDropItem.end()) break;
				for (int i = 0; i < itDropItem->iNumber; i++) {
					...
					g_Log.Add("[MineSystem][GiveRewardItem][%s][%s][Item Type:%d][Index:%d][Level:%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, itDropItem->iItemType, itDropItem->iItemIndex, itDropItem->iLevel);
				}
	...
	if (bIsNotifyAllUser == true && bNotify == true) {
		...
		if (itMapName != this->m_mapMapName.end()) {
			char szTemp[256];
			sprintf(Lang.GetText(0,588), gObj[aIndex].Name, ItemAttribute[ITEMGET(itDropInformation->wType, itDropInformation->wIndex)].Name, itDropInformation->iTotalJewelNumber, g_ConfigRead.server.GetGameServerCode() % 20 + 1, itMapName->second.c_str());
...
BOOL CMineSystem::MiracleSuccessOrFailure(WORD wType, int iStage) {
	if (iStage < 0 || iStage > 5) return FALSE;	
	BOOL IsSuccess = FALSE;

	std::vector<_ST_MINESYSTEM_MINE_MIRACLE_SUCCESS_INFO>::iterator it = this->m_vecMiracleSuccessInfo.begin();

	while (true) {
		if (it == this->m_vecMiracleSuccessInfo.end()) break;
		if (it->wTwinkleType == wType && it->iStage == iStage) {
			if ((GetLargeRand() % this->m_iChanceStartValue) < it->iSuccessRate) IsSuccess = TRUE;
			break;
		}
...
void CMineSystem::ChangeUnconditionalSuccess(int iIsSuccess) {
	if (iIsSuccess == TRUE) this->m_iChanceStartValue = 1;
	else this->m_iChanceStartValue = this->m_iChanceStartValue_Copy;
...
short CMineSystem::GetRepairValueOfJewel(WORD sItemType, WORD sLevel) {
	...
	while (true) {
		if (it == this->m_vecRepairValue.end()) break;
		if (sItemType == ITEMGET(it->sItemType, it->sItemIndex) && sLevel == it->sLevel) return it->sRepairValue;
		it++;
	}
