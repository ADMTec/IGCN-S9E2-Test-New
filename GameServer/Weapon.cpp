...
CWeapon::CWeapon() {
	for (int i = 0; i < MAX_WEAPON_DAMAGED_TARGET_INFO; i++) this->m_WeaponDamagedTargetInfo[i].RESET();
	for (int i = 0; i < MAX_WEAPON_CAL_DAMAGER_INFO; i++) this->m_WeaponCalDamageInfo[i].RESET();
}
CWeapon::~CWeapon() {
	return;
}
...
int CWeapon::CalcAttackDamage(int iObjClass) {
	int iDamage = 0;

	if (iObjClass == 221) iDamage = gWeaponAttackDamage;
	else if (iObjClass == 222) iDamage = gWeaponDefenseDamage;
...
BOOL CWeapon::ATTACK_MissCheck(LPOBJ lpObj, LPOBJ lpTargetObj, int iSkill, int iSkillSuccess, BOOL& bAllMiss) {
	return TRUE;
}
BOOL CWeapon::Attack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf * lpMagic, int iCriticalDamage, int iActionType) {
	...
	if (lpMagic != NULL) iSkill = lpMagic->m_Skill;
	if ((lpTargetObj->Authority & 2) == 2 || (lpTargetObj->Authority & 0x20) == 0x20) return FALSE;
	if (lpObj->MapNumber != lpTargetObj->MapNumber) return FALSE;
	if (lpTargetObj->Type == OBJ_USER) {
		if (gObjIsConnected(lpTargetObj) == FALSE) return FALSE;
	}

	if (gObjAttackQ(lpTargetObj) == 0) return FALSE;

	lpObj->m_TotalAttackCount++;

	int MSBFlag = 0;

	if (iCriticalDamage != 0) MsgDamage = 3;
	else MsgDamage = 0;

	int iAttackDamage = this->CalcAttackDamage(lpObj->Class);
	int iTargetDefense = lpTargetObj->Defense;
	int iDecreaseDefense = BUFF_GetTotalBuffEffectValue(lpObj, EFFECTTYPE_DECREASE_DEFENSE);

	iTargetDefense -= (iTargetDefense * iDecreaseDefense)/100;

	BOOL bAllMiss = FALSE;
	
	if (this->ATTACK_MissCheck(lpObj, lpTargetObj, iSkill, iSkillSuccess, bAllMiss) == FALSE) return FALSE;
	if (bAllMiss != FALSE) iAttackDamage = (iAttackDamage * 30) / 100;
	if (lpTargetObj->EXC_ReduceDamageTaken != 0) {
		int lc11 = iAttackDamage;

		iAttackDamage -= (iAttackDamage * lpTargetObj->EXC_ReduceDamageTaken)/100;
	}

	int iTargetLevel = lpObj->Level / 10;

	if (iAttackDamage < iTargetLevel) {
		if (iTargetLevel < 1) iTargetLevel = 1;

		iAttackDamage = iTargetLevel;
	}

	if (lpTargetObj->m_SkillNumber == 18) {
		if (iAttackDamage > 1) iAttackDamage = iAttackDamage >> 1;
	}
	...
	if (CHECK_Dinorant(lpTargetObj) != FALSE) {
		CItem * Dino = &lpTargetObj->pInventory[8];
		int ReduceDamage = 90 - Dino->IsDinorantReduceAttackDamaege();

		lpObj->Life -= 1.0f;

		if (lpObj->Life < 0.0f) lpObj->Life = 0;
		else iAttackDamage = (iAttackDamage * ReduceDamage) / 100;

		GSProtocol.PROTOCOL_ReFillSend(lpObj->m_Index, lpObj->Life, 0xFF, 0, lpObj->iShield);
	}

	if (CHECK_DarkHorse(lpTargetObj) != FALSE) {
		CItem * DarkHorse = &lpTargetObj->pInventory[8];
		int DHPercent = 100-(DarkHorse->m_PetItem_Level + 30) / 2;

		lpTargetObj->Life -= 1.0f;

		if (lpTargetObj->Life < 0.0f) lpTargetObj->Life = 0;
		else iAttackDamage = (iAttackDamage * DHPercent) / 100;

		GSProtocol.PROTOCOL_ReFillSend(lpTargetObj->m_Index, lpTargetObj->Life, 0xFF, 0, lpTargetObj->iShield);
	}

	if (lpTargetObj->Live != FALSE) {
		if ((BUFF_TargetHasBuff(lpTargetObj, BUFFTYPE_MANA_SHIELD) || BUFF_TargetHasBuff(lpTargetObj, BUFFTYPE_IMPROVED_MANA_SHIELD)) && iAttackDamage > 0) {
			int iWizardSkillDefense = 0;
			int buffIndex = BUFFTYPE_MANA_SHIELD;

			if (BUFF_TargetHasBuff(lpTargetObj, BUFFTYPE_IMPROVED_MANA_SHIELD)) buffIndex = BUFFTYPE_IMPROVED_MANA_SHIELD;

			BUFF_GetBuffEffectValue(lpTargetObj, buffIndex, &iWizardSkillDefense, 0);
			...
			if (iWizardSkillDefense > 0) {
				...
				if (decattackdamage < lpTargetObj->Mana) {
					...
					decattackdamage -= lpTargetObj->Mana;
					lpTargetObj->Mana = 0;
					iAttackDamage -= decattackdamage;
					ManaChange = TRUE;
				}
	...
	if (iAttackDamage >= 5) {
		if (lpTargetObj->Type == OBJ_MONSTER) {
			...
			if (MOUNT_Uniria(lpTargetObj) == FALSE) MSBFlag = TRUE;
		}
	}

	if (ManaChange != FALSE) GSProtocol.PROTOCOL_ManaSend(lpTargetObj->m_Index, lpTargetObj->Mana, (BYTE)-1, 0, lpTargetObj->Stamina);

	lpObj->m_Rest = 0;

	if (iAttackDamage > 0) gObjLifeCheck(lpTargetObj, lpObj, iAttackDamage, 0, MSBFlag, MsgDamage, iSkill, 0, 0);
...
void CWeapon::WeaponAttackProc() {
	for (int i = 0; i < MAX_WEAPON_DAMAGED_TARGET_INFO; i++) {
		if (this->m_WeaponDamagedTargetInfo[i].m_IsUsed == TRUE) {
			if (this->m_WeaponDamagedTargetInfo[i].m_iDamageDealingTime > GetTickCount()) continue;
	...
	for (int i = 0; i < MAX_WEAPON_CAL_DAMAGER_INFO; i++) {
		if (this->m_WeaponCalDamageInfo[i].m_IsUsed == TRUE) {
			if (this->m_WeaponCalDamageInfo[i].m_iWeaponObjIndex == iIndex) {
				if (this->m_WeaponCalDamageInfo[i].m_iLimitTime < GetTickCount()) this->m_WeaponCalDamageInfo[i].RESET();
			}
		}
	}
}