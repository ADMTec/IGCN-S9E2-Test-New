					...
					for (int j = 0; j < 20; j++)
					{
							...
							gObj[this->m_UserData[j].m_nIndex].PartyNumber < 0)
						{
							...
							g_Log.Add("[DSF][Proc_DSFState_None][%s][%s][%s][%d] Not PartyUser Kick Out",
								gObj[this->m_UserData[j].m_nIndex].AccountID, gObj[this->m_UserData[j].m_nIndex].CharacterData->m_RealNameOfUBF,
								gObj[this->m_UserData[j].m_nIndex].Name, gObj[this->m_UserData[j].m_nIndex].CharacterData->m_nServerCodeOfHomeWorld);
						}
					}
...
void CDevilSquareFinal::Set_DSFState_Ready()
{
	...
	for (int i = 0; i < 10; i++)
	{
		if (this->m_PartyDataSave[i].nPartyNo != -1)
		{
			...
			memcpy(PartyUserName.szUserName1, gObj[this->m_PartyDataSave[i].nRequestUserIndex].CharacterData->m_RealNameOfUBF, MAX_ACCOUNT_LEN+1);
			memcpy(PartyUserName.szUserName2, gObj[this->m_PartyDataSave[i].n2ndPartyUserIndex].CharacterData->m_RealNameOfUBF, MAX_ACCOUNT_LEN+1);
...
char CDevilSquareFinal::SetParty(BYTE btSlotOrder, BYTE btDSFType, int nPartyIdx, int nUserIndex)
{
	...
	memcpy(this->m_PartyDataSave[btSlotOrder].szRequestUserRealName, gObj[nUserIndex].CharacterData->m_RealNameOfUBF, MAX_ACCOUNT_LEN+1);
	this->m_PartyDataSave[btSlotOrder].nRequestUserServerCode = gObj[nUserIndex].CharacterData->m_nServerCodeOfHomeWorld;
	this->m_PartyDataSave[btSlotOrder].nRequestUserClass = gObj[nUserIndex].Class;
	this->m_PartyDataSave[btSlotOrder].nRequestUserLevel = gObj[nUserIndex].CharacterData->MasterLevel + gObj[nUserIndex].Level;
	...
	for (int i = 0; i < 2; i++)
	{
		...
		if (useridx > 0 && nUserIndex != useridx)
		{
			...
			memcpy(this->m_PartyDataSave[btSlotOrder].sz2ndPartyUserRealName, gObj[useridx].CharacterData->m_RealNameOfUBF, MAX_ACCOUNT_LEN+1);
			this->m_PartyDataSave[btSlotOrder].n2ndPartyUserServerCode = gObj[useridx].CharacterData->m_nServerCodeOfHomeWorld;
			this->m_PartyDataSave[btSlotOrder].n2ndPartyUserClass = gObj[useridx].Class;
			this->m_PartyDataSave[btSlotOrder].n2ndPartyUserLevel = gObj[useridx].CharacterData->MasterLevel + gObj[useridx].Level;
		}
	}
...
BOOL CDevilSquareFinal::Enter_DSF(int aIndex, BYTE btSlotNum)
{
		...
		if (this->m_UserData[i].m_nIndex == -1)
		{
			if (lpObj->PartyNumber == this->m_PartyDataSave[btSlotNum].nPartyNo)
			{
				...
				g_Log.Add("[DSF][Enter_DSF][%s][%s][%s][%d] UserCnt:[%d], DSFIndex:[%d], Team:%d",
					gObj[aIndex].AccountID, gObj[aIndex].CharacterData->m_RealNameOfUBF, gObj[aIndex].Name,
					gObj[aIndex].CharacterData->m_nServerCodeOfHomeWorld, this->m_nUserCount, gObj[aIndex].m_nDSFIndex,
	...
	if (bResult == TRUE)
	{
		g_Log.Add("[DSF][Enter_DSF][%s][%s][%s][%d] Enter Success, class : %d",
			lpObj->AccountID, lpObj->CharacterData->m_RealNameOfUBF, lpObj->Name,
			lpObj->CharacterData->m_nServerCodeOfHomeWorld, lpObj->CharacterData->DbClass);
		...
		for (int i = 0; i < 10; i++)
		{
				...
				nUserCount == 0)
			{
				g_Log.Add("[DSF][Leave_DSF][%s][%s][%s][%d] All PartyUser Leave : Game is not start, TeamIndex:[%d], PartyUserName1:%s, PartyUserName2:%s",
					gObj[aIndex].AccountID, gObj[aIndex].CharacterData->m_RealNameOfUBF, gObj[aIndex].Name, gObj[aIndex].CharacterData->m_nServerCodeOfHomeWorld,
...
void CDevilSquareFinal::GDReqSetDSFReward_UBF(int iUserIndex, BYTE btDSFType, BYTE btRewardType)
{
	...
	g_Log.Add("[UBF][DevilSquareFinal][GDReqSetDSFWinnerInformation]ACC:%s , NAME :%s,Real NAME :%s, DSFType:%d, RewardType:%d",
		lpObj->AccountID, lpObj->Name, lpObj->CharacterData->m_RealNameOfUBF, btDSFType, btRewardType);
