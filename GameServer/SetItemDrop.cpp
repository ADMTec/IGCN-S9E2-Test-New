...
bool CSetItemDrop::DropItem(LPOBJ lpMonster, LPOBJ lpUser) {
	if (!ObjectMaxRange(lpUser->m_Index)) return false;
	if (!ObjectMaxRange(lpMonster->m_Index)) return false;
	if (GetLargeRand() % 1000000 >= this->m_dwDropUseRate) return false;
...
SETITEM_DROP_ITEM * CSetItemDrop::GetItemData(SETITEM_DROP_RULE *lpRule) {
	...
	if (iSuccessMaxRate < 1) return NULL;
