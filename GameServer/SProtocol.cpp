...
void GJPUserClose(LPOBJ lpObj) {
	...
	pClose.DbClass = lpObj->CharacterData->DbClass;
...
void JGPAccountRequest(SDHP_IDPASSRESULT * lpMsg) {
	...
	switch (lpMsg->result) {
		...
		case 15:
			if (gObjSetAccountLogin(aIndex, szId, lpMsg->UserNumber , lpMsg->DBNumber, szJN) == 0) {
				...
				g_Log.Add("Login [%s] : (%s) Numer: %d / DB: %d", gObj[aIndex].CharacterData->Ip_addr, szId, lpMsg->UserNumber , lpMsg->DBNumber);
			}
	...
	if (lpMsg->result != 1) {
		memset(gObj[aIndex].CharacterData->HWID, 0, sizeof(gObj[aIndex].CharacterData->HWID));
...
void GJPBillCeckRecv(SDHP_BILLSEARCH_RESULT * lpMsg) {
	...
	if (m_ObjBill[aIndex- g_ConfigRead.server.GetObjectStartUserIndex()].SetBill(lpMsg->cCertifyType , lpMsg->PayCode, lpMsg->EndTime , EndsDays) == TRUE) {
		...
		g_Log.Add("[%s] VIP Info Set [Type: %d] [Effect: %d]", lpObj->AccountID, lpObj->CharacterData->VipType, lpObj->CharacterData->VipEffect);
	}
}
...
void LoveHeartEventRecv(SDHP_LOVEHEARTEVENT_RESULT * lpMsg) {
	...
	switch (lpMsg->Result) {
		...
		case 1: // Declare Event Winner
			wsprintf(Msg, Lang.GetText(0,19), szName, number);
			GSProtocol.PROTOCOL_SendServerAnnounce(Msg);
			break;
	}
}
...
void AllNoticeRecv(SDHP_NOTICE * lpMsg) {
	...
	if (strlen(szNotice) > 1) {
		GSProtocol.PROTOCOL_SendServerAnnounce(szNotice);
	}
}
...
void GJReqMapSvrMove(int iIndex, WORD wDesMapSvrCode, WORD wMapNumber, BYTE btX, BYTE btY) {
	...
	if (gObjIsConnected(iIndex) == PLAYER_EMPTY) {
		...
		memcpy(pMsg.szPassword, gObj[iIndex].CharacterData->Password, sizeof(pMsg.szPassword));
		...
		pMsg.btX = btX;
		pMsg.btY = btY;
		pMsg.btSecurityLock = (BYTE)gObj[iIndex].CharacterData->m_bSecurityCheck;
		pMsg.dwSecurityCode = gObj[iIndex].CharacterData->m_iSecurityCode;
...
void JGAnsMapSvrMove(PMSG_ANS_MAPSVRMOVE * lpMsg) {
	...
	if (lpMsg->iResult == 0) {
		...
		if (bRESULT == FALSE) {
			g_Log.AddC(TColor::Red,  "[MapServerMng] Packet Error JG [0x7A] - Failed to find Map Info [%s]-[%s] (%d)", lpObj->AccountID, lpObj->Name, lpMsg->wDstMapSvrCode);
			return;
		}
		...
		g_Log.Add("[MapServerMng] [%s][%s] JG [0x7A] - Move Map Request OK : MAPSVR_D:%d", lpObj->AccountID, lpObj->Name, lpMsg->wDstMapSvrCode);
	} else {
		...
		lpObj->CharacterData->MoveGate = -1;
		lpObj->m_State = 2;
		lpObj->MapNumber = 0;
		lpObj->X = 130;
		lpObj->Y = 130;
		...
		g_Log.AddC(TColor::Red, "[MapServerMng] Packet Error JG [0x7A] - Failed to Authorize User [%s][%s] Result:%d", lpObj->AccountID, lpObj->Name, lpMsg->iResult);
...
void JGAnsMapSvrAuth(PMSG_ANS_MAPSVRAUTH * lpMsg) {
	if (!ObjectMaxRange(lpMsg->iIndex)) {
		g_Log.AddC(TColor::Red,  "[MapServerMng] Packet Error JG [0x7B] - Index out of bound : %d", lpMsg->iIndex);
		return;
	}
	...
	if (lpObj->m_bMapSvrMoveReq == false) {
		g_Log.AddC(TColor::Red,  "[MapServerMng] MapSvr MoveReq Error JG [0x7B] - No Request [%s] (%d)", lpObj->AccountID, lpObj->m_Index);
	...
	switch (lpMsg->iResult) {
		case 0:
			...
			if (!gObjSetAccountLogin(iIndex, szId, lpMsg->iUserNumber, lpMsg->iDBNumber, szPw)) {
				...
				g_Log.Add("Login [%s] : (%s) Number: %d / DB: %d", gObj[iIndex].CharacterData->Ip_addr, szId, lpMsg->iUserNumber, lpMsg->iDBNumber);
				iSendResult = 1;
				bAuthOK = TRUE;
			}
	...
	if (bAuthOK == FALSE) {
		g_Log.AddC(TColor::Red,  "[MapServerMng] [%s][%s] JG [0x7B] - Join Result Failed : RST: [%d], BLK: [%c]", szId, lpMsg->szCharName, lpMsg->iResult, lpMsg->btBlockCode);
		gObjDel(lpMsg->iIndex);
		return;
	}
	...
	gObj[iIndex].CharacterData->m_bSecurityCheck = lpMsg->btSecurityLock;
	gObj[iIndex].CharacterData->m_iSecurityCode = lpMsg->dwSecurityCode;
	...
	g_Log.Add("[MapServerMng] [%s][%s] JG [0x7B] - Join Result OK : RST:%d, BLK:%c", szId, lpMsg->szCharName, lpMsg->iResult, lpMsg->btBlockCode);
}
...
void GJUpdateMatchDBUserCharacters(LPOBJ lpObj) {
	...
	pMsg.DbClass = lpObj->CharacterData->DbClass;
