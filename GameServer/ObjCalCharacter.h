#ifndef OBJCALCHARACTER_H
#define OBJCALCHARACTER_H

#if _MSC_VER > 1000
#pragma once
#endif // _MSC_VER > 1000

#include "user.h"
#include "zzzitem.h"

class CObjCalCharacter
{
public:

	CObjCalCharacter();
	virtual ~CObjCalCharacter();
	
	void Init();

	void CHARACTER_Calc(int aIndex);
	void CHARACTER_CalcSetItemStats(LPOBJ lpObj);
	void GetSetItemOption(LPOBJ lpObj, LPBYTE pSetOptionTable, LPBYTE pSetOptionCountTable, int * pSetOptionCount);
	void CalcSetItemOption(LPOBJ lpObj);
	void CHARACTER_SetItemIncreaseStats(LPOBJ lpObj, int option, int ivalue);
	void CHARACTER_SetItemBonus(LPOBJ lpObj, int option, int ivalue);
	void CHARACTER_SetItemApply(LPOBJ lpObj);
	void CalcMLSkillItemOption(LPOBJ lpObj);
	BOOL ValidItem(LPOBJ lpObj, CItem * lpItem, int pos);
	void CHARACTER_CalcShieldPoint(LPOBJ lpObj);
	void PremiumItemApply(LPOBJ lpObj);

private:

	MULua m_Lua;
};

extern CObjCalCharacter gObjCalCharacter;
#endif