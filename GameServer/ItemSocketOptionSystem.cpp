...
void CItemSocketOptionSystem::Initialize() {
	...
	for (iClearIndex = 0; iClearIndex < 50; iClearIndex++) m_SeedData[iClearIndex].Clear();
	for (iClearIndex = 0; iClearIndex < 5; iClearIndex++) m_SphereData[iClearIndex].Clear();
	for (iClearIndex = 0; iClearIndex < 12; iClearIndex++) m_BonusSocketOptionData[iClearIndex].Clear();
...
bool CItemSocketOptionSystem::LoadOptionScript(char *pchFileName) {
	...
	for (pugi::xml_node option = bonus_option.child("Option"); option; option = option.next_sibling()) {
		...
		for (int i = 0; i < 5; i++) if (BonusOptionData.SocketSlotType[i] != NULL) BonusOptionData.SocketSlotCount++;
...
void CItemSocketOptionSystem::LoadScript() {
	...
	for (pugi::xml_attribute rate = sphere_rate.first_attribute(); rate; rate = rate.next_attribute()) m_SphereDropRate[iCount++] = rate.as_int();

	iCount = 0;

	for (pugi::xml_attribute level = sphere_level.first_attribute(); level; level = level.next_attribute()) m_SphereDropLevel[iCount++] = level.as_int();
	...
	for (pugi::xml_node socket = socketrate.child("Socket"); socket; socket = socket.next_sibling()) {
		...
		for (int i = 0; i < 5; i++) Pool.AddValue(i + 1, SlotRate[i]);
	...
	for (int i = 0; i < 5; i++) m_TwoHandWeaponMakeSocketSlotCountPool.AddValue(i + 1, SlotRate[i]);
}
...
bool CItemSocketOptionSystem::LoadSocketSlotRateFile(char *pchFileName) {
	...
	for (int i = 0; i < 5; i++) this->m_TwoHandWeaponMakeSocketSlotCountPool.AddValue(i + 1, SlotRate[i]);
}
...
BYTE CItemSocketOptionSystem::GetEmptySlotCount(CItem *lpItemData) {
	...
	for (int i = 0;i < 5; i++) if (lpItemData->m_SocketOption[i] == 0xFE) btEmptySlotCount++;
...
void CItemSocketOptionSystem::GetSocketOption(CItem *lpItemData, BYTE *pbSocketOption, BYTE& pbSocketBonusOption) {
	...
	if (lpItemData->IsItem() == FALSE || IsEnableSocketItem(lpItemData) == false) return;
	for (int i = 0; i < 5; i++) pbSocketOption[i] = lpItemData->m_SocketOption[i];
...
bool CItemSocketOptionSystem::IsSocketItem(CItem *lpItemData) {
	...
	for (int i = 0; i < 5; i++ ) if (lpItemData->m_SocketOption[i] != 0xFF)	return true;
...
bool CItemSocketOptionSystem::IsSeedItem(int ItemCode) {
	switch (ItemCode) {
		...
		case ITEMGET(12,63):
		case ITEMGET(12,64):
		case ITEMGET(12,65):			return true;
	}
...
bool CItemSocketOptionSystem::IsSphereItem(int ItemCode) {
	switch (ItemCode) {
		...
		case ITEMGET(12,72):
		case ITEMGET(12,73):
		case ITEMGET(12,74):			return true;
	}
...
bool CItemSocketOptionSystem::IsSeedSphereItem(int ItemCode) {
	...
	if (btItemType == 12) {
		if  (btItemIndex >= 100 && btItemIndex <= 129) return true;
	}
...
BYTE CItemSocketOptionSystem::GetSeedIndex(int ItemCode, BYTE ItemLevel) {
	for (int i = 0; i < 50; i++) {
		if (m_SeedData[i].m_ItemCode == ItemCode && m_SeedData[i].m_ItemLevel == ItemLevel) return m_SeedData[i].m_SeedIndex;
	}
...
bool CItemSocketOptionSystem::CheckItemForSameSeedType(CItem *lpItemData, WORD SeedSphereItemCode, BYTE ItemLevel) {
	...
	for (int i = 0; i < 5; i++) {
		if (lpItemData->m_SocketOption[i] == 0xFF || lpItemData->m_SocketOption[i] == 0xFE)	continue;
...
bool CItemSocketOptionSystem::IsEnableDropSocketItemMap(int iMapIndex) {
	switch (iMapIndex) {
		...
		case MAP_INDEX_HATCHERY:		return true;
	}
...
int CItemSocketOptionSystem::GetSphereDropInfo(int iMonsterLevel) {
	...
	if (m_SphereDropLevel[4] != 0 && m_SphereDropLevel[4] <= iMonsterLevel) iSphereLevel = 5;
	else if (m_SphereDropLevel[3] != 0 && m_SphereDropLevel[3] <= iMonsterLevel) iSphereLevel = 4;
	else if (m_SphereDropLevel[2] != 0 && m_SphereDropLevel[2] <= iMonsterLevel) iSphereLevel = 3;
	else if (m_SphereDropLevel[1] != 0 && m_SphereDropLevel[1] <= iMonsterLevel) iSphereLevel = 2;
	else if (m_SphereDropLevel[0] != 0 && m_SphereDropLevel[0] <= iMonsterLevel) iSphereLevel = 1;
	else return iItemCode;
	for (int i = 0; i < iSphereLevel; i++) if (m_SphereDropRate[i] >= GetLargeRand() % 10000) iItemCode = i + ITEMGET(12,70);
...
int CItemSocketOptionSystem::GetTetraDropInfo(int iMonsterLevel) {
	...
	if (iMonsterLevel >= m_TetraDropMinLevel[2] && iMonsterLevel <= m_TetraDropMaxLevel[2]) iTetraLevel = 3;
	else if (iMonsterLevel >= m_TetraDropMinLevel[1] && iMonsterLevel <= m_TetraDropMaxLevel[1]) iTetraLevel = 2;
	else if (iMonsterLevel >= m_TetraDropMinLevel[0] && iMonsterLevel <= m_TetraDropMaxLevel[0]) iTetraLevel = 1;
	else return iItemCode;
	for (int i = 0; i < iTetraLevel; i++) if (m_TetraDropRate[i] >= GetLargeRand() % 10000) iItemCode = ITEMGET(14,247) - i;

	return iItemCode;
}

BYTE CItemSocketOptionSystem::GetSeedIndexForSeedSpearItemCode(WORD ItemCode, BYTE ItemLevel) {
	for (int i = 0; i < 50; i++) if (this->m_SeedData[i].m_SeedType == ((ITEM_GET_INDEX(ItemCode) - 100) % 6 + 1) && this->m_SeedData[i].m_ItemLevel == ItemLevel) return this->m_SeedData[i].m_SeedIndex;
...
CSeedItem * CItemSocketOptionSystem::GetSeedData(BYTE btSeedIndex) {
	for (int i = 0; i < 50; i++) if (m_SeedData[i].m_SeedIndex == btSeedIndex) return &m_SeedData[i];
	
	return NULL;
}

CSphereItem * CItemSocketOptionSystem::GetSphereData(BYTE btSphereLevel) {
	for (int i = 0; i < 5; i++) if (m_SphereData[i].m_SphereLevel == btSphereLevel) return &m_SphereData[i];
...
BONUS_SOCKET_OPTION * CItemSocketOptionSystem::GetBonusSocketOption(BYTE btBonusSocketOptionIndex) {
	for (int i = 0 ; i < 12; i++) if (m_BonusSocketOptionData[i].OptionIndex == btBonusSocketOptionIndex) return &m_BonusSocketOptionData[i];
...
int CItemSocketOptionSystem::ApplySeedSphereEffect(LPOBJECTSTRUCT lpObj) {
	...
	for (int iEquip = 0; iEquip < INVETORY_WEAR_SIZE; iEquip++) {
		...
		for (int i = 0; i < 5; i++) {
			...
			if (SeedSphereIndex == 0xFF || SeedSphereIndex == 0xFE) continue;
...
void CItemSocketOptionSystem::ApplyBonusOptionEffect(LPOBJECTSTRUCT lpObj) {
	...
	for (int iEquip = 0; iEquip < INVETORY_WEAR_SIZE; iEquip++) {
		...
		if (btBonusSocketIndex == 0xFF || btBonusSocketIndex == 0xFE) continue;
...
void CItemSocketOptionSystem::ApplySetOptionEffect(LPOBJECTSTRUCT lpObj) {
	...
	for (int iUserSocketIndex = 0; iUserSocketIndex < 35; iUserSocketIndex++) {
		btSeedType = GetSocketType(lpObj->CharacterData->m_SeedOptionList[iUserSocketIndex].m_SocketOptionIndex);

		if (btSeedType == SEED_TYPE_NONE)	break;

		btTotalSeedTypeList[btSeedType - 1] += lpObj->CharacterData->m_SeedOptionList[iUserSocketIndex].m_SocketOptionCount;
	}

	for (int iSetOptionIndex = 0; iSetOptionIndex < 2; iSetOptionIndex++) {
		for (int i = 0; i < 6; i++) if (m_SetSocketOptionData[iSetOptionIndex].SeedTypeTable[i] <= btTotalSeedTypeList[i]) btMatchedCount++;
		if (btMatchedCount == 6) SetEffect(lpObj, SOCKET_OPTION_TYPE_SET, m_SetSocketOptionData[iSetOptionIndex].EffectType, m_SetSocketOptionData[iSetOptionIndex].OptionValueType, m_SetSocketOptionData[iSetOptionIndex].OptionValue);
...
bool CItemSocketOptionSystem::SetApplySocketEffect(LPOBJECTSTRUCT lpObj) {
	...
	if (lpObj->Type != OBJ_USER || lpObj->Connected < PLAYER_PLAYING)	return false;
...
void CItemSocketOptionSystem::SetClearSocketEffect(LPOBJECTSTRUCT lpObj) {
	...
	if (lpObj->Type != OBJ_USER || lpObj->Connected < PLAYER_PLAYING)	return;

	int iIndex = 0;

	for (iIndex = 0;iIndex < 35; iIndex++) {
		if (lpObj->CharacterData->m_SeedOptionList[iIndex].m_SocketOptionIndex == 0xFF)	break;

		lpObj->CharacterData->m_SeedOptionList[iIndex].Clear();
	}

	for (iIndex = 0;iIndex < 7; iIndex++) {
		if (lpObj->CharacterData->m_BonusOptionList[iIndex].m_SocketOptionIndex == 0xFF) break;

		lpObj->CharacterData->m_BonusOptionList[iIndex].Clear();
	}

	for (iIndex = 0;iIndex < 2; iIndex++) {
		if (lpObj->CharacterData->m_SetOptionList[iIndex].m_SocketOptionIndex == 0xFF) break;

		lpObj->CharacterData->m_SetOptionList[iIndex].Clear();
	}

	lpObj->CharacterData->m_btRefillHPSocketOption = 0;
	lpObj->CharacterData->m_btRefillMPSocketOption = 0;
	lpObj->CharacterData->m_wSocketOptionMonsterDieGetHp = 0;
	lpObj->CharacterData->m_wSocketOptionMonsterDieGetMana = 0;
	lpObj->CharacterData->m_btAGReduceRate = 0;
}
...
void CItemSocketOptionSystem::SetEffect(LPOBJECTSTRUCT lpObj, BYTE btOptionType, BYTE btEffectType, BYTE btEffectValueType, WORD wEffectValue) {
	if (btEffectType >= 0 && btEffectType <= 9) _SetFireEffect(lpObj, btEffectType, btEffectValueType, wEffectValue);
	else if (btEffectType >= 10 && btEffectType <= 15) _SetWaterEffect(lpObj, btEffectType, btEffectValueType, wEffectValue);
	else if (btEffectType >= 16 && btEffectType <= 20) _SetIceEffect(lpObj, btEffectType, btEffectValueType, wEffectValue);
	else if (btEffectType >= 21 && btEffectType <= 28) _SetWindEffect(lpObj, btEffectType, btEffectValueType, wEffectValue);
	else if (btEffectType >= 29 && btEffectType <= 33) _SetThunderEffect(lpObj, btEffectType, btEffectValueType, wEffectValue);
	else if (btEffectType >= 34 && btEffectType <= 40) _SetEarthEffect(lpObj, btEffectType, btEffectValueType, wEffectValue);
	else if (btEffectType >= 41 && btEffectType <= 43) _SetUniqueEffect(lpObj, btEffectType, btEffectValueType, wEffectValue);
	else if (btEffectType >= 44 && btEffectType <= 45) _SetSetEffect(lpObj, btEffectType, btEffectValueType, wEffectValue);
	for (int i = 0; i < 43; i++) {
		if (lpObj->CharacterData->m_SeedOptionList[i].m_SocketOptionIndex == btEffectType && lpObj->CharacterData->m_SeedOptionList[i].m_SocketOptionValueType == btEffectValueType) {
			lpObj->CharacterData->m_SeedOptionList[i].m_SocketOptionValue += wEffectValue;
			lpObj->CharacterData->m_SeedOptionList[i].m_SocketOptionCount++;
			break;
		}

		if (lpObj->CharacterData->m_SeedOptionList[i].m_SocketOptionIndex == 0xFF) {
			lpObj->CharacterData->m_SeedOptionList[i].m_SocketOptionIndex = btEffectType;
			lpObj->CharacterData->m_SeedOptionList[i].m_SocketOptionValueType = btEffectValueType;
			lpObj->CharacterData->m_SeedOptionList[i].m_SocketOptionValue = wEffectValue;
			lpObj->CharacterData->m_SeedOptionList[i].m_SocketOptionCount = 1;
			break;
		}
	}
}
...
void CItemSocketOptionSystem::_SetFireEffect(LPOBJECTSTRUCT lpObj, BYTE btEffectType, BYTE btEffectValueType, WORD wEffectValue) {
	...
	switch (btEffectType) {
		...
		case SEED_EFFECT_TYPE_ATTACK_SPEED:
			iCalcEffectValue = CalcEffectValue(lpObj->AttackSpeed, wEffectValue, btEffectValueType);
			lpObj->AttackSpeed += iCalcEffectValue;			
			iCalcEffectValue = CalcEffectValue(lpObj->MagicSpeed, wEffectValue, btEffectValueType);
			lpObj->MagicSpeed += iCalcEffectValue;
		break;
		...
		case SEED_EFFECT_TYPE_REDUCE_AG_RATE:
			lpObj->CharacterData->m_btAGReduceRate += wEffectValue;
			
			if (lpObj->CharacterData->m_btAGReduceRate > 100) lpObj->CharacterData->m_btAGReduceRate = 100;
		break;
	}
}
...
void CItemSocketOptionSystem::_SetWaterEffect(LPOBJECTSTRUCT lpObj, BYTE btEffectType, BYTE btEffectValueType, WORD wEffectValue) {
	...
	switch (btEffectType) {
		case SEED_EFFECT_TYPE_SUCCESS_BLOCK:
			iCalcEffectValue = CalcEffectValue(lpObj->EvasionRating, wEffectValue, btEffectValueType);
			lpObj->EvasionRating += iCalcEffectValue;
		break;

		case SEED_EFFECT_TYPE_BLOCK_POWER:
			iCalcEffectValue = CalcEffectValue(lpObj->Defense, wEffectValue, btEffectValueType);
			lpObj->Defense += iCalcEffectValue;
		...
		case SEED_EFFECT_TYPE_SHIELD_BLOCK_POWER:				lpObj->CharacterData->ANCIENT_DefenseWithShields += wEffectValue;				break;
		case SEED_EFFECT_TYPE_REDUCE_DAMAGE:					lpObj->EXC_ReduceDamageTaken += wEffectValue;									break;
		case SEED_EFFECT_TYPE_REFLECT_DAMAGE:					lpObj->EXC_ReflectDamageTaken += wEffectValue;									break;
	}
}
...
void CItemSocketOptionSystem::_SetIceEffect(LPOBJECTSTRUCT lpObj, BYTE btEffectType, BYTE btEffectValueType, WORD wEffectValue) {
	...
	switch (btEffectType) {
		case SEED_EFFECT_TYPE_MONSTER_KILL_REFILL_HP:
			...
			lpObj->CharacterData->m_wSocketOptionMonsterDieGetHp += iAddValue;
		break;

		case SEED_EFFECT_TYPE_MONSTER_KILL_REFILL_MP:
			iAddValue += CalcEffectValue(lpObj->MaxMana, wEffectValue, btEffectValueType);
			lpObj->CharacterData->m_wSocketOptionMonsterDieGetMana += iAddValue;
		break;

		case SEED_EFFECT_TYPE_ATTACK_SKILL_POWER:
			iAddValue = CalcEffectValue(lpObj->CharacterData->ANCIENT_SkillDamage, wEffectValue, btEffectValueType);
			lpObj->CharacterData->ANCIENT_SkillDamage += iAddValue;
		break;

		case SEED_EFFECT_TYPE_ATTACK_ACCURACY:						lpObj->HitRating += wEffectValue;											break;
	}
}
...
void CItemSocketOptionSystem::_SetWindEffect(LPOBJECTSTRUCT lpObj, BYTE btEffectType, BYTE btEffectValueType, WORD wEffectValue) {
	...
	switch (btEffectType) {
		case SEED_EFFECT_TYPE_REFILL_HP_INC:
			iAddValue = CalcEffectValue(lpObj->MaxLife, wEffectValue, btEffectValueType);
			lpObj->CharacterData->m_btRefillHPSocketOption += iAddValue;
		break;

		case SEED_EFFECT_TYPE_MAX_HP_INC:
			iAddValue = CalcEffectValue(lpObj->MaxLife, wEffectValue, btEffectValueType);
			lpObj->AddLife += iAddValue;
			GSProtocol.PROTOCOL_ReFillSend(lpObj->m_Index,(lpObj->AddLife+lpObj->MaxLife),0xFE,0,(lpObj->iMaxShield+lpObj->iAddShield));
		...
		case SEED_EFFECT_TYPE_MAX_MP_INC:
			...
			GSProtocol.PROTOCOL_ManaSend(lpObj->m_Index,(lpObj->AddMana+lpObj->MaxMana),0xFE,0,(lpObj->MaxStamina+lpObj->AddStamina));
			GSProtocol.PROTOCOL_ManaSend(lpObj->m_Index,lpObj->Mana,0xFF,0,lpObj->Stamina);
		break;

		case SEED_EFFECT_TYPE_REFILL_MP_INC:
			iAddValue = CalcEffectValue(lpObj->MaxMana, wEffectValue, btEffectValueType);
			lpObj->CharacterData->m_btRefillMPSocketOption += iAddValue;
		break;

		case SEED_EFFECT_TYPE_MAX_AG_INC:
			iAddValue = CalcEffectValue(lpObj->MaxStamina, wEffectValue, btEffectValueType);
			lpObj->AddStamina += iAddValue;
			GSProtocol.PROTOCOL_ManaSend(lpObj->m_Index,(lpObj->AddMana+lpObj->MaxMana),0xFE,0,(lpObj->MaxStamina+lpObj->AddStamina));
			GSProtocol.PROTOCOL_ManaSend(lpObj->m_Index,lpObj->Mana,0xFF,0,lpObj->Stamina);
		break;

		case SEED_EFFECT_TYPE_REFILL_AG_INC:
			iAddValue = CalcEffectValue(lpObj->CharacterData->ANCIENT_StaminaRegeneration, wEffectValue, btEffectValueType);
			lpObj->CharacterData->ANCIENT_StaminaRegeneration += iAddValue;
		break;

		case SEED_EFFECT_TYPE_MONSTER_KILL_ZEN_INC:						lpObj->MonsterDieGetMoney += wEffectValue;									break;
	}
}
...
void CItemSocketOptionSystem::_SetThunderEffect(LPOBJECTSTRUCT lpObj, BYTE btEffectType, BYTE btEffectValueType, WORD wEffectValue) {
	...
	switch (btEffectType) {
		case SEED_EFFECT_TYPE_EXCELLENT_DAMAGE_INC:
			iAddValue = CalcEffectValue(lpObj->CharacterData->ANCIENT_ExcellentDamage, wEffectValue, btEffectValueType);
			lpObj->CharacterData->ANCIENT_ExcellentDamage += iAddValue;
		break;

		case SEED_EFFECT_TYPE_EXCELLENT_DAMAGE_RATE_INC:				lpObj->C_ExcellentStrikeChance += wEffectValue;								break;
		case SEED_EFFECT_TYPE_CRITICAL_DAMAGE_INC:
			iAddValue = CalcEffectValue(lpObj->CharacterData->ANCIENT_CriticalDamage, wEffectValue, btEffectValueType);
			lpObj->CharacterData->ANCIENT_CriticalDamage += iAddValue;
		break;

		case SEED_EFFECT_TYPE_CRITICAL_DAMAGE_RATE_INC:					lpObj->C_CriticalStrikeChance += wEffectValue;								break;
	}
}
...
void CItemSocketOptionSystem::_SetEarthEffect(LPOBJECTSTRUCT lpObj, BYTE btEffectType, BYTE btEffectValueType, WORD wEffectValue) {
	...
	switch (btEffectType) {
		case SEED_EFFECT_TYPE_STRENGTH_INC: {
			iAddValue = CalcEffectValue(lpObj->CharacterData->Strength + lpObj->AddStrength, wEffectValue, btEffectValueType);
			lpObj->AddStrength += iAddValue;
		}
		break;

		case SEED_EFFECT_TYPE_DEXTERITY_INC: {
			iAddValue = CalcEffectValue(lpObj->CharacterData->Dexterity + lpObj->AddDexterity, wEffectValue, btEffectValueType);
		...
		case SEED_EFFECT_TYPE_VITALITY_INC: {
			iAddValue = CalcEffectValue(lpObj->CharacterData->Vitality + lpObj->AddVitality, wEffectValue, btEffectValueType);
			lpObj->AddVitality += iAddValue;
		}
		break;

		case SEED_EFFECT_TYPE_ENERGY_INC: {
			iAddValue = CalcEffectValue(lpObj->CharacterData->Energy + lpObj->AddEnergy, wEffectValue, btEffectValueType);
...
void CItemSocketOptionSystem::_SetUniqueEffect(LPOBJECTSTRUCT lpObj, BYTE btEffectType, BYTE btEffectValueType, WORD wEffectValue) {
	...
	switch (btEffectType) {
		case SEED_EFFECT_TYPE_UNIQUE_1:				break;
		case SEED_EFFECT_TYPE_UNIQUE_2:				break;
	}
}
...
void CItemSocketOptionSystem::_SetSetEffect(LPOBJECTSTRUCT lpObj, BYTE btEffectType, BYTE btEffectValueType, WORD wEffectValue) {
	...
	switch (btEffectType) {
		case SEED_EFFECT_TYPE_SET_DOUBLE_DAMAGE_RATE_INC:			lpObj->CharacterData->ANCIENT_LethalStrikeChance += wEffectValue;					break;
		case SEED_EFFECT_TYPE_SET_IGNORE_ENERMY_DEFENSE:			lpObj->CharacterData->ANCIENT_FatalStrikeChance += wEffectValue;					break;
	}
}
...
void CItemSocketOptionSystem::SetRequireStatOption(CItem *lpItemdata) {
	if (lpItemdata == NULL)	return;
	if (lpItemdata->IsItem() == FALSE || !IsSocketItem(lpItemdata))	return;
	...
	for (int i = 0; i < 5; i++) {
		...
		} else if (SeedIndex == SEED_EFFECT_TYPE_ITEM_DURABILITY_IMPROVE) lpItemdata->m_ImproveDurabilityRate += SeedSphere.m_SeedOptionValue;
	}
}
...
BYTE CItemSocketOptionSystem::GetMakeSocketSlotCount(int iItemCode) {
	...
	if (ItemAttribute[iItemCode].HaveItemInfo == 1) {
		if (ItemAttribute[iItemCode].TwoHand == TRUE) iSocketSlotCount = this->m_TwoHandWeaponMakeSocketSlotCountPool.GetRandomValue(eRANDOMPOOL_BY_WEIGHT);
		else {
			int ItemType = ITEM_GET_TYPE(iItemCode);

			std::map<int, TRandomPoolMgr>::iterator It = this->m_mapSocketSlotCountPool.find(ItemType);

			if (It == this->m_mapSocketSlotCountPool.end()) return iSocketSlotCount;
...
void CItemSocketOptionSystem::ClearUserData(LPOBJECTSTRUCT lpObj) {
	...
	if (lpObj == NULL) return;
	if (lpObj->Type != OBJ_USER) return;
	for (iIndex = 0; iIndex < 35; iIndex++)	lpObj->CharacterData->m_SeedOptionList[iIndex].Clear();
	for (iIndex = 0; iIndex < 7; iIndex++)	lpObj->CharacterData->m_BonusOptionList[iIndex].Clear();
	for (iIndex = 0; iIndex < 2; iIndex++)	lpObj->CharacterData->m_SetOptionList[iIndex].Clear();
}
...
bool CItemSocketOptionSystem::SetBonusSocketOption(CItem *lpItemData) {
	...
	for (int iSlotIndex = 0; iSlotIndex < 5; iSlotIndex++) {
		if (lpItemData->m_SocketOption[iSlotIndex] != 0xFF && lpItemData->m_SocketOption[iSlotIndex] != 0xFE) {
			...
			if (this->GetSeedSphereLevel(lpItemData->m_SocketOption[iSlotIndex]) == 3 && iSlotIndex < 3) nSeedSphere4thCount++;
		}
	}
	...
	for (int iBonusIndex = 0; iBonusIndex < 6; iBonusIndex++) {
		if (btItemType >= m_BonusSocketOptionData[iBonusIndex].ItemTypeBegin && btItemType <= m_BonusSocketOptionData[iBonusIndex].ItemTypeEnd) {
			for (int iSlotIndex = 0;iSlotIndex < 5; iSlotIndex++) {
				if (m_BonusSocketOptionData[iBonusIndex].SocketSlotType[iSlotIndex] == SeedSphereType[iSlotIndex] && SeedSphereType[iSlotIndex] != NULL) btSocketSlotCount++;
				if (btSocketSlotCount >= m_BonusSocketOptionData[iBonusIndex].SocketSlotCount) {
					...
					if (nSeedSphere4thCount != 3) return false;
				}
			}
...
bool CItemSocketOptionSystem::ClearSocketSlot(CItem* lpTargetItem, BYTE btSocketSlotIndex) {
	...
	if (IsEnableSocketItem(lpTargetItem) == false) return false;
	if (btSocketSlotIndex > 5) return false;
	if (lpTargetItem->m_SocketOption[btSocketSlotIndex] == 0xFF || lpTargetItem->m_SocketOption[btSocketSlotIndex] == 0xFE) return false;
