...
void CKanturuEntranceNPC::NotifyResultEnterKanturuBossMap(int iIndex)
{
		...
		if ( iCurrentState == KANTURU_STATE_BATTLE_OF_MAYA )
		{
			...
			if ( bMoveGateSuccess ==TRUE )
			{
				...
				g_Log.Add("[Kanturu][User Mng] Add User [%s][%s] Current Battle User:%d State:[%d-%d] (CharInfoSave : Class=%d, Level=%d, LvPoint=%d, Exp=%u, Str=%d, Dex=%d, Vit=%d, Energy=%d, LeaderShip=%d, Map=%d, Pk=%d",
					...
					gObj[iIndex].Class, gObj[iIndex].Level, gObj[iIndex].CharacterData->LevelUpPoint,
					gObj[iIndex].CharacterData->Experience, gObj[iIndex].CharacterData->Strength, gObj[iIndex].CharacterData->Dexterity,
					gObj[iIndex].CharacterData->Vitality, gObj[iIndex].CharacterData->Energy, gObj[iIndex].Leadership, 
				...
				{
					gObj[iIndex].CharacterData->m_bKanturuEntranceByNPC = TRUE;
				}
			}
		}
	...
	else
	{
		...
		if (g_ConfigRead.server.GetServerType() != SERVER_CASTLE)
		{
			gObj[iIndex].CharacterData->m_bKanturuEntranceByNPC = FALSE;
		}
	}
}
