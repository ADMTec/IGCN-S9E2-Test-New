...
BOOL CPentagramMixSystem::PentagramMixBoxInit(LPOBJ lpObj)
{
	...
	if (lpObj->CharacterData->pPentagramMixBox != NULL)
	{
		for (int n = 0; n < CHAOS_BOX_SIZE; n++)
		{
			lpObj->CharacterData->pPentagramMixBox[n].Clear();
		}

		for (int n = 0; n < CHAOS_BOX_SIZE; n++)
		{
			lpObj->CharacterData->pPentagramMixBoxMap[n] = -1;
		}

		return TRUE;
	}

	lpObj->CharacterData->pPentagramMixBox = new CItem[CHAOS_BOX_SIZE];
	...
	lpObj->CharacterData->pPentagramMixBoxMap = new BYTE[CHAOS_BOX_SIZE];

	if (lpObj->CharacterData->pPentagramMixBoxMap == NULL)
	{
		delete[] lpObj->CharacterData->pPentagramMixBox;
		return false;
	}
...
BYTE CPentagramMixSystem::PentagramJewelRefine(int iIndex, int iRefineType)
{
	...
	if (lpObj->CharacterData->m_PentagramMixLock == TRUE)
	{
	...
	lpObj->CharacterData->m_PentagramMixLock = TRUE;
	...
	{
		if (lpObj->CharacterData->pPentagramMixBox[i].IsItem() == TRUE)
		{
			for (int iCount = 0; iCount < 4; iCount++)
			{
				if (lpObj->CharacterData->pPentagramMixBox[i].m_Type == ITEMGET(lpRefineOptionData->NeedItem_Type[iCount], lpRefineOptionData->NeedItem_Index[iCount]))
				{
					if (iCount == 0)
					{
						btTempMainAttribute = lpObj->CharacterData->pPentagramMixBox[i].m_BonusSocketOption & 0x0F;
			...
			if (g_PentagramSystem.IsPentagramItem(lpObj->CharacterData->pPentagramMixBox[i].m_Type) == TRUE)
			{
				bPentagramItem = TRUE;
				for (int k = 0; k < 5; k++)
				{
					btSocketOption[k] = lpObj->CharacterData->pPentagramMixBox[i].m_SocketOption[k];
				}
			}
			...
			if (btIsCorrectItem == TRUE)
			{
				...
				ItemIsBufExOption(ExOption, &lpObj->CharacterData->pPentagramMixBox[i]);
				...
				g_Log.Add("[PentagramRefine] User:%s(%s) Source Item:[%s,%d,%d,%d,%d]serial:[%I64d] Dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d] 380:[%d] HO:[%d,%d] Socket[%d,%d,%d,%d,%d], BonusOption[%d]",
					lpObj->AccountID, lpObj->Name, lpObj->CharacterData->pPentagramMixBox[i].GetName(),
					lpObj->CharacterData->pPentagramMixBox[i].m_Level, lpObj->CharacterData->pPentagramMixBox[i].m_Option1,
					lpObj->CharacterData->pPentagramMixBox[i].m_Option2, lpObj->CharacterData->pPentagramMixBox[i].m_Option3,
					lpObj->CharacterData->pPentagramMixBox[i].m_Number, (int)lpObj->CharacterData->pPentagramMixBox[i].m_Durability
					, ExOption[0], ExOption[1], ExOption[2], ExOption[3], ExOption[4], ExOption[5], ExOption[6],
					lpObj->CharacterData->pPentagramMixBox[i].m_SetOption, lpObj->CharacterData->pPentagramMixBox[i].m_ItemOptionEx >> 7,
					g_kJewelOfHarmonySystem.GetItemStrengthenOption(&lpObj->CharacterData->pPentagramMixBox[i]), g_kJewelOfHarmonySystem.GetItemOptionLevel(&lpObj->CharacterData->pPentagramMixBox[i]),
					lpObj->CharacterData->pPentagramMixBox[i].m_SocketOption[0], lpObj->CharacterData->pPentagramMixBox[i].m_SocketOption[1],
					lpObj->CharacterData->pPentagramMixBox[i].m_SocketOption[2], lpObj->CharacterData->pPentagramMixBox[i].m_SocketOption[3],
					lpObj->CharacterData->pPentagramMixBox[i].m_SocketOption[4], lpObj->CharacterData->pPentagramMixBox[i].m_BonusSocketOption);
	...
	if (lpObj->CharacterData->Money < iPentagramMixNeedMoney)
	{
		lpObj->CharacterData->m_PentagramMixLock = FALSE;
	...
	lpObj->CharacterData->Money -= iPentagramMixNeedMoney;
	g_CastleSiegeSync.AddTributeMoney(iChaosTaxMoney);
	GSProtocol.GCMoneySend(iIndex, lpObj->CharacterData->Money);
...
BYTE CPentagramMixSystem::PentagramJewel_Upgrade(int iIndex, int iUpgradeType, int iTargetValue)
{
	...
	if (iUpgradeType == 1)
	{
			...
			if (lpObj->CharacterData->pPentagramMixBox[i].IsItem() == TRUE)
			{
				for (int n = 0; n < 4; n++)
				{
					if (this->m_JewelRankUpgradeNeedItemData[n].TargetItemRank == iTargetValue)
					{
						for (int k = 0; k < 4; k++)
						{
							if (lpObj->CharacterData->pPentagramMixBox[i].m_Type != ITEMGET(this->m_JewelRankUpgradeNeedItemData[n].NeedSourceItem_Type[0], this->m_JewelRankUpgradeNeedItemData[n].NeedSourceItem_Index[0])
								&& lpObj->CharacterData->pPentagramMixBox[i].m_Type != ITEMGET(this->m_JewelRankUpgradeNeedItemData[n].NeedSourceItem_Type[1], this->m_JewelRankUpgradeNeedItemData[n].NeedSourceItem_Index[1])
								&& lpObj->CharacterData->pPentagramMixBox[i].m_Type != ITEMGET(this->m_JewelRankUpgradeNeedItemData[n].NeedSourceItem_Type[2], this->m_JewelRankUpgradeNeedItemData[n].NeedSourceItem_Index[2]))
							{
								if (lpObj->CharacterData->pPentagramMixBox[i].m_Type == ITEMGET(14, 189))
								{
									btPentagramLuckCharm = TRUE;
									iPentagramLuckCharmDur = lpObj->CharacterData->pPentagramMixBox[i].m_Durability;
									bSourceItem = TRUE;
								}

								else if (lpObj->CharacterData->pPentagramMixBox[i].m_Type == ITEMGET(14, 190))
								{
								...
								else if (this->CheckLevelCondition(&lpObj->CharacterData->pPentagramMixBox[i], 7, 0, 0, 1, 0, 0))
								{
									iSetItemCount++;
									iSetItemMoney += lpObj->CharacterData->pPentagramMixBox[i].m_BuyMoney;
									bSourceItem = TRUE;
								}

								else if (g_PentagramSystem.IsPentagramJewel(&lpObj->CharacterData->pPentagramMixBox[i]) == TRUE)
								{
									iPentagramJewelID = lpObj->CharacterData->pPentagramMixBox[i].m_Type;
									iPentagramJewelCurRank = (lpObj->CharacterData->pPentagramMixBox[i].m_BonusSocketOption & 0xF0) >> 4;

									for (int l = 0; l < 5; l++)
									{
										if (iPentagramJewelCurRank == l + 1)
										{
											iSourceJewelLevel = (lpObj->CharacterData->pPentagramMixBox[i].m_SocketOption[l] & 0xF0) >> 4;
										}

										if (lpObj->CharacterData->pPentagramMixBox[i].m_SocketOption[l] >= 0xFE)
										{
										...
										{
											btSocketOption[l] = lpObj->CharacterData->pPentagramMixBox[i].m_SocketOption[l];
											iRankCount++;
										}
									}
							...
							if (lpObj->CharacterData->pPentagramMixBox[i].m_Type == ITEMGET(this->m_JewelRankUpgradeNeedItemData[n].NeedSourceItem_Type[k], this->m_JewelRankUpgradeNeedItemData[n].NeedSourceItem_Index[k]))
							{
								SourceItemCount[k]++;
								bSourceItem = TRUE;
								break;
		...
		if (iRandRate >= iPentagramMixSuccessRate)
		{
			...
			{
				if (lpObj->CharacterData->pPentagramMixBox[i].IsItem() == TRUE &&
					lpObj->CharacterData->pPentagramMixBox[i].m_Type == iPentagramJewelID)
				{
					BYTE ExOption[MAX_EXOPTION_SIZE];
					ItemIsBufExOption(ExOption, &lpObj->CharacterData->pPentagramMixBox[i]);
					...
					g_Log.Add("[PentagramRankUpgradeFail] User:%s(%s) Item:[%s,%d,%d,%d,%d]serial:[%I64d] Dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d] 380:[%d] HO:[%d,%d] Socket[%d,%d,%d,%d,%d], MainAttribute[%d], CurRank[%d]",
						...
						lpObj->CharacterData->pPentagramMixBox[i].m_SocketOption[0], lpObj->CharacterData->pPentagramMixBox[i].m_SocketOption[1],
						lpObj->CharacterData->pPentagramMixBox[i].m_SocketOption[2], lpObj->CharacterData->pPentagramMixBox[i].m_SocketOption[3],
						lpObj->CharacterData->pPentagramMixBox[i].m_SocketOption[4], lpObj->CharacterData->pPentagramMixBox[i].m_BonusSocketOption & 0x0F,
						(lpObj->CharacterData->pPentagramMixBox[i].m_BonusSocketOption & 0xF0) >> 4);
					break;
			...
			if (btPentagramChaosCharm == TRUE)
			{
				...
				for (int i = 0; i < CHAOS_BOX_SIZE; i++)
				{
					if (lpObj->CharacterData->pPentagramMixBox[i].IsItem() == TRUE
						&& lpObj->CharacterData->pPentagramMixBox[i].m_Type == iPentagramJewelID)
					{
						ItemType = ITEM_GET_TYPE(lpObj->CharacterData->pPentagramMixBox[i].m_Type);
						ItemIndex = ITEM_GET_INDEX(lpObj->CharacterData->pPentagramMixBox[i].m_Type);
						Item.m_SocketOption[iPentagramJewelCurRank - 1] &= 0x0F;
						Item.m_Level = lpObj->CharacterData->pPentagramMixBox[i].m_Level;
						iLevel = Item.m_Level;
						Item.m_Durability = lpObj->CharacterData->pPentagramMixBox[i].m_Durability;
						Item.m_Number = lpObj->CharacterData->pPentagramMixBox[i].m_Number;

						Item.Convert(lpObj->CharacterData->pPentagramMixBox[i].m_Type, lpObj->CharacterData->pPentagramMixBox[i].m_Option1,
							lpObj->CharacterData->pPentagramMixBox[i].m_Option2, lpObj->CharacterData->pPentagramMixBox[i].m_Option3,
							lpObj->CharacterData->pPentagramMixBox[i].m_NewOption, lpObj->CharacterData->pPentagramMixBox[i].m_SetOption,
							0, lpObj->CharacterData->pPentagramMixBox[i].m_SocketOption, lpObj->CharacterData->pPentagramMixBox[i].m_BonusSocketOption, 0, CURRENT_DB_VERSION);
						break;
					}
				}
		...
		else
		{
			...
			for (int i = 0; i < CHAOS_BOX_SIZE; i++)
			{
				if (lpObj->CharacterData->pPentagramMixBox[i].IsItem() == TRUE &&
					lpObj->CharacterData->pPentagramMixBox[i].m_Type == iPentagramJewelID)
				{
					lpObj->CharacterData->pPentagramMixBox[i].m_SocketOption[iSuccessRank - 1] = iRankOptionNum;
					lpObj->CharacterData->pPentagramMixBox[i].m_Level = 0;
					BYTE btMainAttribute = lpObj->CharacterData->pPentagramMixBox[i].m_BonusSocketOption & 0x0F;
					lpObj->CharacterData->pPentagramMixBox[i].m_BonusSocketOption = btMainAttribute | 0x10 * iSuccessRank;
					Item.m_Level = lpObj->CharacterData->pPentagramMixBox[i].m_Level;
					Item.m_Durability = lpObj->CharacterData->pPentagramMixBox[i].m_Durability;
					Item.m_Number = lpObj->CharacterData->pPentagramMixBox[i].m_Number;
	...
	else if (iUpgradeType == 0)
	{
		...
		for (int i = 0; i < CHAOS_BOX_SIZE; i++)
		{
			if (lpObj->CharacterData->pPentagramMixBox[i].IsItem() == TRUE)
			{
				for (int j = 0; j < 4; j++)
				{
					if (lpObj->CharacterData->pPentagramMixBox[i].m_Type != ITEMGET(this->m_JewelLevelUpgradeNeedItemData[iTargetValue - 1].NeedSourceItem_Type[0], this->m_JewelLevelUpgradeNeedItemData[iTargetValue - 1].NeedSourceItem_Index[0])
						&& lpObj->CharacterData->pPentagramMixBox[i].m_Type != ITEMGET(this->m_JewelLevelUpgradeNeedItemData[iTargetValue - 1].NeedSourceItem_Type[1], this->m_JewelLevelUpgradeNeedItemData[iTargetValue - 1].NeedSourceItem_Index[1])
						&& lpObj->CharacterData->pPentagramMixBox[i].m_Type != ITEMGET(this->m_JewelLevelUpgradeNeedItemData[iTargetValue - 1].NeedSourceItem_Type[2], this->m_JewelLevelUpgradeNeedItemData[iTargetValue - 1].NeedSourceItem_Index[2])
						&& lpObj->CharacterData->pPentagramMixBox[i].m_Type != ITEMGET(this->m_JewelLevelUpgradeNeedItemData[iTargetValue - 1].NeedSourceItem_Type[3], this->m_JewelLevelUpgradeNeedItemData[iTargetValue - 1].NeedSourceItem_Index[3]))
					{
						if (lpObj->CharacterData->pPentagramMixBox[i].m_Type == ITEMGET(14, 189))
						{
							bPentagramLuckyCharm = TRUE;
							iPentagramLuckyCharmRate = lpObj->CharacterData->pPentagramMixBox[i].m_Durability;
							bSourceItem = TRUE;
						}
						...
						{
							iItemTargetLevel = lpObj->CharacterData->pPentagramMixBox[i].m_Level;
							iItemTargetLevel++;
							iSourceJewelType = lpObj->CharacterData->pPentagramMixBox[i].m_Type;
							iCurRank = (lpObj->CharacterData->pPentagramMixBox[i].m_BonusSocketOption & 0xF0) >> 4;
							iMainAttribute = lpObj->CharacterData->pPentagramMixBox[i].m_BonusSocketOption & 0x0F;
							iOriginJewelCnt++;
							bSourceItem = TRUE;
						}
					...
					if (lpObj->CharacterData->pPentagramMixBox[i].m_Type == ITEMGET(this->m_JewelLevelUpgradeNeedItemData[iTargetValue - 1].NeedSourceItem_Type[j], this->m_JewelLevelUpgradeNeedItemData[iTargetValue - 1].NeedSourceItem_Index[j]))
					{
						iSourceItemCount[j]++;
						bSourceItem = TRUE;
						break;
		...
		if (iRandRate >= iUpgradeSuccessRate)
		{
			for (int i = 0; i < CHAOS_BOX_SIZE; i++)
			{
				if (lpObj->CharacterData->pPentagramMixBox[i].IsItem() == TRUE && lpObj->CharacterData->pPentagramMixBox[i].m_Type == iSourceJewelType)
				{
					BYTE ExOption[MAX_EXOPTION_SIZE];
					ItemIsBufExOption(ExOption, &lpObj->CharacterData->pPentagramMixBox[i]);
			...
			if (bPentagramChaosCharm == TRUE)
			{
				...
				{
					if (lpObj->CharacterData->pPentagramMixBox[i].IsItem() == TRUE
						&& lpObj->CharacterData->pPentagramMixBox[i].m_Type == iSourceJewelType)
					{
						ItemType = ITEM_GET_TYPE(lpObj->CharacterData->pPentagramMixBox[i].m_Type);
						ItemIndex = ITEM_GET_INDEX(lpObj->CharacterData->pPentagramMixBox[i].m_Type);
		...
		else
		{
			...
			for (int i = 0; i < CHAOS_BOX_SIZE; i++)
			{
				if (lpObj->CharacterData->pPentagramMixBox[i].IsItem() == TRUE && lpObj->CharacterData->pPentagramMixBox[i].m_Type == iSourceJewelType)
				{
					BYTE btRankOptionNum = lpObj->CharacterData->pPentagramMixBox[i].m_SocketOption[iCurRank - 1] & 0x0F;
					lpObj->CharacterData->pPentagramMixBox[i].m_Level = iItemTargetLevel;
					lpObj->CharacterData->pPentagramMixBox[i].m_SocketOption[iCurRank - 1] = btRankOptionNum | 0x10 * iItemTargetLevel;
					iItemType = ITEM_GET_TYPE(lpObj->CharacterData->pPentagramMixBox[i].m_Type);
					iItemIndex = ITEM_GET_INDEX(lpObj->CharacterData->pPentagramMixBox[i].m_Type);
					pModifyItem.m_Level = lpObj->CharacterData->pPentagramMixBox[i].m_Level;
					pModifyItem.m_Durability = lpObj->CharacterData->pPentagramMixBox[i].m_Durability;
					pModifyItem.m_Number = lpObj->CharacterData->pPentagramMixBox[i].m_Number;


					pModifyItem.Convert(lpObj->CharacterData->pPentagramMixBox[i].m_Type, lpObj->CharacterData->pPentagramMixBox[i].m_Option1,
						lpObj->CharacterData->pPentagramMixBox[i].m_Option2, lpObj->CharacterData->pPentagramMixBox[i].m_Option3,
						lpObj->CharacterData->pPentagramMixBox[i].m_NewOption, lpObj->CharacterData->pPentagramMixBox[i].m_SetOption,
						0, lpObj->CharacterData->pPentagramMixBox[i].m_SocketOption, lpObj->CharacterData->pPentagramMixBox[i].m_BonusSocketOption, 0, CURRENT_DB_VERSION);
					break;
