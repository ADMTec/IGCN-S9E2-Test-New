...
void QuestExpProgMng::ReqQuestAskComplete(DWORD dwQuestInfoIndexID, int iObjIndex)
{
	...
	if (gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEpisode].m_UserQuestAskInfo[0].GetQuestType() == QUESTEXP_ASK_TUTORIAL_KEY)
	{
		gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEpisode].m_UserQuestAskInfo[0].SetComplete(true);
		gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEpisode].m_UserQuestAskInfo[0].SetValue(1);

		this->SendQuestAskInfoUpdate(iEpisode, iQS, QUESTEXP_ASK_TUTORIAL_KEY, gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEpisode].m_UserQuestAskInfo[0].GetValue(), iObjIndex);
	}
}
...
void QuestExpProgMng::QuestExpGiveUpBtnClick(DWORD dwQuestInfoIndexID, int iObjIndex)
{
	...
	int iSendQS = gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEpisode].GetQuestSwitch();
...
void QuestExpProgMng::ChkUserQuestTypeBuff(QuestBuff* pQuestBuff, UserQuestAskInfo* pUserQuestAskInfo, int iObjIndex)
{
	...
	if (::BUFF_TargetHasBuff(&gObj[iObjIndex], iBuffIndex))
	{
...
{
	int iZen = gObj[iObjIndex].CharacterData->Money;
...
void QuestExpProgMng::DeleteInventoryItem(QuestGetItem* pQuestGetItem, int iObjIndex)
{
	...
	for (int x = 0; x < INVENTORY_SIZE; ++x)
	{
			...
			&& g_QuestExpManager.IsQuestItemAtt(gObj[iObjIndex].pInventory[x].m_Type, QUESTEXP_ITEM_OVERLAP) != true)
		{
			...
			GSProtocol.PROTOCOL_InventoryItemDeleteSend(iObjIndex, x, 1);
...
void QuestExpProgMng::ChkMonsterKillPartyPlay(DWORD dwQuestInfoIndexID, int iAskIndex, QuestMonsterKill* pQuestMonsterKill, OBJECTSTRUCT* lpObj, OBJECTSTRUCT* lpMonsterObj)
{
	...
	if (lpObj->CharacterData->m_UserQuestInfo[iEpisode].GetEpisode() != iEpisode)
	{
		return;
	}

	if (lpObj->CharacterData->m_UserQuestInfo[iEpisode].GetQuestSwitch() != iQS)
	{
		return;
	}

	UserQuestAskInfo* pUserQuestAskInfo = &lpObj->CharacterData->m_UserQuestInfo[iEpisode].m_UserQuestAskInfo[iAskIndex];
	...
	for (int iPartyUserCount = 0; iPartyUserCount < MAX_USER_IN_PARTY; ++iPartyUserCount)
	{
		...
		if (iPartyUserIndex >= 0)
		{
			...
			if (gObj[iPartyUserIndex].Connected >= PLAYER_PLAYING || !lpPartyObj->Live)
			{
				...
				{
					if (CHARACTER_CalcDistance(lpObj, lpPartyObj) <= 9)
					{
...
void QuestExpProgMng::ChkUserQuestType(int iQuestType, OBJECTSTRUCT *lpObj, OBJECTSTRUCT *lpMonsterObj, int iKillerObjIndex)
{
	...
	{
		if (lpObj->CharacterData->m_UserQuestInfo[i].GetEpisode() == 0)
		{
			continue;
		}

		UserQuestInfo* pUserQuestInfo = &lpObj->CharacterData->m_UserQuestInfo[i];
...
void QuestExpProgMng::SendProgressQuestList(int iObjIndex)
{
	...
	for (int i = 0; i < MAX_QUESTEXP_USER_INFO; ++i)
	{
		int iEpisode = gObj[iObjIndex].CharacterData->m_UserQuestInfo[i].GetEpisode();
		int iQS = gObj[iObjIndex].CharacterData->m_UserQuestInfo[i].GetQuestSwitch();
		WORD wProgState = gObj[iObjIndex].CharacterData->m_UserQuestInfo[i].GetQuestProgState();
...
void QuestExpProgMng::SetQuestProg(int iEpisode, int iObjIndex, int iState)
{
	...
	if (gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEpisode].GetQuestProgState() != QUESTEXP_PROG_STATE_TIME_LIMIT)
	{
		gObj[iObjIndex].CharacterData->m_UserQuestInfo[iEpisode].SetQuestProgState(iState);
	}
}
...
void QuestExpProgMng::QuestMonsterItemDrop(DWORD dwQuestInfoIndexID, OBJECTSTRUCT *lpObj, OBJECTSTRUCT *lpMonsterObj)
{
	...
	for (int i = 0; i < MAX_QUESTEXP_USER_INFO; i++)
	{
		int iEpisode = lpObj->CharacterData->m_UserQuestInfo[i].GetEpisode();
		int iQS = lpObj->CharacterData->m_UserQuestInfo[i].GetQuestSwitch();
...
bool QuestExpProgMng::IsQuestDropItem(int iIndex, WORD nType, WORD nLevel)
{
	...
	for (int i = 0; i < MAX_QUESTEXP_USER_INFO; ++i)
	{
		...
		UserQuestAskInfo* pUserQuestAskInfo = &lpObj->CharacterData->m_UserQuestInfo[i].m_UserQuestAskInfo[pQuestDropItemInfo->GetAskIndex() - 1];
...
void QuestExpProgMng::SetQuestTimeLimit(int iObjIndex, DWORD dwQuestIndexID, int iDuration)
{
	...
	for (int i = 0; i < MAX_QUESTEXP_USER_INFO; i++)
	{
		...
		{
			lpObj->CharacterData->m_UserQuestInfo[i].SetStartDate(GetCurrentDate());
			lpObj->CharacterData->m_UserQuestInfo[i].SetEndDate(GetExpireDate(iDuration));
			lpObj->CharacterData->m_UserQuestInfo[i].SetQuestProgState(QUESTEXP_PROG_STATE_TIME_LIMIT);

			SYSTEMTIME tStartTime;
			SYSTEMTIME tEndTime;

			UnixTimeToSystemTime(lpObj->CharacterData->m_UserQuestInfo[i].GetStartDate(), &tStartTime);
			UnixTimeToSystemTime(lpObj->CharacterData->m_UserQuestInfo[i].GetEndDate(), &tEndTime);
