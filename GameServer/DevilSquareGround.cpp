...
void CDevilSquareGround::SendScore() {
	if (this->m_DevilSquareRankList.size() < 1) return;

	BYTE count = 1;
	int iUserCount = this->m_DevilSquareRankList.size();
	UINT64 iExp = 0;
	UINT64 iDefaultExp = 0;
	...
	for (; Itor != this->m_DevilSquareRankList.end(); Itor++) {
		...
		if ((*(Itor))->Type != OBJ_USER) continue;
		if (g_CrywolfSync.CW_OccupationState() == TRUE && g_CrywolfSync.CW_ApplyPenalty() == TRUE) {
			...
			CWpenalty /= 100;
			this->m_DevilSquareScoreInfoTOP10.Score[0].BonusExp *= CWpenalty;
		}
		...
		if (g_MasterLevelSkillTreeSystem.ML_IsMasterCharacter((*(Itor)))) (*(Itor))->CharacterData->MasterExperience += iExp;
		else (*(Itor))->CharacterData->Experience += iExp;

		bool ret = false;

		if (iExp > 0) ret = CHARACTER_LevelUp((*(Itor)), iExp, 0, "Devil Square");
		...
		if (gObjCheckMaxZen((*(Itor))->m_Index, (*(Itor))->m_nEventMoney) == FALSE) {
			int Zen = MAX_ZEN - (*(Itor))->CharacterData->Money;

			(*(Itor))->CharacterData->Money += Zen;
		} else (*(Itor))->CharacterData->Money += (*(Itor))->m_nEventMoney;

		GSProtocol.GCMoneySend((*(Itor))->m_Index, (*(Itor))->CharacterData->Money);

		if (ret == true) GSProtocol.PROTOCOL_PlayerExperienceSend((*(Itor))->m_Index, (WORD)-1, iExp, 0, 0);
		...
		if (iDefaultExp == 0) this->m_DevilSquareScoreInfoTOP10.Score[0].BonusExp = 0;
		else this->m_DevilSquareScoreInfoTOP10.Score[0].BonusExp = iDefaultExp;
