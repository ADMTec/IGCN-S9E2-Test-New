...
BOOL CSetItemOption::LoadTypeInfo(char* filename) {
	...
	for (int i=0; i<MAX_ITEMS; i++) this->m_SetItemTypeInfo[i].IsLoad = FALSE;
	for (int i=0; i<MAX_TYPE_ITEMS; i++) this->m_SetItemDropInfo.IsItemFound[i] = FALSE;
...
BOOL CSetItemOption::LoadOptionInfo(LPSTR filename) {
	...
	for (pugi::xml_node setitem = main.child("SetItem"); setitem; setitem = setitem.next_sibling()) {
		...
		for (int i = 0; i < OPTION_TABLE_SIZE; i++) {
			...
			if ((p->SetOptionTable[i][0] != -1) || (p->SetOptionTable[i][1] != -1)) p->SetOptionCount++;
		}
		...
		for (int i = 0; i < SPECIAL_OPTION_TABLE_SIZE; i++) {
			...
			if (p->SetExPOptionTable[i] != -1) p->SetOptionCount ++;
		}
		...
		p->RequireClass[CLASS_MAGICGLADIATOR] = setitem.attribute("MagicGladiator").as_int();
...
int CSetItemOption::GetMaxSetOptionCount(int optionindex) {
	...
	if (this->m_SetItemOptionInfo[optionindex].IsLoad == 0) return -1;
