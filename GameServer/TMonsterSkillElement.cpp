...
void TMonsterSkillElement::ForceSkillElement(int iIndex, int iTargetIndex) {
	...
	if (this->m_iElementType == MSE_TYPE_STUN && gObj[iTargetIndex].Type == OBJ_USER) iAccessoryEffect = gObj[iTargetIndex].CharacterData->C_StunResistance;
	if (CHARACTER_DebuffProc(iSuccessRate) == FALSE) return;
	if (CHARACTER_PvPResistanceProc(iAccessoryEffect) == TRUE) return;
...
BOOL TMonsterSkillElement::ApplyElementStun(int iIndex, int iTargetIndex)
{
	...
	{
		BUFF_AddBuffEffect(lpTargetObj, DEBUFFTYPE_STUN, 0, 0, 0, 0, 3);
		CHARACTER_Immobilize(iTargetIndex, lpTargetObj->X, lpTargetObj->Y);
	}
...
BOOL TMonsterSkillElement::ApplyElementMP(int iIndex, int iTargetIndex)
{
	...
	{
		lpTargetObj->Mana = 0;
		GSProtocol.PROTOCOL_ManaSend(lpTargetObj->m_Index, lpTargetObj->Mana, 0xFF, 0, lpTargetObj->Stamina);
...
BOOL TMonsterSkillElement::ApplyElementAG(int iIndex, int iTargetIndex)
{
	...
	if ( this->m_iIncAndDecType != MSE_INCDEC_TYPE_NONE )
	{
		if ( this->m_iIncAndDecType < MSE_INCDEC_TYPE_CONSTANTINC )
		{
			iIncDecValue = lpTargetObj->Stamina * this->m_iIncAndDecValue / 100;
	...
	lpTargetObj->Stamina += iIncDecValue;

	if ( lpTargetObj->Stamina < 0 )
	{
		lpTargetObj->Stamina = 0;
		GSProtocol.PROTOCOL_ManaSend(lpTargetObj->m_Index, lpTargetObj->Mana, 0xFF, 0, lpTargetObj->Stamina);

		return TRUE;
	}

	if ( (lpTargetObj->MaxStamina + lpTargetObj->AddStamina) < lpTargetObj->Stamina )
	{
		lpTargetObj->Stamina = lpTargetObj->MaxStamina + lpTargetObj->AddStamina;
		GSProtocol.PROTOCOL_ManaSend(lpTargetObj->m_Index, lpTargetObj->Mana, 0xFF, 0, lpTargetObj->Stamina);
...
BOOL TMonsterSkillElement::ApplyElementDefense(int iIndex, int iTargetIndex)
{
	...
	if ( this->m_iIncAndDecType != MSE_INCDEC_TYPE_NONE )
	{
		if ( this->m_iIncAndDecType < MSE_INCDEC_TYPE_CONSTANTINC )
		{
			iIncDecValue = lpTargetObj->Defense * this->m_iIncAndDecValue / 100;
	...
	if ( IMPERIAL_MAP_RANGE(lpObj->MapNumber) )
	{
		...
		{
			BUFF_AddBuffEffect(lpTargetObj, 58, 23, 10, 0, 0, 10);
		}
	}
...
BOOL TMonsterSkillElement::ApplyElementSummon(int iIndex, int iTargetIndex)
{
	...
	BUFF_RemoveBuffEffect(lpTargetObj, DEBUFFTYPE_POISON);
	BUFF_RemoveBuffEffect(lpTargetObj, DEBUFFTYPE_CHILL);
...
BOOL TMonsterSkillElement::ApplyElementPush(int iIndex, int iTargetIndex)
{
	...
	{
		CHARACTER_KnockbackDistance(lpTargetObj, lpObj,this->m_iIncAndDecValue);
	}
	else
	{
...
BOOL TMonsterSkillElement::ApplyElementRemoveSkill(int iIndex, int iTargetIndex)
{
	...
	gObjUseSkill.ELF_RemoveBuff(lpTargetObj, this->m_iNullifiedSkill);
...
BOOL TMonsterSkillElement::ApplyElementImmuneSkill(int iIndex, int iTargetIndex)
{
	...
	BUFF_AddBuffEffect(lpObj, BUFFTYPE_MONSTER_MAGIC_IMMUNE, 0, 0, 0, 0, this->m_iContinuanceTime);
	return FALSE;
}
...
BOOL TMonsterSkillElement::ApplyElementTeleportSkill(int iIndex, int iTargetIndex)
{
	...
	if (BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_ICE_ARROW) || BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_STUN) || BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_SLEEP) ||
		BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_FREEZE) || BUFF_TargetHasBuff(lpObj, DEBUFFTYPE_EARTHEN_PRISON))
	{
		return FALSE;
	}
	...
	PLAYER_TeleportMagicUse(iIndex, x, y);
...
BOOL TMonsterSkillElement::ApplyElementPoison(int iIndex, int iTargetIndex)
{
	...
	if ( !BUFF_TargetHasBuff(lpTargetObj, DEBUFFTYPE_POISON) )
	{
		if ( this->m_iNullifiedSkill  < 1 )
		{
			if (CHARACTER_ResistanceProc(lpTargetObj, 1) == 0 )
			{
				...
				BUFF_AddBuffEffect(lpTargetObj, DEBUFFTYPE_POISON, EFFECTTYPE_DOT_POISON, 3, 0, 0, this->m_iIncAndDecValue);
			}
		}
		else
		{
...
BOOL TMonsterSkillElement::ApplyElementImmuneAllSkill(int iIndex, int iTargetIndex)
{
	...
	BUFF_AddBuffEffect(lpTargetObj, BUFFTYPE_MONSTER_MAGIC_IMMUNE, 0, 0, 0, 0, DurationTime);
	BUFF_AddBuffEffect(lpTargetObj, BUFFTYPE_MONSTER_MELEE_IMMUNE, 0, 0, 0, 0, DurationTime);
