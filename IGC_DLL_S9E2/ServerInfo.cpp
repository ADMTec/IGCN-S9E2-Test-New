...
void CServerInfo::Load(std::string file) {
	...
	GetPrivateProfileStringA("Main", "Serial", "SVG232Z45MUS9311", szTemp, 17, filedec);
	memcpy(m_Serial, szTemp, 16);
	m_UseLauncher = GetPrivateProfileIntA("Launcher", "Enabled", 0, filedec);
	GetPrivateProfileStringA("Launcher", "ExeName", "Mu.exe", szTemp, 255, filedec);
	...
	m_PatcherFile = std::string(szTemp);
	DeleteFileA(filedec);
}
