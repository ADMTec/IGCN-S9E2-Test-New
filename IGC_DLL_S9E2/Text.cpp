...
CGlFont::CGlFont(TCHAR* Name, int Size, int Weight, BOOL bItalic, BOOL bUnderline) {
	m_Font = CreateFont(Size, 0, 0, 0, Weight, bItalic, bUnderline, FALSE, g_ServerInfo->GetCharset(), OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, FF_DONTCARE|DEFAULT_PITCH, Name);
	width_ = 128;
	height_ = 128;
	spacing_ = Size/3;
	fontSize_ = Size;
...
PAINT_RESULT CGlFont::PaintAlphabet(HDC hDC, bool bMeasureOnly) {
	...
	if(0 == GetTextExtentPoint32(hDC, str, 1, &size)) return PAINT_FAIL;
	...
	for(int i = 32; i < 255; i++) {
		...
		if(!bMeasureOnly) {
			...
			if(0 == ExtTextOut(hDC, x+0, y+0, ETO_OPAQUE, NULL, str, 1, NULL)) return PAINT_FAIL;
