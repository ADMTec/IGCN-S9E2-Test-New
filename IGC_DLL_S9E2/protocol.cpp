...
bool CliProtocolCore(LPBYTE aRecv, BYTE ProtoNum, int len, bool Encrypt) {
	switch(ProtoNum) {
		...
		case 0x27:
			PROTOCOL_ManaSend((PMSG_MANASEND*)aRecv);
		break;
	}
...
void GCRefillSend(PMSG_REFILL* lpMsg) {
	...
	BYTE checksh[3];
	BYTE original[3] = { 0x83, 0xE8, 0x28 };
...
void PROTOCOL_ManaSend(PMSG_MANASEND *lpMsg) {
	if (lpMsg->IPos == 0xFE) {
...
void GCEnableSiegeSkills() {
	...
	*(BYTE*)(0x005B1CBB) = 0xEB;	// all s9
}

void CGSendFilesCrc() {
	PMSG_SEND_CRC msg; // Binds to server-side structure PMSG_REQ_CRCDATA
	msg.h.c = 0xC1;
	msg.h.headcode = 0xFA;
	...
	if (msg.m_Crc[0] >= 0x7FFFFFFF) msg.m_Crc[0] = msg.m_Crc[0] / 2;
	if (msg.m_Crc[1] >= 0x7FFFFFFF) msg.m_Crc[1] = msg.m_Crc[1] / 2;
	if (msg.m_Crc[2] >= 0x7FFFFFFF) msg.m_Crc[2] = msg.m_Crc[2] / 2;
	if (msg.m_Crc[4] >= 0x7FFFFFFF) msg.m_Crc[4] = msg.m_Crc[4] / 2;
...
void GCCustomPost(PMSG_POST_DATA *lpMsg) {
	...
	if (lpMsg->h.subcode == 0x10) {
		...
		sprintf(szMsg, szPostFormat.c_str(), lpMsg->btColorRGB[0], lpMsg->btColorRGB[1], lpMsg->btColorRGB[2], lpMsg->szTag, szName, pText);
	} else if (lpMsg->h.subcode == 0x11) {
		szPostFormat = "<FONT color='#FF%02X%02X%02X'>[%s]%s %s: %s</FONT><BR>";
		sprintf(szMsg, szPostFormat.c_str(), lpMsg->btColorRGB[0], lpMsg->btColorRGB[1], lpMsg->btColorRGB[2], lpMsg->szServerName, lpMsg->szTag, szName, pText);
	}
