	...
	*(DWORD*)CHARSET_ADDRESS = g_ServerInfo->GetCharset();
	...
	if (g_ServerInfo->GetLauncher()) {
		...
		for (int i = 0; i < paramLen; i++) {
			if (parameter[i] < '0' || parameter[i] > '9') isNum = false;
		}
...
BOOL APIENTRY DllMain (HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
	switch (ul_reason_for_call)	{
		case DLL_PROCESS_ATTACH: {
			...
			/*HINSTANCE CameraDLL = LoadLibrary("Camera.dll");

			if (!CameraDLL) {
				MessageBox(NULL, "Camera.dll Is Missing\nDo you want to try again?", "Error", MB_ICONERROR | MB_OK); exit(EXIT_FAILURE);
			}*/
		}
		break;
